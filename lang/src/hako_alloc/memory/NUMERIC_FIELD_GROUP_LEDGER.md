...
- `HAKO-ALLOC-USIZE-FIELD-GROUP-207` selected
  ...
  bool-like readiness / would-execute flags, provider / hook / replacement
  rows, worker/TLS, and threads stay unchanged.
- `HAKO-ALLOC-USIZE-FIELD-GROUP-208` migrated those eleven
  `HakoAllocProviderCallRealApiStubExecutionPilot` owner-local counters to
  exact `usize`, while `last_reason`, report fields, stub execution payloads,
  result-code fields, actual provider API call flags, bool-like readiness /
  would-execute flags, provider / hook / replacement rows, worker/TLS, and
  threads stay unchanged.
- `HAKO-ALLOC-USIZE-FIELD-GROUP-209` selected
  ...
  `closed_host_replacement_reject_count`, `closed_hook_reject_count`, and
  `closed_backend_matcher_reject_count`) as
  `HAKO-ALLOC-USIZE-FIELD-GROUP-210`, while `last_reason`, report fields,
  adapter payloads, external API readiness / executed flags, bool-like
  would-execute flags, provider / hook / replacement rows, worker/TLS, and
  threads stay unchanged.
- `HAKO-ALLOC-USIZE-FIELD-GROUP-210` migrated those twelve
  `HakoAllocProviderCallExternalApiAdapterInventory` owner-local counters to
  exact `usize`, while `last_reason`, report fields, adapter payloads,
...
- `HAKO-ALLOC-USIZE-FIELD-GROUP-211` selected
  `HakoAllocProviderCallExternalApiAdapterPreflight` owner-local counters
  (`preflight_count`, `accepted_count`, `reject_count`,
  ...
  `closed_host_replacement_reject_count`, `closed_hook_reject_count`, and
  `closed_backend_matcher_reject_count`) as
  `HAKO-ALLOC-USIZE-FIELD-GROUP-212`, while `last_reason`, report fields,
...
- `HAKO-ALLOC-USIZE-FIELD-GROUP-213` selected
  ...
  worker/TLS, and threads stay unchanged.
- `HAKO-ALLOC-USIZE-FIELD-GROUP-214` migrated those eleven
  `HakoAllocProviderCallExternalApiCallStubExecutionPilot` owner-local counters
  to exact `usize`, while `last_reason`, report fields, stub execution
