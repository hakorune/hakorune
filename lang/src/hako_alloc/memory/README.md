...
- `object_lifecycle_facade_box.hako` owns MIMAP-014A single-page small
  allocation facade composition. It keeps add/select/acquire/result reporting
  narrow before aligned allocation, realloc, provider activation, hook install,
  or allocator replacement work opens.
...
- `segment_arena_backing_modeled_residence_arena_binding_box.hako` owns MIMAP-252A segment arena backing modeled residence arena-binding. It keeps the residence-binding route narrow, exact, and explicit before any closeout pack opens.
...
- `lifecycle_stats_observer_box.hako` owns M209 lifecycle stats observer surface. It keeps the lifecycle stats observer route narrow, exact, and explicit before any observe/select/mutate/page-source/provider work opens.
- `segment_allocation_modeled_consume_box.hako` owns MIMAP-091A segment allocation modeled consume route. It keeps the consume route narrow, exact, and explicit before any real execution, concurrency, segment-map, atomics, page-source, or provider work opens.
- `segment_arena_backing_modeled_allocation_ledger_release_recycle_applied_state_summary_box.hako` owns MIMAP-308A segment arena backing modeled allocation-ledger release/recycle applied-state summary inventory. It keeps the applied-state summary route narrow, exact, and explicit before any continuation application, lifecycle continuation, raw pointer, segment-map, atomic bitmap, OSVM, worker/TLS, provider activation, or backend matcher work opens.
- `segment_arena_backing_modeled_allocation_ledger_release_recycle_applied_state_summary_diagnostic_box.hako` owns MIMAP-309A segment arena backing modeled allocation-ledger release/recycle applied-state summary diagnostics. It keeps the applied-state summary diagnostics route narrow, exact, and explicit before any continuation application, lifecycle continuation, raw pointer, segment-map, atomic bitmap, OSVM, worker/TLS, provider activation, or backend matcher work opens.
- `reclaim_owner_transfer_execution_box.hako` owns MIMAP-055A reclaim owner-transfer execution. It keeps the execution route narrow, exact, and explicit before any thread scheduling, atomic claim, remote-free drain, owner adoption, reclaim execution, page-source, unreserve, or OS release work opens.
- `reclaim_remote_free_drain_contract_box.hako` owns MIMAP-056A reclaim remote-free drain contract. It keeps the contract route narrow, exact, and explicit before any thread scheduling, atomic claim, remote-free drain, owner adoption, reclaim execution, page-source, unreserve, or OS release work opens.
...
- `reclaim_post_drain_owner_transfer_box.hako` owns MIMAP-058A reclaim post-drain owner-transfer integration route. It keeps the post-drain owner-transfer route narrow, exact, and explicit before any full reclaim, thread scheduling, atomic claim, remote-free drain, owner adoption, reclaim execution, page-source, unreserve, OS release, provider activation, or allocator replacement work opens.
...
- `segment_arena_backing_modeled_arena_slot_box.hako` owns MIMAP-256A segment arena backing modeled arena slot. It keeps the modeled arena-slot inventory route narrow, exact, and explicit before any wider diagnostics, closeout pack, arena backing, segment-map, atomic bitmap, OSVM, worker, provider, or backend matcher work opens.
- `segment_arena_backing_modeled_arena_slot_diagnostic_box.hako` owns MIMAP-257A segment arena backing modeled arena slot diagnostics. It keeps the observer-only arena-slot diagnostics route narrow, exact, and explicit before any wider closeout pack, arena backing, segment-map, atomic bitmap, OSVM, worker, provider, or backend matcher work opens.
...
- `allocator_comparison_representative_benchmark_execution_diagnostic_box.hako` owns MIMAP-445A allocator comparison representative benchmark execution diagnostics. It keeps the representative benchmark execution diagnostics route narrow, exact, and explicit before any benchmark execution, process replacement, hook, backend matcher, or global allocator work opens.
