...
- None. `HAKO-ALLOC-USIZE-FIELD-GROUP-062` migrates the aligned-small metadata
  owner count and its path-local mirror after the page-model lifecycle counter
  group.
...
| `alloc_fast_path_heap_box.hako` | `HakoAllocFastPathHeap` | `bin`, `block_size`, `page_capacity`, `next_page_id`, `alloc_count`, `release_count`, `fallback_count`, `page_create_count`, `reject_count` | event/reject counters are exact `usize` via `HAKO-ALLOC-USIZE-FIELD-GROUP-067`; bin, size/capacity metadata, and next page id stay `i64`. |
...
3. Probe `capacity` / stack-top fields with underflow checks. `294x-41`
   ...
   still require their own owner-local row. `294x-42` extends the same probe to
   exact `usize` stack-top values used as `ArrayBox.get/set` indexes.
