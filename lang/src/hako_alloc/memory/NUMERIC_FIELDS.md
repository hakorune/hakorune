...
- `huge_page_model_box.hako` / `HakoAllocHugePageModel.next_page_id`.
  `HAKO-ALLOC-USIZE-FIELD-GROUP-090` selects this field for
  `HAKO-ALLOC-USIZE-FIELD-GROUP-091` because it is the huge-page model's
  owner-local next page id source, starts at a non-negative `1000`, and is not
  used as a `-1` failure sentinel source. Published page id payloads, pointer
  payloads, size observers, statuses, and page-map entries stay `i64`.
