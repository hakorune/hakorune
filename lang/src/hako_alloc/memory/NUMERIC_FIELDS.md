...
| `allocator_comparison_c_mimalloc_result_summary_inventory_box.hako` | `HakoAllocAllocatorComparisonCMimallocResultSummaryInventory` | `summary_count`, `ready_count`, `blocked_count`, `missing_ledger_reject_count`, `missing_diagnostic_reject_count`, `blocked_diagnostic_reject_count`, `last_reason` | owner-local counters are selected by `HAKO-ALLOC-USIZE-FIELD-GROUP-138` for `HAKO-ALLOC-USIZE-FIELD-GROUP-139`; `last_reason` stays signed reason vocabulary. |
