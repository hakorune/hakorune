...
| `allocator_comparison_c_mimalloc_result_summary_diagnostic_box.hako` | `HakoAllocAllocatorComparisonCMimallocResultSummaryDiagnostic` | `diagnostic_count`, `ready_count`, `blocked_count`, `missing_summary_blocked_count`, `blocked_summary_blocked_count`, `last_reason` | owner-local counters are selected by `HAKO-ALLOC-USIZE-FIELD-GROUP-140` for `HAKO-ALLOC-USIZE-FIELD-GROUP-141`; `last_reason` stays signed reason vocabulary. |
