...
- `Stage1ProgramResultValidationBox`, `Stage1ProgramJsonTextGuardBox`, and `Stage1SourceMirAuthorityBox` now also centralize their text coercion through owner-local `_coerce_text_compat(...)`, so the remaining raw `"" + x` residue in `stage1_cli_env.hako` is limited to helper implementations and intentional JSON string assembly.
- shared Program(JSON) text presence/coercion is isolated in `Stage1ProgramJsonTextGuardBox` inside `stage1_cli_env.hako`; source authority and explicit compat keep their missing-input null returns caller-local so Stage0 does not learn a shared `String|Void` guard body.
- shared Program(JSON) -> MIR checked handoff leaf is isolated in `Stage1ProgramJsonMirCallerBox` inside `stage1_cli_env.hako`; keep the direct `MirBuilderBox.emit_from_program_json_v0(...)` contract out of both source authority and compat keep, and use `Stage1ProgramJsonTextGuardBox.input_present(...)` / `_coerce_text_compat(...)` instead of shared `String|Void` wrappers.
...
- explicit Program(JSON) compat keep is quarantined in `Stage1EmitMirDispatchBox.run_emit_mir_program_json_compat_mode(...)`; current callers are probe/helper-owned only, so keep it outside reduced authority evidence and enter the `Stage1ProgramJsonMirCallerBox` handoff leaf only after caller-local guard/coercion.
- `Stage1ProgramJsonCompatBox` now keeps explicit Program(JSON) input presence and missing-input null return caller-local, while reusing `Stage1ProgramJsonTextGuardBox.input_present(...)` / `_coerce_text_compat(...)` for the shared scalar/text contract.
...
- the explicit Program(JSON) compat entry keeps the final checked emit as a direct handoff to `Stage1ProgramJsonMirCallerBox._emit_mir_from_program_json_text_checked(...)`, so the flow is input guard -> text coercion -> checked emit handoff -> final MIR validation.
