...
Current owner
  ...
  - current `.hako` daily caller still passes recipe payload explicitly via `BackendRecipeBox.compile_route_profile(...)` and then forwards the caller `json_path` directly into `env.codegen.compile_json_path(json_path, "", recipe, compat)` while route evidence env juggling stays inside `backend_route_env_box.hako`
  - staged transport cut is now explicit: `.ll` text can cross the Rust tool seam directly, but daily compile still waits on a `.hako` MIR root entry before `compile_json_path` can leave mainline
  ...
  - `.hako ll emitter` compare/debug templating residue is now split into Rust helper `hako_ll_driver.rs`, and `.ll` tool execution is isolated in `ll_tool_driver.rs`; remaining cleanup is the MIR-root daily transport cut plus compare bridge retirement, not env or template plumbing
  - current daily compile/link owner still stops directly at `env.codegen.compile_json_path(...)` / `env.codegen.link_object(...)`
