...
Current owner
  ...
  - compare/debug residue for `.hako ll emitter` bridge is split between `src/host_providers/llvm_codegen/ll_emit_compare_source.rs` and `src/host_providers/llvm_codegen/ll_emit_compare_driver.rs`; `src/host_providers/llvm_codegen/provider_keep.rs` carries explicit provider keep lanes and provider path resolution; the dedicated `ll_emit_bridge.rs` wrapper has been retired; LLVM tool execution is split further into `src/host_providers/llvm_codegen/ll_tool_driver.rs`; compare proof itself is archive-suite only; `capi_transport.rs` owns the CAPI helpers while `transport_paths.rs` / `transport_io.rs` own the remaining temp-path helpers and compare-source temp materialization; VM resolution, spawn, and stdout extraction now live in the driver
  - `ll_emit/**`
  ...
  - launcher/mainline compile is now root-first; the old explicit legacy MIR(JSON) helper is deleted and remaining compat acceptance is carried by the shared Rust text primitive plus provider/capi keep routes
  ...
  - `.hako ll emitter` compare/debug templating residue is now split across `ll_emit_compare_source.rs` and `ll_emit_compare_driver.rs`; `provider_keep.rs` owns explicit provider keep lanes and provider path resolution, the dedicated `ll_emit_bridge.rs` wrapper has been retired, `.ll` tool execution is isolated in `ll_tool_driver.rs`, and both the Rust compat chokepoint and compiled-stage1 surrogate now share the no-helper Rust text primitive instead of an explicit legacy helper; VM resolution, spawn, and stdout extraction now live in the driver
