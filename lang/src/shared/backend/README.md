...
Current owner
  ...
  - compare/debug residue for `.hako ll emitter` bridge is split between `src/host_providers/llvm_codegen/ll_emit_compare_source.rs` and `src/host_providers/llvm_codegen/ll_emit_compare_driver.rs`; `src/host_providers/llvm_codegen/provider_keep.rs` carries explicit provider keep lanes and provider path resolution; the dedicated `ll_emit_bridge.rs` wrapper has been retired; the legacy MIR(JSON) front door now lives in `src/host_providers/llvm_codegen/legacy_mir_front_door.rs`; LLVM tool execution is split further into `src/host_providers/llvm_codegen/ll_tool_driver.rs`; compare proof itself is archive-suite only; `capi_transport.rs` owns the CAPI helpers while `transport_paths.rs` / `transport_io.rs` own the remaining temp-path helpers and compare-source temp materialization; VM resolution, spawn, and stdout extraction now live in the driver
  - `ll_emit/**`
  ...
  - launcher/mainline compile is now root-first; the legacy compare/archive transport is isolated in the explicit compat helper `compile_object_from_legacy_mir_json(...)` in `src/host_providers/llvm_codegen/legacy_mir_front_door.rs` and no longer the daily transport
  ...
  - W4 exact proof uses `LlvmBackendEvidenceAdapterBox.compile_obj_provider_stopline(...)` so the current `extern_provider` stop-line can be proven on `vm-hako` without reopening the owner facade
  ...
  - `.hako ll emitter` compare/debug templating residue is now split across `ll_emit_compare_source.rs` and `ll_emit_compare_driver.rs`; `provider_keep.rs` owns explicit provider keep lanes and provider path resolution, the dedicated `ll_emit_bridge.rs` wrapper has been retired, the remaining Rust-side compat helper now lives as `compile_object_from_legacy_mir_json(...)` in `src/host_providers/llvm_codegen/legacy_mir_front_door.rs`, and `.ll` tool execution is isolated in `ll_tool_driver.rs`; the compiled-stage1 surrogate now loads MIR(JSON) locally and forwards text into that explicit helper, so the file-based front door is retired from that path; VM resolution, spawn, and stdout extraction now live in the driver
