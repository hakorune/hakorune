...
Current owner
  ...
  - compare/debug residue for `.hako ll emitter` bridge is split between `src/host_providers/llvm_codegen/ll_emit_compare_source.rs`, `src/host_providers/llvm_codegen/ll_emit_compare_driver.rs`, `src/host_providers/llvm_codegen/ll_emit_compare_vm.rs`, and `src/host_providers/llvm_codegen/ll_emit_compare_stdout.rs`; `src/host_providers/llvm_codegen/provider_keep.rs` carries explicit provider keep lanes; the dedicated `ll_emit_bridge.rs` wrapper has been retired; the legacy MIR(JSON) front door is isolated in `src/host_providers/llvm_codegen/legacy_json.rs`; LLVM tool execution is split further into `src/host_providers/llvm_codegen/ll_tool_driver.rs`; compare proof itself is archive-suite only; `capi_transport.rs` owns the CAPI helpers while `transport_paths.rs` / `transport_io.rs` own the remaining temp-path helpers and compare-source temp materialization
  - `ll_emit/**`
  ...
  - `.hako ll emitter` compare/debug templating residue is now split across `ll_emit_compare_source.rs`, `ll_emit_compare_driver.rs`, `ll_emit_compare_vm.rs`, and `ll_emit_compare_stdout.rs`; `provider_keep.rs` owns explicit provider keep lanes, the dedicated `ll_emit_bridge.rs` wrapper has been retired, the legacy MIR(JSON) wrapper is isolated in `legacy_json.rs`, and `.ll` tool execution is isolated in `ll_tool_driver.rs`; the compiled-stage1 surrogate now loads MIR(JSON) locally and forwards text into `emit_object_from_mir_json(...)`, so the file-based front door is retired from that path
