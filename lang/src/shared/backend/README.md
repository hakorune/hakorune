...
Current owner
  ...
  - compare/debug residue for `.hako ll emitter` bridge is split between `src/host_providers/llvm_codegen/ll_emit_compare_source.rs`, `src/host_providers/llvm_codegen/ll_emit_compare_driver.rs`, and `src/host_providers/llvm_codegen/ll_emit_compare_vm.rs`; `src/host_providers/llvm_codegen/provider_keep.rs` carries explicit provider keep lanes; the dedicated `ll_emit_bridge.rs` wrapper has been retired; the legacy MIR(JSON) front door now lives as `emit_object_from_mir_json(...)` in `src/host_providers/llvm_codegen.rs`; LLVM tool execution is split further into `src/host_providers/llvm_codegen/ll_tool_driver.rs`; compare proof itself is archive-suite only; `capi_transport.rs` owns the CAPI helpers while `transport_paths.rs` / `transport_io.rs` own the remaining temp-path helpers and compare-source temp materialization; VM resolution and stdout extraction now live in the driver, and the VM module is spawn-only
  - `ll_emit/**`
  ...
  - launcher/mainline compile is now root-first; the legacy compare/archive transport is isolated in the `emit_object_from_mir_json(...)` facade helper in `src/host_providers/llvm_codegen.rs` and no longer the daily transport
  ...
  - `.hako ll emitter` compare/debug templating residue is now split across `ll_emit_compare_source.rs`, `ll_emit_compare_driver.rs`, and `ll_emit_compare_vm.rs`; `provider_keep.rs` owns explicit provider keep lanes, the dedicated `ll_emit_bridge.rs` wrapper has been retired, the legacy MIR(JSON) wrapper now lives as `emit_object_from_mir_json(...)` in `src/host_providers/llvm_codegen.rs`, and `.ll` tool execution is isolated in `ll_tool_driver.rs`; the compiled-stage1 surrogate now loads MIR(JSON) locally and forwards text into `emit_object_from_mir_json(...)`, so the file-based front door is retired from that path; VM resolution and stdout extraction now live in the driver and the VM module is spawn-only
