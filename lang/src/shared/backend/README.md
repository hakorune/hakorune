...
Current owner
  ...
  - `.hako ll emitter` compare/debug templating residue is now split across `ll_emit_compare_source.rs` and `ll_emit_compare_driver.rs`; `provider_keep.rs` owns explicit provider keep lanes and provider path resolution, the dedicated `ll_emit_bridge.rs` wrapper has been retired, the legacy MIR(JSON) wrapper now lives as `emit_object_from_mir_json(...)` in `src/host_providers/llvm_codegen/legacy_mir_front_door.rs`, and `.ll` tool execution is isolated in `ll_tool_driver.rs`; the compiled-stage1 surrogate now loads MIR(JSON) locally and forwards text into `emit_object_from_mir_json(...)`, so the file-based front door is retired from that path; VM resolution, spawn, and stdout extraction now live in the driver
