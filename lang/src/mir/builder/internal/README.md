...
- Use `program_json_input_contract_box.hako` (`hako.mir.builder.internal.program_json_input_contract`) as the dedicated owner for root Program.body extraction and defs-presence classification before fragment lowers run.
...
- Use `finalize_chain_box.hako` (`hako.mir.builder.internal.finalize_chain`) as the dedicated owner for canonical selfhost-first MIR promotion checks after shared finalize work.
...
- If the root Program.body / defs split needs to grow, extend `program_json_input_contract_box.hako` before widening the outer box again.
