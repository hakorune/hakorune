...
static int hako_llvmc_emit_indexof_line_ir(
  ...
  if (!obj_out || rows != 64 || ops <= 0 || flip_period != 16) {
    return -1;
  }
  ...
  row_mask = rows - 1;
  flip_mask = flip_period - 1;
  if ((rows & row_mask) != 0 || row_mask <= 0 || (flip_period & flip_mask) != 0 || flip_mask <= 0) {
    return -1;
  }
...
static int compile_json_compat_pure(const char* json_in, const char* obj_out, char** err_out) {
  ...
  if (hako_llvmc_match_indexof_line_ascii_seed(json_in, obj_out) == 0) {
    ...
    do {
      ...
      if (need_arr_set)   fprintf(f, "declare i64 @\"nyash.array.set_hih\"(i64, i64, i64)\n");
      if (need_arr_get)   fprintf(f, "declare i64 @\"nyash.array.slot_load_hi\"(i64, i64)\n");
