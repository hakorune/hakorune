...
int main(void) {
  /* The same valid graph is compiled twice. First the root's object 8 is the
   * callee receiver and both normal/fault cleanup paths are valid. Then only
   * the callee receiver_object is changed to 7; V4 must reject that mismatch
  ...
  const char* json_template =
      ...
      "{\"id\":1,\"instructions\":[{\"index\":0,\"instruction\":{\"op\":\"invoke_normal_result\",\"invoke_block\":0,\"dst\":2}}],\"terminator\":{\"index\":1,\"instruction\":{\"op\":\"invoke\",\"operation\":{\"kind\":\"ordinary_call\",\"call\":{\"target\":1,\"receiver\":2,\"args\":[],\"dst\":null},\"result\":\"i64\"},\"fault_frame\":1,\"normal\":3,\"fault\":5}},\"edges\":[{\"target\":3,\"args\":null},{\"target\":5,\"args\":null}]},"
      ...
      "{\"id\":5,\"instructions\":[],\"terminator\":{\"index\":0,\"instruction\":{\"op\":\"invoke\",\"operation\":{\"kind\":\"home_release\",\"object_id\":8,\"value\":2,\"site\":2},\"fault_frame\":1,\"normal\":6,\"fault\":6}},\"edges\":[{\"target\":6,\"args\":null},{\"target\":6,\"args\":null}]},"
  ...
  assert(snprintf(json, sizeof(json), json_template, 7u) > 0);
  write_input(json);
  remove(output_path);
  error = NULL;
  rc = hako_llvmc_compile_published_lifecycle_physical_v4(
      input_path, &session, output_path, &error);
  assert(rc != 0);
  assert(error != NULL);
  assert(strstr(error,
      "[freeze:contract][published-lifecycle-v4/receiver-object-mismatch]") != NULL);
  free(error);
