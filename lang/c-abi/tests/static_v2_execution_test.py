...
with tempfile.TemporaryDirectory(prefix="hakorune-static-v2-") as directory:
    ...
    if sys.argv[3:] == ["--formal-only"]:
        from static_v2_formal_cases import run_formal_cases
        run_formal_cases(compile_case, witness, const, ROOT, KERNEL, ENV, no_core)
        sys.exit(0)

    if sys.argv[3:] == ["--control-only"]:
        from static_v2_control_cases import run_control_cases
        run_control_cases(compile_case, witness, const, ROOT, KERNEL, ENV, no_core)
        sys.exit(0)
    ...
    for label, change, diagnostic in (
        ...
        ("formal-on-instruction", lambda b, f: f["values"][0].update(action=6), "static-v2/value-action"),
