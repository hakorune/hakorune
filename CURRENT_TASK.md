...
- `phase-156x` landed:
  ...
  - smallest useful `DeferredStableBox` slice is now in progress:
    - `materialize_owned_string(...)` issues `DeferredString` payloads through `to_handle_deferred_string(...)`
    - `StableBoxNow` is deferred until `get/with_handle/get_pair/get3/snapshot` require object world access
    - first exact read after this slice:
      - `kilo_micro_concat_hh_len`: `57 -> 51 ms` (plain release, 1x exact probe)
