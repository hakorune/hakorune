...
- current blocker token: `generic-method-route site operands inventory pending`
...
- latest known checkpoint: `291x-459` selects `GenericMethodRouteSite` /
  `GenericMethodRouteOperands` record split
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: inventory `GenericMethodRoute` site/operands consumers
...
- next: inventory `GenericMethodRoute` site/operands consumers
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-459`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: split `GenericMethodRoute` site/operands after inventory
- normalized-shadow / normalization cleanup burst is closed; larger findings
  must move to a new lane
