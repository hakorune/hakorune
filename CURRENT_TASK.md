...
- resume point: continue Phase 293x after `MIMAP-105A`; the next selected
  blocker is `MIMAP-ROW-CADENCE-001`, mimalloc row validation cadence SSOT.
...
- next 293x order:
  1. `MIMAP-ROW-CADENCE-001`: make mimalloc row validation levels explicit
     before the next allocator behavior row
