...
- current blocker token: `phase-291x JoinIR route detector small compatibility export prune pending`
...
- latest known checkpoint: `291x-372` migrated small route detector support
  families to semantic support owner paths
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: verify and prune zero-caller parent compatibility exports for
  small support families
...
- next: JoinIR route detector small compatibility export prune
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-372`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector small compatibility export prune
