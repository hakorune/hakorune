...
- active lane:
  ...
  - implementation mode:
    ...
    - `137x-H14` MIR-owned string search seed route is closed
      ...
      - guard: preserve `NYASH_LLVM_SKIP_INDEXOF_LINE_SEED`; if disabled, the line seed bridge must only activate from MIR metadata, not from C rediscovery
    ...
    - `137x-H15` generic array/text observer route is active
      ...
      - H15.1 design: extend `array_text_observer_routes` with a generic const-UTF8 observer-argument representation; `.inc` maps that metadata to a backend-local ptr+len helper, avoiding per-iteration public StringBox/handle lookup without naming helper symbols in MIR
      - H15.2 design: for `consumer_shape=found_predicate`, generalize the exact seed's literal membership proof into MIR-owned finite text-state observer outcomes; backend emits the predicate from metadata only when candidate/outcome proof is present
      - H15.1 result: seed-off hot loop now emits `nyash.array.string_indexof_hisi(array, row, ptr, len)` for const needles; `nyash.box.from_i8_string_const("line")` is gone from the 400k observer loop, but `Ny AOT` remains ~10-11 ms
      - H15.2 rejected probe: metadata-owned literal membership lowering was tested as `nyash.array.string_indexof_membership_hisisisi(array, row, needle, found_literal, not_found_literal)`, but seed-off still measured `C 5 ms / Ny AOT 10 ms`, `ny_aot_instr=121035205`, `ny_aot_cycles=31691591`; the non-keeper helper ABI was retired before commit
      - H15.2 owner: perf top stayed on the helper closure and parking_lot/RwLock-style array text slot access, not generic `memchr`; exact seed wins because it has loop-local pointer-array residence and pointer equality, not because of a better search helper
      - H15.3 design: stop chasing helper micro-variants; next keeper seam must be a MIR-owned text-state/array-lane residence contract that lets backend keep the 64-row text state loop-local (or an equivalent loop-resident cursor). Periodic const store ptr+len is secondary until the read-side RwLock/call boundary is gone
      - H15.3a implementation slice: consume `indexof_search_micro_seed_route` inside the generic pure-first lane as `text-state residence` when the MIR proof is present; reuse the resident pointer-array emitter, but do not call raw C window scanners and do not treat helper/runtime symbols as MIR truth
      - H15.3a result: with `NYASH_LLVM_SKIP_INDEXOF_LINE_SEED=1`, seed-off `kilo_micro_indexof_line` is back to `C 4 ms / Ny AOT 3 ms`, `ny_aot_instr=6198985`, `ny_aot_cycles=1554378`; route trace shows `stage=indexof_line_text_state_residence result=emit reason=text_state_residence`, and IR has no `slot_load_hi`, `runtime_data`, `from_i8_string_const`, or array indexOf helper calls
      - H15.3b implementation slice: split the generic residence JSON key from the temporary exact bridge key; exact seed wrappers keep reading `indexof_search_micro_seed_route`, while generic pure-first residence reads `array_text_state_residence_route`
      - H15.3b result: trace bundle metadata now exports both keys; `array_text_state_residence_route.consumer_capability=direct_array_text_state_residence`, `residence=loop_local_pointer_array`, and seed-off route trace still emits `indexof_line_text_state_residence`; seed-off `kilo_micro_indexof_line` remains `C 5 ms / Ny AOT 4 ms`, exact remains `C 4 ms / Ny AOT 4 ms`
