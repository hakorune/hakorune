...
- resume point: add a normalized-shadow shared expression lowering facade before
  touching the full legacy entry path
...
- next: normalized-shadow shared expression lowering facade
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-395`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: normalized-shadow shared expression lowering facade
