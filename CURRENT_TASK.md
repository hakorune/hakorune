...
- latest known checkpoint: `291x-359` pruned parent exports for
  legacy-internal-only route detector modules
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: inventory legacy route-shape function exports; do not remove
  functions in the same card
...
- next: JoinIR route detector legacy route function export inventory; do not
  prune functions in the same card
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-359`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector legacy route function export inventory
