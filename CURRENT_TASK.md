...
- latest known checkpoint: `291x-391` moved the remaining route detector
  body-local support family into physical `support` modules and closed legacy
  storage
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: run route detector physical-owner closeout review and choose
  the next compiler-cleanliness seam
...
- next: JoinIR route detector physical owner closeout review
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-391`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector physical owner closeout review
