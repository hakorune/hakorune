...
26. `phase-105 digit OR-chain LLVM parity regression` (active)
27. `phase-110x selfhost execution vocabulary SSOT` (queued)

## Current Front

- Active lane: `phase-105 digit OR-chain LLVM parity regression`
- Active micro: `long digit OR-chain under loop(true) を LLVM EXE parity で戻す`
- Current blocker: `LLVM EXE で long digit OR-chain が 0/1 に潰れる`
- Exact focus: `phase-104 fixture の sequential-flag workaround を外せるまで OR-chain lowering を詰める`
...
- `phase-104` fixed loop(true)+break-only digit fixtures with a sequential-flag workaround; `phase-105` tracks the original long OR-chain parity gap.
