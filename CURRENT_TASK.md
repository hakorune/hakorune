...
12. `phase-146x string semantic boundary tighten` (active)
...
- Active lane: `phase-146x string semantic boundary tighten`
- Active front: `.hako` string semantic owner / VM wrapper / Rust native substrate の stop-line を source/docs で固定する
- Current blocker: string wrapper residue と native substrate helper が source 上でまだ近く見える
- Exact focus: `string_core_box.hako` / `runtime/kernel/string/README.md` / `string_view.rs` / `string_plan.rs` / `string_helpers.rs` を policy-vs-substrate の読みで揃える
...
5. `docs/development/current/main/phases/phase-146x/README.md`
