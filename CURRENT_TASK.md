...
- Active: `K2-wide` next structural follow-up
  ...
  - current capability row is `hako.osvm` truthful `reserve_bytes_i64(len_bytes)`
  - current policy/state row after capability widening is `hako_alloc` truthful `handle reuse policy`
  - next policy/state row after that is `GC trigger threshold policy`
