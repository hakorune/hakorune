...
- the active Rust stop-line now sits in `phase-29cj` on `src/host_providers/mir_builder.rs`; the latest source-route authority / output projection split is landed in `handoff.rs`, and the latest `.hako` cuts above the same stop-line now isolate `MirBuilderBox` unsupported tail plus runner-local payload/input/I/O contracts
...
- `phase-29cj` is now the next live mainline slice:
  - keep `build_surrogate.rs`, `program_json/`, and `program_json_entry/` near thin floor
  - keep the latest Rust handoff split in `src/host_providers/mir_builder/{rs,handoff.rs,decls.rs}` as landed
  - keep the latest `.hako` owner-local cuts landed too: `BuilderUnsupportedTailBox`, `Stage1MirPayloadContractBox`, `Stage1CliProgramJsonInputBox`, and `LauncherArtifactIoBox`
  - the next `.hako` reopen should start from a near-thin-floor reinventory across runner owners before touching `build_box.hako`
  - exact current cut is the latest landed `.hako` route/payload/input/I/O helper split above the existing internal route table; choose the next reopen only after a near-thin-floor reinventory
- `phase-29ci` outer caller audit is now split into two waves:
  ...
  - wave 1 has started: `stage1_cli_env.hako`, `launcher.hako`, `stage1_cli.hako`, and `MirBuilderBox.hako` now keep direct `BuildBox` / `MirBuilderBox` calls behind same-file tiny helpers, with `MirBuilderBox.hako` now splitting the source-entry compat seam into `MirBuilderSourceCompatBox`, the Program(JSON) entry contract into `BuilderProgramJsonInputContractBox`, the func-def pre-lowering gate into `BuilderFuncDefsGateBox`, the dev-only loop-force route into `BuilderLoopForceRouteBox`, and the generic unsupported tail into `BuilderUnsupportedTailBox`
  - `stage1_cli_env.hako` now also keeps MIR payload materialize/structural validation/print behind `Stage1MirPayloadContractBox`, so `Stage1MirResultValidationBox` is down to debug + checked handoff only
  - `stage1_cli.hako` now also keeps source/program-json input resolve behind `Stage1CliProgramJsonInputBox`, so the future-retire raw lane no longer mixes file read / placeholder resolve / BuildBox handoff inline
  - `launcher.hako` now also keeps artifact file I/O and stdout/file output selection behind `LauncherArtifactIoBox`, so the launcher lane no longer keeps read/write routing inline around checked emit/build paths
