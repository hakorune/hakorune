...
- resume point: continue Phase 293x after `MIMAP-095A`; the next selected
  blocker is `MIMAP-096A`, post-segment-allocation-modeled-ledger row
  selection.
...
- next 293x order:
  1. `MIMAP-096A`: select exactly one next row after the modeled segment
