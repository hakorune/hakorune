...
- latest known checkpoint: `291x-392` closed the route detector physical owner
  migration review and refreshed stale live docs that pointed at deleted
  legacy storage
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: inventory the next compiler-cleanliness seam before editing
  more code
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-392`; detailed landed history lives in phase card
