...
- resume point: continue Phase 293x after `MIMAP-045A`; current blocker is
  `MIMAP-045B`, a planning-only row that selects the next single allocator,
  compiler, or language task after the OSVM-backed fast-path unreserve route.
...
- next 293x order:
  1. `MIMAP-045B`: classify the post-fast-path-unreserve state and select one
     next row
