...
- active lane:
  ...
  - current app/runtime gap read:
    ...
    - CoreBox router follow-up has moved `StringBox.length/len/size`, `StringBox.substring/substr`, `StringBox.concat`, and `StringBox.trim` to the Unified value path; remaining cleanup is about 6 family-equivalents, starting with `contains`
