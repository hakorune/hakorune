...
- resume point: continue Phase 293x after `MIMAP-089A`; the next selected
  blocker is `MIMAP-090A`, post-segment-allocation-readiness row selection.
...
- next 293x order:
  1. `MIMAP-090A`: select exactly one next mimalloc row after the segment
