...
- split:
  ...
  - `backend-zero`: accepted pointer / `phase-29ck` queued
    ...
    - landed slice:
      ...
      - latest BE0-min2e boundary-command slice: `lang/c-abi/shims/hako_aot_shared_impl.inc` now builds compile commands with `ny-llvmc --driver boundary`, and `tools/smokes/v2/profiles/integration/apps/phase29ck_boundary_forwarder_min.sh` pins the default `hako_llvmc_compile_json` forwarder path so `HAKO_CAPI_PURE`-unset callers still reach the boundary-owned command route
  ...
  1. thin backend boundary hardening
     ...
     - role:
       ...
       - fixed order:
         ...
         - exact next slice: widen boundary-owned compile coverage beyond `ret_const` / `Global.print` / `StringBox.length` / `RuntimeDataBox.length(StringBox)` into the next unsupported method-shaped seed, starting with narrow `StringBox.indexOf/1` on ASCII literals before broader RuntimeDataBox array/map or method-loop packs
     - acceptance:
