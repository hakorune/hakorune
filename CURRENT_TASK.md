...
- resume point: continue Phase 293x with `MIMAP-189A`,
  the post-segment-map-local-free-reuse-bridge planning row selected by
  MIMAP-188A.
...
- next 293x order:
  1. `MIMAP-189A`: choose the next narrow row after segment-map local-free
     reuse bridge
