...
8. `phase-142x array owner cutover implementation` (active)
9. `phase-143x map owner cutover implementation` (successor)
...
- Active lane: `phase-142x array owner cutover implementation`
- Active front: Array visible semantics を `.hako` owner 実装へ寄せ、Rust を thin ABI facade / compat forwarding / accelerator に押し戻す
- Current blocker: seam 宣言は landed したが、owner 実装の実移管はまだ
- Exact focus: `ArrayBox.push` landed as the first cutover unit; next is `ArrayBox.get/set` を同じ owner-helper 粒度に揃える。`array_core_box.hako` / `array_state_core_box.hako` 側で visible get/set/push semantics を authority owner として持たせ、Rust 側は `array_runtime_any.rs` / `array_runtime_facade.rs` / `array_runtime_substrate.rs` の shell/provider に押し戻す
...
1. `docs/development/current/main/05-Restart-Quick-Resume.md`
2. `docs/development/current/main/15-Workstream-Map.md`
3. `docs/development/current/main/phases/phase-142x/README.md`
...
- `phase-142x` current cutover:
  - `Array owner` seam is no longer just declarative
  - next step is implementation-side authority shift into `.hako`
- `phase-143x` queued cutover:
  - `Map owner` implementation follows after Array settles
