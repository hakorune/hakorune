...
- Active front: split kernel で `kilo_kernel_small_hk` / kilo micro の baseline は取り直した。次は bundle を取って next hot leaf を pin する
- Current blocker: main kilo gap は依然大きいが、next hot leaf がまだ未固定
- Exact focus: `kilo_kernel_small_hk` trace/asm bundle を取り、split kernel 上の next hot leaf を source-backed に選ぶ
