...
- Active front: string const-path reopen は継続中。const empty-flag cache まで landed し、top explicit hot symbol はまだ `concat_const_suffix_fallback`
- Current blocker: `kilo_kernel_small_hk` は `ny_aot_ms=723` まで落ちたが、まだ C (`81ms`) との gap が大きい
- Exact focus: `crates/nyash_kernel/src/exports/string_helpers.rs` の `concat_const_suffix_fallback(...)` を first leaf、`array_string_store_handle_at(...)` を second leaf として詰める
