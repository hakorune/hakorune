...
- resume point: continue Phase 293x with `MIMAP-197A`,
  the post-segment-map-local-free-reuse-ledger-release-bridge planning row
  selected by MIMAP-196A.
...
- next 293x order:
  1. `MIMAP-197A`: choose the next narrow row after segment-map local-free
