...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf is now inside shared producer `lang/src/compiler/build/build_box.hako`: `_prepare_scan_src(...)` still writes resolved `scan_src` into the result map inline after the bundle resolve branch, so the next safe same-owner slice is helper-local resolved-scan-src apply handoff
