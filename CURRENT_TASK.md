...
- latest known checkpoint: `291x-380` migrated body-local promoter callers to
  `support::body_local` or owner-local legacy paths
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: verify and prune zero-caller parent body-local compatibility
  exports
...
- next: JoinIR route detector body-local compatibility export prune
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-380`; detailed landed history lives in phase card
