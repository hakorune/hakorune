...
- resume point: continue Phase 293x after `MIMAP-098A`; the next selected
  blocker is `MIMAP-099A`, post-segment-allocation-modeled-release row
  selection.
...
- next 293x order:
  1. `MIMAP-099A`: select exactly one next row after the modeled segment
     allocation release closeout
