...
- latest known checkpoint: `291x-322` pruned carrier-count-only Case-A shape
  selection so specialized lowerers require update proof or descriptor fallback
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: inventory loop-update AST analyzer index-name heuristic; do
  not mix with Case-A carrier-count pruning
...
- next: JoinIR loop-update index-name heuristic inventory; do not mix with
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-322`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR loop-update index-name heuristic inventory
