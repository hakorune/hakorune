...
26. `phase-110x selfhost execution vocabulary SSOT` (active)

## Current Front

- Active lane: `phase-110x selfhost execution vocabulary SSOT`
- Active micro: `stage / route / backend override / lane / kernel を長寿命 SSOT に固定する`
- Current blocker: `none`
- Exact focus: `run.sh --runtime --runtime-mode exe` と `hakorune --backend vm` の階層差を vocabulary で固定する`
...
- `phase-104` fixed loop(true)+break-only digit fixtures on VM and LLVM EXE.
