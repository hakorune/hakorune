...
- current blocker token: `phase-291x JoinIR LoopFeatures if-phi signature stub prune pending`
...
- latest known checkpoint: `291x-339` inventoried the LoopForm if-phi
  signature helper and found it is a dead reserved seam
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: prune the LoopForm if-phi signature stub; do not change route
  behavior
...
- next: JoinIR `LoopFeatures` if-phi signature stub prune; do not change
  route behavior
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-339`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR `LoopFeatures` if-phi signature stub prune
