...
- latest verified micro ladder (2026-03-18):
  ...
  - current exact leaf slice for `array_getset`: `lang/src/runtime/collections/array_core_box.hako::try_handle(...)` now hoists the shared size-state env/len lookups once per dispatch, so `set/get/push` reuse the same locals instead of re-reading `env` / `regs` in each branch
  - follow-up on the same leaf: `try_handle(...)` now branches before taking the size-state prelude, so `set/get/push` only load the env/len locals they actually need; fresh `kilo_micro_array_getset` recheck stayed flat (`ny_aot_ms=66`), so this lane is still the thickest but no longer has an obvious cheap local split
