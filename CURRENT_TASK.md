...
- resume point: inventory MapLookupFusionRoute public field readers
...
- next: inventory MapLookupFusionRoute public field readers
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-484`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: inventory MapLookupFusionRoute public field readers
- normalized-shadow / normalization cleanup burst is closed; larger findings
  must move to a new lane
