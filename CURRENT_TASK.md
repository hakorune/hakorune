...
- resume point: clean stale normalized-shadow ANF status wording without
  changing ANF routing or accepted expression shapes
...
- next: normalized-shadow ANF status wording cleanup
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-422`; detailed landed history lives in phase card
