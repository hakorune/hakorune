...
- resume point: continue Phase 293x with `MIMAP-203A`,
  the post-segment-map-local-free-reuse-ledger-release-apply-bridge-closeout
  planning row selected by MIMAP-202A.
...
- next 293x order:
  1. `MIMAP-203A`: choose the next narrow row after segment-map local-free
     reuse ledger release apply bridge closeout
