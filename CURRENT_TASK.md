...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed `.hako` slice moves the delegate gate/provider call behind `lang/src/mir/builder/internal/delegate_provider_box.hako::BuilderDelegateProviderBox.try_emit(...)`, leaving `MirBuilderBox.hako` with route sequencing plus local delegate finalize
  ...
  - exact next leaf stays in `lang/src/mir/builder/MirBuilderBox.hako`: the remaining delegate-local finalize seam is `_inject_delegate_user_box_decls(...)` plus `_build_user_box_decls_from_program_json_inline(...)`; the delegate-only probe now reaches the pre-existing `Unknown Box type: hostbridge` residue after `BuilderDelegateProviderBox`, so do not treat that probe failure as a regression from this slice
