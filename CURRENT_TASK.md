...
- split:
  ...
  - `backend-zero`: accepted pointer / `phase-29ck` queued
    ...
    - landed slice:
      ...
      - next backend front is no longer launcher caller cutover; it is pinned to B3a `tools/llvmlite_harness.py` / `src/llvm_py/llvm_builder.py` entry demotion and compiled-stage1 surrogate shrink
