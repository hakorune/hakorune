...
- `phase-29ci` outer caller audit is now split into two waves:
  ...
  - wave 1 has started: `stage1_cli_env.hako`, `launcher.hako`, `stage1_cli.hako`, and `MirBuilderBox.hako` now keep direct `BuildBox` / `MirBuilderBox` calls behind same-file tiny helpers, with `MirBuilderBox.hako` now splitting the source-entry compat seam into `MirBuilderSourceCompatBox` and the remaining func-def pre-lowering gate into `BuilderFuncDefsGateBox`, while the public wrapper methods delegate to private/raw or internal leaves instead of keeping those seams inline
