...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf should now return to runner-owner neighbors: `lang/src/runner/stage1_cli.hako` `_emit_mir_json_from_program_json_checked(...)` and then `lang/src/runner/launcher.hako` `_emit_exe_from_mir_json_checked(...)`; keep `build_box.hako` frozen unless another exact disappearing leaf appears first
