  ...
  1. thin backend boundary hardening
     ...
     - role:
       ...
       - fixed order:
         ...
         - exact next slice: widen boundary-owned compile coverage beyond `ret_const` / `Global.print` / `StringBox.length` / `RuntimeDataBox.length(StringBox)` / `StringBox.indexOf/1` into the next unsupported RuntimeDataBox array/map method-shaped seed before broader method-loop packs
     - acceptance:
