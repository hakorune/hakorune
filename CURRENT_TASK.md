...
28. `phase-160x capability-family inventory` (active)
29. `phase-161x hot-path capability seam freeze` (next)
30. `phase-137x main kilo reopen selection` (successor after capability map lock)
31. `phase-kx vm-hako small reference interpreter recut` (parked after optimization)

## Current Front

- Active lane: `phase-160x capability-family inventory`
- Active front: current Rust helper 群を future capability family に棚卸しし、perf hot path がどの seam に属するかを source-backed に固定する
- Current blocker: capability seam が曖昧なまま最適化を続けると、後で substrate family 化するときに hot path を読み直す二度手間が出る
- Exact focus:
...
- capability-family lock before perf reopen:
  ...
  3. only then resume micro + main kilo tuning
