...
- resume point: continue Phase 293x with `MIMAP-193A`,
  the post-segment-map-local-free-reuse-ledger-bridge planning row selected
  by MIMAP-192A.
...
- next 293x order:
  1. `MIMAP-193A`: choose the next narrow row after segment-map local-free
     reuse ledger bridge
