...
- current BoxShape sidecar:
  ...
  `SELFHOST-PROGRESS-001`, and `MIR-EMIT-SSOT-002` landed; `MIMAP-039A`
  landed the remote-free retry-bound named owner cleanup;
...
- resume point: continue Phase 293x after `MIMAP-039A`; current blocker is
  `MIMAP-039B`, a post-remote-free-retry-bound row selection. It must choose
  one next owner/proof/guard before provider behavior or host allocator
  replacement is opened. VM-LIM-001
...
- next 293x order:
  1. `MIMAP-039B`: post-remote-free-retry-bound row selection
  2. selected next row after `MIMAP-039B`
