...
- latest known checkpoint: `291x-321` inventoried the Case-A carrier-count
  heuristic that selected specialized lowerers without update proof
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: prune Case-A carrier-count-only specialized shape selection;
  keep exact known targets on descriptor fallback
...
- next: JoinIR Case-A carrier-count heuristic prune; keep exact known
  targets on descriptor fallback
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-321`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR Case-A carrier-count heuristic prune
