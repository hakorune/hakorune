...
- resume point: continue Phase 293x after `MIMAP-117A`; the next selected
  blocker is `MIMAP-118A`, post-local-free-page-apply-closeout row selection.
...
- next 293x order:
  1. `MIMAP-118A`: select exactly one next row using the mimalloc row validation
     cadence
