...
- resume point: replace normalized-shadow raw `main`/`loop_step`/`k_exit`/`post_k`
  names with existing canonical-name constants
...
- next: normalized-shadow generic function-name cleanup
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-412`; detailed landed history lives in phase card
