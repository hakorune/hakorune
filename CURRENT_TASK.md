...
- summary:
  ...
  - naming cleanup (2026-03-07, slice 47): recognizer の remaining on-disk residue を semantic path へ揃えた
    ...
    - verification: `rg -n "Pattern Recognizers Module|Pattern Detection|pattern-specific semantics|^/// Pattern:|Recognized patterns|pattern analyzers|if-sum patterns|loop pattern\\.|int_to_str loop pattern" src/mir/builder/control_flow/plan/route_shape_recognizers src/mir/builder/control_flow/plan/generic_loop/body_check_shape_detectors src/mir/builder/control_flow/plan/generic_loop/README.md` = 0 hit
  ...
  - naming cleanup (2026-03-07, slice 49): JoinIR lowering の route cluster を `pattern` 主語から `route` 主語へ寄せた
    ...
    - intent: `loop_pattern_router.rs` / `loop_pattern_validator.rs` / `loop_patterns/` を `loop_route_router.rs` / `loop_route_validator.rs` / `loop_routes/` へ rename し、current lowering path と helper 名を route-first に揃える
    - verification: `rg -n "loop_pattern_router|loop_pattern_validator|LoopPatternValidator|try_lower_loop_pattern_to_joinir|loop_patterns/|loop_patterns::|loop_patterns/tests" src/mir docs/development/current/main/design -g '!src/mir/join_ir/frontend/**' -g '!**/*history*' -g '!**/*archive*'` = 0 hit
    - synced files: `src/mir/builder/control_flow/normalization/{README.md,mod.rs,plan.rs,plan_box.rs,execute_box.rs}` / `src/mir/builder/control_flow/plan/policies/policies/normalized_shadow_suffix_router_box.rs` / `src/mir/loop_canonicalizer/canonicalizer_tests/{parse_number,trim_trailing,trim_leading,parse_string,parse_array,parse_object,escape_skip,skip_whitespace,continue_pattern}.rs` / `src/tests/{joinir_vm_bridge_trim,joinir_vm_bridge_skip_ws}.rs`
