...
- latest known checkpoint: `291x-383` inventoried route detector support
  facades for physical owner migration and chose small-family-first order
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: align `legacy/README.md` and `support/README.md` with the
  current private-legacy/public-support boundary
...
- next: JoinIR route detector legacy/support README boundary cleanup
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-383`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector legacy/support README boundary cleanup
