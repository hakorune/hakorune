...
- current BoxShape sidecar:
  ...
  `SELFHOST-PROGRESS-001`, and `MIR-EMIT-SSOT-002` landed; `MIMAP-032B`
  selected `MIMAP-033A`;
...
- resume point: continue Phase 293x after `MIMAP-032B`; current blocker is
  `MIMAP-033A`, a page-source unreserve adapter row. It must not add facade
  huge unreserve, recommit, or provider behavior. VM-LIM-001
...
- next 293x order:
  1. `MIMAP-033A`: page-source unreserve adapter
  2. post-page-source-unreserve row selection
