...
- Exact focus: `MapBox.set/get/has` landed as owner-helper units; next is `MapBox.len/length/size` を同じ owner-helper 粒度に揃える。`map_core_box.hako` / `map_state_core_box.hako` 側で visible map semantics を authority owner helper として持たせ、Rust 側は `map_aliases.rs` / `map_substrate.rs` / `map_runtime_facade.rs` の shell/provider に押し戻す
