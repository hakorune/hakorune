...
- resume point: continue Phase 293x after `MIMAP-109A`; the next selected
  blocker is `MIMAP-110A`, post-local-free-candidate-ledger row selection.
...
- next 293x order:
  1. `MIMAP-110A`: select exactly one next row using the mimalloc row validation
     cadence
