...
- current blocker token: `phase-291x JoinIR route detector condition-scope physical owner move pending`
...
- latest known checkpoint: `291x-389` moved route detector function-scope
  support into a physical `support` owner directory
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: inventory and move `support::condition_scope` out of private
  `legacy/` storage
...
- next: JoinIR route detector condition-scope physical owner move
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-389`; detailed landed history lives in phase card
