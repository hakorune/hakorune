...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf is now inside shared producer `lang/src/compiler/build/build_box.hako`: `_prepare_scan_src(...)` is near thin floor, so the next safe same-owner slice should be another exact disappearing leaf only if it appears; otherwise move to the next nearby `.hako` owner instead of forcing micro-splits
