...
- resume point: continue Phase 293x with `MIMAP-177A`,
  the post-segment-map-local-free-apply-plan-bridge planning row selected by
  MIMAP-176A.
...
- next 293x order:
  1. `MIMAP-177A`: choose the next narrow row after segment-map local-free
     apply-plan bridge
