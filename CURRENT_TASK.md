...
- resume point: continue Phase 293x with `MIMAP-261A`,
  the segment arena backing modeled source bridge diagnostics selected by
  MIMAP-260A.
...
- next 293x order:
  1. `MIMAP-261A`: add observer-only diagnostics for modeled source bridge
     inventory
