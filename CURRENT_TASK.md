...
- current BoxShape sidecar:
  ...
  `SELFHOST-PROGRESS-001`, and `MIR-EMIT-SSOT-002` landed; `MIMAP-033A`
  landed page-source unreserve adapter adoption;
...
- resume point: continue Phase 293x after `MIMAP-033A`; current blocker is
  `MIMAP-033B`, a post-page-source-unreserve row selection. It must choose one
  next owner/proof/guard before facade huge unreserve or provider behavior is
  opened. VM-LIM-001
...
- next 293x order:
  1. `MIMAP-033B`: post-page-source-unreserve row selection
  2. selected next allocator row after `MIMAP-033B`
