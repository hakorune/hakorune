...
- resume point: continue Phase 293x after `MIMAP-106A`; the next selected
  blocker is `MIMAP-107A`, segment allocation modeled released-span ledger
  route.
...
- next 293x order:
  1. `MIMAP-107A`: add a scalar released-span ledger downstream of MIMAP-104A
     release span facts using the L2 validation cadence
