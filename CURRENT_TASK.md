...
25. `phase-157x observe feature split` (active)
26. `phase-137x main kilo reopen selection` (active consumer after feature split)
27. `phase-kx vm-hako small reference interpreter recut` (parked after optimization)

## Current Front

- Active lane: `phase-157x observe feature split`
- Active front: observer を authority stack の外に固定し、default release から compile-out する
- Current blocker: counter 実装が default release にも入って見えること
- Exact focus:
  - `docs/development/current/main/phases/phase-157x/README.md`
  - `crates/nyash_kernel/Cargo.toml`
  - `crates/nyash_kernel/src/observe/mod.rs`
  - `crates/nyash_kernel/src/observe/contract.rs`
  - `crates/nyash_kernel/src/observe/backend_atomic.rs`
...
7. `docs/development/current/main/phases/phase-157x/README.md`
