...
- current blocker token: `phase-291x JoinIR route detector body-local support family migration pending`
...
- latest known checkpoint: `291x-379` pruned the zero-caller parent
  `loop_condition_scope` compatibility export
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: migrate `loop_body_carrier_promoter` and
  `loop_body_cond_promoter` callers to `support::body_local`
...
- next: JoinIR route detector body-local support family migration
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-379`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector body-local support family migration
