...
- policy remains above Rust, mechanics remain in Rust:
  ...
  - `Rust`: source-lifetime mechanics only
- if the restart needs a 30-second read order, start with:
  1. this block
...
- Current next design slice:
  ...
  - no-behavior-change `source_kind_check` split is now landed in `string_store.rs`
    - `StringLikeProof` is now separated from `SourceLifetimeKeep`
    - `ArrayStoreStrSource::StringLike` now carries:
  ...
  - latest landed string-like proof split:
    - `SourceKindCheck` now records `StringLikeProof` separately from `SourceLifetimeKeep`
    - `execute_store_array_str_slot(...)` now reads source-kind observe truth from the typed source contract instead of repeating local downcasts
    - this is still no-behavior-change; purpose is to narrow the next behavior cut to keep semantics rather than source-kind transport
    - 3-run plain release reread:
  ...
  - close live-source direct-read widening:
    - do not try to satisfy `BorrowedHandleBox::as_str_fast()` through registry/session-backed direct reads
    - keep stable-object read and live-source session read as separate contracts
  - latest landed read-contract naming cleanup:
    - `SourceLifetimeKeep` stable-object text read is now named as stable-object read, not generic fast read
    - `ArrayStoreStrSource::object_ref()` is now `stable_object_fallback_ref()`
    - this is no-behavior-change; it narrows the next cut away from object-centric naming
    - accept-gate reread:
      - `kilo_micro_array_string_store: 173 ms`
  ...
  - latest landed module string dispatch cleanup:
    - removed `plugin/module_string_dispatch.rs` direct `host_handles::get()/to_handle_arc()`
      bypass for compat string handle decode/encode
    ...
    - this keeps the layer boundary on the `value_codec` seam and clears the review's
      only direct `host_handles` bypass in `module_string_dispatch.rs`
    - accept-gate reread:
      - `kilo_micro_array_string_store: 176 ms`
      - `kilo_micro_concat_hh_len: 67 ms`
  ...
  - latest landed by-value `VerifiedTextSource` consumption:
    ...
    - string-like store now also consumes keep by value through:
      ...
      - `store_string_box_from_verified_text_source(...)`
    - regression tests now pin:
      - verified-text retarget success still updates borrowed alias slot
      - verified-text `Err` fallback still preserves `StringView` materialization
      - owned keep store path still preserves borrowed alias handle reuse
    - test gate:
  ...
  - latest landed `string_classify.rs` split:
    - moved `SourceKindCheck` and typed `ArrayStoreStrSource` construction out of
      `string_store.rs` into `value_codec/string_classify.rs`
    - `string_store.rs` now keeps materialize/objectize/publication and store fallback
      execution only
    - this is a no-behavior physical split after the module-string layer bypass removal
    - accept-gate reread:
      - `kilo_micro_array_string_store: 174 ms`
      - `kilo_kernel_small_hk: 715 ms`
    - one `1894 ms` whole-kilo outlier was discarded after the immediate reread
      returned to the current band
