...
- resume point: continue Phase 293x after `MIMAP-083A`; the next selected
  blocker is `MIMAP-084A`, the post-segment-lifecycle-closeout row selection.
...
- next 293x order:
  1. `MIMAP-084A`: select exactly one next row after the scalar segment
     lifecycle closeout
