  ...
  - post-primitive follow-on queue:
    ...
    8. next ready task: resume optimization lane
        ...
        - next substep after the current parity-wave keeper:
          ...
          - fresh broader-corridor reread keeps `kilo_micro_substring_concat` (`instr=5,565,655 / cycles=5,816,743 / cache-miss=9,424 / AOT 4 ms`) as the current micro reopen front, but the next practical `materialization_sink` hotspot is now exploratory `kilo_meso_substring_concat_array_set` (`instr=384,348,305 / cycles=185,411,844 / AOT 40 ms`) on the `array.set + trailing length()` window
