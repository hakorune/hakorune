...
- de-rust migration orchestration lane: `phase-29cc / plugin-wave-rollout`（RNR-05 complete; PLG-03 done; PLG-04-min1..min6 done; wave-1 complete; PLG-05-min1/min2/min3/min4/min5/min6/min7 done; PLG-06-min1/min2/min3/min4 done）
  ...
  - wasm P10 locks（min1 accepted-but-blocked, min2/min3/min4/min5 accepted）:
  ...
  - wasm lane active next:
    - `WSM-P10-min6`（loop/extern warn family native promotion lock）
...
1. wasm lane: `WSM-P10-min6`（loop/extern warn family native promotion lock）へ進める。
2. `WSM-P7..P10-min5` lock（`29cc-184..198`）を維持し、default-only 契約を崩さない。
