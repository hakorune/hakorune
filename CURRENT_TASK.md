...
- current blocker token: `phase-291x JoinIR route detector legacy convenience re-export prune pending`
...
- latest known checkpoint: `291x-364` inventoried unused convenience
  re-exports inside `loop_route_detection/legacy`
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: prune unused convenience re-exports from `legacy/mod.rs`;
  keep owning modules exported
...
- next: JoinIR route detector legacy convenience re-export prune; keep owning
  modules exported
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-364`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector legacy convenience re-export prune
