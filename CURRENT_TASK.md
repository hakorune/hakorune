...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf is now inside shared producer `lang/src/compiler/build/build_box.hako`: `_collect_bundle_inputs(...)` is closer to a field handoff table now, so the next safe same-owner slice should only happen if another exact disappearing leaf appears; otherwise move to the next nearby `.hako` owner instead of forcing more micro-splits
