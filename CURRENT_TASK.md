...
- current blocker token:
  `MIMAP-127A post-local-free-reuse row selection`
...
- resume point: continue Phase 293x with `MIMAP-127A`, a planning-only row
  after the modeled local-free reuse route landed.
...
- next 293x order:
  1. `MIMAP-127A`: review the current segment allocation modeled lane after
     local-free reuse and pick exactly one next row
