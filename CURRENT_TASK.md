...
- the active Rust stop-line now sits in `phase-29cj` on `src/host_providers/mir_builder.rs`; the latest source-route authority / output projection split is landed in `handoff.rs`, and the current active `.hako` leaf is `MirBuilderBox` loop-force route isolation above the same stop-line
...
- `phase-29cj` is now the next live mainline slice:
  ...
  - spend the next owner-local slice in `lang/src/mir/builder/MirBuilderBox.hako` and `lang/src/mir/builder/internal/**`
  - exact current cut is the dev-only loop-force route isolation above the existing internal route table
- `phase-29ci` outer caller audit is now split into two waves:
  ...
  - wave 1 has started: `stage1_cli_env.hako`, `launcher.hako`, `stage1_cli.hako`, and `MirBuilderBox.hako` now keep direct `BuildBox` / `MirBuilderBox` calls behind same-file tiny helpers, with `MirBuilderBox.hako` now splitting the source-entry compat seam into `MirBuilderSourceCompatBox`, the Program(JSON) entry contract into `BuilderProgramJsonInputContractBox`, the func-def pre-lowering gate into `BuilderFuncDefsGateBox`, and the dev-only loop-force route into `BuilderLoopForceRouteBox`, while the public wrapper methods delegate to private/raw or internal leaves instead of keeping those seams inline
