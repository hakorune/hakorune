...
- pure `.hako-only hakorune build` / Stage2 bootstrap closure exact front (2026-03-17):
  ...
  - exact next order is pinned:
    ...
    7. latest `phase-29cl` caller-cutover slice is landed: LLVM direct-call lowering now recognizes compiled-stage1 module-string receivers for `BuildBox.emit_program_json_v0(...)` and `MirBuilderBox.emit_from_program_json_v0(...)`, so the next by-name front is the remaining generic/mainline caller set beyond that pair, not kernel delete
