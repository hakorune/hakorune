...
- resume point: continue Phase 293x after `MIMAP-119A`; the next selected
  blocker is `MIMAP-120A`, post-local-free-integration row selection.
...
- next 293x order:
  1. `MIMAP-120A`: select exactly one next row using the mimalloc row validation
     cadence
