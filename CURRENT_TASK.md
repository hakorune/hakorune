...
37. `phase-120x vm route retirement decision refresh` (active)

## Current Front

- Active lane: `phase-120x vm route retirement decision refresh`
- Active micro: `compat / proof / debug-observability の3 buckets から retirement order を再固定する`
- Current blocker: `none`
- Exact focus: `run.sh` / `selfhost_run_routes.sh` / `tools/selfhost/proof/*` / `phase29x` observability keep の残存理由を refresh し、次に retire する入口を固定する`
