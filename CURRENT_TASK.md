...
- current blocker token: `phase-291x JoinIR route detector body-local physical owner move pending`
...
- latest known checkpoint: `291x-390` moved route detector condition-scope
  support and its private variable analyzer into a physical `support` owner
  directory
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: move the remaining `support::body_local::{carrier, condition}`
  families out of private `legacy/` storage
...
- next: JoinIR route detector body-local physical owner move
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-390`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector body-local physical owner move
