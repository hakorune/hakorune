...
- current blocker token:
  `MIMAP-106A post-validation-cadence row selection`
...
- resume point: continue Phase 293x after `MIMAP-ROW-CADENCE-001`; the next
  selected blocker is `MIMAP-106A`, post-validation-cadence row selection.
...
- next 293x order:
  1. `MIMAP-106A`: select exactly one next row using the mimalloc row
     validation cadence
