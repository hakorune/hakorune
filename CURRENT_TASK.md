...
- structure-first override is now explicit in `phase-29x`:
  ...
  - current layering read is: `.hako -> Rust -> LLVM` is mostly clean, but the remaining cleanup targets are `LlvmBackendBox` env mirror and `ll_emit_bridge.rs` compare/debug templating residue
