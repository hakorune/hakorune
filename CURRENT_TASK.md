...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  - current active owner is now the runner-owner wave `lang/src/runner/{stage1_cli_env.hako,stage1_cli.hako,launcher.hako}` above the Rust stop-line; `lang/src/mir/builder/MirBuilderBox.hako` is near thin floor after the delegate-provider/finalize/finalize-chain cuts
  ...
  - targeted proof for the current front is `bash tools/hakorune_emit_mir_mainline.sh lang/src/runner/{stage1_cli_env.hako,launcher.hako} <tmp-out>` and `bash tools/dev/phase29ch_program_json_cold_compat_probe.sh`
  - exact next leaf should now only continue if another fresh near-thin-floor runner leaf appears; otherwise keep `launcher.hako` / `stage1_cli.hako` / `build_box.hako` frozen and move the next cleanup wave to a different nearby owner
