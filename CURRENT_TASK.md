...
- Active micro: `default=mir` / help wording / explicit caller pins を landed にして closeout を詰める
- Current blocker: major blocker は解消。explicit keep caller の freeze と closeout pointer 更新だけ残る
- Exact focus: default は外した。proof/debug/compat は explicit keep のまま固定し、route-first は `using_e2e_smoke.sh` までで止める
