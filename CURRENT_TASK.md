...
- resume point: continue Phase 293x after `MIMAP-112A`; the next selected
  blocker is `MIMAP-113A`, segment allocation modeled local-free scalar lane
  closeout guard.
...
- next 293x order:
  1. `MIMAP-113A`: close out the scalar local-free lane through MIMAP-111A with
     a manifest-backed row guard
