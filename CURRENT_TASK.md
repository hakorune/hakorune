...
- the active Rust stop-line now sits in `phase-29cj` on `src/host_providers/mir_builder.rs`; current exact leaf is the source-route authority / output projection split in `src/host_providers/mir_builder/handoff.rs` above `module_to_mir_json(...)`
...
- `phase-29cj` is now the next live Rust slice:
  ...
  - exact current cut is the source-route authority / output projection split in `handoff.rs` above `module_to_mir_json(...)`
- `phase-29ci` outer caller audit is now split into two waves:
  ...
  - wave 1 has started: `stage1_cli_env.hako`, `launcher.hako`, `stage1_cli.hako`, and `MirBuilderBox.hako` now keep direct `BuildBox` / `MirBuilderBox` calls behind same-file tiny helpers, with `MirBuilderBox.hako` now splitting the source-entry compat seam into `MirBuilderSourceCompatBox`, the Program(JSON) entry contract into `BuilderProgramJsonInputContractBox`, and the func-def pre-lowering gate into `BuilderFuncDefsGateBox`, while the public wrapper methods delegate to private/raw or internal leaves instead of keeping those seams inline
