...
- resume point: continue Phase 293x after `MIMAP-103A`; the next selected
  blocker is `MIMAP-104A`, segment allocation modeled ledger release span facts
  route.
...
- next 293x order:
  1. `MIMAP-104A`: add scalar release span facts to the modeled segment
     allocation ledger without opening real free execution
