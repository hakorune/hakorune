...
- latest verified micro ladder (2026-03-18):
  ...
  - current exact leaf slice for `array_getset`: `crates/nyash_kernel/src/plugin/handle_helpers.rs::with_object_from_handle_cached_with_epoch(...)` is the cache seam behind `with_array_box(...)` / `array_get_index_encoded_i64(...)`, so array get/set now trims handle-cache cloning instead of widening Hako routing
  - fresh `kilo_micro_array_getset` recheck after the cache-seam split landed at `ny_aot_ms=45` (`ratio_cycles=0.01`), so the lane is still thickest but the next cut should stay on the Rust substrate cache seam rather than reopening `array_core_box.hako`
