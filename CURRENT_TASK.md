...
- current blocker token: `phase-291x JoinIR route detector direct legacy type export inventory pending`
...
- latest known checkpoint: `291x-354` replaced the
  `loop_route_detection` legacy wildcard export with an explicit
  compatibility list
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: inventory direct legacy type re-exports from
  `crate::mir::loop_route_detection::*`; do not delete modules
...
- next: JoinIR route detector direct legacy type export inventory; do not
  delete modules in the same card
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-354`; detailed landed history lives in phase card
