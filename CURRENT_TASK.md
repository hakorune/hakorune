...
- Active micro: `kilo_micro_substring_concat` owner lock
- Current blocker: mainline blocker ではない。順番を崩して `main kilo` や broad array retune に飛ばないことが stop-line
- Exact focus: fixed order `leaf-proof micro -> micro kilo -> main kilo` を守り、`substring_concat` の owner/hot leaf を bundle/asm で固定し、`array_getset` は recheck lane に置く
