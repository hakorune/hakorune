...
2. `docs/development/current/main/phases/phase-296x/296x-683-POST-LOCAL-SSA-CALL-RESULT-FALLBACK-COPY-POLICY-OWNER-REFRESH-REPEAT-001.md`
...
- active work has moved from boot-amortized exact-kernel selection through
  ...
  `POST-LOCAL-SSA-CALL-RESULT-FALLBACK-COPY-POLICY-OWNER-REFRESH-REPEAT-001`; exact resident kernels no
  ...
  post guard surface; 296x-681 removed the selected MIR family, and the next
  step is repeat owner refresh after 296x-682 measured a remaining large body
  gap with low-confidence next-owner selection
