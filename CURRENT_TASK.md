...
- active lane:
  ...
  - current app/runtime gap read:
    ...
    - CoreBox router follow-up has moved `StringBox.length/len/size`, `StringBox.substring/substr`, `StringBox.concat`, `StringBox.trim`, `StringBox.contains`, one-arg `StringBox.lastIndexOf`, `StringBox.replace`, `StringBox.indexOf/find`, `ArrayBox.length/size/len`, `ArrayBox.push`, `ArrayBox.slice`, `ArrayBox.get`, `MapBox.size`, `MapBox.len`, and `MapBox.has` to the Unified value path; remaining route candidates should stay one method family per slice
    - next router inventory order after `ArrayBox.get`: `ArrayBox.pop`, `ArrayBox.set`, `ArrayBox.remove`, `ArrayBox.insert`, `MapBox.get`, then `MapBox.set`
    - contract-first / owner-first backlog: two-arg `StringBox.lastIndexOf(needle, start_pos)`, `MapBox.length` alias, `MapBox.keys/values`, `MapBox.delete/remove/clear`, MapBox write-return and bad-key normalization, String semantic owner cleanup, alias SSOT cleanup, and Map compat/source cleanup
