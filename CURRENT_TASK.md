...
- resume point: inventory `LoopFeatures` if-phi flag redundancy; do not
...
- next: JoinIR `LoopFeatures` if-phi flag redundancy inventory; do not
  change route behavior
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-344`; detailed landed history lives in phase card
