...
- resume point: continue Phase 293x after `MIMAP-047B`; current blocker is
  `MIMAP-048A`, an inventory / proposal row that fixes the OSVM release
  capability boundary without opening release behavior.
...
- next 293x order:
  1. `MIMAP-048A`: document and guard the OSVM release capability boundary
     while keeping release behavior closed
