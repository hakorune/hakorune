...
50. `phase-133x mainline/compiler resume selection` (active)

## Current Front

- Active lane: `phase-133x mainline/compiler resume selection`
- Active micro: vm cleanup を current から外し、次の compiler/mainline lane を固定する
- Current blocker: vm-family caller-zero は parked debt のまま残るが、mainline 再開の blocker ではない
- Exact focus: `phase-132x` を landed として閉じ、current を compiler/mainline 実装に戻す
