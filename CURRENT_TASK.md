...
- current blocker token: `phase-291x JoinIR LoopFeatures LoopForm constants inventory pending`
...
- latest known checkpoint: `291x-348` removed dead
  `LoopFeatures.break_count` / `continue_count` fields
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: inventory LoopForm extractor constants in `LoopFeatures`; do
...
- next: JoinIR `LoopFeatures` LoopForm constants inventory; do not change
  route behavior
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-348`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR `LoopFeatures` LoopForm constants inventory
