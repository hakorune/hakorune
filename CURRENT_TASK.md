...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed Rust slice: `src/runner/stage1_bridge/program_json_entry/request.rs` now keeps request-local bridge execution behind `ProgramJsonEmitRequest::emit_program_json_v0()`, so `program_json_entry/mod.rs` is closer to pure request-build + exit orchestration
