  ...
  - post-primitive follow-on queue:
    ...
    8. next ready task: resume optimization lane
        ...
        - fixed return order from the current bridge shrink:
          ...
          3. only then reopen the `plan window across phi_merge` metadata-contract phase
...
- current landed substring truth:
  ...
  - boundary `pure-first` consumer is now landed for `substring(...).length()`:
    ...
    - structure lock: loop-carried corridor continuity now lives in `src/mir/string_corridor_phi.rs`; `string_corridor_placement` only maps that neutral continuity to optimization candidates, and the next structure box is to lift that seam into a generic MIR phi query / relation helper before any plan-window carry lands
