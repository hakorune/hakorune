...
38. `phase-121x vm backend retirement gate decision` (active)

## Current Front

- Active lane: `phase-121x vm backend retirement gate decision`
- Active micro: `--backend vm` を public explicit gate のまま残すか internal-only へ狭めるかを blocker ベースで判断する`
- Current blocker: `none`
- Exact focus: `args.rs` / `dispatch.rs` / `route_orchestrator.rs` / selfhost compat/proof docs から、gate decision の blocker を current docs に固定する`
