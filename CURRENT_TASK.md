...
- current blocker token:
  `MIMAP-135A post-local-free-reuse-ledger-release row selection`
...
- resume point: continue Phase 293x with `MIMAP-135A`, the planning-only row
  after the modeled local-free reuse ledger release route landed.
...
- next 293x order:
  1. `MIMAP-135A`: select exactly one next allocator or compiler row after
     the local-free reuse ledger release route
