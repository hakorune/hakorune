...
39. `phase-122x vm compat route exit plan` (active)

## Current Front

- Active lane: `phase-122x vm compat route exit plan`
- Active micro: `runtime-route compat / stage-a-compat / raw --backend vm の依存をどの順で外すかを具体化する`
- Current blocker: `none`
- Exact focus: `run.sh` / `selfhost_run_routes.sh` / `stage1_bridge/direct_route` / `route_orchestrator.rs` の compat exit order を source-backed に固定する`
...
- `phase-122x` turns that gate decision into a compat-route exit order: shell surface first, bridge/direct route second, backend gate last.
