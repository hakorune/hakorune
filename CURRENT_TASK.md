...
- pure `.hako-only hakorune build` Rust stop-line / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed Rust slices now keep Program(JSON) `user_box_decls` parse-entry plus explicit-vs-compat precedence behind `Stage1UserBoxDeclHandoff::resolve_user_box_decls()`, keep module metadata splice inside `Stage1ProgramJsonModuleHandoff::into_module_with_user_box_decls()`, keep the metadata-map seam behind `Stage1UserBoxDecls::into_metadata_map()`, replace the raw decl `serde_json::Value` seam with typed `Stage1UserBoxDecl`, retire the free source/program entry wrappers behind `Stage1ProgramJsonModuleHandoff::parse(...)`, `Stage1ProgramJsonInput::into_module_handoff()`, and `SourceProgramJsonHandoff::for_source(...)`, and pull Program(JSON) value parse for decl resolution into `Stage1ProgramJsonInput::{resolve_user_box_decls,parse_user_box_decl_handoff,parse_value}`
  ...
  - exact next leaf stays in `src/host_providers/mir_builder.rs`: the remaining owner-local value seam is now the split between `Stage1ProgramJsonInput::parse_user_box_decl_handoff()` / `parse_value()` and `Stage1UserBoxDeclHandoff::from_program_value(...)` above `module_to_mir_json(...)`
