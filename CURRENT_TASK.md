...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed `.hako` slices move the delegate gate/provider call behind `lang/src/mir/builder/internal/delegate_provider_box.hako::BuilderDelegateProviderBox.try_emit(...)`, the full delegate-side finalize behind `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.finalize_mir(...)`, the shared outer finalize chain behind `lang/src/mir/builder/internal/finalize_chain_box.hako::BuilderFinalizeChainBox.apply(...)`, the shared non-empty Program(JSON) text guard in `stage1_cli_env.hako` behind `Stage1ProgramJsonTextGuardBox.{coerce_text_checked,input_present}(...)`, and the raw/subcmd Program(JSON) marker predicates in `stage1_cli.hako` behind `_program_json_text_present(...)` / `_program_json_has_markers(...)`
  - targeted proof for the current front is `bash tools/hakorune_emit_mir_mainline.sh lang/src/runner/{stage1_cli_env.hako,stage1_cli.hako} <tmp-out>` and `bash tools/dev/phase29ch_program_json_cold_compat_probe.sh`
  - exact next leaf is the mirrored Program(JSON) marker helper in `lang/src/runner/launcher.hako` (`_coerce_program_json_output_checked(...)`, `_coerce_program_json_for_emit_mir_checked(...)`); after that, re-check whether runner owners still expose another exact disappearing leaf before reopening any thin floor
