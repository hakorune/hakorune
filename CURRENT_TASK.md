...
- current blocker token: `phase-291x normalized-shadow legacy physical storage move pending`
...
- latest known checkpoint: `291x-402` inventoried remaining normalized-shadow
  legacy storage and selected moving the implementation into `entry::if_only`
  as the next BoxShape seam
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: move `normalized_shadow/legacy/mod.rs` implementation into
  `entry/if_only.rs` and remove `pub mod legacy`
...
- next: normalized-shadow legacy physical storage move
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-402`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: normalized-shadow legacy physical storage move
