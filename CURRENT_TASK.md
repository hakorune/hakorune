  ...
  1. thin backend boundary hardening
     ...
     - role:
       ...
       - fixed order:
         ...
         - exact next slice: widen boundary-owned compile coverage beyond `ret_const` / `Global.print` / `StringBox.length` into the next method-shaped seed, starting with the smallest `RuntimeDataBox` / method-loop pack that can be locked by a dedicated v1 fixture
     - acceptance:
