...
- latest known checkpoint: `291x-319` inventoried the Case-A synthetic
  update-summary seam where carrier names were treated as update-kind proof
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: prune Case-A name-only synthetic update-summary; do not mix
  with broader carrier-count heuristic cleanup
...
- next: JoinIR Case-A update-summary name-only prune; do not mix with
  broader carrier-count heuristic cleanup
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-319`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR Case-A update-summary name-only prune
