...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed `.hako` slices move the delegate gate/provider call behind `lang/src/mir/builder/internal/delegate_provider_box.hako::BuilderDelegateProviderBox.try_emit(...)` and the delegate-side `user_box_decls` finalize behind `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.inject_user_box_decls(...)`, leaving `MirBuilderBox.hako` with route sequencing plus the shared outer finalize chain
  ...
  - exact next leaf stays in `lang/src/mir/builder/MirBuilderBox.hako` only if another exact disappearing leaf appears in the shared outer finalize chain (`_finalize_delegate_mir(...)` / `_norm_if_apply(...)`); otherwise the next wave should move to runner owners `lang/src/runner/{stage1_cli_env.hako,stage1_cli.hako,launcher.hako}`
