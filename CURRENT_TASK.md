...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf is now inside shared producer `lang/src/compiler/build/build_box.hako`: `_collect_bundle_inputs(...)` still mixes plain `bundles` validation and `require_mods` validation with the remaining options body, so the next safe same-owner slice is the same helper-local validation/assign split only if it stays proof-cheap
