...
- pure `.hako-only hakorune build` Rust stop-line / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed Rust slice now keeps Program(JSON) `user_box_decls` parse-entry plus explicit-vs-compat precedence behind `Stage1UserBoxDeclHandoff::resolve_user_box_decls()`, so `stage1_program_json_module_handoff(...)` now reads as `parse_program_json_module(...)` plus a same-owner decl handoff instead of mixing parse+policy inline
  - targeted proof for the current Rust slice is `cargo test mir_builder -- --nocapture`, `cargo test test_program_json_to_mir_json_with_user_box_decls_keeps_explicit_route_contract -- --nocapture`, `cargo test test_program_json_to_mir_json_with_user_box_decls_prefers_explicit_payload -- --nocapture`, and `cargo test program_json_to_mir_file -- --nocapture`
  - exact next leaf stays in `src/host_providers/mir_builder.rs`: the remaining module handoff/finalize pair around `Stage1ProgramJsonModuleHandoff::emit_mir_json(...)` and `with_stage1_user_box_decls(...)` is the next place to thin before revisiting any bridge / surrogate / shell-helper owner
