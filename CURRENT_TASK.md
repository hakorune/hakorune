...
- Current blocker: `stage1_bridge/direct_route/mod.rs` が binary-only direct route の explicit legacy vm gate を保つ
- Exact focus: `phase29x` compat smokes は route-first contract で green; default `stage1_cli_env.hako` child path は backend-hint free にしたので、次は legacy override entry と direct-route gate の残し方を確認する
