...
- active semantic-structure follow-on:
  ...
  - phase-166x is active; the semantic refresh owner cut and generic `value_origin` owner are landed, and the next order is `generic phi_relation owner -> compat semantic recovery quarantine -> only then boundary/lifecycle extraction decision`
