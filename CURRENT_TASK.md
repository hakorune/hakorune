...
- Active front: split kernel 上の `kilo_kernel_small_hk` を再ベースラインして next hot leaf を pin する
- Current blocker: string const-path と array string-store path のどちらを次の exact leaf に固定するか
- Exact focus: `concat_const_suffix_fallback(...)` を first leaf、`array_string_store_handle_at(...)` を second leaf として bundle/asm を読み直す
