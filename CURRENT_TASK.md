...
- current blocker token: `phase-291x normalized-shadow route-local function-name cleanup pending`
...
- latest known checkpoint: `291x-414` inventoried normalized-shadow route-local
  function-name literals before local vocabulary cleanup
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: move normalized-shadow route-local `join_k`/`k_then`/`k_else`/
  `loop_cond_check` names behind a normalized-shadow-local vocabulary
...
- next: normalized-shadow route-local function-name cleanup
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-414`; detailed landed history lives in phase card
