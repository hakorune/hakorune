...
- Current blocker:
  - no collection/runtime blocker remains, and the smoke runner now has a first-class suite manifest contract; inventory is suite-aware, the first twelve semantic splits have landed, `phase29ck_boundary` now lives under `integration/phase29ck_boundary/{entry,string,runtime_data}/`, `vm_hako_caps` now lives under `integration/vm_hako_caps/{app1,args,compare,env,file,gate,lib,mapbox,misc,open_handle_phi,select_emit}/`, and the first six `phase29cc_wsm` splits (`g3_canvas`, `g2_browser`, `g4`, `p10`, `p5`, and `p6`) now live under `integration/phase29cc_wsm/{g3_canvas,g2_browser,g4,p10,p5,p6}/`, so the next blocker is the next live family split under `integration/apps` (`phase29cc_wsm/p7` next)
  - active phase is now `phase-29cq`: suite-manifest first + suite-aware inventory landed, `phase29ck_boundary`, `vm_hako_caps`, and the first four `phase29cc_wsm` subfamilies (`g3_canvas`, `g2_browser`, `g4`, `p10`) have been split into semantic subtrees, and the next slice continues semantic split of `integration/apps` (`--profile` compat keep, `--suite` opt-in, no mass move)
...
- Later cleanup (not this slice):
  ...
  - smoke hygiene: first future split families have landed at `tools/smokes/v2/profiles/integration/rc_gc_alignment/`, `tools/smokes/v2/profiles/integration/json/`, `tools/smokes/v2/profiles/integration/mir_shape/`, and `tools/smokes/v2/profiles/integration/ring1_providers/`; `phase29ck_boundary` has now been split into `tools/smokes/v2/profiles/integration/phase29ck_boundary/{entry,string,runtime_data}/`, `vm_hako_caps` has now been split into `tools/smokes/v2/profiles/integration/vm_hako_caps/{app1,args,compare,env,file,gate,lib,mapbox,misc,open_handle_phi,select_emit}/`, and `phase29cc_wsm/{g3_canvas,g2_browser,g4,p10,p5,p6}` now live under `tools/smokes/v2/profiles/integration/phase29cc_wsm/`; continue splitting the remaining active families out of `tools/smokes/v2/profiles/integration/apps/` by domain, with `phase29cc_wsm/p7` next
  - smoke hygiene: inventory now reports suite coverage; use the suite-aware report before semantic path splits
- Next exact files:
  - `tools/smokes/v2/profiles/integration/phase29cc_wsm/p7/README.md`
  - `tools/smokes/v2/suites/integration/phase29cc-wsm-p7.txt`
  - `tools/smokes/v2/profiles/integration/phase29cc_wsm/p7/phase29cc_wsm_p7_common.sh`
...
- Execution checklist:
  ...
  - `[ ]` phase-29cq ongoing split: continue splitting the remaining `phase29cc_wsm` families by semantic domain (next family: `phase29cc_wsm/p7`)
...
- immediate: phase-29cq semantic split of the next live family out of `tools/smokes/v2/profiles/integration/apps/` (`phase29cc_wsm/p7` next)
...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed Rust slices keep Program(JSON) `user_box_decls` parse-entry, module metadata splice, and parsed-value decl shaping behind typed owner-local boxes in `src/host_providers/mir_builder.rs`, so the remaining unavoidable Rust seam is just `module_to_mir_json(...)`
  - latest landed Rust slice: the source/plain/guarded Program(JSON)->MIR routes in `src/host_providers/mir_builder.rs` now flow through `Stage1ProgramJsonInput::{emit_guarded_mir_json,emit_plain_mir_json}` and `SourceProgramJsonHandoff::{emit_guarded_program_and_mir_json,emit_plain_program_and_mir_json}`, so the Rust authority seam no longer repeats free wrapper emission paths above `module_to_mir_json(...)`
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps `user_box_decls` parse-policy in `Stage1UserBoxDecls::{parse_program_json,from_program_value,...}` and no longer needs the transient `Stage1ProgramJsonValue` owner, so `Stage1ProgramJsonInput` is closer to a pure handoff builder
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` no longer needs the transient `Stage1ProgramJsonInput` owner; guarded/plain Program(JSON) emit now goes straight through `Stage1ProgramJsonModuleHandoff::parse(...)`, `Stage1UserBoxDecls::parse_program_json(...)`, and `SourceProgramJsonHandoff::{emit_guarded_program_and_mir_json,emit_plain_program_and_mir_json}`
  - latest landed `.hako` slices move the delegate gate/provider call behind `lang/src/mir/builder/internal/delegate_provider_box.hako::BuilderDelegateProviderBox.try_emit(...)`, the full delegate-side finalize behind `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.finalize_mir(...)`, the shared outer finalize chain behind `lang/src/mir/builder/internal/finalize_chain_box.hako::BuilderFinalizeChainBox.apply(...)`, the shared non-empty Program(JSON) text guard in `stage1_cli_env.hako` behind `Stage1ProgramJsonTextGuardBox.{coerce_text_checked,input_present}(...)`, the live MIR result-materialize/validate lane there behind `_materialize_mir_text_with_debug(...)` / `_mir_text_head_is_lbrace(...)` / `_mir_text_has_functions(...)`, the raw/subcmd Program(JSON) marker predicates in `stage1_cli.hako` behind `_program_json_text_present(...)` / `_program_json_has_markers(...)`, the checked emit+output tail there behind `_emit_validated_mir_from_program_json_text(...)` / `_mir_output_has_functions(...)`, the mirrored launcher marker predicates in `launcher.hako` behind the same helper pair, the build-exe compile/link tail there behind `_compile_object_from_mir_path_checked(...)` / `_link_exe_object_checked(...)`, the first shared-producer imports inject tail in `build_box.hako` behind `_inject_imports_fragment_if_present(...)`, the bundle ctx container setup there behind `_ensure_bundle_alias_arrays(...)` / `_ensure_require_mods_array(...)`, the bundle opts validation error handoff there behind `_fail_bundle_ctx(...)`, the plain bundle / require-mod validation/assign there behind `_collect_plain_bundle_inputs(...)` / `_collect_require_mod_inputs(...)`, the named-bundle pair validation/assign there behind `_collect_named_bundle_inputs(...)`, the parse-src narrowing there behind `_main_body_parse_src_if_present(...)`, and the prepared-scan-src result/error/apply/resolve handoff there behind `_new_prepare_scan_src_result(...)` / `_fail_prepare_scan_src(...)` / `_apply_prepare_scan_src_result(...)` / `_resolve_prepare_scan_src_if_needed(...)`
  - targeted proof for the current front is `cargo test mir_builder -- --nocapture`, plus `bash tools/hakorune_emit_mir_mainline.sh lang/src/runner/{stage1_cli_env.hako,launcher.hako} <tmp-out>` and `bash tools/dev/phase29ch_program_json_cold_compat_probe.sh`
  - exact next leaf is the remaining same-owner handoff/finalize cleanup inside `src/host_providers/mir_builder.rs`; keep `launcher.hako` / `stage1_cli.hako` / `build_box.hako` / `lang/src/mir/builder/internal/**` frozen unless a new exact leaf appears there first
  - latest landed Rust slice: bridge-local `src/runner/stage1_bridge/program_json/mod.rs` now keeps read->emit->write payload handoff behind `ProgramJsonOutput::{read_from_source_path,write_to_path}`, so the future-retire bridge root is thinner and no longer exposes a free `load_program_json_output(...)` seam
  - latest landed Rust slice: `src/runner/stage1_bridge/program_json_entry/request.rs` now keeps request-local bridge execution behind `ProgramJsonEmitRequest::execute()`, so `program_json_entry/mod.rs` is closer to pure request-build + exit orchestration
  - latest landed Rust slice: `src/runner/stage1_bridge/program_json_entry/request.rs` now also keeps source/out-path resolution on `ProgramJsonEmitRequest::build(...)`, so the bridge-entry facade no longer depends on a separate free request-builder helper
  - latest landed Rust slice: `src/runner/stage1_bridge/program_json_entry/{request.rs,exit.rs}` now pass a typed `ProgramJsonEmitResponse` between execution and process-exit formatting, so `program_json_entry/mod.rs` no longer carries the response-match tail for the bridge-local emit route
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps guarded Program(JSON)->MIR emission on `Stage1ProgramJsonModuleHandoff::emit_guarded_mir_json()` and strict source Program(JSON) emission on `SourceProgramJsonHandoff::emit_strict_program_json(...)`, so the top-level authority helpers no longer repeat those free wrapper leaves above the stop-line
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps source-route MIR-only projection on `SourceProgramJsonHandoff::emit_guarded_mir_json()`, so `source_to_mir_json(...)` no longer has to destructure the broader `(Program(JSON), MIR(JSON))` handoff tuple at top level
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps explicit Program(JSON) module parse and decl parse on `Stage1ProgramJsonModuleHandoff::{parse_module,parse_user_box_decls}(...)`, so the owner entrypoint `parse(...)` reads as a typed handoff builder instead of mixing both inner parse leaves inline
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps Program(JSON) decl policy on `Stage1UserBoxDecls::{parse_program_value,resolve_decls,collect_compat_decl_names}(...)`, and a new unit proof fixes the compat fallback contract when explicit `user_box_decls` is absent
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps explicit decl name/field materialize on `Stage1UserBoxDecl::{parse_name,parse_fields}(...)`, and a new unit proof fixes the contract that non-string explicit field entries are filtered out instead of leaking into metadata
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps compat defs-name collect on `Stage1UserBoxDecls::{insert_compat_def_box_names,compat_def_box_name}(...)`, so the fallback cluster no longer depends on the free `insert_stage1_def_box_names(...)` helper
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps explicit decl payload extraction on `Stage1UserBoxDecls::{explicit_decl_values,collect_explicit_decls}(...)`, so `explicit_from_program_value(...)` is down to a thin owner-local handoff
  - latest landed Rust slice: `src/host_providers/mir_builder.rs` now keeps decl output projection on `Stage1UserBoxDecls::{into_metadata_entries,into_decl_json_values}(...)`, so metadata/test output materialization is separated from decl resolution in the last remaining high-value cluster
  - latest landed Rust slice: bridge-local `src/runner/stage1_bridge/program_json/mod.rs` now keeps source-path/source-text payload build behind `ProgramJsonOutput::{from_source_path,from_source_text}(...)`, so the future-retire bridge root no longer mixes read and payload-emission leaves inline
  - latest landed Rust slice: bridge-local `src/runner/stage1_bridge/program_json/mod.rs` now also keeps writeback orchestration on `ProgramJsonOutput::emit_to_path(...)`, so `emit_program_json_v0(...)` is reduced to a thin `from_source_path -> emit_to_path` route
  - latest landed Rust slice: `src/runner/stage1_bridge/stub_emit/writeback.rs` now keeps optional MIR dump and file-or-stdout output routing behind `print_dumped_mir_if_requested(...)`, `emit_mir_json_output(...)`, and `emit_program_json_output(...)`, so `write(...)` is closer to pure payload dispatch
  - latest landed Rust slice: `src/runner/stage1_bridge/stub_emit.rs` now keeps timeout/nonzero/parse-write orchestration behind `timeout_exit_code(...)`, `failed_exit_code(...)`, and `parse_and_write_payload(...)`, so `handle_child_output(...)` is down to a thin route-local decision chain
  - latest landed Rust slice: `src/runner/stage1_bridge/stub_emit/parse.rs` now keeps line extraction and parse-error formatting behind `first_*_json_line(...)` / `parse_*_json_line(...)`, and new unit proofs fix the exact missing/parse error prefixes for stub emit payload parsing
...
- string coercion contract cleanup / cross-lane drift (2026-03-16):
  ...
  - latest landed slice: the nearby mini lowers under `lang/src/mir/builder/internal/{lower_return_*,lower_typeop_*,lower_method_array_size_box,lower_method_map_size_box,lower_method_array_push_box,lower_loop_local_return_var_box,lower_if_compare*}.hako` now centralize Program(JSON) entry coercion behind owner-local `_coerce_text_compat(...)`, so their raw `\"\" + x` residue is reduced to helper implementations and intentional JSON assembly only
  - latest landed slice: `lang/src/mir/builder/internal/{builder_config_box,delegate_finalize_box}.hako` now also centralize env/program-json text coercion behind owner-local `_coerce_text_compat(...)`, so the nearby route/config owners no longer repeat raw `\"\" + x` on their remaining compat seams
  - latest landed slice: `lang/src/mir/builder/internal/{lower_load_store_local_box,lower_loop_simple_box,lower_method_map_get_set_box,lower_return_binop_varint_box,lower_return_method_string_length_box,runner_min_box}.hako` now also centralize Program(JSON)/env entry coercion behind owner-local `_coerce_text_compat(...)`, leaving only helper implementations and intentional pattern/JSON string assembly on their raw `\"\" + x` lines
  - latest landed slice: `lang/src/mir/builder/internal/{lower_if_then_else_following_return_box,lower_method_array_get_set_box,lower_newbox_constructor_box,lower_return_binop_box,lower_return_binop_varvar_box,lower_return_string_box}.hako` now also centralize Program(JSON) entry coercion behind owner-local `_coerce_text_compat(...)`, so their raw `\"\" + x` residue is likewise reduced to helper implementations and intentional JSON string assembly only
  - latest landed slice: `lang/src/mir/builder/internal/{lower_if_compare_fold_binints_box,lower_if_compare_fold_varint_box,lower_if_merge_local_return_var_box,lower_loop_multi_carrier_box,registry_authority_box}.hako` now centralize Program(JSON), map-bound, param-index, and registry selector coercion behind owner-local compat helpers, leaving only helper implementations plus intentional pattern/MIR JSON assembly on their raw `\"\" + x` residue path
  - latest landed slice: `lang/src/mir/builder/internal/{prog_scan_box,jsonfrag_normalizer_box,lower_loop_sum_bc_box}.hako` now centralize scanner text coercion, normalizer text/env-flag coercion, and loop-sum trace/program-json coercion behind owner-local compat helpers, leaving only helper implementations plus intentional pattern/MIR JSON assembly on the raw `\"\" + x` residue path
  - latest landed slice: `lang/src/mir/builder/internal/{lower_return_method_array_map_box,lower_if_nested_box,lower_loop_count_param_box}.hako` now centralize entry/toggle/object-scan coercion behind owner-local compat helpers, so their remaining raw `\"\" + x` residue is reduced to helper implementations and intentional pattern/MIR JSON assembly only
  ...
  - next fixed order:
    ...
    3. keep `launcher.hako` / `build_box.hako` / `stage1_cli.hako` / `stage1_cli_env.hako` at thin floor and continue the nearby residue audit owner-by-owner inside `lang/src/mir/builder/internal/**`
  - task rule:
...
- pure `.hako-only hakorune build` gap snapshot (2026-03-14):
  ...
  - Rust authority bucket:
    ...
    - latest tightening: explicit Program(JSON) route now prefers root `user_box_decls` payload, maps it into `MirModule.metadata.user_box_decls`, and emits MIR JSON in-memory through `emit_mir_json_string_for_harness_bin(...)`, so Rust no longer reparses emitted MIR JSON or round-trips through temp MIR files on that route
...
- pure `.hako-only hakorune build` blocker map (2026-03-14):
  - primary Rust-owned blockers:
    1. `src/host_providers/mir_builder.rs`
       ...
       - latest tightening: explicit Program(JSON) entry now keeps module parse behind `Stage1ProgramJsonModuleHandoff::parse_module(...)`, while compat box-name collection stays behind `Stage1UserBoxDecls::{insert_compat_def_box_names,compat_def_box_name}(...)`
       - latest tightening: explicit Program(JSON) entry now also keeps `module_to_mir_json(...)` + user-box finalize handoff behind same-file helper `emit_module_mir_json_with_stage1_user_box_decls(...)`
       ...
       - latest tightening: explicit Program(JSON) `user_box_decls` parse-entry and explicit/compat precedence now stay behind `Stage1UserBoxDeclHandoff::resolve_user_box_decls()`, so the remaining active leaf is the module-handoff/finalize pair above `module_to_mir_json(...)`
       - latest tightening: module metadata splice now also stays inside `Stage1ProgramJsonModuleHandoff::into_module_with_user_box_decls()`, and the old free finalize helpers are retired; the remaining active leaf is the metadata-map seam above `module_to_mir_json(...)`
       - latest tightening: the metadata-map seam is now also closed behind `Stage1UserBoxDecls::into_metadata_map()`, so the remaining active leaf is the module-entry seam at `Stage1ProgramJsonModuleHandoff::parse(...)` -> `Stage1ProgramJsonModuleHandoff::parse_module(...)`
       - latest tightening: `Stage1UserBoxDecls` now holds typed `Stage1UserBoxDecl` entries instead of raw `serde_json::Value`, so explicit/compat decl shaping and metadata-map finalize no longer share a JSON-value seam
       - latest tightening: the free module-entry pair is now folded into `Stage1ProgramJsonModuleHandoff::parse(...)` / `Stage1ProgramJsonInput::into_module_handoff()` / `parse_module()`, and the free source-entry wrapper is now folded into `SourceProgramJsonHandoff::for_source(...)`; the remaining active leaf is the same-owner entry split with `Stage1UserBoxDeclHandoff::parse(...)`
       - latest tightening: Program(JSON) value parse for decl resolution now also stays inside `Stage1ProgramJsonInput::{resolve_user_box_decls,parse_user_box_decl_handoff,parse_value}`, so the remaining active leaf is the typed parsed-value handoff at `Stage1UserBoxDeclHandoff::from_program_value(...)`
       - latest tightening: the typed parsed-value handoff is now folded into `Stage1ProgramJsonValue::{parse,resolve_user_box_decls}`, retiring `Stage1UserBoxDeclHandoff`; the remaining active leaf is the thin parsed-value shim at `Stage1ProgramJsonInput::parse_value()`
       - latest tightening: strict-source public/test entries now share same-file Program(JSON) emit plus shared source-pair helper `emit_program_and_mir_json_for_source(...)`, with guarded/plain MIR handoff exposed through `emit_program_and_guarded_mir_json_for_source(...)` and `emit_program_and_plain_mir_json_for_source(...)`; the live stop-line stays unchanged
       ...
       - latest tightening: explicit-route finalize now also keeps `Program(JSON)` parse/build separate from MIR JSON mutation at `finalize_mir_json_with_stage1_user_box_decls(...)` -> `build_stage1_user_box_decls_from_program_json(...)` -> `build_stage1_user_box_decls(...)` -> `build_stage1_user_box_decls_from_names(...)` -> `inject_user_box_decls_into_mir_json(...)`
  ...
  - authority-replacement ladder (freeze this before more local cleanup):
    ...
    2. finish the Rust-owned authority front at `src/host_providers/mir_builder/lowering.rs`
       ...
       - settled answer: `module_to_mir_json(...)` is the unavoidable Rust host seam; do not try to migrate `MirModule` ownership into `.hako`
    3. only then begin the first `.hako` authority replacement wave
       ...
       - latest tightening: `emit_from_source_v0(...)` is now a thin source-entry shim over owner-local helpers `_coerce_source_text_checked(...)`, `_emit_program_json_from_source_checked(...)`, and `_emit_mir_from_source_program_json_checked(...)`; do not mix this compat-seam cleanup with `emit_from_program_json_v0(...)` policy replacement in the same slice
       - latest tightening: `_emit_internal_program_json(...)` now keeps its unsupported fail-fast tail behind `_fail_internal_unsupported(...)` and `_program_json_has_ternary(...)`, so the visible owner-local route order is loop-force / registry / fallback / fail-fast only
...
- selfhost mainline next:
  ...
  - selfhost / de-rust mainline reading order (restart SSOT):
    ...
    6.25. `docs/development/current/main/phases/phase-29cj/README.md`
       - next Rust-owned retirement wave after `phase-29ci` closeout-ready helper collapse
  ...
  - restart quick entry (2026-03-10):
    ...
    - next queued Rust-owned wave after that: `phase-29cj`
    ...
    - current next owner order after closeout-ready: `build surrogate keep` -> `future-retire bridge` (`phase-29cj`)
  ...
  - `phase-29ch` summary lock:
    ...
    - monitor-only explicit compat keep: `stage1-env-mir-program`
      ...
      - latest shell-helper tightening: `tools/hakorune_emit_mir.sh` now also keeps that remaining non-direct route order behind `emit_mir_json_via_non_direct_routes()`, so the script top-level is closer to pure Stage-B -> route handoff orchestration
      ...
      - latest shell-helper tightening: `tools/selfhost/selfhost_build.sh` now keeps the Program(JSON)->MIR step behind `emit_mir_json_from_program_json_v0()`, so `emit_exe_from_program_json_v0()` no longer mixes MIR generation with ny-llvmc EXE emission inline
      - latest shell-helper tightening: `tools/selfhost/selfhost_build.sh` now also keeps the MIR(JSON)->EXE step behind `emit_exe_from_mir_json()`, so `emit_exe_from_program_json_v0()` is closer to pure orchestration
      - latest shell-helper tightening: `tools/selfhost/selfhost_build.sh` now also keeps top-level post-emit route order behind `dispatch_stageb_downstream_outputs()` and temp-json cleanup behind `cleanup_program_json_tmp_if_needed()`, so the script tail no longer mixes `--json / --mir / --exe / --run` branching inline
      - latest shell-helper tightening: `tools/smokes/v2/lib/test_runner.sh` now also keeps the remaining `verify_program_via_builder_to_core()` no-fallback / Rust CLI fallback / success cleanup tail behind `verify_builder_no_fallback_requested()`, `cleanup_verify_builder_logs()`, and `handle_verify_builder_emit_result()`
      - latest shell-helper tightening: `tools/smokes/v2/lib/test_runner.sh` now also keeps built-MIR route leaves behind `run_built_mir_json_via_hv1_route()`, `run_built_mir_json_via_hako_core_route()`, and `run_built_mir_json_via_core_v0_route()`, so `run_built_mir_json_via_verify_routes()` is now a small route table
      - latest shell-helper tightening: `tools/smokes/v2/lib/test_runner.sh` now also keeps the shared phase2044 env stack behind `run_verify_program_via_builder_to_core_with_env()`, so prefer-builder and primary-no-fallback wrappers are now thin flag wrappers
      - latest shell-helper tightening: `tools/smokes/v2/lib/test_runner.sh` now also keeps the shared phase2160 temp-wrapper + vm-launch contract behind `run_program_json_via_builder_module_vm_with_env()`, so builder-min / registry / preinclude / diag wrappers are thin flag wrappers
      - latest shell-helper tightening: `tools/smokes/v2/lib/test_runner.sh` now also keeps Rust CLI fallback file-shape check and MIR-file execution behind `mir_json_file_looks_like_v0_module()` and `run_built_mir_json_file_via_core_v0()`, so `run_program_json_v0_via_rust_cli_builder()` is closer to pure convert -> verify/run -> cleanup orchestration
      - latest caller-tail collapse: the `phase2044/hako_primary_no_fallback_*` bucket now keeps rc assertion and pass/fail formatting behind `run_hako_primary_no_fallback_canary_and_expect_rc()`, so those scripts are thin fixture wrappers instead of repeating the same execution/check contract
      - latest caller-tail collapse: the stable single-case `phase2044/mirbuilder_provider_*` bucket now also keeps rc assertion and pass/fail formatting behind `run_preferred_mirbuilder_canary_and_expect_rc()`, while `array_length_alias` and `array_push_size_rc` remain explicit keeps until their current `rc=1` blocker is resolved
      - latest caller-tail collapse: the stable `phase2160/builder_min_*` bucket now fully keeps skip/tag/functions boilerplate behind `run_builder_module_tag_canary()`, including `builder_min_return_binop_varvar_canary_vm.sh` via its no-functions-check and soft-rc knobs
      - latest caller-tail collapse: the stable `phase2160/registry_optin_*` builder-vm bucket now also keeps plain and preinclude skip/tag/functions boilerplate behind `run_registry_builder_tag_canary()`, including `registry_optin_canary` and `registry_optin_return_binop_varvar_canary_vm.sh`, while diag/direct probes stay explicit keeps
      ...
      - latest runner tightening: `lang/src/runner/stage1_cli.hako` now also keeps raw/subcmd emit-program checked tail behind `_emit_program_json_raw_with_debug(...)`, `_fail_emit_program_json_null(...)`, and `_coerce_program_json_output_checked(...)`, so the future-retire raw lane no longer mixes BuildBox call, null fail-fast, and Program(JSON) validation inline
      - latest runner tightening: `lang/src/runner/launcher.hako` now keeps its `emit mir-json` checked contract behind `_coerce_program_json_for_emit_mir_checked(...)`, `_emit_mir_from_program_json_text_checked(...)`, and `_coerce_mir_output_checked(...)`, so Program(JSON) validation, MirBuilder call, and MIR output validation are separate owner-local steps there too
      - latest runner tightening: `lang/src/runner/launcher.hako` now also keeps its `emit program-json` checked tail behind `_emit_program_json_raw(...)` and `_coerce_program_json_output_checked(...)`, so the launcher lane no longer mixes BuildBox call and Program(JSON) validation inline
      - latest runner tightening: `lang/src/runner/launcher.hako` now also keeps program-json path load and stdout/file output tails behind `_load_program_json_from_path_checked(...)`, `_print_output_checked(...)`, and `_write_output_checked(...)`, so the launcher lane no longer mixes readback/output side effects inline
      - latest shared-producer tightening: `lang/src/compiler/build/build_box.hako` now keeps outer producer sequencing behind `_emit_program_json_from_scan_src(...)`, parse-source narrowing plus parser call behind `_parse_program_json_from_scan_src(...)`, defs-scan plus defs-fragment build behind `_build_defs_fragment_json(...)`, and defs/imports enrichment behind `_inject_stageb_fragments_json(...)`, so `emit_program_json_v0(...)` no longer mixes those responsibilities after `scan_src` preparation
      - latest shared-producer tightening: `lang/src/compiler/build/build_box.hako` now also keeps imports-fragment build behind `_build_imports_fragment_json(...)`, so `_inject_imports_json(...)` reads as fragment build -> inject only
      - latest shared-producer tightening: `lang/src/compiler/build/build_box.hako` now also keeps defs gate/inject tail behind `_defs_scan_enabled(...)` and `_inject_defs_fragment_if_present(...)`, so `_inject_defs_json(...)` reads as gate -> defs build -> inject only
      - latest `MirBuilderBox` tightening: `emit_from_program_json_v0(...)` now keeps Program(JSON) input validation behind `_coerce_program_json_checked(...)` and route dispatch behind `_emit_mir_from_program_json_text_checked(...)`, so the public authority surface no longer mixes header validation inline with internal/delegate route selection
      - latest `MirBuilderBox` tightening: `_emit_internal_program_json(...)` now keeps loop-force / registry / fallback leaves behind `_try_emit_loop_force_jsonfrag(...)`, `_try_emit_registry_program_json(...)`, and `_try_emit_fallback_program_json(...)`, so internal authority reads as a route table instead of mixing route leaves inline
      - latest `MirBuilderBox` tightening: the delegate compat gate/provider call now lives in `lang/src/mir/builder/internal/delegate_provider_box.hako::BuilderDelegateProviderBox.try_emit(...)`, while local user-box-decls finalize still stays in `MirBuilderBox.hako`, so `_emit_delegate_program_json(...)` now reads as internal gate/provider -> local finalize
      - latest `MirBuilderBox` tightening: the full delegate-side finalize now also lives in `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.finalize_mir(...)`, so `_emit_delegate_program_json(...)` now reads as internal gate/provider -> internal finalize -> shared normalize
      - latest `MirBuilderBox` tightening: the shared outer finalize order and its stable fail tags now live in `lang/src/mir/builder/internal/finalize_chain_box.hako::BuilderFinalizeChainBox.{apply,log_fail}(...)`, so route leaves no longer carry inject/methodize/normalize/fail-tag logic inline
      - latest `MirBuilderBox` tightening: the shared finalize chain now keeps defs inject, methodize gate, and jsonfrag normalize gate behind `_inject_func_defs_checked(...)`, `_methodize_if_enabled_checked(...)`, and `_normalize_jsonfrag_if_enabled_checked(...)`, so `_norm_if_apply(...)` reads as a pure finalize order
    ...
    - final owner order inside `phase-29ch`:
      ...
      - future-retire bridge inventory:
        ...
        - `src/runner/stage1_bridge/program_json_entry/mod.rs` is now a thin bridge-local entry facade for `emit-program-json-v0`; `src/runner/stage1_bridge/program_json_entry/request.rs` owns the request predicate used by `src/runner/mod.rs` for `skip_stage1_stub`, source-path precedence (`stage1::input_path()` aliases first, CLI input fallback second), and exact out-path extraction from the explicit CLI flag; the entry module itself now owns the explicit success/error process-exit formatting used by `src/runner/emit.rs`; `src/runner/stage1_bridge/program_json/mod.rs` is a thin facade plus bridge-local read->emit->write orchestration and owner-1 payload emission, while source-text read policy lives in `src/runner/stage1_bridge/program_json/read_input.rs` and bridge-local writeback policy lives in `src/runner/stage1_bridge/program_json/writeback.rs`
    ...
    - details SSOT:
      ...
      - `phase-29ci` P5 Stage-B malformed Program(JSON) producer pin: `docs/development/current/main/phases/phase-29ci/P5-STAGEB-MALFORMED-PROGRAM-JSON.md`
...
- summary:
  ...
  - dust cleanup (2026-03-08, slice 185): current lowering surface から orphan `bool_expr_lowerer` module を外し、condition lowering の active docs/comment を現状の JoinIR lane に同期した
    - synced files: `CURRENT_TASK.md` / `src/mir/join_ir/lowering/{mod,condition_to_joinir}.rs`
    - intent: `bool_expr_lowerer.rs` は current code 参照が `0` で、active lowering surface では `condition_to_joinir` / `condition_lowerer` が SSOT になっている。unused module declaration と stale module prose を除去し、historical references は archive/private/current-task history lane にだけ残す
    - verification: `git diff --check` PASS; `rg -n "bool_expr_lowerer|BoolExprLowerer" src/mir/join_ir/lowering src/mir/join_ir/frontend src/mir/loop_route_detection -g '!**/archive/**'` = `0 hit`; `cargo check --tests` PASS; `cargo build --release --bin hakorune` PASS
  - dust cleanup (2026-03-08, slice 186): low-risk stale route comments と unused legacy stubs を削って、route/dust lane の残骸をもう一段薄くした
...
- live compat contract lane:
  ...
  - instruction-doc cleanup (2026-03-08, slice 183): active instruction docs の exact archive stem / exact legacy fixture basename を current semantic wrapper / semantic fixture alias 優先に寄せた
    - synced files: `docs/development/current/main/phases/{phase-29ao/P15-REGRESSION-PACK-INCLUDE-PATTERN3-INSTRUCTIONS.md,phase-29ao/P23-STRICT-ADOPT-PATTERN3-IFPHI-FROM-FACTS-INSTRUCTIONS.md,phase-29ao/P25-STRICT-ADOPT-PATTERN5-INFINITE-EARLY-EXIT-FROM-FACTS-INSTRUCTIONS.md,phase-286/P2.6.1-INSTRUCTIONS.md,phase-29ai/P11-PLANNER-PATTERN2-BREAK-SUBSET-WIRE-INSTRUCTIONS.md,phase-29ai/P15-OBSERVE-PATTERN2-PROMOTION_HINT-INSTRUCTIONS.md,phase-29aj/P0-PLANNER-OUTCOME-SSOT-INSTRUCTIONS.md,phase-29ah/P0-REGRESSION-PACK-INCLUDES-PHASE263-PATTERN2-INSTRUCTIONS.md}` / `CURRENT_TASK.md`
    - intent: current guidance では `if_phi_join_vm.sh`, `loop_true_early_exit_{strict_shadow,vm}.sh`, `loop_break_{plan_subset,realworld,body_local,body_local_seg}_vm.sh` と `apps/tests/{if_phi_join_min,loop_true_early_exit_min,loop_break_plan_subset_min,loop_break_realworld_min,loop_break_body_local_min,loop_break_body_local_seg_min}.hako` を先頭に置き、`phase118_pattern3_if_sum_vm.sh`, `phase286_pattern5_break_vm.sh`, `phase29ai_pattern2_break_plan_subset_ok_min_vm.sh`, `phase29ab_pattern2_loopbodylocal*_vm.sh`, `phase263_pattern2_seg_realworld_min_vm.sh` は historical replay basename に後退させる
    - verification: `git diff --check` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS
  - instruction-doc cleanup (2026-03-08, slice 188): `phase-29af` / `phase-29ao` の残り active instruction docs を current semantic wrapper / semantic fixture alias / current route path 優先に揃えた
    - synced files: `docs/development/current/main/phases/{phase-29af/P2-JOINIR-REGRESSION-PACK-ENTRYPOINT-INSTRUCTIONS.md,phase-29ao/P24-STRICT-ADOPT-PATTERN7-SPLITSCAN-FROM-FACTS-INSTRUCTIONS.md,phase-29ao/P27-STRICT-ADOPT-PATTERN6-SCANWITHINIT-SUBSET-FROM-FACTS-INSTRUCTIONS.md,phase-29ao/P28-SHADOW-ADOPT-OBSERVABILITY-TAGS-AND-GATE-SMOKES-INSTRUCTIONS.md,phase-29ao/P37-RELEASE-ADOPT-PATTERN6-SCANWITHINIT-SUBSET-INSTRUCTIONS.md,phase-29ao/P38-RELEASE-ADOPT-PATTERN7-SPLITSCAN-SUBSET-INSTRUCTIONS.md,phase-29ao/P39-RELEASE-ADOPT-PATTERN2-BREAK-SUBSET-INSTRUCTIONS.md,phase-29ao/P41-RELEASE-ADOPT-PATTERN5-INFINITE-EARLY-EXIT-SUBSET-INSTRUCTIONS.md}` / `CURRENT_TASK.md`
    - intent: active instruction docs の current lane では `scan_with_init_*` / `split_scan_*` / `loop_break_release_adopt_vm.sh` / `loop_true_early_exit_release_adopt_vm.sh` と `route_entry/router.rs` を前面に置き、`phase29ao_pattern{2,5,6,7}_*.sh` や `phase29ab_pattern{6,7}_*.hako` は historical compat stem / historical fixture pin token に後退させる
    - verification: `git diff --check` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS
  - instruction-doc cleanup (2026-03-08, slice 189): `phase-29ai/P8/P9/P12` と `phase-29ao/P12/P32/P33/P37/P38` を semantic route name first に揃え、`Pattern6/7/2` は historical label / historical token / historical debug tag としてだけ残る形に整理した
    - synced files: `docs/development/current/main/phases/{phase-29ai/P8-WIRE-PLANNER-INTO-SINGLE_PLANNER-PATTERN6-INSTRUCTIONS.md,phase-29ai/P9-PLANNER-PATTERN7-SPLITSCAN-WIRE-INSTRUCTIONS.md,phase-29ai/P12-FACTS-PATTERN2-LOOPBODYLOCAL-PROMOTION-INSTRUCTIONS.md,phase-29ao/P12-VALUEJOIN-FIRST-REAL-USAGE-PATTERN7-SPLITSCAN-INSTRUCTIONS.md,phase-29ao/P32-STRICT-ADOPT-PATTERN2-REALWORLD-FROM-FACTS-INSTRUCTIONS.md,phase-29ao/P33-PLANNER-DERIVE-PATTERN2-LOOPBODYLOCAL-SMOKES-INSTRUCTIONS.md,phase-29ao/P37-RELEASE-ADOPT-PATTERN6-SCANWITHINIT-SUBSET-INSTRUCTIONS.md,phase-29ao/P38-RELEASE-ADOPT-PATTERN7-SPLITSCAN-SUBSET-INSTRUCTIONS.md}` / `CURRENT_TASK.md`
    - intent: active instruction docs では `ScanWithInit` / `SplitScan` / `LoopBreak` / `LoopBreakBodyLocal` と current semantic fixture alias を先頭に置き、`phase29ab_pattern{2,6,7}_*` / `phase263_pattern2_*` / `[coreplan/shadow_adopt:pattern2_break_subset]` は historical fixture token / historical replay token / historical debug tag として読む形に寄せる
    - verification: `git diff --check` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS
  - instruction-doc cleanup (2026-03-08, slice 190): `phase-29ai/P11` と `phase-29ao/P26/P28` の current-looking compat family phrasing を historical payload/family/debug token 表現に寄せた
    - synced files: `docs/development/current/main/phases/{phase-29ai/P11-PLANNER-PATTERN2-BREAK-SUBSET-WIRE-INSTRUCTIONS.md,phase-29ao/P26-STRICT-ADOPT-PATTERN2-BREAK-SUBSET-FROM-FACTS-INSTRUCTIONS.md,phase-29ao/P28-SHADOW-ADOPT-OBSERVABILITY-TAGS-AND-GATE-SMOKES-INSTRUCTIONS.md}` / `CURRENT_TASK.md`
    - intent: active instruction docs では `LoopBreak subset` / `ScanWithInit` / `SplitScan` を current 主語にし、`DomainPlan::Pattern2Break`, `phase29ab_pattern{2,6,7}_*`, `[coreplan/shadow_adopt:pattern{6,7}_*]` は historical payload / historical fixture family / historical debug tag としてのみ扱う
    - verification: `git diff --check` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS
- code-side residue:
...
- phase/docs residue snapshot:
  ...
  - compat-token cleanup (2026-03-08, slice 192): selfhost filter examples と migration instruction docs の exact legacy stem を semantic alias / generic placeholder first に寄せた
    ...
    - verification: `git diff --check` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS
  - instruction-doc cleanup (2026-03-08, slice 193): `phase-29ai/P11/P12/P15` と `phase-29ao/P32/P33` の exact compat basename を inventory/retirement SSOT pointer に圧縮し、current semantic wrapper / semantic fixture alias を本文の先頭に固定した
    - synced files: `docs/development/current/main/phases/{phase-29ai/P11-PLANNER-PATTERN2-BREAK-SUBSET-WIRE-INSTRUCTIONS.md,phase-29ai/P12-FACTS-PATTERN2-LOOPBODYLOCAL-PROMOTION-INSTRUCTIONS.md,phase-29ai/P15-OBSERVE-PATTERN2-PROMOTION_HINT-INSTRUCTIONS.md,phase-29ao/P32-STRICT-ADOPT-PATTERN2-REALWORLD-FROM-FACTS-INSTRUCTIONS.md,phase-29ao/P33-PLANNER-DERIVE-PATTERN2-LOOPBODYLOCAL-SMOKES-INSTRUCTIONS.md}` / `CURRENT_TASK.md`
    - intent: active instruction docs では `loop_break_{plan_subset,realworld,body_local,body_local_seg}_vm.sh` と `apps/tests/loop_break_{plan_subset,realworld,body_local_min,body_local_seg_min}.hako` を current lane として固定し、exact old basename は `joinir-legacy-fixture-pin-inventory-ssot.md` / `joinir-smoke-legacy-stem-retirement-ssot.md` の pointer 経由だけで読む形へ寄せる
    - verification: `git diff --check` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS
  - compat-lane cleanup (2026-03-08, slice 194): archive replay / selfhost filter retire 条件を SSOT に寄せ、active docs の exact compat basename は pointer-first に整理した
  ...
  - semantic fixture/cases cleanup (2026-03-08, slice 198): current semantic wrappers now use semantic fixture aliases (`loop_simple_while_{strict_shadow,subset_reject_extra_stmt}_min.hako`) and semantic planner case lists (`{loop_break,core_loop_routes,if_phi_join,bool_predicate_accum}_planner_required_cases.tsv`); old basenames stay in compat/historical lanes only
    - synced files: `apps/tests/{loop_simple_while_strict_shadow_min,loop_simple_while_subset_reject_extra_stmt_min}.hako` / `tools/smokes/v2/profiles/integration/joinir/{loop_simple_while_strict_shadow_vm,loop_simple_while_subset_reject_extra_stmt_vm,loop_break_planner_required_pack_vm,core_loop_routes_planner_required_pack_vm,if_phi_join_planner_required_pack_vm,bool_predicate_accum_planner_required_pack_vm,{loop_break,core_loop_routes,if_phi_join,bool_predicate_accum}_planner_required_cases}.tsv` / `docs/development/current/main/design/{joinir-smoke-legacy-stem-retirement-ssot,joinir-legacy-fixture-pin-inventory-ssot,coreplan-shadow-adopt-tag-coverage-ssot}.md` / `docs/development/current/main/phases/phase-29ao/{P19-REGRESSION-PACK-ADD-PATTERN1-STRICT-SHADOW-INSTRUCTIONS,P21-PATTERN1-SUBSET-BODY-IS-STEP-ONLY-INSTRUCTIONS,P35-SHADOW-ADOPT-TAG-COVERAGE-SSOT-AND-PATTERN1-NEGATIVE-GATE-INSTRUCTIONS,P36-RELEASE-ADOPT-PATTERN1-COREPLAN-SKELETON-PILOT-INSTRUCTIONS}.md` / `docs/development/current/main/phases/{phase-29ae,phase-29bq}/README.md` / `CURRENT_TASK.md`
    - verification: `git diff --check` = PASS / `cargo check --tests` = PASS / `bash tools/smokes/v2/profiles/integration/joinir/{loop_simple_while_strict_shadow_vm,loop_simple_while_subset_reject_extra_stmt_vm,loop_break_planner_required_pack_vm,core_loop_routes_planner_required_pack_vm,if_phi_join_planner_required_pack_vm,bool_predicate_accum_planner_required_pack_vm}.sh` = PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` = PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` = PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - compat-contract cleanup (2026-03-08, slice 199): archive replay lane と selfhost filter の現行ルールを pointer-first に固定した。current docs/how-to は semantic wrapper / semantic fixture alias / semantic route substring を先頭に置き、exact archived basename や exact historical filter example は inventory/retirement SSOT に閉じる
    - synced files: `docs/development/current/main/design/{joinir-smoke-legacy-stem-retirement-ssot,joinir-legacy-fixture-pin-inventory-ssot}.md` / `CURRENT_TASK.md`
    - verification: `git diff --check` = PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` = PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` = PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
    ...
    - verification: `git diff --check` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS
  - compat-doc cleanup (2026-03-08, slice 200): active instruction docs から exact archived replay basename / exact historical fixture pin token をさらに引っ込め、pointer-first に整理した
    - synced files: `docs/development/current/main/phases/{phase-29ao/P15-REGRESSION-PACK-INCLUDE-PATTERN3-INSTRUCTIONS.md,phase-29ao/P25-STRICT-ADOPT-PATTERN5-INFINITE-EARLY-EXIT-FROM-FACTS-INSTRUCTIONS.md,phase-29ah/P0-REGRESSION-PACK-INCLUDES-PHASE263-PATTERN2-INSTRUCTIONS.md}` / `CURRENT_TASK.md`
    - intent: active instruction lane は current semantic wrapper / current semantic fixture alias を先頭に置き、exact archived basename と exact historical fixture pin token は retirement/pin inventory SSOT だけに保持させる
    - verification: `git diff --check` PASS / `rg -n "phase118_pattern3_if_sum_vm\\.sh|phase286_pattern5_break_vm\\.sh|phase263_pattern2_seg_realworld_min_vm\\.sh|phase286_pattern5_break_min\\.hako" docs/development/current/main/phases/{phase-29ao,phase-29ah} -g '*.md'` = expected historical-pointer hits only / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - compat-authority cleanup (2026-03-08, slice 201): archive-backed 6本の keep/demotion authority を retirement SSOT に一本化し、coverage docs は retire authority を持たないと明記した
    - synced files: `docs/development/current/main/{design/{joinir-smoke-legacy-stem-retirement-ssot,coreplan-shadow-adopt-tag-coverage-ssot}.md,phases/phase-29ae/README.md}` / `CURRENT_TASK.md`
    - intent: `loop_break_{plan_subset,realworld,body_local,body_local_seg}_vm.sh` / `if_phi_join_vm.sh` / `loop_true_early_exit_vm.sh` の最終判定は caller grep や coverage表ではなく、runtime caller owner + doc owner + archive replay owner の3面で読む形へ固定する
    - verification: `git diff --check` PASS / `rg -n "Archive-backed six-route keep authority|Retire authority note|keep authority / demotion readiness" docs/development/current/main/{design/{joinir-smoke-legacy-stem-retirement-ssot,coreplan-shadow-adopt-tag-coverage-ssot}.md,phases/phase-29ae/README.md}` = expected hits only / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - historical-instruction cleanup (2026-03-08, slice 202): `phase-29aj` / `phase-29an` の planner-first / Skeleton-Feature instruction docs を semantic route family first に揃え、`PatternN / patternN_` は historical label / historical helper token / historical replay lane としてだけ読む形へ整理した
    - synced files: `docs/development/current/main/phases/{phase-29aj/{README.md,P0-PLANNER-OUTCOME-SSOT-INSTRUCTIONS.md,P1-REMOVE-LEGACY-RULES-INSTRUCTIONS.md,P2-CHOSEN_RULE-REMOVE-PATTERN1-PLANNER-FIRST-INSTRUCTIONS.md,P3-PATTERN3-IFPHI-PLANNER-FIRST-INSTRUCTIONS.md,P4-PATTERN4-CONTINUE-PLANNER-FIRST-INSTRUCTIONS.md,P5-PATTERN5-INFINITE-EARLY-EXIT-PLANNER-FIRST-INSTRUCTIONS.md,P7-PATTERN8-BOOLPREDICATE-PLANNER-FIRST-INSTRUCTIONS.md,P8-PATTERN9-ACCUM-CONST-PLANNER-FIRST-INSTRUCTIONS.md,P9-ISOLATE-PHASE286-PATTERN9-LEGACY-INSTRUCTIONS.md,P10-SINGLE_PLANNER-UNIFY-PLANNER_FIRST-SHAPE-INSTRUCTIONS.md},phase-29an/{README.md,P0-SKELETONFACTS-SSOT-INSTRUCTIONS.md,P1-FEATUREFACTS-SSOT-INSTRUCTIONS.md,P2-PLANNER-SKELETON-FEATURE-STAGING-INSTRUCTIONS.md,P4-LOOPFACTS-REQUIRE-SKELETON-FEATURES-INSTRUCTIONS.md,P8-EXITUSAGE-INVARIANTS-INSTRUCTIONS.md}}` / `CURRENT_TASK.md`
    - intent: current docs では `loop_simple_while` / `if_phi_join` / `loop_continue_only` / `loop_true_early_exit` / `bool_predicate_scan` / `accum_const_loop` と `Skeleton/Feature` を本文主語にし、exact `PatternN` / `patternN_` token は historical numbered label / historical helper token / historical replay pack に後退させる
    - verification: `git diff --check` PASS / `rg -n "Pattern[0-9]|pattern[0-9]_" docs/development/current/main/phases/{phase-29aj,phase-29an} -g '*.md'` = 0 hit / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - instruction-doc cleanup (2026-03-08, slice 203): `phase-29af/29aw/29ao/29ap/29be` の current-looking numbered route / old path / exact compat filter wording を semantic route family / semantic wrapper / inventory pointer first に寄せた
    - synced files: `docs/development/current/main/phases/{phase-29af/{P1-BOUNDARY-HYGIENE-CONTRACT-CHECKS-INSTRUCTIONS.md,P2-JOINIR-REGRESSION-PACK-ENTRYPOINT-INSTRUCTIONS.md},phase-29aw/P0-FLOWBOX-TAG-COVERAGE-MAP-INSTRUCTIONS.md,phase-29ao/{P12-VALUEJOIN-FIRST-REAL-USAGE-PATTERN7-SPLITSCAN-INSTRUCTIONS.md,P14-VALUEJOIN-REAL-USAGE-PATTERN2-BREAK-EXITJOIN-INSTRUCTIONS.md,P19-REGRESSION-PACK-ADD-PATTERN1-STRICT-SHADOW-INSTRUCTIONS.md,P21-PATTERN1-SUBSET-BODY-IS-STEP-ONLY-INSTRUCTIONS.md,P22-DEDUP-PATTERN1-CORELOOP-CONSTRUCTION-INSTRUCTIONS.md,P34-PATTERN2-NEGATIVE-SHADOW-ADOPT-TAG-GATES-INSTRUCTIONS.md,P39-RELEASE-ADOPT-PATTERN2-BREAK-SUBSET-INSTRUCTIONS.md,P41-RELEASE-ADOPT-PATTERN5-INFINITE-EARLY-EXIT-SUBSET-INSTRUCTIONS.md,P44-CORELOOPCOMPOSER-V0-PATTERN1-MINIMAL-COMPOSITION-INSTRUCTIONS.md},phase-29ap/P10-NESTED-MINIMAL-COREPLAN-SUBSET-INSTRUCTIONS.md,phase-29ah/P0-REGRESSION-PACK-INCLUDES-PHASE263-PATTERN2-INSTRUCTIONS.md}` / `CURRENT_TASK.md`
    - intent: active instruction docs では `LoopBreak` / `LoopSimpleWhile` / `LoopTrueEarlyExit` / `SplitScan` / `NestedLoopMinimal` と current semantic wrapper / current semantic fixture alias / current `joinir/route_entry/router.rs` path を前面に置き、`phase29ab_pattern2_*` / `phase1883_*` / `pattern1_*` / `pattern2_break.rs` / `pattern_split_scan.rs` は historical replay token / historical implementation file token / retirement inventory pointer に後退させる
    - verification: `git diff --check` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - instruction-doc cleanup (2026-03-08, slice 204): `phase-29ap/29be` の残っていた current-looking `Pattern1/6/9` wording と stale `joinir/patterns/router.rs` path を semantic route family / current `route_entry` path / historical label note に寄せた
    - synced files: `docs/development/current/main/phases/{phase-29ap/{P2-PATTERN1-STDLIB-TO_LOWER-SUBSET-INSTRUCTIONS.md,P3-PATTERN1-STDLIB-JOIN-SUBSET-INSTRUCTIONS.md,P7-PATTERN9-LEGACY-REMOVAL-DECISION-INSTRUCTIONS.md,P9-PATTERN6-NESTED-MINIMAL-MIGRATION-INSTRUCTIONS.md,P11-NESTED-MINIMAL-RELEASE-ADOPT-INSTRUCTIONS.md},phase-29be/{P0-INVENTORY-DOMAINPLAN-FREE-GATE-INSTRUCTIONS.md,P1-REMOVE-DOMAINPLAN-NORMALIZE-IN-COMPOSER-INSTRUCTIONS.md}}` / `CURRENT_TASK.md`
    - intent: active instruction lane では `LoopCharMap` / `LoopArrayJoin` / `NestedLoopMinimal` / `AccumConstLoop` と current `joinir/route_entry/router.rs` path を先頭に置き、`Pattern1/6/9` は historical label としてだけ読む。`Pattern2BreakPlan` のような stale typed-plan wording も `LoopBreak historical typed payload` に薄く寄せる
    - verification: `git diff --check` PASS / `rg -n "Pattern[0-9]|pattern[0-9]_|joinir/patterns/router.rs" docs/development/current/main/phases/phase-29ap docs/development/current/main/phases/phase-29be -g '*.md'` = remaining hits are intentional historical/debug tokens only / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - closeout cleanup (2026-03-08, slice 205): archive replay / selfhost filter の final wording と current route-router comment を同期した
  ...
  - live compat retirement (2026-03-08, slice 212): selfhost subset TSV の exact legacy reason token を low-risk semantic wording に寄せ、inventory SSOT の exact basename 依存も family/pointer 表現へ圧縮した
    - synced files: `CURRENT_TASK.md` / `tools/smokes/v2/profiles/integration/selfhost/planner_required_selfhost_subset.tsv` / `docs/development/current/main/design/joinir-legacy-fixture-pin-inventory-ssot.md`
    - intent: `SMOKES_SELFHOST_FILTER` は `fixture + planner_tag + reason` の live contract のまま維持しつつ、exact legacy token は `fixture` 側に閉じ、`reason` や inventory prose は semantic/family 表現へ寄せる
    - verification: `git diff --check` PASS / `cargo check --tests` PASS / `SMOKES_ENABLE_SELFHOST=1 SMOKES_SELFHOST_FILTER=phase29bq_selfhost_blocker_extract_body_brace_return_min SMOKES_SELFHOST_MAX_CASES=1 RUN_TIMEOUT_SECS=120 bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS
  - live compat retirement (2026-03-08, slice 213): selfhost subset TSV の残っていた exact `promote: phase29bq_*` reason token を 0 にし、broken stem 1件も semantic stem へ修復した
    - synced files: `CURRENT_TASK.md` / `tools/smokes/v2/profiles/integration/selfhost/planner_required_selfhost_subset.tsv`
    - intent: `SMOKES_SELFHOST_FILTER` の exact legacy token は `fixture` 列にだけ残し、`reason` は semantic/selfhost family 表現にそろえる。`selfhost_local_expr_unary_minus_cleanup` の accidental stem break (`unaryus`) もこの slice で修復する
    - verification: `rg -n "promote: phase29bq_" tools/smokes/v2/profiles/integration/selfhost/planner_required_selfhost_subset.tsv` = 0 hit / `git diff --check` PASS / `cargo check --tests` PASS / `SMOKES_ENABLE_SELFHOST=1 SMOKES_SELFHOST_FILTER=phase29bq_selfhost_blocker_return_continue_hetero_simple SMOKES_SELFHOST_MAX_CASES=1 RUN_TIMEOUT_SECS=120 bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS
  - live compat retirement (2026-03-08, slice 214): `route.rs` by-name key の live bucket を `current runtime keep / low-risk retire candidate / historical docs-private caller only / dev-gated compat key` に分類し、retire authority を SSOT に固定した
  ...
  - live compat retirement (2026-03-08, slice 216): dev-gated by-name key の current lane を `nested_if_merge` / `read_quoted` へ昇格し、repo-local current tests/docs を semantic dev key 優先へ移した
    - synced files: `CURRENT_TASK.md` / `src/mir/join_ir/frontend/ast_lowerer/{route.rs,README.md,tests.rs,read_quoted.rs,nested_if.rs}` / `src/tests/phase41_nested_if_merge_test.rs` / `src/mir/join_ir_vm_bridge/tests.rs` / `docs/development/current/main/{design/joinir-frontend-legacy-fixture-key-retirement-ssot.md,phases/phase-29ce/{README.md,P0-LIVE-COMPAT-RETIREMENT-INSTRUCTIONS.md}}`
    - intent: `parse_loop` / `read_quoted_from` は dev-gated compat key へ後退させ、repo-local current caller は `nested_if_merge` / `read_quoted` で統一する。これで live compat token を “current dev key” と “compat key” に分けて読めるようにする
    - verification: `cargo check --tests` PASS / `cargo test --lib nested_if_dev_keys_fail_fast_without_env` PASS / `cargo test --lib read_quoted_dev_keys_fail_fast_without_env` PASS / `NYASH_JOINIR_DEV=1 HAKO_JOINIR_NESTED_IF=1 cargo test --lib phase41_nested_if_merge_path_activation` PASS / `HAKO_JOINIR_READ_QUOTED=1 cargo test --lib test_read_quoted_joinir_to_mir_conversion` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - live compat retirement (2026-03-08, slice 217): dev-gated compat key `read_quoted_from` は route token として caller 0 を確認できたので retire し、`parse_loop` だけを compat 維持に絞った
    - synced files: `CURRENT_TASK.md` / `src/mir/join_ir/frontend/ast_lowerer/route.rs` / `docs/development/current/main/{design/joinir-frontend-legacy-fixture-key-retirement-ssot.md,phases/phase-29ce/{README.md,P0-LIVE-COMPAT-RETIREMENT-INSTRUCTIONS.md}}`
    - intent: `read_quoted_from` の live noise は route token と language/app symbol usage を混同しやすかったため、Program JSON route contract からは外す。actual method symbol `MiniJsonCur.read_quoted_from` / `MiniJsonLoader.read_quoted_from` は別契約のまま維持する
    - verification: `cargo test --lib retired_read_quoted_compat_key_is_rejected` PASS / `cargo check --tests` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - live compat retirement (2026-03-08, slice 218): historical docs-private-only Program JSON key (`jsonparser_skip_ws_*` / `jsonparser_atoi_*` / `jsonparser_parse_number_real`) と dev-gated compat key `parse_loop` は repo-local current caller 0 を確認できたので `route.rs` allowlist から retire し、Program JSON current key を `nested_if_merge` / `read_quoted` に収束させた
    - synced files: `CURRENT_TASK.md` / `src/mir/join_ir/frontend/ast_lowerer/{route.rs,README.md,nested_if.rs,read_quoted.rs}` / `docs/development/current/main/{design/joinir-frontend-legacy-fixture-key-retirement-ssot.md,phases/phase-29ce/{README.md,P0-LIVE-COMPAT-RETIREMENT-INSTRUCTIONS.md}}`
    - intent: `docs/private` の historical replay lane と repo-local current Program JSON route contract を切り離し、selfhost/current compiler の live compat noise を最小化する。`parse_loop` は source function name usage と route token usage を分離し、Program JSON route としては retire する
    - verification: repo-local current caller audit (`jsonparser_skip_ws_*` / `jsonparser_atoi_*` / `jsonparser_parse_number_real` / `parse_loop`) = 0 outside `docs/private` / `cargo check --tests` PASS / `cargo test --lib retired_parse_loop_compat_key_is_rejected` PASS / `cargo test --lib retired_historical_jsonparser_loop_frontend_keys_are_rejected` PASS / `cargo test --lib nested_if_dev_keys_fail_fast_without_env` PASS / `cargo test --lib read_quoted_dev_keys_fail_fast_without_env` PASS / `cargo test --lib retired_read_quoted_compat_key_is_rejected` PASS / `NYASH_JOINIR_DEV=1 HAKO_JOINIR_NESTED_IF=1 cargo test --lib phase41_nested_if_merge_path_activation` PASS / `HAKO_JOINIR_READ_QUOTED=1 cargo test --lib test_read_quoted_joinir_to_mir_conversion` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  ...
  - live compat retirement (2026-03-08, slice 222): selfhost subset TSV と selfhost gate に optional `filter_alias` を追加し、historical fixture basename は `fixture` 列へ残したまま `SMOKES_SELFHOST_FILTER` の semantic substring (`selfhost_cleanup_only_min.hako` / `trim-generic-loop` / `breakfinder-parse-int`) を current contract に昇格した
    - synced files: `CURRENT_TASK.md` / `tools/smokes/v2/profiles/integration/selfhost/{planner_required_selfhost_subset.tsv,phase29bq_selfhost_planner_required_dev_gate_vm.sh,phase29bq_selfhost_stageb_route_parity_smoke_vm.sh}` / `apps/tests/{selfhost_cleanup_only_min.hako,selfhost_trim_generic_loop_min.hako,selfhost_breakfinder_parse_int_min.hako}` / `docs/development/current/main/{design/joinir-legacy-fixture-pin-inventory-ssot.md,phases/phase-29ce/{README.md,P0-LIVE-COMPAT-RETIREMENT-INSTRUCTIONS.md},phase-29bq/{README.md,29bq-91-mirbuilder-migration-progress-checklist.md,29bq-92-parser-handoff-checklist.md,29bq-109-hako-mirbuilder-handler-extraction-backlog.md,29bq-111-hako-mirbuilder-post-m4-lane.md,29bq-113-hako-recipe-first-migration-lane.md,29bq-114-hako-cleanup-integration-prep-lane.md}}`
    - intent: selfhost gate の live filter contract を `fixture + planner_tag + reason + filter_alias` に広げ、current semantic fixture alias を active operations で優先する。historical exact basename は TSV `fixture` 列の pin として維持するので、old exact filter も後方互換で動き続ける
    - verification: `git diff --check` PASS / `bash -n tools/smokes/v2/profiles/integration/selfhost/{phase29bq_selfhost_planner_required_dev_gate_vm.sh,phase29bq_selfhost_stageb_route_parity_smoke_vm.sh}` PASS / `cargo check --tests` PASS / `SMOKES_ENABLE_SELFHOST=1 SMOKES_SELFHOST_FILTER=selfhost_cleanup_only_min.hako SMOKES_SELFHOST_MAX_CASES=1 RUN_TIMEOUT_SECS=120 bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` PASS / `SMOKES_ENABLE_SELFHOST=1 SMOKES_SELFHOST_FILTER=trim-generic-loop SMOKES_SELFHOST_MAX_CASES=1 RUN_TIMEOUT_SECS=120 bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` PASS / `SMOKES_ENABLE_SELFHOST=1 SMOKES_SELFHOST_FILTER=breakfinder-parse-int SMOKES_SELFHOST_MAX_CASES=1 RUN_TIMEOUT_SECS=120 bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` PASS / `SMOKES_ENABLE_SELFHOST=1 SMOKES_SELFHOST_FILTER=phase29bq_selfhost_blocker_breakfinder_parse_int_min SMOKES_SELFHOST_MAX_CASES=1 RUN_TIMEOUT_SECS=120 bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` PASS / `bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_stageb_route_parity_smoke_vm.sh` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - aftercare closeout (2026-03-08, slice 223): `phase-29cd` / `phase-29ce` を accepted に切り替え、active docs の exact compat-wrapper wording は pointer-first に薄くし、low-risk source comments も `route shape` / `legacy shim` 主語へ整理した
  ...
  - live compat retirement (2026-03-09, slice 225): `SMOKES_SELFHOST_FILTER` current contract に fixture-derived semantic alias を追加し、historical fixture basename を維持したまま `parse-map` / `localssa-block-insts-end` のような semantic substring で selfhost subset を切れるようにした
    - synced files: `CURRENT_TASK.md` / `tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` / `docs/development/current/main/{design/joinir-legacy-fixture-pin-inventory-ssot.md,phases/phase-29ce/{README.md,P0-LIVE-COMPAT-RETIREMENT-INSTRUCTIONS.md}}`
    - intent: selfhost filter lane で TSV 全行へ explicit alias をばら撒かず、pinned fixture basename から current semantic alias を導けるようにして operational noise を減らす。exact historical basename は `fixture` pin のまま残す
    - verification: `git diff --check` PASS / `cargo check --tests` PASS / `SMOKES_ENABLE_SELFHOST=1 SMOKES_SELFHOST_FILTER=parse-map SMOKES_SELFHOST_MAX_CASES=1 RUN_TIMEOUT_SECS=120 bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` PASS / `SMOKES_ENABLE_SELFHOST=1 SMOKES_SELFHOST_FILTER=localssa-block-insts-end SMOKES_SELFHOST_MAX_CASES=1 RUN_TIMEOUT_SECS=120 bash tools/smokes/v2/profiles/integration/selfhost/phase29bq_selfhost_planner_required_dev_gate_vm.sh` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS
  - live compat retirement (2026-03-09, slice 226): `route.rs` current accepted Program JSON key set (`test` / `local` / `_read_value_from_pair` / `simple`) を positive test で固定し、`simple` を runtime keep、残り 3 key を fixture/test keep として bucket 分離した
    - synced files: `CURRENT_TASK.md` / `src/mir/join_ir/frontend/ast_lowerer/route.rs` / `docs/development/current/main/design/joinir-frontend-legacy-fixture-key-retirement-ssot.md`
  ...
  - de-rust selfhost follow-up caller bucket refinement (2026-03-09, slice 237): `phase-29cf` の broad `monitor-only keep` を `route authority probe keep` / `done-sync keep` / `current diagnostics keep` / `plugin test keep` に分割し、`route_env_probe.sh` と `phase29x_derust_strict_default_route_vm.sh` の current authority を別 bucket に固定した
    ...
    - verification: `git diff --check` PASS / `cargo check --tests` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - de-rust selfhost route-authority split (2026-03-09, slice 238): `phase-29cf` の `route authority probe keep` を `route observability keep` / `strict-dev priority keep` / `non-strict compat boundary keep` に分解し、3本の役割を exact に固定した
    - synced files: `CURRENT_TASK.md` / `docs/development/current/main/{design/selfhost-bootstrap-route-ssot.md,phases/phase-29cf/{README.md,P0-VM-FALLBACK-AND-BOOTSTRAP-BOUNDARY-INVENTORY.md,29cf-10-vm-fallback-bootstrap-retirement-checklist.md}}`
  ...
  - de-rust Stage2 bootstrap reduction contract (2026-03-09, slice 240): `phase-29cg` の docs-first contract を固定し、raw `target/selfhost/hakorune.stage1_cli emit ...` と `--emit-mir-json ...` は `97` を返す一方、`stage1_contract_exec_mode` は Program(JSON) emit までは通ることを実測で確認した
  ...
  - de-rust Stage2 bootstrap reduction probe (2026-03-09, slice 241): `build_stage1.sh` に `stage1-cli bridge-first bootstrap` path を追加して exact probe した。初回は `stage1_cli_env.hako` build が `Instruction does not dominate all uses!` で ny-llvmc link fail になったため、bridge-first lane の first blocker を helper-heavy `Program(JSON)->MIR` quality として固定した
  - de-rust Stage2 bootstrap reduction probe (2026-03-09, slice 242): `stage1_cli_env.hako` の `_find_matching_pair_inline` を単純化したうえで bridge-first bootstrap を再計測した。exact blocker はいったん `stage1_contract_exec_mode ... emit-mir ...` の `96` fail へ寄って見えたため、この時点では stage1 bridge path の child/module-dispatch return contract を仮説 blocker として固定した
    - implication: helper-def materialization ではなく、まず stage1 bridge path の `MirBuilderBox.emit_from_program_json_v0(...)` route を実測で詰める必要がある
  ...
  - de-rust Stage2 bootstrap reduction bridge unblock (2026-03-09, slice 246): `lang/src/runner/stage1_cli_env.hako` の `emit-mir` fail-fast truth を `mir == null` / `mir == ""` から structural MIR(JSON) validation へ切り替え、`stage1_contract_exec_mode ... emit-mir ...` を green 化した。`HAKO_STAGE1_MODULE_DISPATCH_TRACE=1` では `mir_builder output_bytes=213003` / `output_handle=97` まで返ることを確認した
    - implication: `emit-mir` / child return-path bridge は current blocker ではない。experimental `build_stage1.sh` bridge-first path の blocker は `ny-llvmc` 上の MIR dominance へ戻った
    - verification: `stage1_contract_exec_mode target/selfhost/hakorune.stage1_cli emit-program lang/src/runner/stage1_cli_env.hako ...` -> `defs_len=22`; `STAGE1_CLI_DEBUG=1 HAKO_STAGE1_MODULE_DISPATCH_TRACE=1 stage1_contract_exec_mode ... emit-mir ...` -> `rc=0` + MIR(JSON); `NYASH_BIN=target/selfhost/hakorune.stage1_cli bash tools/selfhost/build_stage1.sh --artifact-kind stage1-cli --out target/selfhost/hakorune.stage1_cli.stage2.bridge --force-rebuild` -> `rc=4`, harness=`Instruction does not dominate all uses!`
  - de-rust Stage2 bootstrap reduction dominance grounding (2026-03-09, slice 247): same `stage1_cli_env.hako` Program(JSON v0) を `--program-json-to-mir` 経由で Rust mainline へ流しても `ny-llvmc` は同じ `Instruction does not dominate all uses!` で fail することを確認した。bridge MIR と direct MIR は byte-identical ではないが、関数集合と block 規模は揃っており、first diff は `Main._build_main_defs_fragment_inline/1` 近傍の call representation (`call` vs `mir_call`) に限られる
    - implication: exact blocker は bridge return-path ではなく、helper-heavy `Program(JSON)->MIR` lowering quality / dominance under `ny-llvmc`
    - verification: `stage1_contract_exec_mode ... emit-mir ...` -> green; `./target/release/hakorune --program-json-to-mir <out> --json-file <prog.json>` -> MIR(JSON) emit green; `NYASH_LLVM_BACKEND=crate bash tools/ny_mir_builder.sh --in <bridge.mir.json> --emit exe ...` -> `rc=4`, `Instruction does not dominate all uses!`; same for direct MIR path
  - de-rust Stage2 bootstrap reduction extern contract grounding (2026-03-10, slice 248): `src/runner/json_v0_bridge/lowering/program.rs` の call-resolution を整理し、`HAKO_MIR_BUILDER_CALL_RESOLVE` は legacy unqualified helper-name upgrade だけを担当、bridge/runtime extern-like names (`env.*`, `nyash.*`) は always-on で `Callee::Extern` に分類する current contract へ固定した
    - implication: `env.console.log` unresolved lane は mainline blocker から外れ、exact blocker は引き続き helper-heavy `Program(JSON)->MIR` dominance / lowering quality under `ny-llvmc`
    - verification: `cargo test --lib maybe_resolve_calls_keeps_env_console_as_extern -- --nocapture` PASS / `cargo test --lib maybe_resolve_calls_keeps_unqualified_names_unresolved_when_toggle_is_off -- --nocapture` PASS
  - de-rust Stage2 bootstrap reduction extern arity grounding (2026-03-10, slice 249): arity-suffixed extern-like dotted names (`env.console.log/1`, `env.get/1`, `nyash.console.log/1`) も current bridge/runtime contract では base-name `Callee::Extern` に正規化するように固定した。release direct strict probe では `env.console.log=Extern(54)`, `env.get=Extern(38)`, `Global=0` を確認した
    - implication: `env.* / nyash.*` callee classification は bridge-first blocker から外れ、next exact blocker は `llvm_py` の join-value / PHI wiring (`Main._build_main_defs_fragment_inline/1` first) に狭まった
    - verification: `cargo test --lib maybe_resolve_calls_normalizes_arity_suffixed_env_console_as_extern -- --nocapture` PASS / `cargo test --lib maybe_resolve_calls_normalizes_arity_suffixed_env_get_as_extern -- --nocapture` PASS / `cargo test --lib maybe_resolve_calls_normalizes_arity_suffixed_nyash_console_as_extern -- --nocapture` PASS / direct strict MIR count `{('env.console.log','Global'):0, ('env.console.log','Extern'):54, ('env.get','Global'):0, ('env.get','Extern'):38}`
  - de-rust Stage2 bootstrap reduction dominance owner narrowing (2026-03-10, slice 250): worker inventory と direct failure evidence を付き合わせ、 exact failing family を `Main._build_main_defs_fragment_inline/1` の join-value / PHI wiring に固定した。MIR では predecessor copy -> join use が保たれているのに、LL では PHI placeholder があるまま downstream use が predecessor-local SSA value を参照し、`Instruction does not dominate all uses!` を起こす
    - next exact owners: `src/llvm_py/builders/function_lower.py`, `src/llvm_py/builders/block_lower.py`, `src/llvm_py/phi_wiring/tagging.py`, `src/llvm_py/utils/values.py`, `src/llvm_py/instructions/binop.py`
  - de-rust Stage2 bootstrap reduction PHI/local-arg hardening (2026-03-10, slice 251): cross-block PHI/global-vmap reuse を strict 化し、`mir_call/arg_resolver.py` と `instructions/call.py` の raw `vmap.get(...)` bypass を除去した。`tools/dev/phase29cg_stage2_bootstrap_phi_verify.sh` を追加し、`stage1_cli_env.hako` の `emit-program -> emit-mir -> --program-json-to-mir -> opt -passes=verify` を `verify_rc=0` で固定した
    - synced files: `CURRENT_TASK.md` / `src/llvm_py/{builders/block_lower.py,instructions/{stringbox.py,call.py,mir_call/arg_resolver.py},phi_manager.py,utils/values.py,tests/{test_phi_manager_snapshot_filter.py,test_resolve_i64_strict_scope.py,test_mir_call_arg_resolver_scope.py,test_mir_call_hot_fallback.py}}` / `tools/dev/phase29cg_stage2_bootstrap_phi_verify.sh`
    - verification: `PYTHONPATH=src/llvm_py python3 -m unittest src/llvm_py/tests/test_phi_manager_snapshot_filter.py src/llvm_py/tests/test_resolve_i64_strict_scope.py src/llvm_py/tests/test_mir_call_arg_resolver_scope.py src/llvm_py/tests/test_mir_call_hot_fallback.py` PASS / `tools/dev/phase29cg_stage2_bootstrap_phi_verify.sh` PASS / `cargo check --tests` PASS / `bash tools/smokes/v2/profiles/integration/joinir/phase29bq_fast_gate_vm.sh --only bq` PASS / `tools/dev/direct_loop_progression_sweep.sh --profile phase29x-probe --allow-emit-fail` PASS (`unexpected_emit_fail_count=0`, `route_blocker_count=0`)
  - de-rust Stage2 bootstrap reduction link-closure grounding (2026-03-10, slice 252): `NYASH_BIN=target/selfhost/hakorune.stage1_cli bash tools/selfhost/build_stage1.sh --artifact-kind stage1-cli --out target/selfhost/hakorune.stage1_cli.stage2.bridge --force-rebuild` を再実測し、bridge-first Stage2 build の current blocker が dominance ではなく link-time unresolved symbol closure に移ったことを確認した。`target/selfhost/hakorune.stage1_cli.stage2.o` の undefined set は `env.console.log` を除き `FuncScannerBox.*`, `StageBJsonBuilderBox.*`, `Stage1UsingResolverBox.*`, `BuildBox.emit_program_json_v0`, `MirBuilderBox.emit_from_program_json_v0`, `BoxTypeInspectorBox.*`, `StringHelpers.int_to_str` に集中し、next owner は helper inclusion / symbol closure である
    - verification: `NYASH_BIN=target/selfhost/hakorune.stage1_cli bash tools/selfhost/build_stage1.sh --artifact-kind stage1-cli --out target/selfhost/hakorune.stage1_cli.stage2.bridge --force-rebuild` -> `rc=4`, linker undefined refs / `nm -u target/selfhost/hakorune.stage1_cli.stage2.o | sed 's/^ *U //' | sort -u` bucketized
  - de-rust Stage2 bootstrap reduction entry-only-closure grounding (2026-03-10, slice 253): `stage1_cli_env.hako` の bridge-first MIR を再棚卸しし、`stage1_contract_exec_mode ... emit-program ...` が materialize する `defs_len=22` は entry-local `Main` helper defs のみであること、同じ reduced MIR では `env.console.log`, `FuncScannerBox.*`, `StageBJsonBuilderBox.*`, `Stage1UsingResolverBox.*`, `BuildBox.emit_program_json_v0`, `MirBuilderBox.emit_from_program_json_v0`, `BoxTypeInspectorBox.*`, `StringHelpers.int_to_str` が `Global` のまま残ることを固定した。したがって current blocker は generic `ny-llvmc` dominance ではなく、bridge-first Stage2 object の helper/source closure と selfhost MIR call classification にある
    - verification: `stage1_contract_exec_mode target/selfhost/hakorune.stage1_cli emit-program lang/src/runner/stage1_cli_env.hako ...` -> `defs=22` (`Main` helper defs only) / `stage1_contract_exec_mode ... emit-mir ...` -> `Global env.console.log=54`, `Global FuncScannerBox.find_matching_brace=2`, `Global MirBuilderBox.emit_from_program_json_v0=1`, etc. / `cargo check --tests` PASS / `phase29bq_fast_gate_vm.sh --only bq` PASS
  - de-rust Stage2 bootstrap reduction mixed-stash review (2026-03-10, slice 254): worker-origin mixed diff は `git stash push -u -m wip/phase29cg-worker-mixed-diff-review` で退避し、lane ごとに再棚卸しした。現時点で採用したのは Rust-side の最小 safe-keep 2件だけで、`src/mir/passes/callsite_canonicalize.rs` と `src/runner/json_v0_bridge/lowering/program.rs` に unsuffixed dotted helper `Global` callee を arity-suffixed definition へ canonicalize する補完を追加した
    - deferred: `tools/selfhost/lib/stage1_contract.sh` は path transport 自体は有益だが current blocker authority を FileBox/path lane に寄せすぎるので separate review へ戻した。`lang/src/mir/builder/**` と `src/llvm_py/**` の broad stash lane も scope が広いため未採用
    - verification: `cargo test --lib mcl5_suffixes_existing_global_callee_when_matching_arity_exists -- --nocapture` PASS / `cargo test --lib maybe_resolve_calls_suffixes_unsuffixed_qualified_defs_when_matching_arity_exists -- --nocapture` PASS / `cargo check --tests` PASS
  - de-rust Stage2 bootstrap reduction surrogate-route grounding (2026-03-10, slice 255): kernel-direct route inventory と bridge-first repro を付き合わせ、current exact blocker を `stage1` surrogate helper/source closure に固定した。`lang.compiler.entry.using_resolver_box.resolve_for_source` は `module_string_dispatch.rs` で intentionally empty-prefix stub、`lang.compiler.build.build_box.emit_program_json_v0` は Rust `source_to_program_json_v0(...)` surrogate へ委譲される。したがって bridge-first `stage1_cli_env.hako` の Program(JSON) は `defs_len=22` / `box=Main` only / `imports=[]` のままで、imported helper owners (`FuncScannerBox.*`, `StageBJsonBuilderBox.*`, `Stage1UsingResolverBox.*`, `BuildBox.emit_program_json_v0`, `MirBuilderBox.emit_from_program_json_v0`, `BoxTypeInspectorBox.*`, `StringHelpers.int_to_str`) が Stage2 object に閉じない
    - implication: current next owner is `crates/nyash_kernel/src/plugin/module_string_dispatch.rs` plus `src/stage1/program_json_v0.rs`; bridge return-path, extern classification, and current LLVM PHI wiring are no longer the active blocker
    - verification: `cargo test -p nyash_kernel invoke_by_name_stage1_using_resolver_route_is_stubbed_empty_in_kernel_dispatch -- --nocapture` PASS / `stage1_contract_exec_mode target/selfhost/hakorune.stage1_cli emit-program lang/src/runner/stage1_cli_env.hako ...` -> `defs_len=22`, `box=Main` only, `imports=[]` / `NYASH_BIN=target/selfhost/hakorune.stage1_cli bash tools/selfhost/build_stage1.sh --artifact-kind stage1-cli --out target/selfhost/hakorune.stage1_cli.stage2.bridge.probe --force-rebuild` -> unresolved helper-owner symbols
