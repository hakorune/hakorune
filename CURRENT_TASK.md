...
- resume point: continue Phase 293x with `MIMAP-185A`,
  the post-segment-map-local-free-integration-bridge planning row selected by
  MIMAP-184A.
...
- next 293x order:
  1. `MIMAP-185A`: choose the next narrow row after segment-map local-free
     integration bridge
