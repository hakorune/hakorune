...
- current main goal:
  ...
  - current narrow pilot landed:
    - `lang/src/runtime/kernel/string/search.hako` now owns `find_index` / `contains` / `starts_with` / `ends_with`
  ...
  - next narrow op order for this lane:
    1. `split_once_index` only after prefix/suffix checks are fixed
...
- design-first tasks:
  ...
  7. `substring_concat` pilot と別に、search/control kernel wave は `.hako` string kernel op set v0 として narrow に切り出し、黙って widen しない
     - next narrow op: `split_once_index`
- acceptance:
