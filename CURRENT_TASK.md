...
- current blocker token: `phase-291x JoinIR route detector condition-scope support family migration pending`
...
- latest known checkpoint: `291x-377` pruned the zero-caller parent
  `function_scope_capture` compatibility export
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: migrate `loop_condition_scope` callers to
...
- next: JoinIR route detector condition-scope support family migration
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-377`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector condition-scope support family migration
