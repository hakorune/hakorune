  ...
  - post-primitive follow-on queue:
    ...
    8. next ready task: resume optimization lane
        ...
        - current parity-wave keeper now landed:
          - metadata-bearing Point local-i64 user-box JSON fixtures now stay green on boundary `pure-first` owner lane without compat replay
        ...
        - next parity box inside that wave:
          - widen the same metadata-bearing local user-box keep smoke from Point/i64 into Bool/Float declared-field routes before moving on to string retained-view work
