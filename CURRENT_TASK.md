...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf is now inside shared producer `lang/src/compiler/build/build_box.hako`: `_bundle_inputs_requested(...)` and `_normalize_bundle_inputs(...)` still repeat the same field-level presence/empty checks, so the next safe same-owner slice is helper-local bundle field predicate/reset
