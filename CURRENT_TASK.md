...
- Exact focus: `ArrayBox.push` を first cutover unit に固定し、`array_core_box.hako` / `array_state_core_box.hako` 側で visible push semantics を authority owner として持たせる。Rust 側は `array_runtime_any.rs` / `array_runtime_facade.rs` / `array_runtime_substrate.rs` の shell/provider に押し戻す
