...
48. `phase-131x vm legacy contract migration` (active)
...
- Current blocker: `src/runner/stage1_bridge/stub_child.rs` と `src/runner/stage1_bridge/env/stage1_aliases.rs` がまだ legacy contract を支えている
...
- `phase-131x` now follows the remaining explicit legacy `vm` contract callers and bridge seams; the legacy mismatch smoke has already been archived and the backend-hint child env is being narrowed.
