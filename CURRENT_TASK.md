...
- latest known checkpoint: `291x-427` moved normalized-shadow suffix router
  under the normalization owner
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: inventory the next compiler-cleanliness seam after
  normalized-shadow suffix router owner move
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-427`; detailed landed history lives in phase card
