...
- Exact focus: `ArrayBox.push/get/set` landed as owner-helper units; next is `size aliases/pop` を同じ owner-helper 粒度に揃える。`array_core_box.hako` / `array_state_core_box.hako` 側で visible array semantics を authority owner として持たせ、Rust 側は `array_runtime_any.rs` / `array_runtime_facade.rs` / `array_runtime_substrate.rs` の shell/provider に押し戻す
