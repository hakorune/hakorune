...
- resume point: continue Phase 293x after `MIMAP-110A`; the next selected
  blocker is `MIMAP-111A`, segment allocation modeled local-free apply plan
  route.
...
- next 293x order:
  1. `MIMAP-111A`: add a scalar local-free apply-plan ledger downstream of the
     candidate ledger without mutating a page free-list
