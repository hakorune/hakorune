...
- summary:
  ...
  - naming cleanup (2026-03-07, slice 81): loop-route 近傍の code comment 見出しを `pattern` から `shape/structure` へ寄せた
    - synced files: `src/mir/{loop_canonicalizer/skeleton_types.rs,join_ir_vm_bridge/{merge_variable_handler,terminator_builder,block_allocator,call_generator}.rs,loop_pattern_detection/legacy/{loop_body_cond_promoter,loop_body_digitpos_promoter}.rs,control_tree/normalized_shadow/{loop_true_if_break_continue.rs,common/loop_if_exit_contract.rs},join_ir/frontend/ast_lowerer/{nested_if,read_quoted}.rs,join_ir/lowering/{digitpos_condition_normalizer,loop_scope_shape/case_a_lowering_shape}.rs}` / `CURRENT_TASK.md`
    - intent: loop-route / normalized-shadow / bridge helper の current-facing comment では `pattern` を設計主語にせず、`shape` / `structure` / `route-specific` の語彙へ寄せる。一般用語の design pattern はこの slice では対象外
    - verification: `rg -n "# Pattern|Pattern Before|Pattern After|pattern-specific metadata|Pattern Example|Pattern Matching|Pattern matched|Pattern: loop\\(true\\)|Pattern shape contract" src/mir/loop_canonicalizer/skeleton_types.rs src/mir/join_ir_vm_bridge/{merge_variable_handler,terminator_builder,block_allocator,call_generator}.rs src/mir/loop_pattern_detection/legacy/{loop_body_cond_promoter,loop_body_digitpos_promoter}.rs src/mir/control_tree/normalized_shadow/{loop_true_if_break_continue.rs,common/loop_if_exit_contract.rs} src/mir/join_ir/frontend/ast_lowerer/{nested_if,read_quoted}.rs src/mir/join_ir/lowering/{digitpos_condition_normalizer,loop_scope_shape/case_a_lowering_shape}.rs` = 0 hit
