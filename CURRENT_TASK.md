...
- pure `.hako-only hakorune build` blocker map (2026-03-14):
  - primary Rust-owned blockers:
    1. `src/host_providers/mir_builder.rs`
       ...
       - latest tightening: the metadata-map seam is now also closed behind `Stage1UserBoxDecls::into_metadata_map()`, so the remaining active leaf is the module-entry seam at `stage1_program_json_module_handoff(...)` -> `parse_program_json_module(...)`
