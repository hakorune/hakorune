...
- current blocker token:
  `GUARD-MANIFEST-003 segment closeout thin wrappers`
...
- resume point: continue Phase 293x after `GUARD-MANIFEST-002`; the next
  selected blocker is `GUARD-MANIFEST-003`, the segment closeout thin-wrapper
  implementation.
...
- next 293x order:
  1. `GUARD-MANIFEST-003`: move the selected recent hako_alloc segment
     closeout guards behind manifest rows while preserving public guard names
