...
- resume point: split loop-update assignment traversal, RHS classification, and
  tests into helper modules without behavior changes
...
- next: JoinIR loop-update summary helper split; do not mix with behavior
  changes
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-331`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR loop-update summary helper split
