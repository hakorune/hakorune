...
- pure `.hako-only hakorune build` Rust stop-line / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed Rust slices now keep Program(JSON) `user_box_decls` parse-entry plus explicit-vs-compat precedence behind `Stage1UserBoxDeclHandoff::resolve_user_box_decls()`, keep module metadata splice inside `Stage1ProgramJsonModuleHandoff::into_module_with_user_box_decls()`, keep the metadata-map seam behind `Stage1UserBoxDecls::into_metadata_map()`, replace the raw decl `serde_json::Value` seam with typed `Stage1UserBoxDecl`, and move the free module-entry pair behind `Stage1ProgramJsonInput::into_module_handoff()` / `parse_module()`
  ...
  - exact next leaf stays in `src/host_providers/mir_builder.rs`: the remaining owner-local entry seam is now `Stage1ProgramJsonInput::into_module_handoff()` plus `Stage1UserBoxDeclHandoff::parse(...)` above `module_to_mir_json(...)`
...
- pure `.hako-only hakorune build` blocker map (2026-03-14):
  - primary Rust-owned blockers:
    1. `src/host_providers/mir_builder.rs`
       ...
       - latest tightening: the free module-entry pair is now also folded into `Stage1ProgramJsonInput::into_module_handoff()` / `parse_module()`, so the remaining active leaf is the same-owner entry split with `Stage1UserBoxDeclHandoff::parse(...)`
