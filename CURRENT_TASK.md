...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  - current active owner is now back above the Rust stop-line at `lang/src/mir/builder/MirBuilderBox.hako`; `src/host_providers/mir_builder.rs` is near thin floor after the typed parsed-value owner cut
  ...
  - latest landed `.hako` slices move the delegate gate/provider call behind `lang/src/mir/builder/internal/delegate_provider_box.hako::BuilderDelegateProviderBox.try_emit(...)`, the delegate-side `user_box_decls` finalize behind `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.inject_user_box_decls(...)`, and the shared outer finalize chain behind `lang/src/mir/builder/internal/finalize_chain_box.hako::BuilderFinalizeChainBox.apply(...)`, leaving `MirBuilderBox.hako` almost purely on route sequencing plus small compat shims
  - targeted proof for the current front is `bash tools/smokes/v2/profiles/integration/core/phase2034/mirbuilder_internal_canary_vm.sh`, `bash tools/hakorune_emit_mir_mainline.sh lang/src/runner/stage1_cli_env.hako <tmp-out>`, and `bash tools/selfhost/refresh_stage1_module_env_snapshot.sh`
  - exact next leaf stays in `lang/src/mir/builder/MirBuilderBox.hako` only if another exact disappearing leaf appears in the remaining compat shims (`_lower_func_defs_if_enabled(...)`, source-entry checked shim); otherwise the next wave should move to runner owners `lang/src/runner/{stage1_cli_env.hako,stage1_cli.hako,launcher.hako}`
...
- selfhost mainline next:
  ...
  - `phase-29ch` summary lock:
    ...
    - monitor-only explicit compat keep: `stage1-env-mir-program`
      ...
      - latest `MirBuilderBox` tightening: the delegate-side `user_box_decls` finalize now also lives in `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.inject_user_box_decls(...)`, so `_emit_delegate_program_json(...)` now reads as internal gate/provider -> internal finalize -> shared normalize
