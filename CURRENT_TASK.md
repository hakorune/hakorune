...
- pure `.hako-only hakorune build` Rust stop-line / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed Rust slices now keep Program(JSON) `user_box_decls` parse-entry plus explicit-vs-compat precedence behind typed owner-local boxes, keep module metadata splice inside `Stage1ProgramJsonModuleHandoff::into_module_with_user_box_decls()`, keep the metadata-map seam behind `Stage1UserBoxDecls::into_metadata_map()`, replace the raw decl `serde_json::Value` seam with typed `Stage1UserBoxDecl`, retire the free source/program entry wrappers behind `Stage1ProgramJsonModuleHandoff::parse(...)`, `Stage1ProgramJsonInput::into_module_handoff()`, and `SourceProgramJsonHandoff::for_source(...)`, and fold parsed-value decl shaping behind `Stage1ProgramJsonValue::{parse,resolve_user_box_decls}`
  - targeted proof for the current Rust slices is `cargo test mir_builder -- --nocapture`, `cargo test user_box_decls -- --nocapture`, and `cargo test program_json_to_mir_file -- --nocapture`
  - exact next leaf stays in `src/host_providers/mir_builder.rs`: the remaining owner-local value seam is now the thin parsed-value shim at `Stage1ProgramJsonInput::parse_value()` above `Stage1ProgramJsonValue::{parse,resolve_user_box_decls}`; if no new disappearing leaf appears after that, switch the next wave back to `.hako` authority replacement at `lang/src/mir/builder/MirBuilderBox.hako`
...
- selfhost mainline next:
  ...
  - `phase-29ch` summary lock:
    ...
    - monitor-only explicit compat keep: `stage1-env-mir-program`
      ...
      - latest `MirBuilderBox` tightening: the delegate compat tail now keeps selfhost-no-delegate gate, provider emit, and local user-box-decls finalize behind `_delegate_disabled(...)`, `_emit_delegate_provider_checked(...)`, and `_inject_delegate_user_box_decls(...)`, so `_emit_delegate_program_json(...)` reads as gate -> provider emit -> local finalize
