...
- split:
  ...
  - `backend-zero`: accepted pointer / `phase-29ck` queued
    ...
    - landed slice:
      ...
      - next backend front is no longer launcher caller cutover; it is pinned to additional compiled-stage1 surrogate shrink and then the next B3c opcode row
      - next B3 front is no longer one Python bucket; it is pinned to the remaining B3c opcode rows and then the next B3d analysis/support row
      - restart handoff: safest next slices are `crates/nyash_kernel/src/plugin/module_string_dispatch/llvm_backend_surrogate.rs` temporary residue shrink, then `src/llvm_py/{resolver.py,type_facts.py}` support-owner demotion
