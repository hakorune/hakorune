  ...
  1. thin backend boundary hardening
     ...
     - role:
       ...
       - fixed order:
         ...
         - exact next slice: widen boundary-owned compile coverage beyond `ret_const` / `Global.print` / `StringBox.length` / `RuntimeDataBox.length(StringBox)` / `StringBox.indexOf/1` / `RuntimeDataBox.length(ArrayBox)` into narrow `RuntimeDataBox.size(MapBox)` before broader method-loop packs
     - acceptance:
