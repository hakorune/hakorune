...
- active lane:
  ...
  - current app/runtime gap read:
    ...
    - CoreBox router follow-up has moved `StringBox.length/len/size`, `StringBox.substring/substr`, and `StringBox.concat` to the Unified value path; remaining cleanup is about 7 family-equivalents, starting with `trim`
