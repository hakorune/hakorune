...
- current landed substring truth:
  ...
  - boundary `pure-first` consumer is now landed for `substring(...).length()`:
    ...
    - current live post-sink benchmark body is now pinned separately by `tools/smokes/v2/profiles/integration/phase137x/phase137x_direct_emit_substring_concat_post_sink_shape.sh`; that smoke now also requires the helper-result `%36` to keep `publication_sink` / `direct_kernel_entry` plans on the live MIR, and the exact `substring_concat` seed now refuses the body unless the same helper-result plan metadata is present, so next seed reduction can rely on proof-bearing metadata instead of only the opcode shape
