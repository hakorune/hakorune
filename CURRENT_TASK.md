...
- current blocker token: `phase-291x JoinIR LoopFeatures helper surface prune pending`
...
- latest known checkpoint: `291x-343` inventoried the remaining
  `LoopFeatures` helper surface and found it is dead diagnostic scaffolding
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: prune remaining `LoopFeatures` helper surface; do not change
  route behavior
...
- next: JoinIR `LoopFeatures` helper surface prune; do not change route
  behavior
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-343`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR `LoopFeatures` helper surface prune
