...
- resume point: continue Phase 293x after `MIMAP-118A`; the next selected
  blocker is `MIMAP-119A`, segment allocation modeled local-free integration
  route.
...
- next 293x order:
  1. `MIMAP-119A`: compose released-span -> local-free candidate -> apply-plan
     -> page-model apply into one allocator-owned integration owner
