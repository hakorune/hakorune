...
- current blocker token: `phase-291x normalized-shadow known-intrinsic comment cleanup pending`
...
- latest known checkpoint: `291x-420` inventoried stale normalized-shadow
  KnownIntrinsic transition wording before cleanup
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: clean stale `KnownIntrinsic` transition wording in
  `expr_lowering_contract.rs` without changing intrinsic behavior
...
- next: normalized-shadow known-intrinsic comment cleanup
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-420`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: normalized-shadow known-intrinsic comment cleanup
