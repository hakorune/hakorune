...
- current blocker token: `phase-291x JoinIR LoopFeatures break-continue count prune pending`
...
- latest known checkpoint: `291x-347` inventoried `LoopFeatures` count
  fields and found `break_count`/`continue_count` are dead diagnostics
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: prune `LoopFeatures.break_count` and
  `LoopFeatures.continue_count`; do not change route behavior
...
- next: JoinIR `LoopFeatures.break_count` / `continue_count` prune; do not
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-347`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR `LoopFeatures.break_count` / `continue_count` prune
