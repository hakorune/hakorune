...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf is now inside shared producer `lang/src/compiler/build/build_box.hako`: `_collect_bundle_inputs(...)` still mixes `bundles`, named-bundle pairs, and require-mod validation in one body, so the next safe same-owner slice is validation/assign helper split only if it stays owner-local and proof-cheap
