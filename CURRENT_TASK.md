...
- resume point: replace live normalized-shadow "fallback to legacy" wording with
  out-of-scope route / route-chaining wording
...
- next: normalized-shadow stale legacy wording cleanup
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-404`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: normalized-shadow stale legacy wording cleanup
