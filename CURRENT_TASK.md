...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed `.hako` slices move the delegate gate/provider call behind `lang/src/mir/builder/internal/delegate_provider_box.hako::BuilderDelegateProviderBox.try_emit(...)`, the full delegate-side finalize behind `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.finalize_mir(...)`, the shared outer finalize chain behind `lang/src/mir/builder/internal/finalize_chain_box.hako::BuilderFinalizeChainBox.apply(...)`, and the shared non-empty Program(JSON) text guard in `stage1_cli_env.hako` behind `Stage1ProgramJsonTextGuardBox.{coerce_text_checked,input_present}(...)`
  ...
  - exact next leaf is the same-owner Program(JSON) marker helper in `lang/src/runner/stage1_cli.hako` (`_materialized_program_json_payload(...)`, `_coerce_program_json_for_emit_mir_checked(...)`, `_coerce_program_json_output_checked(...)`); after that, mirror the same tightening in `lang/src/runner/launcher.hako`
