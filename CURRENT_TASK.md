...
- current landed substring truth:
  ...
  - boundary `pure-first` consumer is now landed for `substring(...).length()`:
    ...
    - the same phi smoke now also pins the header/latch loop semantics (`phi/phi/phi`, positive loop bound, compare `<`, branch, and the latch `const 1` increment), so the remaining exact-seed work is now semantic-boundary close-out rather than more raw body-shape cleanup
    ...
    - remaining close-out count before the next phase:
