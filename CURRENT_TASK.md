...
- pure `.hako-only hakorune build` Rust stop-line / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed Rust slices now keep Program(JSON) `user_box_decls` parse-entry plus explicit-vs-compat precedence behind `Stage1UserBoxDeclHandoff::resolve_user_box_decls()`, and keep module metadata splice inside `Stage1ProgramJsonModuleHandoff::into_module_with_user_box_decls()`, retiring the old free finalize pair
  ...
  - exact next leaf stays in `src/host_providers/mir_builder.rs`: the remaining module finalize seam is now the owner-local metadata map path around `Stage1ProgramJsonModuleHandoff::into_module_with_user_box_decls()` and `stage1_user_box_decl_map()` above `module_to_mir_json(...)`
