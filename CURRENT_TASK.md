...
17. `phase-150x array string-store vertical slice` (active)
18. `phase-151x canonical lowering visibility lock` (next)
19. `phase-kx vm-hako small reference interpreter recut` (parked after optimization)

## Current Front

- Active lane: `phase-150x array string-store vertical slice`
- Active front: `ArrayStoreString` route を `.hako owner -> MIR canonical reading -> Rust executor` で visible に通す
- Current blocker: current concrete symbol `nyash.array.set_his` を canonical contract ではなく ABI/executor detail に押し込むこと
- Exact focus:
  - `.hako` owner route `ArrayStoreString`
  - canonical MIR reading `store.array.str`
  - current executor path `array_string_store_handle_at(...)`
  - do not reopen perf until the canonical reading is visible against current concrete lowering
...
- `phase-137x` is paused, not canceled:
  - perf reopen stays blocked until `phase-150x` and `phase-151x` land
  - do not return early just because the current Rust executor got cleaner
- first exact slices:
