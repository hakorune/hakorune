...
- resume point: make loop-update classification RHS/self-reference first; do
  not mix with Case-A route changes
...
- next: JoinIR loop-update RHS-first classification; do not mix with Case-A
  route changes
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-323`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR loop-update RHS-first classification
