...
- summary:
  ...
  - naming cleanup (2026-03-07, slice 82): `joinir/patterns` の active module surface を `route_entry` へ寄せた
    ...
    - intent: active module/import surface は `crate::mir::builder::control_flow::joinir::route_entry::*` に固定し、`patterns/` は legacy physical path alias としてだけ保持する。docs でも active role と on-disk path を分離し、存在しない `joinir/route_entry/*.rs` を参照しないようにする
