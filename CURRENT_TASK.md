...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed `.hako` slices move the delegate gate/provider call behind `lang/src/mir/builder/internal/delegate_provider_box.hako::BuilderDelegateProviderBox.try_emit(...)`, the full delegate-side finalize behind `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.finalize_mir(...)`, and the shared outer finalize chain behind `lang/src/mir/builder/internal/finalize_chain_box.hako::BuilderFinalizeChainBox.apply(...)`, leaving `MirBuilderBox.hako` almost purely on route sequencing plus small compat shims
