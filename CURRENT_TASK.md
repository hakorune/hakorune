...
- current blocker token: `phase-291x JoinIR route detector condition-scope compatibility export prune pending`
...
- latest known checkpoint: `291x-378` migrated condition-scope callers to
  `support::condition_scope` or legacy owner-local paths
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: verify and prune zero-caller parent `loop_condition_scope`
...
- next: JoinIR route detector condition-scope compatibility export prune
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-378`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector condition-scope compatibility export prune
