...
- selfhost mainline next:
  ...
  - `phase-29ch` summary lock:
    ...
    - monitor-only explicit compat keep: `stage1-env-mir-program`
      ...
      - latest caller-tail collapse: the stable `phase2160/builder_min_*` bucket now also keeps skip/tag/functions boilerplate behind `run_builder_module_tag_canary()`, while `builder_min_return_binop_varvar_canary_vm.sh` remains an explicit keep because it still carries the no-functions-check contract
      - latest caller-tail collapse: the stable `phase2160/registry_optin_compare_*` + `registry_optin_binop_intint` bucket now also keeps skip/tag/functions boilerplate behind `run_registry_builder_tag_canary()`, while `registry_optin_canary`, method-arraymap wrappers, diag/direct probes, and `registry_optin_return_binop_varvar_canary_vm.sh` remain explicit keeps
