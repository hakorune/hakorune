...
49. `phase-132x vm default backend decision` (landed)
50. `phase-133x micro kilo reopen selection` (active)

## Current Front

- Active lane: `phase-133x micro kilo reopen selection`
- Active micro: `micro kilo` を current に固定し、first slice を `kilo_micro_substring_concat` に寄せる
- Current blocker: mainline blocker ではない。順番を崩して `main kilo` や broad array retune に飛ばないことが stop-line
- Exact focus: fixed order `leaf-proof micro -> micro kilo -> main kilo` を守り、`substring_concat` を first owner、`array_getset` は recheck lane に置く
...
- perf reopen order remains fixed:
  - `leaf-proof micro`
  - `micro kilo`
  - `main kilo`
