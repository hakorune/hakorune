...
- resume point: continue Phase 293x with `MIMAP-251A`,
  the post-segment-arena-backing-modeled-no-escape-address-residence-closeout
  row selection selected by MIMAP-250A.
...
- next 293x order:
  1. `MIMAP-251A`: choose the next narrow allocator bridge after modeled
     no-escape address residence closeout
