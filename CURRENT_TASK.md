...
- resume point: inventory legacy module exports that have no non-legacy
  external callers; check internal imports before pruning
...
- next: JoinIR route detector unused legacy module export inventory; do not
  prune modules in the same card
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-356`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector unused legacy module export inventory
