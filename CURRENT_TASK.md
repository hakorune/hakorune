...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf is now inside shared producer `lang/src/compiler/build/build_box.hako`: bundle ctx array setup still repeats between `_collect_bundle_inputs(...)` and `_apply_env_bundle_inputs(...)`, so the next safe same-owner slice is helper-local alias/require array ensure
