...
- resume point: continue Phase 293x after `MIMAP-111A`; the next selected
  blocker is `MIMAP-112A`, post-local-free-apply-plan row selection.
...
- next 293x order:
  1. `MIMAP-112A`: select exactly one next row using the mimalloc row validation
     cadence
