...
- current blocker token:
  `MIMAP-105A post-release-span-facts row selection`
...
- resume point: continue Phase 293x after `MIMAP-104A`; the next selected
  blocker is `MIMAP-105A`, post-release-span-facts row selection.
...
- next 293x order:
  1. `MIMAP-105A`: select exactly one next row after the scalar release span
     facts route
