...
- structure-first override is now explicit in `phase-29x`:
  ...
  - launcher/root-first daily transport cut is landed:
    ...
    - next big surgery is to thin the remaining `compile_json_path` live caller surfaces; the Hako front-door bridge is already retired, so the remaining work is builder/runtime dispatchers and legacy wrappers
    - Hako front-door `env.codegen.compile_json_path` is now retired; explicit legacy compare/archive callers using `hako-ll-compare-v0` still use the archive-later helper path in downstream wrappers
    - builder-side `compile_json_path` recognition is now retired too; remaining reachability now lives in legacy bridge/runtime wrappers only
    - harness-side `src/runner/modes/llvm/object_emitter.rs` has been retired from the active runner path; object emission now goes through the shared harness helper that preserves the old pure-first/harness replay semantics, and the wrapper is deleted
  ...
  - backend-zero thin-up is landed for the daily owner split:
    ...
    - legacy lock `phase29ck_boundary_pure_first_min.sh` still pins the `compat_replay=none` route trace directly while daily owner proof lives in `phase29x`
