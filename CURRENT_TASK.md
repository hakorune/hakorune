...
- Current blocker:
  - no collection/runtime blocker remains, and the smoke runner now has a first-class suite manifest contract; inventory is suite-aware, the first four semantic splits have landed, and `phase29ck_boundary` now lives under `integration/phase29ck_boundary/{entry,string,runtime_data}/`, so the next blocker is the next live family split under `integration/apps` (`vm_hako_caps` first)
  - active phase is now `phase-29cq`: suite-manifest first + suite-aware inventory landed, `phase29ck_boundary` has been split into `integration/phase29ck_boundary/{entry,string,runtime_data}/`, and the next slice continues semantic split of `integration/apps` (`--profile` compat keep, `--suite` opt-in, no mass move)
...
- Execution checklist:
  ...
  - `[ ]` phase-29cq ongoing split: continue splitting the remaining `integration/apps` families by semantic domain (next family: `vm_hako_caps`)
...
- immediate: phase-29cq semantic split of the next live family out of `tools/smokes/v2/profiles/integration/apps/` (`vm_hako_caps` first)
