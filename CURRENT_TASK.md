...
- resume point: continue Phase 293x after `MIMAP-108A`; the next selected
  blocker is `MIMAP-109A`, segment allocation modeled local-free candidate
  ledger route.
...
- next 293x order:
  1. `MIMAP-109A`: add a scalar local-free candidate ledger downstream of
     MIMAP-107A released-span reports using the L2 validation cadence
