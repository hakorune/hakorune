...
- the current front is `kilo_micro_substring_only`:
  ...
  - current cut is the remaining `substring_hii` / `string_len_from_handle` hit-path overhead after widening the alternating two-slice caches
...
- immediate follow-up order for the substring lane:
  1. measure whether the remaining `string_len_from_handle` hit-path cost is mostly observer / trace bookkeeping
...
- Current blocker: WSL のブレが大きいので、`3 runs + perf` でしか delta を採らない
- Current next design slice:
  - keep the `substring` helper seam tight and remove alloc-heavy trace formatting first
  - accept-gate reread:
      - `kilo_micro_substring_only: 46 ms`
      - `kilo_micro_substring_concat: 4 ms`
      - `kilo_micro_array_string_store: 9 ms`
...
- current perf reopen truth:
  ...
  - `kilo_micro_concat_hh_len`: `ny_aot_ms=3`
...
- `phase-156x` landed:
  ...
  - deeper exact probe:
  - `bench_kilo_micro_concat_const_suffix.hako` -> current exact front under `3 runs + perf`
  ...
  - new exact split:
    ...
    - latest exact read: `c_ms=3 / ny_aot_ms=3`
