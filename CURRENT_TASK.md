...
43. `phase-126x vm public gate shrink decision` (active)

## Current Front

- Active lane: `phase-126x vm public gate shrink decision`
- Active micro: `compat/proof/debug contracts を含めて、public vm gate を今 shrink できるかを判断する`
- Current blocker: `compat route smoke contract + stage1 bridge backend-hint chain`
- Exact focus: `compat route smoke contract` を hard blocker、`stage1_bridge` backend-hint chain を次の source seam として固定する
...
- `phase-126x` now decides whether public `--backend vm` can shrink before compat route smokes stop asserting raw vm-family tags.
