...
- latest known checkpoint: `291x-376` migrated `function_scope_capture`
  callers to `loop_route_detection::support::function_scope`
- current no-growth baseline: `classifiers=0 rows=0`; no `.inc`
  method/box string classifiers are allowlisted
- worktree expectation: clean unless the active slice is in progress
- resume point: verify and prune zero-caller parent
...
- next: JoinIR route detector function-scope compatibility export prune
...
- latest cleanup card: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- latest checkpoint: `291x-376`; detailed landed history lives in phase card
  files and the compact `landed_tail` in `CURRENT_STATE.toml`
- next cleanup: JoinIR route detector function-scope compatibility export prune
