...
4. `phase-137x main kilo reopen selection` (active)
5. `phase-kx vm-hako small reference interpreter recut` (parked after optimization)

## Current Front

- Active lane: `phase-137x main kilo reopen selection`
- Active front: `kilo_kernel_small_hk` 再ベースライン + `kilo_micro_substring_concat` / `kilo_micro_array_getset` 再確認
- Current blocker: `nyash_kernel` の構造分割は landed。split kernel 上で `main kilo` を reopen する
- Exact focus: `keep / thin keep / compat glue / substrate candidate` の4層 split を前提に、`main kilo` を split kernel 上で reopen する
