...
- resume point: continue Phase 293x after `MIMAP-072A`; the next selected
  blocker is `MIMAP-073A`, the post-scheduler-consume row selection.
...
- next 293x order:
  1. `MIMAP-073A`: select one follow-up after scheduler ledger consume closeout
