...
- split:
  ...
  - `backend-zero`: accepted pointer / `phase-29ck` queued
    ...
    - landed slice:
      ...
      - next B3 front is no longer one Python bucket; after the resolver/type-facts, phi-manager, mir-analysis, and phi-wiring-analysis rows it is pinned to the next `phi_wiring/**` owner seam (`tagging.py` / `fact_propagation.py` / `wiring.py`)
      - restart handoff: safest next slices are `crates/nyash_kernel/src/plugin/module_string_dispatch/llvm_backend_surrogate.rs` temporary residue shrink if another exact disappearing leaf appears, then the next `phi_wiring/**` support-owner row
