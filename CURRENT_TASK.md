...
- pure `.hako-only hakorune build` authority-replacement / `phase-29cj` exact front (2026-03-16):
  ...
  - exact next leaf is now inside shared producer `lang/src/compiler/build/build_box.hako`: `_prepare_scan_src(...)` still mixes bundle-ctx error handoff, resolve-fail tagging, and result-map shaping, so the next safe same-owner slice is helper-local prepared-scan-src error/result handoff
