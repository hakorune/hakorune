...
- resume point: continue Phase 293x with `MIMAP-187A`,
  the post-segment-map-local-free-integration-bridge-closeout planning row
  selected by MIMAP-186A.
...
- next 293x order:
  1. `MIMAP-187A`: choose the next narrow row after segment-map local-free
     integration bridge closeout
