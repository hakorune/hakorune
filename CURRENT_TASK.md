...
- resume point: continue Phase 293x with `MIMAP-265A`,
  the segment arena backing modeled source accounting diagnostics selected by
  MIMAP-264A.
...
- next 293x order:
  1. `MIMAP-265A`: add observer-only diagnostics for modeled source accounting
     inventory
