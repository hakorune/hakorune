...
11. `phase-145x compat quarantine shrink` (active)
12. `phase-146x string semantic boundary tighten` (next)
13. `phase-137x main kilo reopen selection` (successor)
14. `phase-kx vm-hako small reference interpreter recut` (parked after optimization)

## Current Front

- Active lane: `phase-145x compat quarantine shrink`
- Active front: `host microkernel` glue と `compat quarantine` の境界を source/docs で固定する
- Current blocker: quarantine residue と host-side glue が source 上でまだ近く見える
- Exact focus: `hako_forward_bridge.rs` / `future.rs` / `invoke_core.rs` を host service contract 側に寄せ、`module_string_dispatch/**` を quarantine shrink-only に固定する
...
5. `docs/development/current/main/phases/phase-145x/README.md`
...
- latest bundle read:
  ...
  - `20260406-000358` bundle still has `concat_const_suffix_fallback` as the top explicit hot symbol
  - `array_string_store_handle_at` is the next independent leaf after string const-path
- `phase-137x` is intentionally delayed:
  - perf reopen waits until compat/string cleanup is clean enough not to create rework
- first exact slices:
