...
- pure `.hako-only hakorune build` Rust stop-line / `phase-29cj` exact front (2026-03-16):
  ...
  - latest landed Rust slices now keep Program(JSON) `user_box_decls` parse-entry plus explicit-vs-compat precedence behind `Stage1UserBoxDeclHandoff::resolve_user_box_decls()`, keep module metadata splice inside `Stage1ProgramJsonModuleHandoff::into_module_with_user_box_decls()`, keep the metadata-map seam behind `Stage1UserBoxDecls::into_metadata_map()`, and replace the remaining raw decl `serde_json::Value` seam with typed `Stage1UserBoxDecl`
  ...
  - exact next leaf stays in `src/host_providers/mir_builder.rs`: the remaining owner-local entry seam is now `stage1_program_json_module_handoff(...)` plus `parse_program_json_module(...)` above `module_to_mir_json(...)`
