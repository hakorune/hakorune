...
18. `phase-151x canonical lowering visibility lock` (active)
19. `phase-kx vm-hako small reference interpreter recut` (parked after optimization)

## Current Front

- Active lane: `phase-151x canonical lowering visibility lock`
- Active front: `.hako owner -> MIR canonical reading -> current concrete lowering -> Rust executor` を source-backed に固定する
- Current blocker: canonical MIR readings が docs だけでなく concrete lowering 側にも読めるようにすること
- Exact focus:
  - `const_suffix`
  - `ArrayStoreString`
  - `MapStoreAny`
...
7. `docs/development/current/main/phases/phase-151x/README.md`
