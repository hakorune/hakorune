...
- resume point: continue Phase 293x after `MIMAP-107A`; the next selected
  blocker is `MIMAP-108A`, post-released-span-ledger row selection.
...
- next 293x order:
  1. `MIMAP-108A`: select exactly one next row using the mimalloc row validation
     cadence
