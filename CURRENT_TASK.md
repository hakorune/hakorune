...
14. `phase-147x semantic optimization contract selection` (active)
15. `phase-kx vm-hako small reference interpreter recut` (parked after optimization)

## Current Front

- Active lane: `phase-147x semantic optimization contract selection`
- Active front: `.hako owner -> MIR canonical contract -> Rust executor` を current optimization authority として固定する
- Current blocker: `BorrowedText` / `TextSink` を Rust helper authority に見せず contract-first へ切り替えること
- Exact focus:
  - `.hako` owner-side route vocabulary の freeze
  - MIR canonical ops の最小集合の freeze
  - first vertical slice を `concat const-suffix` に固定
...
- `phase-147x` current design lock:
