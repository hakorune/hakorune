...
- split:
  ...
  - `backend-zero`: accepted pointer / `phase-29ck` queued
    ...
    - landed slice:
      ...
      - next backend front is no longer launcher caller cutover; it is pinned to compiled-stage1 surrogate shrink and B3b `src/llvm_py/{mir_reader.py,build_ctx.py,build_opts.py}` ingest/context demotion
      - next B3 front is no longer one Python bucket; it is pinned to B3b ingest/context first, then opcode lowering
