//! One startup/flush owner; each artifact supplies its explicit result policy.
...
pub extern "C" fn main() -> i32 {
    fn invoke() -> i64 {
        ...
        // SAFETY: the linked generated module supplies the legacy entry ABI.
        unsafe { ny_main() }
    }
    run_entry(invoke, decode_legacy_result)
}

#[cfg(feature = "legacy-entry")]
fn decode_legacy_result(v: i64) -> i64 {
...
/// Invoke a linked normalized-status entry after startup, then flush once.
/// The callback must implement its own FFI safety contract.
#[cfg(feature = "lifecycle-core")]
pub fn run_normalized_entry(entry: fn() -> i64) -> i32 {
    run_entry(entry, |status| match status {
        0..=255 => status,
        _ => 70,
    })
}

fn run_entry(entry: fn() -> i64, project: fn(i64) -> i64) -> i32 {
    ...
    if want_json || want_text {
        ...
        if want_json {
            ...
            eprintln!(
                ...
                sp, br, bw, handles, alloc_count, alloc_bytes, collect_total, collect_sp,
                collect_alloc, last_ms, gc_mode_s, trial["status"].as_str().unwrap(),
                trial["error"].as_str().unwrap_or("none")
            );
        }
        // Threshold warning
...
// This is the actual output projection used by both metrics sinks.
fn trial_metrics_json(
    observation: Option<nyash_rust::runtime::gc_controller::TrialReachability>,
) -> serde_json::Value {
    ...
    serde_json::json!({ "status": status, "nodes": nodes, "edges": edges, "error": error })
}

#[cfg(test)]
#[path = "entry_gc_metrics_tests.rs"]
mod gc_metrics_tests;
