...
pub(super) fn array_slot_store_any(handle: i64, idx: i64, val_any: i64) -> i64 {
    ...
    match decode_array_fast_value(val_any) {
        ...
        ArrayFastDecodedValue::ImmediateF64(v) => array_slot_store_f64(handle, idx, v),
...
pub(super) fn array_slot_store_i64(handle: i64, idx: i64, value_i64: i64) -> i64 {
    array_slot_backend::store_i64(handle, idx, value_i64)
}

#[inline(always)]
pub(super) fn array_slot_store_bool(handle: i64, idx: i64, value_bool: bool) -> i64 {
    if !valid_handle_idx(handle, idx) {
        return 0;
    }
    with_array_box(handle, |arr| {
...
#[inline(always)]
pub(super) fn array_slot_store_f64(handle: i64, idx: i64, value_f64: f64) -> i64 {
    if !valid_handle_idx(handle, idx) {
        return 0;
    }
    with_array_box(handle, |arr| {
