...
// Array RuntimeData any-key gate: validate receiver and decode an array index.
#[inline(always)]
fn with_array_index_or_zero(handle: i64, key_any: i64, f: impl FnOnce(i64) -> i64) -> i64 {
...
pub(super) fn array_runtime_get_any_key(handle: i64, key_any: i64) -> i64 {
    with_array_index_or_zero(handle, key_any, |idx| array_runtime_get_idx(handle, idx))
}

pub(super) fn array_runtime_set_any_key(handle: i64, key_any: i64, val_any: i64) -> i64 {
    with_array_index_or_zero(handle, key_any, |idx| {
        array_runtime_set_idx_any(handle, idx, val_any)
    })
}

pub(super) fn array_runtime_has_any_key(handle: i64, key_any: i64) -> i64 {
    with_array_index_or_zero(handle, key_any, |idx| array_runtime_has_idx(handle, idx))
}
