...
- default Boundary compile now passes one captured `hako_llvmc_physical_contract_v1` row to `hako_llvmc_compile_json_with_options_v1`; the public generic/pure-first/harness exports remain compatibility surfaces, while supported pure-first seeds stay on the mainline
...
- next migration step is `llvmlite` demotion completion outside this crate; inside `ny-llvmc`, Boundary uses the existing versioned options entry and `harness` / `native` remain explicit replay lanes only
