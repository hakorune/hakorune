...
Reading note:
- `pattern2` は original phase log を示す historical route token（label `2`）だよ。
- `compat routes` は後続経路（historical compat tokens: `legacy binding` / `normalized shadow`）の short-hand だよ。
- token-A / token-B は historical file token map だけで使う short-hand だよ。
...
1. **Historical route ordering at the time** (router.rs):
   ...
   - loop_break route の `Ok(None)` は compat routes に進む
...
current live family: `src/mir/builder/control_flow/plan/loop_break/facts/`
...
```
commit 93022e7e1
loop_break label-2 lane の unpromoted LoopBodyLocal を検出したら route 全体を早期終了
...
compat routes へ fallback させる（detection→extract→lower SSOT 維持）
...
- `docs/development/archive/phases/phase-263/README.md` - This file
...
**Main Risks**:
1. `Ok(None)` で compat routes に進んだが、後続 route も処理できない → 別のエラー
   - **Mitigation**: compat routes は汎用処理なので対応可能
...
- representative legacy fixture pin token for the Stage-B realworld seg evidence lane
- archived smoke stem for the same evidence lane (semantic current entry: `loop_break_realworld_vm`)
