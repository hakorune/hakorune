...
Status: Landed
Date: 2026-04-27
Scope: Make SubstringViewsMicroSeedRoute field layout owner-private
Related:
  - docs/development/current/main/CURRENT_STATE.toml
  - docs/development/archive/phases/phase-291x/291x-494-array-getset-micro-route-field-boundary-card.md
  - src/mir/substring_views_micro_seed_plan.rs
  - src/mir/exact_seed_backend_route.rs
  - src/mir/mod.rs
  - src/runner/mir_json_emit/root.rs
---

# 291x-495: SubstringViewsMicro Route Field Boundary

## Goal

Keep the substring views micro seed route record owned by
`substring_views_micro_seed_plan`.
...
- `SubstringViewsMicroSeedRoute` fields are private.
- `SubstringViewsMicroSeedProof` is owner-private and no longer root-exported.
...
- Substring views micro and exact-seed focused tests pass.
...
- `SubstringViewsMicroSeedRoute` now keeps its fields owner-private.
- `SubstringViewsMicroSeedProof` is owner-private and no longer root-exported.
...
```bash
cargo check -q
cargo test -q substring_views_micro_seed
