...
Status: Landed
Date: 2026-04-27
Scope: Make UserBoxKnownReceiverMethodSeedRoute field layout owner-private
Related:
  - docs/development/current/main/CURRENT_STATE.toml
  - docs/development/archive/phases/phase-291x/291x-499-userbox-loop-micro-route-field-boundary-card.md
  - src/mir/userbox_known_receiver_method_seed_plan.rs
  - src/mir/exact_seed_backend_route.rs
  - src/mir/mod.rs
  - src/runner/mir_json_emit/root.rs
---

# 291x-500: UserBox Known Receiver Method Route Field Boundary

## Goal

Keep the UserBox known-receiver-method exact seed route record owned by
`userbox_known_receiver_method_seed_plan`.
...
- `src/mir/userbox_known_receiver_method_seed_plan.rs`
  - owns detection, route field derivation, proof vocabulary, payload vocabulary, and tests
...
- `UserBoxKnownReceiverMethodSeedRoute` fields are private.
- `UserBoxKnownReceiverMethodSeedProof` is owner-private and no longer root-exported.
...
- UserBox known-receiver-method and exact-seed focused tests pass.
...
- `UserBoxKnownReceiverMethodSeedRoute` now keeps its fields owner-private.
- `UserBoxKnownReceiverMethodSeedProof` is owner-private and no longer root-exported.
...
```bash
cargo check -q
cargo test -q userbox_known_receiver_method_seed
