...
Blocker: HAKO-ALLOC-USIZE-FIELD-GROUP-189
Related:
...
# 294x-191 Hako Alloc Usize Provider Activation Dry-Run Counter Selection

## Decision

Select the owner-local `HakoAllocProviderActivationDryRunUnsupportedBehavior`
counters as `HAKO-ALLOC-USIZE-FIELD-GROUP-190`:
...
These fields are monotonic dry-run counters initialized to `0`. The selected
group records unsupported activation outcomes only and does not open provider
activation.
...
`HAKO-ALLOC-USIZE-FIELD-GROUP-190` should migrate only the selected owner-local
counters and update the dry-run unsupported behavior guard to assert exact
