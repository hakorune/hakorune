...
Status: Landed
Date: 2026-05-25
Scope: thin-wrap the MIMAP-TLS-001 worker TLS cache guard root into an impl-backed wrapper.
Related:
  - tools/checks/k2_wide_mimalloc_worker_tls_cache_exe_guard.sh
---

# 295x-145 MIMAP-TLS-001 Worker TLS Cache Thin-Wrap Batch
...
Thin-wrap the MIMAP-TLS-001 worker TLS cache guard root. The batch keeps the
same validation semantics and moves the real shell body into
`tools/checks/impl/`.

Selected root:

- `k2_wide_mimalloc_worker_tls_cache_exe_guard.sh`
...
The MIMAP-TLS-001 worker TLS cache guard root is now easier to scan at the
root level.
