...
| Blocker | `phase-291x substring metadata carrier preflight pending` |
| Next | `inventory substring CoreMethod metadata coverage before touching the legacy mirror row` |
...
2. Inventory `substring` CoreMethod metadata coverage before touching the legacy mirror row.
