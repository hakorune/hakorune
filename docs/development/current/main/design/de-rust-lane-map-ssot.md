...
| C | Runtime Port | Rust VM 依存機能の `.hako VM` 置換（RVP） | `phase-29y/60-NEXT-TASK-PLAN.md` + `phase-29y/81-RUST-VM-TO-HAKO-VM-FEATURE-MATRIX.md` | active（current blocker: `RVP-C24`, capability=`MapBox.get(missing-key)` stale scalar route） |
...
- lane C:
  ...
  - current blocker は `RVP-C24 MapBox.get(missing-key)` stale scalar route。
