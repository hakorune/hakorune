...
| C | Runtime Port | Rust VM 依存機能の `.hako VM` 置換（RVP） | `phase-29y/60-NEXT-TASK-PLAN.md` + `phase-29y/81-RUST-VM-TO-HAKO-VM-FEATURE-MATRIX.md` | active（current blocker: `RVP-C22`, capability=`MapBox.keys()` unimplemented route） |
...
- lane C:
  ...
  - current blocker は `RVP-C22 MapBox.keys()` unimplemented route。
