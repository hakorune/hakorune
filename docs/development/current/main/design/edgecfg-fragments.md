...
Reading note:
- この文書で出てくる numbered route label は、特記がない限り historical traceability 用だけだよ。
...
```
ScanWithInit/SplitScan route builder
...
- **ScanWithInit/SplitScan extractor**: 形は近いが契約違反のケースは `Ok(None)` で流さず freeze（例: SplitScan の `else i = i + 1` 破り、ScanWithInit の `i = i + 1` 破り）

## 直近の導入ステップ（最小で始める）

1. `Frag/ExitKind/EdgeStub` の型を追加（docs+code 入口 SSOT）
2. `seq/if/loop` の合成だけ実装（cleanup/Invoke は後段）
3. 既存 route のうち 1 本だけ `Frag` 合成に寄せる（BoolPredicateScan 推奨）
...
- ScanWithInit/SplitScan contract SSOT: `docs/development/current/main/design/pattern6-7-contracts.md`

補足（Phase 270）:
- simple_while_minimal は test-only stub のため、一般ループの “基準” には使えない。
- Phase 270 では “最小の固定形” を AccumConstLoop として追加し、後で Frag 合成側へ吸収される前提で橋渡しにする。
...
- **Route Preparation**: ScanWithInit/SplitScan hand-rolled locations documented for Phase 281 migration
...
- BoolPredicateScan 適用: P0ではやらない（偽Frag回避、P1から実戦投入）
...
Phase 267: Scan/Split/Predicate routes への展開
- ScanWithInit を Frag 化
- SplitScan を Frag 化
- BoolPredicateScan を Frag 化
- 再利用性の確認（番号ラベル分岐の削減）

現時点では既存 ScanWithInit/SplitScan/BoolPredicateScan 実装や merge/EdgeCFG は未改変（合成能力の証明のみ）。
