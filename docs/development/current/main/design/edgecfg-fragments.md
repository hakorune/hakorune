...
- “分岐の中心” は legacy numbered route label ではなく **ExitKind** と **Frag（fragment）** に置く
- 値の合流は EdgeCFG の **block params + edge-args** で表し、PHI/推測/メタに逃げない
- 形状ラベル（legacy numbered route label）は「Extractor（形の認識）/ Plan（最小要件の抽出）」までに縮退し、merge/配線層へ逆流させない
...
- 番号ラベル（legacy numbered route label）は **回帰テスト名/症状ラベル**としては残して良い
- 実装の中心は `Frag/ExitKind/join(block params)` の合成則になる
- 各 route 実装（legacy numbered-label route）は "Extractor（形の認識）→ Frag 合成呼び出し" の薄い層へ縮退する
...
**Status**: Active SSOT
**Purpose**: Legacy numbered-label absorption destination
**Date**: 2025-12-23
...
**Upstream (Extractor/Normalizer)**: Finish "shape recognition" and extract route-specific knowledge (legacy numbered route labels allowed)
...
- Knows route semantics (scan, split, etc.; legacy numbered route labels may appear only for traceability)
...
## Bridge routes（legacy bridge labels）（撤去条件SSOT）

ここで言う “bridge route” は、既存の JoinIR ルートを壊さずに **最小の固定形を先に通す**ための一時レイヤ。
（例: Phase 270 の legacy bridge label for AccumConstLoop）

- 原則:
  - bridge route（legacy bridge label）は **汎用化しない**（固定形SSOT + fixture/smoke で仕様を固定するだけ）。
  - 将来は `Frag/ExitKind` 合成側へ **吸収して削除**する前提で追加する。

### Bridge contract（テンプレ / SSOT）

bridge route（legacy bridge label）を追加する場合は、最低限この “撤去条件” を先に書く（書けないなら追加しない）。
...
- **置換先（吸収先）の SSOT がある**
  ...
  - 吸収先が未確定な場合でも “層” は確定させる（legacy numbered-label layer にロジックを増やさない）
- **撤去条件（最低限）**
  1. 置換先（吸収先）で同じ fixture/smoke が PASS する
  2. bridge route（legacy bridge label）依存の分岐が router から消せる（最小差分で削除できる）
  3. quick/integration の FAIL 位置が悪化しない（既知Failは増やさない）
- **撤去手順（最小）**
  - router から bridge route（legacy bridge label）を外す
...
- **吸収先（層）**
  - Structured→CFG lowering 層（`Frag/ExitKind` 合成）またはその emission 入口（legacy numbered-label layer は extractor に縮退）
- **撤去条件**
  1. 上記 fixture/smoke が、bridge route（legacy bridge label）を使わない経路で PASS する（Frag/emit_frag 側で loop を構築できる）
...
Phase 267: JoinIR route への適用（legacy numbered route labels are traceability-only）
