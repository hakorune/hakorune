...
- 「番号ラベルで推測分岐」が増殖しやすい領域（legacy `PatternX` labels は長期的に traceability-only へ縮退したい）
...
- “分岐の中心” は legacy pattern label ではなく **ExitKind** と **Frag（fragment）** に置く
- 値の合流は EdgeCFG の **block params + edge-args** で表し、PHI/推測/メタに逃げない
- 形状ラベル（legacy pattern）は「Extractor（形の認識）/ Plan（最小要件の抽出）」までに縮退し、merge/配線層へ逆流させない
...
## 形状ラベルは最終的に消える？（設計としての答え）

消える（実装の中心概念から降格する）。

- 番号ラベル（legacy label: PatternX）は **回帰テスト名/症状ラベル**としては残して良い
- 実装の中心は `Frag/ExitKind/join(block params)` の合成則になる
- 各 route 実装（legacy pattern）は "Extractor（形の認識）→ Frag 合成呼び出し" の薄い層へ縮退する
...
**Status**: Active SSOT
**Purpose**: Legacy pattern-label absorption destination
**Date**: 2025-12-23

### Why Composition is SSOT

Legacy numbered labels are **symptom labels** for regression tests, NOT architectural concepts.
The architectural SSOT is **Frag composition rules** (`seq`/`if`/`loop`/`cleanup`).

**Upstream (Extractor/Normalizer)**: Finish "shape recognition" and extract route-specific knowledge (legacy pattern labels allowed)
...
#### Tier 1: Normalizer (Route-Specific; legacy labels are traceability-only)

**Responsibilities**:
- Allocates `BasicBlockId` (`builder.next_block_id()`)
- Allocates `ValueId` (`builder.next_value_id()`)
- Knows route semantics (scan, split, etc.; legacy pattern labels may appear only for traceability)
- Constructs initial `Frag` with valid blocks/values

**Example** (ScanWithInit route; legacy numbered label is traceability-only):
```rust
...
#### Tier 2: Composition API (Route-Agnostic)
...
**Why no allocation?**
- Separation of concerns: allocation (route-specific) vs wiring (generic)
...
```
ScanWithInit/SplitScan route builder (legacy numbered labels are traceability-only)
    ↓ (allocate blocks/values)
Recipe / VerifiedRecipe → CorePlan
    ↓ (pre-allocated IDs)
...
- **ScanWithInit/SplitScan extractor**: 形は近いが契約違反のケースは `Ok(None)` で流さず freeze（legacy numbered labels are traceability-only; 例: SplitScan の `else i = i + 1` 破り、ScanWithInit の `i = i + 1` 破り）

## 直近の導入ステップ（最小で始める）

1. `Frag/ExitKind/EdgeStub` の型を追加（docs+code 入口 SSOT）
2. `seq/if/loop` の合成だけ実装（cleanup/Invoke は後段）
3. 既存 route のうち 1 本だけ `Frag` 合成に寄せる（BoolPredicateScan 推奨。legacy numbered label is traceability-only）
4. 2 本目で再利用が見えたら “番号ラベルでの枝刈り” を削って合成側へ寄せる
...
- `compose::if_`: then/else が **Normal exit** で step/merge に合流する形（SplitScan route）
- `compose::cleanup`: main frag に cleanup frag（Return/Normal）を合成して早期離脱を明示する形（ScanWithInit route）
- どちらも **Frag の validity を前提** にし、Normalizer が入口で妥当性を保証する
- ScanWithInit/SplitScan contract SSOT（legacy numbered labels are traceability-only）: `docs/development/current/main/design/pattern6-7-contracts.md`

補足（Phase 270）:
- simple_while_minimal（legacy numbered label is traceability-only）は test-only stub のため、一般ループの “基準” には使えない。
- Phase 270 では “最小の固定形” を AccumConstLoop（legacy numbered label is traceability-only）として追加し、後で Frag 合成側へ吸収される前提で橋渡しにする。

## Bridge routes（legacy label: bridge patterns）（撤去条件SSOT）

ここで言う “bridge route” は、既存の JoinIR ルートを壊さずに **最小の固定形を先に通す**ための一時レイヤ。
（例: Phase 270 の legacy bridge label for AccumConstLoop）

- 原則:
  - bridge route（legacy label: bridge pattern）は **汎用化しない**（固定形SSOT + fixture/smoke で仕様を固定するだけ）。
  - 将来は `Frag/ExitKind` 合成側へ **吸収して削除**する前提で追加する。

### Bridge contract（テンプレ / SSOT）

bridge route（legacy label: bridge pattern）を追加する場合は、最低限この “撤去条件” を先に書く（書けないなら追加しない）。

- **固定する fixture/smoke（SSOT）**
  - fixture（最小）と smoke（integration）を必ず紐づける
  - 「何が通れば撤去できるか」を machine-checkable にする
- **置換先（吸収先）の SSOT がある**
  - 番号ラベル列挙（legacy numbered label）の反対側に、必ず “吸収先” を書く（例: `Frag/ExitKind` 合成、もしくは emission 入口）
  - 吸収先が未確定な場合でも “層” は確定させる（legacy pattern層にロジックを増やさない）
- **撤去条件（最低限）**
  1. 置換先（吸収先）で同じ fixture/smoke が PASS する
  2. bridge route（legacy label: bridge pattern）依存の分岐が router から消せる（最小差分で削除できる）
  3. quick/integration の FAIL 位置が悪化しない（既知Failは増やさない）
- **撤去手順（最小）**
  - router から bridge route（legacy label: bridge pattern）を外す
  - fixture/smoke（+ quick）で PASS 維持
  - ファイル削除（または historical へ隔離）し、SSOT から参照を外す

### Phase 271: `AccumConstLoop` bridge route 撤去条件（SSOT）
...
- **固定 fixture/smoke**
  ...
  - smoke: `tools/smokes/v2/profiles/integration/apps/archive/phase270_p0_loop_min_const_vm.sh`
- **吸収先（層）**
  - Structured→CFG lowering 層（`Frag/ExitKind` 合成）またはその emission 入口（legacy pattern層は extractor に縮退）
- **撤去条件**
  1. 上記 fixture/smoke が、bridge route（legacy label: bridge pattern）を使わない経路で PASS する（Frag/emit_frag 側で loop を構築できる）
  2. legacy numbered label が router から削除されても coverage が落ちない（同 fixture が同じルートで通る）
  3. `tools/smokes/v2/run.sh --profile quick` が悪化しない
- **撤去手順**
  - legacy numbered label の router 分岐を削除 → smoke PASS → 実装を削除（または historical 化）
...
- **Route Preparation**: ScanWithInit/SplitScan hand-rolled locations documented for Phase 281 migration (legacy numbered labels are traceability-only)

**Phase 264 (歴史/別案件)**: Entry API Creation (別スコープ: BundleResolver loop fix)

- Note: Phase 264 は別案件（BundleResolver loop route fix; legacy wording: pattern fix）
...
- BoolPredicateScan（legacy numbered label is traceability-only）適用: P0ではやらない（偽Frag回避、P1から実戦投入）
...
Phase 267: Scan/Split/Predicate routes への展開（legacy numbered labels are traceability-only）
- ScanWithInit を Frag 化
- SplitScan を Frag 化
- BoolPredicateScan を Frag 化
- 再利用性の確認（番号ラベル分岐の削減）

現時点では既存 ScanWithInit/SplitScan/BoolPredicateScan 実装（legacy numbered labels are traceability-only）や merge/EdgeCFG は未改変（合成能力の証明のみ）。
...
Phase 267: JoinIR route への適用（legacy pattern labels are traceability-only）
- NormalizedShadow への Frag 適用
- ScanWithInit/SplitScan/BoolPredicateScan を Frag 化
- Branch 生成 + 番号ラベル分岐削減
- fixture + smoke test
