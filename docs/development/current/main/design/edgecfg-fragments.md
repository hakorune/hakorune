...
- 「番号ラベル（legacy label: PatternX）で推測分岐」が増殖しやすい領域（長期的には消したい）

この文書は「番号ラベル列挙（legacy label: PatternX）」を設計の中心にしないために、Structured→CFG の lowering を
**合成代数（fragment composition）**として SSOT 化する。

結論（本書の北極星）:

- “分岐の中心” は番号ラベル（legacy label: PatternX）ではなく **ExitKind** と **Frag（fragment）** に置く
...
#### Tier 1: Normalizer (Route-Specific; legacy labels: PatternX)

**Responsibilities**:
- Allocates `BasicBlockId` (`builder.next_block_id()`)
- Allocates `ValueId` (`builder.next_value_id()`)
- Knows route semantics (scan, split, etc.; legacy pattern labels may appear)
- Constructs initial `Frag` with valid blocks/values

**Example** (ScanWithInit route; legacy label: Pattern6):
```rust
...
```
ScanWithInit/SplitScan Normalizer (legacy labels: Pattern6/7)
...
- **ScanWithInit/SplitScan extractor（legacy labels: Pattern6/7）**: 形は近いが契約違反のケースは `Ok(None)` で流さず freeze（例: SplitScan の `else i = i + 1` 破り、ScanWithInit の `i = i + 1` 破り）

## 直近の導入ステップ（最小で始める）

1. `Frag/ExitKind/EdgeStub` の型を追加（docs+code 入口 SSOT）
2. `seq/if/loop` の合成だけ実装（cleanup/Invoke は後段）
3. 既存 route のうち 1 本だけ `Frag` 合成に寄せる（BoolPredicateScan 推奨、legacy label: Pattern8）
4. 2 本目で再利用が見えたら "番号ラベル（legacy label: PatternX）での枝刈り" を削って合成側へ寄せる
...
- `compose::if_`: then/else が **Normal exit** で step/merge に合流する形（SplitScan route、legacy label: Pattern7）
- `compose::cleanup`: main frag に cleanup frag（Return/Normal）を合成して早期離脱を明示する形（ScanWithInit route、legacy label: Pattern6）
- どちらも **Frag の validity を前提** にし、Normalizer が入口で妥当性を保証する
- ScanWithInit/SplitScan contract SSOT（legacy labels: Pattern6/7）: `docs/development/current/main/design/pattern6-7-contracts.md`

補足（Phase 270）:
- simple_while_minimal（legacy label: Pattern1）は test-only stub のため、一般ループの “基準” には使えない。
- Phase 270 では “最小の固定形” を AccumConstLoop（legacy label: Pattern9）として追加し、後で Frag 合成側へ吸収される前提で橋渡しにする。
...
- **Route Preparation**: ScanWithInit/SplitScan hand-rolled locations documented for Phase 281 migration (legacy labels: Pattern6/7)
...
Phase 267: Scan/Split/Predicate routes への展開（legacy labels: Pattern6/7/8）
- ScanWithInit（legacy label: Pattern6）を Frag 化
- SplitScan（legacy label: Pattern7）を Frag 化
- BoolPredicateScan（legacy label: Pattern8）を Frag 化
- 再利用性の確認（番号ラベル分岐の削減）
...
Phase 267: JoinIR route への適用（legacy label: Pattern）
- NormalizedShadow への Frag 適用
- ScanWithInit/SplitScan/BoolPredicateScan（legacy labels: Pattern6/7/8）を Frag 化
- Branch 生成 + 番号ラベル分岐削減（legacy label: pattern番号分岐）
- fixture + smoke test
