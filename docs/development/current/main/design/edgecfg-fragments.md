...
Legacy pattern labels (Pattern1-9+) are **symptom labels** for regression tests, NOT architectural concepts.
...
**Example** (ScanWithInit route; legacy label `Pattern6` is traceability-only):
```rust
...
```
ScanWithInit/SplitScan route builder (legacy labels `Pattern6/7` are traceability-only)
...
- **ScanWithInit/SplitScan extractor**: 形は近いが契約違反のケースは `Ok(None)` で流さず freeze（legacy labels `Pattern6/7` are traceability-only; 例: SplitScan の `else i = i + 1` 破り、ScanWithInit の `i = i + 1` 破り）

## 直近の導入ステップ（最小で始める）

1. `Frag/ExitKind/EdgeStub` の型を追加（docs+code 入口 SSOT）
2. `seq/if/loop` の合成だけ実装（cleanup/Invoke は後段）
3. 既存 route のうち 1 本だけ `Frag` 合成に寄せる（BoolPredicateScan 推奨。legacy label `Pattern8` is traceability-only）
...
- ScanWithInit/SplitScan contract SSOT（legacy labels `Pattern6/7` are traceability-only）: `docs/development/current/main/design/pattern6-7-contracts.md`

補足（Phase 270）:
- simple_while_minimal（legacy label `Pattern1` is traceability-only）は test-only stub のため、一般ループの “基準” には使えない。
- Phase 270 では “最小の固定形” を AccumConstLoop（legacy label `Pattern9` is traceability-only）として追加し、後で Frag 合成側へ吸収される前提で橋渡しにする。
...
- **置換先（吸収先）の SSOT がある**
  - 番号ラベル列挙（legacy label: PatternX）の反対側に、必ず “吸収先” を書く（例: `Frag/ExitKind` 合成、もしくは emission 入口）
...
### Phase 271: `Pattern9_AccumConstLoop` 撤去条件（SSOT, route name: AccumConstLoop）
...
- **撤去条件**
  ...
  2. legacy label `Pattern9` が router から削除されても coverage が落ちない（同 fixture が同じルートで通る）
  3. `tools/smokes/v2/run.sh --profile quick` が悪化しない
- **撤去手順**
  - legacy label `Pattern9` の router 分岐を削除 → smoke PASS → 実装を削除（または historical 化）
...
- **Route Preparation**: ScanWithInit/SplitScan hand-rolled locations documented for Phase 281 migration (legacy labels `Pattern6/7` are traceability-only)
...
- BoolPredicateScan（legacy label: Pattern8）適用: P0ではやらない（偽Frag回避、P1から実戦投入）
...
Phase 267: Scan/Split/Predicate routes への展開（legacy labels `Pattern6/7/8` are traceability-only）
- ScanWithInit を Frag 化
- SplitScan を Frag 化
- BoolPredicateScan を Frag 化
- 再利用性の確認（番号ラベル分岐の削減）

現時点では既存 ScanWithInit/SplitScan/BoolPredicateScan 実装（legacy labels: pattern6/7/8）や merge/EdgeCFG は未改変（合成能力の証明のみ）。
