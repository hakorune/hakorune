...
- LLVM pure-first unsupported-shape inventory (dev/diagnostic only, `NYASH_LLVM_ROUTE_TRACE=1`): `[llvm-pure/unsupported-shape] recipe=pure-first first_block=<n|-> first_inst=<n|-> first_op=<op|unknown> owner_hint=<mir_normalizer|core_method_contract|backend_lowering|unknown> reason=<no_lowering_variant|...> target_shape_reason=<-|...> target_shape_blocker_symbol=<-|...> target_shape_blocker_reason=<-|...>`. This is a boundary inventory hint only; it must not become C-side shape policy.
