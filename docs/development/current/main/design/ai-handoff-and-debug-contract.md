...
- `.hako ll emitter` owner evidence (always emitted by narrow daily/compare bridge): `[hako-ll/<daily|compare>] chosen_owner=hako-ll-min-v0 accepted=<min-v0|none> first_blocker=<...> acceptance_case=<...> legacy_daily_allowed=<yes|no|unknown>`
...
- LLVM optimization trace (dev/diagnostic only, `NYASH_LLVM_ROUTE_TRACE=1`): `[llvm-route/trace] stage=<generic_walk|mir_call|mir_call_method|generic_method_has_route|lowering_plan|array_rmw_window|array_string_len_window|exact_seed_backend_route> result=<seen|hit|miss> reason=<...> extra=<...>`. Array-string `indexOf` boundary smokes use `stage=mir_call_method reason=indexOf ... string_indexof:1` plus IR helper checks; do not depend on retired exact-window trace tags.
- LLVM pure-first unsupported-shape inventory (dev/diagnostic only, `NYASH_LLVM_ROUTE_TRACE=1`): `[llvm-pure/unsupported-shape] recipe=pure-first first_block=<n|-> first_inst=<n|-> first_op=<op|module_function|unknown> owner_hint=<mir_normalizer|core_method_contract|backend_lowering|unknown> reason=<no_lowering_variant|module_generic_prepass_failed|module_generic_body_emit_failed|...> target_return_type=<-|...> target_shape_reason=<-|...> target_shape_blocker_symbol=<-|...> target_shape_blocker_reason=<-|...>`. This is a boundary inventory hint only; it must not become C-side shape policy.
- Env alias deprecation warning (warn-once, no debug toggle): `[deprecate/env] 'HAKO_CAPI_PURE' is deprecated; use 'HAKO_BACKEND_COMPILE_RECIPE=pure-first'`. This is the P99 retire-target signal; it must not change route selection behavior.
- Env alias retired fail-fast (always-on contract): `[freeze:contract][env/hako_capi_pure_retired] HAKO_CAPI_PURE is retired; use HAKO_BACKEND_COMPILE_RECIPE=pure-first`. This is the P100 route-selector removal signal for C generic FFI and `ny-llvmc` boundary driver callers.
- Program(JSON v0) import-bundle trace (dev/diagnostic only, `NYASH_JSON_V0_IMPORT_TRACE=1`): summary lines are info-level (`phase=<enter|skip|merge.done|fail>`) while detail lines remain debug-only (`phase=merge.begin|guard.set|restore`). Use this route map: `runner::pipe_io::try_run_json_v0_pipe -> core_executor::execute_json_artifact -> json_artifact::load_json_artifact_to_module -> program_json_v0_loader::load_program_json_v0_to_module -> compile_program_json_v0_imports_bundle`.
...
- Contract freeze (ScanWithInit/SplitScan): `[joinir/phase29ab/scan_with_init/contract]`, `[joinir/phase29ab/split_scan/contract]`
...
- `[plan/trace:if_phi_normalize]`
  - Purpose: Inspect IfPhiJoin (route=if_phi_join) normalizer input when a variable is missing.
...
- `[plan/trace:facts_summary]`
  ...
  - Output fields (1 line): `ctx=<...> scan_methods=<0|1> loop_scan_phi_vars=<0|1>`
