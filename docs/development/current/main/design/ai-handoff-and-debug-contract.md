...
- LLVM optimization trace (dev/diagnostic only, `NYASH_LLVM_ROUTE_TRACE=1`): `[llvm-route/trace] stage=<generic_walk|mir_call|mir_call_method|generic_method_has_route|array_rmw_window|array_string_len_window|exact_seed_backend_route|array_string_indexof_select_window|array_string_indexof_branch_window|array_string_indexof_cross_block_select_window|array_string_indexof_interleaved_branch_window|array_string_indexof_interleaved_select_window> result=<seen|hit|miss> reason=<...> extra=<...>`
