...
- LLVM optimization trace (dev/diagnostic only, `NYASH_LLVM_ROUTE_TRACE=1`): `[llvm-route/trace] stage=<generic_walk|mir_call|mir_call_method|generic_method_has_route|array_rmw_window|array_string_len_window|exact_seed_backend_route> result=<seen|hit|miss> reason=<...> extra=<...>`. Array-string `indexOf` boundary smokes use `stage=mir_call_method reason=indexOf ... string_indexof:1` plus IR helper checks; do not depend on retired exact-window trace tags.
