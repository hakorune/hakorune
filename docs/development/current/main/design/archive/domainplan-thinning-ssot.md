...
- LoopSimpleWhile: planner_required で recipe-only 化済み
- LoopCharMap（legacy numbered label is traceability-only）: planner_required で recipe-only 化済み
- LoopArrayJoin（legacy numbered label is traceability-only）: planner_required で recipe-only 化済み
- BoolPredicateScan（legacy numbered label is traceability-only）: planner_required で recipe-only 化済み
- AccumConstLoop（legacy numbered label is traceability-only）: planner_required で recipe-only 化済み
