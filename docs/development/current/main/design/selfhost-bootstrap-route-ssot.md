...
- G1 full is now green again on the same authority contract: `Program(JSON v0)` stays raw-exact and `MIR JSON v0` passes a narrow semantic canonical compare while raw exact diff remains diagnostics evidence
- therefore there is no active reduced-case blocker on the authority route itself; next work is to choose the next non-green widening target without changing the current authority contract
...
- Latest evidence (2026-03-11):
  ...
  - `tools/selfhost_identity_check.sh --mode full --skip-build --bin-stage1 target/selfhost/hakorune.stage1_cli --bin-stage2 target/selfhost/hakorune.stage1_cli.stage2` PASS (`Program JSON v0` raw match; `MIR JSON v0` canonical match, raw diff retained as diagnostics)
