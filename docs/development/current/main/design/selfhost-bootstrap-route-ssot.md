...
- `build_stage1.sh` には experimental `stage1-cli bridge-first` bootstrap path を追加済みで、`stage1_cli_env.hako` の Program(JSON) helper defs は current source では materialize する。ただし `emit-mir` は still `96` で fail し、`HAKO_STAGE1_MODULE_DISPATCH_TRACE=1` でも `lang.mir.builder.MirBuilderBox.emit_from_program_json_v0` hit の後に success/error payload が返らない。一方で same Program(JSON) payload は direct kernel/plugin proof では MIR(JSON) handle を返す。したがって次の exact blocker は internal-only MirBuilder shape acceptance だけでなく、stage1 child/plugin return-path bridge を含む bridge semantics であり、G1 の current contract は default bootstrap keep のまま維持する
