...
- `build_stage1.sh` には experimental `stage1-cli bridge-first` bootstrap path を追加済みで、`stage1_cli_env.hako` の Program(JSON) helper defs は current source では materialize し、`stage1_contract_exec_mode ... emit-mir ...` も current contract では green になった。`HAKO_STAGE1_MODULE_DISPATCH_TRACE=1` では `lang.mir.builder.MirBuilderBox.emit_from_program_json_v0` hit の後に `output_bytes=213003` / `output_handle=97` が返る。さらに bridge/runtime extern-like names (`env.*`, `nyash.*`) は `HAKO_MIR_BUILDER_CALL_RESOLVE` なしでも `Callee::Extern` に分類される。一方で bridge-first Stage2 build は still `ny-llvmc` の `Instruction does not dominate all uses!` で fail し、同じ `stage1_cli_env.hako` Program(JSON v0) を Rust mainline `--program-json-to-mir` へ流した direct MIR でも同じ failure を再現する。したがって次の exact blocker は stage1 child/plugin return-path bridge ではなく、helper-heavy `Program(JSON)->MIR` lowering quality / dominance であり、とくに `llvm_py` の join-value / PHI wiring (`function_lower.py`, `block_lower.py`, `phi_wiring/tagging.py`, `utils/values.py`, `instructions/binop.py`) が次の exact owner になる。G1 の current contract は default bootstrap keep のまま維持する
...
- Latest evidence (2026-02-11):
  - `tools/selfhost_identity_check.sh --mode full --skip-build --bin-stage1 target/selfhost/hakorune.stage1_cli --bin-stage2 target/selfhost/hakorune.stage1_cli.stage2` PASS
