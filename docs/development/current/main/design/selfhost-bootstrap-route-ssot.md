...
- source-only authority case では `stage1_cli_env.hako` が `MirBuilderBox.emit_from_source_v0(...)` を直接使い、explicit supplied Program(JSON) text-only input がある時だけ `MirBuilderBox.emit_from_program_json_v0(...)` を compatibility input shape として残す
