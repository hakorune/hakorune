...
File-level responsibility note:
- `BuildBox.emit_program_json_v0(...)` is the current `source -> Program(JSON v0)` authority
- `compiler_stageb.hako` remains a Stage-B emit/adapter lane, not a second authority owner
- `launcher.hako` is a stage1 CLI facade/orchestration keep, not a bootstrap authority route
- exact file-level keep/shrink/facade policy is owned by
...
- archived exact probe では `target/selfhost/hakorune.stage1_cli` の raw direct contract (`emit ...` / `--emit-mir-json`) は `97` を返す。`tools/archive/legacy-selfhost/engineering/phase29ch_raw_direct_stage1_cli_probe.sh` はその diagnostics-only evidence であり、live authority route ではない。`tools/selfhost/compat/run_stage1_cli.sh` は raw `emit program-json` / `emit mir-json` surface を env contract に変換する compatibility wrapper であり、新しい authority route ではない
...
- `stage1_cli_env.hako::Stage1SourceProgramAuthorityBox` now owns the exact source-only `emit-program` authority handoff: source text coercion and the checked `BuildBox.emit_program_json_v0(...)` call stay there, output validation stays in `Stage1ProgramResultValidationBox`, and `using` import ownership stays in the Stage1 Program(JSON v0) Rust authority (`imports` map) rather than a same-file prefix merge
- `stage1_cli_env.hako::Stage1ProgramJsonMirCallerBox` isolates the checked `MirBuilderBox.emit_from_program_json_v0(...)` handoff leaf; explicit Program(JSON) compat performs caller-local input guard/coercion before entering that leaf, and `Stage1ProgramAuthorityBox` is historical and no longer part of the current contract
...
- source-only authority case では `stage1_cli_env.hako::Stage1SourceMirAuthorityBox` が `MirBuilderBox.emit_from_source_v0(...)` を担当し、explicit supplied Program(JSON) text-only input がある時だけ `Stage1EmitMirDispatchBox.run_emit_mir_program_json_compat_mode/1` が guard 後に `Stage1ProgramJsonMirCallerBox` handoff leaf を再利用する
- `stage1_cli_env.hako::Stage1ProgramJsonTextGuardBox` keeps the shared Program(JSON) text predicate/coercion in front of both Program(JSON) paths; missing-input null returns stay caller-local so Stage0 does not need a shared `String|Void` guard body
...
| `--emit-program-json-v0` | raw Program(JSON v0) compat emit | public compat/deprecated | keep until smoke/tool caller inventory reaches zero; current shell syntax is owned by `tools/lib/program_json_v0_compat.sh` |
...
| --- | --- | --- | --- | --- | --- |
| `SH-GATE-STAGEB` | explicit proof/debug gate の Stage-B コンパイル | `tools/selfhost/proof/run_stageb_compiler_vm.sh --source-file <fixture>` | fixture source (`HAKO_SRC`) | Program(JSON v0) line on stdout | `tools/selfhost/run.sh --gate --planner-required 1` |
| `SH-RUNTIME-SELFHOST` | ランナー経由 selfhost pipeline（opt-in） | `tools/selfhost/run.sh --runtime --runtime-route <compat|mainline>` | `.ny/.nyash` source file | Program(JSON v0) -> MIR/VM execution | runtime execution + fast gate |
| `SH-DIRECT-MIR-RUN` | selfhost_build normal run route | `tools/selfhost/selfhost_build.sh --in <src.hako> --run` | Hako source | direct MIR(JSON) -> `--mir-json-file` exit code | quick selfhost return/binop smokes |
| `SH-JSONRUN` | JSON payload 実行 | `<nyash> --json-file <json>` | Program(JSON v0) or MIR(JSON) | VM exit code / stdout | gate stage2-mainline run step |

運用ルール:
- route は stderr に安定タグで観測する（stdout は比較対象を汚さない）。
- route 追加/変更時は parity smoke も同コミットで更新する。
- `--runtime-mode exe|stage-a-compat` と `stage-a` は compatibility alias として残すが、current SSOT は `--runtime-route mainline|compat` を使う。
- PyVM は historical / opt-in 扱い。runtime/pipe route の既定導線には使わない（詳細: `docs/development/current/main/design/archive/pyvm-retreat-ssot.md`）。
...
5) **Runner/VM execution**
   ...
   - JoinIR VM bridge は `vm-reference` build の explicit `--backend vm` で
     ...
     はactivation条件ではない。
   - Exec route は出力・exitし得る。dev/trace成功、LowerOnly、非strict Exec
     failure の通常VM継続は explicit compatibility behavior として扱う。
...
- `stage2-mainline` is the daily mainline/distribution reading; `stage2+` stays the target umbrella/distribution reading, not a current artifact-kind or wrapper family
...
- Stage2-mainline が daily selfhost mainline として読めること。Stage0 Rust bootstrap は acceptance failure ではなく keep lane として扱う
