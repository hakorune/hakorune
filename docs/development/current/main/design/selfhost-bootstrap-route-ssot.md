...
- supplied Program(JSON) compat path の shell-side SSOT は `tools/selfhost/lib/stage1_contract.sh` (`stage1_contract_run_bin_with_env`) -> `tools/selfhost/run_stage1_cli.sh` (`--from-program-json`) -> `tools/selfhost/lib/identity_routes.sh` (`run_stage1_subcmd_mir_program_compat_route`) で固定する。fresh compiled Stage1/Stage2 artifact の diagnostics probe (`tools/dev/phase29ch_program_json_compat_route_probe.sh`) は現在どちらも `stage1-env-mir-program` を返すため、`stage1-env-mir-legacy` と `stage1-subcmd-mir-program` は cold compat keep として扱う。逆に `tools/dev/phase29ch_program_json_text_only_probe.sh` は fresh Stage1/Stage2 の両方でまだ nonzero のため、remaining compat resolver の中では `*_PROGRAM_JSON` path lane が live である
