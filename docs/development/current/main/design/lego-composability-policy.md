...
- composer の allowlist 例外（`shadow_adopt` など）は、エラーメッセージに legacy route label / `planner_tag` を含める（罠を即特定）。
...
- **Phase 3 完了**: historical planner-payload-era variant を統合（Cluster3/4/5 → LoopCondBreakContinue）
