...
- Note: VarLessLiteral is used for Pattern9 observation (Var < integer literal).
...
- Note: D18 applies incomplete-freeze to Pattern1CharMap only (others keep fallback).
- Note: D19 applies incomplete-freeze to Pattern1ArrayJoin.
- Note: D20 applies incomplete-freeze to Pattern8/9.
...
- Rule: CondCanon → CondProfile is observation-only (no rewrite).
 ...
 - Storage: scan facts carry `cond_profile` for Pattern1CharMap/ArrayJoin/Pattern8/Pattern9 (C20-B).
