...
1. mainline:
   ...
   - caller-side recipe seam now lives in `lang/src/shared/backend/backend_recipe_box.hako`; it now pins `HAKO_BACKEND_COMPILE_RECIPE=pure-first` and `HAKO_BACKEND_COMPAT_REPLAY=harness` before the daily `env.codegen.compile_json_path(...)` handoff, so pure-first policy is owned by caller-side recipe names rather than by the C shim
