...
1. mainline:
   ...
   - `compile_route_profile(...)` also owns the visible `acceptance_case` label for the current narrow evidence row (`runtime-data-array-get-missing-v1`), so the shape-specific reason stays in `.hako` instead of drifting into the C shim
