...
1. mainline:
   ...
   - recipe seam close-sync and `W2a..W2c` inside boundary fallback reliance reduction are now landed; the current exact backend-zero front is `W3` Rust glue thinning, not further recipe widening
