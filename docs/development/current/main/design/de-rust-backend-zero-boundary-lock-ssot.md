...
1. mainline:
   ...
   - caller-side recipe seam now lives in `lang/src/shared/backend/backend_recipe_box.hako`; it pins `HAKO_CAPI_PURE=1` before the daily `env.codegen.compile_json_path(...)` handoff, so pure-first policy is owned by `.hako` callers rather than by the C shim
