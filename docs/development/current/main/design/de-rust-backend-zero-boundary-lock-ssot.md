...
1. mainline:
   ...
   - recipe seam close-sync, `W2a..W2c`, `W3a`, and `W3b` are now landed; the current exact backend-zero front is `W3c` generic compile symbol branch keep-only lock, not further recipe widening
   - `crates/nyash-llvm-compiler/src/boundary_driver_ffi.rs` now delegates compile symbol selection and FFI library candidate resolution into `crates/nyash-llvm-compiler/src/boundary_driver_defaults.rs`, so the FFI file is closer to transport-only glue
