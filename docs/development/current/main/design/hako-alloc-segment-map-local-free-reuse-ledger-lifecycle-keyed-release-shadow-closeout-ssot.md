# Hako Alloc Segment Map Local Free Reuse Ledger Lifecycle-Keyed Release Shadow Closeout SSOT
...
MIMAP-224A remains the daily L2 behavior row. MIMAP-226A only proves that the
same MIR artifact used for preflight can lower to an executable and produce the
same lifecycle-keyed release shadow output.

## Pack

```text
segment-map-local-free-reuse-ledger-lifecycle-keyed-release-shadow
```
...
- dry-run the closeout pack selection and find MIMAP-224A;
- run the MIMAP-224A L2 guard;
- emit one MIR JSON artifact and use that exact artifact for EXE build;
- prove VM and EXE output both keep lifecycle-keyed release shadow behavior and
