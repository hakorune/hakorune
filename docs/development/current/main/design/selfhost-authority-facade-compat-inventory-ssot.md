...
| `lang/src/compiler/entry/compiler_stageb.hako` | Stage-B emit entry | `shrink to adapter` | Stage-B entry no longer owns parser/body/defs/import shaping; output/error handling lives in `StageBOutputBox`; disabled FuncScan no-op harness is removed | keep thin; reopen only for a concrete Stage-B entry blocker |
