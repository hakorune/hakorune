...
- DomainPlan は **翻訳層（label only）** として扱う。意味論は持たない。
- 新しい受理形は **RecipeBlock/RecipeTree を先に定義**する（Recipe-first）。
- DomainPlan 追加は “Recipe を使うためのラベル” に留める。
- Lower は VerifiedRecipe のみを受け取る（未検証 Recipe の直lowerは禁止）。

### DomainPlan → CorePlan entry order (SSOT)

DomainPlan と CorePlan が併存していても “入口順序” が揺れると、観測/検証/受理が二重化して収束しない。
ここでは “どの入口を優先するか” を SSOT として固定する。

Rule:
- **Primary**: RecipeComposer/RecipeTree 経路（Recipe-first）。
- **Secondary (legacy)**: PlanNormalizer 経路（互換/残骸として限定）。新規に依存を増やさない。

Allowed Normalizer residue（現状の限定リスト）:
- pattern1 / pattern5 / pattern8 / pattern9 / scan_with_init / split_scan

Drift checks:
- `rg -n "PlanNormalizer::normalize_" src/mir/builder/control_flow/plan --glob '!*.md'`（依存が増えていないこと）
- `rg -n "DomainPlan::" src/mir/builder/control_flow/plan/recipe_tree --glob '!*.md'` → 0（RecipeTree は label-only を持たない）
...
AST
  ...
  → Facts (観測のみ)
  → Planner (Facts → DomainRecipe)
  → RecipeVerifier (fail-fast: [freeze:contract])
  → Lowerer (VerifiedRecipe only)
  → emit_frag
  → verify / DCE
...
### Minimal type plan (design-only, no code yet)
...
- ✅ M6-min9: `features/{pattern5_infinite_early_exit_pipeline,scan_with_init_pipeline,split_scan_pipeline,split_emit,loop_cond_co_block,loop_cond_bc_nested_carriers}` の型境界（戻り型/参照）を `LoweredRecipe` へ段階移行した（挙動不変）。
  - Drift check: `rg -n "\\[CorePlan\\]|&CorePlan|Vec<CorePlan>|Result<.*CorePlan|Option<Vec<CorePlan>>" src/mir/builder/control_flow/plan/features/pattern5_infinite_early_exit_pipeline.rs src/mir/builder/control_flow/plan/features/scan_with_init_pipeline.rs src/mir/builder/control_flow/plan/features/split_scan_pipeline.rs src/mir/builder/control_flow/plan/features/split_emit.rs src/mir/builder/control_flow/plan/features/loop_cond_co_block.rs src/mir/builder/control_flow/plan/features/loop_cond_bc_nested_carriers.rs` → 0件
...
- ✅ M6-min19: `normalizer/mod.rs` と `normalizer/pattern*.rs` の型境界（戻り型/Vec境界）を `LoweredRecipe` へ段階移行した（挙動不変）。
  - Drift check: `rg -n "\\[CorePlan\\]|&CorePlan|Vec<CorePlan>|Result<.*CorePlan|Option<CorePlan>|Option<Vec<CorePlan>>" src/mir/builder/control_flow/plan/normalizer/mod.rs src/mir/builder/control_flow/plan/normalizer/pattern*.rs --glob '!**/tests.rs' --glob '!*.md'` → 0件
