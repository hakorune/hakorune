...
- ✅ M6-min17: `loop_{scan_v0,scan_methods_v0,scan_phi_vars_v0,bundle_resolver_v0}/pipeline` の型境界（戻り型/Vec境界/参照境界）を `LoweredRecipe` へ段階移行した（挙動不変）。`loop_scan_methods_block_v0` は `COREPLAN-E1-002`、`loop_collect_using_entries_v0` は `COREPLAN-E1-003` で retired。
  - Drift check: `rg -n "\\[CorePlan\\]|&CorePlan|Vec<CorePlan>|Result<.*CorePlan|Option<Vec<CorePlan>>" src/mir/builder/control_flow/plan/loop_scan_v0/pipeline.rs src/mir/builder/control_flow/plan/loop_scan_methods_v0/pipeline.rs src/mir/builder/control_flow/plan/loop_scan_phi_vars_v0/pipeline.rs src/mir/builder/control_flow/plan/loop_bundle_resolver_v0/pipeline.rs` → 0件
  - Note: `CorePlan::Loop` / `CorePlan::Exit` / `CorePlan::Effect` の生成と match は維持（名前置換は M6 後段）
