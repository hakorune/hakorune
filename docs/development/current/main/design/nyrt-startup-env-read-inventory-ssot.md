...
| `src/config/env/stage1.rs` | Stage-1 / selfhost CLI env helpers | `control surface helper` | shared stage1 helper + direct env presence check for `NYASH_NYRT_SILENT_RESULT` | Shared owner for the output-only toggle and the post-`ny_main` metrics cluster. `NYASH_NYRT_SILENT_RESULT`, `NYASH_GC_METRICS_JSON`, `NYASH_GC_METRICS`, and the GC threshold readers are centralized here so the NyRT tail and Stage-1 bridge defaults share the same helper path. |
