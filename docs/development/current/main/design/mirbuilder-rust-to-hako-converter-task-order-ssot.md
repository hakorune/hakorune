...
latest design decision:
  3040 lands the covered EnumMatchShapeSnapshotV1 ProgramJSON traversal slice
  as a scoped Rust ASTNode projector retire-candidate. Match lowering, enum
  runtime semantics, full projector retirement, ProgramJSON full parser,
  HakoAdoption, Source Selfhost, RecipeMatcher, route selection, lowering,
  mutation, ID allocation, and new ABI remain unclaimed.
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-ENUM-MATCH-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=EnumMatch traversal, no match lowering

2. MIRBUILDER-PROGRAMJSON-ENUM-MATCH-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
