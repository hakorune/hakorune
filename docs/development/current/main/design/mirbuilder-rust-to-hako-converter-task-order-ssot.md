...
normal operating rule:
  ...
  `try_extract_loop_skeleton_facts` now has green authority-facade parity; next
  is its scoped HakoAdopted decision.
...
latest design decision:
  `loop_skeleton_facts.authority_facade` has green 3-row parity for
  SkeletonFacts `{ kind=Loop, feature_slots=[] }`; broad crate split,
  feature-slot inference, full AST traversal, lowering, mutation, allocation,
