...
latest design decision:
  3024 lands the covered ExprCompareShapeSnapshotV1 ProgramJSON traversal rows
  as a scoped Rust ASTNode projector retire-candidate. 3025 is active to
  implement real `.hako` ProgramJsonExprLogicalShapeScanV1 traversal for
  covered Expr statement Logical ProgramJSON structure.
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-EXPR-LOGICAL-SHAPE-SCAN-CAPABILITY-001
   status=active; boundary=.hako implementation + fixture + AOT parity gate

2. MIRBUILDER-PROGRAMJSON-EXPR-LOGICAL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=pending; boundary=covered ExprLogicalShapeSnapshotV1 rows only
