...
latest design decision:
  3041 lands real `.hako` ProgramJsonLocalStringLiteralShapeScanV1 traversal
  for covered Local.expr string literal ProgramJSON structure with a 9-row AOT
  parity gate. 3042 is active to mark only covered
  LocalStringLiteralShapeSnapshotV1 rows as Rust ASTNode projector
  retire-candidate.
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-ENUM-MATCH-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=landed; boundary=covered EnumMatchShapeSnapshotV1 rows only

2. MIRBUILDER-PROGRAMJSON-LOCAL-STRING-LITERAL-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=Local.expr string traversal, no StringBox materialization

3. MIRBUILDER-PROGRAMJSON-LOCAL-STRING-LITERAL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=active; boundary=covered LocalStringLiteralShapeSnapshotV1 rows only
