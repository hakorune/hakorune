...
normal operating rule:
  ...
  `try_extract_loop_simple_while_facts` now has green token-snapshot parity; next is its scoped HakoAdopted decision.
...
latest design decision:
  `loop_simple_while_facts.backend_safe_token_snapshot_reducer` has green 13-row parity for simple while fact observation; recipe construction, loop builder composition, lowering, mutation, allocation, and Source Selfhost remain unclaimed. The HakoAdopted decision is next.
