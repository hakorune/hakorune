...
latest design decision:
  3093 lands real ProgramJsonExprMeShapeScanV1 ProgramJSON traversal for covered
  Expr.expr Me shapes with 8-row AOT parity. Next work marks only those rows as
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-EXPR-UNARY-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=real `.hako` ProgramJSON Expr.expr Unary traversal

2. MIRBUILDER-PROGRAMJSON-EXPR-UNARY-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=landed; boundary=only the covered ExprUnaryShapeSnapshotV1 rows

3. MIRBUILDER-PROGRAMJSON-EXPR-ME-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=real `.hako` ProgramJSON Expr.expr Me traversal

next active:
  MIRBUILDER-PROGRAMJSON-EXPR-ME-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
  status=active; boundary=covered expr-me snapshot retire-candidate only
