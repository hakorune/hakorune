...
selected next card:
  not-yet-created; mark expanded If Recipe DTO then-exit payload rows as a scoped Rust ASTNode projector retire-candidate.
...
latest design decision:
  3147 expands IfRecipeDtoSnapshotV1 with 6 then_item=Exit payload rows for the
  expanded Return-only vocabulary; parity is green.
  ...
  3144 marks expanded SeqRecipeDtoSnapshotV1 Return-only rows from 3143 as a
  scoped Rust ASTNode projector retire-candidate; covered rows are 6 and no
  rows are deferred.
  3143 expands SeqRecipeDtoSnapshotV1 with the 6 Return-only rows from the
  shape-kind expanded checkpoint; parity is green.
  3142 marks expanded RecipeShapeKindDtoSnapshotV1 rows as a scoped Rust
  ASTNode projector retire-candidate; covered rows are 6 and no rows are deferred.
  3141 expands RecipeShapeKindDtoSnapshotV1 with 6 Return-only rows for
  StringBox New, id() calls, and StringBox methods; parity is green.
  3140-3139 are the base RecipeShapeKindDtoSnapshotV1 parity and retire
  checkpoints; 3138-3135 are RecipeStmtSeq and RecipePortSig checkpoints.
