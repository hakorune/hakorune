...
latest design decision:
  3059 lands real `.hako` ProgramJsonLocalBlockExprShapeScanV1 ProgramJSON
  traversal for covered Local.expr BlockExpr shapes with 10-row AOT parity.
  3060 is active for the scoped LocalBlockExprShapeSnapshotV1 retire-candidate.
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-LOCAL-BLOCK-EXPR-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=real `.hako` ProgramJSON Local.expr BlockExpr traversal,
   no block expression lowering, prelude execution semantics, route selection, or MIR mutation

2. MIRBUILDER-PROGRAMJSON-LOCAL-BLOCK-EXPR-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=active; boundary=covered LocalBlockExprShapeSnapshotV1 rows only

3. MIRBUILDER-PROGRAMJSON-CAPABILITY-BATCH-CONTINUATION
   status=pending-after-3060; boundary=select and implement next concrete ProgramJSON traversal capability

next active:
  MIRBUILDER-PROGRAMJSON-LOCAL-BLOCK-EXPR-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
  status=active; boundary=mark only covered LocalBlockExprShapeSnapshotV1 rows as retire-candidate
