...
latest design decision:
  3039 lands real `.hako` ProgramJsonEnumMatchShapeScanV1 traversal for
  covered EnumMatch ProgramJSON structure with a 9-row AOT parity gate. 3040 is
  active to mark only covered EnumMatchShapeSnapshotV1 rows as Rust ASTNode
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-ASSIGN-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=landed; boundary=covered AssignShapeSnapshotV1 rows only

2. MIRBUILDER-PROGRAMJSON-ENUM-MATCH-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=EnumMatch traversal, no match lowering

3. MIRBUILDER-PROGRAMJSON-ENUM-MATCH-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=active; boundary=covered EnumMatchShapeSnapshotV1 rows only
