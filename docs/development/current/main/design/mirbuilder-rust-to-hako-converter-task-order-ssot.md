...
normal operating rule:
  ...
  `try_extract_loop_cond_return_in_body_facts` now has green authority-facade
  parity; shape-specific AST traversal, balanced-depth policy evaluation,
  recipe construction, lowering, mutation, allocation, and Source Selfhost
  remain unclaimed. Next is its scoped HakoAdopted decision.
...
latest design decision:
  ...
  unclaimed.
  `loop_cond_return_in_body_facts.authority_facade` has green 13-row parity
  with backend-safe token snapshots; shape-specific AST traversal,
  balanced-depth policy, recipe construction, lowering, mutation, allocation,
