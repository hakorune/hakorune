...
latest design decision:
  LoopCondContinueConditionNotSupportedRejectV1 /
  LoopCondContinueConditionNotSupportedRejectTokenSnapshotV1 is now a one-shape
  Rust ASTNode projector retire-candidate. The initial 5-row ProgramJSON
  snapshot parity queue is exhausted. Runtime dependency removal, full projector
  retirement, HakoAdoption, ProgramJSON full parser, and Source Selfhost remain
  unclaimed. Next work selects a new ProgramJSON traversal shape only if it
  reduces a Rust ASTNode projection slice or exposes a concrete missing HHako
