...
current implementation task:
  ...
  The call_resolution_env_interface_classifier owner is HakoAdopted as the
  one-hundred-twelfth narrow Rust-oracle parity pilot after the green 9-row
  `.hako` EXE parity gate; next is rerun 114.
...
selected next task:
  MIRBUILDER-HAKO-NATIVE-OWNER-PARITY-PILOT-SELECTION-RERUN-114
...
latest design decision:
  call_resolution_env_interface_classifier is now HakoAdopted as a narrow
  Rust-oracle parity pilot owner after a green 9-row `.hako` EXE parity gate.
  Call resolution, backend lowering, and MIR mutation remain Rust. The
