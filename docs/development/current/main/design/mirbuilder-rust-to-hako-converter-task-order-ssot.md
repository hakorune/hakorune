...
## Active Next 3
```text
landed summary: 3264..3313 cond_recipe/compare chain includes BoolRecipe sidecar bridge, ProgramJSON Loop/If cond_recipe producer wiring, verifier validate-only, matcher observe-only input snapshot, shared compare reader, If/Loop relational rows, BoolRecipe lowering observation/consultation, Var rhs reader parity, If/nested-If/top-level Loop Var rhs owner-direct rows, Var-rhs producer closeout, symbolic compare lowering command owner selection, intent-map symbolic command pilot, symbolic command parity, RHS materialization intent pilot/parity, RHS ValueId resolution owner selection, read-only RHS ValueId resolution plan pilot/parity, actual RHS ValueId resolution design-stop, request/response ABI, LiteralI64 constant emission bridge, SymbolRef lookup contract consultation, and SymbolRef contract parity.

next active:
  MIRBUILDER-COMPARE-RHS-SYMBOLREF-LOOKUP-CONTRACT-PARITY-001; status=landed; SymbolRefResolutionContractV1 read-only parity green for simple/renamed no-shadow locals; next=MIRBUILDER-COMPARE-RHS-SYMBOLREF-LOOKUP-BRIDGE-001; status=next; unit=Rust bridge + fixture + gate; only contract-verified no-shadow local rows may return existing ValueId; allocation=0; LocalSSA=0; MIR Compare/Branch=0; runtime_authority=rust_astnode
...
next_after_active_3 =
  MIRBUILDER-COMPARE-RHS-SYMBOLREF-LOOKUP-BRIDGE-001 -> MIRBUILDER-COMPARE-LOCALSSA-FINALIZE-COMPARE-DESIGN-STOP-001
