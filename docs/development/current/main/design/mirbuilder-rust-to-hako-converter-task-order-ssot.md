...
latest design decision:
  3051 lands real `.hako` ProgramJsonExitMarkerShapeScanV1 ProgramJSON
  traversal for covered Break/Continue exit-marker shapes with 10-row AOT
  parity. 3052 is active for the scoped ExitMarkerShapeSnapshotV1 Rust ASTNode
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-LOCAL-MAP-LITERAL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=landed; boundary=covered LocalMapLiteralShapeSnapshotV1 rows only

2. MIRBUILDER-PROGRAMJSON-EXIT-MARKER-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=real `.hako` ProgramJSON Break/Continue marker traversal,
   no CFG construction, exit lowering, route selection, or MIR mutation

3. MIRBUILDER-PROGRAMJSON-EXIT-MARKER-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=active; boundary=covered ExitMarkerShapeSnapshotV1 rows only
