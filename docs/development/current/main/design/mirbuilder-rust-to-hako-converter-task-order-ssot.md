...
## Active Next 3
```text
landed summary: 3264..3301 cond_recipe/compare chain includes BoolRecipe sidecar bridge, ProgramJSON Loop/If cond_recipe producer wiring, verifier validate-only, matcher observe-only input snapshot, shared compare reader, If/Loop relational rows, BoolRecipe lowering observation/consultation, Var rhs reader parity, If/nested-If/top-level Loop Var rhs owner-direct rows, Var-rhs producer closeout, symbolic compare lowering command owner selection, and intent-map symbolic command pilot.

next active:
  MIRBUILDER-COMPARE-LOWERING-SYMBOLIC-COMMAND-PILOT-001; status=landed; intent-map symbolic command snapshot green; next=MIRBUILDER-COMPARE-LOWERING-SYMBOLIC-COMMAND-PARITY-001; status=next; unit=parity/bridge selection; ProgramJSON full dispatcher authority=0; MIR emission=0; mutation=0; allocation=0; runtime_authority=rust_astnode
...
next_documented_task =
  MIRBUILDER-COMPARE-LOWERING-SYMBOLIC-COMMAND-PARITY-001

next_after_active_3 =
  MIRBUILDER-COMPARE-LOWERING-SYMBOLIC-COMMAND-PARITY-001 -> MIRBUILDER-COMPARE-RHS-MATERIALIZATION-OWNER-SELECTION-001
