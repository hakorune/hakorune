...
latest design decision:
  3045 lands real `.hako` ProgramJsonLocalFloatLiteralShapeScanV1 traversal
  for covered Local.expr Float ProgramJSON structure with a 9-row AOT parity
  gate. 3046 is active to mark only covered
  LocalFloatLiteralShapeSnapshotV1 rows as Rust ASTNode projector retire-candidate.
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-LOCAL-NULL-LITERAL-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=Local.expr Null traversal, no null lowering or option semantics

2. MIRBUILDER-PROGRAMJSON-LOCAL-NULL-LITERAL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=landed; boundary=covered LocalNullLiteralShapeSnapshotV1 rows only

3. MIRBUILDER-PROGRAMJSON-LOCAL-FLOAT-LITERAL-SHAPE-SCAN-CAPABILITY-001
...
next active:
  ...
  status=active; boundary=covered LocalFloatLiteralShapeSnapshotV1 rows only
