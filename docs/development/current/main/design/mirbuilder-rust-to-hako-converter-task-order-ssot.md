...
current implementation task:
  MIRBUILDER-CALLEE-GUARD-ME-CALL-CLASSIFIER-HAKO-ADOPTION-DECISION-001.
  The callee_guard_me_call_classifier owner is HakoAdopted as the
  one-hundred-thirteenth narrow Rust-oracle parity pilot after the green
  4-row `.hako` EXE parity gate; next is rerun 115.
...
latest design decision:
  callee_guard_me_call_classifier is now HakoAdopted as a narrow Rust-oracle
  parity pilot owner after a green 4-row `.hako` EXE parity gate. Call
  resolution, backend lowering, and MIR mutation remain Rust. The selected
  next card is `MIRBUILDER-HAKO-NATIVE-OWNER-PARITY-PILOT-SELECTION-RERUN-115`.
