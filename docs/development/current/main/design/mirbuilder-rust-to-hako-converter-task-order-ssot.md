...
latest design decision:
  3081 lands real ProgramJsonContextScopeShapeScanV1 ProgramJSON traversal for
  covered ContextScope statement shapes with 10-row AOT parity. Next work marks
  only those rows as a scoped retire-candidate.
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-EXPR-INDEX-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=landed; boundary=only the covered ExprIndexShapeSnapshotV1 rows

2. MIRBUILDER-PROGRAMJSON-CONTEXT-SCOPE-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=real `.hako` ProgramJSON ContextScope traversal

3. MIRBUILDER-PROGRAMJSON-CONTEXT-SCOPE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=active; boundary=only the covered ContextScopeShapeSnapshotV1 rows

next active:
  MIRBUILDER-PROGRAMJSON-CONTEXT-SCOPE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
  status=active; boundary=covered context-scope snapshot retire-candidate only
