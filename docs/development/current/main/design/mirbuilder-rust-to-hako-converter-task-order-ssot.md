...
current implementation task:
  MIRBUILDER-FASTMEM-LLVM-LAYOUT-TABLE-MEMOP-KIND-CLASSIFIER-HAKO-ADOPTION-DECISION-001.
  The fastmem_llvm_layout_table_memop_kind_classifier owner is HakoAdopted as
  the one-hundred-forty-ninth narrow Rust-oracle parity pilot after the green
  6-row `.hako` EXE parity gate; next is rerun 153.
...
selected next task:
  MIRBUILDER-HAKO-NATIVE-OWNER-PARITY-PILOT-SELECTION-RERUN-153
...
latest design decision:
  fastmem_llvm_layout_table_memop_kind_classifier is now HakoAdopted as a narrow
