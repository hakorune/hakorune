...
latest design decision:
  3069 lands real `.hako` ProgramJsonCallTargetShapeScanV1 ProgramJSON
  traversal for covered Call.name/arity shapes with 10-row AOT parity. Active
  work is 3070: mark only those covered rows as a scoped Rust ASTNode projector
  retire-candidate.
...
2. MIRBUILDER-PROGRAMJSON-CALL-TARGET-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=active; boundary=covered CallTargetShapeSnapshotV1 rows only
...
next_after_active_3 =
  select next ProgramJSON traversal capability after 3070 lands
