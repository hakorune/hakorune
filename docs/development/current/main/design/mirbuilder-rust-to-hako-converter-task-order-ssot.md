...
latest design decision:
  3079 lands real ProgramJsonExprIndexShapeScanV1 ProgramJSON traversal for
  covered Expr.expr Index shapes with 9-row AOT parity. Next work marks only
  those covered ExprIndexShapeSnapshotV1 rows as a scoped retire-candidate.
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-ASSIGNMENT-VALUE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=landed; boundary=only the covered AssignmentValueShapeSnapshotV1 rows

2. MIRBUILDER-PROGRAMJSON-EXPR-INDEX-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=real `.hako` ProgramJSON Expr.expr Index traversal

3. MIRBUILDER-PROGRAMJSON-EXPR-INDEX-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=active; boundary=only the covered ExprIndexShapeSnapshotV1 rows

next active:
  MIRBUILDER-PROGRAMJSON-EXPR-INDEX-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
  status=active; boundary=covered expr-index snapshot retire-candidate only
