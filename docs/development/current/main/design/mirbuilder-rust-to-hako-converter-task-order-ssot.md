...
current implementation task:
  MIRBUILDER-ARRAY-TEXT-COMBINED-REGION-ADD-CONST-ONE-FROM-CLASSIFIER-HAKO-ADOPTION-DECISION-001.
  The array_text_combined_region_add_const_one_from_classifier owner is
  HakoAdopted as the one-hundred-fifth narrow Rust-oracle parity pilot after
  the green 4-row `.hako` EXE parity gate; next is rerun 107.
...
latest design decision:
  array_text_combined_region_add_const_one_from_classifier is now HakoAdopted
  as a narrow Rust-oracle parity pilot owner after a green 4-row `.hako` EXE
  parity gate. Array-text combined region planning, backend lowering, and MIR
