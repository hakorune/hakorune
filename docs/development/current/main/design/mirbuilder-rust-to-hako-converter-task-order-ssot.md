...
latest design decision:
  3057 lands real `.hako` ProgramJsonLocalRecordUpdateShapeScanV1 ProgramJSON
  traversal for covered Local.expr RecordUpdate shapes with 10-row AOT parity.
  3058 is active for the scoped LocalRecordUpdateShapeSnapshotV1
  retire-candidate.
...
## Active Next 3
```text
1. MIRBUILDER-PROGRAMJSON-LOCAL-RECORD-UPDATE-SHAPE-SCAN-CAPABILITY-001
   status=landed; boundary=real `.hako` ProgramJSON Local.expr RecordUpdate traversal,
   no record lowering, field layout semantics, route selection, or MIR mutation

2. MIRBUILDER-PROGRAMJSON-LOCAL-RECORD-UPDATE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
   status=active; boundary=covered LocalRecordUpdateShapeSnapshotV1 rows only

3. MIRBUILDER-PROGRAMJSON-CAPABILITY-BATCH-CONTINUATION
   status=pending-after-3058; boundary=select and implement next concrete ProgramJSON traversal capability

next active:
  MIRBUILDER-PROGRAMJSON-LOCAL-RECORD-UPDATE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
  status=active; boundary=mark only covered LocalRecordUpdateShapeSnapshotV1 rows as retire-candidate
