...
| `MIR-FMEM-008B layout/table producer pilot` | active | Open LLVM/object lowering for verified layout/table MemOps such as `TableIndex`, `FieldLoad`, and `FieldStore`. | First slice preserves `MemOpAccess` symbolic `field_id/table_id` in MIR/JSON and rejects missing ids. Next slice adds `VerifiedMemAccessPlan` rows so lowering can consume accepted layout/table truth instead of recomputing offsets or table representation. No allocator-owner TLS runtime behavior; no bridge retirement; no product allocator activation. |
