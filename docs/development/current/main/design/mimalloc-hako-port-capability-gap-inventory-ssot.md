...
| `MIR-FMEM-007C Python template C diagnostic import guard` | next | Add a lightweight static guard for diagnostic bridge imports. | Normal allocator tools must not import Python-template C payload modules directly; diagnostic payloads route through the bridge guard/build support boundary. |
