...
| `MIR-FMEM-008B layout/table producer pilot` | active | Open LLVM/object lowering for verified layout/table MemOps such as `TableIndex`, `FieldLoad`, and `FieldStore`. | Symbolic `MemOpAccess` ids and `fastmem_access_plans[]` metadata rows are landed. Rows remain `symbolic_only` until canonical layout/table contracts verify offsets, table representation, alignment, and bounds. No allocator-owner TLS runtime behavior; no bridge retirement; no product allocator activation. |
