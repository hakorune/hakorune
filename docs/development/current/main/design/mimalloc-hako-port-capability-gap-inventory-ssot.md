...
| `MIM-PORT-FMEM-001 first hako_alloc body migration pilot` | done | Added the first source-level hako_alloc PageMeta scalar fastmem body pilot and made AST inventory see TableIndex / FieldLoad / FieldStore. | Source/body pilot only: no DirectArray/free-list lowering, block_used mutation, local_free/remote_free, owner mutation, TLS transfer, provider activation, hook, global allocator claim, or winner claim. |
| `MIM-PORT-FMEM-002 PageMeta scalar pilot verified access-plan evidence` | done | Connected the PageMeta scalar pilot to MIR JSON metadata and verified FieldLoad/FieldStore access-plan evidence using the existing FastMemory substrate only. | TableIndex is observed but remains proof-incomplete. No AtomicRemoteHead lowering, TLS backing transfer, provider activation, hook, global allocator claim, or winner claim. |
| `MIM-PORT-FMEM-003 PageMeta TableIndex length/bounds proof surface` | done | Added explicit fastmem table-length and index-range proof annotations so PageMapV0 TableIndex becomes a verified table access. | No lowering-side inference, Type ABI hot lookup, Provider ABI dispatch, AtomicRemoteHead lowering, TLS backing transfer, provider activation, hook, global allocator claim, or winner claim. |
| `MIM-PORT-FMEM-004 PageMeta MIR-to-LLVM producer evidence` | done | Connected the verified PageMeta pilot to MIR-to-LLVM producer lowered-count evidence through an observation-only hako_check report. | Positive TableIndex / FieldLoad / FieldStore lowered-count evidence is now gated by `fastmem-check --inventory`; no owner routing, remote-free behavior, TLS backing transfer, provider activation, hook, global allocator claim, or winner claim. |
| `MIM-PORT-FMEM-005 PageMeta owner_worker_id read-only scalar pilot` | done | Added a separate hako_alloc PageMeta fastmem body that reads `owner_worker_id` as verified `plain_scalar` layout metadata. | This is not owner-runtime behavior: `CurrentAllocOwnerId`, `OwnerEq`, owner mutation, free-list mutation, remote-free, TLS transfer, provider activation, hook, global allocator claim, and winner claim remain closed. |
| `MIM-PORT-FMEM-006 PageMeta free_head read-only pointer observation` | done | Added a separate hako_alloc PageMeta fastmem body that reads `free_head` as pointer-shaped metadata through verified TableIndex/FieldLoad producer evidence. | `free_head` is not returned or stored; `local_free_head`, `remote_head`, DirectArray/free-list mutation, owner runtime, TLS transfer, provider activation, hook, global allocator claim, and winner claim remain closed. |
| `MIM-PORT-FMEM-007 owner equality-only source observation` | done | Added a separate hako_alloc PageMeta fastmem body that observes `mem.currentAllocOwnerId` and `mem.ownerEq` from source and lowers them through the MIR-to-LLVM owner-runtime producer profile. | The equality result is consumed inside the region as a verified `used` FieldStore input. Owner mutation, TLS transfer, same/remote routing, free-list behavior, AtomicRemoteHead, provider activation, hook, global allocator claim, and winner claim remain closed. |
| `MIM-PORT-FMEM-008 local_free_head / free-list source-body preflight` | done | Added a local_free_head source preflight showing AST/MIR metadata can observe the field while MIR-to-LLVM lowering rejects ordinary FieldLoad on `local_free_head` field class. | This is fail-closed evidence only. `local_free_head` ordinary lowering, `remote_head`, AtomicRemoteHead, same/remote routing, TLS transfer, provider activation, hook, global allocator claim, and winner claim remain closed. |
| `MIM-PORT-FMEM-009 free-list mutation substrate selection` | done | Choose the next free-list mutation substrate: a dedicated `local_free_head` field-class gate, a free-list-specific MemOp, or a DirectArray/free-list proof row. | Selected a free-list-specific MemOp family. `local_free_head` ordinary FieldLoad/FieldStore stays rejected; remote-owner routing, AtomicRemoteHead, TLS transfer, and product activation remain closed. |
| `MIM-PORT-FMEM-010 LocalFreePush / LocalFreePop MemOp vocabulary` | done | Add the local free-list MemOp vocabulary and source-intrinsic observation without opening lowering. | LocalFreePush/LocalFreePop-style ops now appear in source/MIR/JSON, but verified lowering remains closed; ordinary `local_free_head` field lowering remains rejected. |
| `MIM-PORT-FMEM-011 verifier-owned LocalFreeList plans` | done | Add verifier-owned plan rows for LocalFreePush/LocalFreePop without opening LLVM lowering. | LocalFreePush/LocalFreePop now emit non-lowerable local-free plan rows with local_free_head contract metadata and missing same-owner/block-next proof evidence; ordinary `local_free_head` field lowering remains rejected. |
| `MIM-PORT-FMEM-012A LocalFreePush precondition evidence` | done | Add source/MIR proof facts for same-owner and local-free block-next preconditions before opening LLVM lowering. | `mem.assumeSameOwner(page, same_owner)` and `mem.assumeLocalFreeBlockNext(block)` now feed LocalFreePush access plans; Push can become verified/lowerable when both proofs exist, while LLVM lowering and Pop remain closed. |
| `MIM-PORT-FMEM-012B LocalFreePush LLVM producer pilot` | done | Lower verified LocalFreePush plans in the MIR-to-LLVM producer without opening ordinary `local_free_head` FieldStore. | Verified LocalFreePush now consumes PageMeta LayoutRef plus `FreeBlockNodeLayoutV0.next` access material; LocalFreePop, remote-owner routing, AtomicRemoteHead, TLS transfer, and product activation remain closed. |
| `MIM-PORT-FMEM-013 LocalFreePop route` | planned | Design and open the pop side of the page-local free-list route. | Must consume verifier-owned plans only and keep remote-free, AtomicRemoteHead, TLS transfer, owner slot reuse, and product activation closed. |
