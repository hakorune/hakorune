...
| `MIR-FMEM-007 Python template C bridge retirement` | next | Remove the Python-template C semantic bridge after producer-neutral parity is proven. | Retirement must not remove optional MIR-to-C debug/diff artifact support if that lane is later added. |
