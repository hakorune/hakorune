...
| `MIR-FMEM-007B Remaining Python template C quarantine/delete inventory` | next | Inventory remaining Python-template C bridge template/report/smoke files and remove or quarantine non-baseline runtime entrypoints. | Optional MIR-to-C debug/diff artifact support remains separate and must be generated from MIR MemOps. |
