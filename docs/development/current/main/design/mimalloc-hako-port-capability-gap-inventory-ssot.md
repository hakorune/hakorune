...
| `MIR-FMEM-007D Python template C diagnostic payload keep/archive decision` | next | Decide whether the remaining diagnostic payloads stay until allocator-owner layout/table MemOps cover replacement-front behavior, or whether fixed-slot-only payloads can be archived first. | Must not delete parity/report fixtures before MIR-to-LLVM replacement-front evidence can replace their baseline role. |
