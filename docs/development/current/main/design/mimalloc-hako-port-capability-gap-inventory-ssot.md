...
| `MIR-FMEM-008A producer-slice selection` | next | Select the smallest MIR-to-LLVM replacement-front producer slice after the diagnostic lifecycle bridge evidence is complete. | Docs/report/acceptance selection only; no lowering behavior change; product activation, hooks, global allocator claim, and winner claim stay closed. |
| `MIR-FMEM-008B layout/table producer pilot` | planned | Open LLVM/object lowering for verified layout/table MemOps such as `TableIndex`, `FieldLoad`, and `FieldStore`. | No allocator-owner TLS runtime behavior; no bridge retirement; no product allocator activation. |
| `MIR-FMEM-008C owner-runtime producer pilot` | planned | Open LLVM/object lowering for allocator owner runtime MemOps such as `CurrentAllocOwnerId` and `OwnerEq` plus matching report counters. | No TLS backing transfer, owner slot reuse as active owner, hooks, global allocator claim, or winner claim. |
| `MIR-FMEM-008D producer-neutral parity/readiness` | planned | Prove MIR-to-LLVM layout/table/owner-runtime evidence can replace the quarantined Python-template C diagnostic baseline. | Only after this may the reference closeout and bridge-retirement wording cleanup run; remaining payloads stay quarantined until parity is explicit. |
