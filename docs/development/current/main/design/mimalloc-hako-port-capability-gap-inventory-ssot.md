...
| `MIM-FMEM-008 fastmem source syntax pilot` | current | Connect parser output to MIR MemOp region metadata after `PARSER-FMEM-001..006` proved dual parser parse-only parity. | Source-derived fastmem inventory/check works; contract-less `unsafe` / `fastmem` remains rejected; execution/lowering beyond metadata stays closed. |
