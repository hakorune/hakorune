...
| `MIR-FMEM-008D owner-runtime producer pilot` | planned | Open LLVM/object lowering for allocator owner runtime MemOps such as `CurrentAllocOwnerId` and `OwnerEq` plus matching report counters. | No TLS backing transfer, owner slot reuse as active owner, hooks, global allocator claim, or winner claim. |
