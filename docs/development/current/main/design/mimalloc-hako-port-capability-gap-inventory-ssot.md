...
| `MIR-FMEM-008B layout/table producer pilot` | next | Open LLVM/object lowering for verified layout/table MemOps such as `TableIndex`, `FieldLoad`, and `FieldStore`. | No allocator-owner TLS runtime behavior; no bridge retirement; no product allocator activation. |
