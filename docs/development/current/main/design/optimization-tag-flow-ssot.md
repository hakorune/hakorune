...
|---|---|---|---|---|
| Language optimization annotations | `@rune Inline(prefer)`, `@rune Contract(pure)`, `@rune IntrinsicCandidate(...)` (`@hint/@contract/@intrinsic_candidate` plus `Hint(inline/noinline)` are compat aliases where applicable) | parser / Program(JSON) metadata + MIR verifier for narrow Contract rows + MIR `inline_plans` preservation + M11c-soft-leaf MIR inline | No | `Contract(no_alloc/no_safepoint)` is verifier-checked; `Inline(prefer)` can trigger narrow same-module MIR leaf inline but is not backend-active |
...
- canonical optimization metadata surface is `@rune Inline/Hint/Contract/IntrinsicCandidate`.
...
- `IntrinsicCandidate`, `Contract(pure)`, and `Contract(readonly)` remain
  metadata-only here.
- `Inline(prefer)` flows into MIR-owned InlinePlan metadata and may be consumed by
  the M11c-soft-leaf MIR optimizer row for narrow same-module pure leaf calls.
  Compat `Hint(inline)` maps to the same advisory request during the migration
  window.
...
- [inline-plan-ssot.md](/home/tomoaki/git/hakorune-selfhost/docs/development/current/main/design/inline-plan-ssot.md)
