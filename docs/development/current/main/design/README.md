...
Membership / role / precedence の唯一のownerは `registry/`（V1 sharded
manifest + shards）。`INDEX.md` は navigation index でauthorityを持たない。
このREADMEは navigation-only viewであり、文書を掲載してもauthorityには
昇格させない。
...
- JoinIR target lowerer thinning（target-specific lowerer の共通 seam / neutral loop-target policy / VM execution 境界SSOT）: `docs/development/current/main/design/joinir-target-lowerer-thinning-ssot.md`
...
- Generic Loop post-effect debt classification（M4の全段階分類・V0/V1 precedence・winner equivalence の設計停止）: `docs/development/current/main/design/joinir-generic-post-effect-debt-classification-ssot.md`
- Loop family observation policy（DirectAccum caller-zero Candidate/Declined/Unresolved/Rejected境界と、Nested等の次行順序）: `docs/development/current/main/design/loop-family-observation-policy-ssot.md`
...
- REPL MirInterpreter interactive session（新Interpreterを作らず既存 `MirInterpreter` を complete-cell 対話実行エンジンへ育てる順序SSOT）: `docs/development/current/main/design/repl-mir-interpreter-interactive-session-ssot.md`
...
- Hako Alloc mimalloc port identity boundary（`hako_alloc` は mimalloc 移植の `.hako` 本体、Python-template C は明示 diagnostic baseline、MIR-to-LLVM が primary producer direction という境界SSOT）: `docs/development/current/main/design/hako-alloc-mimalloc-port-identity-boundary-ssot.md`
