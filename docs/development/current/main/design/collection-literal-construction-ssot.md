...
Decision: D0 remains open at the existing final-owner handoff. The source and child physical owners exist; the missing authority transfer is an affine, owner-indexed child Birth actual/ABI retention that the existing compiled-entry and physical program can consume once. Do not issue a second semantic receipt or let C infer a child target from MIR.
Smallest next slice: design the owner-indexed extension of the existing finalized handoff, including exact child Birth target/receiver/actual membership, duplicate/omission rejection, and cleanup coordinates shared from `issue_cleanup_coordinates(program.functions())`; then record normal, Birth Fault/unfinished reclaim, and Map/cleanup Fault order. Keep the C child gates closed until that handoff is accepted.
Exclusive Stop edge: root-only `local_commits` filtering plus root-only `issue_birth_calls_for_owner` consumption. Preserve owner/receiver/actual identity, existing child `PhysicalBoundary`, compiled-entry membership checks, and all ordinary New/Home fail-fast paths while this D0 is open.
