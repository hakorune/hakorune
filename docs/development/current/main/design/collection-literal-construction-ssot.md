...
#### Three-owner local binding frontier: accepted I0

Decision: implement exactly two local Calls A/B followed by terminal Call C to three distinct ordinary Map owners; keep the two-owner I0 closed and widen only after source-ordered local binding retention is in place.
...
Accepted BoxShape: existing pending and exit local-binding storage becomes source-ordered grouped rows, `Vec<(OwnedExprSiteV1, Bindings)>`, keyed by the existing owner. `emit_local` passes the exact site; the recorder verifies owner and the next source local-Call site before push. Duplicate, foreign, out-of-order, overflow and empty groups reject before mutation. Transfer, finishing rebind and finalization validate the complete sequence; flattening is allowed only at the existing physical boundary. No new receipt or schema is issued.
...
Implementation checkpoint: the existing pending map and `RootHomeExitEntry::Call.local_bindings` now retain exact site-tagged groups; `selected/terminal_call::emit_local` passes the site, and root-exit/finishing consumers validate the complete sequence before the physical boundary flattens it. `map_target_owners` is widened to three only with an explicit two-local cohort guard, so a repeated-target fourth Call rejects before install. Focused semantic 12/12 and natural direct/linked Pair exit30 pass. Keep the existing prior-home rejection, cleanup builder and owner index.
Delete-set: the same-owner-only insertion/replacement path, flat binding storage that loses the site, and any `unwrap_or_default()` or missing-group acceptance. Do not delete the existing cleanup or co-seal owners.
...
Smallest next slice: widen the existing owner cap from4 to5 and require exactly four local Calls in preflight; reuse source-ordered groups, formal checks, cleanup owners and physical collection.
...
Acceptance: the ignored ordinary-child test reaches existing selected C/runtime direct EXE and linked OBJ exit30; root+child Map and current two/four-owner evidence remain green. Passed after correcting the one-site row expectation; no default or inferred authority was added.

### Five-owner local binding frontier: selected I0

Decision: admit one bounded cohort of four source-local Calls A/B/C/D followed by terminal Call E for five distinct ordinary Map owners. Keep `A -> B -> C -> D -> optional root Map -> E`; do not widen root-before-first-Call or callee shape.
Source authority + canonical issuer: existing `issue_local_i64_call`, `RootHomeFlow.local_calls`, resolver affine Call rows, `direct_call_lifecycle::co_seal_lifecycle`, and each owner's original Completion. No new semantic issuer or receipt.
Non-authority: owner count alone, declaration count, MIR Call count, physical function arrays, Map names, or group length cannot issue admission.
...
Owner/caller census and implementation (2026-09-10, landed `c3fa1f8a75`): `issue_ordinary_source_cohort_v1` and `OrdinaryNewAdmissionClaimV1` remain the constructor issuers; `RawInvocationChildPortV1::try_take_ordinary_new_claim` plus `selected::emit` consume the exact selected owner; `complete_new_emissions` and `validate_finalized_child_emissions` retain/recheck New, Birth, HomeRelease, and PhysicalBoundary bindings. The final handoff now scans all completed local commits, retains caller-scoped `FinalizedBirthActualsV1` beside the unique Birth ABI definitions, and rejects destination/site owner drift. `physical_program` and compiled-entry resolve ordinary caller owners through the existing selected result cohort, while JSON matching also requires the physical caller index. Focused root Birth, child-only Birth+Map/New, JSON, and cleanup-coordinate tests pass. The C stops remain the child `new_box`, Birth call, and HomeRelease/Reclaim gates in `hako_llvmc_ffi_lifecycle_v4_indexed_flow.inc`; no C activation is claimed.
Decision: D0 is accepted at the existing final-owner handoff. The source and child physical owners already exist; the missing transfer is an affine, owner-indexed child Birth actual/ABI retention that the existing compiled-entry and physical program will consume once. No second semantic receipt is issued and C never infers a child target from MIR.
Accepted I0 shape: keep `FinalizedRootHandoffV1` as the sole final artifact owner. Root source retains only root terminal/call cleanup; the final handoff retains the unique Birth ABI definitions and caller-scoped `FinalizedBirthActualsV1` rows for every selected/finishing owner. Birth ABI owner (constructor definition) and actual owner (caller/New site) remain distinct.
Fail-fast boundary: selected/finishing owner, exact New site, destination, receiver, target, argument rows, and Birth function membership must match. Duplicate/omitted actuals reject. A Birth used only by a child still selects `Births`; root absence cannot erase child actuals. ValueId alone, function name, declaration presence, or C role cannot issue a match.
Consumer chain: existing finalized handoff -> all Birth definitions in `physical_program` -> caller-aware compiled-entry Birth calls -> JSON encoder matching caller function index plus target/receiver/arguments -> existing C consumer. `issue_cleanup_coordinates(program.functions())` remains the single cleanup-coordinate owner.
Exclusive delete-set: root-only `local_commits` filtering plus root-only `issue_birth_calls_for_owner` consumption. Preserve owner/receiver/actual identity, existing child `PhysicalBoundary`, compiled-entry membership checks, and all ordinary New/Home fail-fast paths. C child New/Birth/HomeRelease gates remain a later execution slice.
Acceptance: the bounded Rust handoff is landed at `c3fa1f8a75`: the root Birth regression and a child-only Birth+Map/New source retain the actual, produce a caller-indexed compiled-entry Birth call, encode the physical JSON Birth call, and share cleanup coordinates. Duplicate/omitted/foreign rows remain rejected by the existing membership checks. C child gates and direct/linked exit30 remain the next design row, not this I0.
...
co-sealed one source-local exact-i64 Call before that terminal Call, retained the
Normal result/Fault continuation in the existing root owner, and carried both
physical calls through direct/linked Pair exit30. Field return, New/Home transfer,
nested or conditional calls, callee-to-callee calls, recursion, third calls, and
generic fallback remain outside the candidate.
...
valid. The selected consumer closes the scoped typed-formal capability, with I64
formals feeding Map install, literal30 after cleanup, Map Fault cleanup using the
Borrowed frame, no callee report/dispose, and draft/finishing validation of the
same obligations. Wider consumers must retain those obligations and add their
own physical evidence; selected artifact/EXE evidence is required by the
enclosing series.
