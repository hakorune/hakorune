...
Smallest next slice: extend `map_target_owners` to the bounded four-owner set and require exactly three local Calls in `preflight_map_install`; this source-admission slice is now green in semantic 14/14. Next connect the same groups to direct/linked artifact and Fault-prefix evidence, reusing the physical Call collection.
