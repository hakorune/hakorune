...
Read-only owner/caller census (2026-09-10): `issue_ordinary_source_cohort_v1` and `OrdinaryNewAdmissionClaimV1` issue the constructor relation; `RawInvocationChildPortV1::try_take_ordinary_new_claim` plus `selected::emit` consume it for the exact child owner; `complete_new_emissions` and `validate_finalized_child_emissions` retain/recheck New, Birth, HomeRelease, and PhysicalBoundary bindings. `seal_finalized_root_birth_handoff` currently filters `local_commits` to the root owner, and `issue_birth_calls_for_owner` consumes only `root_source.birth_actuals()` for that root. `physical_program::issue_lifecycle_physical_program` therefore publishes Birth functions from the root handoff only; `physical_program_json::encode_birth_call` has no child Birth target/actual to bind. The C stops are the child `new_box` guard, root-only Birth call flow, and root-only HomeRelease/Reclaim flow in `hako_llvmc_ffi_lifecycle_v4_indexed_flow.inc`.
...
Acceptance: one root+child Birth source and one child-only Birth source retain all actuals, reject duplicate/omitted/foreign owner rows, produce per-caller compiled-entry Birth calls without ValueId collision, and encode matching JSON. No C child gate or direct/linked exit30 claim is made by this I0.
