...
Task 1 remains active: the resolved_semantics caller-prefix issuer now co-seals exact destination, covered statements, reverse prior-Home order and required New unwind site into the existing claim. Target take checks prior local installations; aliases add no Home, unknown/entry-demand/Handle-argument/override prefixes never become empty cleanup. Completed ledger rows retain the source relation. Next implement outward Fault and retained exit projection on this same chain under constructor-birth-new-lifecycle-ssot; no new input census or detached receipt. Runtime cleanup/reclaim/Birth execution/EXE remain open and Birth backend rejection stays intact. Prior statement evidence remains 97/1 accepted baseline; no manifests changed.
