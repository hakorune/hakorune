...
| 4 landed | `lang/c-abi/tests/published_lifecycle_v4_receiver_identity_test.c` plus the existing V4 admission owner — `MIRBUILDER-PHYSICAL-NEGATIVE-TEST-PROOF-R0` | Make the receiver fixture the reusable base for physical integrity negatives: first prove a valid normal path and valid fault/cleanup path, then mutate exactly one field and require the check's stable named rejection. Extend the existing admission reason propagation only for a concrete selected negative; do not accept a generic nonzero or `unsupported-cohort` as isolated proof. | Landed at `5a117a96a2` with guard follow-up `a451f939f8`; receiver keeps the valid normal/fault base and receiver-only named mismatch, while Map reuses a valid graph, mutates only `layouts[0].fields`, and asserts parser-owned `abi-layout`. Map physical execution passes including 26 malformed inputs; no second verifier or broad error taxonomy. |

| 5 landed | `selfhost::json::parse_mir_json_v0_line` — `MIR-CALL-LEGACY-READER-STOP-R0` (strict/dev `boxcall` owner-unit) | The existing strict/dev helper is the pre-effect boundary for `stage_a_route`, `stage_a_compat_bridge`, and the stage1 stub parser. It rejects `op: "boxcall"` with `[freeze:contract][callsite-retire:legacy-boxcall]` before `mir_json_v0::parse_mir_v0_to_module`; release/v0 compatibility remains on its existing edge. | Landed before this R7 selection at `4e1d6f92fb`; the existing strict/dev named-reject and release compatibility tests prove the bounded stop. The shared `module.rs` `boxcall` arm and `LegacyCallV0` carrier remain until their other callers reach caller-zero. |
...
* The physical negative-test proof row closed at `5a117a96a2` with the
  receiver base and the Map parser-owned layout mutation. Its reusable helper
  and named-reject guard remain available for the next concrete physical
  integrity check; no blanket error taxonomy was introduced.

The selected execution frontier now moves to
...
```text
manifest 92ad589d44a51c28ed66cd31064851b229419408ed8de4610e4fbcbac32806e1
runner   eb829d6ab7c062974adc912428622db9dccf602fa8991197298b16865b88ee0
newbox   script 014f30f18f30f1fb4759f074cd53eefca99553d1520fafad9db745402ae98d0f source 80b07fe145e5fe61b2d326620fd5b1929c452124e3fe5c5528d4aeb96be54a7b
method   script 5e1e6ab7dc806cddaad18abf03b0273665385ecfdbfca8bb9ab03e442b35e451 source f6e962309f166f4d2cc8993c4c388ec45d7ab3173e70b1641d9389fbe22ee16d
birth    script 610d8f506261942b78196a47a78c3ebd7bb0e3c82836bea3f3a6f744a7678ef1 source bbd957398d16ae9e77c1b2bd3c510e8b6ba03c694d97ed14807f8ffe320aa0a4
birth-param script bb98b3149a2c3e66e19d2aa5b6fe837a5c94057773844e8c65d1c56b1577c50e source 6c9feec4c2bbaab48be2f3c71f07e0fcff0fd686b36b1aec387f298fa36ca7b7
untyped  script 3239bcf7ec156cd8830346c036a08b76226b76fcff5e9871ede82bc0c6308eeb source 456e7de7f583b9cbc36f3b5abfb479fd64baec822259631a581ce68f442febdb
boxtorrent script 2d8f1f7e41b5f496a1992cbfb4cff19a79999ae17ebdd158060e463521aa3b4d source 0c22fe686c826f718889e6ac1ff96ff3f8a3ff22628401c4d7f93d75ebbf1ae7
binary   script 4fe22d3a4bc1fd4350be090eb0a26783ed103bdcfc5dabf31a00a73ff69feecc source 27de7bedca3e05bfb9facbbc5417ac1eb623a720b53c150b42e0e8646f12fa1c
mimalloc  script 03009535b3a11929aa1f8ccfd386b2bcc388af161888dcfd7bf91968d6552871 source 63f688bef954d29ef91930e861b0721293789c4e74af2c6f1dae25b6ed84b772
allocator script 42ccf8ba637c4a8c82a226342d2ec66b9249688f64986d1b279e4e7bc2db44cb source 9ce97be74bbe747d95d40ef0471ed44c4043b5be3f3dd9143a2b12954ef8f94f
...
##### MIR-CALLABLE-LOOP-ORDINARY-READY-PORT-P0 (design closed 2026-09-10)
...
##### `MIR-CALLABLE-LOOP-ORDINARY-READY-PORT-I0` (closed 2026-09-10)
...
That adapter evidence does not establish two integrity properties now required
before the next Loop consumer: equality between the moved relation owner and
the active ledger owner, and reuse of the planner's increment classification.
The previous Ready I0 claim remains limited to source-row consumption; these
...
Decision: preserve the existing Ready remainder and admit one grouped
          BodyOnlyRebind cohort through the same source-aware consumer; do not
          relabel Outside or mix body-only rows into a Ready carrier implicitly.
Source authority + canonical issuer: CallableSemanticLoweringState and
          CallableLoopSourceProjectionV1 issue the existing owner/lineage and
          (site, role) rows; the existing GenericLoop Facts issuer remains the
          sole shape/policy issuer; RawInvocationSourceContextV1 owns lineage.
Non-authority: Outside diagnostic arrays alone, AST/name/ordinal/ValueId
          inference, Builder maps, LoopRouteContext, a second Facts/Recipe
          issuer, a second semantic receipt, or legacy fallback/retry.
Fail-fast boundary: before RecipeComposer/PlanLowerer/Builder effects, validate
          owner, loop/condition/body lineage, the Ready remainder, complete
          BodyRead/BodyRebind coverage, duplicate/foreign/missing rows, nested
          sites, and policy drift. Other Outside kinds remain typed terminals.
Smallest next slice: extend the existing source projection disposition with
          one move-only composite carrying the Ready schedule and grouped
          body-only rows, then consume both through the existing source-aware
          adapter for one nonnested positive fixture and strict ledger finish.
Non-claims: no new Facts/Recipe authority, no second semantic receipt, no
...
adapter, including the active callable ledger and current BodyOnly rows;
...
Two integrity conditions are part of this design stop, before the next consumer
is admitted. The composite's relation owner must equal the active callable
ledger owner; a matching loop/site alone is insufficient. The increment binding
and step must be consumed from the existing planner classification, rather than
being rediscovered by a first-match predicate in the composer. A relation-owner
...
| --- | --- | --- |
| `typed_object_untyped_field_min_exe`, `typed_object_birth_param_min_exe` | `local-commit/emission-local-copy-drift`, `root-call-entry-missing` | `MIRBUILDER-UNTYPED-OBJECT-STORAGE-D0`; `NoSafeSlice / ParkedSealed` until source-backed storage is either migrated to an explicit supported type or issued with a tagged dynamic/opaque ABI. Do not repair from MIR, receiver names, or C defaults. **Update 2026-09-25:** the D0 materialized at `mirbuilder-untyped-object-storage-d0-2026-09-25.md`; `typed_object_birth_param_min` landed green via declared-storage migration + `--emit-exe` route move (`MIRBUILDER-UNTYPED-OBJECT-STORAGE-S0`, source hash now differs from the pinned receipt below). `typed_object_untyped_field_min` remains sealed (handle-field newbox + dynamic storage owner still absent). |
...
before starting was circular; no external team or new semantic issuer is needed.
Source authority + canonical issuer: the already-issued physical input; the
existing lifecycle target session owns only LLVM resources and object emission.
Non-authority: timing results, pinned-text/legacy emitters, source names, or
...
The shared V4 caller also serves Map, NativeArray, ordinary children, and the
accepted receiver method; Pair alone cannot prove its replacement correct.
The native-library owner holds the finite inventory, ordered implementation
steps, exclusive deletion, and acceptance. This turn closes design only;
`CURRENT_STATE` names the next implementation row without starting it.
...
This closes the Rust source/lifecycle I0. The selected physical callee-identity
implementation was subsequently landed in I1, while its receiver-mismatch
coverage remains a separate reopened R0 task; the historical I0 evidence must
not be read as physical identity coverage.
