...
Task 1 control schema is implemented under constructor-birth-new-lifecycle-ssot: Invoke/normal-only projection/ReturnFault and verifier/CFG/optimizer handling; instruction tests 21/21, vocabulary sync 1/1 and quick lib/vm-reference checks pass. Next establish the hidden frame at existing method/Main entries before body descent, bind exact New/Birth emissions through the claim port, then consume exits before finalization. Missing/foreign/wrong-type frames and binding reject; source arity is unchanged. Tasks 2–3 connect runtime/reclaim/default storage and typed-C transport, then task 4 closes EXE/OBJ/failure proof and old-edge deletion. Backend remains UnsupportedBeforeObject; task 1 and production cutover are not complete. No metadata/KeepAlive, name/order recovery or baseline change.
