...
Task 1 Invoke/FaultFrameEnter schema and intrinsic frame/Normal-result verification are implemented under constructor-birth-new-lifecycle-ssot; Invoke tests 4/4, instruction tests 18/18, vocabulary 1/1 pass. Next existing method/Main entry owners emit the selected frame before descent, bind exact New/Birth through the claim port and consume exits before finalization. Frame is never a source parameter/type; missing/foreign/wrong-type and ordinary-value escape reject. Tasks 2–3 connect runtime/default-store reclaim and typed-C, then task 4 closes EXE/OBJ/failure proof and old-edge deletion. Backend remains UnsupportedBeforeObject; task 1 and production cutover are not complete. No metadata/KeepAlive, name/order recovery or baseline change.
