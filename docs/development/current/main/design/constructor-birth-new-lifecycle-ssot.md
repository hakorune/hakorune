...
Exact FieldSet validates expected object identity/type and slot under the same storage owner before mutation. Bind the admitted storage profile across allocation/write/read/reclaim: SafeMutex and SingleThreadExact only; pinned/direct mismatch rejects, never fallback. Exact ObjectFieldGet requires a readonly checked indexed i64 getter under this same owner (profile/handle/type/slot plus Normal-only out-slot); mismatches are physical InvalidContract, not source Fault and never zero substitution. Existing field_get_i64_hii is excluded because it defaults to zero and can retry storage. Tests: true zero/min/max, wrong type/slot/profile, unchanged output/storage on reject. No new source Invoke/receipt.
