...
Next private-view Decision (worker reviewed): `into_artifact_parts` returns its already retained root key only after exhaustive validation; the existing generic compile terminal forwards that output after strict verification and fallible commit preparation. A parent-private view constructor shares one projection scan with generic construction, borrows the exact root and derives Birth roles solely from existing canonical definition keys. No new receipt/table, name recovery, clone, readmission or mutable processing after validation; generic try_new remains lifecycle-fenced. Test missing/unavailable/foreign/residual payloads, renamed root identity, callback-once/error, Unit result absence and generic clone rejection. Do not activate production lifecycle admission before the typed C consumer exists.
Remapper physical split retains its public owner/test paths. Initial unavailable-cleanup test stopped at unaccepted FINI syntax; replaced by real-source-row physical-state evidence, not source/EXE proof.
