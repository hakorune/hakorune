...
Next FieldGet Decision: retain exact read OwnedExprSite, receiver site, actual receiver BindingRef (alias distinct from Home), Home binding and CanonicalFieldRef in existing New ledger; stage rows until the entire terminal succeeds. Existing RawOrdinaryNewClaimPort selects a PreparedRawFieldRead route before receiver descent; lower receiver once, compare against value_for_exact_binding, emit mandatory ObjectFieldGet {dst,base,field}. No optional exact-plus-string authority, new receipt, ChildPort field or supertrait. Revalidate emitted block/dst/base/field after finishing. Put methods in a child owner: ordinary_new_coseal is757 lines.
FieldGet acceptance: unchanged Pair two exact reads; repeated field at distinct sites; alias exact receiver; failed Add sibling leaves zero committed read rows; duplicate/foreign/missing site, receiver/ordinal mismatch, residual emission and post-finish drift reject. Selected property/local_field_base/type/origin/name inference and string FieldGet are bypassed without retry; unselected compatibility stays fenced. Then replace selected clone/refresh export, name-plan allocation, i64 Birth handling, FieldSet trap conversion and lifecycle i64 returns in the same series. Full Home/native/unsafe ownership remains separately bounded; fixed Pair EXE30/OBJ cannot be waived or replaced.
