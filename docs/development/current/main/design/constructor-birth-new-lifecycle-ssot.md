...
Runtime owner: `typed_object_store_backend` checked indexed insert/store/reclaim and `exports/fault` C APIs landed. Existing published kinds 1–7 lack control/frame/result lanes; MIR JSON rejects lifecycle instructions, C function/Return emission hardcodes i64, and row residual validation currently follows object generation. Correct these before activation; retain the lifecycle fence.
