...
Borrowed-terminal checkpoint: existing normal pipeline prepares commit and strictly verifies final typed MIR before synchronous consumption; EXE and llvm-boundary OBJ callers are connected, generic lifecycle admission stays fenced. Fresh normal pipeline20/20 (including callback once/error/no-retry, compatibility bypass and fixed Pair rejection), view22/22 with unchanged test names, package97/97; quick lib llvm-boundary+vm-reference and pointer/M7-S green. Deleted unused EXE relay and duplicate host readmission. No CLI Birth execution claim: source eligibility, typed C control/result ABI, pre-artifact residual/retry cut and fixed Pair EXE30/OBJ remain required.
...
Runtime owner: `typed_object_store_backend` checked indexed insert/store/reclaim and `exports/fault` C APIs landed. Published residual validation now precedes both object emitters; typed rows forbid exact-seed dispatch and select opt/llc directly instead of legacy C-API probe/retry. Fresh C build, real-ABI residual/no-object regression and view22/22 pass. Existing kinds1–7 still lack control/frame/result lanes; JSON rejects lifecycle and C function/Return hardcodes i64. Correct these before activation; retain the fence.
