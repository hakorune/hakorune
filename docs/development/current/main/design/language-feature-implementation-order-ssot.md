...
| A6 | `L206 contract syntax metadata capsule` | `requires`, `ensures`, `invariant` | syntax parse and metadata capsule | verifier facts, runtime contract insertion, invariant checking | Stage1 contract lowering owns the metadata shape. |
| A7 | `L207 transition metadata capsule` | `transition Enum.Value -> Enum.Value by method` | transition declaration parse and enum-reference metadata | transition legality, lifecycle verifier, `state` keyword | Stage1 transition checker owns lifecycle facts. |
