...
| `GENERIC-LEGACY-OBSERVATION-FRONT-G0` | one case through a fixed direct VM front -> Loop-reached receipt or named pre-Loop owner failure | landed: immutable G0 receipt names the pre-Loop raw-structured owner; S0-I0 preserves the primary error; S1-I0 closes Dynamic FieldAccess Receiver receipt transport and records the next MethodCall receiver boundary | do not patch Generic, widen support, or declare route/disposition data from a failed front; current design stop is `GENERIC-RAW-STRUCTURED-METHOD-RECEIVER-RECEIPT-S2-D0` |
...
| --- | --- | --- |
| `LOOP-JOINSIG-MIXED-FALLTHROUGH-D0` | shared logical JoinSig branch-arm contract for one-sided Break/Continue plus normal fallthrough and per-arm payloads | design/implementation dependency for S6B; keep `LoopRecipeV1` unchanged, no source observer/producer/physical route until green; implementation updates JoinSig README and loop Recipe reference in the same commit |
