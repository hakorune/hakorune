...
| `GENERIC-LEGACY-OBSERVATION-FRONT-G0` | one case through a fixed direct VM front -> Loop-reached receipt or named pre-Loop owner failure | landed: immutable G0 receipt names the pre-Loop raw-structured owner; fresh S0-I0 receipt proves transactional completion exposes the primary callable-semantic error | do not patch Generic, widen support, or declare route/disposition data from a failed front; resolver variable admission remains a separate design row `GENERIC-RAW-STRUCTURED-DEMANDS-ADMISSION-S1-D0` |
