...
| `LOOP-RECIPE-PHYSICALIZER-MODULE-SPLIT-R0` | flat topology canary -> `loop_recipe_physicalizer/{mod,topology,tests}.rs` facade | behavior unchanged; old flat file deleted; no new operation shape or physical owner | current row; physical block receipt, leaf emission, selector, retry/fallback, production, and legacy deletion remain closed |
