...
| `LOOP-RECIPE-OPERATION-PHYSICALIZER-DESIGN-STOP` | neutral operation/effect parity -> one move-only operation physical demand + borrowed canonical services + item-3 carrier bridge contract | accepted after worker review: single demand owner, Stage-A preflight, Stage-B whole-session discard, parent placement plus child-entry carrier seed, typed `NoSafeSlice` matrix | design closed; no Builder mutation, operation MIR, selector, retry/fallback, production caller, or legacy deletion |
| `LOOP-RECIPE-OPERATION-PHYSICALIZER-CANARY-S0` | accepted physical demand + `ReadyLoopEntryV1` + borrowed canonical services -> one ConstI64/ReadBinding canary receipt | fresh session, one operation emission, typed preflight rejects, whole-session discard/fresh reuse, all touched files under 800 lines | caller-zero test-only; no Return/DraftSeal/publication/selector/retry/fallback/legacy deletion |
