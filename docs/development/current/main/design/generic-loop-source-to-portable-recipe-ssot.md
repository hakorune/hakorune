...
| `GENERIC-LEGACY-OBSERVATION-FRONT-G0` | one case through a fixed direct VM front -> Loop-reached receipt or named pre-Loop owner failure | landed: direct canonical fixture probe is `failed-before-loop` at `RawStructuredChildScopePortV1::complete_exact_demands_v1` with `expressions=1 bodies=0`; immutable JSON receipt and focused/shared guards are green | do not patch Generic, widen support, or declare route/disposition data from a failed front; next repair is `GENERIC-RAW-STRUCTURED-DEMANDS-REPAIR-S0-D0` |
