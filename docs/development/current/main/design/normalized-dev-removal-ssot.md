...
- `binding_context` / `loop_route_detection::legacy`（current physical path: `src/mir/loop_route_detection/legacy/`、historical token/retire条件は `docs/development/current/main/design/archive/route-physical-path-legacy-lane-ssot.md` を参照） / `join_ir::lowering` / frontend 周辺の conditional fields / helpers を通常実装へ吸収または削除する。
