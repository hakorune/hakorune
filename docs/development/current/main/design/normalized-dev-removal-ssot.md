...
- `bridge_joinir_to_mir(...)` は Structured path のみを使う。境界情報は
  conversion ではなく後段の normalized merge が所有する。
...
- `binding_context` / `loop_route_detection::support`（route detector
  `legacy/` storage は 291x-391 で閉鎖済み。historical token/retire条件は
  `docs/development/current/main/design/archive/route-physical-path-legacy-lane-ssot.md`
  を参照） / `join_ir::lowering` / frontend 周辺の conditional fields /
  helpers を通常実装へ吸収または削除する。
