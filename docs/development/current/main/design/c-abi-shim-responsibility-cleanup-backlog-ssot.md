# C ABI Shim Responsibility Cleanup Backlog (SSOT)

> Split out from mirbuilder-rust-to-hako-converter-task-order-ssot.md so the
> task-order stays a lean ordering doc. This is a BoxShape cleanup lane only:
> no converter semantic-claim change, no backend-local special cases. Content
> below is unchanged from the source split.
