...
| --- | --- | --- | --- |
| `canonical simplification substrate` | `substrate` | `landed mechanism` | current optimizer schedule runs placement/effect prepass, semantic simplification, memory-effect, then a pure DCE cleanup rerun. CSE is conservative and same-basic-block only: without an exact supported rewrite the MIR is unchanged and the elimination count is zero. See [src/mir/optimizer/core.rs](/home/tomoaki/git/hakorune-selfhost/src/mir/optimizer/core.rs), [src/mir/passes/dce.rs](/home/tomoaki/git/hakorune-selfhost/src/mir/passes/dce.rs), [docs/development/current/main/phases/phase-227x/README.md](/home/tomoaki/git/hakorune-selfhost/docs/development/current/main/phases/phase-227x/README.md). |
...
| `language-level optimization metadata` | `producer` | `scaffold + live-narrow verifier/transform` | `@rune Hint/Contract/IntrinsicCandidate` is documented and parsed. `Contract(no_alloc/no_safepoint)` is MIR-verifier checked. Inline is MIR-owned `InlinePlan`: annotated narrow same-product leaf transform is live, while post-inline simplification, required residual-call proof, default automatic planning, generic instances, and structured inline remain parked in the ordered C-speed task family. Backends must not infer inline policy from rune strings or symbol names. See [docs/development/current/main/design/optimization-hints-contracts-intrinsic-ssot.md](/home/tomoaki/git/hakorune-selfhost/docs/development/current/main/design/optimization-hints-contracts-intrinsic-ssot.md), [docs/development/current/main/design/optimization-tag-flow-ssot.md](/home/tomoaki/git/hakorune-selfhost/docs/development/current/main/design/optimization-tag-flow-ssot.md), [docs/development/current/main/design/inline-plan-ssot.md](/home/tomoaki/git/hakorune-selfhost/docs/development/current/main/design/inline-plan-ssot.md). |
...
| Area | Role | Status | Current truth |
| --- | --- | --- | --- |
| `LLVM fact export` | `exporter` | `landed mechanism (narrow)` | conservative `readonly` and `nocapture` are applied late at builder finalization; the export-attrs consistency guard prevents strong attr drift in active export points, and the runtime-decl readonly fact guard requires manifest `readonly` rows to carry `memory = "read"`. `M10c-pre` now locks `handle_*` vs `native_ptr_*` return proof vocabulary, but stronger attrs/metadata such as `noalias`, `nonnull`, `dereferenceable`, `parallel_accesses`, and TBAA remain backlog until M10c verifier/export gates consume native pointer proofs. See [src/llvm_py/instructions/llvm_attrs.py](/home/tomoaki/git/hakorune-selfhost/src/llvm_py/instructions/llvm_attrs.py), [tools/checks/k2_wide_export_attrs_consistency_guard.sh](/home/tomoaki/git/hakorune-selfhost/tools/checks/k2_wide_export_attrs_consistency_guard.sh), [docs/development/current/main/phases/phase-261x/README.md](/home/tomoaki/git/hakorune-selfhost/docs/development/current/main/phases/phase-261x/README.md). |
...
- post-inline cleanup, required-inline residual-call proof, and default
  automatic leaf planning are not live
