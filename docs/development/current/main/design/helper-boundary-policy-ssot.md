...
この文書は narrow tuning policy SSOT であって、allocator series 全体の owner doc ではない。
allocator/handle の broader policy-state boundary は
`hako-alloc-policy-state-contract-ssot.md` を正本にする。
