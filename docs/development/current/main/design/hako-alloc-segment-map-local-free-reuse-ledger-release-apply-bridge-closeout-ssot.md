# Hako Alloc Segment Map Local Free Reuse Ledger Release Apply Bridge Closeout SSOT
...
MIMAP-200A proves the daily L2 behavior:
...
representative L3 EXE evidence for the pack.
...
- dry-run the closeout pack selection and include `MIMAP-200A`;
- run the MIMAP-200A L2 guard;
- emit MIR once, build EXE from the exact MIR artifact, and run the EXE;
- assert VM and EXE output parity for the release apply bridge proof app;
