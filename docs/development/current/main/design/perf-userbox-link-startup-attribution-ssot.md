...
The next owner is the ret0.exe env exact top-symbol distribution inside the
startup executable contribution as one owner, not separate stdio / registry
work.

`PERF-USERBOX-023` is the required next probe:
...
If the env exact top-symbol distribution still carries meaningful executable
