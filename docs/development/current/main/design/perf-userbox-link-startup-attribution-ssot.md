...
`PERF-USERBOX-025` showed the nyash_kernel_runtime exact top symbols, and
`PERF-USERBOX-026` split the registry exact top symbols inside that owner.
The next owner is the ret0.exe nyash_kernel_runtime runtime exact-top-symbol
distribution inside the startup executable contribution as one owner, not
separate env / stdio / path work.

`PERF-USERBOX-027` is the next probe to land:
...
If the runtime exact top-symbol distribution still carries meaningful
