...
The next owner is the ret0.exe bucket symbol variability distribution inside
the startup executable contribution as one owner, not separate env / stdio /
registry work.

`PERF-USERBOX-019` is the required next probe:
...
bucket_env_symbol_mode=...
bucket_env_symbol_mode_count=...
bucket_once_symbol_mode=...
bucket_once_symbol_mode_count=...
bucket_path_symbol_mode=...
bucket_path_symbol_mode_count=...
bucket_minimal_main_symbol_mode=...
bucket_minimal_main_symbol_mode_count=...
bucket_nyash_kernel_runtime_symbol_mode=...
bucket_nyash_kernel_runtime_symbol_mode_count=...
...
If the ret0.exe bucket representative-symbol distribution still carries
