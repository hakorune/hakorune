...
The next owner is the ret0.exe bucket variability distribution inside the startup executable contribution as one owner, not separate env / stdio / registry work.

`PERF-USERBOX-018` is the required next probe:
...
If the ret0.exe bucket distribution still carries meaningful executable
