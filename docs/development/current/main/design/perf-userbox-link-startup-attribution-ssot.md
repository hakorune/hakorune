...
inside that owner, and `PERF-USERBOX-032` now splits the registry
register-many exact top symbols inside that owner. Keep the nyash_kernel_runtime
lane closed from env / stdio / path and only split within the exact registry
