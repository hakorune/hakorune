...
The path exact top-symbol distribution flatlined in `PERF-USERBOX-024`, so
the next owner is the ret0.exe nyash_kernel_runtime exact top-symbol
distribution inside the startup executable contribution as one owner, not
separate env / stdio / registry work.

`PERF-USERBOX-025` is the required next probe:
...
trial_count=3
startup_runs_per_trial=50
primary_bucket_env_count=...
primary_bucket_once_count=...
primary_bucket_path_count=...
primary_bucket_minimal_main_count=...
primary_bucket_nyash_kernel_runtime_count=...
primary_bucket_alloc_count=...
primary_bucket_ffi_count=...
primary_bucket_string_count=...
ret0_top_count_min=...
ret0_top_count_max=...
...
If the nyash_kernel_runtime exact top-symbol distribution still carries
meaningful executable contribution, keep splitting inside the startup
executable owner. If it goes flat, stay on the loader/libc floor and do not
reopen env / stdio / registry work.
