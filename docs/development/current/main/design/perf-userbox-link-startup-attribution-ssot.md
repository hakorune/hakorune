...
the box_factory exact top symbols next. Keep the nyash_kernel_runtime lane
closed from env / stdio / path; when the raw report shifts to box_factory,
pivot the next owner there instead of forcing registry-only reads.
