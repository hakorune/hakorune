...
the box_factory exact top symbols next and `PERF-USERBOX-043` records whether
the box_factory plugin/factory_type focus is actually present. Keep the
nyash_kernel_runtime lane closed from env / stdio / path; when the raw report
