# Hako Alloc Segment Map Local Free Reuse Ledger Release Bridge Closeout SSOT
...
MIMAP-196A proves the daily L2 behavior:
...
- dry-run the closeout pack selection and include `MIMAP-196A`;
- run the MIMAP-196A L2 guard;
- emit MIR once, build EXE from the exact MIR artifact, and run the EXE;
- assert VM and EXE output parity for the release bridge proof app;
