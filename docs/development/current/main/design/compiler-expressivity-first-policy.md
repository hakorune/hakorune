...
## 追加の設計ガード: “完成品キット” を増やさない（SSOT）

表現力不足を unblock するために箱を増やすとき、**完成品キット（大箱の増殖）**が負債化しやすい。
このリポジトリは “compiler をきれいに作る” を最優先にするため、次を運用ルールとして固定する。

- 原則: **BoxShape → BoxCount の順**で進める（入口集約/SSOT化/共通部品化 → 最小箱の追加）
- 禁止（原則）: skeleton/PHI/walker を複製した箱の追加（完成品キット化）
- 数だけで増える軸（例: clusterN）は、箱を増やす前に parameter/table SSOT を検討する

詳細 SSOT: `docs/development/current/main/design/lego-composability-policy.md`
...
3. `StepPlacement/StepMode`（`InlineInBody`, strict/dev only）で “step を動かさない” ループを受理し、selfhost canary の freeze を compiler 側で unblock する（SSOT: `docs/development/current/main/design/coreloop-stepmode-inline-in-body-ssot.md`）
4. `Loop` を “構造箱” として扱える v1（LoopFrame + `Break/Continue(depth)`）へ進み、nested loop の自然な合成を可能にする（Phase 29bs）
5. selfhost/.hako 側の回避コードを減らし、削除可能な形へ収束させる
...
追加の “土台スロット”（語彙を増やさずに合成を保つ）:

- `ContinueTarget`（LoopFrame slot）: `continue` の飛び先は “常に step_bb” とは限らないため、LoopFrame で明示する
- `Cleanup region boundary`（CleanupWrap とセット）: cleanup の適用範囲と exit ルーティングを局所検証できるようにする
