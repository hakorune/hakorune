...
| `M34 pointer atomic vocabulary docs lock` | `live-docs` | capability substrate + docs | reserves native-pointer atomic load/store/CAS facade and route vocabulary after M33 while keeping AtomicCoreBox methods, MIR routes, `.inc`, NyRT exports, pointer attrs, and allocator policy inactive |
| `M35 native pointer atomic route proof` | `next-card` | MIR extern route + pure-first EXE | waits for M34 and native pointer proof boundaries; proves one pointer-shaped atomic route without noalias/nonnull widening |
