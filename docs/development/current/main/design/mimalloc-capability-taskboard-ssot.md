...
24. `M10c LLVM export attrs widening`
25. `M11c-required-vocab substrate-only Lowering(inline_required)`
26. `M11c-required-verify verifier-backed required inline acceptance`
27. `M12 mimalloc raw-page proof`
28. `M13 allocator fast-path EXE proof`
...
disabled. The next implementation target after that is still blocked `M10c`:
