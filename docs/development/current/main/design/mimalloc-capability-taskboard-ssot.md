...
| `M6 hako.atomic useful rows` | `live-narrow` | capability substrate | memory-order vocabulary plus ordered fence row are live; fixed-slot CAS/load/store/fetch_add are live through M27-M30 EXE proof rows; M35 activates only the direct native-pointer store route; ordered fixed-slot args and pointer load/CAS remain future splits; no allocator policy inside atomic module |
...
| `M36 TLS + pointer remote-free composition proof` | `next-card` | allocator app composition proof | waits for M35; composes TLS cache-slot and pointer atomic rows without owning production allocator policy |
