...
| `DEL-003 Stage1 delegate exposes lowering` | Rust Program-level lowering shape is recorded as `293x-274`; it resolves typed delegate target fields, checks method existence, rejects collisions, and stages forwarding methods. This does not close normal Resolver/MIR production activation: delegate forwarding remains production 0 / `ParkedSealed` pending `DELEGATE-SELFHOST-VALUE0-D0` and verified expansion. | Stage1 shape landed; production activation parked |
