...
- エラーは “どこで落ちるか” が 1 行で辿れる（`[plan/reject]` / allowlist の DomainPlan 表示など）。
...
3) DomainPlan → CorePlan の入口順序を SSOT 固定（normalizer 経路 vs recipe composer）
...
- pattern1/5/8/9, scan_with_init, split_scan
...
- **Phase ?: Pattern番号 → Feature-set 表現へ移行**
