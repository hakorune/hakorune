...
- 291x-754: old cfg-test `simple_while_coreloop_builder` shelf was retired with
  the coreloop v0/v1 composer shelf. Future simple-while expansion belongs to
  active recipe-tree / feature owners.
- test-only legacy harness は 291x-711 で棚卸しを開始。呼び手ゼロの
  `normalizer/loop_break.rs` は撤去済みで、break 系の再拡張は
  `ExitMap` / feature pipeline 側へ寄せる。
