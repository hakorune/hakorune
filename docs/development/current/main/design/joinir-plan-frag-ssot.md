...
### 対象（Scope）
- JoinIR line（意味ルート分類、legacy label: Pattern1-9）における Plan → Frag → MIR merge の限定されたパイプライン
...
| 用語 | 定義 |
|------|------|
| **JoinIR line** | (Route classification/Plan; legacy label: Pattern detection) → (Frag+Boundary) → (MIR merge) の限定されたパイプライン |
| **Plan** | ルート意味抽出・正規化フェーズ（DomainPlan → CorePlan） |
...
- **ルート識別**: どの制御意味ルート（legacy label: Pattern1-9）か
...
- **ルート分類知識**: legacy label: Pattern1-9 の違いを知らない（CorePlanのみ処理）
...
| **Fragでのルート識別** | ルート分類知識はPlan側（legacy label: Pattern識別） |
...
- **Route Classification**: 同じルート分類アルゴリズム（legacy label: Pattern Detection）
