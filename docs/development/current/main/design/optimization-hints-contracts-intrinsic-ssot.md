...
- `Lowering(inline_required)` is preserved into MIR-owned InlinePlan metadata
  as `request=required`, `verified=false`; required inline verifier/backend use
  is still disabled
...
| `InlinePlan` (MIR-owned metadata) | live for `Hint(inline/noinline/hot/cold)` and `Lowering(inline_required)` preserve | n/a | todo for required inline acceptance | n/a | soft same-module leaf inline only | live-narrow |
