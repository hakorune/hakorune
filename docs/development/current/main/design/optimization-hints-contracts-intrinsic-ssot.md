...
- optimization metadata is still not backend-active
- `Contract(no_alloc)` and `Contract(no_safepoint)` have a live-narrow MIR
  verifier check; backend export/use is still disabled
- inline planning is not backend-active; `Hint(inline/noinline/hot/cold)` must
  first be preserved into MIR-owned InlinePlan metadata before any transform
...
|---|---|---|---|---|---|---|
| `Hint` (`@rune Hint` / `@hint`) | done | done (noop) | n/a | n/a | todo | MIR InlinePlan preserve live |
...
| `InlinePlan` (MIR-owned metadata) | live for `Hint(inline/noinline/hot/cold)` preserve | n/a | todo for required inline | n/a | todo | live-narrow |

補足:
1. 現在の parser 実装は `noop` 契約（意味不変）で固定。
2. `Contract(no_alloc)` / `Contract(no_safepoint)` は MIR verifier
   で検査されるが、backend 最適化/export にはまだ使わない。
3. `Hint(...)` は MIR `inline_plans` に保存されるが、registry gate /
   backend 利用が揃うまでは backend-active ではない。
...
-> best-effort MIR transform or keep-call
```
...
-> verifier
-> fail-fast if not accepted
```

Backends must not inline by reading source rune strings or matching function
names. They emit already-inlined MIR or already-selected intrinsic/capability
routes.
...
この仕様は現時点で `Provisional`。`Contract(no_alloc)` /
`Contract(no_safepoint)` の MIR verifier は live-narrow だが、backend
optimization/export はまだ active ではない。backend 利用へ昇格するには
次を満たすこと。
