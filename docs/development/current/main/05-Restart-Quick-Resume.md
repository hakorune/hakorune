...
- lane:
  ...
  - CoreBox surface catalog work is landed and now a reference lane;
    ...
    `ArrayBox.slice`, `MapBox.size`, `MapBox.len`, and `MapBox.has`, are on
    the Unified value path; remaining cleanup is ArrayBox
    `get/set/pop/remove/insert` and remaining MapBox rows
