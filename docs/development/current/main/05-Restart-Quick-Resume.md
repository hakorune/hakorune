...
- current blocker token: `MIMAP-014C allocation fast-path stats observers`
...
- continue `phase-293x` after MIMAP-014B facade small allocation fallback; next blocker is MIMAP-014C allocation fast-path stats observers over the facade-owned object lifecycle queue; VM-LIM-001 is parked
