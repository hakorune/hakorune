...
- current blocker token: `MIMAP-046A OSVM-backed fast-path unreserve fail-fast diagnostics`
...
- continue `phase-293x` after `MIMAP-045B`; current blocker is `MIMAP-046A`,
  the OSVM-backed fast-path unreserve fail-fast diagnostics row
- next row: add duplicate/stale/unknown fast-path unreserve diagnostics without
  widening the success route
