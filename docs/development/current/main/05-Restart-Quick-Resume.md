...
- current blocker token: `MIMAP-046B post-fast-path-unreserve-failfast row selection`
...
- continue `phase-293x` after `MIMAP-046A`; current blocker is `MIMAP-046B`,
  the planning-only post-fast-path-unreserve-failfast row selection
- next row: select exactly one allocator/compiler/language task and write its
  focused card before implementation
