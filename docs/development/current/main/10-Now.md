...
- Repo cleanup lane: `phase-29cr`（P0/P1 landed; P2 landed: box arithmetic / box operators / runner plugin init + core-ish root relocations; P3 landed: MIR navigation root + builder / join_ir / loop_canonicalizer / passes / control_tree entry READMEs; P4 first slice landed: `helpers_pure_value.rs` / `helpers_layout.rs` / `helpers_value.rs` + `rc_insertion` facade split + `loop_break_helpers_common.rs` + `loop_break_helpers_break_if.rs` + `loop_break_helpers_realworld.rs` + `loop_break_trim_whitespace_helpers.rs`; next P4: `loop_break_helpers.rs`; fixed order は `root hygiene -> CURRENT_TASK slim -> src top-level cleanup -> src/mir navigation-first cleanup`）
