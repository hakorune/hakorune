...
- current active step is `phase-29x backend owner cutover prep`; the phase29x LLVM-only daily gate is green, `LLVMEmitBox` stays compat/proof keep, `CodegenBridgeBox` has no daily dependency, `phase2111` explicit emit/link proofs are archived, `phase251` lowering canaries are quarantined, `phase2044` semantics are separated by bucket runners, compat selfhost wrapper ownership is explicitized, and the next exact focus is `phase2044 llvmlite monitor-only keep bucket cleanup / manifest alignment`.
