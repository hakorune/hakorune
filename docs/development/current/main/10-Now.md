...
- top queued cut: `M3 overwritten-store / DSE widening beyond the landed same-block cut`
...
- `phase260x` is landed:
  - the memory-effect owner seam and stats surface now sit on their own top-level pass, and `M2` is landed
