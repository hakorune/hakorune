...
- Bootstrap retire lane: active Rust front is `phase-29cj` on `src/host_providers/mir_builder.rs`; current exact cut is the typed Program(JSON) input/value seam above `module_to_mir_json(...)`, while bridge/helper waves stay closeout-ready.
