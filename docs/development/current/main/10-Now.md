...
- optimization snapshot:
  ...
  - row status:
    ...
    - `6 DCE 強化`: partial; first reachable-only cross-block cut is landed in `phase176x`, first effect-sensitive `KeepAlive` pruning is landed in `phase177x`, the first generic no-dst pure cleanup slice is landed in `phase181x`, and unreachable block pruning is landed in `phase182x`
