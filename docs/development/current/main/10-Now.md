...
- current lane:
  ...
  - current implementation focus: MapBox get missing-key contract review
  - current phase goal:
    ...
    - next row reviews `MapBox.get(missing-key)` without mixing element
      publication
  ...
  - current blocker token: `MapBox get missing-key contract review`
  - execution mode:
