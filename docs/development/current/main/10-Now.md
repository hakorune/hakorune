...
- current implementation focus:
  ...
  - the `ny-llvmc` parity proving slice is also landed:
    ...
    - metadata-bearing Point local-i64 user-box fixture is now green on `phase163x_boundary_user_box_metadata_keep_min.sh` via boundary `pure-first` owner lane without compat replay
  ...
  - pre-optimization cleanup/doc sync is now landed too:
    ...
    - next parity box in that wave: accept the same metadata-bearing Point local-i64 user-box route through a single local `copy` alias
