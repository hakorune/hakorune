...
- current implementation focus:
  ...
  - pre-optimization cleanup/doc sync is now landed too:
    ...
    - structure lock: loop-carried corridor continuity now lives in the neutral helper `src/mir/string_corridor_phi.rs`; `string_corridor_placement` only maps that continuity to optimization candidates
