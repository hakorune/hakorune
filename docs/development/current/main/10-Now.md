...
- Structure-first implementation lane: `phase-29x`
  ...
  - current exact read:
    ...
    - `route.rs` compare/archive shrink is landed; next fixed order is `keep .ll tool seam -> thin remaining transport temp-path helper residue -> archive/delete retirement`
