...
- Repo cleanup lane: `phase-29cr`（P0/P1 landed; P2 landed: box arithmetic / box operators / runner plugin init + core-ish root relocations; P3 first slice landed: MIR navigation root + builder README + join_ir / loop_canonicalizer / passes entry READMEs; fixed order は `root hygiene -> CURRENT_TASK slim -> src top-level cleanup -> src/mir navigation-first cleanup`）
