...
- current implementation focus:
  ...
  - pre-optimization cleanup/doc sync is now landed too:
    ...
    - separate phase, not this cut: relax `phi_merge` or `call` / `boxcall` / `return` barriers only with a metadata-contract update first; `kilo_micro_substring_concat` still owes its next exact move to that loop-carried `phi_merge` route
