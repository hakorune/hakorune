...
- optimization snapshot:
  ...
  - landed generic-memory docs/facts follow-on: `phase199x generic memory DCE docs-facts phase`
  ...
  - landed generic-memory code follow-on: `phase200x dead Load pruning on private carriers`
    ...
    - first cut is intentionally narrow: same-carrier `Store`, `Call` escape, `Debug`, terminator-adjacent cleanup, and non-copy carrier merges stay outside this cut
  ...
  - landed generic placement/effect proving follow-on: `phase213x sum outer-box sinking consumer seed`
    - current sum lowering now reads `placement_effect_routes` first for the selected local-aggregate route
    - `sum_placement_selections` / `sum_placement_layouts` remain compatibility fallback
  - landed generic placement/effect proving follow-on: `phase214x user-box local body consumer seed`
    - current user-box local aggregate seeding now reads folded `placement_effect_routes` first for selected local-body subjects
    - thin-entry subject lookup remains compatibility fallback
  - landed generic placement/effect proving follow-on: `phase215x thin-entry consumer seed`
    - current thin-entry consumer seeding now reads folded `placement_effect_routes` first for selected thin-entry rows
    - `thin_entry_selections` remains compatibility fallback
  ...
  - landed generic placement/effect structure follow-on: `phase218x shared placement/effect route reader seam`
    - current boundary sum and user-box helpers now share one folded `placement_effect_routes` reader/matcher seam
    - legacy metadata fallbacks remain intact
  ...
  - `1 generic placement / effect`: partial; string corridor candidates, sum placement chains, thin-entry inventory/selection, the first folded `placement_effect_routes` owner seam, the placement-relevant `agg_local` fold-up, the first sum/user-box/thin-entry/boundary-sum-helper/boundary-user-box-helper consumer proving slices, and the first shared boundary route-reader seam are landed, but the top-level generic transform layer is still backlog
...
- current implementation focus:
  ...
  - pre-optimization cleanup/doc sync is now landed too:
    ...
    - targeted boundary proof is now pinned on `string_direct_kernel_plan_len_window_min_v1.mir.json`; the smoke `phase137x_boundary_string_direct_kernel_plan_len_min.sh` must hit `placement_effect_route_window` and keep `nyash.string.len_h` / `nyash.string.substring_len_hii` out of the lowered IR
