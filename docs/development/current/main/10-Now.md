...
inputs remain separate. The caller-zero S6A Facts/producer slice, exact
program-owned `Main.main` resolver ingress, and typed C/D/U/R envelope are now
landed with focused 2/2/8/11 cardinality tests. The active closeout is
duplicate-role/source-identity rejection and complete source-site coherence
negatives; read `CURRENT_STATE.toml` and the active workstream for its exact
execution brief.
