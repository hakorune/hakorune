...
- current implementation focus:
  ...
  - pre-optimization cleanup/doc sync is now landed too:
    ...
    - current live post-sink benchmark body is now pinned separately by `tools/smokes/v2/profiles/integration/phase137x/phase137x_direct_emit_substring_concat_post_sink_shape.sh`; that smoke now requires the helper-result `%36` to keep `publication_sink` / `direct_kernel_entry` plans and the scalar consumers `%88/%89` to keep `direct_kernel_entry` candidates on the live MIR, so the next exact-seed shrink can target metadata-backed scalar consumers instead of relying only on opcode positions
    - the separate `phi_merge` blocker is now pinned too by `tools/smokes/v2/profiles/integration/phase137x/phase137x_direct_emit_substring_concat_phi_merge_blocker.sh`; live direct MIR still carries `%21 = phi([4,0], [22,20])` and `%22 = phi([36,19])`, while proof-bearing `string_corridor_candidates` still stop at helper result `%36`
    ...
    - separate phase, not this cut: relax `phi_merge` or `call` / `boxcall` / `return` barriers only with a metadata-contract update first; `phase137x_direct_emit_substring_concat_phi_merge_blocker.sh` is the live blocker guard for that loop-carried route, and the post-store observer lane still owes `array.set + trailing length()` facts before widening beyond the new store-only sink
