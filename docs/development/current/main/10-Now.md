...
- active micro task: `47xC2 stage_a_route.rs source->MIR first switch`
- next micro task: `47xC3 stage_a_compat_bridge.rs explicit Program(JSON) fallback shrink`
...
- `phase-47x` is active; it removes the last live helper-route defaults from `--backend vm`, keeps explicit vm compat mode locked, and is now switching Stage-A to source->MIR first
