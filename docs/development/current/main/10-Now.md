...
- current lane:
  ...
  - current app gap read:
    ...
    - CoreBox router follow-up has moved `StringBox.length/len/size`, `StringBox.substring/substr`, `StringBox.concat`, `StringBox.trim`, and `StringBox.contains` to the Unified value path; remaining cleanup is about 5 family-equivalents, starting with one-arg `lastIndexOf`
