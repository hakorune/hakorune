...
- current implementation focus:
  ...
  - pre-optimization cleanup/doc sync is now landed too:
    ...
    - remaining string publication backlog is now narrower: broader `store` / host-boundary publication, plus final emitted-MIR return-carrier cleanup only if that later needs a direct guard
