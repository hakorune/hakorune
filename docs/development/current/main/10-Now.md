  ...
  - current exact result:
    ...
    - current direct-path optimization reading is fixed:
      ...
      - current `main kilo` reading is now fixed:
        ...
        - the next exact perf cut is now the edit-loop producer side (`substring(0, split) -> substring(split, len) -> concat3_hhh -> set_his`), not more observer acceptance; keep MIR concat3 canon as a probe lane for now and do not promote it beyond the current owner proof
        - asm target is now fixed again: C already reaches `load -> strstr -> malloc/memcpy append -> store` with `%64/%8` folded to mask/test, and Hakorune now reaches `get_hi -> string_indexof_hih -> concat_hs -> set_his` on the branch loop, so the remaining hot delta is the edit loop `substring x2 -> concat3_hhh -> set_his`
