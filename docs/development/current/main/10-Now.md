...
- current implementation focus:
  ...
  - pre-optimization cleanup/doc sync is now landed too:
    ...
    - the same phi smoke now also pins the exact-seed loop semantics (`phi/phi/phi`, positive loop bound, compare `<`, branch, and the latch `const 1` increment), so the remaining exact-seed work is now a semantic-boundary close-out rather than more raw body-shape cleanup
    ...
    - immediate close-out count before the next phase is three small boxes:
      1. decide/retire the preheader/exit semantic guard
      2. decide/retire the header/latch semantic guard
      3. land the exact-seed closing cut and then move on
    ...
    - fixed return order:
      ...
      3. landed first narrow `plan window across phi_merge` cut on the single-input backedge phi `%22`; keep merged `%21` non-window and treat any broader widening as a separate metadata-contract phase
