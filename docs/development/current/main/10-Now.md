...
- Repo cleanup lane: `phase-29cr`（P0/P1 landed; P2 landed: box arithmetic / box operators / runner plugin init + core-ish root relocations; P3 landed: MIR navigation root + builder / join_ir / loop_canonicalizer / passes / control_tree entry READMEs; P4 first slice landed: `helpers_pure_value.rs` / `helpers_layout.rs` / `helpers_value.rs`; next P4: physical clustering of `src/mir` giants; fixed order は `root hygiene -> CURRENT_TASK slim -> src top-level cleanup -> src/mir navigation-first cleanup`）
