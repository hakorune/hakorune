...
- current implementation focus:
  ...
  - the `ny-llvmc` parity proving slice is also landed:
    ...
    - metadata-bearing Point local-i64 and Flag local-bool user-box fixtures are now green on `phase163x_boundary_user_box_metadata_keep_min.sh` via boundary `pure-first` owner lane without compat replay, including the Point single-copy alias route
  ...
  - pre-optimization cleanup/doc sync is now landed too:
    ...
    - next parity box in that wave: widen the same metadata-bearing local user-box keep smoke into Float declared-field routes
