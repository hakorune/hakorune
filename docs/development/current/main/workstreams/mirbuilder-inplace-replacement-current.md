...
Smallest next slice: OWN-FIELD-CONTAINER-DEST-D0 fixes checked intrinsic Map allocation identity/lifetime through Normal projection, install/root end and descriptor/C consumer. Infallible Clone forbids Owned in public NyashBox MapBox: keep Native-only public and non-NyashBox checked facades over common storage, one table per Map. Accepted premise reset and counterexample are in the ownership task. Retire selected birth_h publication with actual cutover; no intake or source execution claim.
