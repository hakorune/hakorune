...
Smallest next slice: carry the exact ordinary child through the existing owner-indexed physical state, then connect existing ordinary ABI/cleanup; static-child draft capture and artifact finishing now recheck the same captured PhysicalBoundary after construction/New and field/terminal validation.
...
Checkpoint: child Completion/terminal, root-exit/literal/field progress and PhysicalBoundary retention/recheck are complete after S6C; child symbol coverage is included. Mixed AppMain/ordinary Map, ordinary install/ABI, and Physical Map execution remain stopped. Selected child-symbol-drift, child/root/field tests pass; source max759.
