...
Smallest next slice: consolidate existing New ledger expression/local/check progression before adding Map progress. Preserve Installed-before-Checked, unavailable completion and batch preflight. Source snapshot BoxShape is closed (6 tests); four per-entry arrays replaced with initial order and issued transfer/replacement points. Quality queue owns ledger/terminal tasks and acceptance. Map binding/C execution remain open; source install Stop stays until source/OBJ/EXE and old-edge cutover.
