...
closed — Generic candidate S1; Direct S1; Nested S1; LoopTrue S1; LoopCond S1 -> caller-zero observers, tests/guards/docs green, no selector/Recipe/Builder/MIR/production. closed — common admission D0 -> worker-reviewed five-row `AdmissionWindow`, LoopCond C/D/U/R source contract, resolver brand, assembler≠selector; shared family guard/docs synced. next Generic normalization, then assembler; selector/production/retirement closed.
```
