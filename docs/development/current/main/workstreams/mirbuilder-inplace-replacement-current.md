...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private invocation ownership is verified; next is the accepted Named observation binding in the existing MIR planner. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: bind copied C Named observations into existing MapBodyIndex and close demand/domain/original lanes together. Handle follows allocating outcome; Alias(0) follows original NewBox.args[0] through fixed point and gets an explicit V2 action. Delete three provisional Named treatments. Validate exact sites/duplicates/required failures; preserve Map-only Float and mixed old uses. Query/host/source remain pending; full contract is in the Map owner.
...
Checkpoint: private invocation is verified at79c919352e. C worker plus Rust audit close query role/readiness design: entry identity selects conditional walker; definition eligibility is not a query prerequisite. Actual query caller will own program observation cache and wire uniqueness. Next MIR binding is a planner dependency, not production cutover. Prior C/lifetime/settings/output evidence remains valid.
Private invocation consolidation is implemented. Conditional representation is distinct from execution coverage; pattern success/partial skips remain real output concerns. Query/session, host/frame/alias/capability binding and Map C cutover remain open.
