...
Decision: retained static V2 host and raw/Core literal cutover are implemented; public V1 ingress and six literal emission edges are retired. Preserve distinct generic copy/boxed and checked Home contracts. The bounded callable slice now selects the AppMain root plus one exact called ordinary child Map owner for scalar Value-only install through the existing owner-indexed row and lifecycle physical consumer; wider ownership remains separate.
...
Smallest next slice: widen ordinary Map ownership only after the selected scalar Value consumer has a stable artifact acceptance; keep static-child capture and artifact finishing on the same captured PhysicalBoundary after construction/New and field/terminal validation.
Seed/result/index share `Rc`, S6C consumes exclusively, and the selected root plus one exact called ordinary Map owner is admitted only for scalar Value rows with no New/Home transfer. Uncalled or additional ordinary owners, unknown values, and ownership transfer remain stopped; preserve unsupported install/exporter edges until the physical consumer is complete.
