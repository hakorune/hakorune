...
Smallest next slice: native Map GC owner projection and typed snapshot/trace failure through RcDiagnostic and kernel metrics. Remove GC get_data/skip and module-root empty-on-error; replace last counters with one finite observation, nonComplete counts=null. OWN-FIELD-CONTAINER-DEST-D0 owns finite scope and acceptance. Native clone semantics retained; JSON/public read/owned entry/end and checked ABI/source activation remain open.
