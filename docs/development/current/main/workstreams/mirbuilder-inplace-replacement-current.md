...
| D | Loop prerequisite closure in `joinir-loop-selfhost-recipe-pipeline-ssot.md` M10 | Semantic co-seal is `NoSafeSlice`; transfer/boundary/If/Exit rows are closed. S6C has no selected consumer/delete set; S6D D0 is `NoSafeSlice` (typed predicate/Recipe-JoinSig issuer/consumer/delete tuple absent); S6E awaits observed dispositions. No D row is executable yet. |
