...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary` (4/7): QUALIFIED-PREFLIGHT-S0, UNTYPED-OBJECT-STORAGE-S0, EXACT-USIZE-S0, MAIN-IMPORT-ARITY-SCOPE-S0 landed (json_stream->`loop-cond-item-unsupported`, boxtorrent->`birth-global-legacy-stopped`). Latent `entry-shape-mismatch` proven for all `main(args)`; next `MAIN-WRAPPER-PARAM-ENTRY-D0`. Gates 2-4 queued. |
