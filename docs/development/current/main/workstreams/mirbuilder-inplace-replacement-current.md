...
Caller-zero Prelude, After, Tail/Completion, and DraftSeal are closed: real Prelude -> seven operations -> sealed After -> exact Tail ABI/Completion -> typed finish -> DraftSeal prepare/commit, with no collector publication. Next is docs-only `CALLABLE-LOOP-PRODUCTION-EDGE-D0`: census one named caller and old edge before any switch; production selection, Generic G0 parity, retry/fallback, and legacy retirement remain closed.
