...
Smallest next slice: design-only D0-SIG-CLOSE: choose the package-lifecycle mapper owner, census/close package-only install escape, freeze installed-wrapper mutability and scoped &mut raw-port reborrow, map the Cataloged old-payload delete set, and write focused acceptance/guard registration; no code, profile widening, AST rescan, public receipt, or package field.
