...
Smallest next slice: change the existing pending and exit local-binding storage to source-ordered site-tagged groups, validate each group against the co-sealed local Call sequence before push/transfer/rebind/finalize, then widen `map_target_owners` from two to three.
