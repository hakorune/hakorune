...
Smallest next slice: design actual Map native/owned entry storage plus public/kernel observer and consuming-end mapping. Detached opaque outcome contract is accepted; native remove/clear lock-held Drop is removed (Map8 pass, reentry/once-only/capacity). OWN-FIELD-CONTAINER-DEST-D0 owns finite storage-to-observer/end scope. Keep one payload table and explicit unavailable versus missing. Checked ABI/lifecycle execution and public cutover follow; no native-only completion claim.
