...
Smallest next slice: design-only D0-SIG-CLOSE-F: freeze C' package-private install plus one-shot crate::mir callback/token into a Builder-private by-value package+loan bundle, then close scoped &mut raw-port reborrow, Cataloged old-payload delete set, and focused acceptance/guard registration; no code, profile widening, AST rescan, public receipt, or package field.
