...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Step1 domain closure and C Named emitter-selection extraction are verified; next is preflight binding design. Literal issuers stay unchanged until consumers are ready.
...
Non-authority: old generic i64/origin, typed-plan presence, runtime tests and incidental linkage cannot supply kinds, selected consumer or source acceptance. Rust must not copy Named-spelling dispatch or infer the C walker.
...
Smallest next slice: one read-only worker audits AllocationPreflightEntryRoleBindingMissing: reuse C program-view/function planning to bind role, immutable program and array setting to one borrowed preflight/compile session. Integrate one Decision and bounded next implementation before query/frame edits. Prescan reconciliation stays explicit; do not copy a Rust Named classifier or open Map source production.
...
Checkpoint: shared C selector removes both emitter priority copies; full C build, selector boundary test, published-row proof and corridor guard pass. Both-walker60 physical cases match parent d28498d591 in return/diagnostic and machine-code/relocation digest (32 objects,28 rejects); alias exit30. Prescan/global layout rejects are dependency evidence. Prior Map-focused24 remains valid; Named preflight, boxed-sum binding, leaf/frame and C Map consumers remain open.
