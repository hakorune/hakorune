...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary` (4/7): ORDINARY-NEW-HANDLE-ARG-S0 landed — `Handle` selected-argument kind + observation-preserving fallback coseal (declared parameter contracts); json → `static-result-ingress/no-exact-static-target` (`env.get/1` in imported `StringHelpers`). D10 open: builtin/imported static receiver authority census. Remaining: D5-sealed forks, NamedArray/toolchain/inference. Gates 2-4 queued. |
