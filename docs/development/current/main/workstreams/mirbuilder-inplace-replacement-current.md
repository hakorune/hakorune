...
Smallest next slice: audit MIR/physical/C Value install with explicit representation proof. Inline CheckedMap runtime/ABI completed: storage7/kernel9, fresh archive descriptor1/export/header/guards pass. Source/C install Stop remains. Before ordinary callable connection, replace crossed-scopes-only cleanup summary with actual Completion obligations. C budget repetition is existing thinning task13; no broad optimizer or generic fallback.
