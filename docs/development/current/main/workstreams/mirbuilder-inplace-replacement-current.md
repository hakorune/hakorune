...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | R6-S0/S1/S2 landed. R6-S3 landed 2026-09-25: `reject_selected_dynamic_legacy_callsites` now rejects every `LegacyCallV0` carrier (`call-legacy-carrier`) on the selected-Dynamic artifact lane — the last open re-entry into ny_llvmc production; shared predicate stays narrow for v0 boxcall egress. R7 caller-zero D0 closed `NoSafeSlice` 2026-09-25: LegacyCallV0 + func slot stay live via two quarantined ingress minters (unified-off Value; v0 boxcall Method) — reopen when both retire. Next: `MIR-CALL-INGRESS-MINTER-RETIRE-D0` selects one bounded ingress-minter retirement; aggregate deletions frontier-paused meanwhile. |
