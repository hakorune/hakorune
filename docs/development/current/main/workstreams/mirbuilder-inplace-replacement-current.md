...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: S4 landed (all armed loop lanes take/install Selected handoffs via `lower_with_source_publication`); S5 landed — source-backed emit-mir-json publishes via artifact validation + `build_published_body_root`, discharging named-array retention. EXE now at `ordinary-new/local-commit/artifact-unowned-lifecycle-site`. VM `ingest/1` unchanged (ledger-less). Gates 2-4 queued. |
