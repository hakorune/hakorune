...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary` (4/7): QUALIFIED-PREFLIGHT-S0, UNTYPED-OBJECT-STORAGE-S0, EXACT-USIZE-S0, MAIN-IMPORT-ARITY-SCOPE-S0, MAIN-WRAPPER-PARAM-ENTRY-S0 landed (`main(args)` adopts injector locals via `StaticInjectedLocals`; diversion uses declared arity). Remaining classes: `birth-global-legacy-stopped` x3, loop-cond/NamedArray/toolchain/inference x1 each; next `OWNER-SELECTION-D3`. Gates 2-4 queued. |
