---
Status: Fast — MIR-CALL-CORE-R6-D1U-CONCAT-LEN-EXTERN-ISSUER-I0
Date: 2026-08-25
...
```text
  current decision  = MIR-CALL-CORE-R6-D1U-CONCAT-LEN-EXTERN-ISSUER-I0
  implementation    = D1T ConcatSubstring landed at a1e856fa25; D1U is the selected paired ConcatSubstringLen family (selected writers 9 -> 7, concat 6 -> 4)
  mode              = fast
  production stop   = the paired left/right Len writers only, exact Extern/dst/args/effects, and no one-sided or adjacent family
...
Decision: fast — replace only the paired ConcatSubstringLen direct Call literals with the thin canonical constructor, preserving fresh ValueIds, middle length Const/Add, outer destination, effects, spans, hints, and phase-9 fusion.
Source authority + canonical issuer: ConcatSubstringLenPlan owns left/right source windows, outer dst, middle length, and effects; the Len variant fixes SUBSTRING_LEN_EXTERN and MirInstruction::call is the issuer for both calls.
Non-authority: apply-side ValueId allocation, value-type metadata, middle arithmetic, legacy func, fusion/DCE, hint/CFG, backend lookup, PURE defaults, and retry.
Fail-fast boundary: outer length, concat triplet, const middle, and both substring shapes must hold; otherwise Len plan=0 and both Calls, arithmetic, and hint publication remain zero.
Smallest next slice: `concat_corridor_apply.rs:305,325` only; selected writers 9 -> 7 and concat family 6 -> 4.
Non-claims: InsertMid/receiver/unified/array/canonicalizer writers, collector/plan, ValueId/fusion schedule, Call final shape, Method(None), ingress contract, MirCall/CallFlags, schema, metadata, backend/wire, fallback, and cleanup.
...
```text
Now
 MIR-CALL-CORE-R6-D1U-CONCAT-LEN-EXTERN-ISSUER-I0 -> fast selected: paired left/right ConcatSubstringLen writers in concat_corridor_apply.rs; exact Extern/dst/source-window/effects and fusion/Return parity only; selected writers 9 -> 7 and concat family 6 -> 4
