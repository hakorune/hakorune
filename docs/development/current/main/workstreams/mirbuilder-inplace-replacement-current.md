...
#### MAP-LITERAL-THREE-ORDINARY-OWNER-COVERAGE-D0 — design stop

Decision: design exactly two local Calls A/B plus one terminal Call C for three distinct ordinary Map owners; no implementation permission until local binding retention is closed.
...
Smallest next slice: design `root_call_entry` multi-site binding storage/finishing and duplicate/site-drift checks, then widen `map_target_owners` from two to three only.
