...
Smallest next slice: close exact-site query against the real pattern/region/skip owners and original MIR host binding. Pattern match can emit/fail before generic continuation; reset-batch can retain partial skips after rejection. Do not classify Bypassed from metadata or duplicate clean matchers. Deferred plan observation is ready, but full session ABI requires one actual role/skip owner and caller/delete-set; see Map owner.
...
Checkpoint: document/config owners and diagnostic-free definition observation are verified. Plan retains result/overflow detail; sole prescan sends original first-error diagnostics at its unchanged stage. Owner test, C build/published/corridor, Named60/role22 parent comparison, document ASan8 and Dynamic LLVM/origins/machine code/relocations pass.
Settings mutation16 remains prior evidence; changed sources below800. Host V1/file deletion, input-aware capability, alias demand/domain/original lanes, pattern/partial skip, boxed-sum, leaf/frame and Map C consumers remain CutoverBlockerOpen.
