...
Smallest next slice: actual Map Invoke/key/outcome emission progress and operation-derived projection at the shared ledger/cleanup consumers. Installed-Home lookup and end-operation issuance are consolidated (8 tests); Map binding is not yet issued. Ownership task records full mixed-cleanup callers and repaired parent fixture debt. Preserve empty-Map observation, unused aliases and prior Maps; source install Stop stays until source/OBJ/EXE cutover.
