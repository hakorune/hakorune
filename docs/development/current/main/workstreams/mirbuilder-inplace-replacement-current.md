...
closed — GENERIC-G0-CANDIDATE-S1; Direct S1; Nested S1; LoopTrue S1 -> caller-zero observers, tests/guards/docs green, no selector/Recipe/Builder/MIR/production. closed — LOOP-FAMILY-COMMON-ADMISSION-WINDOW-D0 -> worker-reviewed five-row `AdmissionWindow` design, resolver brand, C/D/U/R algebra, assembler≠selector; docs synced. next Cell A LoopCond D0, then Generic normalization and assembler; selector/production/retirement closed.
```
