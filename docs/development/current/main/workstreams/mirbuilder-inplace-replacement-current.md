...
closed — GENERIC-G0-CANDIDATE-S1; Direct S1; Nested S1 -> caller-zero observers, tests/guard/docs green, no selector/Recipe/Builder/MIR/production; Nested preserves forest errors. design-stop — LOOP-FAMILY-LOOPTRUE-OBSERVATION-S1 -> worker-reviewed source-to-neutral matrix and finite caller-zero slice fixed; next is identity extension, test adapter, observer/tests, shared guard, and reference sync. Selector/production/physical/retry/fallback/legacy retirement closed.
```
