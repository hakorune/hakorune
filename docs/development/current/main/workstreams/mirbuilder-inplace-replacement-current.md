...
Smallest next slice: accepted BoxShape moving existing ordinary result/selected/formal rows through successful consuming scope finish into completed/finalized owner. Original Completion suffices for bounded literal terminal Call; Loop JoinSig was an incorrect premise. No new receipt or AST retention. Preserve execution Stops; next Invoke dependency requires Normal-only I64 and full pending cleanup. Exact acceptance is in Map owner finalization frontier.
