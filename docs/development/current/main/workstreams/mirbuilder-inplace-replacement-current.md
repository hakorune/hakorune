...
Smallest next slice: implement accepted CheckedMap I64/Bool/Residence payload and checked value-install ABI, sharing the existing install state machine. Source capability slice closed at990a992738 (package146/control33/guards). Map owner fixes runtime caller inventory, failure order and acceptance. No scalar Box/residence wrapper, source/C promotion or generic fallback; retain Indexed Home consumer and install Stop.
