...
Smallest next slice: close MapOwnedCanonicalObjectResidenceMissing interface/profile/thread identity, then connect source Normal/transfer flow and existing root cleanup. PlainI64NoHook compatibility is semantic only: current negative indexed object identities cannot enter positive host Handle5 clone storage. Keep escape Stops. Reuse ordered root bindings and first-Fault suffixes; separate install from detached-old end. Source/runtime/public cutover remains open.
...
Checkpoint: source integer-evidence Stop passes package119, including both parent5ccd2302ce field+Bool failures and 12 new mixed/coarse-operand rejection cases with no retained terminal/field rows. Exact pair, integer-only nested Add and standalone results remain green. Pointer/corridor and diff checks pass; changed source max628. The preceding parent baseline receipt remains historical evidence in OWN-FIELD-CONTAINER-DEST-D0; those two reds are now closed. No whole-lib or Map cutover claim.
