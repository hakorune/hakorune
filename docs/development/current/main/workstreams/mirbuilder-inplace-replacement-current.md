...
Smallest next slice: select the exact ordinary child via owner-indexed Completion and terminal relation, carry it into the existing physical state and owner-indexed root-exit/literal progress, then connect existing ordinary ABI/cleanup; seed/result/index share `Rc`, S6C consumes exclusively, mixed AppMain/ordinary Maps remain stopped. Preserve install/exporter Stops; EXE/linked OBJ/Fault remain open.
...
Static V2 evidence and parent-confirmed non-plugins baseline are recorded in the collection SSOT; no whole Map/R7 closure.
Checkpoint: owner-indexed Completion and terminal relation retention plus owner-keyed root-exit/literal progress land after S6C without a second semantic relation owner or mutable root slot; mixed AppMain/ordinary Map remains stopped. Package158/158 and the ordinary-child literal ownership test pass; source max751. Physical Map execution/intake/cutover remains open.
