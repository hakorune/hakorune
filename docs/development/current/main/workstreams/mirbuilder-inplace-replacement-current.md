...
| F | `LOOP-PRODUCTION-SELECTION-D0` → Loop/M10b → M11 → M12 | D0 + S0 manifest (44 rows) landed. **M10b-I0-R0 card open** (design_stop): switch = source -> facts -> 5-family window -> family selector -> demand -> producer -> recipe -> one physicalizer; P1 spine + P2 edge (caller-zero) -> R0 flip. **Coverage-gap design stop**: 4 of 8 attested routes lack an admitting family — window-only switch would freeze accepted programs; family authority must be named before P1 resumes. |
