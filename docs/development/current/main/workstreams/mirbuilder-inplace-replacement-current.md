...
Smallest next slice: capture unchanged runtime mode flags plus the separate exact-slot-helper predicate once at file ingress. Three settings/eight reads feed Named, field/method and runtime requirements. The Map owner supersedes the two-bit premise and requires 12 input classes plus post-capture ambient mutation; helper must not become a runtime bit. No new global/session ABI or source switch; session observation/host/alias mapping remains open.
...
Checkpoint: one file owner now parses/frees the selected pure-first document; validator/census/route/generic consumers borrow it. C build, published rows and corridor pass; Named60/role22 match prior baseline, Dynamic LLVM/origins/machine code/relocations match.
ASan/counters pass eight success/error/pattern/Dynamic cases. Independent planned stores and repeated document reads/frees are removed. Map-focused24 stays dependency evidence; session, alias-demand, boxed-sum, leaf/frame and Map consumers remain open.
