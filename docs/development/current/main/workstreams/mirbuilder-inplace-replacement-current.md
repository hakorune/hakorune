...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: COMPOSITE-RETURN-EXIT-S0 landed — composite LoopBreak admits return-in-body `loop(cond){…return…}` (`RootBreakMissing`→`RootExitMissing`); `starts_with/3` lowers. json → exit-free `loop(cond)` declines (EXE `StringHelpers.trim/1`, VM `JsonStreamAggregator.ingest/1`). D13 open: exit-free loop ownership census. Remaining: D5-sealed forks, NamedArray/toolchain/inference. Gates 2-4 queued. |
