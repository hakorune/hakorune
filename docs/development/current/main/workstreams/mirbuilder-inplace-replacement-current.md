...
Smallest next slice:
  `CHRONIC-MEASUREMENT-SITE-OWNER-MAP-D0` is the only next design cell: strict-verify the tracked 185-row neutral receipt against frozen scope/hash/non-empty-range rules, then classify every site with explicit owner/role/successor evidence; expectation pinning, guard registration, and deletion remain closed.
