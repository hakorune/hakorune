...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary` (4/7): TARGET-ONLY-EMISSION-S0 landed — `member_route` StaticReceiver TargetOnly arm emits via `lower_target_only_static_result_publication_v1`; json → `ordinary-new/argument-source-unavailable` (`statsFor` `new UserStats(user)`). D9 open: ordinary-new argument-source census. Remaining: D5-sealed forks, F3b/F3c, NamedArray/toolchain/inference. Gates 2-4 queued. |
