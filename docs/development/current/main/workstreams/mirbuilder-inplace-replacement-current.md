---
Status: Active workstream — fast path (R5b-B0 Rust interpreter terminal)
Date: 2026-08-25
...
```text
  current decision  = MIR-CALL-CANONICAL-RUST-INTERPRETER-NONE-REJECT-B0-I0-R0
  implementation    = R1, D1, Topology-I0, R2-I0, R3 D0, R3a, R3b, R4a, R4b, R4c, R4d, R4e, R4f, Query D0/T0, R5 D0, and R5a closed; R5b-B0 active
  mode              = fast
  production stop   = before R5b method fallback, R5c/R5d, and Call core field deletion
  exit              = selected Rust `None` edge rejects before register lookup; typed dispatch parity, shared guard, and no R6 claim
fallback / retry  = 0
```
...
7. R5 D0 is accepted from the worker census; R5a optimizer-only is closed at
   `e36f86e869`. R5b-B0 Rust VM `None -> func` rejection is active; printer/JSON/native remain
...
```text
Now
 MIR-CALL-CANONICAL-RUST-INTERPRETER-NONE-REJECT-B0-I0-R0
  -> reject selected Rust VM `None -> func` before register/module lookup

Next (not selected)
  -> printer-only, JSON D0, native D0; then R6 gate
