...
Smallest next slice: carry the existing ordinary Call row through the owner-indexed physical state and finalized root handoff, project that exact I64 Call through the existing physical program/compiled-entry contract with its cleanup coordinates, then connect the existing ABI/formals/result/cleanup consumer; static-child draft capture and artifact finishing recheck the same captured PhysicalBoundary after construction/New and field/terminal validation.
Seed/result/index share `Rc`, S6C consumes exclusively, mixed AppMain/ordinary Maps remain stopped. Preserve install/exporter Stops; EXE/linked OBJ/Fault remain open.
...
Checkpoint: child Completion/terminal, root-exit/literal/field progress and PhysicalBoundary retention/recheck are complete after S6C; the existing ordinary Call payload is rebound after finishing and moved once into the finalized root source handoff, child symbol coverage is included, and the Rust physical program/compiled-entry contract now carries the exact ordinary I64 Call plus existing cleanup coordinates.
Mixed AppMain/ordinary Map, C ordinary-call schema/emission, ordinary ABI/install, and Physical Map execution remain stopped. Selected child-symbol-drift, Call-handoff, child/root/field and physical Call-admission tests pass; source max759.
...
Private invocation/query consolidation is implemented. Conditional representation is distinct from execution coverage; pattern success/partial skips remain real output concerns. The static V2 public host and raw/Core source checkpoint is verified; remaining callable selection and wider source obligations stay open in the collection SSOT.
