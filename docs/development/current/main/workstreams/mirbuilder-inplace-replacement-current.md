...
closed — Loop-family observer/admission/selector receipts are caller-zero; D4-S4-S0=NoSafeSlice and Generic selection/handoff witnesses are closed. `GENERIC-G0-DEMAND-S3-D0` is accepted; current `GENERIC-G0-DEMAND-S3-I0-R0` is caller-zero: selector owns one canonical lease, handoff borrows only a private brand, and S3 retains exact source roles without Recipe keys. Generic Recipe, Builder/MIR, production, and retirement remain closed.
```
