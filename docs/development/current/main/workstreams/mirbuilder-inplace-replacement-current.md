...
Smallest next slice: I0 must carry the accepted two-Call source relation through the selected production artifact path, then prove OBJ/EXE Pair exit30 and the existing cleanup verifier before deleting the remaining single-terminal-only edge.
...
The new two-Call source and MIR consumer tests prove local result binding, owner-scoped co-seal, strict CFG verification, and two physical I64 Call terminators; artifact exit30 remains the next evidence.
Uncalled or additional ordinary Map owners, New/Home transfer, unknown scalar values, and wider physical Map install/value ownership remain stopped. Ordinary Call ABI/OBJ/EXE/Fault evidence is closed for the bounded Pair and the root-plus-one-ordinary-child scalar cohort; no whole Map/R7 claim. Selected owner-gate, child-symbol-drift, Call-handoff, child/root/field, physical Call-admission and ignored Pair/Bool/ordinary-child runtime tests pass; source max759.
