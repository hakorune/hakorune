...
Smallest next slice: actual Map source emission/progress and local completion via the existing callable child port/ledger, then mixed cleanup/finishing and C consumption. New ledger progress is consolidated (16 tests), preserving Installed-before-Checked and unavailable completion; source snapshots are closed. Share terminal dispatch when root validation is touched. Map binding/C execution remain open; source install Stop stays until source/OBJ/EXE and old-edge cutover.
