...
Smallest next slice: source-connected terminal Call Invoke with Normal-only I64 and full pending cleanup in existing emitter/ledger. Successful scope retention is implemented (package155/bridge4; root24 pass/4 parent5ac03c0e47 baseline reds; max712). Original Completion and affine row remain authority; no new receipt. Preserve ordinary install/artifact Stops until execution evidence; exact next mapping and baseline names are in Map owner finalization frontier.
