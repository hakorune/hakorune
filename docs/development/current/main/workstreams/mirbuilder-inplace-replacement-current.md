...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | All production `LegacyCallV0` ingress minters retired (R6-S0..S5; `boxcall` wire stays a v0 contract; llvmlite projection kept). Caller-zero D1..D3 accepted bounded; S6 deleted the canonicalize Method arm; S7 `ec9a78254a` deleted `canonicalize_legacy_array_write_calls`; next `MIR-CALL-R7-CANONICALIZE-LEGACY-CLOSURE-ARM-DELETE-S8` deletes the dead `LegacyCallV0{Closure}` arm. |
