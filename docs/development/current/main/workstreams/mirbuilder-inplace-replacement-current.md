...
| E | Loop/M8 S6/M9 S7 | S0 `798551e8de`; S6D `b0fe3bf6a2`; S6E `0bdfbda1da`; **S6G `b419b84c3d`** all19 closeout; M8 closed. **S7A `f6c1d71015`** (D1): `.hako` wire subtree; no `.hako` calls/Map/Array executable. **S7B1 `f4b1432929`**: M8A wire + parity vs producer issuers; M9 producer half deferred. **S7B2 landed**: M8B `variable_accum_break_v1` wire — first If/Exit/exit-row coverage; parity vs `produce_variable_accum_break_recipe_v1`. |
