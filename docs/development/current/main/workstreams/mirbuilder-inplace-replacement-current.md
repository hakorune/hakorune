...
| E | Loop/M8 S6/M9 S7 | Landed 3 App-Main0 D0s; reds baseline. S0 `798551e8de`: 270 edge rows. S6D `b0fe3bf6a2`. S6E `0bdfbda1da` (cursor 18 Exact-only). **S6G landed caller-zero** `b419b84c3d`: 19-row observation set + coverage proof + `NoCandidate` arm + 19-route parity receipt; M8 coverage closed. **S7A wire D0 open**: `.hako` `LoopRecipeArtifactV1` wire subtree + stdout entry + Rust normalize parity harness, caller-zero. |
