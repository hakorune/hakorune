...
Smallest next slice: thin Map source snapshots before binding the new emitter; retain initial order and exact transfer/replacement deltas at the existing source owner. Map Invoke roles/lifetime and JSON dependency pass check plus12/7 tests. Then connect actual source progress/mixed cleanup; no Map binding/C execution yet. Quality queue owns acceptance and existing SSA/global-state tasks. Source install Stop stays until source/OBJ/EXE and old-edge cutover.
