...
Source authority + canonical issuer: no final non-empty Cataloged issuer exists; a future private pre-adapter root owner must co-seal owner/site/source/session/brand with an already-issued exact Callee. Exact catalog and GC rows remain final authorities. Cataloged I0 preserves LocalValue/Extern parity only after that issuer exists. Raw lineage maps once at the package-to-raw boundary; the consumer calls MirInstruction::call without rerunning the resolver.
...
Smallest next slice: D0-SIG-CLOSE-F.1 issuer-facts reconcile: keep `prepare_cataloged_target_v1` explicitly transitional, reject reuse of MethodCall/static-publication or exact-i64 products for ordinary Cataloged calls, name the missing non-empty same-session issuer/co-seal, and freeze the pre-adapter owner before any RawDirectCallDispositionLoanV1 code; S0 shelf split landed at 4061c21f45.
