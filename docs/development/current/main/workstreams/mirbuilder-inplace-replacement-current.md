...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: ENV-DIRECT-RECEIVER-S0 landed — bare `env.<method>` joins `EnvMethod` route via `get_env_method_spec`; json → `callable-semantic-lowering/placement-local-missing` (`local dbg` in bare block). D11 open: bare-block local declaration census. Remaining: D5-sealed forks, NamedArray/toolchain/inference. Gates 2-4 queued. |
