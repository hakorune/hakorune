...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private query/MIR binding, compile/cancel and demanded actions are verified; owned V2 rows are verified; next is C frame/expanded-index consumption before public host cutover. Literal issuers stay unchanged until consumers are ready.
...
Checkpoint: owned V2 rows pass Map36/live1. Explicit call activity now admits private V2 NULL/0 while V1 stays nonempty; C build, rows, ASan lifetime22, Named60/definition22 parent equality, query17/live1 and guards pass. Next C consumer owns staging plus both residual ledgers, including early-pattern exits; actual Map emission/public cutover remain open.
