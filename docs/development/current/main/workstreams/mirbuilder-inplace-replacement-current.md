...
Smallest next slice: design the opaque session binding against late plan errors/partial progress, exact-site unavailable/bypassed/invalid outcomes, host body/library lifetime and original MIR alias demand. Reuse document and config owners. Require one real caller switch and delete-set before ABI code; standalone query export is not authorized.
...
Checkpoint: document and allocation config have one invocation owner. C build/published/corridor, Named60/role22 parent comparison, document ASan8 and Dynamic LLVM/origins/machine code/relocation comparison pass. Settings mutation16 covers both walkers' allocation/field/get and runtime output; helper is independent of runtime flags.
The touched1104-line generic method file is now dispatch278/direct Array311/String stores518; post-split C build, mutation16, Named60 and corridor pass. Map-focused24 remains dependency evidence; opaque session, alias-demand, boxed-sum, leaf/frame and Map consumers remain open.
