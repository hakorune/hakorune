...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary` (4/7): ME-RECEIVER-SITE-S0 landed — `exact_source_receiver_value` port hook consumes located `Me`/`This` MethodCall `Receiver` sites; json → `static-call/legacy-fallback-retired` (`JsonLine.stringField/2` in `ingestLine`). D7 open: static-call issuance owner census. Remaining: D5-sealed coverage forks, F3b/F3c, NamedArray/toolchain/inference. Gates 2-4 queued. |
