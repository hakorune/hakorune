...
Smallest next slice: remove native Map remove/clear lock-held Drop through existing owner; retain Bool/capacity and verify reentry/once-only teardown. Detached opaque outcome contract is accepted in the Map SSOT. OWN-FIELD-CONTAINER-DEST-D0 orders real entry/observer/end, checked ABI plus lifecycle consumer, then source activation/old-edge retirement. Native dependency repair does not complete owned intake or physical execution.
