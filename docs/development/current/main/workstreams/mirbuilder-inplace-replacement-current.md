...
Smallest next slice: build owned V2 frame rows and expanded-function/caller lanes from the completed demanded action map, then implement both C consumers. Capability consumes the completed frame. Public session/host/source switch and V1 retirement follow atomically; no isolated public query, Map-through-V1 or another readiness census. Exact owners/acceptance remain in Map owner.
...
Checkpoint: complete leaf domain/action mapping is implemented; Map33 plus live C-query integration1 and guards pass. Exact boxed site plans preserve Bool i64/String/Handle/unit distinctions; missing plans, unsupported producers, unseeded cycles and original-required Float reject. Private query's non-Map EXE30/Map cancel evidence remains valid. Full frame/C/source cutover stays open.
