...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | R6-S0/S1/S2 landed. R6-S3 landed 2026-09-25: `reject_selected_dynamic_legacy_callsites` now rejects every `LegacyCallV0` carrier (`call-legacy-carrier`) on the selected-Dynamic artifact lane — the last open re-entry into ny_llvmc production; shared predicate stays narrow for v0 boxcall egress; ingress minters (unified-off Value, v0 boxcall Method) remain quarantined. Next: R7 caller-zero selection D0 — aggregate deletions (LegacyCallV0, func, optional callee/receiver) open only after production writer/reissuer/reader zero evidence. |
