...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: S0 landed (qualified-route predicate -> QualifiedUnbound only) + UNTYPED-OBJECT-STORAGE-S0 landed (birth_param_min declared-storage migration + `--emit-exe` route; receipt 4/7; untyped_field stays sealed sentinel). Next: `MIRBUILDER-EXE-ACCEPTANCE-EXACT-USIZE-D0` (boxtorrent/mimalloc usize param contract owner). Gates 2-4 queued. |
