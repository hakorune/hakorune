...
| F | `LOOP-PRODUCTION-SELECTION-D0` → Loop/M10b → M11 → M12 | D0 + S0 manifest (44 rows) landed. **M10b-I0-R0 card open**: P1 spine + P2 edge (caller-zero) -> R0 flip. **P1 landed**: 5 attempt adapters promoted; `issue_loop_node_winner_recipe_v1` caller-zero (lease -> 5 rows -> window -> coverage -> selector -> family demand -> producer -> recipe); 7 tests green; finding — GenericG0 unreachable via node window (typed Unresolved terminal; G0 stays function-level). Next: P2 physicalizer edge. |
