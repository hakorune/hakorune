...
Smallest next slice: query-to-frame binding design. Bind readiness and unique wire coordinates to the retained invocation/program; identify actual MIR demand/domain/original-lane consumer before public query. Preserve compile diagnostic order and keep conditional representation separate from optimizer coverage. One bounded slice; no query-only sibling or Map source switch.
...
Checkpoint: one private in-place invocation owns document/config/outcomes/program/definition storage; both core callers share init/destroy. Separate core product storage/arguments are deleted; ordered readers and mutable cursors are preserved. Plan unit/C build/Named60/role22/settings16/ASan lifetime16/published rows/corridor pass; Dynamic LLVM/origins/machine code match. Changed sources below800.
