...
Smallest next slice: audit ordinary callable actual Completion borrowing and frame/result ABI (Map owner step4). AppMain known Value source cutover passes package148 and 16 direct EXE/linked OBJ exit30 cases with mixed Value Fault cleanup. Its two representable-Value Stop edges are replaced; unknown kind/ordinary/Script/transferred-lambda restrictions remain. Crossed scopes cannot prove cleanup-free. Prior broad map_ reds stay parent-confirmed baseline.
