...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: NESTED-PROGRAM-ITEM-SITE-S0 landed — `body_item_site` collapses nested `Program` roots to walk-canonical `[stmt, ProgramBody(i)]`; json → `loop winner selection declined: zero selected family candidates`. D12 open: loop family admission census. Remaining: D5-sealed forks, NamedArray/toolchain/inference. Gates 2-4 queued. |
