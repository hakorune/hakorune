---
Status: Active workstream — fast path (R5c printer terminal)
Date: 2026-08-25
...
```text
  current decision  = MIR-CALL-CANONICAL-PRINTER-TERMINAL-I0-R0
  implementation    = R1, D1, Topology-I0, R2-I0, R3 D0, R3a, R3b, R4a, R4b, R4c, R4d, R4e, R4f, Query D0/T0, R5 D0, R5a, and R5b-B0 closed; R5c active
  mode              = fast
  production stop   = before R5c JSON/native and Call core field deletion
  exit              = typed printer projection parity, legacy rendering explicit, shared guard, and no JSON/R6 claim
fallback / retry  = 0
```
...
7. R5 D0 is accepted; R5a is closed at `e36f86e869`, R5b-B0 Rust VM
   ...
   is active; JSON/native remain separately bounded or design-stop rows;
   PyVM/reference/Python are `ParkedSealed`.
...
```text
Now
 MIR-CALL-CANONICAL-PRINTER-TERMINAL-I0-R0
  -> project stored Callee in MIR display without reading func

Next (not selected)
  -> JSON D0, native D0; then R6 gate
