...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | R6-S0..S5 landed 2026-09-25; all production `LegacyCallV0` ingress minters retired (`boxcall` wire stays a v0 contract; llvmlite lane keeps its documented ExplicitCompatibility projection). Caller-zero D1/D2 accepted bounded; S6 deleted the canonicalize Method repair arm + helpers; next: `MIR-CALL-R7-LEGACY-ARRAY-WRITE-CANON-DELETE-S7` — delete `canonicalize_legacy_array_write_calls` + refresh call site (residual named-stop backstop). |
