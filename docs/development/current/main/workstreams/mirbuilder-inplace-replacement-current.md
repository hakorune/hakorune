...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: DECLARED-INSTANCE-LOOP-LOCATOR-S3 landed — armed locator threaded into callable-loop source lanes; loop-body `me.method` consumes exact row and emits canonical `Callee::SameModuleInstance`. EXE advances to `StaticResultPublicationResidual` (`JsonLine.find/3` loop-body initializer). VM `ingest/1` unchanged (ledger-less). D16 open: selected static-call coverage census. Gates 2-4 queued. |
