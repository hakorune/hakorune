...
Smallest next slice: design-only D0-SIG-CLOSE-F: freeze the opaque one-shot Builder consumer/token bridge (no arbitrary callback), mutable Builder bundle lifetime, package-only test boundary, scoped &mut raw-port reborrow, Cataloged old-payload delete set, and focused acceptance/guard registration; no code, profile widening, AST rescan, public receipt, or package field.
