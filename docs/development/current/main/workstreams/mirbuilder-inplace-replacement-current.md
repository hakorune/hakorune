...
Smallest next slice: finish exact producer admission and owned v2 frame backing, using finite physical operation selection separately from result representation; retain the existing ownership Stop. Step2 covers both prepasses/emitters, PHI/status tail labels, formal/leaf/context handling and malformed C input; step3 proves natural Map-only OBJ/EXE, mixed Integer/Bool formal, NUL/Float values and six-edge deletion.
...
Substrate checkpoint: MIR/Core, borrowed index, original-demand propagation and Operation wire selection implemented. Map-focused18 and C header layout check pass; prior v1 Stop1/alias1/vocabulary20 remain dependency evidence. Producer audit preserves CopyOwned Stop and requires direct Integer comparison before generic dynamic-string dispatch. Bounded reader evidence is in the Map owner. Next: exact producer admission/frame backing; reader/ingress closure and C consumers remain open.
