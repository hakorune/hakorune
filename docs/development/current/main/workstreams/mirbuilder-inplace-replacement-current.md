...
Smallest next slice:
  `MIR-CALL-D1B-DIRECT-CALL-SOURCE-OWNER-LINEAGE-COSEAL-D1` is the selected design stop: reuse existing forest, selected-map, constructor-batch, and source-site authorities to preserve Cataloged/TopLevel/InstanceConstructor lineage through one same-session co-seal. The D0 inventory review and chronic A2 cleanup remain explicitly parked; no target synthesis, caller=None collapse, or deletion authority is implied.
