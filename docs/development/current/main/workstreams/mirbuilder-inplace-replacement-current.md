...
| F | `LOOP-PRODUCTION-SELECTION-D0` → Loop/M10b → M11 → M12 | D0 + S0 manifest (44 rows) landed. **M10b-I0-R0 card open**: switch = source -> facts -> 5-family window -> family selector -> demand -> producer -> recipe -> one physicalizer; P1 spine + P2 edge (caller-zero) -> R0 flip. Family probe: 4 of 8 attested routes lack a family — resolved by D0 boundary (parked families; typed-decline Freeze; 40 corpus rows covered). **P1 spine in progress (fast)**. |
