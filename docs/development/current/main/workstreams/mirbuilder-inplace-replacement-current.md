...
Caller-zero Prelude and `CALLABLE-LOOP-AFTER-CLOSURE-P0` are closed: real Prelude -> seven operations (`Pure=4`, `Read=2`, `Write=1`) -> fixed CFG/identity seal -> `ReadyLoopAfterContinuationV1`. `CALLABLE-LOOP-TAIL-COMPLETION-P0` reads Tail, validates ABI, claims Completion/return once, and forwards move-only profile-close evidence; finish must consume it through a non-no-op closure. DraftSeal, production selection, and legacy retirement remain closed.
