...
Smallest next slice: extract one C selector for the two actual Named emitters, preserving priority, lazy typed-plan validation, alias conditions and terminal differences. Delete their duplicated selection branches; C build and both-walker focused evidence required. Prescans and query exports stay unchanged. Then resolve preflight role/program/config binding, finish leaf/frame/C and natural Map-only OBJ/EXE plus retirement.
...
Substrate checkpoint: domain closure and physical operation selection added; Map-focused24 pass, including mixed/seedless recursive formals, seeded PHI/Copy, unknown Select inputs and String/Handle separation. Prior wire/header and v1 Stop/alias/vocabulary checks remain dependency evidence. Named, boxed-sum binding, complete leaf admission, frame backing and C consumers remain open; an empty operation map is not leaf acceptance.
