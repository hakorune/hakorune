...
Decision: bounded root-plus-one-called-ordinary-child scalar Map admission and direct/linked exit30 are closed. The accepted D0 adds one source-local exact-i64 Call followed by the existing terminal exact-i64 Call; both are retained under the existing Completion/root-flow owner and consumed by the existing direct-call loan.
Source authority + canonical issuer: existing `home_new_prefix` terminal/local Call issuer plus `direct_call_lifecycle` exact target/formal/site and Completion co-seal; existing owner-indexed Completion and finalized handoff remain the owners. No new semantic receipt or second instruction graph.
Non-authority: physical `ordinary_call` Option, JSON function count, C name/row scans, MIR shape, or a passing synthetic transport cannot issue Normal continuation, result binding, or Fault cleanup. Field/New/Home and compatibility paths remain outside this decision.
Fail-fast boundary: the accepted D0 rejects nested/conditional/recursive Calls, foreign or stale sites, unsupported result uses, prior-Home local Calls, and uncalled/additional Map owners before transport or artifact. The selected local physical consumer returns through the existing callable fault frame; it does not reclassify root cleanup.
Smallest next slice: delete the remaining single-terminal-only production edge now that the accepted two-Call source relation passes the selected artifact path, OBJ/EXE Pair exit30, and the existing cleanup verifier.
Seed/result/index share `Rc`, S6C consumes exclusively, and the selected root plus one exact called ordinary Map owner is admitted only for scalar Value rows with no New/Home transfer. Additional local Calls, unknown values, ownership transfer, and wider Map owners remain stopped; preserve unsupported install/exporter edges until the artifact evidence closes.
Use jobs4 locked quick focused Rust, selected C build/proofs when changed, existing pointer/corridor guards; sources stay below800. Static V2 evidence and parent-confirmed non-plugins baseline are recorded in the collection SSOT; no whole Map/R7 closure.
...
The role-based physical JSON, C admission/index/emitter, direct EXE and linked OBJ are connected for the existing Pair cohort: exits30, bool/range cases exit70 with Fault103, and cleanup probe passes. The root-plus-called-ordinary-child scalar-Map source admission and direct/linked artifact test both exit30; selected lifecycle parameter/return capability checks are scoped to the emitted ordinary function, and ordinary-call scalar state/emission is consumed without a storage lease.
The new two-Call source and MIR consumer tests prove local result binding, owner-scoped co-seal, strict CFG verification, and two physical I64 Call terminators. The selected production physical program now carries both calls; direct EXE and linked OBJ both exit30, and the cleanup/finishing verifier passes.
Wider ordinary Map owners, New/Home transfer, unknown scalar values, and whole Map/R7 claims remain stopped. The selected owner-gate, child-symbol-drift, Call-handoff, child/root/field, physical Call-admission and ignored Pair/Bool/ordinary-child/two-Call runtime tests pass; bounded Pair ABI/OBJ/EXE/Fault evidence is closed and the remaining production retirement is the single-terminal-only edge.
