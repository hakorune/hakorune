...
Smallest next slice: Map-scoped OWN-HOME-FLOW0-S0 candidate capability/availability API under OWN-FIELD-CONTAINER-DEST-D0. Root join placement and pre/post-commit state table are fixed there: source flow first, root co-seal second, physical transaction third, runtime/public cutover last. Script uses its own pre-effect forest seal; no ordinary-New ledger reuse. Existing source shape is closed; no scalar-only completion or runtime retain workaround.
...
Checkpoint: Map source-shape tests20, continuation rejection tests3 and existing Script direct/NonDirect acceptance tests2 pass; pointer/corridor guards pass. MapLiteral->Other information loss is removed; MissingParent becomes explicit UnsupportedMapEntryCall. No C changes: prior normal/ASan434, control106 and parent-equal Named60/definition22 remain historical evidence. Ownership co-seal and public/source cutover are not claimed.
