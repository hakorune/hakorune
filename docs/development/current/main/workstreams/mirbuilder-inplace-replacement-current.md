...
Smallest next slice: existing V2 static-host same-invocation query/frame/compile connection per collection SSOT; config/final-address ownership and V2 error order accepted in collection/ABI SSOT; migrate dedicated V1 proofs, no isolated query or V1 retry. Generic Map keeps copy/boxed semantics. Local queues11/14 closed (lookup2/parity6/source13/EXE26/completion12). Raw/Script/Core/static-host/V1/six-edge and alias scans remain open; no R7 claim.
