...
| D | Loop prerequisite closure in `joinir-loop-selfhost-recipe-pipeline-ssot.md` M10 | S6D remains separate M8 producer/observation. Active Call row: B3 `StringHelpers.split_lines/1`; imported static method and nested-push stop are identified, physical LoopCond success is unproven. Next close carrier/operation/effect and co-seal mappings, connect current LoopCond/GeneralIf owner, then after M8/M9/M10 switch this caller and retire its selected old edge. |
