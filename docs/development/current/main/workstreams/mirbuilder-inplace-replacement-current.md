...
#### MAP-LITERAL-FIVE-ORDINARY-OWNER-COVERAGE-I0 — selected; prior four-owner and ordinary-child rows closed
...
Smallest next slice: widen only the existing owner cap from4 to5 and require exactly four local Calls in preflight; reuse source-ordered groups, formal checks, cleanup owners and physical collection.
...
Use jobs4 locked quick focused Rust, selected C/runtime artifact proof, existing pointer/corridor guards; sources stay below800. The one source Call site maps to one ordinary row; no whole Map/R7 closure.
...
Direct EXE and linked OBJ both exit30. Fault-order prefix probes show later owners are skipped. The four-owner source admission passes semantic 14/14, and direct/linked exit30 plus per-prefix Fault/cleanup probes pass. The ordinary-child Value-only recheck passes with one Call row and root+child Pair exit30. This row now owns five-owner source admission and physical evidence.
...
The ordinary-child Value-only artifact recheck is closed: the source has one Call site, and direct/linked exit30 passes for child-only and root+child cases. Two-owner admission and distinct-A/B Pair exit30 are closed; four-owner source-ordered retention and bounded owner widening are closed.
