...
Smallest next slice: opaque Map/outcome ABI and descriptor/session/symbol revision, then operation-derived C projection and mixed cleanup. Common table/checked facade/SafeMutex residence are implemented; Map13/JSON5/GC7/kernel2 pass (ownership task owns receipt). Disposal rejects live Map/Ready outcome; callbacks span no mutable ABI borrow. Unused aliases add no Home; later New Fault ends prior Maps. Source install Stop stays until actual cutover.
