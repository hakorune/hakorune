...
Smallest next slice: resume exact source destination demand and commit-aware availability under OWN-FIELD-CONTAINER-DEST-D0. Prefix local-flow separation and source terminal integer-evidence Stop are implemented. Map candidate/commit issuance remains absent; do not infer ownership from ordinary Handle support or Trivial alone. Root co-seal, physical transaction and runtime/public cutover remain ordered dependencies.
