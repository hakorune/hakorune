...
Smallest next slice:
  ...
  The selected design stop is `CHRONIC-MEASUREMENT-SCOPE-D0`: freeze the finite
  token/range boundary and two-axis classification before any scanner or source
  deletion. No deletion is implied by the navigation closeout.
...
4. `CHRONIC-MEASUREMENT-SCOPE-D0` — selected design stop: freeze finite
   panic/dead_code boundaries, independent compile-domain/role axes, and the
   shared expectation TSV contract. It is docs-only and does not authorize a
   scanner, guard execution, or deletion.
5. `DEAD-CODE-REMEASURE-D0` -> `CHRONIC-MEASUREMENT-EXPECTATION-I0` ->
