...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary` (4/7): INSTANCE-STATIC-INGRESS-S0 landed — claim-ingress admits `Cataloged` instance callers when the probed target resolves via `declaration_for`; json → `static-result-ingress/target-only` (`JsonLine.stringField/2`). D8 open: TargetOnly emission census. Remaining: D5-sealed forks, F3b/F3c, NamedArray/toolchain/inference. Gates 2-4 queued. |
