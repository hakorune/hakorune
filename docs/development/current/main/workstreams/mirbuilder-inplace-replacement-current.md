...
Smallest next slice: design the detached-old/no-old install Normal result and one-time end consumer in existing lifecycle Invoke/result/root progress owners. Source Map flow and common install Stop are implemented; do not add unconnected operations or route through plain static-V2 writes. OWN-FIELD-CONTAINER-DEST-D0 owns the exact next brief, runtime intake prerequisite and eventual selected old-edge deletion.
