...
closed — GENERIC-G0-CANDIDATE-S1; Direct S1; Nested S1 -> caller-zero observers, guard/docs green, no selector/Recipe/Builder/MIR/production; Nested preserves forest errors. closed — LOOP-FAMILY-LOOPTRUE-OBSERVATION-S1 -> identity-bearing AST-free projection, cfg(test) adapter, pure observer, 9 policy + 8 projection tests, shared guard, reference sync green; selector/production/physical/retry/fallback/legacy remain closed; next common admission design.
```
