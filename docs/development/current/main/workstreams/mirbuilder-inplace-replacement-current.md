...
Smallest next slice: single-entry source/destination -> commit -> replacement cleanup -> Normal/Fault design under OWN-FIELD-CONTAINER-DEST-D0. Owned-slot target is accepted in reference: transfer compatible Home/carrier once, no implicit share; keep committed new ownership on cleanup Fault and existing Bool deletion. Bind existing resolver and semantic-package co-seal before Recipe. Broader slot/runtime obligations and escape Stops remain open.
