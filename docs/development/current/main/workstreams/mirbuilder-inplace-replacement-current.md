...
closed — Generic/Direct/Nested/LoopTrue/LoopCond S1 observers + common admission D0 are caller-zero; no selector/Recipe/Builder/MIR/production. closed — S0C neutral-owner R0: structural facts owns the move-only aggregate; tests/guards/docs green. closed — Generic normalization: cfg(test) adapter + neutral C/D/U/R observer, 12+7 tests and shared guard green; next assembler; selector/production/retirement closed.
```
