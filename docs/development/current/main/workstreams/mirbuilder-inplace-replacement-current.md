...
Smallest next slice: OWN-FIELD-CONTAINER-DEST-D0 single-entry candidate/destination issuance and commit co-seal at the matching source root. Map ordered keys/exact EntryValue preservation and explicit pre-classification Script call Stop are implemented. Script forest follows callable package; no OrdinaryNew ledger reuse, inferred Home receipt or executor widening. Cleanup/runtime escape and full cutover remain open.
