...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary` (4/7): LOOP-COND-COND-UPDATE-S0 landed — `ConditionalUpdateIf` arm via existing port facade; json → `incomplete-consumption` (Receiver site). D6 open: receiver-consumption owner census. Remaining: D5-sealed coverage forks, F3b/F3c, NamedArray/toolchain/inference. Gates 2-4 queued. |
