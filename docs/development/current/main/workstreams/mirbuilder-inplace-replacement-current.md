...
#### MAP-LITERAL-THREE-ORDINARY-OWNER-COVERAGE-I0 — closeout complete; MAP-LITERAL-FOUR-ORDINARY-OWNER-COVERAGE-I0 active
...
Smallest next slice: record the passing four-owner direct/linked artifact and Fault-prefix evidence, then close this I0; keep source-ordered groups, prior-home rejection and physical collection unchanged.
...
Direct EXE and linked OBJ both exit30. Fault-order prefix probes show later owners are skipped. This three-owner I0 is closed; the four-owner source admission passes semantic 14/14, and direct/linked exit30 plus per-prefix Fault/cleanup probes now pass. Closeout remains.
...
The ordinary-child Value-only artifact still has a known baseline red (`ordinary_calls().len()` actual1/expected2), reproduced on parent 1a876c3fba and current, and is excluded from this I0 evidence. Two-owner admission and distinct-A/B Pair exit30 are closed; this I0 owns three-owner source-ordered local binding retention and the bounded owner-set widening.
