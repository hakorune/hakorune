...
Current execution: `RAW-LOCATED-CONTROL-BODY-PORTALS0-I0-R0`
History:       Git history and the short landed tail below
```

## Current execution brief

`RAW-LOCATED-CONTROL-BODY-PORTALS0-I0-R0` — batch 2 of at most 5

```text
Change:
  remove ControlBody from the fixed unlocated portal set by carrying exact
  parent-relative sites through the existing selected control-body portals.

Contract:
  RawLegacy stays raw. The existing carrier executes once and never retries or
  reselects. No new portal variant or navigation policy may be added.

Done:
  ControlBody construction sites decrease to zero; selected control descendants
  retain exact lineage/site; build, parity, and candidate reuse are green.

Stop:
  no clone/reparse, dynamic reason, located-to-unlocated retry, role duplication,
  zero old-edge reduction, new variant, or sixth series commit.
```
