...
Smallest next slice: selected MIR/physical/C InstallValue per Map owner step2, then source connection step3. Source retention implemented (package148/control33, guards pass): exact literal/alias evidence, unknown formal kind retained. Runtime ABI verified. Source install Stop and ordinary actual-Completion cleanup/frame/result remain; indexed budget repetition stays task13.
