...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: S6+S7 landed; D20 decided (SSA family = carrier binding desync — reads resolve binding-id SSA store, not `variable_map`); **S8 landed** — carrier phis publish into `values[binding]`; document consume reports document verify per D19; emit-mir-json green, 65 phis retained, `find/3` wrong-code closed. Next blocker: `ingest/1` loop family-selection decline (`zero selected family candidates`). Gates 2-4 queued. |
