...
Smallest next slice: root-owned Map candidate/obligation issuance and source consumer against the accepted one-entry API in OWN-FIELD-CONTAINER-DEST-D0. Nonconsuming preparation, same-obligation commit, distinct local/fresh/carrier precommit cleanup and duplicate-use refusal are fixed. No new transfer-policy census or placeholder receipt. Root co-seal, key/Map cleanup, physical transaction and runtime/public cutover remain required.
