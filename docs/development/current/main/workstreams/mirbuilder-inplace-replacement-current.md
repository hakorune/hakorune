...
| D | Loop prerequisite closure (M10) | Active B3 `StringHelpers.split_lines/1`; test asserts nested-push stop, not live rows/terminal. D2 remains design-stop: substring target/law and ArrayPush source Fault/commit unresolved; ABI status is not source outcome. Resolve via CoreMethod owner or name missing authority; map bindings/`s` proof and specify one V2 co-seal. Then observe rows, connect LoopCond/GeneralIf, meet M8/M9/M10, switch and delete selected edge. Physical success unproven. |
