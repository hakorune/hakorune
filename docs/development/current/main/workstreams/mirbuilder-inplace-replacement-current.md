...
Decision: retained static V2 host and raw/Core literal cutover are implemented; public V1 ingress and six literal emission edges are retired. Preserve distinct generic copy/boxed and checked Home contracts. The bounded callable slice now selects one ordinary child Map owner for scalar Value-only install through the existing owner-indexed row and the lifecycle physical consumer; wider ownership remains separate.
...
Seed/result/index share `Rc`, S6C consumes exclusively, and the selected single ordinary Map owner is admitted only for scalar Value rows with no New/Home transfer. Mixed AppMain/ordinary Maps, unknown values, and ownership transfer remain stopped; preserve unsupported install/exporter edges until the physical consumer is complete.
...
The role-based physical JSON, C admission/index/emitter, direct EXE and linked OBJ are connected for the existing Pair cohort: exits30, bool/range cases exit70 with Fault103, and cleanup probe passes. The ordinary-child scalar-Map source admission and direct/linked artifact test both exit30; selected lifecycle parameter/return capability checks are scoped to the emitted ordinary function, and ordinary-call scalar state/emission is consumed without a storage lease.
Mixed AppMain/ordinary Map, New/Home transfer, unknown scalar values, and wider physical Map install/value ownership remain stopped. Ordinary Call ABI/OBJ/EXE/Fault evidence is closed for the bounded Pair and one ordinary-child scalar cohort; no whole Map/R7 claim. Selected owner-gate, child-symbol-drift, Call-handoff, child/root/field, physical Call-admission and ignored Pair/Bool/ordinary-child runtime tests pass; source max759.
