...
Smallest next slice: carry the existing ordinary Call row through the owner-indexed physical state and finalized root handoff, then connect its existing ABI/formals/result/cleanup consumer; static-child draft capture and artifact finishing recheck the same captured PhysicalBoundary after construction/New and field/terminal validation.
...
Checkpoint: child Completion/terminal, root-exit/literal/field progress and PhysicalBoundary retention/recheck are complete after S6C; the existing ordinary Call payload is rebound after finishing and moved once into the finalized root source handoff, and child symbol coverage is included. Mixed AppMain/ordinary Map, ordinary ABI/install, and Physical Map execution remain stopped. Selected child-symbol-drift, Call-handoff, child/root/field tests pass; source max759.
