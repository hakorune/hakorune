...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private V2 rows/query and value consumers are implemented; Original PHI reference/width closure and physical escape blockers precede public cutover. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: original PHI predecessor references/width normalization within the existing physical owner (future boxed aliases/Select). Typed/direct object escape, intrinsic allocator session binding and selected boxed comparison remain CutoverBlockerOpen. Input-aware capability and atomic public session/host/source switch with V1 retirement follow. No Map-through-V1 or readiness census.
...
Checkpoint: private Formal/expanded index, cross-call closure, signatures and ingress implemented. Normal/ASan each pass396 full cases (175 kernel readbacks, eight traps, non-host probe); final focused85 pass each, including added duplicate/residual/StaticMethod checks. C build/corridor/pointer pass; Named60/definition22 match parent. Expanded old leaf/signature/i64-only argument paths are excluded. Original PHI reference/width and physical escape remain open; no public cutover claim.
