...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Owned V2 rows, private query and allocation/ExactBits execution are verified; remaining value/expanded-function consumers precede public host cutover. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: complete value projection consumers and the shared expanded-function index, joining typed calls to exact definitions/Formal rows and rejecting unshadowed old-ABI ingress. Capability consumes the completed frame. Public session/host/source switch and V1 retirement follow atomically; no isolated public query, Map-through-V1 or another readiness census. Exact owners/acceptance remain in Map owner.
...
Checkpoint: private allocation/ExactBits consumers pass 49 compile cases, 30 linked-kernel readbacks and 8 injected traps, including String bytes and Map/field backedges. Staged residual rejection preserves prior object. C regressions/guards pass. Expanded functions and remaining actions reject explicitly; public V2 and host/source cutover stay closed.
