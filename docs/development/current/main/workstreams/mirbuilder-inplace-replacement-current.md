...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Owned V2 rows, private query and exact/operation/direct String/Map value execution are verified; remaining Original/transfer/expanded-function consumers precede public host cutover. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: consume remaining Original rows at actual producer success, fixing boxed tail/Bool alias handling in the existing owner. Then transfer/formal and shared expanded-function consumers join exact definitions and reject old-ABI ingress. Capability and atomic public session/host/source switch with V1 retirement follow. No isolated public query, Map-through-V1 or readiness census; exact owners/acceptance remain in Map owner.
...
Checkpoint: private value consumers pass 99 compile cases in normal/ASan builds, 68 linked-kernel readbacks and 8 injected traps, including all seven Operation rows, mixed Bool widths, String/Map handles and earlier backedges. Missing/wrong input closure rejects before output. Named60/definition22 match parent; C build/guards pass. Other Original producers, transfer/formal and expanded functions remain unsupported; public V2 and host/source cutover stay closed.
