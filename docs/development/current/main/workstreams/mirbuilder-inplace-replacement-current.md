...
closed — GENERIC-G0-CANDIDATE-S1; Direct S1; Nested S1; LoopTrue S1 -> caller-zero observers, tests/guards/docs green, no selector/Recipe/Builder/MIR/production. closed — common admission D0 and LoopCond D0 -> worker-reviewed five-row `AdmissionWindow`, LoopCond C/D/U/R source contract, resolver brand, assembler≠selector; docs synced. next LoopCond S1 implementation, then Generic normalization and assembler; selector/production/retirement closed.
```
