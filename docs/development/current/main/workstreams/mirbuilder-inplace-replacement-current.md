...
Smallest next slice: close explicit owned-residence read/clone/delete/clear/end consumers before intake. Root non-Clone residence with kernel SafeMutex end is accepted; TLS rejects before mutation until confinement is proved. Current NyashBox clone/share/read APIs cannot hide owned values as missing or ordinary Boxes. Then connect source transfer and existing root cleanup. The insertion lock-held native Drop edge is removed; no source ownership or public cutover claim.
...
Checkpoint: MapBox::insert_key_str commits replacement under lock, then drops displaced native Box after unlock. Focused Map6 passes, including reentrant write-lock acquisition, committed value42, old Drop once and no displaced Drop for missing insertion. Key-domain/order tests stay green; pointer/corridor and diff checks pass. Changed sources max597. Remove/clear, user finalization and owned-slot intake remain open; no C/EXE claim.
