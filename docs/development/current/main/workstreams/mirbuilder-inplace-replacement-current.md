...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private invocation and MIR Named binding are verified; next is accepted private query-to-planner integration; complete frame/C precedes public host cutover. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: private invocation query/program cache/unique coordinates -> real MIR binding, with shared retained-input typed rows compile. Preserve public V1 gate/read order. One test-only driver retains the owner across Rust planning and compile/cancel; non-Map witnesses compile, Map witnesses cancel before V1. Delete repeated program read and pathname-only internal coupling; no public query export. Full contract/retirement is in Map owner.
...
Checkpoint: Named MIR binding is verified atde2113241d (focused28/C build/published rows/guards). C+host audit confirms V2 header has no full frame/C consumer; public session switch waits for it. Private query integration is the accepted next dependency, using actual C results rather than synthetic-only observations. No Map execution/cutover claim.
