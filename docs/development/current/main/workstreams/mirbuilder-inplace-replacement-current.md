...
Smallest next slice: source connection per Map owner step3. MIR/physical/C InstallValue implemented: actual OBJ/EXE30, Fault disposal, 26 negative inputs; Rust13+1+1 and guards pass. Bool uses the existing i64 lane (incorrect i1 proposal withdrawn). Source Stop and ordinary actual-Completion cleanup/frame/result remain. Five broad map_ reds match parent6effe70603. Indexed eager budget scans removed; task13 retains preceding physical scans.
