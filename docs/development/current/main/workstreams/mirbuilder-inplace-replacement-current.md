...
Smallest next slice: implement accepted terminal AppMain Call co-seal in existing disposition/Completion owners; preserve early target validation and bind lifecycle after callee contracts. Ordinary terminal retention is implemented (package150, source max700); formal I64 retention remains verified. Literal I64 arguments, Cataloged Map callee, explicit I64 result. Selected scalar edge/install Stop retire with execution; no new wrapper or repeated census.
