...
closed — GENERIC-LEGACY-CORPUS-UNIVERSE-P0 and GENERIC-LEGACY-OBSERVATION-FRONT-G0 landed: checked 25-column/389-case inventory plus direct strict VM receipt; the canonical case fails before Loop in the `raw_expression_dispatch/mod.rs` BinaryOp arm for `StringifyOperator.apply/1` `Body(1)/IfCondition`, with raw-structured expressions=1/bodies=0; no Generic route/production/physical/retry/delete authority. Next GENERIC-RAW-STRUCTURED-DEMANDS-REPAIR-S0-D0.
```
