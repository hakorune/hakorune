...
Decision: retained static V2 host and raw/Core literal cutover are implemented; public V1 ingress and six literal emission edges are retired. Preserve distinct generic copy/boxed and checked Home contracts. The bounded callable slice now selects one ordinary child Map owner for scalar Value-only install through the existing owner-indexed row; wider ownership and backend parameter capability remain separate.
...
Smallest next slice: open ordinary Map install/value at the selected physical consumer after the existing parameter-entry backend capability is closed; keep static-child capture and artifact finishing on the same captured PhysicalBoundary after construction/New and field/terminal validation.
...
The role-based physical JSON, C admission/index/emitter, direct EXE and linked OBJ are connected for the existing Pair cohort: exits30, bool/range cases exit70 with Fault103, and cleanup probe passes. The new ordinary-child Map source admission is green in the 161-case package suite and the C shim builds; source-to-artifact execution remains behind the existing parameter-entry backend capability gate.
Mixed AppMain/ordinary Map, New/Home transfer, unknown scalar values, and the remaining physical ordinary Map install/value cutover remain stopped. Ordinary Call ABI/OBJ/EXE/Fault evidence is closed for the bounded Pair cohort; no whole Map/R7 claim. Selected owner-gate, child-symbol-drift, Call-handoff, child/root/field, physical Call-admission and ignored Pair/Bool runtime tests pass; source max759.
