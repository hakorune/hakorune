...
ordered after B-prime correction
  ...
  3. close `JOINIR-GENERIC-RESOLVED-CARRIER-SOURCE-BRIDGE0-D1`; run `JOINIR-GENERIC-RESOLVED-CARRIER-SOURCE-MATRIX-V0ONLY-NORECURSIVE0-D2-S1` as the next cfg(test)-only source-backed non-target row, with production issuer/adapter/selector prohibited, then close M7 five-family parity, M8 all-19 producers, and M9 host parity
