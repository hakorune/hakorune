...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private V2 rows/query and value consumers are implemented; Original i1 PHI normalization is implemented; runtime escape ownership is the next design boundary. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: design runtime-owned retention/reclaim coordination for indexed SafeMutex objects per Map SSOT. Preserve identity/shared mutation, child lifetime and actual profile agreement. Production keep/promotion issuer and exclusive delete-set are absent; keep current escape Stops. TLS/direct escape, intrinsic binding and boxed comparison remain included blockers before capability and atomic public/source switch with V1 retirement.
...
Checkpoint: after-group selected i1 original normalization passes normal/ASan434 each (control106), selected C build, pointer/corridor and parent-equal Named60/definition22. Both walkers exercise selected/nonmapped i64 consumers, Copy, future backedges, multiple PHIs and original exit30 while retaining i1 input Stops. Source max712. Runtime escape audit finds no keep/promotion issuer or safe delete-set; next work is design_stop, not public cutover.
