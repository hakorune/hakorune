...
Smallest next slice: capture existing allocation/runtime mode bits once at file ingress and pass to the borrowed core. Remove the five nested environment readers; method/Named/typed emission and runtime requirement use the same value. The Map owner records finite inventory and four-combination mutation acceptance. No new global/session ABI or source switch; session observation/host/alias mapping remains open.
