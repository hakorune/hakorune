...
Smallest next slice: exact obligation-preserving slot compatibility and candidate flow under OWN-FIELD-CONTAINER-DEST-D0. Map acquisition/key/end law is fixed in lifecycle.md: native keys, reverse successful live-install cleanup, same-responsibility completion. Existing callable row/Script product supply same-issuance core/body shape; no new adapter. Prior Home membership is not compatibility. Runtime/public cutover remains open.
