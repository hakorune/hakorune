...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. V2 rows/query and exact/operation/host-backed Original execution are implemented; Phi/Select/Formal/expanded consumers and physical escape blockers precede public cutover. Literal issuers stay unchanged until consumers are ready.
...
Checkpoint: normal/ASan each pass239 private compile cases, 125 kernel readbacks, eight traps and a non-host probe. Copy/NamedAlias forward both lanes and retain demanded originals; missing/cyclic inputs and wrong outcomes reject. C build/guards pass; Named60/definition22 match parent. Phi/Select/Formal, expanded ingress, physical escape and public cutover remain open.
