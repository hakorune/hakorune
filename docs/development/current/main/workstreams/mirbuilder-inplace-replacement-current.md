...
Smallest next slice: close implemented AppMain Call source co-seal (package153/control33/corridor, source max739), then audit existing finalized/compiled-entry handoff and Invoke continuation owner. Map calls stop before scalar arguments; original caller Completion and callee contracts remain authoritative. Ordinary install/artifact Stops remain until execution evidence; no wrapper or repeated source census.
