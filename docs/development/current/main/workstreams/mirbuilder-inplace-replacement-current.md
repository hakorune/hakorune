...
Smallest next slice: read-only transferred lambda Map membership/consumer audit per [collection SSOT](../design/collection-literal-construction-ssot.md#declared-root-map-install-selection), then source-owned mixed-formal capability/residence mapping. Declared-root unissued install success is retired (Map11 and unchanged Script6/EXE12 pass). No generic fallback or whole Map/R7 claim.
