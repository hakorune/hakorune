...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | R6-S0 boundary selected (S0-A) and landed 2026-09-25: callee->callable-key+receiver projection now owned once by `src/mir/ssot/callable_key.rs` (was duplicated in `published_backend_view` + `verification/invoke`). Sibling split S0-B (transport v0/v1 emitter) selected next; S0-D carrier fan-in unification declined as non-neutral. Aggregate writer/reader/reissuer/re-entry deletion remains open; R6-S1+ still requires per-row tuples. |
