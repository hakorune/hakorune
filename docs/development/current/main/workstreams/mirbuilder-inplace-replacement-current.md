...
Smallest next slice: implement common Map storage and Native-only public/non-NyashBox checked facades with real SafeMutex residence end. Caller-owned opaque lifetime is accepted in OWN-FIELD-CONTAINER-DEST-D0; Normal alone acquires. Preserve candidate on precommit failure and reverse end outside locks. Then descriptor/ABI and mixed-origin cleanup: unused aliases add no Home; later New Fault must end prior Maps. Source install Stop remains until full cutover.
