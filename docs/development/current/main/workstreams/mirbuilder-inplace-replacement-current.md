...
| D | Loop prerequisite closure in `joinir-loop-selfhost-recipe-pipeline-ssot.md` M10 | D2 corrected circular connected-issuer entry. Design scalar LoopCond first: name real caller/delete tuple, source typing/effect and After-return APIs. S6D remains producer-only; later production Promote obeys M10 and retires its selected edge. B3 String/Array joins follow. S6E awaits observations; no implementation permission yet. |
