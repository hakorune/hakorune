...
#### MAP-LITERAL-WIDER-ORDINARY-OWNER-COVERAGE-I0 — active implementation

Decision: admit the two-exact-owner scalar Map slice through the existing owner index and install preflight; source and cleanup design is closed.
Source authority + canonical issuer: same resolver session exact Call target/site, existing affine loan, Completion/root-flow co-seal and owner index.
Non-authority: declaration existence, Map detection, MIR shape, physical function count, or owner count cannot issue admission.
Fail-fast boundary: root plus distinct called ordinary owners A/B only; local A, optional root Map, terminal B is the accepted order. Third/nested/conditional/recursive, uncalled, New/Home, field, unknown, native escape and root-before-first-Call Map reject.
Smallest next slice: the bounded `map_target_owners` and owner-set preflight are landed; retain prior_homes rejection and existing cleanup builder while closing the I0 evidence.
Non-claims: no new receipt/schema, extra Call shape, all ordinary owners, general local-Call cleanup, native Map read/clone, whole Map/R7 closure.
