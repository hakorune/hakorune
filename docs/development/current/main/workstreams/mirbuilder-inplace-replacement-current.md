...
Smallest next slice: opaque Map/key/outcome ABI with one descriptor/session/symbol revision, then C projection/mixed cleanup. The key region preserves key-before-child and child-Fault cancellation; preflight rejects before consumption. Runtime Map13/JSON5/GC7/kernel2 remains dependency evidence. The ownership task/reference owns exact states/status/consumer inventory. No live ABI borrow spans callbacks; source install Stop remains until actual cutover.
