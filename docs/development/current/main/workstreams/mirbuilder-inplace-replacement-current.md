...
Smallest next slice: accepted direct-local Map/prior ordinary-New Home source flow through install Stop. Exact acquisition and scoped descriptor check; one Completion phase/terminal result. prepare_install rejects sealed Map demand before catalog/body effects and returns the package; normal harness stage is CatalogInstall. Include alias/reuse, duplicate/Fault, empty Map and direct prepare callers. Full contract: OWN-FIELD-CONTAINER-DEST-D0; no source-only cutover or new rich error receipt.
...
Checkpoint: New candidate owns construction/destruction/Birth before scanning; field callback borrows it and repeated source/construction lookup is removed. Package121/Completion33 pass, including unavailable-descriptor retention and candidate-order rejection. Claim/handoff transfer remains once. Pointer/corridor and diff checks pass; source max623. Previous JSON baseline remains unchanged. No Map transfer/intake/C/EXE claim.
