...
ordered after B-prime correction
  ...
  3. close `JOINIR-GENERIC-RESOLVED-CARRIER-SOURCE-BRIDGE0-D1` as one cfg(test)-only S2A handoff bridge; keep the full source-backed matrix at `JOINIR-GENERIC-RESOLVED-CARRIER-SOURCE-MATRIX-DESIGN0-D2`, with production issuer/adapter/selector prohibited, then close M7 five-family parity, M8 all-19 producers, and M9 host parity
