...
Smallest next slice: connect the now-owned candidate evidence and exact Map EntryValue to source install/Fault successors and one Completion cleanup result. NewSite-to-Binding alone still lacks obligation evidence. Reuse the accepted phase mapping in OWN-FIELD-CONTAINER-DEST-D0; no separate Map receipt or physical-state inference. Physical phase emission, native observer/read/end and owned-slot intake remain required before public cutover.
