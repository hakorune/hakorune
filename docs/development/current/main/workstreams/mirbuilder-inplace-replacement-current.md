...
Smallest next slice: selected C physical input/emission, then source OBJ/EXE and old-edge cutover. Mixed New/Map/root finishing, artifact correspondence and Map cleanup coordinates pass focused checks, including actual simplify and full Pair finishing. Source install Stop remains; see the ownership checkpoint for evidence and baseline classification.
