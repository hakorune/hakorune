...
Smallest next slice: consume the existing ordinary Map install/result contract on the selected physical path; keep static-child capture and artifact finishing on the same captured PhysicalBoundary after construction/New and field/terminal validation.
Seed/result/index share `Rc`, S6C consumes exclusively, mixed AppMain/ordinary Maps remain stopped. Preserve unsupported install/exporter edges until the selected Map consumer is complete.
...
The role-based physical JSON, C admission/index/emitter, direct EXE and linked OBJ are connected: Pair exits30, bool/range cases exit70 with Fault103, and cleanup probe passes.
Mixed AppMain/ordinary Map and the remaining Map install/value cutover remain stopped. Ordinary Call ABI/OBJ/EXE/Fault evidence is closed for this bounded Pair cohort; no whole Map/R7 claim. Selected child-symbol-drift, Call-handoff, child/root/field, physical Call-admission and ignored Pair/Bool runtime tests pass; source max759.
