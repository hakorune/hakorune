...
Smallest next slice: BoxShape the complete selected pure-first document boundary. Existing file wrapper parses/owns/frees once; validator, census, route readers and compile core borrow the same document. Remove repeated reads and inner document frees in the finite Map-owner inventory; preserve check order and other resource cleanup. Parent role/Named/Dynamic plus parse/free and error/pattern-path evidence required. No opaque session ABI or source switch yet.
...
Checkpoint: physical definition plan replaces both independent planned stores/readers. C build, owner boundary test, published-row proof and corridor guard pass. Named60 and role/metadata/capacity22 cases match parent a15e1d402a; selected Dynamic source producer succeeds and parent comparison matches LLVM, origins, machine code/relocations. Emitted state stays separate. Prior Map-focused24 remains dependency evidence; session, alias-demand, boxed-sum, leaf/frame and C Map consumers remain open.
