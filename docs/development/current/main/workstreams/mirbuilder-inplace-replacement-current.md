...
Smallest next slice: design-only D0-SIG-CLOSE-F: freeze the concrete nested-result callback/token signature, mutable Builder bundle lifetime, package-only test boundary, scoped &mut raw-port reborrow, Cataloged old-payload delete set, and focused acceptance/guard registration; no code, profile widening, AST rescan, public receipt, or package field.
