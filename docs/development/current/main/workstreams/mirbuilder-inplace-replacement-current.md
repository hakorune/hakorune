...
ordered after B-prime correction
  ...
  3. D1, D2-S1, D2-S2, D3-S0, D2-S3, D2-S4, and D2-S5-S1 are cfg(test)-only closed. D3-S1 is accepted as the two-column disposition/winner-correctness design boundary; its V1-only Local+step child is the current cfg(test)-only row and must not add production issuer/selector/route authority
