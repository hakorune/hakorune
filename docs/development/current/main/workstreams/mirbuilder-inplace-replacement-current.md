...
Smallest next slice: implement the accepted source preflight with root claims and one Local annotation policy over retained initializer/alias relations, then source OBJ/EXE and old-edge cutover. Common Stop remains until connection; see the ownership source preflight Decision. C consumer and NativeArray regression are verified at bb74fa8078.
