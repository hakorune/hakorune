...
Smallest next slice: connect actual Map emitter, LocalCommit enum progress and shared Local Copy/ReuseInitializer selection together. Ownership Decision retains existing initializer relations; exact alias binding chain and batch preflight prevent value-only reuse. No new Map registry/trait or path-derived declaration. Then mixed cleanup/finishing and C consumption; schema-only changes do not close the series. Source install Stop stays until source/OBJ/EXE and old-edge cutover.
