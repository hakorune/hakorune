...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | R6-S0..S3 + S4 landed 2026-09-25; sole `LegacyCallV0` minter = v0 `boxcall` parser. Deprecation D0 closed `NoSafeSlice` (wire is Phase-0 emit contract + vm-hako + LLVM allowlists — product decision). Next: `MIR-CALL-V0-BOXCALL-MINT-PROMOTE-D0` — carrier-only promote feasibility (canonicalize coupling evidence). |
