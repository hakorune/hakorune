...
Smallest next slice: intrinsic Map acquisition/key/construction-end source contract in the Map construction owner, then exact obligation-preserving slot compatibility and candidate flow under OWN-FIELD-CONTAINER-DEST-D0. Existing callable semantic row and Script product supply core/body shape together; no new adapter or core-only extraction. Prior Home membership is not destination compatibility. One-entry transfer laws remain fixed; runtime/public cutover stays open.
