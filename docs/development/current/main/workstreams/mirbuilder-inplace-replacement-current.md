...
Smallest next slice: resolve NamedAllocationPhysicalAdmissionMissing in the existing allocation owner: bind selected result ABI to the actual consumer and account for generic/same-module precedence. No leaf/frame code until that mapping is accepted. Then finish producer/frame, both C walkers and natural Map-only OBJ/EXE plus six-edge retirement.
