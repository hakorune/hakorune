...
closed — Generic/Direct/Nested/LoopTrue/LoopCond S1 caller-zero observers and common admission D0 are green; no selector/Recipe/Builder/MIR/production. closed — Generic S0C neutral-owner Refactor Series: `loop_structural_facts::generic_g0` owns the move-only S0B/S0C aggregate and compiler projection is issuer-only; behavior unchanged, tests/guards/docs green. next Generic normalization, then assembler; selector/production/retirement closed.
```
