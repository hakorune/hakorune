...
Smallest next slice: close Map placement rejection/batch coverage, then mixed cleanup finishing/compiled-entry coordinates and shared terminal dispatch. Actual emitter/common LocalCommit/exact alias placement now pass four same-source callable-to-MIR cases with strict verification, original root validation and result mutation rejection. Source install Stop remains until C/source OBJ/EXE and old-edge cutover; see ownership checkpoint.
