...
Smallest next slice: select the integrated actual C query/host binding to MapBodyIndex. Reuse accepted program-observation cache, identity/wire validation and scoped Library/handle lifetime; do not re-audit definition readiness or all optimizer skips. Name the compile consumer and complete-frame dependency before public exports. No isolated query sibling, source switch or V1 Map fallback.
...
Checkpoint: Named observations now bind to exact MIR sites; demand/domain/original lanes consume Handle/Alias(0) together. V2 action8 distinguishes Named alias from Copy. Focused map_literal_28 passes (four new binding witnesses); C build/published rows/corridor/pointer guards pass; changed sources below800. Synthetic outcomes prove planner dependency only; actual query/frame/source-to-artifact remains open.
