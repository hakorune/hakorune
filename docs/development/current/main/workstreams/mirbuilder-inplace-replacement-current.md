...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | R6-S0/S1 landed. R6-S2 landed 2026-09-25: `PublishedMirBackendView` admission stops consuming `LegacyCallV0{Callee::Global}` — clean legacy Global rows route `UnsupportedBeforeObject`, selected admission raises `SelectedNormalUsesLegacyCallV0` unconditionally; func-carrier rows keep named errors; other legacy kinds stay `ExplicitCompatibility`. Next: R6-S3 compatibility-quarantine D0. Aggregate deletions stay open for R7. |
