...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | R6-S0 landed (S0-A callable-key SSOT + S0-B emitter split). R6-S1 Global cohort landed 2026-09-25: `Callee::Global` promoted to typed `MirInstruction::call`, ordinary-new birth edge stopped (`birth-global-legacy-stopped`), `emit_global_unified`+`name_const` deleted; no mint of `LegacyCallV0{Global}`; other cohorts declined — S1 exhausted. Next: R6-S2 backend-boundary D0. Aggregate deletions stay open for R7. |
