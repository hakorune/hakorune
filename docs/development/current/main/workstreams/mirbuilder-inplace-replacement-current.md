...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: S6+S7 landed; D20 decided (SSA family = loop-carrier binding desync — phis emitted but reads resolve via binding-id SSA store, not `variable_map`; `find/3` verify-clean wrong-code); S8 open (carrier binding publication into the resolved read authority). VM `ingest/1` unchanged (ledger-less). Gates 2-4 queued. |
