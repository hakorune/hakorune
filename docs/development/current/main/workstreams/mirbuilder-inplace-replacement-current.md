...
Smallest next slice: OWN-FIELD-CONTAINER-DEST-D0 closes public Map read/clone and one-table Native/Owned residence/end before intake. Native JSON Result is implemented: JSON5/Map8/GC7 pass; source errors propagate before destination writes, input Drop runs before lock/commit, and get_data is removed. Exact acceptance is in the ownership task. Source dispatch, owned intake, checked ABI and source execution remain open.
