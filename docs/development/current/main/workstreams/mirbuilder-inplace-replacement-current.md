...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. V2 rows/query and exact/operation/host-backed Original execution are implemented; transfer/expanded-function consumers and physical escape blockers precede public host cutover. Literal issuers stay unchanged until consumers are ready.
...
Checkpoint: normal/ASan each pass162 private compile cases, 104 kernel readbacks, eight traps and a non-host domain probe. Original allocation/Call/boxed rows consume only after emission; boxed tails/Bool aliases and projected String concat are corrected. Rust36 pass/one ignored; Named60/definition22 match parent, C build/corridor guard pass. Transfer/formal, expanded ingress, physical escape and public cutover remain open.
