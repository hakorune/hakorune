...
| F | `LOOP-PRODUCTION-SELECTION-D0` → Loop/M10b → M11 → M12 | D0 boundary + S0 manifest (44 rows) landed. **M10b-I0-R0 card open** (design_stop): switch = frozen source -> facts -> 5-family window -> `select_canonical_loop_family_v1` -> demand -> producer -> recipe -> one physicalizer; decomposition P1 spine + P2 physical edge (caller-zero) -> R0 atomic flip. Probe: Compatibility-mode loops run `route_loop`->`generic_loop_v1` today. |
