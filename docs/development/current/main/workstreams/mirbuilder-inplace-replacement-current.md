...
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | R6-S0..S3 + S4 landed 2026-09-25: unified-off Value mint promoted to `MirInstruction::call{Value}` (v0 wire byte-parity pinned; `[vm-reference/canonical-call]` + `UnsupportedBeforeObject` stops). Sole `LegacyCallV0` minter left: v0 `boxcall` parser. Next: `MIR-CALL-V0-BOXCALL-DEPRECATION-D0` — bounded retirement or NoSafeSlice (product wire decision). |
