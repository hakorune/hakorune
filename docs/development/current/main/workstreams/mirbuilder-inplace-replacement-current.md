...
Smallest next slice: connect retained New construction/end evidence and exact Map entries to source install/Fault successors, then existing Completion/physical end. NewSite-to-Binding alone is not obligation evidence; root binding lists alone lack pre/postcommit Fault state. Read/observer migration gates owned-slot intake, not source/physical development. MapOwnedReadProjectionConsumerMissing remains open; no source-only cutover claim.
