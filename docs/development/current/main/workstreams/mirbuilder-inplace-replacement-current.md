...
| F | `LOOP-PRODUCTION-SELECTION-D0` → Loop/M10b → M11 → M12 | D0 boundary decided (winner = canonical chain: 3 plan arms + 3 Main0 routes + Callable edge). **S0 landed**: `generic-m10b-deletion-manifest-v1.tsv` — 44 rows frozen with live caller counts (75 prod/9 test), drift recorded. Next `M10b-I0-R0` atomic switch + adapter removal; no repetition of landed G0 proofs. |
