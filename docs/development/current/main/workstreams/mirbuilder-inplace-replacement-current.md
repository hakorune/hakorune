...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Step1 domain closure and C Named emitter-selection extraction are verified; definition-plan ownership is also extracted; single-owner document extraction is verified; next is session/config
binding design. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: bind retained conditional Named outcomes to actual dispatch and required frame consumption using the extracted explicit-root selector. Preserve real entry identity, lazy Invalid/Unsupported outcomes and late diagnostics. Query does not issue reachability/Bypassed; no whole-skip census or duplicate matcher. Confirm storage/lifetime and caller/delete-set before session code; Map source switch remains later.
...
Checkpoint: explicit-root Named selector and shared typed-plan lookup serve both actual emitters and existing layout readers. C build, lazy selector negatives, duplicate-plan first-match/document-isolation test, Named60/role22 parent comparison, settings16, published rows and corridor pass. Changed sources below800.
Document/config/deferred diagnostic ownership remains landed. Conditional representation is distinct from execution coverage; pattern success/partial skips remain real output concerns, not query prerequisites. Retained consumption, host/frame/alias/capability binding and Map C cutover remain open.
