...
Smallest next slice: co-seal terminal AppMain Cataloged Call normal/Fault/result in existing disposition/Completion owners (Map owner frontier); literal I64 arguments to exact I64 formals, supported Map callee. Source audit closed; declaration kind retention implemented (package149/control33). Retain ordinary terminal relation before physical activation; selected scalar edge and ordinary install Stop retire in the same series. No repeated census, synthetic New continuation or scalar retry.
