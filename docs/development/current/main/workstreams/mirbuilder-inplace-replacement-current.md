...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private V2 rows/query and value consumers are implemented; After-group i1 PHI normalization and physical escape blockers precede public cutover. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: accepted after-whole-group original i1 PHI normalization at both existing walker group ends, with shared i64 reference consumption. Keep original SSA/types and pending i1-input Stops. Typed/direct escape, intrinsic allocator binding and boxed comparison remain CutoverBlockerOpen; capability and atomic public/source switch with V1 retirement follow. No Map-through-V1.
...
Checkpoint: original i64 references and shared i1 input validation pass normal/ASan426 each, focused control98, selected C build, pointer/corridor and parent-equal Named60/definition22. Both walkers exercise original exit30, Copy chains and nonmapped PHIs. Interim i1 Copy->constant30 bypass is fixed and regression-tested; no baseline waiver. Read-only audit closes next after-group i1 mapping. Source max646; no public cutover claim.
