---
Status: Closeout — MIR-CALL-CORE-R6-D1U-CONCAT-LEN-EXTERN-ISSUER-I0
Date: 2026-08-25
...
```text
  ...
  implementation    = D1U paired ConcatSubstringLen family landed at 75427a9aa2; selected writers 9 -> 7 and concat family 6 -> 4
  mode              = closeout
...
Decision: landed — replace only the paired ConcatSubstringLen direct Call literals with the thin canonical constructor, preserving fresh ValueIds, middle length Const/Add, outer destination, effects, spans, hints, and phase-9 fusion.
...
Smallest next slice: closeout only — receipt records `75427a9aa2`, default and emit-mir 1/1 focused tests, the 777-line guard, and the 441-warning baseline; no further implementation is admitted here.
