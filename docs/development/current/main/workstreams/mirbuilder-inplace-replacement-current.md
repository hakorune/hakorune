...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private V2 rows/query and value consumers are implemented; Formal/expanded consumers, original PHI reference/width closure and physical escape blockers precede public cutover. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: Formal/expanded consumers join one definition/call index and reject old-ABI ingress. Original PHI predecessor references/width normalization (future boxed aliases/Select), typed/direct object escape, intrinsic allocator session binding and selected boxed comparison stay CutoverBlockerOpen. Capability and atomic public session/host/source switch with V1 retirement follow; no Map-through-V1 or readiness census.
...
Checkpoint: f242a2320a records Phi/Select evidence (normal/ASan315 plus final control78). Both exact Static/Free callers now share argument/output/consumption; Original67 including four ordered/aliased argument readbacks passes. C build/corridor/pointer pass; Named60/definition22 match parent. Expanded index/domain/signature/ingress task order is fixed in Map SSOT. Formal and original PHI reference/width closure remain open; no public cutover claim.
