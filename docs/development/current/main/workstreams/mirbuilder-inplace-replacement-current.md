...
Smallest next slice: declaration capability plus existing Map/Home-aware verifier -> ordinary Completion seed/result under one source loan. Successful Dynamic second verifier/owned row is retired (package140 including same-pointer/APrime tests). Preserve separate AppMain root and S6C take. Declined Dynamic is outside this deletion; no generic fallback or Map execution claim.
