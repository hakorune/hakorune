...
| E | Loop/M8 S6/M9 S7 | S0 `798551e8de` 270 edges; S6D `b0fe3bf6a2`; S6E `0bdfbda1da` cursor18; **S6G `b419b84c3d`** all19 closeout + `NoCandidate`; M8 closed. **S7A landed caller-zero** (D1): `.hako` `loop_recipe/` single-file emitter + wire fixture + Rust parity harness (10 tests, all 3 normalizations equal vs Rust artifact); executable subset lacks `.hako` method calls/Map/Array literals — S7B cohorts must name their mechanism. |
