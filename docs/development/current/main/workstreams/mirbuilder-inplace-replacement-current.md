...
Smallest next slice: design remaining JSON raw Map observation with real fallible conversion/set terminal and reconcile public read/clone before Owned entry intake. GC owner projection now reaches actual metrics with typed unavailable, not silent skip; root GC7/module1/Map8 and kernel entry1 pass. OWN-FIELD-CONTAINER-DEST-D0 retains finite observer/entry/end inventory. JSON get_data remains; checked ABI/lifecycle and source OBJ/EXE activation remain open.
