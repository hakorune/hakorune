...
| D | Loop prerequisite closure (M10) | Active B3 `StringHelpers.split_lines/1`. Test asserts nested-push stop; live terminal/rows unobserved. Generic resolver-bound V2 issuer is absent; `source_bound_core.rs` is the V1 extension seam. CoreMethod lacks per-op Fault/mutation-on-Fault facts. Next define outcomes + one V2 co-seal; observe rows, connect existing LoopCond/GeneralIf, then meet M8/M9/M10, switch caller and delete selected edge. Physical success unproven. |
