...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private invocation/query-to-MIR binding and retained compile/cancel are verified; next is complete V2 frame/C consumption before public host cutover. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: complete the accepted V2 frame/leaf/expanded-function projection and both C consumers, binding existing Named observations and original lanes. Capability consumes the completed frame. Public session/host/source switch and V1 retirement follow atomically; no isolated public query, Map-through-V1 or another readiness census. Exact owners/acceptance remain in Map owner.
...
Checkpoint: private query integration passes actual Rust binding and same-invocation non-Map compile/EXE30 after input-file deletion, Map cancel and planner-reject cancel. Query17, focused28 plus ignored integration1, ASan lifetime16, settings16, Named60/definition22 parent comparison, Dynamic LLVM/provenance/machine comparison, rows and guards pass. No Map execution/cutover claim.
