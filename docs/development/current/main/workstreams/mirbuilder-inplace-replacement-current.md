...
#### MAP-LITERAL-WIDER-ORDINARY-OWNER-COVERAGE-D0 — design stop

Decision: design the two-exact-owner scalar Map slice; no implementation permission until source/owner/cleanup relation is closed.
...
Fail-fast boundary: root plus two exact called ordinary owners only; third/nested/conditional/recursive, uncalled, New/Home, field, unknown, native escape reject before install.
Smallest next slice: design and accept the owner-set/cleanup matrix, then replace `single_map_target_owner` and one-child install preflight only.
Non-claims: no new receipt/schema, extra Call shape, all ordinary owners, native Map read/clone, whole Map/R7 closure.
