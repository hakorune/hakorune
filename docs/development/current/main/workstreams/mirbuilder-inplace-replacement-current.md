...
Smallest next slice: design value-capable checked Map storage/install ABI (Map owner task2). Source capability -> Home-aware ordinary Completion is implemented: Value/TransferHome, exact formal coverage, sole result ownership and Indexed-consumer refusal; package146/control33/guards pass, max699. Preserve install Stop until runtime and ordinary callable Fault/finishing consumers exist. No repeat source census.
