...
Smallest next slice: exact owning entry/per-callable Completion design per [collection SSOT](../design/collection-literal-construction-ssot.md#owning-entry-and-ordinary-callee-accepted-dependency-design). Lambda-to-selected-C is ParkedSealed by existing export/physical Stops. Source capability, checked storage and ordinary-callee Fault/cleanup gaps are concrete; do not promote generic clone or physical scalar tags. No whole Map/R7 claim.
