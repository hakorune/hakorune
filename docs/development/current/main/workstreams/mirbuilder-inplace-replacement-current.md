...
#### MAP-LITERAL-FIVE-ORDINARY-OWNER-COVERAGE-I0 — closed; MAP-LITERAL-ORDINARY-NEW-HOME-CHILD-BIRTH-HANDOFF-I0 selected
...
Smallest next slice: move caller-scoped child Birth actual/ABI rows into the existing final handoff, make compiled-entry/JSON Birth consumption caller-aware, and retain shared cleanup coordinates; keep C child New/Home activation outside this I0.
...
Use the existing final-owner and physical-boundary owners with focused Rust tests and pointer/corridor guards. Source issuer and child validation are closed; only root-only Birth actual retention/consumption is replaced. C child gates remain stopped. Sources stay below800; no whole Map/R7 closure.
...
Evidence: four-owner semantic14/14 and direct/linked exit30+Fault cleanup pass; ordinary-child Value-only one-row/root+child exit30 pass; five-owner semantic15/15 and direct/linked exit30+A-E Fault cleanup pass. New/Home D0 accepts this Rust-only handoff I0; C child gates remain stopped.
...
The ordinary-child Value-only artifact recheck is closed: the source has one Call site, and direct/linked exit30 passes for child-only and root+child cases. Two-owner admission, four-owner retention, and five-owner bounded widening are closed. The selected New/Home I0 is Rust-only Birth actual/ABI handoff and caller-aware compiled-entry/JSON consumption; C child New/Home, field, and unknown values remain stopped.
