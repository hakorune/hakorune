...
Decision: bounded root-plus-one-called-ordinary-child scalar Map admission and direct/linked exit30 are closed. Enter design stop `MAP-LITERAL-MULTI-CALL-CLEANUP-D0`; physical ordinary_call Option-to-collection alone is not an implementation slice because the source issuer emits only a terminal Call.
Source authority + canonical issuer: existing `home_new_prefix` terminal Call issuer plus `direct_call_lifecycle` exact target/formal/site and Completion co-seal; existing owner-indexed Completion and finalized handoff remain the candidate owners. No new semantic receipt or second instruction graph.
...
Fail-fast boundary: until the design is accepted, reject nonterminal/second/nested/conditional/recursive Calls, foreign or stale sites, unsupported result uses, and uncalled/additional Map owners before transport or artifact. No physical caller switch.
Smallest next slice: D0 must specify one root main with one local I64 Call followed by the existing terminal I64 Call, literal actuals, and Value-only Maps; co-seal source order, Normal continuation, result binding, Fault cleanup, target inventory, and the old edge to retire. Decide whether existing wire schema suffices before I0.
Seed/result/index share `Rc`, S6C consumes exclusively, and the selected root plus one exact called ordinary Map owner is admitted only for scalar Value rows with no New/Home transfer. Uncalled or additional ordinary owners, unknown values, ownership transfer, and any second Call remain stopped; preserve unsupported install/exporter edges until the D0 source continuation and cleanup contract is accepted.
