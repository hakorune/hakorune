...
#### MAP-LITERAL-FOUR-ORDINARY-OWNER-COVERAGE-I0 — closed; MAP-LITERAL-ORDINARY-CHILD-VALUE-ARTIFACT-RECHECK-I0 active
Decision: implement exactly two local Calls A/B plus one terminal Call C for three distinct ordinary Map owners; keep the source order and owner checks in the existing root-call entry and widen only the bounded owner set.
...
Fail-fast boundary: A/B locals precede an optional root Map, then terminal C; fourth/nested/conditional/recursive, uncalled, root-before-first-Call Map/Home, New/Home, field, unknown and native escape reject.
Smallest next slice: correct the stale ordinary-child `ordinary_calls().len()` expectation from2 to1, then run its existing direct/linked Pair exit30 evidence; keep source/formal/cleanup owners and physical collection unchanged.
...
Use jobs4 locked quick focused Rust, selected C/runtime artifact proof, existing pointer/corridor guards; sources stay below800. The one source `main -> helper` Call site must produce one ordinary row; no whole Map/R7 closure.
...
Direct EXE and linked OBJ both exit30. Fault-order prefix probes show later owners are skipped. The four-owner source admission passes semantic 14/14, and direct/linked exit30 plus per-prefix Fault/cleanup probes pass. The next row only repairs the stale ordinary-child test expectation; a downstream physical failure must be classified separately.
...
The ordinary-child Value-only artifact red is a selected next-row test mismatch (`ordinary_calls().len()` actual1/expected2), reproduced on parent 1a876c3fba and current; it is not a production semantic claim. Two-owner admission and distinct-A/B Pair exit30 are closed; four-owner source-ordered retention and bounded owner widening are closed.
