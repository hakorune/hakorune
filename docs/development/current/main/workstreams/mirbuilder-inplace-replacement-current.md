...
Smallest next slice: fast S0-SHELF-SPLIT: move the NormalCallableSemanticPackagePortV1 impl from install.rs:504-738 into install/lowering_port.rs with an ordinary module declaration and mechanical child-path fixes only; preserve API/behavior/production callers, add no #[path] bridge, keep every touched/new source below 760, and defer all semantic loan/Cataloged I0 work.
