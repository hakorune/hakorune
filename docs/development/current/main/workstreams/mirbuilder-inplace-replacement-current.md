...
closed — GENERIC-G0-CANDIDATE-S1 -> Less/+Add disposition; 28 tests/guards/docs green; no selector/Recipe/Builder/MIR/production. closed — LOOP-FAMILY-DIRECT-OBSERVATION-S1 -> test-only AST-free observer; 7 tests/guard/docs green; no selector/Recipe/Builder/MIR/production. closed design — LOOP-FAMILY-NESTED-OBSERVATION-S1 -> exact projection matrix + lossless forest-error prerequisite; no producer/selector/physical authority. next — caller-zero implementation.
```
