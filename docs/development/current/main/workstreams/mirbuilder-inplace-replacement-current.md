...
Caller-zero Prelude materialization is closed: resolver-backed result and distinct Loop initializer publish separate bindings. Tail-only read is blocked until the open After receipt is closed; next is `CALLABLE-LOOP-AFTER-CLOSURE-P0`, then Tail/Completion and fresh-session `finish_for_draft_seal` -> DraftSeal; production selection and legacy retirement remain closed.
