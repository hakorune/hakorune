...
#### MAP-LITERAL-FIVE-ORDINARY-OWNER-COVERAGE-I0 — closed; MAP-LITERAL-ORDINARY-NEW-HOME-LIFETIME-D0 selected
...
Smallest next slice: design-audit one exact ordinary Map callee with `new Page()` and a Value Map, tracing existing constructor claim, Birth, root-exit, PhysicalBoundary and cleanup owners; no implementation or ordinary New/Home activation before the chain is explicit.
...
Use read-only owner/caller audit and the existing pointer/corridor guards; design stop forbids code, fixture, fallback, and production switch. Sources stay below800; no whole Map/R7 closure.
...
Direct EXE and linked OBJ both exit30. Fault-order prefix probes show later owners are skipped. The four-owner source admission passes semantic 14/14, and direct/linked exit30 plus per-prefix Fault/cleanup probes pass. The ordinary-child Value-only recheck passes with one Call row and root+child Pair exit30. Five-owner source admission passes semantic 15/15; direct/linked exit30 and A-E Fault-prefix/cleanup probes pass. New/Home lifetime remains stopped pending D0.
...
The ordinary-child Value-only artifact recheck is closed: the source has one Call site, and direct/linked exit30 passes for child-only and root+child cases. Two-owner admission, four-owner retention, and five-owner bounded widening are closed. New/Home, field, and unknown values remain fail-fast.
