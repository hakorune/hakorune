...
Smallest next slice: operation-derived MIR/key/Map/outcome projection and mixed-origin cleanup, then C Map emission using session layouts. Opaque ABI/descriptor V2/session2 is implemented: Rust31, actual archive/symbol checks, C ABI execution and 10 session refusals pass; ownership task owns receipt. Preserve unused aliases without new Home and prior Maps on later New Fault. Source install Stop stays until source/OBJ/EXE and old-edge cutover.
