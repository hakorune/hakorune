...
#### MAP-LITERAL-FIVE-ORDINARY-OWNER-COVERAGE-I0 — closed; MAP-LITERAL-ORDINARY-NEW-HOME-CHILD-BIRTH-HANDOFF-D0 selected
...
Smallest next slice: design the existing final-owner handoff for one exact ordinary Map callee with `new Page()` and a Value Map: child Birth actual/ABI retention, compiled-entry consumption, and shared cleanup coordinates; no implementation or C child New/Home activation before the chain is explicit.
...
Use read-only owner/caller audit and the existing pointer/corridor guards; the source issuer and child PhysicalBoundary are present, but root-only Birth actual retention/consumption is the open transfer. Design stop forbids code, fixture, fallback, and production switch. Sources stay below800; no whole Map/R7 closure.
...
Evidence: four-owner semantic14/14 and direct/linked exit30+Fault cleanup pass; ordinary-child Value-only one-row/root+child exit30 pass; five-owner semantic15/15 and direct/linked exit30+A-E Fault cleanup pass. D0 isolates root-only child Birth actual retention/compiled-entry; C child gates stopped.
...
The ordinary-child Value-only artifact recheck is closed: the source has one Call site, and direct/linked exit30 passes for child-only and root+child cases. Two-owner admission, four-owner retention, and five-owner bounded widening are closed. Ordinary child New/Home is stopped at root-only Birth actual retention/compiled-entry consumption; field and unknown values remain fail-fast.
