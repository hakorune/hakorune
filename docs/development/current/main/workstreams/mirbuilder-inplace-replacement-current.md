...
Smallest next slice: change existing native JSONBox::set to Result, propagate nested Map observation errors before destination write, and retire get_data after owner-borrow migration. Preserve success Box and no child cloning; input disposal occurs outside destination lock. OWN-FIELD-CONTAINER-DEST-D0 owns finite errors, Rust signature change and acceptance. No source dispatch or Owned activation; public Map read/clone/end and checked ABI remain open.
