...
Caller-zero Prelude materialization is closed. `CALLABLE-LOOP-AFTER-CLOSURE-P0` is green: real Prelude -> seven operations (`Pure=4`, `Read=2`, `Write=1`) -> fixed CFG edges -> canonical CFG/identity seal -> `ReadyLoopAfterContinuationV1`. Next is exact Tail-to-ValueId plus one Completion claim; finish/DraftSeal, production selection, and legacy retirement remain closed.
