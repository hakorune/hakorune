...
| F | `LOOP-PRODUCTION-SELECTION-D0` → Loop/M10b → M11 → M12 | **D0 opened** (`loop-production-selection-d0-2026-09-24.md`): winner = existing canonical chain (3 plan arms + 3 Main0 root routes + Callable edge); in-boundary M10 seals closed, PRESESSION-I0/ALWAYS scoped outside with reopen triggers; next `GENERIC-M10B-DELETION-MANIFEST-S0`, then switch + adapter removal. |
