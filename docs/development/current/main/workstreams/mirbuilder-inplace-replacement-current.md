...
Caller-zero Prelude materialization is closed: resolver-backed result and distinct Loop initializer publish separate bindings. Next is Tail-to-ValueId and fresh-session `finish_for_draft_seal` -> DraftSeal; production selection and legacy retirement remain closed.
