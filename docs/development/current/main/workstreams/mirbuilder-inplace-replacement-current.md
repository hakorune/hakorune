...
Smallest next slice: move ordinary Map Completion/terminal into its existing per-callable ledger held by the result row; accessors borrow the same product. Then select the exact child ledger and connect ordinary ABI. Caller two-ingress cleanup/finishing is implemented for 0/1/2 Homes. Preserve ordinary install/exporter Stops; EXE/linked OBJ/Fault evidence remains open. Accepted owner placement and deletion are in Map owner.
