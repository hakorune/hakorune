...
Smallest next slice: select the proved source Map cohort through common install, prove source OBJ/EXE and retire its old edges. Mixed finishing and C physical consumer pass focused/runtime tests; NativeArray host regression also passes. Source install Stop remains; see the ownership checkpoint for evidence and baseline classification.
