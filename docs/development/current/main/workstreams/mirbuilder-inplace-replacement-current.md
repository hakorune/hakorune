...
Smallest next slice: connect source Normal acquisition/Map transfer flow to root Completion/Recipe cleanup under OWN-FIELD-CONTAINER-DEST-D0. Exact PlainI64NoHook descriptor compatibility is closed; a Map later in the source retains New claims but leaves terminal cleanup unavailable. Do not repeat descriptor/root-loan census or add accessor-only receipts. Map end policy is fixed; actual source/physical/runtime consumers and public cutover remain open.
