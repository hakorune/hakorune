...
| --- | --- | --- |
| A | `MIRBUILDER-FINAL-ACCEPTANCE-SCOPE` in final-pipeline | Closed: the fixed 11-entry EXE receipt and exclusions are recorded; no acceptance run remains to select here. Its scope feeds convergence after required owner cutovers. |
| B | M7-S remaining owner-unit Stop/Promote/Delete | Historical census closed `NoSafeSlice__M7SNoRemainingUnsharedDeleteSet`; it selected no action. Shared owners permit finite caller migration and selected old-edge removal under the current R7 contract. Keep other readers; do not require an unshared owner or repeat this census. |
| C | Call/R7 `MIR-CALL-COMPATIBILITY-RETIRE-R7` | All production `LegacyCallV0` ingress minters retired (S4/S5); D1..D4 caller-zero censuses landed S6 (Method arm), S7 `ec9a78254a` (array-write repair), S8 `b9d6853bef` (Closure arm), S9 (Global no-op arm). D4 worker census: remaining surface = interpreter/published-view fail-fast boundaries, compat owners, variant lifetime, shared readers — `NoSafeSlice`; reopen on v0 wire or llvmlite lane retirement. |
| D | Loop prerequisite closure (M10) | B3 D2 is family-local `NoSafeSlice`: production substring route/codepoint outcome and ArrayPush provider/failure/commit authorities are missing. Reopen only on tracked route/provider plus source-outcome evidence; D3, LoopCond connection, cutover and selected-edge deletion remain open. |
| E | Loop/M8 S6/M9 S7 | S0 `798551e8de`; S6D `b0fe3bf6a2`; S6E `0bdfbda1da`; **S6G `b419b84c3d`** all19 closeout; M8 closed. **S7A `f6c1d71015`** (D1): `.hako` wire subtree (scalar/string only). **S7B1–S7B5 `f4b1432929`..`f96fec7d8a`**: M8A/B/C(V2)/D/E wire + parity vs producer issuers; M9 producer half deferred. **S7G landed**: all19 parity — 8 wire-backed routes (incl. DirectAccum/Nested/LoopTrue live-producer emitters) + 11 declined pinned in census; literal M9 Done deferred. |
| F | `LOOP-PRODUCTION-SELECTION-D0` → Loop/M10b → M11 → M12 → C0 | R0 `6e88441c0b`; M11 `9f55aea31b`/`8dff99ef22`; M12 `682acbc2ca`→`b50e8b9a56`; baseline `e53c0693c5`. C0 `25a030b5f3`/`f8f76d5ade`/`94ffe5e5cc`: 605-row census (457 durable/134 test/7 bootstrap/7 orphan-retired); move manifest = 598 keep, homes already satisfy four-box split. Cleanup steps 2-3 landed (topology rebase, root surface, context census/split, role manifest, JoinModule disposition); final audit closed at row G. |
| G | `REPO-FINAL-CONVERGENCE-AUDIT0-G0` and physical cleanup | Closed 2026-09-25: 34-record disposition matrix + `repo_final_convergence_guard.sh` registered/green; import hygiene fixed; source-backed gates 15/15+164/164 pass on a vm-reference build; 11-entry EXE 2/9 failures assigned to `MIRBUILDER-FINAL-ACCEPTANCE-SCOPE`. Recheck closed: gate-1 unsatisfied; `MIR-CALL-R6S0-BOUNDARY-SELECTION-D0` accepted S0-A 2026-09-25. |
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: DEAD-ROUTE-SOLE-FAMILY-S1 landed — TypedDeclined `LoopBreakRecipe` no longer suppresses live route predicates nor contends for `sole_family`; `trim/1` front-selects `LoopCondBreakContinue`, EXE reaches `SourceCallOutsideSelectedFamily` (condition `s.substring`, D5). VM `ingest/1` unchanged (ledger-less). D14 open: condition call-coverage census. Remaining: D5-sealed forks, NamedArray/toolchain/inference. Gates 2-4 queued. |
...
Map literal remains CutoverBlockerOpen for wider source coverage; raw/Core now emit IntrinsicMap/write through the retained V2 host. The former Named(MapBox)/birth/set edges are retired in the bounded checkpoint. The [Map owner task](../design/collection-literal-construction-ssot.md#map-literal-selected-construction-design) contains exact issuers, physical gap, inventory, deletion and acceptance. No Exhausted/R7 claim.
#### MAP-LITERAL-FIVE-ORDINARY-OWNER-COVERAGE-I0 — closed; MAP-LITERAL-ORDINARY-NEW-HOME-CHILD-BIRTH-HANDOFF-I0 closed; C execution D0 next
Decision: implement exactly four source-local Calls A/B/C/D plus one terminal Call E for five distinct ordinary Map owners; keep source order and owner checks in the existing root-call entry and widen only the bounded owner set.
Source authority + canonical issuer: resolver exact Call target/site observations, existing affine loan, Completion/root-flow co-seal and owner index.
Non-authority: declaration existence, Map detection, MIR shape, physical function count, owner count, or a collection length cannot issue admission.
Fail-fast boundary: A/B/C/D locals precede an optional root Map, then terminal E; sixth/repeated, uncalled, root-before-first-Call Map/Home, nested/conditional/recursive, New/Home, field, unknown and native escape reject.
Smallest next slice: implement the accepted existing C child New/Birth/HomeRelease consumer and its exact selected-C/OBJ/EXE boundary; keep all other child shapes and whole-Map routes stopped.
Non-claims: no new receipt/schema, general local-Call cleanup, all ordinary owners, native Map read/clone, whole Map/R7 closure.
The existing final-owner and physical-boundary owners are reused. Rust handoff implementation landed at c3fa1f8a75; focused root/child compiled-entry, JSON, and cleanup tests pass. C D0 is accepted at 5f62746179; C I0 is limited to the five root-only assumptions and one ordinary child path; sources stay below800; no whole Map/R7 closure.
Checkpoint: child Completion/terminal, root-exit/literal/field progress and PhysicalBoundary retention/recheck are complete after S6C; the existing ordinary Call payload is rebound after finishing and moved once into the finalized root source handoff, child symbol coverage is included, and the Rust physical program/compiled-entry contract carries the exact ordinary I64 Call plus cleanup coordinates.
The role-based physical JSON, C admission/index/emitter, direct EXE and linked OBJ are connected for the existing Pair cohort: exits30, bool/range cases exit70 with Fault103, and cleanup probe passes. The two-distinct-owner scalar-Map source admission and direct/linked artifact test both exit30; selected lifecycle parameter/return capability checks are scoped to each emitted ordinary function, and ordinary-call scalar state/emission is consumed without a storage lease.
The new two-Call source and MIR consumer tests prove local result binding, owner-scoped co-seal, strict CFG verification, and two physical I64 Call terminators. The three-owner source now retains two site-tagged local groups, the repeated-target fourth Call is rejected, and grouped duplicate/order/foreign/omission negatives pass.
Evidence: four-owner semantic14/14 and direct/linked exit30+Fault cleanup pass; ordinary-child Value-only one-row/root+child exit30 pass; five-owner semantic15/15 and direct/linked exit30+A-E Fault cleanup pass; c3fa1f8a75 passes root Birth, child-only Birth+Map/New, caller-indexed compiled-entry, JSON Birth emission, and cleanup coordinates. C child gates remain stopped.
Wider ordinary Map owners, New/Home transfer, unknown scalar values, and whole Map/R7 claims remain stopped. Owner-gate, child-symbol-drift, Call-handoff, child/root/field, physical Call-admission and ignored Pair/Bool/two-Call tests pass; Pair ABI/OBJ/EXE/Fault evidence and the single-terminal retirement are closed.
The ordinary-child Value-only artifact recheck is closed: the source has one Call site, and direct/linked exit30 passes for child-only and root+child cases. Two-owner admission, four-owner retention, and five-owner bounded widening are closed. The selected New/Home I0 is closed as a Rust-only Birth actual/ABI handoff with caller-aware compiled-entry/JSON consumption; C child New/Home, field, and unknown values remain stopped for the next D0.
...
Non-claims: existing runtime ABI evidence covers ABI8/String2/codec38/legacy Map11 only; object destination/lifetime dependency remains open. Static raw/Core source cutover has landed; natural mixed-formal callable Map and wider owning-slot coverage remain open. Bounded checked execution and Script evidence do not close R7. No blanket non-Map tagged rewrite, private-symbol wrapper or retained production v1 fallback.
