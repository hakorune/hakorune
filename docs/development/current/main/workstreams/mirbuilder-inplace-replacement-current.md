...
Smallest next work: close Map-demanded formal ingress/non-Map consumers and versioned compiler frame in the [Map owner](../design/collection-literal-construction-ssot.md#published-operand-projection-inventory). Selected Static Integer/Bool calls prove formal demand is real; retain kind/payload at exact demanded positions, one body. Runtime store/String ABI is fixed but unimplemented. No repeated broad census; code stays unchanged until mapping closes.
