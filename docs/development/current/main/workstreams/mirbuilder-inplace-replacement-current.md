...
Smallest next slice: accepted candidate-descriptor ownership step in ordinary_new_coseal. Resolve existing construction/destruction/Birth once in candidate source order; field callback borrows the same candidate, deleting repeated construction_for/tuple reclassification. Preserve unavailable descriptors and move claims/handoffs once. Multiple-error diagnostic order can change; no supported admission widening. Acceptance and Map phase mapping: existing OWN-FIELD-CONTAINER-DEST-D0 owner section.
...
Checkpoint: callable lowering loan retains the same row body shape (including Map EntryValue order); batch29/package119/Completion33/low-level absent-shape1 pass. Missing row still rejects. Worker Option-reader audit found no production admission widening. Pointer/corridor and diff checks pass; changed source max483. Previous JSON checkpoint:444 pass/4 baseline fail/2 ignored; identical parent a3ee23938d failure bodies. No Map transfer/intake/C/EXE claim.
