...
Smallest next slice: MapOwnedReadProjectionConsumerMissing. Keep missing distinct from projection error through a checked selected result consumer; source borrow and Dynamic self-contained publication are separate. Bare i64/zero legacy reads cannot carry owned-unavailable. Native JSON observation clone edges are removed; GC/raw table, public read/clone/delete/clear/end and owned-slot intake remain open. No accessor-only adapter or error-to-zero fallback.
...
Checkpoint: JSONBox observes stored Array/Map children by borrow, removing two clone edges; top-level disposal point retained. JSON filter: current444 pass/4 fail/2 ignored; parent a3ee23938d442/4/2. The same four failure bodies match exactly (baseline debt); two new tests cover stored-value observation, zero clones and independent JSON output. Pointer/corridor and diff checks pass; changed source max285. No owned-slot/C/EXE claim.
