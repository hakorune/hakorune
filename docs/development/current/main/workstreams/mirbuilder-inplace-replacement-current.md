...
| F | `LOOP-PRODUCTION-SELECTION-D0` → Loop/M10b → M11 → M12 | [M10b-I0-R0-VAR](../investigations/mir-call-variable-accum-recurrence-production-i0-2026-09-24.md) landed `b579959a3d`; R0 flip + ordered-scheduler retirement landed `6e88441c0b`; guard re-pin + test split `a97f564250`. Gates/guards green. M11-A retired the coreloop composer husks + varmap inventory re-point; M11-B retired the located_loop husk cluster (the callable_result pins were dormant, existence-only guard code — no active contract). Residual: M12. |
