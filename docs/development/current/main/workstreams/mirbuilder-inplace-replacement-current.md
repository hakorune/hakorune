...
Smallest next slice: producer-owned stable original i64 references and shared mapped/nonmapped i64 PHI inputs, per accepted Map SSOT decision. Preserve old rDst/types and existing i1 paths; pending i1 normalization stays open. Typed/direct escape, intrinsic allocator binding and boxed comparison remain CutoverBlockerOpen; capability and atomic public/source switch with V1 retirement follow. No Map-through-V1.
...
Checkpoint: private Formal/expanded implementation at270643e311 has normal/ASan396 full and final focused85 each; C/guards and parent Named60/definition22 pass. Read-only audit now closes the next i64-PHI mapping and identifies downstream nonmapped PHIs as mandatory consumers. No code/fixture changed in this design checkpoint. Original-value execution witnesses and i1 boundaries are required; no public cutover claim.
