...
Smallest next slice: select the exact ordinary child via owner-indexed Completion, carry it into the existing physical state and owner-indexed root-exit progress, then connect existing ordinary ABI/cleanup; seed/result/index share `Rc`, S6C consumes exclusively, mixed AppMain/ordinary Maps remain stopped. Preserve install/exporter Stops; EXE/linked OBJ/Fault remain open.
...
Checkpoint: owner-indexed Completion retention and owner-keyed root-exit progress land after S6C without a second completion or mutable root slot; mixed AppMain/ordinary Map remains stopped. Package157/157 passes; source max747. Physical Map execution/intake/cutover remains open.
