...
Smallest next slice: finite bound ordinary/Map origin and emission progress shared by claim preflight, later-New failures and root cleanup; then Map/key/outcome MIR and C projection. Mixed-cleanup audit in the ownership task names all consumers, including empty-Map observation and retained failure suffixes. Nonempty root graph projection is verified (5 graph tests plus Pair finishing); Map operations remain open. Preserve aliases/prior Maps; install Stop stays until source/OBJ/EXE cutover.
