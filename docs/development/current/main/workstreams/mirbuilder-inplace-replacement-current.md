...
Smallest next slice: select the exact ordinary child via owner-indexed Completion and connect existing ordinary ABI/cleanup; seed/result/index share `Rc`, S6C consumes exclusively, mixed AppMain/ordinary Maps remain stopped. Preserve install/exporter Stops; EXE/linked OBJ/Fault remain open.
...
Checkpoint: owner-indexed Completion retention lands after S6C with pointer identity; mixed AppMain/ordinary Map remains stopped. Package157/157 passes; source max747. Physical Map execution/intake/cutover remains open.
