...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Step1 domain closure and C Named emitter-selection extraction are verified; next is the accepted physical definition-plan extraction. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: BoxShape planned leaf/same-module storage and metadata reader into one private C definition-plan owner. Production membership/iteration consumes it; delete the old independent arrays/add/membership code. Preserve capacities256/1024, duplicate counts, order, partial failure, emitted-state separation, leaf eligibility and Dynamic launch. C build, actual-role/metadata/capacity parent comparison and existing proofs required. No query/session activation or source switch.
