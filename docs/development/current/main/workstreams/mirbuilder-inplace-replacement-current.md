...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: S6 landed (`into_document_parts` document completion); S7 landed (Document call-edge policy + non-Integer-result reclassification to `UnsupportedBeforeObject`). EXE past `call-argument-type-drift`, now at merge/Phi dominance drift. VM `ingest/1` unchanged (ledger-less). Gates 2-4 queued. |
