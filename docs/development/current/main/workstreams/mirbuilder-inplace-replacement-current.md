...
| H | unified selfhost resume owner | Gate 1 `real-apps-exe-boundary`: COND-SUBSTRING-COVERAGE-S2 landed — CoreMethod placement arm is an allowed set per (op,arity) (`StringSubstring/2` now Body+Condition); `trim/1` condition `s.substring` armed, EXE advances to `DeclaredInstanceLocatorNotConsumed` at package complete. VM `ingest/1` unchanged (ledger-less). D15 open: locator issue vs consumption census. Gates 2-4 queued. |
