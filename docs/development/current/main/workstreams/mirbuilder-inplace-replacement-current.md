...
Smallest next slice: design-only S0-SHELF-SPLIT after D0-SIG-CLOSE-F acceptance: freeze the behavior-neutral move of the NormalCallableSemanticPackagePortV1 impl from the ~738-line install.rs into install/lowering_port.rs, with an ordinary module declaration, unchanged API/behavior/production callers, no #[path] bridge, and every touched/new source below 760; no semantic loan code, profile widening, AST rescan, public receipt, or package field.
