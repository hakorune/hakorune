...
Smallest next slice: read-only session lifecycle/ABI audit (StaticInvocationDocOwnershipMissing). Bind the existing program view and extracted definition plan to one C-owned parsed document, with a doc-taking compile consumer and explicit settings. Name open/query/compile/close ownership, failure cleanup and host library lifetime before adding exports. Keep source issuers closed; do not create a second role planner.
