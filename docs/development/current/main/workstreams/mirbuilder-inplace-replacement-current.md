...
closed — Loop-family S1 observers + common D0 caller-zero; selector/Recipe/Builder/MIR/production=0. closed — S0C neutral owner, Generic normalization, R0 evidence retention, resolver window lease S0, common admission assembler S1, and caller-zero selector S2; tests/guards/docs green (R0: 89; lease: 3; assembler: 6; selector: 3). The new selector consumes only Ready five-row windows; old family_selection remains historical. next — source-to-Recipe handoff design; production/retirement closed.
```
