...
Smallest next slice:
  `CHRONIC-MEASUREMENT-SITE-OWNER-MAP-D0` is the only next design cell: use the tracked 185-row neutral receipt to classify every site with explicit owner/role/successor evidence; expectation pinning, guard registration, and deletion remain closed.
