...
| D | Loop prerequisite closure in `joinir-loop-selfhost-recipe-pipeline-ssot.md` M10 | S6D remains separate M8 producer/observation. Active Call row: B3 `StringHelpers.split_lines/1`; imported static method is confirmed, while nested-push stop is only the test's assertion and physical LoopCond success is unproven. Next settle carrier/operation/effect/Fault and co-seal mapping, observe live issuer rows, then after M8/M9/M10 connect, switch this caller, and retire its selected old edge. |
