...
Smallest next slice: Map-slot destination/transfer Decision under existing OWN-FIELD-CONTAINER-DEST-D0, per Map SSOT. Raw/Core issue neither Home transfer nor Dynamic carrier forwarding; runtime-first retain is withdrawn. Separate Trivial/Home/Shared/carrier/borrowed input, failure commit, read, replacement/remove and parent end. Keep escape Stops; runtime/profile, TLS/direct, intrinsic binding and boxed comparison remain included cutover blockers.
