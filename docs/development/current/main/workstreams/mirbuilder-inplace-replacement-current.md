...
Decision: implement the accepted [v2 frame](../design/collection-literal-construction-ssot.md#versioned-compiler-frame-decision) in three steps: MIR/Core plus frame contract; both C consumers; atomic source/host switch with v1 and six literal-edge retirement. Private document/config/Named outcome ownership is verified; next is invocation/session binding design. Literal issuers stay unchanged until consumers are ready.
...
Smallest next slice: read-only invocation/session premise audit: bind the existing program/definition owners and retained outcomes to one owner; identify conditional walker selection, host/frame consumer and close-before-library-drop lifetime. Reuse the Map inventory; no query-only sibling, duplicated optimizer or source switch. Accept one bounded implementation slice before code.
...
Checkpoint: retained Named outcomes serve both actual emitters through indexed identity lookup; both direct selector edges are deleted. C build, selector/outcome units, Named60/role22 parent comparison, settings16, ASan lifetime16, published rows/corridor pass. Dynamic LLVM/origins/machine code/relocations match. OOM stops both dispatchers before object; changed sources below800.
Document/config/deferred diagnostic/outcome ownership is implemented. Conditional representation is distinct from execution coverage; pattern success/partial skips remain real output concerns. Invocation/session, host/frame/alias/capability binding and Map C cutover remain open.
