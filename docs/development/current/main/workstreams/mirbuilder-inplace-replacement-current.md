...
`LOOP-RECIPE-OPERATION-EMITTER-CONST-S0` is closed: exact-block Const/type-fact emission,
typed pre-rejects, whole-session discard, and fresh-session repeat are green. Full Loop physicalization, production activation, retirement, and ReadBinding implementation remain closed; final order follows `mirbuilder-final-pipeline-ssot.md`.
