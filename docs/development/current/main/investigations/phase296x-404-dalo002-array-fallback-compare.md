...
Status: Draft
Date: 2026-05-30
Scope: row404 DALO-002 array fallback compare
Related:
...
| --- | --- | --- | --- | --- |
| `_lower_array_collection_method_call` | array surface | `ArrayBox` | `296-346` | Keeps fail-safe arity defaults, then tries the exact direct-array lane, then the canonical array symbol table. |
...
The array and map branches are fallback surfaces, not primary owners.
