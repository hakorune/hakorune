...
`MIR-CALLABLE-LOOP-GENERIC-TERMINAL-PORT-P0` in `design_stop`.
...
the caller-zero relation view is now landed as a borrowed observation product.
The next terminal-port design is the only implementation permission. No
normalizer consumer, fixture, production switch, or fallback is authorized in
that design stop.
