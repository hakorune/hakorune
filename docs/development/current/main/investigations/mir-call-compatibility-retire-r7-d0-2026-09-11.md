...
Windows capability and shared schema removal. Design is delegated to the bounded
parser/contract I0 successor card.

| Order | Design task | Required output / exit |
| --- | --- | --- |
| 1 | Exact Program membership | Freeze the five partitions and select the bounded migratable subset: direct `Str`/`String` literal receiver + `length|size` + zero method args, and `New(StringBox)` with exactly one direct string constructor arg, no field initializers + `length|size` + zero method args. Keep direct/New `indexOf`, extra constructor/method args, embedded/opaque or non-literal strings, malformed/missing values, and unrecognized methods on compatibility/null paths; an empty string literal is valid. Record that the old first-string scan is broader than this membership and is not authority. |
| 2 | Co-sealed source-to-contract issuer | Keep the owner in `src/stage1/program_json_v0/authority.rs` inside the existing parse/lower transaction (`source_to_program_json_v0_impl` -> `ast_to_program_json_v0_with_imports`): issue Program-v0 and a source-only `Stage1StringBoxSourceProductV1` together, then co-seal the borrowed `CoreMethodContractBox` row. The product must carry source owner/path, MethodCall parent/receiver/ordered args, selector/arity, `New(StringBox)` site/class/constructor args, field-initializer/type-argument classification, and a lowering-issued one-to-one Program-v0 output anchor/crosswalk entry; duplicate/missing path, receiver/class, arity, or unclassified relation is a pre-publication typed rejection. `VerifiedResolvedMethodCallSourceV1` remains a route-neutral carrier candidate, not this issuer: it expects a FunctionDeclaration and has no New/StringBox membership. |
| 3 | Selected source-enriched Rust handoff | Make `Stage1ProgramJsonModuleHandoff` the sole successor owner: design `SourceProgramJsonAuthority::for_source` to return one source artifact `(program_json, Stage1StringBoxSourceProductV1, Stage1ProgramJsonCrosswalkV1)` and add `Stage1ProgramJsonModuleHandoff::from_source_artifact(...)` with the terminal pipeline `from_source_artifact -> json_v0_bridge::parse_json_v0_to_module -> admit_stringbox_source -> Stage1FinalizedMirModule`. `Stage1ProgramJsonModuleHandoff::parse(&str)` remains the body-only compatibility seam; do not derive anchors by a post-hoc JSON/MIR scan. Lowering accumulates typed expected-shape anchors beside JSON; admission validates the product against exactly one existing `MirInstruction::Call`, consumes it once, and rejects zero/multiple/unconsumed or mismatched anchors. It must not physically rewrite the bridge output. Keep Hako registry/fallback and `env.mirbuilder.emit` as compatibility capsules; direct MIR rejects schema 2.0 and v0 `call`/`mir_call`, so op/schema spelling is not migration. |
| 4 | Conditional caller-switch I0 | Do not switch the Hako registry/fallback callers in this issuer slice: they receive body-only Program JSON and cannot supply the source product. The source-owning upstream is `SourceProgramJsonAuthority::for_source` once it returns the enriched artifact; only after that caller can pass the co-sealed product may both old callers switch and remove the exact selected `_emit_length_mir`, `_emit_size_mir`, and `_emit_new_stringbox_boxcall0` edges in one series. Until that handoff exists, retain both compatibility capsules and all `indexOf` edges; never manufacture caller-zero. |
...
`src/host_providers/mir_builder/handoff.rs::Stage1ProgramJsonModuleHandoff` is the selected consumer owner; its current Hako callers receive body-only JSON and cannot issue the product. `src/runner/json_v0_bridge/lowering/expr/call_ops.rs` is a separate Rust Program-v0 ingress comparison, not the Hako output consumer.
`src/stage1/program_json_v0/{authority.rs,lowering.rs}` supplies Program-v0 structure and is the proposed single issuer owner; its scope flattening and multi-local expansion mean the crosswalk must be issued during lowering, not reconstructed from JSON indices. `src/host_providers/mir_builder/handoff.rs` currently loses provenance and needs the explicit product/crosswalk API. `src/mir/resolved_semantics/{body_shape.rs,selected_new_arguments.rs}` provide partial relations but no issuer.
`program_json_expr_method_shape_scan.hako` and the New shape scan are parity
summaries, explicitly not Recipe/route issuers.

D0 closes only when membership, issuer, consumer and planned deletion agree; here they now agree for the bounded length/size artifact slice, so this D0 closes and the next I0 owns implementation.
