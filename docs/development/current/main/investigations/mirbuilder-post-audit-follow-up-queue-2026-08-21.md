...
| `MIR-CALLABLE-LOOP-PHI-GENERIC-SEMANTIC-DEMAND-D0` | High (**active design stop; bounded first cohort**) | [`generic semantic-demand D0`](./mir-callable-loop-phi-generic-semantic-demand-d0-2026-09-11.md); selected callable ingress callback plus one demand issuer | callback-borrowed resolver input/ledger → source-bound GenericLoopV1 candidate before `VerifiedLoopOperationPhysicalDemandV1` | first prove the callback-borrow handoff, then choose one non-nested straight-line-I64 producer that co-seals Recipe/Core/operation/effect/After, or retain `NoSafeSlice`; no RawInvocationChildPort semantic field, broad GenericLoopV1 promotion, old Composer fallback, or new physical issuer |
