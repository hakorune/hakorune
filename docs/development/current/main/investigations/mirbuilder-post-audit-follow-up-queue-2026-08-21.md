...
| `MIR-CALLABLE-LOOP-PHI-SOURCE-INDEX-HEADER-HANDOFF-D0` | **High (design stop reopened 2026-09-11 by production-entry probe)** | [`session-entry I0 card`](./mir-callable-loop-phi-session-entry-i0-2026-09-11.md) | existing resolver/module owner for the exact `VerifiedCallableIndexV1` + `VerifiedCallableHeaderV1` pair and the selected prefix's exact prelude target contract at the selected non-AppMain child boundary | identify one existing issuer/owner and one scoped index/header/target handoff, or explicitly restrict Method prefix shapes and prove a valid FreeStatic caller; preserve `VerifiedCallableFunctionLoweringInputV1::issue` and `MissingPreludeTarget`; no name/method-selector/catalog-key repair, physical-signature substitute, main-only index sharing, second issuer, fallback, fixture, or production switch |
