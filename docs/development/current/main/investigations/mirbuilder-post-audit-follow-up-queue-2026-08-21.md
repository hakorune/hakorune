...
| `MIR-CALL-NORMAL-TYPED-ADMISSION-D0` | `PublishedMirBackendView` / selected object admission | normal Static/Free/Print published callback | LegacyCallV0 mixed-shape acceptance in the selected normal view | typed `MirInstruction::Call(MirCall)` remains CanonicalTyped; LegacyCallV0-only remains ExplicitCompatibility; any mixed selected normal module rejects before body serialization/object creation with a named terminal; no new issuer or fallback |
