...
| `MIR-CALLABLE-LOOP-PHI-GENERIC-PHYSICAL-DEMAND-D0` | High (**active design stop; `NoSafeSlice__GenericRecipeLacksPhysicalDemand`**) | [`generic physical-demand D0`](./mir-callable-loop-phi-generic-physical-demand-d0-2026-09-11.md); `CallableGenericLoopV1SemanticRecipeV1` / `PlanBuildOutcome` | compiler-side physical-demand owner, before any Builder session or raw Loop consumer | prove reuse or define one accepted compiler-side projection to `PreparedLoopOperationProgramV1`; preserve source-bound item/binding/site and continuation/After relations; no AST reparse, name lookup, old Composer fallback, or second PHI issuer |
