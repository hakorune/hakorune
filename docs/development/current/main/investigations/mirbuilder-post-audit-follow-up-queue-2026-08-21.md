...
The verification prerequisite is already owned by
`DEV-GATE-LIB-BASELINE-REFRESH-R0` in the D1B manifest.  A current gate
revalidation must reuse that row; this queue deliberately does not create a
second health-repair task.
...
## 2026-09-10 edge-health findings (guard row closed)
...
| `MIR-R7-LEGACY-CENSUS-RECONCILE-D0` | **Reconciled 2026-09-11**; final-pipeline SSOT / inventory terminal | R7 writer/reader/env census and test-only Loop PHI inventory | stale manifest snapshot after selected admission source additions | observation-only manifest records the current 243 Legacy rows/127 files, 5 env routes/4 families, and 4 test-only Loop-PHI files/1009 LOC (252 rows) with includes/excludes and reopen triggers; no R7 deletion claim |
| `MIR-CALL-NORMAL-TYPED-ADMISSION-D0` | `PublishedMirBackendView` / selected object admission | normal Static/Free/Print published callback | LegacyCallV0 mixed-shape acceptance in the selected normal view | selected `MirInstruction::Call(MirCall)` remains CanonicalTyped; the generic compatibility view remains unchanged; any selected module containing both a typed selected call and LegacyCallV0 rejects before body serialization/object creation with a named terminal; legacy-only selected output stays on its existing route until its producer cutover; no new issuer or fallback |
| `MIR-PUBLISHED-VIEW-ADMISSION-SCAN-REUSE-R0` | Medium; `PublishedMirBackendView` selected-admission owner | the existing selected normal object admission path | duplicate module observation/scan performed after view construction; the view's admission disposition and borrowed product are too close | reuse the existing module observation to apply the selected admission policy once; mixed CanonicalTyped + LegacyCallV0 still rejects before serialization/object creation with a named terminal; legacy-only behavior remains on its current compatibility route; no second module traversal, source re-resolution, new receipt, or claim that legacy retirement is complete |
...
## 2026-09-10 callable Loop bridge findings (generic semantic-demand D0 active; follow-up rows queued)
...
held by the physical-demand D0 until its existing-product audit is complete.
That audit is closed as `NoSafeSlice__GenericRecipeLacksPhysicalDemand`; the
bounded semantic-demand successor is now the active design stop. No production
normalizer/consumer cutover is claimed.
...
| `MIR-CALLABLE-LOOP-PHI-GENERIC-PHYSICAL-DEMAND-D0` | High (**closed as `NoSafeSlice__GenericRecipeLacksPhysicalDemand`; successor active**) | [`generic physical-demand D0`](./mir-callable-loop-phi-generic-physical-demand-d0-2026-09-11.md); `CallableGenericLoopV1SemanticRecipeV1` / `PlanBuildOutcome` | audit boundary before any Builder session or raw Loop consumer | existing-product reuse is refuted; keep old Composer quarantined and open the bounded semantic-demand owner below; no partial projection, AST reparse, name lookup, or second PHI issuer |
| `MIR-CALLABLE-LOOP-PHI-GENERIC-SEMANTIC-DEMAND-D0` | High (**accepted design; CallableSingleLoop first cohort selected 2026-09-11**) | [`generic semantic-demand D0`](./mir-callable-loop-phi-generic-semantic-demand-d0-2026-09-11.md); selected callable ingress callback plus the existing CallableSingleLoop issuers | callback-borrowed resolver input/ledger → source-bound CallableSingleLoop syntax/map/Recipe → common operation/effect/After demand | lend the callback context once, consume the existing CallableSingleLoop product once, then reopen session-entry I0; broad GenericLoopV1 remains compatibility-only. No RawInvocationChildPort semantic field, Generic relabeling, old Composer fallback, or second physical issuer |
...
| `MIR-CALLABLE-LOOP-PROFILE-COVERAGE-REAL-OBSERVATION-R0` | **Landed `c3bdb94f76`** | `loop_recipe_physicalizer/callable_lowerer.rs` + `tail_completion.rs` | selected CallableSingleLoop profile close | profile close receives counts derived from the completed dispatch; a changed dispatch count reaches the named coverage reject; no hard-coded production tuple, duplicate counting owner, semantic or runtime change |
...
Decision: **landed at `c3bdb94f76`**. Derive the tuple once from the existing
