...
| `MIR-CALLABLE-LOOP-PHI-SOURCE-INDEX-HEADER-HANDOFF-D0` | **High (design stop reopened 2026-09-11 by production-entry probe)** | [`session-entry I0 card`](./mir-callable-loop-phi-session-entry-i0-2026-09-11.md) | existing resolver/module owner for the exact `VerifiedCallableIndexV1` + `VerifiedCallableHeaderV1` pair at the selected non-AppMain child boundary | identify one existing issuer/owner and scoped caller handoff; preserve `VerifiedCallableFunctionLoweringInputV1::issue` checks and observer-only `from_exact_parts_without_callable`; no name repair, physical-signature substitute, main-only index sharing, second issuer, fallback, fixture, or production switch |
