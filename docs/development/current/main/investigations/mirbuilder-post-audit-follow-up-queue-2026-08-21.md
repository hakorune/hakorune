...
| `MIR-R7-LEGACY-CENSUS-RECONCILE-D0` | **Landed `ba8c88f85f`**; final-pipeline SSOT / inventory terminal | R7 writer/reader/env census and test-only Loop PHI inventory | stale counts and unregistered test-only inventory rows only | observation-only manifest records 239 Legacy rows/127 files, 5 env routes/4 families, and 4 test-only Loop-PHI files/1009 LOC with includes/excludes and reopen triggers; no R7 deletion claim |
...
| `MIR-CALL-CANONICAL-CORRIDOR-GUARD-I0` | **ParkedSealed__NormalLegacyAdmissionMissing**; existing corridor guard | normal typed corridor (future) | guard-only assertions while normal admission still accepts typed LegacyCallV0 | reopen after the normal admission row supplies a named pre-artifact reject and positive/negative evidence; no guard duplication |
