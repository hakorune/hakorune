...
Source authority + canonical issuer: `CanonicalParserSourceHandoffV1::new`
is the sole issuer of `NormalParserSourceLineageV1` and the front-door receipt.
The source-plan request/classifier is only a validator/transport consumer and
must not recreate lineage or re-read source bytes.
...
| `AstOnlyFixture` | explicit AST-only test constructor | mark non-canonical; no lineage validation | test terminal | never enter canonical frontdoor |
| `CompatibilitySource` | parser disposition carries compatibility cohort | preserve typed compatibility reason | compatibility owner/design stop | never become canonical A/NonCandidate |
...
```text
production `NormalParserSourceLineageV1::issue` call sites outside
`parser_source_handoff.rs` = 0 (test/fixture constructors remain `cfg(test)` only)
