...
- `src/stage1/program_json_v0/record_payload.rs`
  - record payloads are renamed to hidden payload boxes `__NyVariantPayload_<Enum>_<Variant>`
- `src/stage1/program_json_v0/lowering.rs`
...
- `src/backend/mir_interpreter/handlers/sum_ops.rs`
  - `handle_variant_make()` always builds `InstanceBox("__NyVariant_<Enum>")`
  - payload is stored through `__variant_payload`
- `src/llvm_py/instructions/sum_runtime.py`
...
- `src/llvm_py/instructions/sum_ops.py`
  ...
  - but `lower_variant_make()` still starts by `lower_newbox(..., runtime_box_name(enum_name), ...)`
...
2. thin-entry inventory for local sum values so LLVM/native can avoid the forced outer `__NyVariant_*` box on non-escaping paths
