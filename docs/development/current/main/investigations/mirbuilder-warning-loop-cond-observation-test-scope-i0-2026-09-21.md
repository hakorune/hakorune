...
Task: MIRBUILDER-WARNING-LOOP-COND-OBSERVATION-TEST-SCOPE-I0
Date: 2026-09-21
Parent: mirbuilder-warning-baseline-refresh-i77-2026-09-21.md
Implementation permission: true for cfg(test) scoping of the five LoopCond observation re-exports only
NextCard: MIRBUILDER-WARNING-BASELINE-REFRESH-I78
---

# MirBuilder LoopCond observation test scope I0

## Six-line brief

```text
Decision: remove the production re-export edge for five caller-zero LoopCond
  observation symbols while retaining existing test facades.
Source authority + canonical issuer: loop_cond_break_continue_observation.rs.
...
Smallest next slice: add cfg(test) to the parent re-export and run LoopCond
  observation/admission tests plus the quick library check.
Non-claims: no LoopCond observation semantic change, no admission change, no
  production route change, no test deletion, and no blanket warning cleanup.
```

## Census and acceptance

The five symbols are defined in `loop_cond_break_continue_observation.rs`.
Production code uses the owner module directly; parent-facade imports are in
`loop_cond_break_continue_observation_tests.rs`, `family_admission_tests.rs`,
