...
— the `local main` literal's `EntryStore(Opaque)` entries
(`"params" => []`, `"locals" => []` — the `"name" => "main"` text entry
is admitted by C9-1) and the
`%{"functions" => [main]}` argument literal's
`OwnershipShare(MapLocal)` + `ArgumentHandoff` — plus
`MirJsonEmitBox.to_json`'s own body admission. The `"blocks" => blocks`
borrowed-handle entry is admitted by C5c-3; the next bounded slice is
`EntryStore(EmptyArray)` (fresh-array residence or owned-empty decision
