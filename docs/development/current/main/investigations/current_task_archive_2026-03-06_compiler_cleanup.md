Status: Historical
Date: 2026-03-06
Scope: Archive extracted from CURRENT_TASK during root-pointer thinning (compiler cleanup lane).
Related:
- CURRENT_TASK.md
...
# CURRENT_TASK Archive (2026-03-06 compiler cleanup)

This file archives the detailed 2026-03-06 compiler-cleanup handoff that was removed from `CURRENT_TASK.md` to keep the root pointer thin.

## Extracted restart handoff and detailed fixed order
...
- this round commits:
  ...
  - `dbd29f159` docs(plan): route-align remaining coreplan and plan-frag wording
    ...
    - verify: `cargo build --release --bin hakorune` PASS、`phase29bq_fast_gate_vm.sh --only bq` PASS、`phase29x-probe` PASS（`unexpected_emit_fail=0 / route_blocker=0`）
  - `9f1c13c69` refactor(plan): route-align residual policy and helper comments
    - `plan/mod.rs` / `route_prep_pipeline.rs` / `policies/*` / `loop_true_counter_extractor.rs` / `condition_env_builder.rs` / `joinir/merge/{entry_params,exit_args_collector}.rs` の comment/doc を route 主語へ同期（挙動不変）
    - legacy enum/module/function 名は据え置き、注釈のみ `legacy label` を追加
    - verify: `cargo build --release --bin hakorune` PASS、`phase29bq_fast_gate_vm.sh --only bq` PASS、`phase29x-probe` PASS（`unexpected_emit_fail=0 / route_blocker=0`）
  ...
  - `6da61129a` refactor(plan): retire single-planner payload extract branch
    ...
    - verify: `cargo build --release --bin hakorune` PASS、`phase29bq_fast_gate_vm.sh --only bq` PASS
  - `97715da9c` refactor(plan): retire nested planner payload fallback path
