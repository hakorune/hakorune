---
Status: historical BoxShape; superseded by the materializer exit-ledger correction
Date: 2026-08-16
...
The old Stage0 cleanup/catch inventory is superseded and is not an authority for
this route. The accepted C-prime language policy has terminal Fault and no
catchable Fault; a future materializer must additionally consume an exact
backend no-unwind capability before any residence effect. This card does not
mint that capability.
...
has a stable exit identity. A noreturn trap is accepted only when an active
backend no-unwind capability is present; it does not require cleanup after the
trap edge and must never fall through to Return.

The missing compile-time witness is deliberately an exit capability, not a
runtime token carrier or a second exit ledger. The predecessor prototype named
`PinnedTextResidenceExitObligationV1` copied site/block/value rows and is not
the successor authority. The corrected candidate is a private, non-`Clone`
`PreparedTextFormalExitFinishSetV1` containing only the function/plan/frame/
residence/session stamp. It is issued beside the existing validated
Completion/DraftSeal exit projection; it cannot be constructed from a block
number, return ordinal, JSON metadata, or the runtime lease token. A later
lifecycle implementation consumes this capability together with the move-only
`TextFormalCallResidenceV1`, while `PreparedFunctionExitSetV1` remains the sole
site/block/value ledger.
...
The predecessor BoxShape is retained as historical evidence for the explicit-
value `Single` and `ExactTwo` vocabulary. `VerifiedFunctionCompletionV1` and
`PreparedFunctionExitSetV1` remain the sole site-keyed exit authorities; the
successor must name an active backend no-unwind capability rather than reuse
the superseded Stage0 prose. No I0 is authorized until the stamp-only finish
capability is designed. It may not store a runtime token, change DraftSeal
Return placement, or add a lifecycle MIR op.
...
NoSafeSlice::PinnedTextExitFinishCapabilityDuplicatesLedger
NoSafeSlice::PinnedTextNoUnwindCapabilityUnissued
