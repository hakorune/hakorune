...
Decision: accepted
Date: 2026-02-28
Scope: runtime cutover Step-1 として V0 ABI slice（4語彙）を docs-first で固定する。
Related:
...
execution-path-zero へ向けた最初の ABI 実装範囲を 4 語彙に固定し、実装拡散を防ぐ。
...
4. `map_size_i64`
   - 入力: Map handle（borrowed）
   - 出力: i64 size（owned primitive）
...
- VM low-level で `map_size_i64` を受ける正本入口は
  `lang/src/runtime/collections/map_core_box.hako` とする。
...
4. `phase29cc_runtime_v0_adapter_fixtures_vm.sh` で
   `string_len` と `map_size_i64` の adapter route 契約（registry + handler + core box）が監査される。
