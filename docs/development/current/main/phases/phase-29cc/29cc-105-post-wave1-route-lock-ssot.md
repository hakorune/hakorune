...
2. `WSM-02a`: assignment/local path unblock（`Copy`/`ReleaseStrong` 最小対応）
3. `WSM-02b`: ExternCall coverage expansion（1 extern familyずつ）
4. `WSM-02c`: BoxCall coverage expansion（1 method familyずつ）
5. `WSM-02d`: runtime parity 影響を `phase-29y` gates で検証
