...
Decision: accepted
Date: 2026-02-27
Scope: WSM-P5-min3（.hako-only roadmap P5）として default route を hako-lane へ切替し、route policy 境界 gate を lock する。
Related:
...
## Decision
1. route 選択は `default -> hako-lane(bridge)` とする。
2. legacy policy 値は `WSM-P5-min10` で hard-remove されるため、default route のみを fixture で lock する。
...
1. `src/runner/modes/wasm.rs`
   ...
   - （歴史注記）`legacy-wasm-rust` は `WSM-P5-min10` で hard-remove 済み。
2. route contract tests
   - `wasm_compile_route_policy_default_maps_to_hako_bridge_contract`
   - （更新）legacy route test は min10 で削除。
