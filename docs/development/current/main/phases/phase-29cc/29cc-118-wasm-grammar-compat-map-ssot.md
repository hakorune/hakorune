...
| external call | `MirInstruction::Call { callee: Extern }` | partial | supported: `env.console.log`, `env.canvas.fillRect`, `env.canvas.fillText` in `src/backend/wasm/codegen/instructions.rs` |
| method call | `MirInstruction::Call { callee: Method }` | partial | supported methods: `toString`, `print`, `equals`, `clone`, `log` in `src/backend/wasm/codegen/builtins.rs` |
...
| local/assignment | `Copy`, `ReleaseStrong` 等が出る経路あり | unsupported (current) | `src/mir/builder/stmts/variable_stmt.rs`, `src/mir/builder/builder_build.rs`, no matching arms in `src/backend/wasm/codegen/instructions.rs` |
...
- `WSM-02a`: assignment/local path unblock（`Copy`/`ReleaseStrong` など頻出命令の最小対応）
