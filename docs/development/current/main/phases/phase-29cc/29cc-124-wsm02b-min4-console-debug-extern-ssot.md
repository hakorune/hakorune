...
Decision: accepted
Date: 2026-02-26
Scope: wasm lane `WSM-02b-min4` として `env.console.debug` の ExternCall coverage を最小追加し、fail-fast 境界を維持する。
Related:
...
# 29cc-124 WSM-02b-min4 Console Debug Extern SSOT

## 0. Goal

`WSM-02b` の最小4件目として、`env.console.debug` を WASM backend で受理する。
...
In scope:
1. `src/backend/wasm/extern_contract.rs` に `env.console.debug` を追加
...
1. `env.console.debug` は `(ptr, len)` の2引数契約で `console_debug` import に接続する
2. unsupported extern は従来通り `Unsupported extern call: ...` で fail-fast
3. `console.debug` JS bridge を通じて browser runtime で可視化可能にする
...
- `WSM-02b-min4` は完了。
- wasm lane active next は `WSM-02c-min1`（BoxCall coverage expansion）とする。
