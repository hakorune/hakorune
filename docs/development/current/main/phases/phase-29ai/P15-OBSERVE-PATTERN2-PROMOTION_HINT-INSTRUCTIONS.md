# Phase 29ai P15: Observe loop_break promotion hint (strict/dev)

Date: 2025-12-29
Status: Historical reference (implemented)
Scope: loop_break body-local promotion hint を strict/dev 限定で観測可能にする（仕様不変）
Goal: promotion hint が付与されている事実を stable tag で固定し、次の Plan/Frag 吸収フェーズへ進む前提を揃える

Historical note:
- P15 の original text は `RuleKind::Pattern2` や `Pattern2LoopBodyLocalFacts` を参照していたけど、
  current runtime では `PlanBuildOutcome` + `facts.facts.loop_break_body_local()` + strict-only tag で観測するよ。
- fixture file 名や archive basename は legacy pin token として inventory SSOT に残しているよ。

## Objective

- loop_break body-local facts が取れるときだけ、strict/dev 限定で安定タグを出して観測できるようにする
...
実装位置:
- `try_build_outcome()` の planner outcome を受け取った直後

補足:
- planner outcome は `build_plan_with_facts_ctx(...)` の結果を使い、facts 直抽出はしない

ガード条件（全部満たすときだけ出す）:
- `crate::config::env::joinir_dev::strict_enabled()` が true
- planner outcome から `facts.facts.loop_break_body_local()` が取れる

出力するタグ（stderr 推奨、1行固定）:
- TrimSeg: `[plan/loop_break/promotion_hint:TrimSeg]`
- DigitPos: `[plan/loop_break/promotion_hint:DigitPos]`
...
- historical archive stem は retirement SSOT を参照
...
期待:
- seg_min: `[plan/loop_break/promotion_hint:TrimSeg]`
- digit_pos_min: `[plan/loop_break/promotion_hint:DigitPos]`
