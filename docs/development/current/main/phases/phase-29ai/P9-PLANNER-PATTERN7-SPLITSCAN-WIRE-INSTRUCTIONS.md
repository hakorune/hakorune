# Phase 29ai P9: Planner support + wiring（SplitScan subset / historical label 7）
...
- `facts/loop_facts.rs`, `planner/build.rs`, `plan/extractors/pattern7_split_scan.rs`,
  `legacy_rules::pattern7::extract` は P9 実行時の historical path/token だよ。
...
- Current strict-shadow smoke: `tools/smokes/v2/profiles/integration/joinir/split_scan_strict_shadow_vm.sh`
...
- execution-time historical extractor token `pattern7_split_scan` と齟齬が出そうなら、まずはその extractor が要求する
...
- `Ok(Some(plan))` なら採用。
- `Ok(None)` なら execution-time fallback route にフォールバック。
...
- **観測差分**: planner `Ok(None)` でログを出さない。採用時も既存の route log を維持する。
