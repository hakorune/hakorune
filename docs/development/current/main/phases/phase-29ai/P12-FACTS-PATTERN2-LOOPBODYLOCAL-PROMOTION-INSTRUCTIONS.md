...
Historical note:
- historical type/path/pin token 群は inventory SSOT に退避しているよ。
...
- `apps/tests/loop_break_body_local_min.hako`（A-4 DigitPos）
- `apps/tests/loop_break_body_local_seg_min.hako`（A-3 Trim / seg）

exact historical fixture token は
