# Phase 29ai P12: Facts SSOT（LoopBreak body-local promotion / historical label 2）
...
- `Pattern2LoopBodyLocalFacts`, `pattern2_loopbodylocal_facts.rs`,
  `phase29ab_pattern2_loopbodylocal_*` は P12 実行時の historical type/path/pin token だよ。
...
- `apps/tests/loop_break_body_local_min.hako`（historical fixture pin token: `phase29ab_pattern2_loopbodylocal_min.hako` / A-4 DigitPos）
- `apps/tests/loop_break_body_local_seg_min.hako`（historical fixture pin token: `phase29ab_pattern2_loopbodylocal_seg_min.hako` / A-3 Trim / seg）
