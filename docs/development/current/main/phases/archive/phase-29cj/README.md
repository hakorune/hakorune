...
- reopening the archived `registry_optin_method_arraymap_direct_canary_vm.sh` tombstone as helper-retirement debt
- widening `.hako` live/bootstrap caller contracts
- mixing authority migration back into `phase-29ch`

## Current Read

- phase status: `formal-close-synced`
...
1. keep the strict source-authority and surrogate buckets frozen
   - `src/stage1/program_json_v0/authority.rs`
   - `crates/nyash_kernel/src/plugin/module_string_dispatch/build_surrogate.rs`
   - these are exact owners, but no longer active phase fronts
2. keep bridge and `.hako` helper waves closed
   - `program_json/` and `program_json_entry/` stay near thin floor
   - `.hako` owner/helper local cleanup stays `close-synced`
3. keep the archived direct-lower probe as monitor evidence only; it does not block close sync
4. do not confuse this phase close sync with the primary pure-`.hako` blocker
   ...
   - the latest exact `.hako` cuts now cover `MirBuilderBox` unsupported tail, raw-subcmd input cleanup, and checked payload/I/O cleanup above the same shared stop-line
...
- `.hako` owner/helper local thinning wave: `close-synced`
- `phase-29cj` overall: `formal-close-synced`
...
- frozen near-thin-floor `.hako` owners after the last reinventory:
  1. `lang/src/mir/builder/MirBuilderBox.hako`
  2. `lang/src/runner/stage1_cli_env.hako`
  3. `lang/src/runner/stage1_cli.hako`
  4. `lang/src/runner/launcher.hako`
...
- `tools/smokes/v2/profiles/archive/core/phase2160/registry_optin_method_arraymap_direct_canary_vm.sh` is no longer cleanup debt; it is archived tombstone evidence and should stay outside the shared-helper accounting
...
- the active leaf on that stop-line is no longer bridge-local; it is the source-route authority / output projection split above `module_to_mir_json(...)`, where strict source `Program(JSON)` emission and `(Program(JSON), MIR(JSON))` projection no longer live on the same owner
...
- the latest compat-fallback leaf also keeps defs-name collect on `Stage1UserBoxDecls::{insert_compat_def_box_names,compat_def_box_name}(...)`, so the fallback cluster no longer depends on the free `insert_stage1_def_box_names(...)` helper
- the latest explicit-payload leaf also keeps explicit decl payload extraction on `Stage1UserBoxDecls::{explicit_decl_values,collect_explicit_decls}(...)`, so `explicit_from_program_value(...)` is down to a thin owner-local handoff
- the latest decl-projection leaf also keeps metadata/test output projection on `Stage1UserBoxDecls::{into_metadata_entries,into_decl_json_values}(...)`, so decl resolution is now separated from output materialization in the last remaining high-value cluster
- the latest explicit-entry leaf now keeps Program(JSON) module parse behind `Stage1ProgramJsonModuleHandoff::parse_module(...)`, while compat defs iteration stays behind `Stage1UserBoxDecls::{insert_compat_def_box_names,compat_def_box_name}(...)`; the active front stays in `src/host_providers/mir_builder.rs`
- the latest bridge-local bucket slice also keeps source-path/source-text payload build behind `ProgramJsonOutput::{from_source_path,from_source_text}(...)` in `src/runner/stage1_bridge/program_json/mod.rs`, so the future-retire bridge root no longer mixes read and payload-emission leaves inline
- the latest bridge-local bucket slice also keeps writeback orchestration on `ProgramJsonOutput::emit_to_path(...)`, so `emit_program_json_v0(...)` is now just a thin `from_source_path -> emit_to_path` route
- the latest stub-emit bucket slice also keeps optional MIR dump and file-or-stdout output routing behind `print_dumped_mir_if_requested(...)`, `emit_mir_json_output(...)`, and `emit_program_json_output(...)` in `src/runner/stage1_bridge/stub_emit/writeback.rs`, so `write(...)` is closer to pure payload dispatch
- the latest stub-emit bucket slice also keeps timeout/nonzero/parse-write orchestration behind `timeout_exit_code(...)`, `failed_exit_code(...)`, and `parse_and_write_payload(...)` in `src/runner/stage1_bridge/stub_emit.rs`, so `handle_child_output(...)` is down to a thin route-local decision chain
- the latest stub-emit bucket slice also keeps line extraction and parse-error formatting behind `first_*_json_line(...)` / `parse_*_json_line(...)` in `src/runner/stage1_bridge/stub_emit/parse.rs`, and the new unit proofs fix the exact missing/parse error prefixes for stub emit payload parsing
...
- the latest explicit/source handoff bundle now keeps shared route detail behind `Stage1ProgramJsonModuleHandoff`, `SourceProgramJsonAuthority`, `SourceProgramJsonOutputHandoff`, and `with_phase0_mir_json_env(...)`, so both live entries read as owner-local handoff -> `module_to_mir_json(...)`
...
- the latest explicit-route shaping cut now keeps Program(JSON) `user_box_decls` parse-entry and explicit-vs-compat precedence behind typed owner-local decl helpers, so the module handoff reads as module parse + same-owner decl handoff while compat fallback stays inside `Stage1UserBoxDecls`
...
- the latest explicit-route finalize cut also retires that metadata-map seam by moving it behind `Stage1UserBoxDecls::into_metadata_map()`, so the remaining active leaf is now the module-entry seam at `Stage1ProgramJsonModuleHandoff::parse(...)` -> `Stage1ProgramJsonModuleHandoff::parse_module(...)`
...
- the latest explicit-route entry cut now also folds the free module-entry wrapper into `Stage1ProgramJsonModuleHandoff::parse(...)`, and the source-entry wrapper now lives on `SourceProgramJsonAuthority::for_source(...)`; the remaining active leaf is the source-route authority / output projection split above the same Rust stop-line
...
- the latest explicit-route finalize split now keeps metadata splice and plain/guarded MIR emission behind `Stage1FinalizedMirModule`, so `Stage1ProgramJsonModuleHandoff` is down to typed `MirModule + user_box_decls` aggregation above the shared Rust stop-line
...
- the nearby future-retire bridge error-prefix leaf now stays directly in `src/stage1/program_json_v0.rs`, so `src/stage1/program_json_v0/authority.rs` no longer mixes bridge-specific error wrapping with strict source authority and no standalone `bridge_shim.rs` keep remains
...
- the next landed tightening on that same owner keeps reduced-artifact mode/env resolution behind `Stage1ModeContractBox.resolve_mode(...)`, so `Main.main()` is down to pure mode dispatch over the exact `stage1-env-program` / `stage1-env-mir-source` contract
...
- the next landed tightening on that same owner keeps the remaining non-empty Program(JSON) text guard behind `Stage1ProgramJsonTextGuardBox.{coerce_text_checked,input_present}(...)`, so `Stage1ProgramJsonMirCallerBox` and `Stage1ProgramJsonCompatBox` no longer duplicate same-file presence checks before checked handoff
- the next landed tightening on the same owner now moves MIR text materialization, structural payload predicates, and the final print/fail tail into same-file `Stage1MirPayloadContractBox`, so `Stage1MirResultValidationBox` is down to selected-input debug -> materialize/debug -> checked payload handoff
- the next landed tightening on the same owner also keeps compat tiny leaves behind `_has_explicit_program_json_text(...)` and `_fail_mixed_source_mode(...)`, so `Stage1ProgramJsonCompatBox` stays closer to a pure checked handoff owner
...
- the next tightening on that same owner now keeps the source-entry compat seam behind owner-local `MirBuilderSourceCompatBox`, so `emit_from_source_v0(...)` is now a thin shim without touching `emit_from_program_json_v0(...)` policy itself
- the next tightening on that same owner now moves the generic unsupported/no-match tail into `lang/src/mir/builder/internal/unsupported_tail_box.hako::BuilderUnsupportedTailBox.fail(...)`, so `_emit_internal_program_json(...)` now shows only loop-force / registry / fallback / fail-fast route order
...
- the next landed tightening on `lang/src/runner/stage1_cli.hako` moves source/program-json input resolve into same-file `Stage1CliProgramJsonInputBox`, and `Stage1CliConfigBox.from_env()` remains the live env contract owner for canonical `mode/backend/source_path/source_text/program_json_path`, so `stage1_main(...)` no longer re-reads env inline and `Stage1Cli` no longer keeps placeholder resolve / file readback / path-source shaping inline
...
- the next landed tightening on that same owner also keeps shared Program(JSON) marker predicates behind `_program_json_text_present(...)` and `_program_json_has_markers(...)`, so inline payload detection, emit-program output validation, and emit-mir input validation no longer duplicate the same marker test
- the next landed tightening on that same owner also keeps the checked emit+output tail behind `_emit_validated_mir_from_program_json_text(...)` and `_mir_output_has_functions(...)`, so `_emit_mir_json_from_program_json_checked(...)` now reads as Program(JSON) coercion -> checked emit handoff only
...
- the next landed tightening on `lang/src/runner/launcher.hako` also keeps outer argv/bootstrap parse+validate behind same-file `LauncherInputContractBox`, so `cmd_build_exe(...)`, `cmd_emit_program_json(...)`, `cmd_emit_mir_json(...)`, and `run_bootstrap_from_env()` now read as request parse -> checked execute only
...
- the next landed tightening on that same owner also keeps shared Program(JSON) marker predicates behind `_program_json_text_present(...)` and `_program_json_has_markers(...)`, so launcher emit-program output validation and emit-mir input validation no longer duplicate the same marker test
- the next landed tightening on that same owner now moves artifact file I/O and stdout/file output routing into same-file `LauncherArtifactIoBox`, so the launcher lane no longer keeps readback/writeback policy inline around checked emit/build paths
- the next landed tightening on `lang/src/runner/launcher.hako` also keeps program-json path load and stdout/file output tails behind same-file `LauncherArtifactIoBox`, so the launcher lane no longer mixes readback/output side effects inline
- the next landed tightening on that same owner also keeps build-exe compile/link fail-fast tails behind `_compile_object_from_mir_path_checked(...)` and `_link_exe_object_checked(...)`, so `_emit_exe_from_mir_json_checked(...)` now reads as path resolve/write -> compile -> link only
...
- the next landed tightening on that same owner also keeps prepared-scan-src result/error/resolve handoff behind `_new_prepare_scan_src_result(...)`, `_fail_prepare_scan_src(...)`, `_apply_prepare_scan_src_result(...)`, and `_resolve_prepare_scan_src_if_needed(...)`, so `_prepare_scan_src(...)` no longer mixes result-map allocation, error tagging, resolved-scan-src apply, and bundle-resolve branching with bundle/env orchestration
- the next landed tightening on that same owner also keeps bundle ctx container setup behind `_ensure_bundle_alias_arrays(...)` and `_ensure_require_mods_array(...)`, so `_collect_bundle_inputs(...)` and `_apply_env_bundle_inputs(...)` no longer duplicate alias/require array allocation
- the next landed tightening on that same owner also keeps bundle opts validation error handoff behind `_fail_bundle_ctx(...)`, so `_collect_bundle_inputs(...)` no longer repeats `set("error") -> return bundle_ctx` across bad-bundle, mismatch, duplicate, and bad-require exits
- the next landed tightening on that same owner also keeps named-bundle pair validation and assign behind `_collect_named_bundle_inputs(...)`, so `_collect_bundle_inputs(...)` no longer carries the whole `bundle_names/bundle_srcs` mismatch/duplicate loop inline
- the next landed tightening on that same owner also keeps plain `bundles` and `require_mods` validation/assign behind `_collect_plain_bundle_inputs(...)` and `_collect_require_mod_inputs(...)`, so `_collect_bundle_inputs(...)` is closer to a field-by-field handoff table
- the next landed tightening on that same owner also keeps main-body parse-src narrowing behind `_main_body_parse_src_if_present(...)`, so `_resolve_parse_src(...)` now reads as main-body handoff -> fallback-to-scan-src only
...
- the next landed tightening on that same owner also keeps the imports inject tail behind `_inject_imports_fragment_if_present(...)`, so `_inject_imports_json(...)` now mirrors the defs path as imports build -> inject-if-present only
...
- accepted proof for that launcher slice stays on `tools/hakorune_emit_mir_mainline.sh lang/src/runner/launcher.hako ...`; `launcher.hako` is now `strict-safe` on the authority source route, so the old adjacent strict source-route reject (`dev-local-alias-sugar`) is no longer the expected outcome for that file
- the next landed tightening on the same owner keeps route sequencing behind `BuilderFuncDefsGateBox.lower_if_enabled(...)`, `_emit_internal_program_json(...)`, and `_emit_delegate_program_json(...)`, so the outer box no longer repeats raw env/hostbridge branching inline
...
- the next landed tightening on the same owner also moves the full delegate-side finalize into `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.finalize_mir(...)`, so `_emit_delegate_program_json(...)` now reads as internal gate/provider -> internal finalize -> shared normalize
- the next landed tightening on the same owner also moves the shared outer finalize order and its stable fail tags into `lang/src/mir/builder/internal/finalize_chain_box.hako::BuilderFinalizeChainBox.{apply,log_fail}(...)`, so route leaves no longer carry inject/methodize/normalize/fail-tag logic inline
...
- the latest runner tightening now also keeps `Stage1ProgramJsonCompatBox` on explicit Program(JSON) checked handoff through `_emit_mir_from_text_checked(...)`, while `Stage1MirResultValidationBox` now delegates payload materialize/validate/print to `Stage1MirPayloadContractBox` after the existing debug hooks
...
- `tools/smokes/v2/lib/test_runner.sh` now also keeps the explicit registry arraymap direct lower probe behind `run_direct_lower_box_canary()`, `run_direct_lower_box_vm_to_stdout_file()`, and `direct_lower_box_runner_code()`, so the archived monitor script `tools/smokes/v2/profiles/archive/core/phase2160/registry_optin_method_arraymap_direct_canary_vm.sh` remains archived tombstone evidence and the helper-local shell wave is now near thin floor
