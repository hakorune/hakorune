...
### ✅ Legacy label-5 early-exit lane 削除（488行）

**削除理由**: Plan line 完全運用化により、legacy label-5 early-exit lane は完全にデッドコード化

**削除ファイル**:
- historical path token: old label-5 early-exit file under the retired `joinir/patterns/` lane (488行)

**関連削除**:
- `router.rs` の `LOOP_PATTERNS` テーブルから legacy label-5 entry 削除
- `mod.rs` から old label-5 early-exit file 宣言削除
- `router.rs` のドキュメント更新（historical numbered priority を label-5 なしへ同期）

**影響範囲**:
- legacy label-5 early-exit lane は Plan line 経由で処理されるため、機能退行なし
- `LOOP_PATTERNS` テーブルの historical 優先順位は `LoopTrueEarlyExit -> LoopContinueOnly -> IfPhiJoin -> LoopSimpleWhile -> LoopBreak` から `LoopContinueOnly -> BoolPredicateScan -> AccumConstLoop -> IfPhiJoin -> LoopSimpleWhile -> LoopBreak` に変更
...
| 項目 | 削減数 |
|------|--------|
| ソースファイル | 1ファイル（old label-5 early-exit file） |
| 総削減行数 | 488行 |
| router.rs エントリ | 5行（legacy label-5 entry） |
| mod.rs 宣言 | 1行 |

### Historical Route Priority の変更

**削除前**（Phase 131-11+）:
```
LoopTrueEarlyExit (most specific historical lane) → LoopContinueOnly → IfPhiJoin → LoopSimpleWhile → LoopBreak
```

**削除後**（Phase 286+）:
```
LoopContinueOnly → BoolPredicateScan → AccumConstLoop → IfPhiJoin → LoopSimpleWhile → LoopBreak
```

**注**: legacy numbered labels 5/6/7 は Plan line 経由で処理（`PLAN_EXTRACTORS` テーブル）
...
#### 1. label-5 early-exit historical normalizer logic 分離（430行）
- historical file token `normalizer/label5.rs` 相当の独立モジュール - label-5 early-exit lane 専用正規化
- label-5 early-exit lane の複雑な構造展開ロジックを独立モジュール化
...
**期待効果**:
- メンテナンス性向上
- テスト分離（label-5 early-exit 正規化のみをユニットテスト）
- 責任分離（SRP原則）

### (legacy) LOOP_PATTERNS テーブル完全削除

**背景**: 全 historical numbered route labels が Plan line 経由になれば、`LOOP_PATTERNS` テーブルは不要

**前提条件**:
- historical numbered labels 1-4 の Plan line 移行完了（Phase 286 時点で完了）
- historical numbered labels 8-9 の Plan line 移行完了（Phase 286 時点で完了）
...
1. **Phase 286（完了）**: legacy label-5 early-exit lane 削除 + Warning クリーンアップ
...
- 機会を待つ：P1 は historical numbered labels 1-4 完全移行のタイミングで実施
...
- [Route移行ログ](../phase-273/README.md) - historical numbered labels 6/7 の Plan line移行（Phase 273）

## 検証項目

### ✅ Phase 286 完了検証

- [x] label-5 early-exit file 削除（488行）
- [x] router.rs の `LOOP_PATTERNS` から legacy label-5 entry 削除
- [x] mod.rs から old label-5 early-exit file 宣言削除
...
- [ ] normalizer.rs 分割完了（P0）
  - [ ] historical file token `pattern5.rs` 独立（430行）
  - [ ] helpers.rs 共通化（700行）
  - [ ] normalizer.rs 縮小（300行）
- [ ] LOOP_PATTERNS テーブル削除（P1）
  - [ ] 全 historical numbered route labels の Plan line経由確認
