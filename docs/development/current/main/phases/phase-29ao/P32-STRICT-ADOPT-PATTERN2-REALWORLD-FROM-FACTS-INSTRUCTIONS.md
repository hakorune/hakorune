...
# Phase 29ao P32: LoopBreak real-world（phase263 seg / historical fixture token / label 2）を planner subset に引き上げ、strict/dev で Facts→CorePlan を踏ませて SSOT 化する

Date: 2025-12-30  
Status: Ready for execution  
Goal: current semantic fixture alias `apps/tests/loop_break_realworld_min.hako` が JoinIR 回帰 SSOT に入っている以上、strict/dev では **必ず Facts→CorePlan shadow adopt（=tag）を踏む**状態にする。
...
現状、LoopBreak の strict/dev shadow adopt は “planner 由来の subset” のみが対象で、`loop_break_realworld*` current semantic lane に対応する historical fixture family は shadow adopt を踏まない（=タグで検知できない）。
...
- strict/dev の real-world smoke で、`loop_break_realworld_vm.sh` が **shadow adopt historical debug tag** を必須で満たす
  - 流用タグ: `[coreplan/shadow_adopt:pattern2_break_subset]`
...
狙い:
- `loop_break_realworld_min.hako` を “subset” として planner で一意に選べるようにする（曖昧さを `Freeze` ではなく `Ok(None)` に倒す）。
...
### Step 2: planner を拡張して `loop_break_realworld*` current semantic lane を “planner 由来の LoopBreak” に引き上げる
...
方針:
- composer 側は既存の `DomainPlan::LoopBreak` shadow adopt 経路を使い、historical debug tag `[coreplan/shadow_adopt:pattern2_break_subset]` を流用する。
...
これにより、回帰パックの `loop_break_realworld_vm` coverage lane と、その historical replay lane の両方で **自動でゲートされる**。
