...
現状、LoopBreak の strict/dev shadow adopt は “planner 由来の subset” のみが対象で、`loop_break_realworld*` current semantic lane に対応する historical fixture family `phase263_pattern2_*` は shadow adopt を踏まない（=タグで検知できない）。
...
- historical replay basename:
  - current wrapper: `loop_break_realworld_vm.sh`
...
これにより、回帰パックの `loop_break_realworld_vm` coverage lane と、その historical replay token `phase263_pattern2_` に紐づく evidence lane の両方で **自動でゲートされる**。
