---
Status: Ready
Scope: Stage-2 pilot（release既定でも CorePlan を採用する）を LoopSimpleWhile subset のみで開始する（仕様不変）
Related:
...
# Phase 29ao P36: Stage-2 pilot — release adopt LoopSimpleWhile CorePlan skeleton (subset)

## 目的

- これまでの Stage-1（strict/dev の shadow adopt）で “CorePlan が正しい” を検出できる導線は整った。
- P36 は Stage-2 の最初の一歩として、**release既定でも** LoopSimpleWhile subset（historical label 1）を `Facts → CorePlan(skeleton) → emit` に寄せる。
- ただし観測（タグ）・既定挙動（意味論/エラー文字列/恒常ログ）は不変を守る。

## 非目的

- 他の route family の release flip（P37+）
- 新しい env var の追加
- by-name の分岐追加
...
- LoopSimpleWhile subset の安全ゲート（body=step-only）は既に固定済み:
  - `src/mir/builder/control_flow/plan/policies/loop_simple_while_subset_policy.rs`
...
- 追加する関数（例）:
  - `try_release_adopt_core_plan_for_loop_simple_while(...) -> Result<Option<CorePlan>, String>`
- 条件:
  - historical plan variant token `DomainPlan::Pattern1SimpleWhile(_)` に対応する LoopSimpleWhile lane であること
  - `outcome.facts` が存在し、facts 側も `loop_simple_while` を持つこと
...
### Step 2: router で LoopSimpleWhile のみ release adopt を優先する
...
- 形:
  ...
  - release: `try_release_adopt_core_plan_for_loop_simple_while()` を試す（タグ出力なし）
  - どちらも `PlanVerifier::verify(&core_plan)` → `PlanLowerer::lower(...)`
  - 失敗時は既存どおり `lower_via_plan(...)` にフォールバック

Fail-Fast 方針:
- strict/dev: LoopSimpleWhile が選ばれたのに facts が欠ける / mismatch は `Err(...)` で落とす（silent fallback禁止）
