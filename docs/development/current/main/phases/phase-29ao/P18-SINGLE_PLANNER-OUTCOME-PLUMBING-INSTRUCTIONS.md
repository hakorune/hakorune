...
Scope: 仕様不変。P17 で router に入った “LoopSimpleWhile strict/dev shadow adopt” が planner を 2 回呼ぶのを、構造で解消する。
...
- LoopSimpleWhile shadow adopt の “strict/dev only” 条件を変える（今回は構造だけ）
- 新 env var 追加、恒常ログ追加
...
`route_loop()` が
1) `single_planner::try_build_outcome(ctx)` の内部で planner を実行し
2) strict/dev + LoopSimpleWhile のときに router で `planner::build_plan_with_facts_ctx(..)` をもう一度実行して facts を取り直す
という二重実行になっている。
...
### API整理（current live surface）

- `single_planner` の current live surface は
  - `try_build_outcome(ctx) -> Result<PlanBuildOutcome, String>`
- router は `outcome.plan` と `outcome.facts` を同じ outcome から読む

### router は新APIを使う

- router は `PlanBuildOutcome` を受け取り、P17 の strict/dev LoopSimpleWhile adopt で **outcome.facts** を使う（planner を呼び直さない）。
...
やること:
- `rules.rs` の current entrypoint `pub(super) fn try_build_outcome(ctx) -> Result<planner::outcome::PlanBuildOutcome, String>` を使う
  - outcome の生成は既存と同じ（PlannerContext + planner::build_plan_with_facts_ctx）
  - promotion_hint の出力位置・条件は現状維持
  - ルール順序と routeログも現状維持
- `mod.rs` で current live surface `pub(in crate::mir::builder) fn try_build_outcome(ctx) -> Result<PlanBuildOutcome, String>` を使う
...
やること:
- `single_planner::try_build_outcome(ctx)` を使う
- strict/dev + LoopSimpleWhile の adopt では `outcome.facts` を使う
