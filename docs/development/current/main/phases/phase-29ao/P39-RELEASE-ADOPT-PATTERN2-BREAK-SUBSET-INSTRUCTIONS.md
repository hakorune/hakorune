...
# Phase 29ao P39: Stage-2 — release adopt LoopBreak subset (planner-derived only)
...
- LoopBreak の NotApplicable / Freeze 系を release adopt する（subset外、従来経路維持）
- 新しい env var 追加
...
### Step 1: composer に LoopBreak release adopt 入口を追加
...
追加する関数（例）:
- `try_release_adopt_core_plan_for_loop_break_subset(...) -> Result<Option<CorePlan>, String>`

採用条件（安全ゲート）:
- `domain_plan` が `DomainPlan::LoopBreak(_)` のときだけ
- `outcome.plan` が `Some(DomainPlan::LoopBreak(_))` のときだけ（planner 由来のみ採用）
- `outcome.facts` が存在し、`facts.facts.loop_break` が `Some` のときだけ

合成:
- 既存の `compose_coreplan_for_loop_break_subset(builder, facts, ctx)` を再利用

失敗時（release既定）:
- `Ok(None)` または `Err(_)` はすべて `Ok(None)` に丸めて、従来経路へフォールバック（仕様不変）

### Step 2: router の非strict経路で LoopBreak release adopt を接続
...
狙い:
- 既存の LoopBreak subset smoke は strict/dev タグ検証に寄っており、release adopt 経路が踏まれない可能性がある。
- “strictを付けない” integration smoke を 1 本追加して、非strict 実行でも安定することを固定する。

追加（例）:
- `tools/smokes/v2/profiles/integration/joinir/loop_break_release_adopt_vm.sh`
...
- `git add -A`
- `git commit -m "phase29ao(p39): release adopt loop-break subset"`
