...
- P17/P23/P24/P25/P26/P27 で複数 route family に strict/dev shadow adopt を入れた。
...
推奨タグ:
- ScanWithInit historical debug tag: `[coreplan/shadow_adopt:pattern6_scan_with_init]`
- SplitScan historical debug tag: `[coreplan/shadow_adopt:pattern7_split_scan]`

（必要なら LoopSimpleWhile / IfPhiJoin / LoopTrueEarlyExit / LoopBreak subset も後で追加できるが、P28 では “P24/P27 を確実に踏む” 目的で 6/7 を優先）
...
既存の historical fixture family `phase29ab_pattern6_` / `phase29ab_pattern7_` は複数 fixture を含み、contract/freezes も混ざるため、
“tag が出る fixture” を 1 本に固定する smoke を新設する。

追加ファイル:
- `tools/smokes/v2/profiles/integration/joinir/scan_with_init_strict_shadow_vm.sh`
  - fixture: `apps/tests/scan_with_init_ok_min.hako`
...
- `tools/smokes/v2/profiles/integration/joinir/split_scan_strict_shadow_vm.sh`
  - fixture: `apps/tests/split_scan_ok_min.hako`
  - strict: `HAKO_JOINIR_STRICT=1`
  - 期待: exit=3（既存 smoke と同じ）かつ tag を含む

回帰パックに追加:
- `tools/smokes/v2/profiles/integration/joinir/phase29ae_regression_pack_vm.sh`
  - `run_filter "scan_with_init_strict_shadow_vm" "scan_with_init_strict_shadow_vm"`
  - `run_filter "split_scan_strict_shadow_vm" "split_scan_strict_shadow_vm"`
...
- `git add -A`
- `git commit -m "phase29ao(p28): add shadow adopt tags + gate smokes for scan/split"`
