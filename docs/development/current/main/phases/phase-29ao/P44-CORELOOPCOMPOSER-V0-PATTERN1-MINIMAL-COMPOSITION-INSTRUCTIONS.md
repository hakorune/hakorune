...
# Phase 29ao P44: CoreLoopComposer v0 — minimal composition (LoopSimpleWhile skeleton only; historical label 1)

## 目的

P43 で作った `CoreLoopComposer v0` は未接続・常に `Ok(None)` の足場になっている。
P44 では **最小の実合成**として LoopSimpleWhile（historical label 1, SimpleWhile subset）の “Loop skeleton” のみを v0 で組み立てられるようにし、
...
- scan_with_init / split_scan / loop_break / if_phi_join / loop_true_early_exit の v0 合成（P45+）
- exitmap/cleanup/value-join の本格合成（v0 は skeleton のみ）
- 新しい env var 追加

## 実装

### Step 1: `try_compose_core_loop_v0` を LoopSimpleWhile skeleton だけ `Some(CorePlan::Loop)` にする
...
- `facts.facts.loop_simple_while.is_some()`（既存 subset の SSOT を利用）
...
- 既存の helper を再利用して “core loop skeleton” を組み立てる:
  - `src/mir/builder/control_flow/plan/normalizer/simple_while_coreloop_builder.rs`（`build_simple_while_coreloop`）
...
### Step 3: LoopSimpleWhile の shadow/release adopt 経路を v0 に統一（内部配線のみ）

対象（例）:
- `src/mir/builder/control_flow/plan/composer/shadow_adopt.rs`

方針:
- LoopSimpleWhile の Facts→CorePlan(skeleton) 合成は `coreloop_v0` を使う（重複実装を増やさない）。
...
追加/更新:
- `coreloop_v0.rs`:
  - LoopSimpleWhile facts あり + exitmap/cleanup empty → `Some(CorePlan::Loop(..))`
...
- `git add -A`
- `git commit -m "phase29ao(p44): coreloop composer v0 composes loop simple while skeleton"`
