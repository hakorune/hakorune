...
2. legacy label 2 専用だったケースが後続経路に流れて挙動変化
...
- 既存の LoopBreak（historical label 2）構造（BodyInit → Break）と `LoopBodyLocalEnv` に収まる。

## Derived slot contract (minimal)

- 対象は **LoopBreak（historical label 2）break 条件で参照される LoopBodyLocal 1 変数**。
