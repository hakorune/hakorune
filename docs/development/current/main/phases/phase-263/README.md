...
1. **Historical route ordering at the time** (router.rs):
   - loop_simple_while は loop_break route の**前に**試される（legacy numbered labels: Pattern1 → Pattern2）
...
#### File 2: historical `pattern2_lowering_orchestrator.rs` - Ok(None) を検出して早期終了
...
- **Before**: `[cf_loop/pattern2] Variable not found: seg` (legacy Pattern2 processing continued)
...
1. historical path token: `src/mir/builder/control_flow/joinir/patterns/pattern2_steps/promote_step_box.rs`
