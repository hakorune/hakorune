...
Historical route token: `pattern2` (numbered label: `2`)

Reading note:
- 下の debug tag / path token に出る `pattern2` は original phase log を示す historical token だよ。
- token-A / token-B という呼び方は、この README 内だけの short-hand だよ。
  - token-A = old label-2 step-lane promotion file
  - token-B = old label-2 lowering orchestrator file
...
- Router が後続 route（historical compat tokens: `legacy binding` / `normalized shadow`）に進む
...
1. **Historical route ordering at the time** (router.rs):
   - loop_simple_while は loop_break route の**前に**試される（current route family order; historical numbered labels were 1 → 2）
   - loop_break route が失敗しても loop_simple_while には fallback **しない**（loop_simple_while は既に試された後）
   - loop_break route の `Ok(None)` は後続 route（historical compat tokens: `legacy binding` / `normalized shadow`）に進む
...
#### token-A: old label-2 step-lane promotion file - 戻り値を Option 化

**修正箇所**: historical path token `promote_step_box.rs`
historical root lane at the time: same old step lane under the retired JoinIR route-entry predecessor
...
#### token-B: old label-2 lowering orchestrator - `Ok(None)` を検出して早期終了

**修正箇所**:
- exact historical basename kept in archived evidence lane
- same historical root lane as token-A above
...
- `Ok(None)` を返すことで、router.rs が次の処理（historical compat tokens: `legacy binding` / `normalized shadow`）に進む
...
- promote_step_box.rs: 戻り値を Result<Option<_>, String> に変更
  ...
  - token-B: Ok(None) 検出で早期 return
  - minimal seg evidence fixture: test simplification

後続 route（historical compat tokens: `legacy binding` / `normalized shadow`）へ fallback させる（detection→extract→lower SSOT 維持）
...
#### Minimal Repro Test
```bash
./target/release/hakorune --backend vm --verify <same minimal seg evidence fixture as above>
```
- **Result**: Still failing but with **DIFFERENT error** (progress!)
- **Before**: `[cf_loop/pattern2] Variable not found: seg` (legacy label-2 lane kept running)
...
### Core Implementation
1. same historical file token A as implementation section above (`promote_step_box.rs` under the same old step lane)
...
2. same historical file token B as implementation section above
   - Early return on `Ok(None)` detection

### Evidence Fixture Pair
1. representative legacy fixture pin token for the minimal seg reproduction
2. representative archived smoke stem for the same minimal seg evidence lane

### Implementation Files
- token-A (`promote_step_box.rs`) - Promotion step with Option-wrapped return and Reject bifurcation
- token-B (lowering orchestrator historical basename kept in the archived evidence lane) - Orchestrator with early-return on `Ok(None)`

### Test Files
- same evidence pair as above (historical fixture pin token + archived smoke stem) - SSOT fixture + VM smoke
...
**Main Risks**:
1. `Ok(None)` で後続 route（historical compat tokens: `legacy binding` / `normalized shadow`）に進んだが、後続 route も処理できない → 別のエラー
   - **Mitigation**: 後続 route（historical compat tokens: `legacy binding` / `normalized shadow`）は汎用処理なので対応可能
2. historical label-2 lane 専用だったケースが後続 route に流れて挙動変化
...
- ✅ Step 2: token-B 修正（Ok(None)検出）
- ✅ Step 2: Commit 2（正しいFix実装）
- ✅ Step 3-1: cargo test --lib PASS（1368/1368 - 退行なし）
- ✅ Step 3-2: same minimal seg fixture のエラーメッセージ変化確認（progress）
...
- 既存の `loop_break` route（historical numbered label `2`）構造（BodyInit → Break）と `LoopBodyLocalEnv` に収まる。

## Derived slot contract (minimal)

- 対象は **`loop_break` route（historical numbered label `2`）の break 条件で参照される LoopBodyLocal 1 変数**。
...
- representative legacy fixture pin token `phase263_pattern2_seg_realworld_min.hako` (Stage‑B 実ログ最小化)
- archived smoke stem `phase263_pattern2_seg_realworld_min_vm.sh` (semantic current entry: `loop_break_realworld_vm`)
...
- **loop_break prep box**:
  - historical prep-box token under the same old route-entry predecessor lane
