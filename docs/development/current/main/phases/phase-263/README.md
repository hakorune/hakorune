...
historical root lane at the time: `pattern2_steps/` under the old `joinir/patterns/` lane
...
### Core Implementation
1. same historical file token A as implementation section above (`promote_step_box.rs` under the same old `pattern2_steps/` lane)
...
- **loop_break prep box**:
  - historical path token: `pattern2_inputs_facts_box.rs` under the same old `joinir/patterns/` lane
