...
1. **Historical route ordering at the time** (router.rs):
   - loop_simple_while は loop_break route の**前に**試される（current route family order; historical numbered labels were Pattern1 → Pattern2）
...
**修正箇所**: historical path token `pattern2_lowering_orchestrator.rs`
same historical root lane as File 1 above
...
1. same historical promote-step token as implementation section above (`promote_step_box.rs` under the same old `pattern2_steps/` lane)
