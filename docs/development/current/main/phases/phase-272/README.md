...
- Phase 269（BoolPredicateScan Frag; historical label: Pattern8）: `docs/development/current/main/phases/phase-269/README.md`
- Phase 270（AccumConstLoop bridge; historical label: Pattern9）: `docs/development/current/main/phases/phase-270/README.md`

# Phase 272（P0）: scan_with_init / split_scan を Frag+emit_frag へ吸収（段階適用）

## ステータス

- **P0.1（ScanWithInit; historical label: Pattern6）**: ✅ 完了（Frag+emit_frag 経路へ移行）
- **P0.2（SplitScan; historical label: Pattern7）**: ✅ 完了（Frag+emit_frag 経路へ移行）
...
### ScanWithInit（historical label: Pattern6 / index_of）
...
### SplitScan（historical label: Pattern7 / split）
...
- `BoolPredicateScan`（historical label: Pattern8）の構造（参考）:
...
- scan_with_init route の JoinIRConversionPipeline 経路を撤去し、Frag 経路へ切り替え
  ...
  - same historical path lane as scope boundary (`pattern6_scan_with_init.rs`)
- P0 スコープ:
...
### 狙い
- SplitScan（historical label: Pattern7）の terminator 配線（if/loop の遷移）を Frag に集約し、副作用（`result.push`）を含む形でも CFG を壊さない。
...
- split_scan route の JoinIRConversionPipeline 経路を撤去し、Frag+emit_frag 経路へ切り替え
  ...
  - same historical path lane as scope boundary (`pattern7_split_scan.rs`)
...
Phase 272 P0.2 完了後、`ScanWithInit` / `SplitScan` / `BoolPredicateScan`（historical labels: Pattern6/7/8）の重複を以下へ収束した（仕様不変）:
...
1. ScanWithInit / SplitScan（historical labels: Pattern6/7）の fixture/smoke が Frag 経路で PASS
