...
- `tools/smokes/v2/lib/test_runner.sh` is now being thinned inside the shared harness itself: the full `MirBuilderBox.emit_from_program_json_v0(...)` fallback inside `verify_program_via_builder_to_core()` now lives behind `emit_mir_json_via_full_mirbuilder()`, so the remaining helper-local tail is the Rust CLI fallback lane rather than the direct Hako full-mirbuilder call
...
- the forced full-mirbuilder canary route under `tools/smokes/v2/profiles/integration/core/phase2044/mirbuilder_provider_emit_core_exec_canary_vm.sh` is still blocked by `[Phase 88] Ring0Context not initialized`; keep that as a separate runtime/entry blocker rather than folding it into the helper-local delete-order slice
