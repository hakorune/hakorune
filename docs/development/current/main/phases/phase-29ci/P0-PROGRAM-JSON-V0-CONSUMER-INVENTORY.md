...
| `future-retire bridge entry` | [src/runner/stage1_bridge/program_json_entry/mod.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/stage1_bridge/program_json_entry/mod.rs), [src/runner/stage1_bridge/program_json_entry/request.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/stage1_bridge/program_json_entry/request.rs), [src/runner/emit.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/emit.rs), [src/runner/mod.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/mod.rs) | `program_json_entry::{emit_program_json_v0_requested, emit_program_json_v0_and_exit}` | delegate-only entry; `mod.rs` no longer owns this bridge entry directly and now queries `program_json_entry/mod.rs` instead of reading the emit flag itself, while `emit.rs` no longer owns bridge-specific branch or process-exit formatting; request building/source-path precedence now stays in `program_json_entry/request.rs` |
...
- `ProgramJsonEntryOutcome` is gone from the outer caller surface; `program_json_entry/mod.rs` now owns success/error process-exit handling too
