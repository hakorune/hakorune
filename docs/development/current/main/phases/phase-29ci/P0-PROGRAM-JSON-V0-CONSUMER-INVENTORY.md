...
- retreat note: `phase2160/registry_optin_*` remains a separate legacy caller bucket because `HAKO_MIR_BUILDER_REGISTRY[_ONLY]`, delegate-off, and debug env stacks are still explicit in those scripts; do not mix that env-heavy bucket into the builder-min collapse
