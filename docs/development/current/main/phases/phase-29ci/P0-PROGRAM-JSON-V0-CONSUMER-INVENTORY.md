...
| --- | --- | --- | --- |
| `current authority` | [src/host_providers/mir_builder/authority.rs](/home/tomoaki/git/hakorune-selfhost/src/host_providers/mir_builder/authority.rs), [src/host_providers/mir_builder/lowering/program_json.rs](/home/tomoaki/git/hakorune-selfhost/src/host_providers/mir_builder/lowering/program_json.rs) | `emit_program_json_v0_for_strict_authority_source(...)`, `program_json_to_mir_json(...)` | current `stage1-env-mir-source` authority still materializes Program(JSON v0) before MIR(JSON); `authority.rs` owns `source -> Program(JSON v0)` while `lowering/program_json.rs` owns the real `Program(JSON v0) -> MIR(JSON)` lowering blocker |
