...
- current Rust authority is now physically split: `src/host_providers/mir_builder.rs` is only a thin façade, `user_box_decls.rs` owns the shared source-route handoff, `lowering.rs` owns the real `Program(JSON v0) -> MIR(JSON)` blocker, and `lowering/ast_json.rs` is explicit legacy compat keep; use that split when deciding the next pure-`.hako` blocker slice
