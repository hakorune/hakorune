...
- `tools/selfhost/selfhost_build.sh --run` is still upstream-blocked by that same default Stage-B freeze on `apps/tests/hello_simple_llvm.hako`; this slice only isolates the Core-Direct consumer lane and must not be read as route unblocking
- `tools/selfhost/selfhost_build.sh --exe` is still upstream-blocked by that same default Stage-B freeze on `apps/tests/hello_simple_llvm.hako`; this slice only isolates the EXE consumer lane and must not be read as route unblocking
