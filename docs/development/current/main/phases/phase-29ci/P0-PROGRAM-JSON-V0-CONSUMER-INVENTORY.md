...
| --- | --- | --- | --- |
| `current authority` | [src/host_providers/mir_builder/authority.rs](/home/tomoaki/git/hakorune-selfhost/src/host_providers/mir_builder/authority.rs), [src/host_providers/mir_builder/lowering/program_json.rs](/home/tomoaki/git/hakorune-selfhost/src/host_providers/mir_builder/lowering/program_json.rs) | `emit_program_json_v0_for_strict_authority_source(...)`, `program_json_to_mir_json(...)` | current `stage1-env-mir-source` authority still materializes Program(JSON v0) before MIR(JSON); `authority.rs` owns `source -> Program(JSON v0)` while `lowering/program_json.rs` owns the real `Program(JSON v0) -> MIR(JSON)` lowering blocker; exact live callers of that lowering owner are the source authority itself plus [crates/nyash_kernel/src/plugin/module_string_dispatch.rs](/home/tomoaki/git/hakorune-selfhost/crates/nyash_kernel/src/plugin/module_string_dispatch.rs) `emit_from_program_json_v0(...)` and [src/backend/mir_interpreter/handlers/extern_provider.rs](/home/tomoaki/git/hakorune-selfhost/src/backend/mir_interpreter/handlers/extern_provider.rs) `env.mirbuilder.emit` (with [src/backend/mir_interpreter/handlers/calls/global.rs](/home/tomoaki/git/hakorune-selfhost/src/backend/mir_interpreter/handlers/calls/global.rs) now acting as a thin delegate rather than a fourth owner) |
...
- exact `Program(JSON v0) -> MIR(JSON)` lowering caller accounting is now pinned: the remaining live caller owners are `authority.rs` (current source authority), `module_string_dispatch.rs` (explicit Program(JSON) kernel route), and `extern_provider.rs` (runtime `env.mirbuilder.emit` bridge); this is narrower than “all of mir_builder” and should guide the next de-Rust slice
- runtime `env.mirbuilder.emit` is now owner-local to `extern_provider.rs`; `calls/global.rs` delegates instead of keeping a second direct lowering branch
