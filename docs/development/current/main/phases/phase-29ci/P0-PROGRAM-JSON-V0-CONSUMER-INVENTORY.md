...
| `future-retire bridge` | [src/runner/stage1_bridge/program_json/mod.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/stage1_bridge/program_json/mod.rs), [src/runner/stage1_bridge/program_json/read_input.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/stage1_bridge/program_json/read_input.rs), [src/runner/stage1_bridge/program_json/emit_payload.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/stage1_bridge/program_json/emit_payload.rs), [src/runner/stage1_bridge/program_json/writeback.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/stage1_bridge/program_json/writeback.rs) | `emit_program_json_v0_for_stage1_bridge_emit_program_json(...)` | `program_json/mod.rs` is now a thin facade; source-text read policy, bridge-local payload emission, and bridge-local writeback policy are owner-local submodules under the same future-retire bucket |
| `future-retire bridge entry` | [src/runner/stage1_bridge/program_json_entry.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/stage1_bridge/program_json_entry.rs), [src/runner/emit.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/emit.rs), [src/runner/mod.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/mod.rs) | `program_json_entry::{emit_program_json_v0_requested, emit_program_json_v0_and_exit}` | delegate-only entry; `mod.rs` no longer owns this bridge entry directly and now queries `program_json_entry.rs` instead of reading the emit flag itself, while `emit.rs` no longer owns bridge-specific branch or process-exit formatting; source-path precedence now stays in `program_json_entry.rs` too |
...
- `ProgramJsonEntryOutcome` is gone from the outer caller surface; `program_json_entry.rs` now owns success/error process-exit handling too
- bridge-local source-path precedence (`stage1::input_path()` aliases first, CLI input fallback second) also moved into `program_json_entry.rs`, so `program_json/mod.rs` no longer depends on `CliGroups`
