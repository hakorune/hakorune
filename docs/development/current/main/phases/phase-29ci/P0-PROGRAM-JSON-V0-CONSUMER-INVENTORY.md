...
- the adjacent `phase2160/registry_optin_*` bucket now partially collapses through `run_program_json_via_registry_builder_module_vm(...)`, so the plain registry wrappers no longer own temp wrapper creation or the default registry/debug env stack inline
- retreat note: `registry_optin_method_arraymap_canary_vm.sh`, `registry_optin_method_arraymap_get_canary_vm.sh`, `registry_optin_method_arraymap_get_diag_canary_vm.sh`, `registry_optin_method_arraymap_direct_canary_vm.sh`, and `registry_optin_return_binop_varvar_canary_vm.sh` remain explicit legacy keep because they still carry preinclude-heavy, diag, or direct-lower contracts; do not mix those files into the shared registry collapse
