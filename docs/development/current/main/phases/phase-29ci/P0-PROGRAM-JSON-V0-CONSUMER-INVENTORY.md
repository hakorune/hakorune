...
- the adjacent `phase2160/registry_optin_*` bucket now collapses through two exact helper contracts: plain wrappers use `run_program_json_via_registry_builder_module_vm(...)`, and the preinclude-heavy wrappers use `run_program_json_via_registry_builder_module_vm_with_preinclude(...)`
- retreat note: `registry_optin_method_arraymap_get_diag_canary_vm.sh` and `registry_optin_method_arraymap_direct_canary_vm.sh` remain explicit legacy keep because they still carry diag or direct-lower contracts; do not mix those files into the shared registry collapse
