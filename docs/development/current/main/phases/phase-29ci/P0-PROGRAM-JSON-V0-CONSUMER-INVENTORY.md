...
- `tools/selfhost/selfhost_build.sh` still has an explicit `HAKO_USE_BUILDBOX=1` build-contract keep in code, but `apps/tests/hello_simple_llvm.hako` now proves a different split: the default compiler Stage-B lane emits `Extern(log 42) + Var("eturn")`, while the BuildBox keep still emits the old full-source `static/ox/ain` payload
...
- `tools/selfhost/selfhost_build.sh --run` now fails downstream on the default Stage-B payload with `JSON v0 bridge error: undefined variable: eturn`
- `tools/selfhost/selfhost_build.sh --exe` now fails on that same default payload with `Program(JSON v0) parse error: undefined variable: eturn`
- exact producer-side root-cause evidence is now pinned in `P5-STAGEB-MALFORMED-PROGRAM-JSON.md`: default Stage-B no longer emits the old full-source payload, but it still emits malformed Program(JSON v0) upstream of shell extraction, while `BuildBox.emit_program_json_v0(...)` remains pinned as the separate full-source parse owner
