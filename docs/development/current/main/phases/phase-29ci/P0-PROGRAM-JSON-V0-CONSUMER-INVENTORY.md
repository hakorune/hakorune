...
| --- | --- | --- | --- |
| `current authority` | [src/host_providers/mir_builder/authority.rs](/home/tomoaki/git/hakorune-selfhost/src/host_providers/mir_builder/authority.rs), [src/host_providers/mir_builder/lowering/program_json.rs](/home/tomoaki/git/hakorune-selfhost/src/host_providers/mir_builder/lowering/program_json.rs) | `emit_program_json_v0_for_strict_authority_source(...)`, `program_json_to_mir_json(...)` | current `stage1-env-mir-source` authority still materializes Program(JSON v0) before MIR(JSON); `authority.rs` owns `source -> Program(JSON v0)` while `lowering/program_json.rs` owns the real `Program(JSON v0) -> MIR(JSON)` lowering blocker; exact live callers of that lowering owner are the source authority itself plus [crates/nyash_kernel/src/plugin/module_string_dispatch.rs](/home/tomoaki/git/hakorune-selfhost/crates/nyash_kernel/src/plugin/module_string_dispatch.rs) `emit_from_program_json_v0(...)` and the shared runtime/plugin bridge owner [src/runtime/mirbuilder_emit.rs](/home/tomoaki/git/hakorune-selfhost/src/runtime/mirbuilder_emit.rs), which is used by [src/backend/mir_interpreter/handlers/extern_provider.rs](/home/tomoaki/git/hakorune-selfhost/src/backend/mir_interpreter/handlers/extern_provider.rs) and [src/runtime/plugin_loader_v2/enabled/extern_functions.rs](/home/tomoaki/git/hakorune-selfhost/src/runtime/plugin_loader_v2/enabled/extern_functions.rs) |
...
- source-route Program(JSON) tuple leakage is gone from kernel dispatch: `authority.rs` now injects `user_box_decls` itself and cross-crate source callers stay on `source_to_mir_json(...)`, while `source_to_program_and_mir_json(...)` is test-only owner-local evidence
...
- worker audit on that caller trio says the safest next owner-local slice is the kernel/plugin route in `module_string_dispatch.rs`: keep the route-local `user_box_decls` splice for now, but narrow the host-provider call from `program_json_to_mir_json_with_imports(..., BTreeMap::new())` to `program_json_to_mir_json(...)`
