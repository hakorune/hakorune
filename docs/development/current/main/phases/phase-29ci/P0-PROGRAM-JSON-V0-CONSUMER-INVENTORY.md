...
- `tools/selfhost/selfhost_build.sh` still has an explicit `HAKO_USE_BUILDBOX=1` build-contract keep in code, but `apps/tests/hello_simple_llvm.hako` now proves a one-owner remainder: the default compiler Stage-B lane is healthy again (`Extern(log 42) + Return(Int 0)`), while the BuildBox keep still emits the old full-source `static/ox/ain` payload
...
- `tools/selfhost/selfhost_build.sh --exe` is now green on that same repaired default payload
- exact producer-side root-cause evidence is now pinned in `P5-STAGEB-MALFORMED-PROGRAM-JSON.md`: default Stage-B is fixed for this fixture, while `BuildBox.emit_program_json_v0(...)` remains pinned as the separate full-source parse owner
