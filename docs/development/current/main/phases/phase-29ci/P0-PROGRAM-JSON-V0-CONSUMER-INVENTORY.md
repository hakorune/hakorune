...
- worker audit on that caller trio said the safest next owner-local slice was the kernel/plugin route in `module_string_dispatch.rs`; that narrowing is now landed, so the remaining kernel-side leaf is the route-local `user_box_decls` splice rather than the host-provider call variant
