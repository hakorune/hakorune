...
| --- | --- | --- | --- |
| `current authority` | [src/host_providers/mir_builder.rs](/home/tomoaki/git/hakorune-selfhost/src/host_providers/mir_builder.rs), [src/host_providers/mir_builder/lowering.rs](/home/tomoaki/git/hakorune-selfhost/src/host_providers/mir_builder/lowering.rs) | `emit_program_json_v0_for_strict_authority_source(...)`, `program_json_to_mir_json(...)` | current `stage1-env-mir-source` authority still materializes Program(JSON v0) before MIR(JSON); the dedicated `authority.rs` adapter is retired, shared `source -> Program(JSON v0) -> user_box_decls -> MIR(JSON)` shaping plus the live explicit Program(JSON) route now live directly in `mir_builder.rs`, and the shared Rust stop-line `module_to_mir_json(...)` also lives there now; `lowering.rs` is the test-only Program(JSON)->MIR evidence seam, while runtime/plugin `env.mirbuilder.emit` lowers via [src/runtime/mirbuilder_emit.rs](/home/tomoaki/git/hakorune-selfhost/src/runtime/mirbuilder_emit.rs) and direct [src/runner/json_v0_bridge/mod.rs](/home/tomoaki/git/hakorune-selfhost/src/runner/json_v0_bridge/mod.rs) imports support before reusing only shared MIR(JSON) emission |
...
- `lang/src/runner/launcher.hako` now also keeps its caller-side source/program-json choreography behind same-file helpers (`_emit_program_json_from_source_path_checked(...)`, `_emit_mir_json_from_source_path_checked(...)`, `_load_program_json_from_path_or_source(...)`, `_print_or_write_output(...)`), so `cmd_build_exe(...)`, `cmd_emit_program_json(...)`, and `cmd_emit_mir_json(...)` no longer repeat the read→Program→MIR→write tail inline
- accepted evidence for that launcher slice currently stays on `tools/hakorune_emit_mir_mainline.sh lang/src/runner/launcher.hako ...`; the stricter `stage1_contract_exec_mode ... emit-program launcher.hako ...` route still hits the adjacent strict source-route reject (`dev-local-alias-sugar`) and is therefore not used as this slice's acceptance proof
- `.hako` owner thinning is also valid for `lang/src/runner/stage1_cli.hako`; its direct `BuildBox.emit_program_json_v0(...)` checked path now stays behind an owner-local helper instead of remaining duplicated between `emit_program_json(...)` and `_mode_emit_program(...)`, and its direct `MirBuilderBox.emit_from_program_json_v0(...)` checked path now also stays behind an owner-local helper instead of remaining inline in `emit_mir_json(...)`
- `lang/src/runner/stage1_cli.hako` now also keeps its source/program-json orchestration tail behind same-file helpers (`_resolve_emit_program_source_text(...)`, `_resolve_program_json_for_emit_mir(...)`, `_resolve_program_json_for_run(...)`, `_load_program_json_from_path_or_source(...)`), and `stage1_main(...)` now reuses the existing `_resolve_*` env readers instead of re-reading that contract inline
- `.hako` owner thinning is also valid for `lang/src/runner/stage1_cli_env.hako`; its authority box now keeps direct `BuildBox.emit_program_json_v0(...)` behind a same-file helper shared by authority emit and defs-synthesis lowering, and the remaining direct `MirBuilderBox.emit_from_program_json_v0(...)` checked path now also stays behind same-file helper `Stage1ProgramJsonMirCallerBox` shared by source authority and explicit Program(JSON) compat keep
...
- shared producer `lang/src/compiler/build/build_box.hako` now also keeps bundle resolve decision + merged `scan_src` materialization behind `_bundle_inputs_requested(...)` / `_resolve_scan_src_from_bundle_ctx(...)`, keeps parse→fragment-enrichment sequencing behind `_build_program_json_from_scan_src(...)`, and keeps the defs→imports post-parse tail behind `_inject_stageb_fragments_json(...)`, so the outer `emit_program_json_v0(...)` entrypoint no longer mixes those responsibilities inline
...
- `tools/hakorune_emit_mir.sh` now also keeps the direct `MirBuilderBox.emit_from_program_json_v0(...)` checked path behind a generated wrapper-local `_emit_mir_checked(...)` helper, so helper-local owner thinning can proceed without touching `selfhost_build.sh` or `test_runner.sh`
...
- `tools/selfhost/selfhost_build.sh` now also keeps the generated `BuildBox.emit_program_json_v0(...)` checked path behind wrapper-local `_emit_program_json_checked(...)`, so the `HAKO_USE_BUILDBOX=1` keep no longer repeats that direct checked path inline in the generated runner body
...
- `tools/smokes/v2/lib/test_runner.sh` now also keeps the duplicated embedded `MirBuilderBox.emit_from_program_json_v0(...)` checked path behind wrapper-local generator `builder_module_program_json_runner_code()`, so both module-vm helper lanes share the same generated `_emit_mir_checked(...)` contract instead of repeating it inline
...
- current Rust authority is now physically split: `src/host_providers/mir_builder.rs` owns the live source-route handoff, explicit Program(JSON) route, shared `user_box_decls` shaping, and shared MIR(JSON) stop-line, while `lowering.rs` is the test-only Program(JSON)->MIR evidence seam and `lowering/ast_json.rs` is explicit legacy compat keep; use that split when deciding the next pure-`.hako` blocker slice
- source-route Program(JSON) tuple leakage is gone from kernel dispatch: cross-crate source callers stay on `source_to_mir_json(...)`, while `source_to_program_and_mir_json(...)` is test-only owner-local evidence, and `user_box_decls` injection is now shared inside `src/host_providers/mir_builder.rs`
