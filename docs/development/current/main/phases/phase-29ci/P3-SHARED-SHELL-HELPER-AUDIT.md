...
3. keep `tools/smokes/v2/lib/test_runner.sh` on helper-local slices only: builder lanes, shape/result routing, verify-tail policy, tagged-stdout policy, and builder-module env/render are isolated now, so the next slice moves to the stdout-file wrapper seam without touching the smoke tail yet
