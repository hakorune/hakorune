...
- exact W9 proof for that tagged-stdout seam now lives in `tools/dev/phase29ci_test_runner_tagged_stdout_probe.sh`; heavy `phase2160/builder_min_*` wrappers stay monitor-only while the helper-local contract is pinned directly
...
- exact W10 proof for that seam now lives in `tools/dev/phase29ci_test_runner_builder_envrender_probe.sh`, which pins the rendered runner contract plus route/common env application without relying on the heavy phase2160 wrapper canaries
...
- exact W11 proof for that seam now lives in `tools/dev/phase29ci_test_runner_stdout_file_probe.sh`, which pins rc preservation, stdout-file capture, stderr suppression, and registry/preinclude runner selection without relying on the heavy phase2160 wrapper canaries
- the phase2160 module-load dehang interrupt is now landed too: `IfMirEmitBox` and `CompatMirEmitBox` replaced the remaining giant MIR JSON concat bodies on the hot if/fallback path, and the bounded-loop fixes in `lower_return_loop_strlen_sum_box.hako` plus `ParserStmtBox.parse_opt_annotation(...)` removed the stray import-time freezes that were still stretching the module-load path
...
- exact W13 proof for that seam now lives in `tools/dev/phase29ci_test_runner_registry_tagged_stdout_probe.sh`, and `phase2160/registry_optin_method_arraymap_get_diag_canary_vm.sh` remains the thin wrapper canary for the diag side of that helper layer
...
- exact W14 proof for that seam now lives in `tools/dev/phase29ci_test_runner_method_arraymap_probe.sh`, and the `phase2160/registry_optin_method_arraymap*` wrappers remain light regression canaries rather than the exact helper-local acceptance line
...
3. keep `tools/smokes/v2/lib/test_runner.sh` on helper-local slices only: builder lanes, shape/result routing, verify-tail policy, tagged-stdout policy, builder-module env/render, stdout-file wrappers, the phase2160 module-load dehang, the tagged-stdout caller layer, the registry-specialized tagged-stdout layer, and the method-arraymap fallback synth + token-check layer are isolated now, so the next move is a reinventory for near-thin-floor vs smoke-tail boundary without touching the 43-file tail yet
