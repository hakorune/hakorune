...
- `tools/selfhost/selfhost_build.sh` no longer shows the old `hello_simple_llvm` freeze split, but the helper still sits on malformed-output debt: the default `compiler.hako --stage-b --stage3` lane now emits `Extern(log 42) + Var("eturn")`, while the `HAKO_USE_BUILDBOX=1` emit-only lane still emits the old `static/ox/ain` full-source payload
...
- `tools/selfhost/selfhost_build.sh --run` now fails downstream of Program(JSON) production, not upstream in JoinIR: the default Stage-B payload is malformed, and the Core-Direct consumer fails through `--json-file` with `JSON v0 bridge error: undefined variable: eturn`
- `tools/selfhost/selfhost_build.sh --exe` now fails on that same default Stage-B payload, surfacing as `Program(JSON v0) parse error: undefined variable: eturn`
