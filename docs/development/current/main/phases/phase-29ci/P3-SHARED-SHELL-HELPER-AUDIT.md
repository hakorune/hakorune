...
- `tools/smokes/v2/lib/test_runner.sh` now also owns the repeated phase2160 registry launch stack behind `run_program_json_via_registry_builder_module_vm()`, so the non-diag/non-direct `registry_optin_*` wrappers no longer repeat temp `.hako` wrapper creation or the default registry/debug env contract inline
- retreat note: `registry_optin_method_arraymap_canary_vm.sh`, `registry_optin_method_arraymap_get_canary_vm.sh`, `registry_optin_method_arraymap_get_diag_canary_vm.sh`, `registry_optin_method_arraymap_direct_canary_vm.sh`, and `registry_optin_return_binop_varvar_canary_vm.sh` remain explicit legacy keep because they still carry preinclude-heavy, diag, or direct-lower contracts that should not be hidden behind the shared registry helper yet
