...
- retreat note: `registry_optin_method_arraymap_get_diag_canary_vm.sh` and `registry_optin_method_arraymap_direct_canary_vm.sh` remain explicit legacy keep because they still carry diag or direct-lower contracts that should not be hidden behind the shared registry helpers yet
