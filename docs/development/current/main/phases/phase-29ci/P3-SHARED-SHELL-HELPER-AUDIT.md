...
- `tools/smokes/v2/lib/test_runner.sh` now also keeps the remaining no-fallback / Rust CLI fallback / success cleanup route order behind `verify_builder_no_fallback_requested()`, `cleanup_verify_builder_logs()`, and `handle_verify_builder_emit_result()`, so `verify_program_via_builder_to_core()` is now close to a pure emission -> tail-policy entrypoint
...
- `tools/smokes/v2/lib/test_runner.sh` now also keeps the shared phase2160 temp-wrapper + vm-launch contract behind `run_program_json_via_builder_module_vm_with_env()`, so builder-min / registry / preinclude / diag wrappers are now thin flag wrappers over one owner-local launch helper
...
- `tools/smokes/v2/lib/test_runner.sh` now also owns the repeated phase2044 provider/core env stack behind `run_verify_program_via_preferred_mirbuilder_to_core()`, so the first smoke-tail caller bucket no longer repeats `HAKO_PREFER_MIRBUILDER=1` + using/AST/top-level-main shell setup inline
- `tools/smokes/v2/lib/test_runner.sh` now also owns the repeated phase2044 Hako PRIMARY no-fallback env stack behind `run_verify_program_via_hako_primary_no_fallback_to_core()`, so that caller bucket no longer repeats `HAKO_PRIMARY_NO_FALLBACK=1` + `HAKO_MIR_BUILDER_INTERNAL=1` + using/AST/top-level-main shell setup inline
...
- `tools/smokes/v2/lib/test_runner.sh` now also owns skip/tag/functions boilerplate for the stable `phase2160/registry_optin_compare_*` + `registry_optin_binop_intint` subset behind `run_registry_builder_tag_canary()`, so those caller scripts are now thin fixture wrappers too (`registry_optin_canary`, method-arraymap wrappers, diag/direct probes, and `registry_optin_return_binop_varvar_canary_vm.sh` remain explicit keeps)
- `tools/smokes/v2/lib/test_runner.sh` now also owns skip/tag/functions/token-check boilerplate for the stable `phase2160/registry_optin_method_arraymap*` wrappers behind `run_registry_method_arraymap_canary()`, so those caller scripts are now thin fixture wrappers too (`registry_optin_method_arraymap_get_diag_canary_vm.sh` and `registry_optin_method_arraymap_direct_canary_vm.sh` remain explicit diag/direct keeps)
...
- retreat note: `registry_optin_method_arraymap_direct_canary_vm.sh` remains the only explicit registry legacy keep because it still carries a direct-lower contract that should not be hidden behind the shared registry helpers
