...
3. keep `tools/smokes/v2/lib/test_runner.sh` on helper-local slices only: builder lanes, shape/result routing, verify-tail policy, tagged-stdout policy, builder-module env/render, and stdout-file wrappers are isolated now, so the next slice moves to the tagged-stdout caller layer above stdout capture without touching the smoke tail yet
