...
- `tools/selfhost/selfhost_build.sh` no longer shows the old `hello_simple_llvm` freeze split: on `apps/tests/hello_simple_llvm.hako`, both the default `compiler.hako --stage-b --stage3` lane and the `HAKO_USE_BUILDBOX=1` emit-only lane now exit `0` for `--json`, but both emit the same malformed Program(JSON v0) payload (`undefined variable: static` when consumed downstream), so helper-local structure work must not be mixed with “BuildBox keep rescues this fixture” claims anymore
...
- `tools/selfhost/selfhost_build.sh --mir` is now green on `apps/tests/hello_simple_llvm.hako` because it uses the source-direct route; that success must not be misread as proof that the helper's Program(JSON) production lane is healthy
- `tools/selfhost/selfhost_build.sh --run` now fails downstream of Program(JSON) production, not upstream in JoinIR: the emitted Program(JSON v0) is malformed, and the Core-Direct consumer fails through `--json-file` with `JSON v0 bridge error: undefined variable: static`
- `tools/selfhost/selfhost_build.sh --exe` now fails on that same malformed Program(JSON v0) payload, surfacing as `Program(JSON v0) parse error: undefined variable: static`
