...
- retreat note: the adjacent `phase2160/registry_optin_*` bucket still keeps explicit `HAKO_MIR_BUILDER_REGISTRY[_ONLY]` / debug / delegate env stacks in the caller files; treat that as the next legacy bucket instead of mixing it into the builder-min collapse
