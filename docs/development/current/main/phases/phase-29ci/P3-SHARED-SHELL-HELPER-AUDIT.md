...
- `tools/selfhost/selfhost_build.sh --exe` remains upstream-blocked by that same default Stage-B freeze on `apps/tests/hello_simple_llvm.hako`; isolating `emit_exe_from_program_json_v0()` is consumer-lane cleanup only, not route repair
