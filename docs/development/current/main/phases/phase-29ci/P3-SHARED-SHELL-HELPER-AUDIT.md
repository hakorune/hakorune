...
3. keep `tools/smokes/v2/lib/test_runner.sh` on helper-local slices only: builder lanes, shape/result routing, verify-tail policy, tagged-stdout policy, builder-module env/render, stdout-file wrappers, the phase2160 module-load dehang, the tagged-stdout caller layer, and the registry-specialized tagged-stdout layer are isolated now, so the next slice moves to the method-arraymap fallback synth + token-check layer centered on `run_registry_method_arraymap_canary()` without touching the smoke tail yet
