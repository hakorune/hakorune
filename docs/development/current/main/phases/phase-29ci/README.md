...
- retreat note:
  ...
  - bridge-local Program(JSON v0) read->emit->write orchestration and owner-1 payload emission are now owner-local to `src/runner/stage1_bridge/program_json/pipeline.rs`, while read policy / writeback stay in `read_input.rs` / `writeback.rs`, so `program_json/mod.rs` keeps shrinking toward a pure facade
