...
- retreat note:
  ...
  - `tools/selfhost/selfhost_build.sh` no longer shows the old `hello_simple_llvm` freeze split, and the default compiler Stage-B lane is now healthy again: it exits `0`, emits `Extern(log 42) + Return(Int 0)`, and downstream `--json-file` / `--run` / `--exe` all pass for this fixture, while `HAKO_USE_BUILDBOX=1` still emits the old full-source `static/ox/ain` payload
  ...
  - exact current root cause for `hello_simple_llvm` is now pinned separately in `P5-STAGEB-MALFORMED-PROGRAM-JSON.md`: the default Stage-B lane is fixed for this fixture, and the remaining malformed Program(JSON v0) producer is BuildBox keep on the old full-source parse boundary
