...
- retreat note:
  ...
  - bridge-local Program(JSON v0) file read policy and payload emission are now owner-local to `src/runner/stage1_bridge/program_json/read_input.rs` and `src/runner/stage1_bridge/program_json/emit_payload.rs`, so `program_json/mod.rs` keeps shrinking toward a pure facade
