...
- retreat note:
  ...
  - `phase2160/registry_optin_*` now partially collapses through `run_program_json_via_registry_builder_module_vm(...)`: the non-diag/non-direct registry wrappers share one exact launch contract
  - legacy keep to preserve in docs: `registry_optin_method_arraymap_canary_vm.sh`, `registry_optin_method_arraymap_get_canary_vm.sh`, `registry_optin_method_arraymap_get_diag_canary_vm.sh`, `registry_optin_method_arraymap_direct_canary_vm.sh`, and `registry_optin_return_binop_varvar_canary_vm.sh` still keep explicit preinclude/diag/direct contracts and should retire separately from the shared registry launch bucket
