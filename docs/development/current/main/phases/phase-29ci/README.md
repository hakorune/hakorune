...
- retreat note:
  - compiled-stage1 build surrogate is not deletable yet, but it is now intended to shrink behind a single owner-local module; route match, handler ownership, and build-box/launcher MIR handoff regression coverage all live there now, so retirement no longer needs shared route-table or root-test edits
  - future-retire bridge entry is also close to owner-local: direct emit-flag reads now stay inside `src/runner/stage1_bridge/**`, the entry facade plus exact success/error process-exit formatting live in `program_json_entry/mod.rs`, request building/source-path precedence lives in `program_json_entry/request.rs`, and outer root-runner files remain only as thin caller contracts (`src/runner/mod.rs`, `src/runner/emit.rs`)
  - bridge-local Program(JSON v0) read->emit->write orchestration and owner-1 payload emission are now owner-local to `src/runner/stage1_bridge/program_json/mod.rs`, while read policy / writeback stay in `read_input.rs` / `writeback.rs`, so the bridge cluster keeps shrinking without widening outer callers
  ...
  - the first smoke-tail caller bucket is now moving onto that shared harness: `phase2044/mirbuilder_provider_*` wrappers use `run_verify_program_via_preferred_mirbuilder_to_core()`, so repeated prefer-provider env setup no longer lives inline in each script
