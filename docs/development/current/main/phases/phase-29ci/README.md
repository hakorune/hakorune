...
- retreat note:
  ...
  - `phase2160/registry_optin_*` now splits into two exact launch contracts: the plain registry wrappers use `run_program_json_via_registry_builder_module_vm(...)`, and the preinclude-heavy wrappers use `run_program_json_via_registry_builder_module_vm_with_preinclude(...)`
  - legacy keep to preserve in docs: `registry_optin_method_arraymap_get_diag_canary_vm.sh` and `registry_optin_method_arraymap_direct_canary_vm.sh` still keep explicit diag/direct contracts and should retire separately from the shared registry launch buckets
