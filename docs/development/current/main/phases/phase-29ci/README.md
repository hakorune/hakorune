...
- retreat note:
  ...
  - legacy keep to preserve in docs: `phase2160/registry_optin_*` still owns explicit `HAKO_MIR_BUILDER_REGISTRY[_ONLY]` / delegate-off / debug env stacks, so that bucket should be reduced separately after builder-min closeout
