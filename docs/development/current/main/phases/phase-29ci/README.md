...
- retreat note:
  ...
  - `tools/selfhost/selfhost_build.sh` no longer shows the old `hello_simple_llvm` freeze split, but the current truth is now two malformed producer shapes rather than one: the default compiler Stage-B lane exits `0` and emits `Extern(log 42) + Var("eturn")`, while `HAKO_USE_BUILDBOX=1` still emits the old full-source `static/ox/ain` payload
  ...
  - exact current root cause for `hello_simple_llvm` is now pinned separately in `P5-STAGEB-MALFORMED-PROGRAM-JSON.md`: the default Stage-B lane no longer falls straight back to the old full-source payload, but it still emits malformed Program(JSON v0) (`eturn`) upstream of shell extraction, while BuildBox keep remains on the old full-source parse boundary
