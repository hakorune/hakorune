...
- retreat note:
  ...
  - the outer caller audit now has two explicit waves: `.hako` live/bootstrap owners first, then shared shell helper keep; keep `program_json/mod.rs` monitor-only while those waves are executed separately
