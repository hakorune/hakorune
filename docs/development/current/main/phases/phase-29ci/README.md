...
- retreat note:
  ...
  - `tools/selfhost/selfhost_build.sh` no longer shows the old `hello_simple_llvm` freeze split: both the default compiler Stage-B lane and the `HAKO_USE_BUILDBOX=1` emit-only lane now exit `0` for `--json`, but they emit the same malformed Program(JSON v0) payload, so this bucket stays in audit/retreat mode as malformed-output debt rather than JoinIR-freeze debt
