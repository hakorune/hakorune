...
**Date**: 2025-12-27
**Status**: Complete (NestedLoopMinimal lowering + merge/latch fix)
...
- 既知の事実: `LoopForm (=LoopShape)` にはネスト情報が無いので、**LoopFormベースの NestedLoopMinimal（historical label `6`）検出/ルーティングでは実装できない**
...
- inner loop: LoopSimpleWhile 相当（historical label `1`; break/continue 無し）
- outer loop: LoopSimpleWhile 相当（historical label `1`; break/continue 無し）を優先
- それ以外:
...
- Lowering: `src/mir/join_ir/lowering/loop_routes/nested_minimal.rs`
  - historical physical path at the time: old label-6 nested-minimal lowering file in the old `joinir/patterns` lane
...
特に NestedLoopMinimal（historical label `6`）では `loop_step`（outer）から `inner_step` へ tail-call するため、以下を SSOT として固定する：
...
3. **Docs update**
   - Phase 188.1 の “historical label-6 specification” が design であることは維持
...
NestedLoopMinimal（historical label `6`）を通す過程で露出しやすい “暗黙ルール” を SSOT 化して、今後の nested/generalization を楽にする：
