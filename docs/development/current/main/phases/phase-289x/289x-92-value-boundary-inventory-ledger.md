...
| `289x-1g` demand ledger | current profile/helper/caller names are mapped to `ValueDemand`, `StorageDemand`, `PublishDemand`, and `MutationDemand` |
...
| `289x-1g` | BoxShape docs-only | exact demand ledger for current profile/helper/caller names |
