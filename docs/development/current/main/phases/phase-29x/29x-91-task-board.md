...
| `99F` | W1 | landed | file-move / shim order lock | docs say move-first, shim-second, delete-last |
| `99E1` | W1 | landed | lock `extern_provider.hako` split target | runtime owner and compat codegen stub are separated on paper |
| `99E2` | W1 | landed | lock `llvm_codegen.rs` split target | thin tool boundary and legacy MIR front door are separated on paper |
| `99E3` | W1 | landed | lock `LlvmBackendBox` split target | owner API and evidence adapter are separated on paper |
| `99E4` | W1 | landed | lock compat proof-box homes | `CodegenBridgeBox` / `LLVMEmitBox` leave owner-looking paths on paper |
| `99F1` | W1 | landed | move payload / wrapper paths first | compat payload and wrapper homes move before behavior changes |
| `99F2` | W1 | landed | move proof boxes with re-export only | old paths keep thin shims only if required |
| `99F3` | W1 | landed | keep behavior stable until references update | discovery / runner imports stay green before delete |
| `99F4` | W1 | landed | delete old entrypoints last | only after caller inventory and archive evidence are explicit |
...
| `99W1` | post-W6 watch | active | lock watch-1 caller groups | `compat_codegen_receiver.rs` upstream groups are explicit and separate from the surrogate watch |
| `99W2` | post-W6 watch | queued | lock watch-1 replacement contract gap | `emit_object(mir_json_text) -> object path` replacement conditions are explicit |
| `99X1` | post-W6 watch | queued | lock watch-2 caller groups | compiled-stage1 surrogate upstream groups are explicit under `module_string_dispatch` |
| `99X2` | post-W6 watch | queued | lock watch-2 replacement contract gap | surrogate replacement conditions are explicit |
