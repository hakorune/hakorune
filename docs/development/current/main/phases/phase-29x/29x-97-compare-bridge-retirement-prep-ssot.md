...
- the dedicated compare/debug helper module is retired; `ll_emit_compare_driver.rs` now carries the archive-later compare orchestration surface while `ll_emit_compare_vm.rs` carries VM spawn and `ll_emit_compare_stdout.rs` carries stdout extraction, `ll_emit_compare_source.rs` carries source rendering, `provider_keep.rs` carries explicit provider keep lanes, `capi_transport.rs` owns explicit CAPI helpers, and `transport_paths.rs` / `transport_io.rs` own the temp-path helpers and compare-source temp-file materialization
