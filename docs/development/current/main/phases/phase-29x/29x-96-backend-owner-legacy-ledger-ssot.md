...
| `env.codegen.compile_json_path` / `src/host_providers/llvm_codegen::{mir_json_to_object,mir_json_file_to_object}` | launcher/path-gated daily compile transport plus legacy tool path | evidence/legacy tool path only | launcher switches to root-first daily compile | demote first, delete later | flipped `.hako ll emitter` daily profiles already bypass this path through `compile_ll_text`; launcher temp MIR paths still depend on it |
