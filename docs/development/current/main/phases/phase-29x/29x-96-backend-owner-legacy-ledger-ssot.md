...
| `env.codegen.compile_json_path` / `src/host_providers/llvm_codegen::{mir_json_to_object,mir_json_file_to_object}` | current daily compile transport | evidence/legacy tool path only | `.hako` MIR root entry or equivalent root hydrator landed | demote first, delete later | mainline launcher still depends on this path today |
