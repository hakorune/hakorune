...
| `src/host_providers/llvm_codegen/legacy_mir_front_door.rs::compile_object_from_legacy_mir_json(...)` | legacy MIR(JSON) front door | archive-later | compare/archive callers only | archive-later | daily root-first compile bypasses this surface |
