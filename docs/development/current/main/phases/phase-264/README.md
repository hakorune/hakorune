...
**Route routing の流れ**:
1. **BoolPredicateScan** (historical label: `8`): REJECT - loop condition right is not .length()
2. **IfPhiJoin** (historical label: `3`): MATCHED → but rejects "Not an if-sum shape"
3. **LoopSimpleWhile / LoopBreak** (historical labels: `1/2`) は試されない
...
→ carrier_count = 2 → **IfPhiJoin**（historical if-phi numbered token: `3`）に分類される
...
- carrier_count > 1 → IfPhiJoin（historical if-phi numbered token: `3`）という分類は過度に保守的
- 実際には:
...
- 問題: LoopBreak（historical label: `2`）の A-3 Trim, A-4 DigitPos 両方で reject
- 最小再現との違い:
  - **最小再現** (F2): 単純な条件付き代入 → LoopSimpleWhile（historical label: `1`）で PASS ✅
  - **実際の BundleResolver** (F5): 複雑なネスト構造 → LoopBreak（historical label: `2`）で reject ❌

**Phase 264 P1 の課題**:
- BundleResolver.resolve/4 のような複雑ループは新しい promotion pattern (A-5 等) が必要
- または LoopBreak（historical label: `2`）の対象外として別の lowering 経路で処理

**Phase 264 P0 の成果**:
- 簡単な条件付き代入ループの誤分類を修正 ✅
- IfPhiJoin（historical label: `3`）への誤ルーティング防止 ✅
...
- IfPhiJoin heuristic was too conservative: carrier_count > 1 → IfPhiJoin（historical if-phi numbered token: `3`）
- Problem: Simple conditional assignment (seg = if x then "A" else "B") was
  incorrectly classified as IfPhiJoin（historical if-phi numbered token: `3`）, which only handles if-sum shapes
...
- same historical if-phi rejection lane as above（basename omitted here to reduce repeated token noise）
