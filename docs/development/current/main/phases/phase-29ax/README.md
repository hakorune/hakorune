...
- P2: `joinir_purity_gate_vm.sh` で `*_JOINIR_DEBUG/DEV` を都度解除し、環境汚染を除去
- P3: closeout（docs-only）

## Inventory (P1)

- joinir_purity_gate_vm: is_integer strict/dev の tag missing が出た
  - 原因: smoke 実行間で `*_JOINIR_DEBUG/DEV` が残留し、strict 実行の観測条件が揺れた
  - 対応: `joinir_purity_gate_vm.sh` で毎回 `unset` して直列化（P2）
