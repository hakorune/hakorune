...
| `MIMAP-133A` | selected current | Post-local-free-reuse-ledger-closeout row selection. | current planning row |
...
`MIMAP-133A` selects exactly one next row after the modeled local-free reuse
ledger closeout. Prefer allocator behavior progress unless the next row exposes
a concrete compiler acceptance blocker.
