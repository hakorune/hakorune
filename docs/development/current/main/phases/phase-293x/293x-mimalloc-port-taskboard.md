...
| `MIMAP-137A` | selected current | Post-local-free-reuse-ledger-release-closeout row selection. | current planning row |
...
Current row:
`MIMAP-137A` selects exactly one next mimalloc / hako_alloc allocator row or a
