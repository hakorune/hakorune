...
`MIMAP-136A` is the closeout guard row selected by `MIMAP-135A`.

It should freeze the `MIMAP-134A` local-free reuse ledger release route before
the lane selects broader allocator behavior.
...
- `apps/hako-alloc-segment-allocation-modeled-local-free-reuse-ledger-release-proof/`
...
```text
bash tools/checks/run_row_guard.sh --only hako-alloc-segment-allocation-modeled-local-free-reuse-ledger-release-closeout
