...
- Added the allocator comparison baseline diagnostic owner.
- Added the proof app, design SSOT, guard, manifest row, and module export.
- Classified missing C mimalloc baseline, missing hako_alloc baseline, missing
  throughput target, missing memory-usage target, missing workload matrix,
  invalid repeat count, and closed seam leaks.
