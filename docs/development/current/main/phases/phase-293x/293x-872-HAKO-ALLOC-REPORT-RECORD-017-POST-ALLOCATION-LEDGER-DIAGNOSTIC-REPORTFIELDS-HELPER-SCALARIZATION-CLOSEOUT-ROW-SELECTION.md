...
Select `HAKO-ALLOC-REPORT-RECORD-018`:
...
```text
It is the next adjacent segment-arena-backing diagnostic owner, has the same
scalar-only report-box construction shape as the allocation-ledger diagnostic
owner, and remains inside the existing record-local scalarization SSOT.
```
