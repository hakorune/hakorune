...
k2_wide_mimalloc_atomic_fetch_add_exe_guard.sh
k2_wide_mimalloc_substrate_route_inventory_guard.sh
```

## Implementation

- Added `k2_wide_mimalloc_allocator_atomic_route_guard.sh` as the consolidated
  allocator-facing fixed-slot atomic route acceptance guard.
- The guard runs the existing CAS/load/store/fetch_add pure-first EXE proof
  guards plus the mimalloc substrate route inventory guard.
- The guard also rejects out-of-scope ordered fixed-slot implementation rows,
  pointer fetch_add rows, remote-free policy vocabulary, and provider/hook
  vocabulary in the atomic substrate owner.
