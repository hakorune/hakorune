...
Status: Landed
Date: 2026-04-27
Scope: Split GenericMethodRoute site and operands into named records
Related:
  - CURRENT_TASK.md
...
# 291x-461: GenericMethodRoute Site/Operands Record

## Goal

Make `GenericMethodRoute` structurally distinguish call-site identity and MIR
operands from surface compatibility, observed evidence, and decided backend
metadata.
...
- `GenericMethodRoute` now keeps call-site identity in
  `GenericMethodRouteSite`.
- `GenericMethodRoute` now keeps MIR operands in
  `GenericMethodRouteOperands`.
- JSON field names, route matching, helper symbols, `.inc` behavior, and
