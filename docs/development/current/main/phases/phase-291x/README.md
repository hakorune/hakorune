...
- Latest landed cleanup target: read `latest_card_path` in
  `docs/development/current/main/CURRENT_STATE.toml`
- Next implementation target: quarantine/retire the stale `MirCallNeedPolicy`
...
3. `docs/development/current/main/phases/phase-291x/291x-291-mir-call-need-policy-owner-audit-card.md`
...
- latest known cleanup checkpoint: `291x-291`
- BuildBox thinning series is closed; residual MapBox.has sentinel retirement
  ...
  `MirCallRoutePolicy` export is retired; `MirCallNeedPolicy` owner audit is
  closed; next cleanup is stale need-policy export quarantine
