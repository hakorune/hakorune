...
5. landed second seam slice:
  ...
  - `.hako` recipe seam is now the stable visible owner for `acceptance_policy=boundary-pure-seed-matrix-v1` and the current narrow `acceptance_case` rows (`ret-const-v1`, `hello-simple-llvm-native-probe-v1`, `runtime-data-array-get-missing-v1`, `runtime-data-string-length-ascii-v1`, `runtime-data-array-length-v1`, `runtime-data-array-push-v1`, `runtime-data-map-size-v1`, `runtime-data-array-has-missing-v1`, `runtime-data-map-has-missing-v1`, `runtime-data-map-get-missing-v1`, `array-string-indexof-branch-v1`, `array-string-indexof-cross-block-select-v1`, `array-string-indexof-select-v1`, `string-indexof-ascii-v1`, `string-length-ascii-v1`, `method-call-only-small-compat-v1`)
  - current `micro kilo` reopen does not broaden `RuntimeDataBox` again; exact next work is the interleaved producer-block guard family after `get -> indexOf(const)`, and RuntimeData peel is deferred until the same blocker family recurs after that cut
