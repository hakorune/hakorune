...
5. landed second seam slice:
  ...
  - current `micro kilo` reopen does not broaden `RuntimeDataBox` again; the full block-26 interleaved observer family is now pinned, so the next exact work is dedicated seed shadow / route ownership on the same artifact rather than another missing observer shape, and RuntimeData peel stays deferred until that family still recurs after the direct-path proof
