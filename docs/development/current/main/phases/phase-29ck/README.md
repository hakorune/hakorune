...
5. landed second seam slice:
  ...
  - caller-side recipe seam now lives in `lang/src/shared/backend/backend_recipe_box.hako`; it owns the pure-first compile preflight and link recipe normalization, and `.hako` daily compile now passes explicit recipe payload into `env.codegen.compile_json_path(...)` while `lang/c-abi/shims/hako_llvmc_ffi.c` keeps the remaining transport-only compat replay logic
