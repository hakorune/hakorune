...
5. landed second seam slice:
  ...
  - follow-up pure-first slice: default boundary compile now carries caller-side recipe ownership from `.hako` and Rust boundary callers; `BackendRecipeBox.compile_route_profile(...)` is the current `.hako` recipe owner, `.hako` daily compile passes its explicit `compile_json_path(..., "", "pure-first", "none")` payload while explicit keep stays at `compile_keep_profile(..., "harness")`; Rust transport mirrors those names to env only at the C handoff, and `lang/c-abi/shims/hako_llvmc_ffi.c` owns recursion-safe transport/fallback execution; the old boundary locks for `ret_const_min_v1` and `hello_simple_llvm_native_probe_v1` now live in `tools/smokes/v2/suites/integration/phase29ck-boundary-legacy.txt`, while active daily owner proof moved to `phase29x_backend_owner_daily_{ret_const,print}_min.sh`
  ...
  - follow-up pure-string-length slice: the narrow ASCII-literal `StringBox.length/size` v1 seed is now a legacy boundary lock, and active daily owner proof moved to `tools/smokes/v2/profiles/integration/phase29x/derust/phase29x_backend_owner_daily_string_length_min.sh`
  ...
  - follow-up pure-string-indexof slice: the narrow ASCII-literal `StringBox.indexOf/1` seed is now a legacy boundary lock, and active daily owner proof moved to `tools/smokes/v2/profiles/integration/phase29x/derust/phase29x_backend_owner_daily_string_indexof_min.sh`
  ...
  - follow-up pure-runtime-data-array-push slice: the same generic pure-first lane now also accepts narrow `RuntimeDataBox.push` when the receiver is an `ArrayBox`, and `tools/smokes/v2/profiles/integration/archive/phase29ck_boundary/runtime_data/phase29ck_boundary_pure_runtime_data_array_push_min.sh` now lives as the retired boundary lock while the daily owner proof moved to `phase29x/derust`
  ...
  - follow-up pure-substring-concat-loop slice: the same boundary-owned pure-first lane now also accepts a narrow ASCII `substring + concat + length + rotate-substring` loop seed without reopening harness fallback; `tools/smokes/v2/profiles/integration/phase29ck_boundary/string/phase29ck_boundary_pure_substring_concat_loop_min.sh` is now legacy-only and the active daily owner proof moved to `phase29x_backend_owner_daily_substring_concat_loop_min.sh`
  - follow-up pure-indexof-line slice: the same boundary-owned pure-first lane now also accepts a narrow ASCII `indexOf("line")` loop seed with stable array routing; `tools/smokes/v2/profiles/integration/phase29ck_boundary/string/phase29ck_boundary_pure_indexof_line_min.sh` is now legacy-only and the active daily owner proof moved to `phase29x_backend_owner_daily_indexof_line_min.sh`
