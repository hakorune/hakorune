...
5. landed second seam slice:
  ...
  - next widening target is no longer the narrow ArrayBox `get` seed; it is the broader method-loop packs, followed by the `.hako` recipe seam that moves seed/route/compat policy out of `lang/c-abi/shims/hako_llvmc_ffi.c`
