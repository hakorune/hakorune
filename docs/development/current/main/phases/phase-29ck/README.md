...
5. landed second seam slice:
  ...
  - next widening target is no longer the narrow ArrayBox `get` seed; it is the broader method-loop packs, followed by gradual shrink of the remaining `lang/c-abi/shims/hako_llvmc_ffi.c -> ny-llvmc --driver harness` compat surface
