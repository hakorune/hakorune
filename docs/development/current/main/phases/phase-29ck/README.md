...
5. landed second seam slice:
  ...
  - next unsupported pure-first target is now `StringBox.indexOf/1` on ASCII literals; take that before broader RuntimeDataBox array/map or method-loop packs so the harness keep surface still shrinks one method shape at a time
