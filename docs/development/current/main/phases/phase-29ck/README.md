...
5. landed second seam slice:
  ...
  - follow-up pure-runtime-data-array-push slice: the same generic pure-first lane now also accepts narrow `RuntimeDataBox.push` when the receiver is an `ArrayBox`, and `tools/smokes/v2/profiles/integration/phase29ck_boundary/runtime_data/phase29ck_boundary_pure_runtime_data_array_push_min.sh` now lives as the retired boundary lock while the daily owner proof moved to `phase29x/derust`
