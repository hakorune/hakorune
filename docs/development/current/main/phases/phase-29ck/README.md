...
5. landed second seam slice:
  ...
  - current `micro kilo` reopen does not broaden `RuntimeDataBox` again; exact next work is a direct-path analysis-only recipe sidecar for `get -> indexOf(const) -> compare -> select`, and RuntimeData peel is deferred until the same blocker family recurs after that cut
