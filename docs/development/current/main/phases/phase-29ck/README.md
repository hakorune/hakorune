...
5. landed second seam slice:
  ...
  - next unsupported pure-first target is now `RuntimeDataBox.push(ArrayBox immediate value)`, followed by `RuntimeDataBox.has(ArrayBox empty index)`, before broader method-loop packs
