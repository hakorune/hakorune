...
`stageb_emit_program_json_v0_fixture()` used to be a thin shared wrapper for
the `phase29bq` fixture producers. After later cleanup/archival, the function
no longer has live callers, while the actual shell compat owner remains
`tools/lib/program_json_v0_compat.sh`.
