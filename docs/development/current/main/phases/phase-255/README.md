...
Phase 254 で scan_with_init route（historical numbered label: `6`）の実装が完了したが、integration テストで失敗：
...
### 現状の仕様（early numbered-route line; historical labels `1-5`）
...
**利点**:
- 既存の early numbered-route line（historical labels `1-5`）に影響なし
...
### Task 3: scan_with_init route（historical numbered label: `6`）の boundary 構築を修正

**current route files**:
- `src/mir/join_ir/lowering/scan_with_init_minimal.rs`
- `src/mir/builder/control_flow/plan/facts/loop_scan_with_init.rs`

**historical path token**: old scan_with_init helper basename under the old `joinir/patterns/` lane
...
- ❌ early numbered-route line（historical labels `1-5`）の動作を変更しない（regression 禁止）
...
- ✅ early numbered-route line（historical labels `1-5`）の既存テストがすべて PASS（regression なし）
