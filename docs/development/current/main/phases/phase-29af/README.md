# Phase 29af P0: LoopBreak Boundary Hygiene (SSOT)

Goal: loop_break route の boundary 情報の歪みを SSOT で整理し、将来の回帰を防ぐ（仕様不変）。
...
- boundary 構築: `src/mir/builder/control_flow/plan/loop_break_steps/emit_joinir_step_box.rs`
  - historical path token: `emit_joinir_step_box.rs` under the old `pattern2_steps/` lane
