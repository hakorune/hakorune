...
| `compare/` | compare-op pin (`compare_ge` still gate-live; `compare_ported` is now monitor-only in `vm-hako-core.txt`) | `1` | `1` via `vm-hako-core.txt` | `proof/native-reference/native_backend_compare_eq_canary_vm.sh` + `native_backend_compare_lt_canary_vm.sh` for the generic compare proof lane; `compare_ge` still needs its final non-vm_hako cutover decision | freeze -> cutover | `compare_ported` is no longer gate-owned and `compare_ge` has an explicit non-vm_hako decision |
...
| `compare_ge_ported_vm.sh` + `const_void_ported_vm.sh` | narrow single-purpose residues | product LLVM acceptance pack or explicit archive decision | remaining wave `1b` rows after the first partial cut |
