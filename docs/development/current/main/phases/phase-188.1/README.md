...
**Status**: ✅ Capability gate unblock 完了 / ❌ NestedLoopMinimal route（検出・lowering）は Phase 188.2+ に deferred
...
- ❌ **Nested loop の自動検出（LoopFormベース）**は未実装  
  `loop_route_detection::extract_features()` は `max_loop_depth=1` / `has_inner_loops=false` の固定値（TODO）で、NestedLoopMinimal route 分岐は到達しない
- ❌ **NestedLoopMinimal route lowering** は未実装  
  `src/mir/join_ir/lowering/loop_routes/nested_minimal.rs` はインフラ（stub）で、現状 `None` を返す
...
従って、ネスト深さの検査や NestedLoopMinimal route の自動検出を **LoopFormレイヤーだけで完結させることはできない**。
次の実装は Phase 188.2 で「StepTree側を使うか / LoopRegion（親子構造）を実装するか」を docs-first で決める。

## NestedLoopMinimal Route Specification

**Note**: このセクションは「目標仕様（design）」であり、Phase 188.1 では gate unblock のみ完了。実装は Phase 188.2+。

### Supported Forms (ONLY)

**Shape**: outer loop_simple_while + inner loop_simple_while

```nyash
// Outer loop: loop_simple_while-compatible (no break/continue)
loop(outer_cond) {
  // ... outer loop body before inner ...

  // Inner loop: loop_simple_while-compatible ONLY (no break/continue)
  loop(inner_cond) {
    // ... inner loop body ...
  }
...
**Requirements**:
- **Outer loop**: loop_simple_while-compatible - no break/continue
- **Inner loop**: loop_simple_while-compatible - no break/continue
...
2. **Inner loop with break/continue**:
   ...
   Hint: Only loop_simple_while-compatible inner loops are supported
   ```

3. **Outer loop with break/continue**:
   ```
   [joinir/nested_loop/outer_control_flow] Outer loop has break/continue (not supported)
   Hint: Only loop_simple_while-compatible outer loops are supported
   ```
...
- **Same route shape as loop_simple_while**: Both loops use tail-recursive step function shape
...
2. ✅ **NestedLoopMinimal route kind**: `LoopRouteKind::NestedLoopMinimal` classification added
...
- ❌ **NestedLoopMinimal route lowering logic**: `lower_nested_loop_minimal_to_joinir()` returns `None` (stub)
...
- Use `max_loop_depth` from StepTree for NestedLoopMinimal route detection
...
1. **`src/mir/join_ir/lowering/loop_routes/nested_minimal.rs`**
   - Phase 188.1: インフラ（stub）。現状 `lower_nested_loop_minimal_to_joinir()` は `None` を返す

### Modified Files

1. **`src/mir/loop_route_detection/mod.rs`** (~15 lines)
   - Add `LoopRouteKind::NestedLoopMinimal`
...
4. **LoopForm → route detection** (`loop_route_detection::classify()`):
   ```rust
   // Phase 188.1: NestedLoopMinimal route kind defined, but detection logic is STUB
   ...
   {
       return LoopRouteKind::NestedLoopMinimal;  // Never reached (stub)
   }
   ```

5. **Route → Lowering** (`loop_route_router.rs`):
   ```rust
...
- NestedLoopMinimal route の実装（ネスト検出のSSOT決定 + lowering 実装）
- break/continue in nested loops (loop_break / loop_continue_only inner/outer combinations)
- Multiple inner loops (siblings)
- 2+ level nesting (3+ loop depth)
- Nested loops with PHI (if_phi_join inner/outer combinations)
...
**Reality**: NestedLoopMinimal route kind exists, but automatic detection is NOT implemented (LoopForm limitation)
