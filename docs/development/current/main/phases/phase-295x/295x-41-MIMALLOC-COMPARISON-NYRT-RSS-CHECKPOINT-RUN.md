...
Status: Landed
Date: 2026-05-25
Scope: run NyRT self-RSS checkpoint diagnostic on empty no-output exact-EXE.
Blocker: MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-RUN-295X-001
Related:
  - docs/development/current/main/phases/phase-295x/295x-40-MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-DIAGNOSTIC.md
  - tools/checks/k2_wide_phase295x_nyrt_rss_checkpoint_run_guard.sh
---

# 295x-41 Mimalloc Comparison NyRT RSS Checkpoint Run
...
printing or the empty app body. The main step is currently around global plugin
host initialization.
...
```bash
bash tools/checks/k2_wide_phase295x_nyrt_rss_checkpoint_run_guard.sh
git diff --check
