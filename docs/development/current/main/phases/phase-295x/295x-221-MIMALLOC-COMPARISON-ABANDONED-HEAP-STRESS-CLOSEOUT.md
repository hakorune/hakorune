...
Related:
  - docs/development/current/main/phases/phase-295x/295x-220-MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-EMPTY-EXE-FOOTPRINT-DIAGNOSTIC.md
...
# 295x-221 Abandoned Heap Stress Closeout
...
The footprint diagnostic showed that the no-output control remains the right
empty-baseline reference for the abandoned-heap stress path, while the
evidence-printing empty app is still a fixed-cost observation rather than a
winner claim.

The ELF PT_LOAD footprint is much smaller than the observed empty exact-EXE
RSS, so the next seam should observe runtime entry checkpoints rather than
start broad shrinking.
