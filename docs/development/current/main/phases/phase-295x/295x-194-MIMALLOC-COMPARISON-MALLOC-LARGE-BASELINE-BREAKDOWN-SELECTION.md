...
Status: Landed
Date: 2026-05-25
Scope: select a narrow `malloc-large` exact-EXE baseline breakdown seam.
Blocker: MIMALLOC-COMPARISON-MALLOC-LARGE-BASELINE-BREAKDOWN-SELECTION-295X-001
Related:
  ...
  - tools/checks/k2_wide_phase295x_malloc_large_baseline_breakdown_selection_guard.sh
---

# 295x-194 Malloc-Large Baseline Breakdown Selection
...
The next row should observe the fixed `malloc-large` exact-EXE baseline before
any shrink work. The selected diagnostic combines:
...
```bash
bash tools/checks/k2_wide_phase295x_malloc_large_baseline_breakdown_selection_guard.sh
git diff --check
