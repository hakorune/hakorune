...
これは JoinIR IfPhiJoin route（historical numbered label `3`, if-only）+ call merge の**ネスト版**であり、VM/LLVM EXE 両方で同じ動作をすることが品質保証の要件。
...
### JoinIR IfPhiJoin route（historical label `3`, if-only）のネスト
- **外側if**: a == 1 の分岐
