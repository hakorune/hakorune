...
これは JoinIR IfPhiJoin route（historical numbered label: Pattern3, if-only）+ call merge の**ネスト版**であり、VM/LLVM EXE 両方で同じ動作をすることが品質保証の要件。
...
### JoinIR IfPhiJoin route（historical label: Pattern3, if-only）のネスト
- **外側if**: a == 1 の分岐
- **内側if**: b == 1 の分岐（外側then内部にネスト）
- **Call merge**: 3箇所のf()呼び出しが最終的に1つのvに集約

### MIR生成の観点
- ネストしたif-onlyが正しく IfPhiJoin route として認識される
