...
2. by-name fixture key の live set / retired set / inventory-only set を確認する
   - `current runtime keep` / `retired Program JSON compat key` / `retired historical docs-private-only key` /
     `current dev key` / `retired dev-gated compat key` の bucket を SSOT に固定する
   - exact current matrix は `phase-29ce/README.md` の `Current Program JSON key matrix` を正本とする
...
- `phase29bq_selfhost_planner_required_dev_gate_vm.sh`
  - filter 対象は `fixture + planner_tag + reason + filter_alias + fixture-derived semantic alias`
