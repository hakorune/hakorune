...
17. FileBox kernel roundtrip tests are now direct-contract
   ...
   - `plugin/invoke/by_name.rs` now keeps an explicit built-in FileBox compat branch for `readBytes` and `write`; `open`, `read`, and `close` are retired from that keep branch
   - the Python-side explicit compat helper now isolates `readBytes` only; `open`, `read`, and `close` are direct-route through `nyash.file.open_hhh`, `nyash.file.read_h`, and `nyash.file.close_h`
...
6. `P21-BYN-MIN5-HARD-RETIRE-EXECUTION-PACK.md` is the current exact front
   - `FileBox.open`, `FileBox.read`, and `FileBox.close` execution slices are landed
   - next exact slice is `FileBox.readBytes`
