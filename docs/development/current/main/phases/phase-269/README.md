...
### アーキテクチャ図
```
historical bool_predicate_scan entry
  ↓ P1: emission 入口に委譲
...
- same historical bool_predicate_scan lane as P1 scope above
...
4. `HAKORUNE_BIN=./target/release/hakorune bash` the same archived smoke stem listed in the P1 scope above
...
- historical helper token from the old bool_predicate_scan lane から bool_predicate_scan Frag lower を呼び出す
...
- ✅ same legacy fixture pin token as the P1 scope above を作成
- ✅ same archived smoke stem as the P1 scope above を作成
