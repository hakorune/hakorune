...
| `apps/tests/phase29bq_pattern2_break_recipe_only_min.hako` | `loop_break/facts.rs`（legacy key: `pattern2_break`） | `recipe_tree/composer.rs`（loop_break recipe） | `loop_break route`（legacy: `normalizer::normalize_pattern2_break`） | `pattern2 recipe-only` |
