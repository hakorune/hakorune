...
This lane is back in blocker=`none` steady-state. Keep it failure-driven and capture the next exact blocker before promoting broader lane work.
...
- LoopContinueOnly multidelta gate: `./tools/smokes/v2/profiles/integration/joinir/loop_continue_only_multidelta_planner_required_vm.sh`（legacy wrapper token details live in `joinir-smoke-legacy-stem-retirement-ssot.md`）
