...
1) CondProfile stabilization → 2) VerifiedRecipe boundary tightening → 3) GenericLoopV1 acceptance closure → 4) LoopCond recipe-only tag pinning → 5) DomainPlan residue cleanup → 6) idx_var contract → 7) Selfhost/OOM
...
- composer allowlist（`shadow_adopt` 等）の罠を減らす（DomainPlan名を含むエラー/チェックリスト/自動検出）
...
- Pattern4Continue multidelta gate: `./tools/smokes/v2/profiles/integration/joinir/phase29bq_pattern4continue_multidelta_planner_required_vm.sh`
...
| --- | --- | --- | --- | --- |
| `apps/tests/phase118_pattern3_if_sum_min.hako` | `pattern3_if_phi/facts.rs` | `(legacy)` | `pattern3_pipeline.rs` | `if_sum min` |
...
| `apps/tests/phase29bq_pattern2_break_recipe_only_min.hako` | `pattern2_break_facts.rs` | `recipe_tree/composer.rs` | `normalizer::normalize_pattern2_break` | `pattern2 recipe-only` |
...
- Recipe-first: Phase C10 Pattern5InfiniteEarlyExit recipe-first migration completed (builder, matcher verification, composer, recipe-only gate).
- Recipe-first: Phase C9 Pattern4Continue recipe-first migration completed (builder, matcher verification, composer, recipe-only gate).
- Recipe-first: Phase C13 Pattern1ArrayJoin recipe-first migration completed (builder with IfV2+Stmt×2, matcher verification, composer, recipe-only gate).
- Recipe-first: Phase C14 Pattern6–9 (ScanWithInit / SplitScan / BoolPredicateScan / AccumConstLoop) recipe-first migration completed (builder, matcher verification, composer, recipe-only gate).
...
- Recipe-first: Phase C12 Pattern1CharMap recipe-first migration completed (builder, matcher verification, composer, recipe-only gate).
- Recipe-first: Phase C11 Pattern1SimpleWhile recipe-first migration completed.
...
7. [x] StepMode: `InlineInBody` の “一般化” と verifier 契約の強化（S1+S2+S3 完了。Pattern1SimpleWhile source + runtime fixture pin を固定）
...
11. [x] `Pattern2BreakPlan.step_placement` を `Pattern2StepPlacement` enum に置換して Facts→Planner→Normalizer の分岐を明文化する（dev-only subset は維持）
