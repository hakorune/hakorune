...
| `38xB delete-ready shim sweep` | queued-next | drained shim のうち current pointer drain 済みのものを delete する |
| `38xC archive-later queue freeze` | queued | current/historical docs drain が残る shim を archive-later に固定する |
| `38xD closeout` | queued | cleanup queue と next source lane を current docs に戻す |
...
| `tools/hako_check_deadblocks_smoke.sh` | delete-ready | canonical path already lives under `tools/hako_check/deadblocks_smoke.sh`; no live executable callers remain |
...
- active macro wave: `38xA legacy smoke archive`
