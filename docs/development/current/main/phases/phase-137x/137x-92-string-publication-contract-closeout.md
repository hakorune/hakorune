...
3. **137x-C: structure completion gate before perf return**（after 137x-B）
...
Status: satisfied. 137x-B completed the container / primitive design cleanout,
and 137x-C completed structure alignment. Owner-first optimization return is
137x-D, while `publish.any` and runtime-wide
`Value Lane Architecture` implementation remain blocked here.
