...
- current-state token: `phase-293x packed ArrayBox auto-use boundary`
...
- retired in `phase-292x`: the raw C-side block/op scanner in `hako_llvmc_match_substring_concat_loop_ascii_seed(...)` is deleted; exact seed bridge selection now consumes `metadata.exact_seed_backend_route` plus the selected `StringKernelPlan.loop_payload` key.
...
- current landed substring truth:
  ...
  - broader-corridor keeper repair is now landed:
    ...
    - landed follow-on `phase-169x` added the narrow relation vocabulary that lets later substring-concat smokes pin preserving-vs-stop-at-merge semantics without treating numeric value ids as SSOT
    ...
    - current live post-sink metadata is now pinned separately by `phase137x_direct_emit_substring_concat_post_sink_shape.sh`; that smoke requires the selected `StringKernelPlan` key, `[slice, const, slice]` parts, middle literal, and loop payload instead of stale hard-coded helper value ids
    - `phase137x_direct_emit_substring_concat_phi_merge_contract.sh` now pins the landed metadata-contract follow-on too: live direct MIR still carries a loop-carried string phi lane, relation metadata keeps one preserving lane and one stop-at-merge lane for the selected concat-triplet corridor, and the exact backend route points at the selected `StringKernelPlan` key
    - the route-level guard is now `phase137x_direct_emit_substring_concat_route_contract.sh`, which proves `metadata.exact_seed_backend_route.tag = "substring_concat_loop_ascii"` is consumed before the compatibility ladder
...
- next active cut:
  ...
  9. fixed task order:
     ...
     - step 16: landed narrow metadata-contract follow-on in `phase-169x`; the smoke now pins preserving-vs-stop-at-merge relation metadata through selected `StringKernelPlan` keys instead of numeric value-id literals
     - step 17: any broader plan-window carry across the merged phi, or `call` / `boxcall` / `return` barrier relaxation, still requires another metadata-contract update first; `phase137x_direct_emit_substring_concat_phi_merge_contract.sh` remains the live guard for that stop line
