...
- current landed substring truth:
  ...
  - broader-corridor keeper repair is now landed:
    ...
    - current live post-sink shape is now pinned separately by `phase137x_direct_emit_substring_concat_post_sink_shape.sh`, and that smoke now also requires the helper-result `%36` to keep `publication_sink` / `direct_kernel_entry` plans on the live MIR; the exact `substring_concat` seed now rejects the body unless the same helper-result plan metadata is present, and the phase29x daily smoke uses the same post-sink contract as its daily owner proof
