...
MIMALLOC-COMPARISON-VSLICE-PAGE-HEAP-USIZE-REFRESH-001:
  selected current after 294x-267. Compose the page-heap non-id exact `usize`
  closeout guard with the existing mimalloc comparison vertical-slice closeout
  guard. Do not migrate page/handle ids by momentum after page-heap non-id
  closeout.
```
...
- [x] Migrate `HakoAllocPage.capacity` to exact `usize` (`294x-262`).
  - Stop line preserved page id, free_top, handle ids, method parameter
    surfaces, and requested_sizes array payload semantics.
- [ ] Select the next explicit non-negative production field group as
  `HAKO-ALLOC-USIZE-FIELD-GROUP-NEXT-SELECTION-008`.
- [x] Select `HakoAllocPage.free_top` as
  `HAKO-ALLOC-USIZE-FIELD-GROUP-263-PAGE-HEAP-FREE-TOP-001`
  (`294x-263`).
  - Stop line: keep page id, handle ids, method parameter surfaces, and
    requested_sizes array payload semantics current-lane.
- [x] Migrate `HakoAllocPage.free_top` to exact `usize` (`294x-264`).
  - Stop line preserved page id, handle ids, method parameter surfaces, and
