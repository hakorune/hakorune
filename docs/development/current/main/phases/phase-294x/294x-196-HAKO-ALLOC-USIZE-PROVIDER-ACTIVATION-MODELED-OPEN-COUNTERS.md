...
Status: Landed
Date: 2026-05-24
Scope: provider activation modeled-open pilot owner-local counter exact `usize` migration.
Blocker: HAKO-ALLOC-USIZE-FIELD-GROUP-194
Related:
  - docs/development/current/main/design/usize-semantic-foundation-ssot.md
  - docs/development/current/main/phases/phase-294x/294x-195-HAKO-ALLOC-USIZE-PROVIDER-ACTIVATION-MODELED-OPEN-COUNTER-SELECTION.md
...
# 294x-196 Hako Alloc Usize Provider Activation Modeled-Open Counters

## Decision

Migrate only the selected owner-local
`HakoAllocProviderActivationModeledOpenPilot` counters to exact `usize`
storage:
...
```bash
bash tools/checks/k2_wide_hako_alloc_provider_activation_modeled_open_pilot_guard.sh
