...
Blocker: HAKO-ALLOC-USIZE-FIELD-GROUP-207
Related:
...
# 294x-209 Hako Alloc Usize Provider Call Real API Stub Counter Selection

## Decision

Select the owner-local `HakoAllocProviderCallRealApiStubExecutionPilot`
counters as `HAKO-ALLOC-USIZE-FIELD-GROUP-208`:
...
These fields are monotonic stub-execution/reject counters initialized to `0`.
The selected group records model-space provider API stub execution evidence
only; actual provider API calls, host replacement, hooks, backend matcher
...
`HAKO-ALLOC-USIZE-FIELD-GROUP-208` should migrate only the selected owner-local
counters and update the provider-call real API stub execution guard to assert
