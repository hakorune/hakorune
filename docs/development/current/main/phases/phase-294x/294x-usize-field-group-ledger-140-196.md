...
HAKO-ALLOC-USIZE-FIELD-GROUP-208:
  landed by 294x-210. Migrated only those selected provider-call real API stub
  execution owner-local counters to exact `usize`; kept `last_reason`, report
...
```text
HAKO-ALLOC-USIZE-FIELD-GROUP-209:
  landed by 294x-211. Selected the owner-local
  `HakoAllocProviderCallExternalApiAdapterInventory` counters
  ...
  `closed_host_replacement_reject_count`, `closed_hook_reject_count`, and
  `closed_backend_matcher_reject_count`) as
  `HAKO-ALLOC-USIZE-FIELD-GROUP-210`. Keep `last_reason`, report mirrors,
  adapter payloads, external API readiness / executed flags, bool-like
  would-execute flags, and provider call / hook / replacement seams separate.
```
...
HAKO-ALLOC-USIZE-FIELD-GROUP-211:
  landed by 294x-213. Selected the owner-local
  `HakoAllocProviderCallExternalApiAdapterPreflight` counters
  (`preflight_count`, `accepted_count`, `reject_count`,
  `missing_inventory_reject_count`, `rejected_inventory_reject_count`,
  `missing_adapter_reject_count`, `invalid_adapter_reject_count`,
  `already_executed_reject_count`, `closed_execution_reject_count`,
  `closed_host_replacement_reject_count`, `closed_hook_reject_count`, and
  `closed_backend_matcher_reject_count`) as
  `HAKO-ALLOC-USIZE-FIELD-GROUP-212`. Keep `last_reason`, report mirrors,
...
```text
HAKO-ALLOC-USIZE-FIELD-GROUP-212:
  landed by 294x-214. Migrated only those selected provider-call external API
  adapter preflight owner-local counters to exact `usize`; kept `last_reason`,
  report mirrors, preflight payloads, external API readiness / executed flags,
...
```text
HAKO-ALLOC-USIZE-FIELD-GROUP-213:
  landed by 294x-215. Selected the owner-local
  `HakoAllocProviderCallExternalApiCallStubExecutionPilot` counters
  (`execution_count`, `accepted_count`, `reject_count`,
  `missing_preflight_reject_count`, `rejected_preflight_reject_count`,
  `not_ready_reject_count`, `already_executed_reject_count`,
  `closed_execution_reject_count`, `closed_host_replacement_reject_count`,
  `closed_hook_reject_count`, and `closed_backend_matcher_reject_count`) as
  `HAKO-ALLOC-USIZE-FIELD-GROUP-214`. Keep `last_reason`, report mirrors, stub
  execution payloads, result-code fields, actual external provider API call
  flags, bool-like would-execute flags, and provider call / hook / replacement
  seams separate.
```
...
```text
HAKO-ALLOC-USIZE-FIELD-GROUP-214:
  landed by 294x-216. Migrated only those selected provider-call external API
  call stub execution owner-local counters to exact `usize`; kept
  `last_reason`, report mirrors, stub execution payloads, result-code fields,
  actual external provider API call flags, bool-like would-execute flags,
  provider calls, host replacement, hooks, global allocator install,
  worker/TLS, atomics, provider package / DLL generation, and
  #[global_allocator] out of scope.
```
...
```text
HAKO-ALLOC-USIZE-FIELD-GROUP-215:
  landed by 294x-217. Selected the owner-local
  `HakoAllocSegmentArenaBackingNoEscapePointerResidencePilot` counters
  (`residence_count`, `accepted_count`, `reject_count`,
  ...
  Keep `last_reason`, report mirrors, pointer tokens, no-escape scope fields,
...
```text
HAKO-ALLOC-USIZE-FIELD-GROUP-220:
  landed by 294x-224. Migrated only the selected pointer-derived lookup
  execution pilot accept/reject counters to exact `usize`; kept `last_reason`,
  report mirrors, lookup result tokens, arena handle tokens, private pointer
  tokens, lifecycle identity fields, byte-count payloads, bool-like
  would-execute flags, dereference, arena release/recycle, segment-map
  mutation, atomic bitmap execution, OSVM, worker/TLS, providers, backend
  matchers, provider package / DLL generation, and #[global_allocator] out of
  scope.
```
...
```text
HAKO-ALLOC-USIZE-FIELD-GROUP-222:
  landed by 294x-226. Migrated only the selected facade huge page-model route
  owner-local counters to exact `usize`; kept
  `HakoAllocObjectLifecycleFacadeHugePageModelReport`, report mirrors, final
  result fields, status/reason vocabularies, page/block ids, pointer-like
  payloads, requested/committed size mirrors, fallback source fields,
  `HakoAllocHugePageModel` fields already owned by earlier rows, huge release,
  unregister, unreserve, decommit, OSVM page-source execution, remote-free,
  worker/TLS, atomics, providers, backend matchers, provider package / DLL
  generation, hooks, host replacement, and #[global_allocator] out of scope.
```
