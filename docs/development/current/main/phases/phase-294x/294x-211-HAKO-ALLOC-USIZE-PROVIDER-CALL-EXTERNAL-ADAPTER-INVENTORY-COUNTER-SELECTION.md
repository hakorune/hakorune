...
Blocker: HAKO-ALLOC-USIZE-FIELD-GROUP-209
Related:
...
# 294x-211 Hako Alloc Usize Provider Call External Adapter Inventory Counter Selection

## Decision

Select the owner-local `HakoAllocProviderCallExternalApiAdapterInventory`
counters as `HAKO-ALLOC-USIZE-FIELD-GROUP-210`:
...
These fields are monotonic external-provider-adapter inventory/reject counters
initialized to `0`. The selected group records external adapter availability
only; external provider API calls, host replacement, hooks, backend matcher
installation, worker/TLS, and global allocator install remain closed.
...
`HAKO-ALLOC-USIZE-FIELD-GROUP-210` should migrate only the selected owner-local
counters and update the provider-call external API adapter inventory guard to
