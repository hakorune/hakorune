...
| production `hako_alloc` fields | Facade-local stats, page-map owner counters, page-map release event/reject counters, and realloc path counters through alloc-copy-release fallback are exact `usize`; page/heap/queue/handle state remains `i64` until its own field-group row. |
