...
| production `hako_alloc` fields | Facade-local stats and page-map owner counters are exact `usize`; page/heap/queue/handle state remains `i64` until its own field-group row. |
