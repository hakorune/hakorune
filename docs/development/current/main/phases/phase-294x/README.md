...
| production `hako_alloc` fields | Facade-local stats, page-map owner counters, and page-map release event/reject counters are exact `usize`; page/heap/queue/handle state remains `i64` until its own field-group row. |
