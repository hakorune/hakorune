# 296x-1601 MIRBUILDER-CONVERTER-COVERAGE-HYGIENE-CARRIER-INFO-SNAPSHOT-HARNESS-IMPLEMENTATION-TOUCH-SET-CONTRACT-001
...
Define the implementation touch set for the selected CarrierInfo snapshot
harness typed execution path. The contract stays consultation-only and names
the first concrete slice that a later implementation would touch, without
...
owner: MirBuilder converter coverage hygiene CarrierInfo snapshot harness
...
```text
selected_slice=CarrierInfo_snapshot_harnesses
selected_candidate=CarrierInfo
...
- the selected harness slice stays CarrierInfo snapshot harnesses only
- the first implementation touch is the selected CarrierInfo snapshot harness slice
...
- any change to `mirbuilder_carrier_snapshot_artifacts.py`
