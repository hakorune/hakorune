...
Scope: Select the route owner for
  ...
  after the loop continue-if owner row.
Related:
  - docs/development/current/main/phases/phase-296x/296x-1297-COREPLAN-PARSE-PROGRAM2-LOOP-CONTINUE-IF-OWNER-SELECTION-001.md
---

# COREPLAN-PARSE-PROGRAM2-GUARD-PROG-NUMERIC-PARSE-LOOP-OWNER-SELECTION
...
The fixture is a numeric parse loop with an exit-if assignment plus `break` and
an if-only conditional update. It produces the expected output `5` with rc `0`.
There is no semantic failure and no evidence that forcing
...
NYASH_BIN=target/debug/hakorune \
  ...
  --only selfhost_parse_program2_guard_prog_numeric_parse_loop_min
```
...
```text
next_task=COREPLAN-SCAN-METHODS-NESTED-LOOP-DEPTH1-NO-BREAK-OR-CONTINUE-OWNER-SELECTION-001
```

## Report

```text
output_contract=coreplan-parse-program2-guard-prog-numeric-parse-loop-owner-selection-v0
route_expectation_updated=flowbox_adoption_tag
compiler_code_changed=0
semantic_output_already_green=1
focused_gate_green=1
full_phase_gate_progressed_to_next_blocker=phase29bq_selfhost_blocker_scan_methods_nested_loop_depth1_no_break_or_continue_min
summary=ok
```
