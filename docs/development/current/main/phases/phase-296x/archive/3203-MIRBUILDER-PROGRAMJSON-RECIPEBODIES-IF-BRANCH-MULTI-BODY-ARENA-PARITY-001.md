# 3203 - ProgramJSON RecipeBodies If Branch Multi-Body Arena Parity
...
Implement the first multi-body RecipeBodies-like arena DTO in `.hako`:

```text
Program.body = [
  Local,
  If(then=[Local], else=[Print]),
  Return
]
```
...
- The implementation owner is
  `ProgramJsonRecipeBodiesIfBranchArenaBuilderBox`.
...
`MIRBUILDER-PROGRAMJSON-RECIPEBODIES-IF-BRANCH-MULTI-BODY-ARENA-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001`
