...
selected_next_task=HAKO-LIFECYCLE-RESOLVER-READONLY-SKELETON-001
selected_reason=CarrierInfo snapshots and merge_from now have focused fixture
evidence. A resolver can start as diagnostics-only if it reads only frozen
fixtures, denies join_id-dependent paths, and does not become a selection or
emission owner.
```
...
join_id vocabulary retirement/design decision:
  parked; read-only resolver must deny join_id-dependent paths

trim_helper lifecycle inventory/probe:
  parked; read-only resolver must not claim trim route ownership
```
