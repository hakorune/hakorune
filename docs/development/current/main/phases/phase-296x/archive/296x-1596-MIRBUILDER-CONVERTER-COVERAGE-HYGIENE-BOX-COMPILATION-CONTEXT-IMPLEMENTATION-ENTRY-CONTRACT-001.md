# 296x-1596 MIRBUILDER-CONVERTER-COVERAGE-HYGIENE-BOX-COMPILATION-CONTEXT-IMPLEMENTATION-ENTRY-CONTRACT-001
...
Define the implementation entry contract for the selected BoxCompilationContext
typed execution harness IR. The contract stays consultation-only and names
the entry point at which implementation work would actually begin, without
...
input: implementation-start boundary contract
output: implementation-entry contract
```
...
The implementation entry must stay minimal and explicit:

- the selected harness slice stays BoxCompilationContext only
- the entry point marks the first actual implementation touch
- the entry point does not widen route selection
- the entry point does not open the nightly rustc adapter path
- the entry point does not open runtime fallback
- the entry point does not add new family selection
...
```text
the implementation entry is explicit
