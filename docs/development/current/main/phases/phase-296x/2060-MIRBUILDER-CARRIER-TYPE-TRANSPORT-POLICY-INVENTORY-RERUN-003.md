# 2060 - MIRBUILDER-CARRIER-TYPE-TRANSPORT-POLICY-INVENTORY-RERUN-003
...
Inventory carrier/type transport lanes after
`MIRBUILDER-MISSING-PROJECTION-POLICY-CLUSTER-RESOLUTION-V4` selected the
TypeTransportMissing blocker lane.

This card does not choose a carrier policy by return-type count. Multiple
transport lanes are present, so the next step is evidence inventory.
...
decision:
  SelectCarrierTypeTransportEvidenceInventoryRerun003
...
tool:
  tools/rust_lifecycle/
    mirbuilder_carrier_type_transport_policy_inventory_rerun_003.py

guard:
  tools/checks/
    rust_lifecycle_mirbuilder_carrier_type_transport_policy_inventory_rerun_003_guard.sh
```
