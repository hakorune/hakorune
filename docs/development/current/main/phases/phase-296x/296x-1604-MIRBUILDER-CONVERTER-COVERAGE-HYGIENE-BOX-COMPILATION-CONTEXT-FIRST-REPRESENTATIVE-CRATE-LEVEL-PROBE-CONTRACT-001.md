# 296x-1604 MIRBUILDER-CONVERTER-COVERAGE-HYGIENE-BOX-COMPILATION-CONTEXT-FIRST-REPRESENTATIVE-CRATE-LEVEL-PROBE-CONTRACT-001
...
Define the first representative easy-tier crate-level probe contract for the
selected BoxCompilationContext harness path. The contract stays
consultation-only and names the probe boundary that would exercise the now
landed harness wrapper rows, without opening code changes.

## Scope

```text
BoxCount: one consultation probe contract
owner: MirBuilder converter coverage hygiene BoxCompilationContext harness
input: landed harness wrapper rows
output: representative crate-level probe contract
```
...
The first representative easy-tier crate-level probe must stay minimal and
explicit:

- the probe stays BoxCompilationContext only
- the probe exercises the landed harness wrapper rows
- the probe does not widen route selection
- the probe does not open the nightly rustc adapter path
- the probe does not open runtime fallback
- the probe does not add new family selection
...
```text
the representative crate-level probe contract is explicit
...
no code change is started by this probe contract
```
