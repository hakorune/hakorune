# 2126 - MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-DELETE-SURFACE-HAKO-ADOPTION-DECISION-001
...
Adopt `write_delete_surface_policy_classifier` as a narrow `.hako` parity
pilot owner after the green 1-row Rust-oracle parity gate.

This decision adopts only the `DeleteSurfacePolicy / MapDeleteAny`
classifier/policy mirror. It does not adopt WriteScalarI64Routes direct
closeout, ScalarKnownTransportAxis closeout, runtime mutation authority,
publication execution, Source Selfhost, or full MirBuilder conversion.
...
parity_gate:
  tools/checks/
    rust_lifecycle_mirbuilder_carrier_type_scalar_known_write_delete_surface_parity_gate.sh
...
reason_token:
  WriteDeleteSurfaceRustOracleParityGateGreen
...
```text
tools/checks/
  rust_lifecycle_mirbuilder_carrier_type_scalar_known_write_delete_surface_hako_adoption_decision_guard.sh
```
