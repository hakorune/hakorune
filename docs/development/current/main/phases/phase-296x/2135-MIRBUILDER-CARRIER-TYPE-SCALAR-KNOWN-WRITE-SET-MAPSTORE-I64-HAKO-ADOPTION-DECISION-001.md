# 2135 - MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-I64-HAKO-ADOPTION-DECISION-001
...
Adopt `write_set_mapstore_i64_policy_classifier` as a narrow `.hako` parity
pilot owner after the green 1-row Rust-oracle parity gate.

This decision adopts only the `SetSurfacePolicy / MapStoreI64`
classifier/policy mirror. `MapStoreAny` stays deferred behind the Any write
boundary. This card does not adopt SetSurfacePolicy as a whole.
...
reason_token:
  WriteSetMapStoreI64RustOracleParityGateGreen
...
```text
tools/checks/
  rust_lifecycle_mirbuilder_carrier_type_scalar_known_write_set_mapstore_i64_hako_adoption_decision_guard.sh
```
