# 3380 - MIRBUILDER-SCALAR-KNOWN-FASTPATH-HAKO-GENERATED-TYPED-ARTIFACT-BASIS-COLLECTION-SCALAR-I64-001
...
Define the basis for connecting `CollectionScalarI64Routes` to the live Rust
fast path through a checked-in generated typed `.hako` artifact consumed as
shadow evidence.

This card is basis-only. It does not create the `.hako` source, does not create
the generated Rust artifact, does not connect `collection_read_routes`, and
does not switch route authority to `.hako`.
...
return/value/publication/effect:
  ScalarI64 / ScalarI64 / NoPublication / observe
...
```text
tools/checks/
  rust_lifecycle_mirbuilder_scalar_known_fastpath_hako_generated_typed_artifact_basis_collection_scalar_i64_guard.sh
```
