# 296x-1599 MIRBUILDER-CONVERTER-COVERAGE-HYGIENE-BOX-COMPILATION-CONTEXT-INITIAL-PATCH-SEQUENCE-CONTRACT-001
...
Define the initial patch sequence for the selected BoxCompilationContext
typed execution harness IR. The contract stays consultation-only and names
the first patch order that a later implementation would need, without
opening code changes.

## Scope

```text
BoxCount: one consultation patch-sequence contract
owner: MirBuilder converter coverage hygiene BoxCompilationContext harness
input: implementation wiring contract
output: initial patch sequence contract
```
...
The initial patch sequence must stay minimal and explicit:

- the selected harness slice stays BoxCompilationContext only
- the first patch remains the smallest harness-only rewrite step
- the patch sequence does not widen route selection
- the patch sequence does not open the nightly rustc adapter path
- the patch sequence does not open runtime fallback
- the patch sequence does not add new family selection
...
```text
the initial patch sequence is explicit
...
no code change is started by this patch-sequence contract
```
