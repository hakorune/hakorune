# 2993 - MIRBUILDER-PROGRAMJSON-SECOND-STMT-NOT-RETURN-REJECT-RETIRE-RUST-ASTNODE-PROJECTOR-CANDIDATE-001

Status: landed

## Scope

Mark only the `LoopCondContinueSecondStmtNotReturnRejectV1` ProgramJSON
...
expected_summary = accepted=0;reason=unsupported_shape
```

## Evidence

- Fixture:
  `docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-second-stmt-not-return-reject-retire-rust-astnode-projector-candidate-v0.json`
- Guard:
  `tools/checks/rust_lifecycle_mirbuilder_programjson_second_stmt_not_return_reject_retire_rust_astnode_projector_candidate_guard.sh`
...
`LoopCondContinueSecondStmtNotReturnRejectTokenSnapshotV1` is a
retire-candidate for the single `LoopCondContinueSecondStmtNotReturnRejectV1`
shape only.
