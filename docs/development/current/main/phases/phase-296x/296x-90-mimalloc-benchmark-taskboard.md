...
```text
HAKO-MIMALLOC-PERF-POST-KEEPER-TAXONOMY-REFRESH-296X-001:
  Refresh in-process gap taxonomy after the first keeper optimization.
```
...
| 59 | `HAKO-MIMALLOC-PERF-POST-KEEPER-TAXONOMY-REFRESH-296X-001` | Current | Refresh in-process gap taxonomy after the first keeper optimization. |
| 60 | `HAKO-MIMALLOC-PORT-FEATURE-GAP-INVENTORY-296X-001` | Planned | Inventory missing mimalloc port pieces without mixing feature-port work into optimization rows. |
| 61 | `HAKO-MIMALLOC-PROVIDER-PACKAGE-REAL-ENTRYPOINT-SELECTION-296X-001` | Planned | Select how much of real `.hako` mimalloc should be exposed through provider package explicit API. |
| 62 | `HAKO-MIMALLOC-HAKMEM-LDPRELOAD-SHIM-DECISION-296X-001` | Planned | Decide whether to build a hakmem-compatible malloc/free export shim after explicit provider evidence. |
| 63 | `HAKO-MIMALLOC-HAKMEM-LDPRELOAD-SHIM-SMOKE-296X-001` | Planned | Build and smoke-test an optional LD_PRELOAD-compatible shim without enabling normal host allocator replacement. |
| 64 | `HAKO-MIMALLOC-PERF-PARITY-SELFHOST-HANDOFF-GATE-296X-001` | Planned | Decide whether the `.hako` mimalloc evidence is strong enough to return focus toward selfhosting. |
...
### Row 60 - Port Feature Gap Inventory
...
### Row 61 - Real Provider Entrypoint Selection
...
### Row 62 - Hakmem LD_PRELOAD Shim Decision
...
### Row 63 - Hakmem LD_PRELOAD Shim Smoke
