# 2143 - MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-ANY-PARITY-GATE-001
...
`write_set_mapstore_any_policy_classifier` `.hako` implementation.

This card proves the 2142 `.hako` classifier output matches the 2142 Rust
oracle parity fixture for the scoped `SetSurfacePolicy / MapStoreAny` pilot.
The Any write boundary remains declared metadata only.
...
```text
output_contract =
  rust-lifecycle-mirbuilder-carrier-type-scalar-known-write-set-mapstore-any-parity-gate-v0
...
reason_token:
  WriteSetMapStoreAnyParityGateGreen
