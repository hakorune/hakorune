...
Next:
  Use the ret0.exe nyash_kernel_runtime build-registry exact-top-symbol
  ...
  nyash_kernel_runtime, and PERF-USERBOX-029 now isolates the build-registry
  exact top symbols inside nyash_kernel_runtime. Keep env / stdio / path
  closed; only split within the exact build-registry subfamily when further
