...
Next:
  Use the ret0.exe nyash_kernel_runtime build exact-top-symbol distribution to
  choose the next owner. The path exact top-symbol distribution flatlined in
  PERF-USERBOX-024, PERF-USERBOX-026 isolated the registry exact top symbols
  inside nyash_kernel_runtime, PERF-USERBOX-027 isolated the runtime exact top
  symbols inside nyash_kernel_runtime, and PERF-USERBOX-028 now isolates the
  build exact top symbols inside nyash_kernel_runtime. Keep env / stdio /
  path closed; only split within the exact build subfamily when further
  granularity is worth landing. Runtime-build-off, entry-path-prep-off, and
  ring0-init-off remain historical evidence for the entry floor and should
  stay closed unless the current executable owner flatlines.
```
