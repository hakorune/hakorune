...
Next:
  ...
  symbols inside nyash_kernel_runtime, and PERF-USERBOX-037 now isolates the
  registry rebuild-cache gc_mode exact top symbols inside nyash_kernel_runtime.
  Keep env / stdio / path closed; only split within the exact registry
  subfamily when further granularity is worth landing.
