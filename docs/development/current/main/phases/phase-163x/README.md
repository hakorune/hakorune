  ...
  - post-primitive follow-on queue:
    ...
    3. recommended next cut = `sum placement/effect pilot`
      ...
      - focused `ny-llvmc` proving slice is now landed:
        ...
        - `tools/smokes/v2/profiles/integration/phase163x/phase163x_boundary_sum_metadata_keep_min.sh` now pins the same no-replay contract across the metadata-bearing `variant_project`, copied `variant_project`, direct `variant_tag`, and copied-`variant_tag` fixtures
      - next active substep stays `phase163x-sum-thin-entry-cutover`; the remaining Variant* optimization inventory is now fixed in this order:
        1. land direct `variant_tag` keep-lane proof on `tag_only`
        2. land direct `variant_tag` keep-lane proof on `tag_f64_payload`
        3. land direct `variant_project` keep-lane proof on `tag_f64_payload`
        4. land direct `variant_tag` keep-lane proof on `tag_handle_payload`
        5. land direct `variant_project` keep-lane proof on `tag_handle_payload`
        6. only after the direct layout proofs are green, add single-`copy` alias parity for the non-`i64` layouts
