  ...
  - post-primitive follow-on queue:
    ...
    4. after that, run a separate `ny-llvmc` parity wave
      ...
      - immediate next task after the parity keeper:
        ...
        - active string bridge-retirement follow-on: `phase-179x` now has the first explicit backend-consumable `StringKernelPlan` export; exporter-side `string_corridor_facts` / `relations` / `candidates` were already present, and the next cut is metadata-first seed consumption rather than another benchmark-shape proof
