  ...
  - post-primitive follow-on queue:
    ...
    4. after that, run a separate `ny-llvmc` parity wave
      - proving slice is now landed:
        ...
        - metadata-bearing Point local-i64 user-box fixture is now green on `tools/smokes/v2/profiles/integration/phase163x/phase163x_boundary_user_box_metadata_keep_min.sh` via boundary `pure-first` owner lane without compat replay
      ...
      - immediate next parity box:
        - accept the same metadata-bearing Point local-i64 user-box route through a single local `copy` alias on the boundary `pure-first` owner lane
