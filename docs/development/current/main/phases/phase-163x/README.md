  ...
  - post-primitive follow-on queue:
    ...
    4. after that, run a separate `ny-llvmc` parity wave
      ...
      - immediate next task after the parity keeper:
        ...
        - active string bridge-retirement follow-on: `phase-179x` now fixes the next missing seam as an explicit backend-consumable `StringKernelPlan`; exporter-side `string_corridor_facts` / `relations` / `candidates` are already present, so the next cut is schema/export first rather than another benchmark-shape proof
