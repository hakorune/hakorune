...
| RVP-C22 | `MapBox.keys()` zero-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set(...); local ks = m.keys(); print(ks.size())`) で発生 | blocked（shape is accepted, but vm-hako runtime still stops at `op=boxcall0 method=keys`） | `apps/tests/vm_hako_caps/mapbox_keys_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_keys_block_vm.sh` | blocked | 2026-03-21: `MapBox.delete(key)` port の次 exact blocker is keys runtime route |
