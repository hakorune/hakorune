...
| RVP-C26 | `MapBox.set(non-string key, value)` visible bad-key contract | quick map collection smoke (`new MapBox(); print(m.set(123, 1))`) で発生 | blocked（shape is accepted, but vm-hako runtime still reports `[vm-hako/unimplemented op=boxcall method=set]`） | `apps/tests/vm_hako_caps/mapbox_set_bad_key_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_set_bad_key_block_vm.sh` | blocked | 2026-03-21: `RVP-C25` port の次 exact blocker is bad-key `set` visible contract |
