...
| RVP-C18 | `MapBox.size()` zero-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set(...); m.size()`) で発生 | blocked（`set` は通過済みだが、vm-hako runtime が `op=boxcall0 method=size` で停止） | `apps/tests/vm_hako_caps/mapbox_size_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_size_block_vm.sh` | blocked | 2026-03-21: collection owner cutover の next exact blocker は `size` zero-arg boxcall runtime route |
