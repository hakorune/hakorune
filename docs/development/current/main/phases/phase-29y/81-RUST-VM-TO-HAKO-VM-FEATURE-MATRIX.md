...
| RVP-C25 | `MapBox.get(non-string key)` visible bad-key contract | quick map collection smoke (`new MapBox(); print(m.get(123))`) で発生 | blocked（shape is accepted, but vm-hako runtime still prints stale scalar `0` instead of stable `[map/bad-key]` text） | `apps/tests/vm_hako_caps/mapbox_get_bad_key_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_get_bad_key_block_vm.sh` | blocked | 2026-03-21: `RVP-C24` port の次 exact blocker is bad-key `get` visible contract |
