...
| RVP-C19 | `MapBox.get(key)` one-arg `boxcall` value semantics | quick map collection smoke (`new MapBox(); m.set("a", 42); print(m.get("a"))`) で発生 | blocked（shape is accepted, but vm-hako runtime still returns stale scalar `0` instead of the stored value） | `apps/tests/vm_hako_caps/mapbox_get_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_get_block_vm.sh` | blocked | 2026-03-21: collection owner cutover の next exact blocker is `MapBox.get(key)` value semantics |
