...
| RVP-C20 | `MapBox.has(key)` one-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set("a", 42); print(m.has("a"))`) で発生 | subset-check + vm-hako runtime accepted（stale `op=boxcall1 method=has` blocker removed; minimal presence `1/0` route pinned） | `apps/tests/vm_hako_caps/mapbox_has_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_has_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now preserves minimal per-key presence state for `has(key)` and the one-arg route is pinned as ported |
| RVP-C21 | `MapBox.delete(key)` one-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set("a",1); m.set("b",2); m.delete("a")`) で発生 | blocked（shape is accepted, but vm-hako runtime still stops at `op=boxcall1 method=delete`） | `apps/tests/vm_hako_caps/mapbox_delete_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_delete_block_vm.sh` | blocked | 2026-03-21: `MapBox.has(key)` port の次 exact blocker is delete runtime route |
