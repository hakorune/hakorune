...
| RVP-C28 | `MapBox.setField(non-string key, value)` visible bad-key contract | quick map field smoke (`new MapBox(); print(m.setField(456, 1))`) で発生 | blocked（shape is accepted, but vm-hako runtime still reports `[vm-hako/unimplemented] ... op=boxcall(setField:args>1)`） | `apps/tests/vm_hako_caps/mapbox_setfield_bad_key_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_setfield_bad_key_block_vm.sh` | blocked | 2026-03-21: `RVP-C27` port の次 exact blocker is bad-key `setField` visible contract |
