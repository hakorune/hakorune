...
| RVP-C17 | `MapBox.set(key, value)` multi-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set("a", 42)`) で発生 | blocked（`newbox(MapBox)` は通過済みだが、subset-check が `boxcall(set:args>1)` で停止） | `apps/tests/vm_hako_caps/mapbox_set_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_set_block_vm.sh` | blocked | 2026-03-21: collection owner cutover の quick map acceptance を `.hako VM` で再利用しようとすると、現在の exact blocker は `set` の 2-arg boxcall contract mismatch |
