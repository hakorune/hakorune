...
| RVP-C24 | `MapBox.get(missing-key)` visible missing-key contract | quick map collection smoke (`new MapBox(); print(m.get("nope"))`) で発生 | blocked（shape is accepted, but vm-hako runtime still prints stale scalar `0` instead of stable missing-key text/tag） | `apps/tests/vm_hako_caps/mapbox_get_missing_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_get_missing_block_vm.sh` | blocked | 2026-03-21: `MapBox.clear()` port の次 exact blocker is missing-key `get` visible contract |
