...
| RVP-C20 | `MapBox.has(key)` one-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set("a", 42); print(m.has("a"))`) で発生 | subset-check + vm-hako runtime accepted（stale `op=boxcall1 method=has` blocker removed; visible `true/false` parity pinned） | `apps/tests/vm_hako_caps/mapbox_has_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_has_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now preserves per-key presence state for `has(key)` and direct print parity (`true/false`) is fixed with the one-arg route |
| RVP-C21 | `MapBox.delete(key)` one-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set("a",1); m.set("b",2); m.delete("a")`) で発生 | runtime ported（stale `op=boxcall1 method=delete` blocker removed; `has("a")==false` and `size()==1` pinned） | `apps/tests/vm_hako_caps/mapbox_delete_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_delete_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now removes per-key presence/value state on `delete(key)` and preserves visible `has/size` parity |
| RVP-C22 | `MapBox.keys()` zero-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set(...); local ks = m.keys(); print(ks.size())`) で発生 | runtime ported（stale `op=boxcall0 method=keys` blocker removed; ArrayBox-like `size()==2` pinned） | `apps/tests/vm_hako_caps/mapbox_keys_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_keys_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now returns an ArrayBox-like box token for `keys()` and preserves visible `keys().size()` parity |
| RVP-C23 | `MapBox.clear()` zero-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set(...); m.clear(); print(m.size()); print(m.has("a")); local ks = m.keys(); print(ks.size())`) で発生 | runtime ported（stale `op=boxcall0 method=clear` blocker removed; `size()==0`, `has("a")==false`, `keys().size()==0` pinned） | `apps/tests/vm_hako_caps/mapbox_clear_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_clear_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now clears per-key presence/value state and preserves visible `size/has/keys` reset parity |
| RVP-C24 | `MapBox.get(missing-key)` visible missing-key contract | quick map collection smoke (`new MapBox(); print(m.get("nope"))`) で発生 | subset-check + vm-hako runtime accepted（stale scalar `0` removed; stable `[map/missing] Key not found: <key>` text pinned） | `apps/tests/vm_hako_caps/mapbox_get_missing_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_get_missing_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now returns the stable missing-key text/tag for `MapBox.get(missing-key)` |
| RVP-C25 | `MapBox.get(non-string key)` visible bad-key contract | quick map collection smoke (`new MapBox(); print(m.get(123))`) で発生 | subset-check + vm-hako runtime accepted（stale scalar `0` removed; stable `[map/bad-key] key must be string` text pinned） | `apps/tests/vm_hako_caps/mapbox_get_bad_key_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_get_bad_key_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now returns the stable bad-key text for `MapBox.get(non-string key)` |
| RVP-C26 | `MapBox.set(non-string key, value)` visible bad-key contract | quick map collection smoke (`new MapBox(); print(m.set(123, 1))`) で発生 | subset-check + vm-hako runtime accepted（stale unimplemented `set` route removed; stable `[map/bad-key] key must be string` text pinned） | `apps/tests/vm_hako_caps/mapbox_set_bad_key_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_set_bad_key_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now returns the stable bad-key text for `MapBox.set(non-string key, value)` |
| RVP-C27 | `MapBox.getField(non-string key)` visible bad-key contract | quick map field smoke (`new MapBox(); print(m.getField(123))`) で発生 | subset-check + vm-hako runtime accepted（stale unimplemented `getField` route removed; stable `[map/bad-key] field name must be string` text pinned） | `apps/tests/vm_hako_caps/mapbox_getfield_bad_key_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_getfield_bad_key_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now returns the stable bad-key field text for `MapBox.getField(non-string key)` |
| RVP-C28 | `MapBox.setField(non-string key, value)` visible bad-key contract | quick map field smoke (`new MapBox(); print(m.setField(456, 1))`) で発生 | subset-check + vm-hako runtime accepted（stale `boxcall(setField:args>1)` route removed; stable `[map/bad-key] field name must be string` text pinned） | `apps/tests/vm_hako_caps/mapbox_setfield_bad_key_ported_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_setfield_bad_key_ported_vm.sh` | ported | 2026-03-21: vm-hako runtime now returns the stable bad-key field text for `MapBox.setField(non-string key, value)` |
