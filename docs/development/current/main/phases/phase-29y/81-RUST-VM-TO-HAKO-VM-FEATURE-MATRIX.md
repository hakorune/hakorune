...
| RVP-C27 | `MapBox.getField(non-string key)` visible bad-key contract | quick map field smoke (`new MapBox(); print(m.getField(123))`) で発生 | blocked（shape is accepted, but vm-hako runtime still reports `[vm-hako/unimplemented op=boxcall1 method=getField]`） | `apps/tests/vm_hako_caps/mapbox_getfield_bad_key_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_getfield_bad_key_block_vm.sh` | blocked | 2026-03-21: `RVP-C26` port の次 exact blocker is bad-key `getField` visible contract |
