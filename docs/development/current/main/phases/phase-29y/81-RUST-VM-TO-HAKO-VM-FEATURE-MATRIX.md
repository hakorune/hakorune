...
| RVP-C23 | `MapBox.clear()` zero-arg `boxcall` | quick map collection smoke (`new MapBox(); m.set(...); m.clear(); print(m.size()); print(m.has("a")); local ks = m.keys(); print(ks.size())`) で発生 | blocked（shape is accepted, but vm-hako runtime still stops at `op=boxcall0 method=clear`） | `apps/tests/vm_hako_caps/mapbox_clear_block_min.hako` | `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_clear_block_vm.sh` | blocked | 2026-03-21: `MapBox.keys()` port の次 exact blocker is clear runtime route |
