...
- `rc_gc_alignment_g2_fast_milestone_gate.sh` is currently blocked on the G-RC-1 lifecycle parity case `rc_explicit_drop_min`:
...
- This blocker is separate from the `.hako` VM entry wiring slice and should be fixed in the RC/GC parity lane before optional GC entry can reopen.
