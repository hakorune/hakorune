...
- On the default build, `rc_gc_alignment_g1_lifecycle_parity_vm_llvm.sh` and `rc_gc_alignment_g5_mode_invariance_vm_llvm.sh` are harness-gated and skip when hakorune is built without `--features llvm-harness`.
- The previously observed `rc_explicit_drop_min` mismatch is only relevant in `llvm-harness` builds:
  - VM keeps the expected `0`
  - LLVM harness currently exits `1`
  - observed LLVM stderr includes `Plugin internal error (code: -5)` and `Result: 1`
- The default lane gate is therefore not blocked on this mismatch; optional GC entry stays guarded by the RC/GC matrix and should only reopen if a harness-enabled build reproduces the parity failure.
