...
- builtin file/string/array boxcall helpers は `mir_vm_s0_boxcall_builtin.hako` に寄せ、`mir_vm_s0_boxcall_exec.hako` は codegen/generic fallback を中心に保つ。
- call / externcall / newbox / ret helpers は `mir_vm_s0_call_exec.hako` に寄せ、`mir_vm_s0_exec_dispatch.hako` は data/block glue を中心に保つ。
- LlvmBackendBox compile/link の apply helpers は `mir_vm_s0_codegen.hako` に寄せ、`mir_vm_s0_boxcall_exec.hako` は codegen/generic fallback routing を中心に保つ。
...
- C14 ported 契約:
  - `vm_hako_caps/app1/app1_summary_contract_ported_vm.sh` が PASS
- C15 blocked 契約:
  - `vm_hako_caps_app1_summary_contract_block_vm.sh` が PASS（full fixture timeout blocker）
- vm-hako capability gate:
  - `vm_hako_caps/gate/phase29y_vm_hako_caps_gate_vm.sh` が PASS
- mainline/lane gate:
...
1. `bash tools/smokes/v2/profiles/integration/vm_hako_caps/app1/app1_summary_contract_ported_vm.sh`
2. `bash tools/smokes/v2/profiles/integration/apps/archive/vm_hako_caps_app1_summary_contract_block_vm.sh`
3. `bash tools/smokes/v2/profiles/integration/vm_hako_caps/gate/phase29y_vm_hako_caps_gate_vm.sh`
...
- call/externcall/newbox/ret helpers が `mir_vm_s0_call_exec.hako` 側へ寄っている
