---
Status: Active (Y1/Y2/Y3 done, min1/min2/min3 done, RVP-0..RVP-5 done, RING1-CORE-06..09 done; reopened by RVP-C24)
...
- Current blocker（脱Rust selfhost runtime lane）は `RVP-C24`。
- 2026-03-21 reopen:
  ...
  - the next exact blocker is `RVP-C24 MapBox.get(missing-key)` stale scalar route.
...
- active status:
  - RVP open blocker は `RVP-C24 MapBox.get(missing-key)` stale scalar route.
...
- next-1: `RVP-C24`
- exact blocker:
  - capability: `MapBox.get(missing-key)` visible missing-key contract
  - current route: `.hako VM` / `vm-hako` runtime
  - current blocked behavior: runtime prints stale scalar `0` instead of stable missing-key text/tag
  - blocked pin:
