---
Status: Active (Y1/Y2/Y3 done, min1/min2/min3 done, RVP-0..RVP-5 done, RING1-CORE-06..09 done; reopened by RVP-C20)
...
- Current blocker（脱Rust selfhost runtime lane）は `RVP-C20`。
- 2026-03-21 reopen:
  ...
  - the next exact blocker is `RVP-C20 MapBox.has(key)` unimplemented route.
...
- active status:
  - RVP open blocker は `RVP-C20 MapBox.has(key)` unimplemented route.
...
- next-1: `RVP-C20`
- exact blocker:
  - capability: `MapBox.has(key)` one-arg `boxcall`
  - current route: `.hako VM` / `vm-hako` runtime
  - current blocked behavior: runtime stops at `[vm-hako/unimplemented op=boxcall1 method=has]`
  - blocked pin:
