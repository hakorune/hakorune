---
Status: Active (Y1/Y2/Y3 done, min1/min2/min3 done, RVP-0..RVP-5 done, RING1-CORE-06..09 done; reopened by RVP-C21)
...
- Current blocker（脱Rust selfhost runtime lane）は `RVP-C21`。
- 2026-03-21 reopen:
  ...
  - `RVP-C20 MapBox.has(key)` is now ported and pinned by `vm_hako_caps_mapbox_has_ported_vm.sh`.
  - the next exact blocker is `RVP-C21 MapBox.delete(key)` unimplemented route.
...
- active status:
  - RVP open blocker は `RVP-C21 MapBox.delete(key)` unimplemented route.
...
- next-1: `RVP-C21`
- exact blocker:
  - capability: `MapBox.delete(key)` one-arg `boxcall`
  - current route: `.hako VM` / `vm-hako` runtime
  - current blocked behavior: runtime stops at `[vm-hako/unimplemented op=boxcall1 method=delete]`
  - blocked pin:
...
- latest complete: `RVP-C20`（`MapBox.has(key)` one-arg boxcall accepted and pinned）
- next-1: `RVP-C21`
