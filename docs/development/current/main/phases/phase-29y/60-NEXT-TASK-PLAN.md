---
Status: Active (Y1/Y2/Y3 done, min1/min2/min3 done, RVP-0..RVP-5 done, RING1-CORE-06..09 done; reopened by RVP-C19)
...
- Current blocker（脱Rust selfhost runtime lane）は `RVP-C19`。
- 2026-03-21 reopen:
  ...
  - the next exact blocker is `RVP-C19 MapBox.get(key)` stale-zero semantics.
...
- active status:
  - RVP open blocker は `RVP-C19 MapBox.get(key)` stale-zero semantics.
...
- next-1: `RVP-C19`
- exact blocker:
  - capability: `MapBox.get(key)` one-arg `boxcall`
  - current route: `.hako VM` / `vm-hako` runtime
  - current blocked behavior: stale scalar `0` is returned instead of the stored value
  - blocked pin:
    - `tools/smokes/v2/profiles/integration/apps/vm_hako_caps_mapbox_get_block_vm.sh`
- after `RVP-C19` closes:
  - `map_basic_get_set_vm.sh`
  returns to collection owner cutover acceptance as a valid quick smoke
- already returned after `RVP-C18`:
  - `map_len_size_vm.sh`
