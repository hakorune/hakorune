...
- the next landed tightening on `lang/src/compiler/build/build_box.hako` keeps shared producer sequencing behind owner-local helpers too: `_bundle_inputs_requested(...)` / `_resolve_scan_src_from_bundle_ctx(...)` now isolate bundle resolve decision + merged `scan_src` materialization, `_build_program_json_from_scan_src(...)` now isolates parse→fragment-enrichment sequencing, and `_inject_stageb_fragments_json(...)` now owns the defs→imports post-parse tail
