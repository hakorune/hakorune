...
- the normal registry-first `Program(JSON v0) -> MIR(JSON)` authority block now lives in `lang/src/mir/builder/internal/registry_authority_box.hako`, so `MirBuilderBox.hako` is thinner and keeps route sequencing + compat tails around that internal authority owner
