...
- the latest stop-line leaf also keeps guarded Program(JSON)->MIR emission on `Stage1ProgramJsonModuleHandoff::emit_guarded_mir_json()` and strict source Program(JSON) emission on `SourceProgramJsonHandoff::emit_strict_program_json(...)`, so the top-level authority seam no longer needs those free wrapper helpers
- the latest stop-line leaf also keeps source-route MIR-only projection on `SourceProgramJsonHandoff::emit_guarded_mir_json()`, so `source_to_mir_json(...)` no longer destructures the broader `(Program(JSON), MIR(JSON))` handoff tuple at top level
