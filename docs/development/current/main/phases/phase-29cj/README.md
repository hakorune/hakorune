...
- the latest explicit-route finalize leaf also keeps `Program(JSON)` parse/build separate from MIR JSON mutation at `finalize_mir_json_with_stage1_user_box_decls(...)` -> `build_stage1_user_box_decls_from_program_json(...)` -> `inject_user_box_decls_into_mir_json(...)`
