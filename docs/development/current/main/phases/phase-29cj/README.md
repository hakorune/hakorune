...
- the latest strict-source public/test leaf also keeps Program(JSON) emission and guarded/plain MIR handoff behind same-file helpers `emit_program_json_for_source(...)`, `emit_program_and_guarded_mir_json_for_source(...)`, and `emit_program_and_plain_mir_json_for_source(...)`; the live stop-line still stays in `src/host_providers/mir_builder.rs`
