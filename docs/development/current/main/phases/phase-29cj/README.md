...
- the next landed tightening on that same owner also keeps prepared-scan-src result/error handoff behind `_new_prepare_scan_src_result(...)`, `_fail_prepare_scan_src(...)`, and `_apply_prepare_scan_src_result(...)`, so `_prepare_scan_src(...)` no longer mixes result-map allocation, error tagging, and resolved-scan-src apply with bundle/env orchestration
