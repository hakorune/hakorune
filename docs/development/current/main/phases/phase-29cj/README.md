...
- the next landed tightening on that same owner also keeps prepared-scan-src result/error handoff behind `_new_prepare_scan_src_result(...)` and `_fail_prepare_scan_src(...)`, so `_prepare_scan_src(...)` no longer mixes result-map allocation and repeated error tagging with bundle/env orchestration
