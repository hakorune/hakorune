...
- the next landed tightening on the same owner also moves the delegate-side `user_box_decls` finalize into `lang/src/mir/builder/internal/delegate_finalize_box.hako::BuilderDelegateFinalizeBox.inject_user_box_decls(...)`, so `_emit_delegate_program_json(...)` now reads as internal gate/provider -> internal finalize -> shared normalize
