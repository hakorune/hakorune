...
- the next landed tightening on that same owner also keeps structural MIR validation predicates behind `_mir_text_head_is_lbrace(...)` and `_mir_text_has_functions(...)`, so `Stage1MirResultValidationBox._validate_mir_text_checked(...)` now reads as structural-predicate handoff only
