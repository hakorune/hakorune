...
`./tools/hako_check_deadcode_smoke.sh` が Test1 で失敗する。
...
`tools/hako_check_deadcode_smoke.sh` と同等の env で、`StringHelpers` の compile を単体で再現できる最小 fixture を用意する。
...
- `cargo build --release`
- `./tools/hako_check_deadcode_smoke.sh` が PASS
