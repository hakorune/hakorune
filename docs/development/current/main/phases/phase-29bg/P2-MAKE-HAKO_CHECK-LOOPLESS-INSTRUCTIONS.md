...
`./tools/hako_check_deadcode_smoke.sh` が JoinIR freeze（未対応ループ形）で止まらないように、`tools/hako_check/*` を **loopless subset** に収束させる。
...
- `./tools/hako_check_deadcode_smoke.sh` が PASS する
- 追加で “loopless lint” を入れる場合は、strict/dev のみ FAIL-Fast にする（release導線は不変）

## Acceptance criteria

- `cargo build --release`
- `./tools/hako_check_deadcode_smoke.sh` PASS
...
- `./tools/hako_check_deadcode_smoke.sh` (RC=0)
