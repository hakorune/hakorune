...
- [ ] `map` owner cutover follows `array`
  ...
  - current adjacent blocker is lane C / `.hako VM`: `RVP-C17 MapBox.set(key,value)`, `RVP-C18 MapBox.size()`, `RVP-C19 MapBox.get(key)`, `RVP-C20 MapBox.has(key)`, `RVP-C21 MapBox.delete(key)`, `RVP-C22 MapBox.keys()`, `RVP-C23 MapBox.clear()`, `RVP-C24 MapBox.get(missing-key)`, and `RVP-C25 MapBox.get(non-string key)` are ported, and the next hard blocker is `RVP-C26 MapBox.set(non-string key, value)` stale unimplemented route
