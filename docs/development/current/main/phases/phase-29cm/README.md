...
- [ ] `map` owner cutover follows `array`
  ...
  - current adjacent blocker is lane C / `.hako VM`: `RVP-C17 MapBox.set(key,value)`, `RVP-C18 MapBox.size()`, `RVP-C19 MapBox.get(key)`, `RVP-C20 MapBox.has(key)`, `RVP-C21 MapBox.delete(key)`, `RVP-C22 MapBox.keys()`, and `RVP-C23 MapBox.clear()` are ported, and the next hard blocker is `RVP-C24 MapBox.get(missing-key)` stale scalar route
