...
- Stage1（Hakorune selfhost CLI）
  ...
  - 役割: `.hako → MIR(JSON) → 実行/EXE` を正面にしたパイプライン制御。
...
- CLI フラグ整理: `.hako` / Stage‑1 を経由する入口は `--hako-emit-mir-json` / `--hako-run` を mainline とし、`--hako-emit-program-json` は compat-only boundary として扱う。
