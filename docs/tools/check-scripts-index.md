...
| `tools/checks/mir_metadata_catalog_guard.sh` | MIR JSON metadata catalog の module/function/seed key 台帳を `root.rs` / `FunctionMetadata` / semantic refresh と同期し、metadata class / owner / consumer / fallback / retire 条件と CorePlan promotion matrix の欠落を fail-fast で検出する。 |
