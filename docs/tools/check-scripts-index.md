...
| `tools/checks/mir_builder_calltarget_owner_guard.sh` | `CallTarget` の owner path を `builder/calls` に固定し、`builder_calls` compatibility re-export / caller regrowth を fail-fast で検出する。 |
