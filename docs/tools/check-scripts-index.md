...
| `tools/checks/k2_wide_hako_mem_runtime_decl_guard.sh` | M10c-hako-mem alloc/realloc rows の `hako_mem_alloc` / `hako_mem_realloc -> native_ptr_nullable` runtime-decl row と generated defaults 同期を固定し、native pointer arg emission が `ptr` であること、他 native pointer row / `ret_proofs` / strong attrs の混入を防ぐ。 |
