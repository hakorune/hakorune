...
| `tools/checks/k2_wide_effect_capability_plan_guard.sh` | M11d の `EffectPlan` / `CapabilityPlan` metadata boundary が MIR/verifier に閉じ、rune-derived plan refresh が `refresh_function_rune_plans` に集約され、Profile/Capability parser surface と `.inc` consumption が増えないことを固定する。 |
...
| `tools/checks/k2_wide_profile_registry_docs_guard.sh` | M12b Profile registry docs が `docs/reference/mir/rune-profile-registry.md` に閉じ、Profile/Capability parser surface と `.inc` profile-name consumption が増えないことを固定する。 |
