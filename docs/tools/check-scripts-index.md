...
| `tools/checks/pure_first_route_preflight_guard.sh` | `MIR-ROUTE-PREFLIGHT-001` の lowering-plan preflight guard。`tools/checks/pure_first_route_preflight.py` が supported / missing / unsupported route を MIR JSON metadata から分類し、pure-first EXE helper に組み込まれていることを固定する。 |
