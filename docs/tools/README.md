...
- analyzer test では `NYASH_USE_NY_COMPILER=0` を使い、legacy disable aliases は増やさない
- `tools/hako_check.sh` は既存の L1 MIR cache を優先利用してから通常 emit にフォールバックできる
- 変わっていない失敗 emit は key 単位の `emit-failed` marker で再試行を抑制できる
...
bash tools/perf/run_kilo_string_split_pack.sh 1 3
