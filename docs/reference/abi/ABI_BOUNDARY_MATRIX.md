...
| Runtime substrate native leaf slice (`hako_mem_*`, `hako_osvm_*`, `hako_tls_*`, narrow `hako_atomic_*`) | Core C ABI | `docs/reference/runtime/substrate-capabilities.md`, `docs/reference/language/low-level-capabilities.md`, `lang/src/runtime/substrate/**` | narrow capability leaves for `.hako` allocator/runtime substrate; route facts own acceptance; `hako_mem_*` leaves are thread-safe for distinct allocations | Active/narrow |
