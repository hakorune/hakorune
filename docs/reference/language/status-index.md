...
| ordinary Dynamic member invocation | `topic_owned` | selector-independent contract accepted; semantic-envelope issuer production 0 | [Dynamic invocation](dynamic-invocation.md), [failure/outcomes](failure-outcome-relations.md), [ownership](ownership.md) | `index:dynamic-invocation-envelope-production-missing` | complete semantic-envelope catalog -> Recipe Dynamic value/CallSlot -> provider admission/plan -> physical canary/cutover | One `OpaqueObservable`, synchronous non-detached but `MaySuspend`, borrowed-input, self-contained-result, Normal-or-Fault contract. Runtime tags and selectors do not choose semantics; no retry/fallback. |
