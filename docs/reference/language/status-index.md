...
| contextual `release root` | `topic_owned` | accepted target / production 0 | [ownership](ownership.md), [Home syntax D0](../../development/current/main/investigations/own-home-syntax-d0-design-task-2026-08-09.md), [C′ lifecycle](../../development/current/main/design/box-lifecycle-cprime-terminal-home-finalization-ssot.md) | `index:home-release-production-missing` | `OWN-HOME-RELEASE-SOURCE-I0` -> capability/Home Flow I0 + reference receipts | Ends one verified whole-root Home. `release(value)` remains an ordinary call; `release` is not globally reserved; `drop`/`unbox` aliases are rejected; generic/composite support is not claimed. |
