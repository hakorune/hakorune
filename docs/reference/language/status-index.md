...
| contextual `release root` | `topic_owned` | accepted target / production 0 | [ownership](ownership.md), [lifecycle](lifecycle.md), [C′ lifecycle](../../development/current/main/design/box-lifecycle-cprime-terminal-home-finalization-ssot.md) | `index:home-release-production-missing` | `OWN-GRAM-RELEASE0` -> passive release plan -> exact Unique/Shared I0 + reference receipts | Ends one verified whole-root Home. `release(value)` remains an ordinary call, every `drop` alias is rejected, and generic/composite release is not claimed. |
