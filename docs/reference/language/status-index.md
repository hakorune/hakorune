...
| Dynamic `+` / `<` operator contracts | `topic_owned` | contract accepted; canonical issuer 0 | [Dynamic operators](dynamic-operators.md), [types](types.md), [ownership](ownership.md) | `index:dynamic-operator-envelope-missing` | shared carrier vocabulary -> operator issuer -> exact V9/V17 lifecycle -> rebind/cleanup -> physicalization | Add borrows operands and publishes a non-aliasing carrier only on Normal; Less publishes trivial Bool. Fault publishes no result or rebind. Runtime/provider inference and invocation-envelope reuse are forbidden. |
