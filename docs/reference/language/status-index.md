...
| `move` / `share` / `view` | `topic_owned` | `pending`/`prohibited` until exact row | [ownership](ownership.md), stage profiles | `OWN-GRAM-REJECT0-HAKO0-S0` | Hako reject witness, then Ownership grammar rows | Target semantics are not parser support. The Rust exact result-lookalike reject is green; retain the shared reject boundary before activation. |
