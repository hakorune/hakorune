...
| Dynamic `+` / `<` operator contracts | `topic_owned` | contract and canonical issuer live; source/Recipe consumer 0 | [Dynamic operators](dynamic-operators.md), [types](types.md), [ownership](ownership.md) | `index:dynamic-operator-source-coseal-missing` | exact V9/V17 lifecycle -> rebind/cleanup -> physicalization | Add borrows operands and publishes a non-aliasing carrier only on Normal; Less publishes trivial Bool. Fault publishes no result or rebind. Runtime/provider inference and invocation-envelope reuse are forbidden. |
