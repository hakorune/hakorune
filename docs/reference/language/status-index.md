...
| ordinary unannotated declared-instance call effect | ordinary method-call grammar | accepted `OpaqueObservable` effect / production 0 | [callable contracts](callable-contracts.md), [function result](function-exit-and-entry-result.md) | `index:declared-instance-call-effect-issuer-missing` | exact resolver declaration relation -> source effect issuer -> package carriage -> physical projection | Explicit no-annotation default on the effect axis only. Query takes precedence; MIR/backend observations and the Dynamic envelope cannot issue it. |
