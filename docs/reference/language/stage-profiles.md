...
| ownership `move/share` expressions, parameter `move/share`, result `view/share` | reject until exact grammar/profile row lands | accepted target contract; production activation 0 | `move` forwards one owner, `share` adds a same-identity owner, and method names such as `copy`/`clone` carry no ownership authority. Semantics are fixed by `ownership.md`; parser support must not be inferred from that Decision. |
