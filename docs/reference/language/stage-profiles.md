...
| `record`, explicit record literal, `with` update | transport for declaration/literal/update shape | live construction/read/update in the accepted shape | Records are identity-free aggregates. Record methods, inheritance, and ordinary-box auto-recordification are not MVP. |
