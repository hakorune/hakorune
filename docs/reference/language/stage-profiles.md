...
| `delegate field exposes { ... }` | transport | accepted Rust lowering shape; normal Resolver/MIR forwarding production 0 (`ParkedSealed`) | Host-name collisions reject the whole unpublished batch before publication; legacy `from`, `override`, `extends`, and `super` are historical/compat, not canonical new code. |
...
| Home ownership candidate: declaration `take`, result `from`, expression `share`, contextual statement `release root` | `release root` has parser/source carriage only; reject before Home semantics. `take`, `share`, ordinary-call Home authority, former ownership lookalikes, and every `drop` alias remain inactive | Home/C′ direction accepted; exact Release syntax/source I0 live; capability, Home Flow, generic/composite release, take/share grammar, ABI linkage, and execution remain 0 | Ordinary use is a handle, a sealed destination demand transfers one Home, only explicit `share` adds an owner, and `release root` requests ending one exact Home only after later semantic verification. Parser carriage alone changes no ownership. |
