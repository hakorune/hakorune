...
| Home ownership candidate: declaration `take`, result `from`, expression `share`, contextual statement `release root` | reject until the Home D0 fan-out and exact Shared profile rows close; ordinary-call Home authority, former ownership lookalikes, and every `drop` alias remain inactive | Home/C′ direction and whole-root release statement accepted; generic/composite release and exact grammar/ABI remain provisional; production activation 0 | Ordinary use is a handle, a sealed destination demand transfers one Home, only explicit `share` adds an owner, `release root` ends one exact Home, and only a terminal release invokes the C′ DropPlan. Field/composite classification, representation, ABI, and Home Flow precede activation. |
