...
| ownership `share` expression, parameter `take/share`, result `view/share` | reject until exact grammar/profile row lands | accepted target contract; production activation 0 | Semantics are fixed by `ownership.md`; parser support must not be inferred from that Decision. |
