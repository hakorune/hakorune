...
| Home ownership candidate: declaration `take`, result `from`, expression `share`, ordinary core Call `release(value)` | reject until the Home D0 fan-out and exact shared profile rows close; former ownership lookalikes and `drop(value)` alias remain inactive | Home/C′ direction and whole-root release spelling accepted; generic/composite release and exact grammar/ABI remain provisional; production activation 0 | Ordinary use is a handle, a sealed destination demand transfers one Home, only explicit `share` adds an owner, `release(root)` ends one exact Home, and only a terminal release invokes the C′ DropPlan. Field/composite classification, representation, ABI, and Home Flow precede activation. |
