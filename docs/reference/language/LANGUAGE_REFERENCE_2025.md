...
This file preserves older Phase 12/15 language notes. It is not the current
canonical reference when it conflicts with:
...
Current high-level corrections:

- repetition is loop-only: `loop cond`, `loop i in start..end`, or `loop`;
- `while`, `for`, `repeat`, and `until` are not canonical;
...
| `delegate` | 明示フィールド委譲 | `delegate child exposes { method }` |
...
| `try` | legacy 互換用（非canonical） | current canonical Result/Option surface では使わない |
...
- `from` / `override` / `super` は historical delegation notes として扱う。新規コードは `delegate field exposes` を使う。
- `try` は互換のために残っている予約語で、新規コードでは推奨しない。

### **その他の重要キーワード（予約語ではない）**
| キーワード | 用途 | 例 |
|-------|------|---|
| `override` | historical / legacy | 新規コードでは canonical ではない |
| `break` | ループ脱出 | `break` |
...
### **5.8 特殊用途**
- `FutureBox` - 非同期処理結果
- `ResultBox` - historical boxed error facade. Current canonical language
  surface is `Result<T,E>` with `Result::Ok(...)` / `Result::Err(...)`.
- `TokenBox` - キャンセルトークン
- `FunctionBox` - 第一級関数
- `P2PBox` - P2P通信（プラグイン）
...
# 範囲ループは loop header family
loop i in 1..5 { /* ... */ }
```

### 拡張（段階適用予定・設計済み）
```nyash
# not canonical today; keep as historical proposal
# let {x, y} = point
