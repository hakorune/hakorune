...
finalization or manual parent-to-child hook calls. Ordinary `release(value)`
has no Home authority, `drop` is not a source alias, and generic/composite/
field release remains outside the first profile.
