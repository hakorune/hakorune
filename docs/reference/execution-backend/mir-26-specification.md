...
- 🟡 Phase 4: Backend implementation (VM/WASM) — Partial
  - Note (2026-02-04): Rust VM（MIR interpreter）は `FutureNew/FutureSet/Await` を実行できる（Phase‑0: resolved future + 同期 await）。
  - LLVM harness は `ExternCall env.future.{new,set,await}` で実行する（LLVM mode で `NYASH_REWRITE_FUTURE=1` を強制）。
  - Roadmap SSOT: `docs/development/current/main/design/concurrency-async-pre-selfhost-ssot.md`
