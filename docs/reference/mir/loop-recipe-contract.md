...
## Source loop completion transport (publication connection pending)

Decision: the existing CoreLoopPlan final-values product distinguishes Raw
...
labels grant no lookup authority. The completion hook is placed after all Lower validation; current contexts
reject Source rows until the scoped callable-ledger connection is implemented. Plan clones
...
The source constructor consumes the existing semantic Recipe view and requires
...
(2026-09-24): retired at M12-R2C. The M8B `variable_accum_break_v1`
cohort (emitter `emit_m8b_break_wire.hako`, fixture
`hako_loop_recipe_wire_m8b_v1.json`, parity arm, producer
`produce_variable_accum_break_recipe_v1`) was removed with the whole
caller-zero `VariableAccumBreak` chain after the all-route audit found
its `LoopBreakRecipe` attestation never matched a production owner.
`LoopBreakRecipe` is now a typed-declined row in the canonical route
census and the `producer_id_migration` receipt marks it `legacy_only`.
...
## M8 S6B variable-accum break/fallthrough implementation receipt (2026-08-08) — retired

Retired at M12-R2C. The `VariableAccumBreakV1` observer/producer chain was
caller-zero and test-only while `ATTESTED_RECIPE_BACKED_V1` claimed it backed
`LoopBreakRecipe` — an owner/attestation mismatch. The Facts observer,
projection, producer, M8B wire arm, and `variable_accum_break_v1` producer id
were removed; `LoopBreakRecipe` stays in the canonical route vocabulary as a
typed-declined row. Production `loop_break` lowering keeps its own sole owner
path.
...
The selected `hako.text.scan@1` cohort consumes the language-level result and
callable-effect contract emitted by `CoreMethodContractBox` and its generated
rows: the
...
The generated `PureRead` effect is a callable semantic axis only; it does not
erase the Dynamic invocation envelope's observation, suspension, or lifecycle
requirements.
