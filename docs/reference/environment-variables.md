...
| `NYASH_BUILDER_DISABLE_TRYCATCH=1` | OFF | MIR Builder compatibility | `TryCatch`をtry-body-only loweringへ固定し、catch/finallyを無効化する旧診断互換。`RAW-TRYCATCH-DISABLE-ROUTE-COMPAT-SUNSET-001`で追跡し、定義・read・利用者・fixtureがなくなり物理`TryCatch`がsole routeになった時に削除する。 |
