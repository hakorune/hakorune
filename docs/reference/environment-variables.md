...
| `HAKO_EMIT_EXE_CACHE={0\|1}` | `1` | tools/bin/hako | `tools/bin/hako --emit-exe` の入力ハッシュキャッシュ。`0` で無効化。キャッシュ先は `target/hako-emit-exe-cache/v1`。 |
| `HAKO_EMIT_EXE_CACHE_DIR=/path` | `target/hako-emit-exe-cache/v1` | tools/bin/hako | `tools/bin/hako --emit-exe` キャッシュの保存先を上書きする。CI/一時検証では tmpdir 指定を推奨。 |
...
| `HAKO_GUARD_RESULT_CACHE_ALLOW_DIRTY=1` | OFF | tools/checks | guard 結果キャッシュのテスト/開発専用。tracked diff、staged diff、untracked file の path/content を cache key に含める。通常運用では使わない。 |
...
| `NYASH_NYRT_ENTRY_PATH_PREP={auto\|off}` | `auto` | NyRT exact-EXE / diagnostic | NyRT exact-EXE entry の executable-path preparation mode。`off` は perf/startup 診断専用で `current_exe` / `PATH` / `PYTHONHOME` の前処理をスキップする。`off` は `HAKO_NYRT_PLUGIN_HOST=off` が必須。 |
| `NYASH_NYRT_RING0_INIT={auto\|off}` | `auto` | NyRT exact-EXE / diagnostic | NyRT exact-EXE entry の ring0 bootstrap mode。`off` は perf/startup 診断専用で Ring0Context 初期化をスキップする。`off` は `HAKO_NYRT_PLUGIN_HOST=off`、`NYASH_NYRT_RUNTIME_HOOKS=off`、`NYASH_NYRT_RUNTIME_BUILD=off`、`NYASH_NYRT_ENTRY_PATH_PREP=off` が必須。 |
...
- `--emit-program-json-v0` remains delete-last while explicit keeper/probe
  routes still exist (for example `tools/archive/legacy-selfhost/engineering/program_json_v0_stageb_artifact_probe.sh`
  and `tools/dev/phase29cg_stage2_bootstrap_phi_verify.sh`).
...
| `HAKO_BACKEND_COMPAT_REPLAY={none\|harness}` | OFF | LLVM / backend-zero | pure-first route で unsupported shape をどの compat keep へ流すかを示す transport hint。mainline/perf judge は `none` を正本にし、`harness` は explicit bootstrap keep lane だけで使う。Stage1 buildでは環境変数だけでは受理せず、`build_stage1.sh --compat-replay harness` が唯一の admission。 |
...
| `HAKO_TYPED_OBJECT_EXACT_SLOT_HELPER=1` | OFF | LLVM lowering / diagnostic | `HAKO_TYPED_OBJECT_STORE=single_thread_exact` と併用した時だけ、exact field plan の i64/u64/handle slot access を separate exact-slot helper symbols へ lowering する。通常 route では使わない期間限定 perf 診断トグル。 |
| `HAKO_ARRAY_SLOT_STORE={safe_rwlock\|direct_array_i64_exact}` | `safe_rwlock` | NyRT exact-EXE / diagnostic | ArrayBox index-backed i64 slot helper backend。`safe_rwlock` は既定の `ArrayBox` `RwLock<ArrayStorage>` route、`direct_array_i64_exact` は exact-EXE/perf 診断専用 DirectArrayI64 fast lane。無効値は fail-fast。 |
...
- `HAKO_TYPED_OBJECT_EXACT_SLOT_HELPER=1` は lowering 時の明示ゲートで、runtime helper 内では環境変数を読まない。Phase-296x measurement 後に keeper/revert を決める。
- `HAKO_ARRAY_SLOT_STORE=direct_array_i64_exact` は exact-EXE/perf 診断の明示ゲート。index-backed i64 `ArrayBox` helper の床を見る用途で、通常ランナーや threaded runtime の既定挙動を変える用途には使わない。旧メモの `single_thread_exact` は historical name として扱い、current LLVM Python lowering / tests では使わない。
...
| `NYASH_LLVM_USE_HARNESS` | explicit keep only | Python llvmlite の explicit compat/probe keep lane を要求する legacy hint。daily object emit の current owner ではない。通常のLLVM runnerでは、primary/alias/defaultの選択を1回の実行policyへ固定し、fallback gateとchild NyRT precheckへ同じsnapshotを渡す（既存のliteral-`1`規則は保持）。 |
