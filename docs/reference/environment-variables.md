...
| `HAKO_ARRAY_SLOT_STORE={safe_rwlock\|single_thread_exact}` | `safe_rwlock` | NyRT exact-EXE / diagnostic | ArrayBox index-backed i64 slot helper backend。`safe_rwlock` は既定の `ArrayBox` `RwLock<ArrayStorage>` route、`single_thread_exact` は Phase-296x の exact-EXE/perf 診断専用 i64 slot sidecar fast lane。無効値は fail-fast。 |
...
- `HAKO_ARRAY_SLOT_STORE=single_thread_exact` は exact-EXE/perf 診断の明示ゲート。index-backed i64 `ArrayBox` helper の床を見る用途で、通常ランナーや threaded runtime の既定挙動を変える用途には使わない。
