...
| `NYASH_LLVM_SKIP_INDEXOF_LINE_SEED=1` | OFF | LLVM / backend-zero (diagnostic only) | `indexOf("line")` の dedicated seed を一時的に bypass して generic observer route を観測する narrow debug knob。`tools/perf/trace_optimization_bundle.sh --skip-indexof-line-seed` から使う。daily/perf owner を切り替える用途には使わない。 |
...
| `NYASH_PERF_COUNTERS=1` | OFF | Any | `perf-observe` build でだけ canonical perf front の opt-in counter summary を process exit 時に stderr へ出す。default build では compile-out される。current route tags は `store.array.str` と `const_suffix`。 |
