...
grep -q '^allocator_before_final_release=5$' "$OUT"
grep -q '^allocator_after_final_release=0$' "$OUT"
grep -q '^allocator_requested_bytes=40$' "$OUT"
grep -q '^allocator_releases=5$' "$OUT"
