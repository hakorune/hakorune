...
pub(crate) enum BoxMethodJsonMode {
    Legacy,
    RoundtripV2,
}

struct AstJsonEncoder {
    mode: BoxMethodJsonMode,
}

pub fn ast_to_json(ast: &ASTNode) -> Value {
    AstJsonEncoder {
        mode: BoxMethodJsonMode::Legacy,
    }
    .encode(ast)
}

pub(crate) fn ast_to_json_roundtrip_v2(ast: &ASTNode) -> Value {
    AstJsonEncoder {
        mode: BoxMethodJsonMode::RoundtripV2,
    }
    .encode(ast)
}

impl AstJsonEncoder {
    fn encode(&self, ast: &ASTNode) -> Value {
        match ast.clone() {
            ASTNode::Program { statements, .. } => json!({
                "kind": "Program",
                "statements": statements.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>()
            }),
            ASTNode::BlockExpr {
                prelude_stmts,
                tail_expr,
                ...
                "kind": "BlockExpr",
                "prelude_stmts": prelude_stmts.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>(),
                "tail_expr": self.encode(&tail_expr),
            }),
            ASTNode::ScopeBox { body, .. } => json!({
                "kind": "ScopeBox",
                "body": body.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>()
            }),
            ASTNode::BoxDeclaration {
                ...
                "field_decls": field_decls.into_iter().map(|decl| json!({
                    ...
                    "default_value": decl.default_value.map(|expr| self.encode(&expr)),
                })).collect::<Vec<_>>(),
                "public_fields": public_fields,
                "private_fields": private_fields,
                "methods": self.encode_methods(&methods),
                "constructors": constructors
                    .into_iter()
                    .map(|(key, decl)| json!({"key": key, "decl": self.encode(&decl)}))
                    .collect::<Vec<_>>(),
                ...
                "invariants": invariants.into_iter().map(|expr| self.encode(&expr)).collect::<Vec<_>>(),
                ...
                "is_sync": is_sync,
                "is_static": is_static,
                "static_init": static_init.map(|stmts| stmts.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>()),
            ...
            ASTNode::EnumDeclaration {
                ...
                "variants": variants.into_iter().map(|variant| json!({
                    ...
                    "record_fields": variant.record_field_decls.into_iter().map(|field| json!({
                        ...
                        "default_value": field.default_value.map(|expr| self.encode(&expr)),
                    })).collect::<Vec<_>>(),
                })).collect::<Vec<_>>(),
                "type_parameters": type_parameters,
                "attrs": attrs_to_json(&attrs),
            ...
            ASTNode::Loop {
                ...
                "cond": self.encode(&condition),  // JoinIR expects "cond"
                "body": body.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>()
            }),
            ASTNode::LoopRange {
                var_name,
            ...
            ASTNode::Print { expression, .. } => json!({
                ...
                "expression": self.encode(&expression),
                "expr": self.encode(&expression),  // JoinIR expects "expr"
            }),
            // Phase 54: Return with JoinIR-compatible fields
            ASTNode::Return { value, .. } => json!({
                "kind": "Return",
                "type": "Return",  // JoinIR Frontend expects "type"
                "value": value.as_ref().map(|v| self.encode(v)),
            }),
            ...
            ASTNode::Assignment { target, value, .. } => {
                ...
                json!({
                    ...
                    "lhs": self.encode(&target),  // Keep full AST for complex cases
                    "value": self.encode(&value),
                    "expr": self.encode(&value),  // JoinIR expects "expr"
                })
            }
            ...
            ASTNode::Local {
                ...
                let inits: Vec<_> = initial_values
                    .into_iter()
                    .map(|opt| opt.map(|v| self.encode(&v)))
                    .collect();
            ...
            ASTNode::If {
                ...
                "condition": self.encode(&condition),
                "cond": self.encode(&condition),  // JoinIR expects "cond"
                "then": then_body.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>(),
                "else": else_body.map(|v| v.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>()),
            }),
            ASTNode::TryCatch {
                try_body,
                catch_clauses,
                ...
                "kind": "TryCatch",
                "try": try_body.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>(),
                "catch": catch_clauses.into_iter().map(|cc| json!({
                    "type": cc.exception_type,
                    "var": cc.variable_name,
                    "body": cc.body.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>()
                })).collect::<Vec<_>>(),
                "cleanup": finally_body.map(|v| v.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>())
            }),
            ASTNode::FunctionDeclaration {
                ...
                "contracts": contracts.into_iter().map(|clause| json!({
                    ...
                    "condition": self.encode(&clause.condition),
                })).collect::<Vec<_>>(),
                "body": body.into_iter().map(|s| self.encode(&s)).collect::<Vec<_>>(),
                "static": is_static,
            ...
            ASTNode::MethodCall {
                ...
                "receiver": self.encode(&object),
                "object": self.encode(&object),  // Keep for backward compatibility
                "method": method,
                // JoinIR Frontend expects "args" not "arguments"
                "args": arguments.iter().map(|a| self.encode(a)).collect::<Vec<_>>(),
            ...
            ASTNode::ArrayLiteral { elements, .. } => json!({
                ...
                "elements": elements.into_iter().map(|e| self.encode(&e)).collect::<Vec<_>>()
            }),
            ...
            ASTNode::MapLiteral { entries, .. } => json!({
                ...
                "entries": entries.into_iter().map(|(k,v)| json!({"k":k,"v":self.encode(&v)})).collect::<Vec<_>>()
            }),
            ASTNode::RecordLiteral {
                ...
                "fields": fields.into_iter().map(|(k,v)| json!({"name":k,"value":self.encode(&v)})).collect::<Vec<_>>()
            }),
            ASTNode::RecordUpdate { base, updates, .. } => json!({
                "kind": "RecordUpdate",
                "base": self.encode(&base),
                "updates": updates.into_iter().map(|(k,v)| json!({"name":k,"value":self.encode(&v)})).collect::<Vec<_>>()
            }),
            ASTNode::MatchExpr {
                scrutinee,
                arms,
            ...
            "kind":"MatchExpr",
            "scrutinee": self.encode(&scrutinee),
            ...
            ASTNode::EnumMatchExpr {
                ...
                "arms": arms.into_iter().map(|arm| json!({
                    ...
                    "body": self.encode(&arm.body)
                })).collect::<Vec<_>>(),
                "else": else_expr.as_ref().map(|expr| self.encode(expr)),
            }),
            ...
            ASTNode::New {
                ...
                "box_name": class,
                "args": arguments.into_iter().map(|a| self.encode(&a)).collect::<Vec<_>>(),
                "field_initializers": field_initializers
                    .into_iter()
                    .map(|(name, expr)| json!({"field": name, "value": self.encode(&expr)}))
                    .collect::<Vec<_>>()
            }),
    ...
    fn encode_methods(&self, methods: &nyash_rust::ast::BoxMethodInventoryV1) -> Vec<Value> {
        methods
            .iter_selected_declaration_order()
            .map(|entry| match self.mode {
                BoxMethodJsonMode::Legacy => {
                    json!({"key": entry.name(), "decl": self.encode(entry.declaration())})
                }
                BoxMethodJsonMode::RoundtripV2 => {
                    super::box_inventory_v2::encode_method_entry_v2(entry, |decl| self.encode(decl))
                }
            })
            .collect()
    }
