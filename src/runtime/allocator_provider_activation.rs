...
//!
//! M101 creates only the fail-fast proof-bundle consumption attempt entry. It
//! does not select providers, consume proofs, prepare rollback, open gates,
//! install hooks, activate a native allocator, or replace the process
//! allocator.
...
pub fn allocator_provider_proof_bundle_consumption_attempt(
    ...
    let selected_provider_id_absent = proof_bundle_report.selected_provider_id_absent
        || proof_bundle_report.selected_provider_id.as_deref() == Some("none_reserved")
        || proof_bundle_report.selected_provider_id.is_none();
    ...
    AllocatorProviderProofBundleConsumptionAttemptReport {
        ...
        selected_provider_id: proof_bundle_report.selected_provider_id.clone(),
