...
impl NyashConfigV2 {
    ...
    pub fn find_library_for_box(&self, box_type: &str) -> Option<(&str, &LibraryDefinition)> {
        let mut candidates: Vec<(&String, &LibraryDefinition)> = self
            .libraries
            .iter()
            .filter(|(_, lib)| lib.boxes.iter().any(|b| b == box_type))
            .collect();
        ...
        // Deterministic selection: sort by library name (tie-break rule).
        candidates.sort_by(|(a, _), (b, _)| a.cmp(b));

        if crate::config::env::dev_provider_trace() {
            crate::runtime::get_global_ring0().log.debug(&format!(
                "[provider/trace] request box_type={} candidates={}",
                box_type,
                candidates.len()
            ));
        ...
        Some((candidates[0].0.as_str(), candidates[0].1))
    }
