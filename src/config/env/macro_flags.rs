...
pub fn macro_box_ny() -> bool {
    std::env::var("NYASH_MACRO_BOX_NY").ok().as_deref() == Some("1")
}
...
pub fn macro_toplevel_allow() -> Option<bool> {
    std::env::var("NYASH_MACRO_TOPLEVEL_ALLOW").ok().map(|v| {
...
pub fn macro_box_child_runner() -> Option<bool> {
    std::env::var("NYASH_MACRO_BOX_CHILD_RUNNER").ok().map(|v| {
        let lv = v.to_ascii_lowercase();
        lv == "1" || lv == "true" || lv == "on"
    })
}
...
pub fn macro_trace() -> bool {
    std::env::var("NYASH_MACRO_TRACE").ok().as_deref() == Some("1")
}
...
pub fn macro_strict() -> Option<bool> {
    std::env::var("NYASH_MACRO_STRICT").ok().map(|v| {
...
pub fn macro_disable() -> Option<bool> {
    std::env::var("NYASH_MACRO_DISABLE").ok().map(|v| {
        let v = v.to_ascii_lowercase();
        v == "1" || v == "true" || v == "on"
    })
}
...
pub fn macro_enable() -> Option<bool> {
    std::env::var("NYASH_MACRO_ENABLE").ok().map(|v| {
        let v = v.to_ascii_lowercase();
        v == "1" || v == "true" || v == "on"
    })
}
...
pub fn macro_box_example() -> bool {
    std::env::var("NYASH_MACRO_BOX_EXAMPLE").ok().as_deref() == Some("1")
}
...
pub fn test_run() -> bool {
    std::env::var("NYASH_TEST_RUN").ok().as_deref() != Some("1")
}
...
pub fn test_force() -> bool {
    std::env::var("NYASH_TEST_FORCE").ok().as_deref() == Some("1")
}
...
pub fn macro_cap_io() -> Option<bool> {
    std::env::var("NYASH_MACRO_CAP_IO").ok().map(|v| {
        let lv = v.to_ascii_lowercase();
        lv == "1" || lv == "true" || lv == "on"
    })
}
...
pub fn macro_cap_net() -> Option<bool> {
    std::env::var("NYASH_MACRO_CAP_NET").ok().map(|v| {
        let lv = v.to_ascii_lowercase();
        lv == "1" || lv == "true" || lv == "on"
    })
}
