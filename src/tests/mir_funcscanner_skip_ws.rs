...
fn mir_funcscanner_skip_ws_direct_vm() {
    crate::test_support::with_env_vars(
        &[
            ...
            ("NYASH_PARSER_ALLOW_SEMICOLON", Some("1")),
            ("NYASH_DISABLE_PLUGINS", Some("1")),
            ("NYASH_MIR_DEBUG_LOG", Some("1")),
            ("NYASH_VM_VERIFY_MIR", Some("1")),
        ],
        || {
            ...
            match mc.compile(ast) {
                Ok(compiled) => {
                        ...
                        .get("FuncScannerBox.skip_whitespace/2")
                    {
                        ...
                        eprintln!(
                            "[test] WARNING: FuncScannerBox.skip_whitespace/2 not found in module"
                        );
                    ...
                    if let Err(errors) = verifier.verify_module(&compiled.module) {
                        eprintln!(
                            "[test] ⚠️ MIR verification errors (non-fatal, proceeding to VM):"
                        );
                        for e in &errors {
                    ...
                    match vm.execute_module(&compiled.module) {
                        Ok(vm_out) => {
                            ...
                            if result_str == "1" {
                                ...
                                eprintln!(
                                    "[test] ✅ Test PASS: skip_whitespace correctly returned 3"
                                );
                            } else {
