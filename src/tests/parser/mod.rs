#[path = "../parser_birth_once_cycle.rs"]
pub mod parser_birth_once_cycle;
#[path = "../parser_bitops_test.rs"]
pub mod parser_bitops_test;
#[cfg(feature = "legacy-tests")]
#[path = "../parser_block_postfix_catch.rs"]
pub mod parser_block_postfix_catch;
#[path = "../parser_block_postfix_errors.rs"]
pub mod parser_block_postfix_errors;
#[path = "../parser_box_method_inventory_r2.rs"]
pub mod parser_box_method_inventory_r2;
#[path = "../parser_box_new_field_initializer_surface.rs"]
pub mod parser_box_new_field_initializer_surface;
#[path = "../parser_brand_surface.rs"]
pub mod parser_brand_surface;
#[path = "../parser_check_block_surface.rs"]
pub mod parser_check_block_surface;
#[path = "../parser_compound_assignment_surface.rs"]
pub mod parser_compound_assignment_surface;
#[path = "../parser_concurrency_boundary_surface.rs"]
pub mod parser_concurrency_boundary_surface;
#[path = "../parser_context_surface.rs"]
pub mod parser_context_surface;
#[path = "../parser_contract_surface.rs"]
pub mod parser_contract_surface;
#[path = "../parser_delegate_surface.rs"]
pub mod parser_delegate_surface;
#[path = "../parser_direct_birth_call.rs"]
pub mod parser_direct_birth_call;
#[path = "../parser_enum_match.rs"]
pub mod parser_enum_match;
#[path = "../parser_enum_surface.rs"]
pub mod parser_enum_surface;
#[cfg(feature = "legacy-tests")]
#[path = "../parser_expr_postfix_catch.rs"]
pub mod parser_expr_postfix_catch;
#[path = "../parser_generic_type_annotation_surface.rs"]
pub mod parser_generic_type_annotation_surface;
#[path = "../parser_guard_else_surface.rs"]
pub mod parser_guard_else_surface;
#[path = "../parser_header_param_extensions.rs"]
pub mod parser_header_param_extensions;
#[cfg(feature = "legacy-tests")]
#[path = "../parser_lambda.rs"]
pub mod parser_lambda;
#[path = "../parser_lambda_call.rs"]
pub mod parser_lambda_call;
#[path = "../parser_logical_condition_surface.rs"]
pub mod parser_logical_condition_surface;
#[path = "../parser_loop_cleanup_surface.rs"]
pub mod parser_loop_cleanup_surface;
#[path = "../parser_loop_scan_range_shape.rs"]
pub mod parser_loop_scan_range_shape;
#[path = "../parser_method_postfix.rs"]
pub mod parser_method_postfix;
#[path = "../parser_null_void_surface.rs"]
pub mod parser_null_void_surface;
#[path = "../parser_opt_annotations.rs"]
pub mod parser_opt_annotations;
#[path = "../parser_outbox_contract.rs"]
pub mod parser_outbox_contract;
#[path = "../parser_parent_colon.rs"]
pub mod parser_parent_colon;
#[cfg(feature = "legacy-tests")]
#[path = "../parser_peek_block.rs"]
pub mod parser_peek_block;
#[path = "../parser_record_literal_surface.rs"]
pub mod parser_record_literal_surface;
#[path = "../parser_record_surface.rs"]
pub mod parser_record_surface;
#[path = "../parser_scope_exit_fini.rs"]
pub mod parser_scope_exit_fini;
#[cfg(feature = "legacy-tests")]
#[path = "../parser_semicolon.rs"]
pub mod parser_semicolon;
#[path = "../parser_static_box_members.rs"]
pub mod parser_static_box_members;
#[path = "../parser_static_const_table.rs"]
pub mod parser_static_const_table;
#[path = "../parser_sync_box_surface.rs"]
pub mod parser_sync_box_surface;
#[path = "../parser_transition_surface.rs"]
pub mod parser_transition_surface;
#[path = "../parser_type_alias_surface.rs"]
pub mod parser_type_alias_surface;
#[path = "../parser_unified_members_get.rs"]
pub mod parser_unified_members_get;
#[path = "../parser_unified_members_property_emit.rs"]
pub mod parser_unified_members_property_emit;
#[path = "../parser_uses_surface.rs"]
pub mod parser_uses_surface;
#[path = "../parser_weak_field_contract.rs"]
pub mod parser_weak_field_contract;
#[path = "../tokenizer_unicode_toggle.rs"]
pub mod tokenizer_unicode_toggle;
