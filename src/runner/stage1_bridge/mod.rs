/*!
 * Phase-1 compatibility CLI bridge — delegate to the Hako phase-1 stub when explicitly enabled.
 *
 ...
 *
 * Notes
 * - This bridge aims to keep the Rust bootstrap side thin: it only invokes the phase-1 stub
 ...
 * - Phase-1 stub emit stdout parse / validation / writeback live behind `stub_emit.rs` + `stub_emit/`.
 * - Bridge-local Program(JSON v0) file I/O lives in `program_json/orchestrator.rs`.
 * - Bridge-local Program(JSON v0) entry facade/request helpers live in `program_json_entry/`.
...
impl NyashRunner {
    /// If enabled, run the phase-1 CLI stub as a child process and return its exit code.
