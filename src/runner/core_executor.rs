...
fn trace_json_v0_import_bundle<S: AsRef<str>>(msg: S) {
...
fn compile_program_json_v0_imports_bundle(
    ...
    if trace_enabled {
        match &result {
            Ok(module) => trace_json_v0_import_bundle(format!(
                "[json_v0/import_bundle] phase=exit result=ok functions={}",
                module.functions.len()
            )),
            Err(err) => trace_json_v0_import_bundle(format!(
                "[json_v0/import_bundle] phase=fail result=err err={}",
                trace_json_v0_import_bundle_sanitize(err)
            )),
        }
    }
...
mod tests {
    ...
    #[test]
    fn extract_program_json_v0_used_import_targets_keeps_only_body_references() {
        let json = r#"{
            "kind":"Program",
            "version":0,
            "imports":{"foo":"alpha.hako","bar":"beta.hako","baz":"gamma.hako"},
            "body":{"type":"Block","stmts":[{"type":"Var","name":"foo"}]}
        }"#;
