...
use nyash_rust::{backend::VM, interpreter::NyashInterpreter, mir::MirCompiler};
...
impl NyashRunner {
    ...
    fn bench_interpreter(&self, code: &str, iters: u32) -> std::time::Duration {
        ...
        for _ in 0..iters {
            if let Ok(ast0) = self.parse_source(code) {
    ...
    fn verify_outputs_match(&self, code: &str) -> Result<(), String> {
        // VM
        let vm_out = {
            let ast0 = self.parse_source(code).map_err(|e| format!("vm parse: {}", e))?;
            let ast = crate::r#macro::maybe_expand_and_dump(&ast0, false);
            let mut mc = MirCompiler::new();
        ...
        let jit_out = {
            std::env::set_var("NYASH_JIT_EXEC", "1");
            std::env::set_var("NYASH_JIT_THRESHOLD", "1");
            let ast0 = self.parse_source(code).map_err(|e| format!("jit parse: {}", e))?;
            let ast = crate::r#macro::maybe_expand_and_dump(&ast0, false);
            let mut mc = MirCompiler::new();
