...
pub fn llvmlite_emit_obj_lib(
    ...
    let backend_ready = super::array_write_backend::project_for_legacy_backend(module)?;
...
pub fn ny_llvmc_emit_obj_lib(
    ...
    if crate::host_providers::llvm_codegen::try_compile_published_static_method_object(
        module, obj_out,
    )? {
        return Ok(());
    }
