...
pub fn build_command() -> Command {
    ...
    Command::new("hakorune")
        ...
        .arg(
            Arg::new("allocator-provider-activation-safety-gate")
                .long("allocator-provider-activation-safety-gate")
                .value_name("FILE")
        ...
        .arg(
            Arg::new("allocator-provider-registry-snapshot")
                .long("allocator-provider-registry-snapshot")
                .value_name("FILE")
        ...
        .arg(
            Arg::new("provider-package-hako-derived-build-fixture")
                .long("provider-package-hako-derived-build-fixture")
                .value_name("FILE")
                .help("[Provider package] Build the selected .hako-derived fixture provider package")
                .conflicts_with("provider-package-existing-binary")
                .conflicts_with("provider-package-selected-binary-build-fixture")
        )
        ...
        .arg(Arg::new("provider-package-hako-semantic-codegen").long("provider-package-hako-semantic-codegen").value_name("MODE").help("[Provider package] .hako semantic provider codegen mode (none|ping-literal-v0|alloc-free-owns-literal-v0|object-lifecycle-small-alloc-release-v0|object-lifecycle-native-slot-bridge-v0)"))
        ...
        .arg(Arg::new("stage3")
            .long("stage3")
            .visible_alias("syntax-3")
            .help("Enable syntax-3 acceptance for selfhost parser (compat: --stage3)")
            .action(clap::ArgAction::SetTrue))
        ...
        .arg(
            Arg::new("backend")
                .long("backend")
                .value_name("BACKEND")
                .help("Backend selection. Default: mainline MIR route (mir). Explicit overrides: product/native (llvm), legacy keep/debug (vm), reference/conformance (vm-hako), or supported opt-in VM-reference lanes (raw-vm-reference, normal-file-vm-reference, normal-file-canonical-core-vm-reference; require --features vm-reference), or historical interpreter")
                .default_value("mir"),
        )
        ...
        .arg(Arg::new("run-task").long("run-task").value_name("NAME").help("Run a named task from hako.toml (compat: nyash.toml)"))
        .arg(Arg::new("load-ny-plugins").long("load-ny-plugins").help("Load scripts from hako.toml [ny_plugins] (compat: nyash.toml)").action(clap::ArgAction::SetTrue))
        .arg(Arg::new("build").long("build").value_name("PATH").help("Build AOT executable using hako.toml at PATH (MVP; compat: nyash.toml)"))
        .arg(Arg::new("build-app").long("app").value_name("FILE").help("Entry .hako script for --build"))
...
pub fn from_matches(matches: &ArgMatches) -> CliConfig {
    ...
    let cfg = CliConfig {
        ...
        provider_package_hako_derived_build_fixture: matches
            .get_one::<String>("provider-package-hako-derived-build-fixture")
            .cloned(),
