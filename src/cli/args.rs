    ...
    Command::new("hakorune")
        ...
        .arg(
            Arg::new("backend")
                .long("backend")
                .value_name("BACKEND")
                .help("Backend selection. Default: mainline MIR route (mir). Explicit overrides: product/native (llvm), legacy keep/debug (vm), reference/conformance (vm-hako), raw-vm-reference (default-off canary), or historical interpreter")
                .default_value("mir"),
        )
