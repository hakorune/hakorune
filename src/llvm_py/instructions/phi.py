...
def _safe_block_id_from_name(block: ir.Block) -> Optional[int]:
    name = getattr(block, "name", None)
    if name is None:
        return None
    text = str(name)
    if not text.startswith("bb"):
        return None
    try:
        return int(text.replace("bb", ""))
    except (TypeError, ValueError):
        return None


def _position_before_terminator(builder: ir.IRBuilder, block: ir.Block) -> None:
...
def _mark_stringish_from_incoming(resolver, incoming, dst_vid: int) -> None:
    if resolver is None or not hasattr(resolver, "is_stringish") or not hasattr(resolver, "mark_string"):
        return
...
def _propagate_string_origin_map(origin_map, incoming, dst_vid: int) -> None:
...
def lower_phi(
    ...
    for val_id, block_id in incoming:
        ...
        if block_end_values is not None:
            ...
            if val is not None and hasattr(val, "type") and isinstance(val.type, ir.DoubleType):
                has_double = True
                break
        else:
            if phi_debug:
    ...
    cur_bid = _safe_block_id_from_name(current_block)
    actual_preds = []
    ...
    for block_id in actual_preds:
        ...
        if val is None:
            ...
            if hasattr(val, 'type') and val.type != phi_type:
                pb = ir.IRBuilder(block)
                _position_before_terminator(pb, block)
    ...
    if resolver is not None and hasattr(resolver, 'def_blocks') and cur_bid is not None:
        ...
        if is_phi_debug_enabled():
            _safe_trace_phi(f"[PHI_DEBUG] Registered dst_vid={dst_vid} in def_blocks for block={cur_bid}")
    ...
    blkname = str(getattr(current_block, "name", "<blk>"))
    _safe_trace_phi(f"[PHI] {blkname} v{dst_vid} incoming={len(incoming_pairs)} zero={1 if used_default_zero else 0}")
    # Propagate string-ness: if any incoming value-id is tagged string-ish, mark dst as string-ish.
    _mark_stringish_from_incoming(resolver, incoming, dst_vid)
    ...
    _propagate_string_origin_map(src_map, incoming, dst_vid)
    ...
    _propagate_string_origin_map(lit_map, incoming, dst_vid)
