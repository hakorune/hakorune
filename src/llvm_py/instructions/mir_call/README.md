...
- `builders/closure_split_contract.py`
  - owns closure capture classification and env-layout scalarization classification.
  - route-local closure lowering reads this contract so empty-env vs capture-env
    classification does not drift while thin-entry specialization stays deferred.
