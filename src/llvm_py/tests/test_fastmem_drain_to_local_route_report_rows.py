...
class TestFastMemDrainToLocalRouteReportRows(unittest.TestCase):
    def test_selection_report_rows_pin_next_route_slice(self):
        rows = dict(
            build_report_rows(
                _drain_to_local_mir(),
        ...
        self.assertEqual(rows["replacement_front_selected_memop_kinds"], "AtomicRemoteHeadDrain")

    def test_producer_report_rows_keep_route_open_until_producer(self):
        rows = dict(
            build_report_rows(
                _drain_to_local_mir(),
                object_out=Path(tempfile.gettempdir()) / "fastmem_drain_to_local_route.o",
