...
class TestFastMemBranchCfgPreflightReportRows(unittest.TestCase):
    def test_branch_cfg_preflight_report_rows_pin_next_slice(self):
        rows = dict(
            build_report_rows(
                _drain_local_list_mir(),
                object_out=Path(tempfile.gettempdir())
                / "fastmem_branch_cfg_preflight.o",
                profile="fastmem-branch-cfg-preflight",
            )
        )

        self.assertEqual(rows["fastmem_branch_cfg_preflight"], "1")
        self.assertEqual(rows["replacement_front_selected_route"], "fastmem_branch_cfg_preflight")
        self.assertEqual(rows["replacement_front_next_producer_slice"], "fastmem_branch_cfg_lowering_preflight")
