...
class TestFastMemDrainLocalListMutationVocabularyReportRows(unittest.TestCase):
    def test_vocabulary_report_rows_pin_verifier_slice(self):
        rows = dict(
            build_report_rows(
                _drain_local_list_mir(),
                object_out=Path(tempfile.gettempdir())
                / "fastmem_drain_local_list_mutation_vocabulary.o",
                profile="remote-free-drain-local-list-mutation-vocabulary-preflight",
            )
        )

        self.assertEqual(rows["fastmem_atomic_remote_head_drain_local_list_mutation_vocabulary_preflight"], "1")
        ...
        self.assertEqual(rows["atomic_remote_head_drain_local_list_mutation_lowerable_count"], "1")
        self.assertEqual(rows["replacement_front_next_producer_slice"], "atomic_remote_head_drain_local_list_mutation_verifier_preconditions")
