...
class TestFastMemRemoteOwnerBranchRouteBodyPreflightReportRows(unittest.TestCase):
    def test_route_body_preflight_report_rows_pin_next_slice(self):
        rows = dict(
            build_report_rows(
                _drain_local_list_mir(),
                object_out=Path(tempfile.gettempdir())
                / "fastmem_remote_owner_branch_route_body_preflight.o",
                profile="remote-owner-branch-route-body-preflight",
            )
        )

        self.assertEqual(rows["fastmem_remote_owner_branch_route_body_preflight"], "1")
        self.assertEqual(rows["replacement_front_selected_route"], "remote_owner_branch_route_body_preflight")
        self.assertEqual(rows["replacement_front_next_producer_slice"], "fastmem_branch_cfg_preflight")
