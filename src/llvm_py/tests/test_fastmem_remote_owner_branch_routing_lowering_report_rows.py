...
from test_fastmem_remote_owner_branch_routing_preflight_report_rows import (
    _drain_local_list_mir,
)


class TestFastMemRemoteOwnerBranchRoutingLoweringReportRows(unittest.TestCase):
    def test_lowering_report_rows_pin_route_body_slice(self):
        rows = dict(
            build_report_rows(
                _drain_local_list_mir(),
                object_out=Path(tempfile.gettempdir())
                / "fastmem_remote_owner_branch_routing_lowering.o",
                profile="remote-owner-branch-routing-lowering",
            )
        )

        self.assertEqual(rows["fastmem_remote_owner_branch_routing_lowering_producer_pilot"], "1")
        self.assertEqual(rows["replacement_front_selected_route"], "remote_owner_branch_routing_lowering_producer_pilot")
        self.assertEqual(rows["replacement_front_next_producer_slice"], "remote_owner_branch_route_body_preflight")
