...
class TestFastMemRemoteOwnerBranchRoutingPreflightReportRows(unittest.TestCase):
    def test_preflight_report_rows_pin_route_slice(self):
        rows = dict(
            build_report_rows(
                _drain_local_list_mir(),
                object_out=Path(tempfile.gettempdir())
                / "fastmem_remote_owner_branch_routing_preflight.o",
                profile="remote-owner-branch-routing-preflight",
            )
        )

        self.assertEqual(rows["fastmem_remote_owner_branch_routing_preflight"], "1")
        self.assertEqual(rows["replacement_front_selected_route"], "remote_owner_branch_routing_preflight")
        self.assertEqual(rows["replacement_front_next_producer_slice"], "remote_owner_branch_routing_lowering_preflight")
