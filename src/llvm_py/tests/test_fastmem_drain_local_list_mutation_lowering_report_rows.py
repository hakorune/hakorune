...
class TestFastMemDrainLocalListMutationLoweringReportRows(unittest.TestCase):
    def test_lowering_report_rows_pin_producer_slice(self):
        rows = dict(
            build_report_rows(
                _drain_local_list_mir(),
                object_out=Path(tempfile.gettempdir())
                / "fastmem_drain_local_list_mutation_lowering.o",
                profile="remote-free-drain-local-list-mutation-lowering",
            )
        )

        self.assertEqual(rows["fastmem_atomic_remote_head_drain_local_list_mutation_lowering_producer_pilot"], "1")
        self.assertEqual(rows["replacement_front_selected_memop_kinds"], "DrainRemoteListToLocal")
        ...
        self.assertEqual(rows["atomic_remote_head_drain_local_list_mutation_open"], "1")
        ...
        self.assertEqual(rows["replacement_front_next_producer_slice"], "remote_owner_branch_routing_preflight")
