...
fn emit_with_condition_env(
    ...
    emit_carrier_update_with_env(carrier, update, alloc_value, &update_env, instructions)
}
...
fn test_emit_const_update() {
    ...
    let result =
        emit_with_condition_env(&carrier, &update, &mut alloc_value, &env, &mut instructions);
