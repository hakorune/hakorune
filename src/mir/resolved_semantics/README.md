...
The owner-link I0 is closed, but the next body-facts row is a `NoSafeSlice`:
...
complete effect/control evidence. The current design stop is therefore
`CALLABLE-BODY-RESOLVED-SHAPE-ISSUER-D0`. That issuer must run during the
existing parser-private syntax lease and return one neutral AST-free body-shape
inventory co-sealed to the exact owner/body root, parser provenance, resolver
brand, source sites, and complete coverage. It must not reopen parser
transport or add a second owner issuer. Only after that inventory lands may a
private Query observer issue body facts; conformance remains unopened.
...
the resolved-shape issuer design stop and its later Query projection.
...
The D3-S2-P3 bounded independent-column family-overlap census is now closed as
cfg(test)-only evidence:
`JOINIR-GENERIC-RESOLVED-CARRIER-FAMILY-OVERLAP-CENSUS0-D3-S2-P3`. Raw
Generic rows retain mode/carrier/schedule evidence, while resolved rows
retain NestedPredicate/DirectAccum/A+ or an explicit canonical rejection.
They have no common owner/frame/source brand; fixture labels are
reporting-only and may not co-seal them. A non-empty pair reports only typed
`UnresolvedStop(FamilyOverlap)` and adds no shared classifier, winner,
selector, Recipe, BindingKey, Builder, MIR, or production caller.
...
an empty contract or a FreeStatic alias. The semantic issuer must consume the
handoff by value and issue a fresh resolver catalog/type brand; parser and
function-owner brands remain provenance/membership only.
...
bridge owns BindingRef-to-annotation provenance, and `loop_route_policy` owns
  ...
  cfg(test)-only exact source-unit receipt/map witness. S1-S1 is now closed by
  the non-`Clone` co-sealed receipt witness: it checks owner/origin/source-kind
  ...
  was added. S1-S2-D0 fixed the two-stage boundary; S1-S2-S0 closed the
