...
fn raw_cursor_reaches_preloop_nested_instance_argument_with_exact_catalog_identity() {
    actual_parser_add_fixture::with_raw_source_view_inputs(|catalog, caller, sites| {
        let view = VerifiedRawCallableSourceViewV1::verify(catalog, caller)
            .expect("catalog-backed Raw cursor");
        let exact_call = VerifiedSourceMethodCallSiteV1::verify(catalog, caller, sites[0].clone())
...
fn raw_cursor_rejects_wrong_role_and_out_of_bounds_before_lowering() {
    actual_parser_add_fixture::with_raw_source_view_inputs(|catalog, caller, _sites| {
        let view = VerifiedRawCallableSourceViewV1::verify(catalog, caller)
            .expect("catalog-backed Raw cursor");
        let body = view.root_body();
        assert!(matches!(
...
fn raw_cursor_rejects_foreign_view_even_when_catalog_source_looks_equal() {
    actual_parser_add_fixture::with_raw_source_view_inputs(|left_catalog, caller, _sites| {
        actual_parser_add_fixture::with_raw_source_view_inputs(
            |right_catalog, right_caller, _right_sites| {
                let left = VerifiedRawCallableSourceViewV1::verify(left_catalog, caller)
                    .expect("left cursor");
                let right = VerifiedRawCallableSourceViewV1::verify(right_catalog, right_caller)
                    .expect("right cursor");
