...
#[test]
fn records_arraybox_length_self_arg_route_from_typed_param_copy() {
    let mut function = make_function();
    function.signature.params = vec![MirType::Box("ArrayBox".to_string())];
    ...
    block.add_instruction(method_call(Some(3), "ArrayBox", "length", 2, vec![2]));
