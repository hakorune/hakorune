//! Main0 derived-predicate semantic-program demand issuance.
//!
//! The issuer consumes the complete source-backed Main0 derived-predicate
//! product once and keeps the operation/effect, context, JoinSig
//! continuation, tail, and control receipts together until the next
//! consumer; no caller can re-pair those rows through separate arguments.
//! It mirrors `main0_in_body_step_semantic_program.rs` for the third
//! bounded profile.
...
/// One Main0 derived-predicate semantic-program parent. The co-seal already
...
#[derive(Debug)]
pub(in crate::mir) struct VerifiedMain0DerivedPredicateSemanticProgramV1 {
...
impl VerifiedMain0DerivedPredicateSemanticProgramV1 {
    ...
    pub(in crate::mir) fn into_prepared_operation_demand(
        self,
    ) -> Result<PreparedMain0DerivedPredicateOperationDemandV1, LoopOperationPhysicalDemandRejectV1>
    {
        let Self {
            operation_effect,
            input,
        ...
        let operation =
            VerifiedLoopOperationPhysicalDemandV1::issue(context, operation_effect, continuation)?
                .prepare_all()?;
        Ok(PreparedMain0DerivedPredicateOperationDemandV1 {
            input,
            operation,
            tail,
            control,
...
/// Source-free, one-shot prepared demand for the Main0 derived-predicate
...
/// (plus test probes).
#[derive(Debug)]
pub(in crate::mir) struct PreparedMain0DerivedPredicateOperationDemandV1 {
    input: VerifiedLoopInitializedLocalInputSourceSetV1,
...
impl PreparedMain0DerivedPredicateOperationDemandV1 {
...
/// Canonical Main0 derived-predicate issuer. The complete recipe/co-seal
/// product is the only input; separate context/Core/continuation arguments
/// are rejected by construction because this API has no such shape.
pub(in crate::mir) fn issue_main0_derived_predicate_semantic_program_v1(
    product: VerifiedMain0DerivedPredicateRecipeProductV1,
) -> VerifiedMain0DerivedPredicateSemanticProgramV1 {
