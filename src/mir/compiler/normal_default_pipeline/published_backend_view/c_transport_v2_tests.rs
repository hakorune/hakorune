...
fn map_literal_v2_exact_bits_do_not_use_lossy_original_registers() {
    for (action, kind, bits) in [
        ...
        (ProjectionAction::ExactF64(0x7ff8_0000_0000_0042), ValueKind::F64, 0x7ff8_0000_0000_0042),
        (ProjectionAction::ExactF64((-0.0_f64).to_bits()), ValueKind::F64, 1 << 63),
...
fn map_literal_v2_original_actions_cannot_omit_their_producer() {
    for (action, kind, encoding) in [
        (ProjectionAction::OriginalI64, ValueKind::I64, OriginalEncoding::I64Bits),
        (ProjectionAction::OriginalBoolI64, ValueKind::Bool, OriginalEncoding::I64Bits),
        (ProjectionAction::OriginalBoolI1, ValueKind::Bool, OriginalEncoding::BoolI1ZeroExtend),
        (ProjectionAction::OriginalHandle, ValueKind::Handle, OriginalEncoding::I64Bits),
    ] {
        let row = row(action);
