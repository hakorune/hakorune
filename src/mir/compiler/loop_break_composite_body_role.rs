...
pub(crate) fn issue_loop_break_composite_body_role_map_v1(
    ...
    let root = issue_loop_role(input, root_stmt, root_body, &members, forest)?;
    let root_segments = root_stmt.site().node().segments();
    let method_call_sites = input
        .function()
        .method_calls()
        .filter_map(|(site, _)| {
            site.node()
...
fn issue_loop_role(
    ...
    let roles = issue_body_roles(input, body, members, forest)?;
...
fn issue_body_roles(
    ...
    for index in 0..body.statements().len() {
        ...
        roles.push(issue_statement_role(input, &stmt, members, forest)?);
    }
...
fn issue_statement_role(
    ...
    if let Some((_evidence, _)) = members.iter().find(|(evidence, _)| evidence.site() == stmt.site())
    {
        let body = input
            ...
            .map_err(|_| LoopBreakCompositeBodyRoleRejectV1::SourceNavigation)?;
        return issue_loop_role(input, stmt, &body, members, forest);
    }
    ...
    if let ASTNode::If { else_body, .. } = stmt.node() {
        let then_body = input
            ...
            .map_err(|_| LoopBreakCompositeBodyRoleRejectV1::SourceNavigation)?;
        let then_roles = issue_body_roles(input, &then_body, members, forest)?;
        let else_roles = if else_body.is_some() {
            let body = input
                .source()
                .child_body_from_stmt(stmt, BodyChildRoleV1::IfElse)
                .map_err(|_| LoopBreakCompositeBodyRoleRejectV1::SourceNavigation)?;
            Some(issue_body_roles(input, &body, members, forest)?)
        } else {
            None
        };
