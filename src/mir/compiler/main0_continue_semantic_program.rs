//! Main0 Continue semantic-program demand issuance.
//!
//! Caller-zero owner: this module is wired by the Main0 canonical-root
//! handoff slice; until a production route selects it, no production caller
//! exists (recheck at the production-switch slice).
//!
//! The issuer consumes the complete source-backed Main0 Continue product
...
//! `callable_semantic_program.rs` minus the callable-only prelude contract.
...
/// One Main0 Continue semantic-program parent. The co-seal already issued
/// every row; this wrapper is the single demand-construction point so the
/// prepared program cannot be re-paired from independent pieces.
#[derive(Debug)]
pub(in crate::mir) struct VerifiedMain0ContinueSemanticProgramV1 {
...
impl VerifiedMain0ContinueSemanticProgramV1 {
    ...
    pub(in crate::mir) fn into_prepared_operation_demand(
        self,
    ) -> Result<PreparedMain0ContinueOperationDemandV1, LoopOperationPhysicalDemandRejectV1>
    {
        let Self {
            operation_effect,
            input,
        ...
        let operation =
            VerifiedLoopOperationPhysicalDemandV1::issue(context, operation_effect, continuation)?
                .prepare_all()?;
        Ok(PreparedMain0ContinueOperationDemandV1 {
            input,
            operation,
            tail,
            control,
...
/// Source-free, one-shot prepared demand for the Main0 Continue profile. It
/// is an aggregate of already-issued rows, not a new semantic authority; the
/// only consumer is the prepared Main0 operation handoff (plus test probes).
#[derive(Debug)]
pub(in crate::mir) struct PreparedMain0ContinueOperationDemandV1 {
    input: VerifiedLoopInitializedLocalInputSourceSetV1,
...
impl PreparedMain0ContinueOperationDemandV1 {
...
/// Canonical Main0 Continue issuer. The complete recipe/co-seal product is
...
pub(in crate::mir) fn issue_main0_continue_semantic_program_v1(
    product: VerifiedMain0ContinueRecipeProductV1,
) -> VerifiedMain0ContinueSemanticProgramV1 {
