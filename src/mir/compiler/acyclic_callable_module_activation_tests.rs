...
#[test]
fn one_call_rhs_implicit_fallthrough_preserves_call_phi_and_baseline() {
    let branch = function(
        "branch_call_rhs_implicit",
        vec![
            local("result", variable("x")),
            ASTNode::If {
                condition: Box::new(binary(BinaryOperator::Greater, variable("x"), integer(0))),
    ...
    let branch_function = &result.module.functions["branch_call_rhs_implicit/1"];
    let calls = branch_function
        .blocks
        .values()
        .flat_map(|block| &block.instructions)
    ...
    let merge = branch_function
        ...
        .expect("implicit Call-RHS merge block");
    let input_blocks = inputs
        .iter()
        .map(|(block, _)| *block)
        .collect::<std::collections::BTreeSet<_>>();
