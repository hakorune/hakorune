...
use crate::mir::resolved_semantics::{
    FunctionSemanticResolverSessionV1, ResolveScriptForestOutcomeV1, ScriptResolverDeferredV1,
...
pub(super) fn resolve_normal_script_source_v1<'source>(
    ...
    let view = ScriptSyntaxViewV1::from_program(source_ast)
        .ok_or_else(|| Box::<str>::from("[mir/script-semantic/source-root] expected Program"))?;
    let outcome = declaration_facts
        .with_record_schema_demand_view(|record_schemas| {
    ...
    match outcome {
        ResolveScriptForestOutcomeV1::Complete(forest) => {
            Ok(Some(NormalScriptResolutionV1::Complete(
                VerifiedScriptSemanticSourceV1::seal_ast_with_forest(source_ast, forest, window)
