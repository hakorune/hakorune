...
#[test]
fn selected_variable_compound_assignment_composes_without_retry() {
    let roots = [
        compound_assignment(variable("x"), BinaryOperator::Add, integer(2)),
        block_expr(
    ...
    for root in roots {
        let mut legacy = MirBuilder::new();
        legacy.enter_function_for_test("variable_compound_assignment_parity/0".to_owned());
        let old = crate::mir::builder::emission::constant::emit_integer(&mut legacy, 7).unwrap();
        seed_binding(&mut legacy, "x", old);
        let legacy_value = drive_raw_legacy_expression_v1(&mut legacy, root.clone()).unwrap();

        let mut selected = MirBuilder::new();
        selected.enter_function_for_test("variable_compound_assignment_parity/0".to_owned());
        let old = crate::mir::builder::emission::constant::emit_integer(&mut selected, 7).unwrap();
        seed_binding(&mut selected, "x", old);
        let selected_value = drive_selected(&mut selected, root).unwrap();
    ...
    let mut selected = MirBuilder::new();
    selected.enter_function_for_test("variable_compound_assignment_failure/0".to_owned());
    let error = drive_selected(
        &mut selected,
        compound_assignment(variable("missing"), BinaryOperator::Add, variable("rhs")),
    )
    .unwrap_err();
    assert!(error.contains("Undefined variable: missing"));
    assert!(spanned_instructions(&selected).is_empty());
    ...
    let error = drive_selected(
        &mut selected,
        compound_assignment(variable("x"), BinaryOperator::Add, variable("rhs")),
    )
    .unwrap_err();
    assert!(error.contains("Undefined variable: rhs"));
    assert_eq!(
    ...
    drive_selected(
        &mut selected,
        compound_assignment(variable("x"), BinaryOperator::Add, integer(4)),
    )
    .unwrap();
}
