...
impl PreparedModuleFinalizationDeclarationMetadataV1 {
    ...
    pub(super) fn commit_into(self, module: &mut MirModule) -> Result<(), String> {
        if module.metadata.canonical_object_membership.is_some() {
            return Err(
                "[freeze:contract][mir/object-definition/membership-already-installed]".into(),
            );
        }
...
mod tests {
    ...
    fn canonical_membership_is_id_projected_and_drift_never_repairs() {
        ...
        context
            .user_defined_boxes
            .insert("Plain".into(), vec!["value".into()]);
        context
            .user_box_field_decls
            .insert("Plain".into(), vec![field("value", Some("i64"), false)]);
        ...
        assert!(
            PreparedModuleFinalizationDeclarationMetadataV1::prepare(&wrong_context)
                .commit_into(&mut module)
                .is_err()
        );
        assert!(
            module.metadata.user_box_decls.is_empty(),
        ...
        assert!(
            crate::mir::semantic_refresh::refresh_module_semantic_metadata(&mut module).is_err()
        );
        assert_eq!(module.metadata.canonical_object_membership, expected);
        assert_eq!(
            module.metadata.user_box_field_decls["Plain"][0]
                .declared_type_name
                .as_deref(),
            Some("StringBox"),
            "no repair on error"
        ...
        module
            ...
            .insert(
                "Plain".into(),
                hakorune_mir_defs::CanonicalObjectIdV1::from_declaration_index(9).unwrap(),
            );
        assert!(module.validate_object_definition_membership().is_err());
