...
pub(super) fn lower_cond_to_if_plans(
    ...
    let cond_plans = lower_cond_expr_to_if_plans(builder, &bindings, &cond.tail_expr, then_plans, else_plans, joins, error_prefix)?;
...
fn lower_cond_expr_to_if_plans(
    ...
    match expr {
        ASTNode::UnaryOp { operator: UnaryOperator::Not, operand, .. } => {
            ...
            lower_cond_expr_to_if_plans(builder, phi_bindings, operand, new_then, new_else, joins, error_prefix)
        }
        ASTNode::BinaryOp { operator: BinaryOperator::And, left, right, .. } => {
            ...
            for j in &joins_for_right {
                let ty = builder.type_ctx.get_type(j.then_val).or_else(|| builder.type_ctx.get_type(j.else_val)).cloned().unwrap_or(MirType::Unknown);
            ...
            let inner_plans = lower_cond_expr_to_if_plans(builder, phi_bindings, right, then_plans, else_plans_for_right, right_joins, error_prefix)?;

            // Outer joins use intermediate (preserves short-circuit semantics)
            let outer_joins: Vec<CoreIfJoin> = joins.iter().zip(intermediate_vals.iter()).map(|(j, (_, intermediate, _))| CoreIfJoin {
                name: j.name.clone(),
                dst: j.dst,
                pre_val: j.pre_val,
            ...
            lower_cond_expr_to_if_plans(builder, phi_bindings, left, inner_plans, else_plans, outer_joins, error_prefix)
        }
        ASTNode::BinaryOp { operator: BinaryOperator::Or, left, right, .. } => {
            ...
            // Pass original plans directly (no Copy augmentation)
            let inner_plans = lower_cond_expr_to_if_plans(builder, phi_bindings, right, then_plans_for_right, else_plans_for_right, right_joins, error_prefix)?;
            ...
            lower_cond_expr_to_if_plans(builder, phi_bindings, left, then_plans, Some(inner_plans), outer_joins, error_prefix)
        }
        _ => {
            let (cond_id, cond_effects) = lower_cond_value_expr(builder, phi_bindings, expr, error_prefix)?;
            debug_log_cond_if_lit3_origin(builder, &cond_effects);
            let mut plans = effects_to_plans(cond_effects);
            plans.push(CorePlan::If(CoreIfPlan { condition: cond_id, then_plans, else_plans, joins }));
            Ok(plans)
        }
    }
}
...
fn normalize_empty_branches(
    ...
    let then_plans = if then_plans.is_empty() { vec![CorePlan::Seq(Vec::new())] } else { then_plans };
    let else_plans = else_plans.map(|plans| if plans.is_empty() { vec![CorePlan::Seq(Vec::new())] } else { plans });
