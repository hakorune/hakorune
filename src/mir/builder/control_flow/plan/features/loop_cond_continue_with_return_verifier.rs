//! Route-local verifier for `LoopCondContinueWithReturn`.
//!
//! Scope:
//! - verify route-specific PHI closure invariants after materialization
//! - keep continue-with-return contract checks out of the pipeline body

use crate::mir::builder::control_flow::plan::features::loop_cond_continue_with_return_phi_materializer::LoopCondContinueWithReturnPhiClosure;
use crate::mir::builder::control_flow::plan::{CoreExitPlan, CorePlan, LoweredRecipe};
use crate::mir::BasicBlockId;

pub(in crate::mir::builder) fn verify_loop_cond_continue_with_return_phi_closure(
    phi_closure: &LoopCondContinueWithReturnPhiClosure,
    ...
    if phi_closure.continue_target() != continue_target {
        return Err(format!(
            "{error_prefix}: phi_closure continue_target {:?} != expected {:?}",
    ...
    let Some(CorePlan::Exit(CoreExitPlan::ContinueWithPhiArgs { phi_args, .. })) =
        ...
        return Err(format!(
            "{error_prefix}: continue-with-return body must end with ContinueWithPhiArgs"
        ));
    };
...
mod tests {
    ...
    #[test]
    fn loop_cond_continue_with_return_verifier_accepts_min_shape() {
        let closure = LoopCondContinueWithReturnPhiClosure::new(
    ...
    #[test]
    fn loop_cond_continue_with_return_verifier_rejects_missing_continue_exit() {
        let closure = LoopCondContinueWithReturnPhiClosure::new(
            vec![dummy_phi(2), dummy_phi(3)],
            vec![("i".to_string(), ValueId(2))],
            BasicBlockId(12),
        ...
        let err = verify_loop_cond_continue_with_return_phi_closure(
            &closure,
            &body_plans,
            BasicBlockId(12),
            BasicBlockId(12),
    ...
    #[test]
    fn loop_cond_continue_with_return_verifier_rejects_wrong_phi_count() {
        let closure = LoopCondContinueWithReturnPhiClosure::new(
            vec![dummy_phi(2)],
            vec![("i".to_string(), ValueId(2))],
            BasicBlockId(12),
