...
pub(super) fn verify_loop_scan_methods_v0_recipe(
    ...
    for (idx, segment) in scan_methods.recipe.segments.iter().enumerate() {
        match segment {
            LoopScanSegment::Linear(recipe) => match recipe {
                LinearBlockRecipe::NoExit(block) => {
                    let ctx = format!("loop_scan_methods_v0_linear_no_exit_{idx}");
                    verify_no_exit_block_recipe(block, &ctx)?;
                }
                LinearBlockRecipe::ExitAllowed(block) => {
                    let ctx = format!("loop_scan_methods_v0_linear_exit_allowed_{idx}");
