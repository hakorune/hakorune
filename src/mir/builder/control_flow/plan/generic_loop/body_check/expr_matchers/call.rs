...
use crate::mir::builder::control_flow::plan::generic_loop::body_check_extractors::{is_loop_var_minus_one, is_loop_var_plus_one};
...
pub(in crate::mir::builder) fn matches_is_space_call(expr: &ASTNode, loop_var: &str) -> bool {
    let ASTNode::MethodCall { method, arguments, .. } = expr else {
        return false;
    };
    if method == "_is_space"
        && arguments
...
pub(in crate::mir::builder) fn matches_substring_call_with_loop_var(expr: &ASTNode, loop_var: &str) -> bool {
    let ASTNode::MethodCall { method, arguments, .. } = expr else {
        return false;
    };
    if method != "substring" {
