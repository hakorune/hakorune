...
fn try_lower_loop_scan_methods_nested_segment_plans(
    ...
    try_lower_loop_scan_methods_nested_stmt_only(
        builder,
        current_bindings,
        carrier_step_phis,
        break_phi_dsts,
...
pub(in crate::mir::builder) fn lower_loop_scan_methods_nested_segment(
    ...
    if let Some(plans) = try_lower_loop_scan_methods_nested_segment_plans(
        builder,
        current_bindings,
        carrier_step_phis,
        break_phi_dsts,
...
mod tests {
    ...
    #[test]
    fn loop_scan_methods_nested_segment_dispatch_returns_none_without_stmt_only_recipe() {
        let mut builder = MirBuilder::new();
        let mut current_bindings = BTreeMap::new();
        let carrier_step_phis = BTreeMap::new();
        let break_phi_dsts = BTreeMap::new();
        ...
        let plans = try_lower_loop_scan_methods_nested_segment_plans(
            &mut builder,
            &mut current_bindings,
            &carrier_step_phis,
            &break_phi_dsts,
