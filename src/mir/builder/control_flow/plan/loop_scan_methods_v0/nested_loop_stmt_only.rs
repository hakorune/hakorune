...
fn try_lower_loop_scan_methods_nested_stmt_only_plans(
...
fn apply_loop_scan_methods_nested_stmt_only_plans(
    builder: &mut MirBuilder,
    current_bindings: &mut BTreeMap<String, crate::mir::ValueId>,
    plans: &[LoweredRecipe],
) {
...
pub(in crate::mir::builder) fn try_lower_loop_scan_methods_nested_stmt_only(
    ...
    let Some(plans) = try_lower_loop_scan_methods_nested_stmt_only_plans(
        builder,
        current_bindings,
        carrier_step_phis,
        break_phi_dsts,
    ...
    apply_loop_scan_methods_nested_stmt_only_plans(builder, current_bindings, &plans);
    Ok(Some(plans))
}
...
mod tests {
    ...
    fn loop_scan_methods_nested_stmt_only_returns_none_without_stmt_only_recipe() {
        ...
        let result = try_lower_loop_scan_methods_nested_stmt_only_plans(
            &mut builder,
    ...
    #[test]
    fn loop_scan_methods_nested_stmt_only_wrapper_returns_none_without_stmt_only_recipe() {
        let mut builder = MirBuilder::new();
        let mut current_bindings = BTreeMap::new();
        let carrier_step_phis = BTreeMap::new();
        let break_phi_dsts = BTreeMap::new();
