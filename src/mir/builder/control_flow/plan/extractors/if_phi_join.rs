...
pub(crate) fn extract_loop_with_if_phi_parts(
    condition: &ASTNode,
    body: &[ASTNode],
) -> Result<Option<IfPhiJoinParts>, String> {
    // Phase 1: Validate condition (reuse loop_simple_while extractor)
    use super::loop_simple_while::validate_loop_condition_structure;
    let loop_var = match validate_loop_condition_structure(condition) {
        Some(var) => var,
        None => return Ok(None),
    };
    ...
    if has_forbidden_control_flow_for_if_phi_join(body) {
...
/// Check for forbidden control flow for if_phi_join.
///
...
fn has_forbidden_control_flow_for_if_phi_join(body: &[ASTNode]) -> bool {
    for stmt in body {
        if has_forbidden_control_flow_recursive_if_phi_join(stmt) {
            return true;
        }
    }
    false
...
fn has_forbidden_control_flow_recursive_if_phi_join(node: &ASTNode) -> bool {
    match node {
