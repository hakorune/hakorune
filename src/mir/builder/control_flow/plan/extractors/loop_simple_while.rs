...
#[derive(Debug, Clone)]
pub(crate) struct LoopSimpleWhileParts {
    pub loop_var: String,
    // Note: condition/body は ctx から再利用（AST 丸コピー不要）
}

/// Extract loop_simple_while parts.
///
...
pub(crate) fn extract_loop_simple_while_parts(
    condition: &ASTNode,
    body: &[ASTNode],
) -> Result<Option<LoopSimpleWhileParts>, String> {
    // Phase 1: Validate condition structure (比較 + 左が変数)
    ...
    Ok(Some(LoopSimpleWhileParts { loop_var }))
}
...
mod tests {
    ...
    fn test_extract_simple_while_success() {
        ...
        let result = extract_loop_simple_while_parts(&condition, &body);
        assert!(result.is_ok());
