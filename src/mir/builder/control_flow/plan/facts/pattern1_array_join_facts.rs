...
#[derive(Debug, Clone)]
pub(in crate::mir::builder) struct Pattern1ArrayJoinFacts {
    pub loop_var: String,
...
pub(in crate::mir::builder) fn try_extract_pattern1_array_join_facts(
    condition: &ASTNode,
    body: &[ASTNode],
    observation: &ScanConditionObservation,
) -> Result<Option<Pattern1ArrayJoinFacts>, Freeze> {
    ...
    Ok(Some(Pattern1ArrayJoinFacts {
        loop_var,
...
#[cfg(test)]
mod tests {
    use super::{try_extract_pattern1_array_join_facts, Pattern1ArrayJoinFacts};
    ...
    #[test]
    fn pattern1_array_join_facts_extracts_minimal_join_shape() {
        let condition = binop(
            BinaryOperator::Less,
            v("i"),
        ...
        let facts = try_extract_pattern1_array_join_facts(&condition, &body, &observation)
        .expect("Ok")
        .expect("Some facts");
        ...
        let _: Pattern1ArrayJoinFacts = facts;
    }

    #[test]
    fn pattern1_array_join_facts_rejects_else_branch() {
        let condition = binop(
            BinaryOperator::Less,
            v("i"),
            method_call(v("arr"), "length", vec![]),
        ...
        let observation = scan_condition_observation(&condition_shape, &step_shape);
        let facts =
            try_extract_pattern1_array_join_facts(&condition, &body, &observation).expect("Ok");
