...
#[derive(Debug, Clone)]
pub(in crate::mir::builder) struct Pattern1CharMapFacts {
    pub loop_var: String,
...
pub(in crate::mir::builder) fn try_extract_pattern1_char_map_facts(
    condition: &ASTNode,
    body: &[ASTNode],
    observation: &ScanConditionObservation,
) -> Result<Option<Pattern1CharMapFacts>, Freeze> {
...
#[cfg(test)]
mod tests {
    use super::{try_extract_pattern1_char_map_facts, Pattern1CharMapFacts};
    ...
    #[test]
    fn pattern1_char_map_facts_extracts_minimal_tolower_shape() {
        ...
        let facts = try_extract_pattern1_char_map_facts(&condition, &body, &observation)
            .expect("Ok")
            .expect("Some");
        ...
        let _: Pattern1CharMapFacts = facts;
    }

    #[test]
    fn pattern1_char_map_facts_rejects_nonmatching_body() {
        let condition = ASTNode::BinaryOp {
            operator: BinaryOperator::Less,
            left: Box::new(v("i")),
            right: Box::new(method_call(v("s"), "length", vec![])),
