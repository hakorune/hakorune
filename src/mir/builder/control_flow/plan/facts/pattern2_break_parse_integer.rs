...
pub(super) fn try_extract_pattern2_break_parse_integer_subset(
    condition: &ASTNode,
    body: &[ASTNode],
) -> Option<Pattern2BreakFacts> {
    if let Some(range_based) =
        try_extract_pattern2_break_parse_integer_range_subset(condition, body)
    {
...
fn try_extract_pattern2_break_parse_integer_range_subset(
    condition: &ASTNode,
    body: &[ASTNode],
) -> Option<Pattern2BreakFacts> {
    let loop_var =
