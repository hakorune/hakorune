...
- next movable symbols now concentrate on `loop_scan_phi_vars_v0` pipeline entry cleanup and `loop_cond` remaining route import cleanup (`continue-only` + `continue-with-return`)
