//! Recipe definition for loop_scan_phi_vars_v0 (recipes-owned surface).
//!
//! The route remains family-local under `plan/`, but the recipe vocabulary is
//! owned here so non-`plan/` callers can depend on a stable top-level surface.
