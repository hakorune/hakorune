...
impl ModuleDraftCollectorV1 {
    ...
    pub(in crate::mir::builder) fn prepare_canonical_drain(
        ...
        if self.has_retained_construction() {
            return Err(reject(
                self,
                CanonicalCollectorDrainErrorV1::RetainedConstruction,
            ));
        }
...
mod tests {
    ...
    fn retained_construction_rejects_all_alternative_drain_preflights() {
        ...
        use crate::mir::raw_physical_drain::{
            RawPhysicalCallableMainDispositionV1, RawPhysicalDrainManifestV1,
            RawPhysicalDrainRouteV1,
        };
        let brand = ModuleInvocationBrandV1::test_with_ordinal(91);
        let make = || {
            let key =
                crate::mir::builder::CanonicalSameModuleCallableKeyV1::birth_constructor("Page", 0);
        ...
        let raw = RawPhysicalDrainManifestV1::new(
            brand,
        ...
        let (collector, error) = collector
            .prepare_raw_drain(&raw, brand)
            .unwrap_err()
            .into_parts();
        assert_eq!(
            error,
            super::super::RawCollectorDrainErrorV1::RetainedConstruction
        );
        assert!(collector.has_retained_construction());
        let shell =
            ModuleLoweringShellV1::from_empty_module(crate::mir::MirModule::new("empty".into()))
                .unwrap();
