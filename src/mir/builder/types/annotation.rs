...
fn infer_array_method_return_type(method: crate::boxes::array::ArrayMethodId) -> Option<MirType> {
    match method {
        ...
        crate::boxes::array::ArrayMethodId::Join => Some(MirType::String),
...
mod tests {
    ...
    fn corebox_surface_aliases_use_catalog_return_type() {
        ...
        assert_eq!(
            infer_return_type("ArrayBox.indexOf/1"),
            Some(MirType::Integer)
        );
        assert_eq!(infer_return_type("ArrayBox.join/1"), Some(MirType::String));
        ...
        assert_eq!(infer_return_type("MapBox.length/0"), Some(MirType::Integer));
