//! Main0 in-body-step physical lowerer canary.
//!
...
use super::main0_in_body_step_lowerer::lower_main0_in_body_step_function_draft_v1;
use crate::mir::builder::normal_main0_in_body_step_prepared_operation::PreparedMain0InBodyStepLoopIngressV1;
...
const IN_BODY_STEP_SOURCE: &str = r#"
static box Main {
    main() {
        local i = 0
        local tmp = 0
...
fn observe_current_function(builder: &MirBuilder) -> (usize, bool, usize) {
    let function = builder
        .function_state
        .current_function
        .as_ref()
        .expect("selected function still open");
    let mut phi_backedge_from_add = false;
    ...
    for (block, block_data) in &function.blocks {
        ...
        for instruction in &block_data.instructions {
            ...
            {
                add_value = Some((*block, *dst));
            }
        }
    }
    if let Some((_, add_dst)) = add_value {
        phi_backedge_from_add = function.blocks.values().any(|block_data| {
            block_data.instructions.iter().any(|instruction| {
                matches!(
                    instruction,
                    crate::mir::MirInstruction::Phi { inputs, .. }
                        if inputs.iter().any(|(_, value)| *value == add_dst)
                )
            })
        });
    }
...
#[test]
fn main0_in_body_step_lowers_to_one_draft_seal() {
    let (installed, context) = installed_package(IN_BODY_STEP_SOURCE);
    let mut port = installed.begin_lowering(&context).expect("same catalog");
    let (key, identity) = app_main_relation(&context);

    let selection = select_main0_in_body_step_source_v1(&port, &key, &identity)
        .expect("selection observes the installed source");
    let Main0InBodyStepSourceSelectionV1::Selected(product) = selection else {
        panic!("exact Main0 in-body-step source must be selected")
    };

    let mut builder = MirBuilder::new();
    let observation = port
        .with_app_main_root_lowering_input(&key, &identity, |input, _identity| {
            let ingress = PreparedMain0InBodyStepLoopIngressV1::issue(input, product)
            ...
            let ready = lower_main0_in_body_step_function_draft_v1(
                &mut outer,
                program,
                "Main.main/0".to_owned(),
            )
            .expect("Main0 in-body-step draft");
            let (block_count, phi_backedge_from_add, predecessor_edges) =
