//! Main0 Continue physical lowerer canary.
//!
...
use super::main0_continue_lowerer::lower_main0_continue_function_draft_v1;
...
#[test]
fn main0_continue_lowers_to_one_draft_seal() {
    let (installed, context) = installed_package(CONTINUE_SOURCE);
    let mut port = installed.begin_lowering(&context).expect("same catalog");
    let (key, identity) = app_main_relation(&context);

    let selection = select_main0_continue_source_v1(&port, &key, &identity)
        .expect("selection observes the installed source");
    let Main0ContinueSourceSelectionV1::Selected(product) = selection else {
        panic!("exact Main0 Continue source must be selected")
    };
    ...
    let observation = port
        .with_app_main_root_lowering_input(&key, &identity, |input, _identity| {
            ...
            let ready = lower_main0_continue_function_draft_v1(
                &mut outer,
                program,
                "Main.main/0".to_owned(),
            )
            .expect("Main0 Continue draft");
