...
D1T (`MIR-CALL-CORE-R6-D1T-CONCAT-SUBSTRING-EXTERN-ISSUER-I0`) landed at
`a1e856fa25`: one `ConcatSubstringPlan` writer now uses the collector-owned
shared-source proof, exact five-argument order, and effects carried unchanged
...
The landed D1U row (`75427a9aa2`) is the paired `ConcatSubstringLenPlan`
family: its left
...
### MIR Call R6 D1W StoreSharedReceiverSubstring issuer (landed)

D1W is the bounded final reconstruction family selected after the D1V
upper-worker census and landed at `938e060028`. The existing typed `Method(Some)` substring/slice call
...
The first callable Loop production edges are now the selected static
DirectAccum branch and the bounded Generic G0 top-level `FreeFunction`
consumer. The latter borrows the exact selected function input from the normal
package, selects `CanonicalCallableRouteV1::GenericG0`, and hands one pending
canonical child session to the existing collector/Single publication owner.
A static FreeFunction has two physical `i64` lanes because it has no receiver;
the three-lane declared-instance Generic fixture remains outside this I0.
Cataloged static/instance methods, general Loop families, and OBJ/EXE output
...
## Main0 canonical root lifecycle (D0 production handoff)
...
`module_lowering_invocation_resolved_loan.rs::complete_main0_root_draft_v1`
