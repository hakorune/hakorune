...
/// Lower if-only StepTree to Normalized JoinModule (Phase 123-128 baseline).
///
...
fn lower_return_from_tree(
    ...
    match node {
            ...
            for n in nodes {
                match n {
                    ...
                    StepNode::Stmt {
                        ...
                        return lower_return_value(value_ast, body, next_value_id, env, contract);
                    }
                    StepNode::If { .. } => {
                        // Phase 123 P3: Lower If node
                        return lower_if_node(n, body, next_value_id, env, contract);
                    }
                    _ => {
        ...
        StepNode::Stmt {
            kind: StepStmtKind::Return { value_ast },
            ..
        } => lower_return_value(value_ast, body, next_value_id, env, contract),
        StepNode::If { .. } => {
            // Phase 123 P3: Lower If node
            lower_if_node(node, body, next_value_id, env, contract)
        }
        _ => {
...
fn lower_if_node(
    ...
    } = node
    {
        ...
        *next_value_id += 1;
        ...
        verify_branch_is_return_literal(then_branch)?;
...
fn verify_branch_is_return_literal(
    ...
    match branch {
            ...
            StepNode::Block(nodes) => {
                ...
                verify_branch_is_return_literal(&nodes[0])
            }
            _ => Err(format!(
