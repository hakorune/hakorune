...
uncovered, so an owner storing them
...
An `EmptyArray` entry is the sealed `NestedArray { elements: [] }` row:
it lowers through `InstallEmptyArray{map, key}` — no value operand, no
minted host handle — and lands as `CheckedMapPayload::EmptyArray` via
`nyash.map.checked_install_empty_array_v1`, an owned-empty marker whose
end is trivial.
