...
impl OrdinaryNewClaimLedgerV1 {
    pub(super) fn root_home_exit_is_complete(&self) -> bool {
        let Some(Ok(completion)) = &self.root_completion else { return true; };
        !matches!(completion.cleanup().terminal_homes(), Some(Ok(_)))
            || matches!(*self.root_exit.borrow(), RootHomeExitProgress::Unavailable | RootHomeExitProgress::Emitted(_))
    }

    pub(crate) fn prepare_root_home_exit(
        &self, owner: FunctionOwnerIdV1, site: &SourceNodeSiteV1,
    ) -> Result<bool, String> {
        let Some(Ok(completion)) = &self.root_completion else { return Ok(false); };
        if completion.owner() != owner { return Ok(false); }
        let Some(Ok(homes)) = completion.cleanup().terminal_homes() else { return Ok(false); };
        if !completion.explicit_site().is_some_and(|expected| expected.node() == site) {
        ...
        for binding in homes {
            ...
            let row = candidates.next().ok_or_else(|| freeze("root-home-not-installed"))?;
            if candidates.next().is_some() { return Err(freeze("duplicate-root-home")); }
        ...
        *progress = if available { RootHomeExitProgress::Prepared(operands) }
            else { RootHomeExitProgress::Unavailable };
        Ok(available)
    }

    pub(crate) fn begin_root_home_exit(&self) -> Result<Vec<(CanonicalObjectIdV1, ValueId)>, String> {
        let mut progress = self.root_exit.borrow_mut();
        ...
        let RootHomeExitProgress::Prepared(operands) =
            std::mem::replace(&mut *progress, RootHomeExitProgress::Emitting) else { unreachable!() };
        Ok(operands)
    }

    pub(crate) fn record_root_home_exit(&self, bindings: Vec<(BasicBlockId, MirInstruction)>) -> Result<(), String> {
        let mut progress = self.root_exit.borrow_mut();
    ...
    fn validate_root_home_exit(&self, function: &MirFunction) -> Result<(), String> {
        let Some(Ok(completion)) = &self.root_completion else { return Ok(()); };
        if !matches!(completion.cleanup().terminal_homes(), Some(Ok(_))) { return Ok(()); }
    ...
    pub(crate) fn register_new_root(&self, owner: FunctionOwnerIdV1) -> Result<(), String> {
        ...
        if self.root_completion.as_ref().and_then(|row| row.as_ref().ok())
