...
/// A borrowed `Handle` entry on a non-terminal call argument still fails
/// closed at the loan layer: the co-sealed `Map` edge exists only for
/// lifecycle-admitted terminal calls, so `local r = read_a(%{...})` has
/// no corroborated edge to carry the handoff — the package never reaches
/// the undertaking's shape admission.
#[test]
fn borrowed_handle_entry_on_a_non_terminal_argument_stays_edge_gated() {
    let result = issue(
        "static box Work { read_a(m: MapBox): i64 { return m.get(\"a\") } }
         static box Main { main() { return 0 } drive(h) { local r = read_a(%{\"k\" => h, \"a\" => 9}) return r } }",
    );
    assert!(matches!(
        result,
...
/// A map carrying a `Borrowed` handle entry is readable now that the entry
/// installs under the `BorrowedHandle` payload tag: the seal admits the
/// combination, and a checked get on the tagged entry records Fault 104 at
/// runtime instead of silently misreading handle bits as a scalar.
#[test]
fn owned_local_map_get_with_borrowed_handle_entry_reads_as_tagged_fault() {
    let package = issue(
...
/// A call-returned map local (`local m = make_map()`) is readable: the
/// callee could only have installed tagged payloads (borrowed entries
/// cannot ride `ReturnHandoff`), so the seal admits the read and the
/// checked get dispatches on the tag at runtime — `I64` value, `0`-Normal
/// missing, Fault 104 for every other payload kind.
#[test]
fn call_returned_map_get_issues_relation_and_installs() {
    let package = issue(
