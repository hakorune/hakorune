...
summary. Direct unbranched exits remain outside this bounded view because the
current JoinSig summary does not retain their exact origin item.
...
## M8 S6B variable-accum break/fallthrough producer — retired

Retired at M12-R2C with the whole caller-zero `VariableAccumBreakV1` chain
(Facts observer, projection, producer, M8B wire arm, producer id). Its
`LoopBreakRecipe` row in `ATTESTED_RECIPE_BACKED_V1` claimed portable-producer
backing that never matched a production owner; the attestation row and
producer id were removed, and `LoopBreakRecipe` is now a typed-declined
canonical route. The `producer_id_migration` receipt marks it
`legacy_only`.
