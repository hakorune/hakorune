...
use super::schema_v2::LoopRecipeV2;
...
impl LoopRecipeSourceClaimVerifierV1 {
    pub(super) fn verify(
        ...
        verify_loop_source_shape(
            recipe.loops.iter().map(|node| (node.key, node.parent)),
            binding,
        )
    }

    pub(super) fn verify_v2(
        recipe: &LoopRecipeV2,
        binding: LoopRecipeSourceBindingV1,
    ) -> Result<StructurallyVerifiedLoopRecipeSourceClaimV1, Reject> {
        verify_loop_source_shape(
...
fn verify_loop_source_shape(
    loops: impl IntoIterator<Item = (LoopNodeKeyV1, Option<LoopNodeKeyV1>)>,
    binding: LoopRecipeSourceBindingV1,
) -> Result<StructurallyVerifiedLoopRecipeSourceClaimV1, Reject> {
    let loops = loops.into_iter().collect::<Vec<_>>();
    let expected = loops.len();
    ...
    for ((_, parent), row) in loops.iter().zip(&binding.loops) {
        let Some(parent_loop) = *parent else {
            if !matches!(
