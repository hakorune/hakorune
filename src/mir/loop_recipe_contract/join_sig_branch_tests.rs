...
fn loop_true_branch_exit_join_sig_is_deterministic_and_has_no_backedge() {
    ...
    assert_eq!(branch.then_exit.exit_item, LoopItemKeyV1::new(1));
    assert_eq!(branch.then_exit.role, LoopJoinEdgeRoleV1::Break);
    assert_eq!(branch.else_exit.exit_item, LoopItemKeyV1::new(2));
    assert_eq!(branch.else_exit.role, LoopJoinEdgeRoleV1::Continue);
...
#[test]
fn loop_true_branch_exit_join_sig_keeps_implicit_else_rejected() {
...
#[test]
fn loop_true_branch_exit_join_sig_rejects_divergent_branch_writes() {
    let mut recipe = branch_recipe();
