...
/// The lifecycle consumer has a selected ordinary-function set. Each physical
/// body already carries the exact parameter ValueIds; only those functions
/// must satisfy the i64 entry contract. Generic backend callers keep the
/// broader module-wide gate above until their own consumers are cut over.
pub(crate) fn enforce_lifecycle_parameter_entry_backend_supported(
    ...
    if input.entry().ordinary_calls().is_empty() {
        return Ok(());
    }
    let mut checked = BTreeSet::new();
    for ordinary in input.entry().ordinary_calls() {
        ...
        if !checked.insert(function_index) {
            continue;
        }
