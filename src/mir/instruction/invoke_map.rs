...
pub enum InvokeNormalResultKind { Handle, Map, MapKey, MapOutcome }

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MapInvokeOperation {
    New,
    PrepareKey { utf8: String },
    InstallIndexed {
        map: ValueId,
        key: ValueId,
...
impl MapInvokeOperation {
    pub fn normal_result_kind(&self) -> Option<InvokeNormalResultKind> {
        match self {
            ...
            Self::InstallIndexed { .. } => Some(InvokeNormalResultKind::MapOutcome),
    ...
    pub fn effects(&self) -> EffectMask {
        match self {
            ...
            Self::InstallIndexed { .. } => EffectMask::WRITE.add(Effect::Alloc).add(Effect::Control),
            Self::EndOutcome { .. } | Self::End { .. } => EffectMask::WRITE
                .union(EffectMask::MUT).union(EffectMask::IO).add(Effect::Control),
        }
    }
    pub fn used_values(&self) -> Vec<ValueId> {
        match self {
            Self::New | Self::PrepareKey { .. } => vec![],
            Self::InstallIndexed { map, key, value, .. } => vec![*map, *key, *value],
    ...
    pub fn rewrite_values(&mut self, mut rewrite: impl FnMut(&mut ValueId)) {
        match self {
            ...
            Self::InstallIndexed { map, key, value, .. } => {
                rewrite(map); rewrite(key); rewrite(value);
            }
