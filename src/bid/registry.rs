...
impl PluginRegistry {
    ...
    pub fn load_from_config(path: &str) -> BidResult<Self> {
        let debug_log = env::debug_plugin() || env::cli_verbose_enabled();
        if debug_log {
            get_global_ring0().log.debug(&format!(
                "[bid/registry] load_from_config path={}",
                path
            ));
        }
        ...
        let config = NyashConfigV2::from_file(path).map_err(|e| {
            if debug_log {
                get_global_ring0().log.warn(&format!(
                    "[bid/registry] Failed to parse config: {}",
                    e
                ));
            }
        ...
        for (lib_name, lib_def) in &config.libraries {
            if debug_log {
                get_global_ring0().log.debug(&format!(
                    "[bid/registry] Processing library: {} -> {}",
                    lib_name, lib_def.path
                ));
            }
            ...
            if debug_log {
                get_global_ring0().log.debug(&format!(
                    "[bid/registry] Loading plugin from: {}",
                    plugin_path
                ));
            }
            ...
            for box_name in &lib_def.boxes {
                if debug_log {
                    get_global_ring0().log.debug(&format!(
                        "[bid/registry] Registering box type: {}",
                        box_name
                    ));
                }
                ...
                if let Some(box_config) = config.get_box_config(lib_name, box_name, &raw_config) {
                    if debug_log {
                        get_global_ring0().log.debug(&format!(
                            "[bid/registry] Type ID: {}",
                            box_config.type_id
                        ));
                        get_global_ring0().log.debug(&format!(
                            "[bid/registry] ABI version: {}",
                            box_config.abi_version
                        ));
                        get_global_ring0().log.debug(&format!(
                            "[bid/registry] Methods: {}",
                            box_config.methods.len()
                        ));
                    }
                    ...
                    for (method_name, method_def) in &box_config.methods {
                        if debug_log {
                            get_global_ring0().log.debug(&format!(
                                "[bid/registry] Method: {} method_id={}",
                                method_name, method_def.method_id
                            ));
                        }
                    ...
                    if debug_log {
                        get_global_ring0().log.warn(
                            "[bid/registry] Plugin loading temporarily disabled (migrating to invoke-only)",
                        );
                    }
                }
            }
        ...
        if debug_log {
            get_global_ring0().log.debug(&format!(
                "[bid/registry] Registry loaded with {} box types",
                reg.type_info.len()
            ));
        }
...
pub fn init_global_from_config(path: &str) -> BidResult<()> {
    ...
    if debug_log {
        get_global_ring0().log.debug(&format!(
            "[bid/registry] init_global_from_config path={}",
            path
        ));
    }
