...
fn main() {
    ...
    if let Some(exit_code) =
        nyash_rust::cli::maybe_reject_grammar_profile_legacy_env_conflict(&config)
    {
        std::process::exit(exit_code);
    }
    ...
    if let Some(exit_code) =
        nyash_rust::cli::maybe_run_allocator_provider_activation_safety_diagnostic(&config)
    {
        std::process::exit(exit_code);
    }
    if let Some(exit_code) =
    ...
    if let Some(exit_code) =
        nyash_rust::cli::maybe_run_allocator_provider_selection_decision_diagnostic(&config)
    {
        std::process::exit(exit_code);
    }
    if let Some(exit_code) =
        nyash_rust::cli::maybe_run_allocator_provider_proof_bundle_consumption_diagnostic(&config)
    {
        std::process::exit(exit_code);
    }
    if let Some(exit_code) =
    ...
    if let Some(exit_code) =
        nyash_rust::cli::maybe_run_provider_package_selected_binary_build(&config)
    {
        std::process::exit(exit_code);
    }
