...
pub(in crate::host_providers::llvm_codegen) fn compile_published_static_v2(
    ...
    type Query =
        unsafe extern "C" fn(*mut c_void, *const c_char, usize, u32, u32, *mut u32) -> c_int;
    type Compile = unsafe extern "C" fn(
        *mut c_void,
        *const PublishedStaticFrameHeaderV2,
        *const c_char,
        *mut *mut c_char,
    ) -> c_int;
    ...
    unsafe fn result(rc: c_int, error: *mut c_char) -> Result<(), String> {
        ...
        if rc == 0 {
            ...
            Err(message.unwrap_or_else(|| "static V2 compilation failed".into()))
        }
    }
    unsafe {
        ...
        let query: Query = *library
            .get(b"hako_llvmc_static_query_v2\0")
            .map_err(|e| e.to_string())?;
        let compile: Compile = *library
            .get(b"hako_llvmc_static_compile_v2\0")
            .map_err(|e| e.to_string())?;
        let close: Close = *library
            .get(b"hako_llvmc_static_close_v2\0")
            .map_err(|e| e.to_string())?;
        ...
        let frame =
            PublishedStaticMethodCFrameV2::from_view_with_query(view, |name, block, ordinal| {
                ...
                match query(
                    ...
                    0 => Ok(Some(match consumer {
                        0 => Consumer::Array,
                        1 => Consumer::DirectArray,
                        2 => Consumer::Map,
                        3 => Consumer::File,
                        4 => Consumer::AliasOperandZero,
                        5 => Consumer::TypedObject,
                        6 => Consumer::InvalidPlan,
                        7 => Consumer::Unsupported,
...
pub(in crate::host_providers::llvm_codegen) fn compile_published_static_v2(
    _view: &PublishedMirBackendView<'_>,
    _body: &str,
    _output: &Path,
    _opts: &Opts,
) -> Result<(), String> {
