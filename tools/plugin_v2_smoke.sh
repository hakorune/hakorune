...
# Basic assertions: plugin loader ran/configured and FileBox provider came from plugin.
...
if [[ "${NYASH_PLUGIN_V2_FUNCTIONAL:-0}" == "1" && -f "$FUNC_APP" ]]; then
  ...
  if [[ $code -eq 0 ]]; then
    # We don't assert printed values strictly; only that it ran to completion.
