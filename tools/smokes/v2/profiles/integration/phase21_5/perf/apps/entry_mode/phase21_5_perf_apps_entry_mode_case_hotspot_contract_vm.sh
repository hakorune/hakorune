...
#
# Contract pin:
# - bench_apps_entry_mode_compare summary contains case-level fields:
#   source_cases_median_ms / mir_shape_prebuilt_cases_median_ms / case_delta_ms / case_winner
# - hotspot_case_delta points to the max abs(case_delta_ms) case.
# - hotspot_case_delta.significant follows PERF_APPS_ENTRY_MODE_SIGNIFICANCE_MS.

set -euo pipefail

SMOKE_NAME="phase21_5_perf_apps_entry_mode_case_hotspot_contract_vm"
...
COUNT="${PERF_APPS_ENTRY_MODE_CASE_SAMPLES:-3}"
...
hotspot_source="$(printf '%s\n' "$summary_line" | jq -r '.hotspot_case_delta.source_ms // -1')"
hotspot_prebuilt="$(printf '%s\n' "$summary_line" | jq -r '.hotspot_case_delta.mir_shape_prebuilt_ms // -1')"
...
perf_apps_assert_positive_uint "$SMOKE_NAME" "hotspot_case_delta.source_ms" "$hotspot_source" "$summary_line" || exit 1
perf_apps_assert_positive_uint "$SMOKE_NAME" "hotspot_case_delta.mir_shape_prebuilt_ms" "$hotspot_prebuilt" "$summary_line" || exit 1
...
if [ "$hotspot_winner" != "source" ] && [ "$hotspot_winner" != "mir_shape_prebuilt" ]; then
  echo "$summary_line"
  test_fail "$SMOKE_NAME: hotspot_case_delta.winner must be source|mir_shape_prebuilt (got: $hotspot_winner)"
  exit 1
fi
...
for case_name in "${APPS_WALLCLOCK_CASE_NAMES[@]}"; do
  source_ms="$(printf '%s\n' "$summary_line" | jq -r ".source_cases_median_ms[\"${case_name}\"] // -1")"
  prebuilt_ms="$(printf '%s\n' "$summary_line" | jq -r ".mir_shape_prebuilt_cases_median_ms[\"${case_name}\"] // -1")"
  delta_ms="$(printf '%s\n' "$summary_line" | jq -r ".case_delta_ms[\"${case_name}\"] // -999999")"
  winner="$(printf '%s\n' "$summary_line" | jq -r ".case_winner[\"${case_name}\"] // \"\"")"

  perf_apps_assert_positive_uint "$SMOKE_NAME" "source_cases_median_ms.${case_name}" "$source_ms" "$summary_line" || exit 1
  perf_apps_assert_positive_uint "$SMOKE_NAME" "mir_shape_prebuilt_cases_median_ms.${case_name}" "$prebuilt_ms" "$summary_line" || exit 1
  ...
  if [ "$winner" != "source" ] && [ "$winner" != "mir_shape_prebuilt" ]; then
    echo "$summary_line"
    test_fail "$SMOKE_NAME: case_winner.${case_name} must be source|mir_shape_prebuilt (got: $winner)"
    exit 1
  fi

  expected_delta=$(( prebuilt_ms - source_ms ))
  ...
  if [ "$source_ms" -le "$prebuilt_ms" ]; then
