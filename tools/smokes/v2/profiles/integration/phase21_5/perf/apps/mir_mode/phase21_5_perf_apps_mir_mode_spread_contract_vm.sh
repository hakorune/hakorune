#!/bin/bash
# phase21_5_perf_apps_mir_mode_spread_contract_vm.sh
#
# Contract pin:
# - bench_apps_mir_mode_compare.sh --json-lines N outputs:
...
SMOKE_NAME="phase21_5_perf_apps_mir_mode_spread_contract_vm"
...
COUNT="${PERF_APPS_MIR_MODE_SPREAD_SAMPLES:-3}"
...
emit_total_ms="$(printf '%s\n' "$summary_line" | jq -r '.emit_total_ms // -1')"
prebuilt_total_ms="$(printf '%s\n' "$summary_line" | jq -r '.prebuilt_total_ms // -1')"
...
for v in "$samples" "$emit_total_ms" "$prebuilt_total_ms" "$delta_ms" "$delta_ms_min" "$delta_ms_median" "$delta_ms_max"; do
  if ! [[ "$v" =~ ^-?[0-9]+$ ]]; then
    echo "$OUT"
