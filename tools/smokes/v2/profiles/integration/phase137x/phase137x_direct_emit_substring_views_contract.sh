#!/bin/bash
# phase-137x direct emit smoke for the substring-views exact seed route
#
# Contract:
# 1) direct MIR owns the exact seed payload and function-level backend tag for
#    `bench_kilo_micro_substring_views_only.hako`.
...
#    runtime/public substring/length helpers.
...
if [ "${SMOKES_FORCE_LLVM:-0}" != "1" ] && ! can_run_llvm; then
    test_skip "phase137x_direct_emit_substring_views_contract: LLVM backend not available"
    exit 0
fi

SMOKE_NAME="phase137x_direct_emit_substring_views_contract"
EMIT_ROUTE="$NYASH_ROOT/tools/smokes/v2/lib/emit_mir_route.sh"
MIR_BUILDER="$NYASH_ROOT/tools/ny_mir_builder.sh"
INPUT="$NYASH_ROOT/benchmarks/bench_kilo_micro_substring_views_only.hako"
...
route = metadata.get("substring_views_micro_seed_route")
if not isinstance(route, dict):
    raise SystemExit("missing substring_views_micro_seed_route")
if route.get("proof") != "kilo_micro_substring_views_only_5block":
    raise SystemExit("wrong substring_views proof")
if route.get("source_len") != 16 or route.get("loop_bound") != 300000:
    raise SystemExit("wrong substring_views payload")

backend = metadata.get("exact_seed_backend_route")
if not isinstance(backend, dict):
    raise SystemExit("missing exact_seed_backend_route")
if backend.get("tag") != "substring_views_only_micro":
    raise SystemExit("wrong exact seed backend route tag")
if backend.get("source_route") != "substring_views_micro_seed_route":
    raise SystemExit("wrong exact seed backend route source")
if backend.get("proof") != "kilo_micro_substring_views_only_5block":
    raise SystemExit("wrong exact seed backend route proof")
PY
then
    echo "[INFO] emitted MIR:"
    sed -n '1,240p' "$OUT_JSON" || true
    test_fail "$SMOKE_NAME: MIR metadata did not expose the substring-views exact backend route"
    exit 1
fi
...
if ! grep -Fq "stage=substring_views_only_micro result=emit reason=exact_match" "$BUILD_LOG"; then
    echo "[INFO] compile log:"
    tail -n 160 "$BUILD_LOG" || true
    test_fail "$SMOKE_NAME: substring-views exact emitter did not match"
    exit 1
fi
...
test_pass "$SMOKE_NAME: PASS (substring-views exact seed is selected by function-level backend route tag)"
