#!/bin/bash
# phase-137x direct emit smoke for the array get/set micro exact seed route
#
# Contract:
# 1) direct MIR owns both the inner array RMW window and the whole-function
#    exact seed route for `bench_kilo_micro_array_getset.hako`.
...
if [ "${SMOKES_FORCE_LLVM:-0}" != "1" ] && ! can_run_llvm; then
    test_skip "phase137x_direct_emit_array_getset_micro_contract: LLVM backend not available"
    exit 0
fi

SMOKE_NAME="phase137x_direct_emit_array_getset_micro_contract"
EMIT_ROUTE="$NYASH_ROOT/tools/smokes/v2/lib/emit_mir_route.sh"
MIR_BUILDER="$NYASH_ROOT/tools/ny_mir_builder.sh"
INPUT="$NYASH_ROOT/benchmarks/bench_kilo_micro_array_getset.hako"
...
seed = metadata.get("array_getset_micro_seed_route")
if not isinstance(seed, dict):
    raise SystemExit("missing array_getset_micro_seed_route")
expected_seed = {
    "size": 128,
    "ops": 2000000,
    "init_push_count": 1,
    ...
    "loop_index_phi_value": 29,
    "accumulator_phi_value": 33,
    "accumulator_next_value": 40,
    "return_value": 33,
...
if backend.get("tag") != "array_getset_micro":
    raise SystemExit(f"wrong exact seed backend route tag: {backend}")
if backend.get("source_route") != "array_getset_micro_seed_route":
    raise SystemExit(f"wrong exact seed backend route source: {backend}")
if backend.get("proof") != "kilo_micro_array_getset_7block":
...
then
    ...
    sed -n '1,260p' "$OUT_JSON" || true
    test_fail "$SMOKE_NAME: MIR metadata did not expose the array get/set exact backend route"
    exit 1
fi
...
if ! grep -Fq "stage=array_getset_micro result=emit reason=exact_match" "$BUILD_LOG"; then
    echo "[INFO] compile log:"
    tail -n 160 "$BUILD_LOG" || true
    test_fail "$SMOKE_NAME: array get/set exact emitter did not match"
    exit 1
fi
