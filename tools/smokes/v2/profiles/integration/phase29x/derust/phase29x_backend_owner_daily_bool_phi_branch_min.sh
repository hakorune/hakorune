#!/bin/bash
# Phase 29x: backend-owner daily bool-phi/branch owner flip smoke
...
if ! command -v llc >/dev/null 2>&1 && ! command -v llc-18 >/dev/null 2>&1; then
    test_skip "phase29x_backend_owner_daily_bool_phi_branch_min: llc not found"
    exit 0
fi

SMOKE_NAME="phase29x_backend_owner_daily_bool_phi_branch_min"
...
test_pass "$SMOKE_NAME: PASS (bool phi/branch uses .hako ll emitter as daily owner)"
