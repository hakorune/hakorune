#!/bin/bash
# Phase 29x: backend-owner daily RuntimeDataBox.get(ArrayBox) owner flip smoke
...
if ! command -v llc >/dev/null 2>&1 && ! command -v llc-18 >/dev/null 2>&1; then
    test_skip "phase29x_backend_owner_daily_runtime_data_array_get_min: llc not found"
    exit 0
fi

SMOKE_NAME="phase29x_backend_owner_daily_runtime_data_array_get_min"
...
if ! echo "$out" | grep -Fq "[hako-ll/daily] chosen_owner=hako-ll-min-v0 accepted=min-v0 first_blocker=none acceptance_case=runtime-data-array-get-missing-v1 legacy_daily_allowed=no"; then
    echo "[INFO] daily output:"
    echo "$out" | head -n 120 || true
...
test_pass "$SMOKE_NAME: PASS (RuntimeDataBox.get(ArrayBox) uses .hako ll emitter as daily owner)"
