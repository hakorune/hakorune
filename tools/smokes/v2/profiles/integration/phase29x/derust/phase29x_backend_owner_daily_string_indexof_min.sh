#!/bin/bash
# Phase 29x: backend-owner daily StringBox.indexOf owner flip smoke
...
if ! command -v llc >/dev/null 2>&1 && ! command -v llc-18 >/dev/null 2>&1; then
    test_skip "phase29x_backend_owner_daily_string_indexof_min: llc not found"
    exit 0
fi

SMOKE_NAME="phase29x_backend_owner_daily_string_indexof_min"
...
if ! echo "$out" | grep -Fq "[hako-ll/daily] chosen_owner=hako-ll-min-v0 accepted=min-v0 first_blocker=none acceptance_case=string-indexof-ascii-v1 legacy_daily_allowed=no"; then
    echo "[INFO] daily output:"
    echo "$out" | head -n 120 || true
...
test_pass "$SMOKE_NAME: PASS (StringBox.indexOf uses .hako ll emitter as daily owner)"
