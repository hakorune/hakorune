#!/bin/bash
# Phase 29x: backend-owner daily RuntimeDataBox.length(StringBox) owner flip smoke
...
if ! command -v llc >/dev/null 2>&1 && ! command -v llc-18 >/dev/null 2>&1; then
    test_skip "phase29x_backend_owner_daily_runtime_data_length_min: llc not found"
    exit 0
fi

SMOKE_NAME="phase29x_backend_owner_daily_runtime_data_length_min"
...
if ! echo "$out" | grep -Fq "[hako-ll/daily] chosen_owner=hako-ll-min-v0 accepted=min-v0 first_blocker=none acceptance_case=runtime-data-string-length-ascii-v1 legacy_daily_allowed=no"; then
    echo "[INFO] daily output:"
    echo "$out" | head -n 120 || true
...
test_pass "$SMOKE_NAME: PASS (RuntimeDataBox.length(StringBox) uses .hako ll emitter as daily owner)"
