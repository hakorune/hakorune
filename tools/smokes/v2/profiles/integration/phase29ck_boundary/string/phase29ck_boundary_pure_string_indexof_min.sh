...
if [ "${SMOKES_FORCE_LLVM:-0}" != "1" ] && ! can_run_llvm; then
    test_skip "phase29ck_boundary_pure_string_indexof_min: LLVM backend not available"
    exit 0
fi

if ! command -v llc >/dev/null 2>&1 && ! command -v llc-18 >/dev/null 2>&1; then
    test_skip "phase29ck_boundary_pure_string_indexof_min: llc not found"
    exit 0
fi

FIXTURE="$NYASH_ROOT/apps/tests/mir_shape_guard/string_indexof_ascii_min_v1.mir.json"
NY_LLVM_C="$NYASH_ROOT/target/release/ny-llvmc"
OUT_OBJ="${TMPDIR:-/tmp}/phase29ck_boundary_pure_string_indexof_min_$$.o"
BUILD_LOG="${TMPDIR:-/tmp}/phase29ck_boundary_pure_string_indexof_min_$$.log"
...
if [ ! -f "$FIXTURE" ]; then
    test_fail "phase29ck_boundary_pure_string_indexof_min: fixture missing: $FIXTURE"
    exit 1
fi

if [ ! -x "$NY_LLVM_C" ]; then
    test_fail "phase29ck_boundary_pure_string_indexof_min: ny-llvmc missing: $NY_LLVM_C"
    exit 1
fi
...
if [ "$BUILD_RC" -eq 124 ]; then
    test_fail "phase29ck_boundary_pure_string_indexof_min: compile timed out (>${RUN_TIMEOUT_SECS}s)"
    exit 1
fi
...
if [ "$BUILD_RC" -ne 0 ]; then
    ...
    test_fail "phase29ck_boundary_pure_string_indexof_min: boundary default still relied on harness fallback (rc=$BUILD_RC)"
    exit 1
fi

if [ ! -f "$OUT_OBJ" ]; then
    test_fail "phase29ck_boundary_pure_string_indexof_min: object missing: $OUT_OBJ"
    exit 1
fi
