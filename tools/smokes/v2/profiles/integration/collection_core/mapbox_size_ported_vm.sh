#!/bin/bash
# RVP-C18: collection-core witness for MapBox.size()
#
# Contract:
# 1) MIR preflight must contain `mir_call(MapBox.size,args=0)`.
...
vm_hako_caps_assert_mir_jq \
  "$SMOKE_NAME" \
  "$TMP_MIR" \
  '.functions[]?.blocks[]?.instructions[]? | select(.op=="mir_call" and .mir_call.callee.type=="Method" and .mir_call.callee.box_name=="MapBox" and .mir_call.callee.name=="size" and (.mir_call.args|length)==0)' \
  "MIR missing mir_call(MapBox.size,args=0) shape" || exit 1
