#!/bin/bash
# phase29y_hako_emit_mir_mapbox_keys_exec_contract_vm.sh
# Contract pin:
# - selfhost emit must preserve `mir_call(MapBox.keys,args=0)` in MIR.
...
SMOKE_NAME="phase29y_hako_emit_mir_mapbox_keys_exec_contract_vm"
INPUT_FIXTURE="${1:-$NYASH_ROOT/apps/tests/phase29y_hako_emit_mir_mapbox_keys_exec_min.hako}"
RUN_TIMEOUT_SECS="${RUN_TIMEOUT_SECS:-20}"

phase29y_binary_only_require_input_and_bin "$SMOKE_NAME" "$INPUT_FIXTURE" || exit 2
phase29y_binary_only_prepare_workdir "$INPUT_FIXTURE" "phase29y_hako_emit_mapbox_keys_exec"
cleanup() {
...
if ! jq -e '.functions[]?.blocks[]?.instructions[]? | select(.op=="mir_call" and .mir_call.callee.type=="Method" and .mir_call.callee.box_name=="MapBox" and .mir_call.callee.name=="keys" and (.mir_call.args|length)==0)' \
  "$PHASE29Y_BINARY_ONLY_WORKDIR/out.mir" >/dev/null 2>&1; then
  tail -n 80 "$PHASE29Y_BINARY_ONLY_WORKDIR/out.mir" || true
  test_fail "$SMOKE_NAME: emitted MIR missing MapBox.keys"
  exit 1
fi
...
if printf '%s\n' "$RUN_OUTPUT" | rg -q '^\[vm/method/stub:(keys|size)\]$'; then
  phase29y_binary_only_tail_output
...
test_pass "$SMOKE_NAME: PASS (.hako emit preserves MapBox.keys and rc=0)"
