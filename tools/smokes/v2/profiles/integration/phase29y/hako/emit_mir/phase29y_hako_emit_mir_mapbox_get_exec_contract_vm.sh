#!/bin/bash
# phase29y_hako_emit_mir_mapbox_get_exec_contract_vm.sh
# Contract pin:
# - selfhost emit must preserve `mir_call(MapBox.get,args=1)` in MIR.
# - emitted MIR must execute through `--mir-json-file` without vm_hako helpers.
# - visible result must stay fixed at `42` with rc=0.
...
SMOKE_NAME="phase29y_hako_emit_mir_mapbox_get_exec_contract_vm"
INPUT_FIXTURE="${1:-$NYASH_ROOT/apps/tests/phase29y_hako_emit_mir_mapbox_get_exec_min.hako}"
RUN_TIMEOUT_SECS="${RUN_TIMEOUT_SECS:-20}"

phase29y_binary_only_require_input_and_bin "$SMOKE_NAME" "$INPUT_FIXTURE" || exit 2
phase29y_binary_only_prepare_workdir "$INPUT_FIXTURE" "phase29y_hako_emit_mapbox_get_exec"
cleanup() {
...
if ! jq -e '.functions[]?.blocks[]?.instructions[]? | select(.op=="mir_call" and .mir_call.callee.type=="Method" and .mir_call.callee.box_name=="MapBox" and .mir_call.callee.name=="get" and (.mir_call.args|length)==1)' \
  "$PHASE29Y_BINARY_ONLY_WORKDIR/out.mir" >/dev/null 2>&1; then
  tail -n 80 "$PHASE29Y_BINARY_ONLY_WORKDIR/out.mir" || true
  test_fail "$SMOKE_NAME: emitted MIR missing MapBox.get"
  exit 1
fi
...
if printf '%s\n' "$RUN_OUTPUT" | rg -q '^\[vm/method/stub:(get)\]$'; then
  phase29y_binary_only_tail_output
...
test_pass "$SMOKE_NAME: PASS (.hako emit preserves MapBox.get and rc=0)"
