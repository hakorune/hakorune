#!/usr/bin/env bash
# Build EXE from minimal Hako (if/compare) via Program(JSON)->MIR(JSON) robust route and check rc.
set -euo pipefail
...
cat >"${TMP_HAKO}" <<'HAKO'
static box Main { method main(args) { if (1 == 1) { return 5 } else { return 0 } } }
HAKO
