#!/usr/bin/env bash
# Opt-in canary for MirBuilderMin: return.method.arraymap (get)
set -euo pipefail
...
if ! grep -q "\[mirbuilder/min:return.method.arraymap\]" "$tmp_stdout"; then
  echo "[SKIP] min tag not observed"; exit 0
fi
mir=$(awk '/\[MIR_BEGIN\]/{flag=1;next}/\[MIR_END\]/{flag=0}flag' "$tmp_stdout")
if [[ -z "$mir" ]] || ! echo "$mir" | grep -q '"functions"'; then echo "[SKIP] MIR missing functions (min)"; exit 0; fi
echo "[PASS] builder_min_method_arraymap_get"
exit 0
