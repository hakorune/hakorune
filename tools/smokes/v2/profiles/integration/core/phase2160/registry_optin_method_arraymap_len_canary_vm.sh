...
if ! grep -q "\[mirbuilder/registry:return.method.arraymap\]" "$tmp_stdout"; then
  echo "[SKIP] registry tag not observed (return.method.arraymap len)"; exit 0
fi
...
# Token checks: mir_call + method=length + 0 args
if ! echo "$mir" | grep -q '"op":"mir_call"'; then echo "[SKIP] mir_call op missing"; exit 0; fi
if ! echo "$mir" | grep -q '"method":"length"'; then echo "[SKIP] method=length missing"; exit 0; fi
if ! echo "$mir" | grep -q '"args":\[\]'; then echo "[SKIP] args[0] missing"; exit 0; fi
echo "[PASS] registry_optin_method_arraymap_len"
exit 0
