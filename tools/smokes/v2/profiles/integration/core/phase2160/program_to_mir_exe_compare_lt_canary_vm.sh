#!/usr/bin/env bash
# Build EXE from minimal Hako (if/compare Lt) via Program(JSON)->MIR(JSON) robust route and check rc.
set -euo pipefail
...
cat >"${TMP_HAKO}" <<'HAKO'
static box Main { method main(args) { if (1 < 2) { return 6 } else { return 0 } } }
HAKO
...
rc=$?
if [[ "$rc" -eq 6 ]]; then
  echo "[PASS] program_to_mir_exe_compare_lt (rc=6)"
  exit 0
fi
echo "[SKIP] EXE rc != 6 (rc=${rc})"
exit 0
