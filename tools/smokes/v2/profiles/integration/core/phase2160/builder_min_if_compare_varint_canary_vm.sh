...
if ! grep -q "\[mirbuilder/min:if.compare.varint\]" "$tmp_stdout"; then
  echo "[SKIP] min tag not observed (if varint)"; exit 0
fi
mir=$(awk '/\[MIR_BEGIN\]/{flag=1;next}/\[MIR_END\]/{flag=0}flag' "$tmp_stdout")
if [[ -z "$mir" ]] || ! echo "$mir" | grep -q '"functions"'; then echo "[SKIP] MIR missing functions (min/if varint)"; exit 0; fi
echo "[PASS] builder_min_if_compare_varint"
exit 0
