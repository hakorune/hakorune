#!/usr/bin/env bash
# Opt-in canary for MirBuilderMin: return.method.arraymap (push)
set -euo pipefail
...
# Return(Method Var recv 'a', method 'push', args [Int 42])
PROG='{"version":0,"kind":"Program","body":[{"type":"Return","expr":{"type":"Method","recv":{"type":"Var","name":"a"},"method":"push","args":[{"type":"Int","value":42}]}}]}'
...
if ! grep -q "\[mirbuilder/min:return.method.arraymap\]" "$tmp_stdout"; then
  echo "[SKIP] min tag not observed (push)"; exit 0
fi
mir=$(awk '/\[MIR_BEGIN\]/{flag=1;next}/\[MIR_END\]/{flag=0}flag' "$tmp_stdout")
if [[ -z "$mir" ]] || ! echo "$mir" | grep -q '"functions"'; then echo "[SKIP] MIR missing functions (min/push)"; exit 0; fi
echo "[PASS] builder_min_method_arraymap_push"
exit 0
