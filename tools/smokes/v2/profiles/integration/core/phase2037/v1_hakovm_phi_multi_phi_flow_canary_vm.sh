#!/bin/bash
# v1 φ: multiple phi instructions in a block
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"; if ROOT_GIT=$(git -C "$SCRIPT_DIR" rev-parse --show-toplevel 2>/dev/null); then ROOT="$ROOT_GIT"; else ROOT="$(cd "$SCRIPT_DIR/../../../../../../../../.." && pwd)"; fi
source "$ROOT/tools/smokes/v2/lib/test_runner.sh"; require_env || exit 2

tmp_json="/tmp/mir_v1_phi_multi_phi_$$.json"
cat > "$tmp_json" <<'JSON'
{
  "schema_version": "1.0",
  "functions": [
...
if [ "$rc" -eq 9 ]; then
  echo "[PASS] v1_hakovm_phi_multi_phi_flow_canary_vm"
  exit 0
fi
echo "[FAIL] v1_hakovm_phi_multi_phi_flow_canary_vm (rc=$rc, expect 9)" >&2; exit 1
