...
prog_json_path="/tmp/prog_2044_map_rc_$$.json"
...
rm -f "$prog_json_path"
if [ "$rc" -ne 1 ]; then
  echo "[FAIL] provider map set/size RC → core rc=$rc (expected 1)" >&2
  exit 1
fi

echo "[PASS] phase2044/mirbuilder_provider_map_set_size_rc_core_exec_canary_vm"
exit 0
