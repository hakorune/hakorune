#!/bin/bash
# hv1 inline: MapBox per‑recv size state — first recv gets size=1（rc=1）, second stays 0（not checked here）
set -euo pipefail
...
tmp="/tmp/hv1_map_two_recv_first_$$.json"; printf '%s' "$json" > "$tmp"
set +e
...
if [[ "$rc" =~ ^-?[0-9]+$ ]] && [ "$rc" -eq 1 ]; then
  echo "[PASS] hv1_inline_map_two_recv_first_rc1_canary_vm"
  exit 0
fi
# Older binaries may not support Map per‑recv sizing → rc=0 を SKIP
if [[ "$rc" =~ ^-?[0-9]+$ ]] && [ "$rc" -eq 0 ]; then
  echo "[SKIP] hv1_inline_map_two_recv_first_rc1_canary_vm (Map per‑recv sizing not available)" >&2
  exit 0
fi
echo "[FAIL] hv1_inline_map_two_recv_first_rc1_canary_vm (rc=$rc, expect 1)" >&2
exit 1
