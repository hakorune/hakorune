#!/bin/bash
# Logical cross: ( (1<2) || (0<0) ) && (4<5) → rc=1
set -euo pipefail
...
json='{"schema_version":"1.0","functions":[{"name":"main","blocks":[
  ...
  {"id":12,"instructions":[{"op":"const","dst":21,"value":{"type":"i64","value":1}},{"op":"ret","value":21}]},
  {"id":13,"instructions":[{"op":"const","dst":22,"value":{"type":"i64","value":0}},{"op":"ret","value":22}]}
]}]}'

tmp="/tmp/logical_cross_$$.json"; printf '%s' "$json" > "$tmp"
set +e
...
if [[ "$rc" =~ ^-?[0-9]+$ ]] && [ "$rc" -eq 1 ]; then
  echo "[PASS] logical_and_or_cross_primary_canary_vm"
  exit 0
fi
echo "[FAIL] logical_and_or_cross_primary_canary_vm (rc=$rc, expect 1)" >&2
exit 1
