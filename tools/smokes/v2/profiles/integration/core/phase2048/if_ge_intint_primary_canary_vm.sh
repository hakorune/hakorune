#!/bin/bash
# If (>=) int-int — PRIMARY hv1 inline rc=2
set -euo pipefail
...
json='{"schema_version":"1.0","functions":[{"name":"main","blocks":[
  {"id":0,"instructions":[
    ...
    {"op":"compare","dst":3,"lhs":1,"rhs":2,"cmp":"Ge"},
  ...
  {"id":2,"instructions":[
    {"op":"const","dst":11,"value":{"type":"i64","value":2}},
    {"op":"ret","value":11}
  ]}
]}]}'

tmp="/tmp/if_ge_ii_$$.json"; printf '%s' "$json" > "$tmp"
set +e
...
if [[ "$rc" =~ ^-?[0-9]+$ ]] && [ "$rc" -eq 2 ]; then
  echo "[PASS] if_ge_intint_primary_canary_vm"
  exit 0
fi
echo "[FAIL] if_ge_intint_primary_canary_vm (rc=$rc, expect 2)" >&2
exit 1
