...
if [ "$rc" -eq 42 ]; then
  echo "[PASS] mirbuilder_prefer_mirbuilder_match_core_exec_canary_vm"
  exit 0
fi
echo "[FAIL] mirbuilder_prefer_mirbuilder_match_core_exec_canary_vm (rc=$rc, expect 42)" >&2; exit 1
