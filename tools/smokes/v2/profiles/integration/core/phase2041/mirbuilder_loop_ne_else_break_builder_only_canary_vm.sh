...
tmp_json="/tmp/program_loop_ne_else_break_$$.json"
cat > "$tmp_json" <<'JSON'
...
if [ $rc -ne 0 ]; then
  echo "[FAIL] mirbuilder_loop_ne_else_break_builder_only_canary_vm (builder-only structure)" >&2
  exit 1
fi
echo "[PASS] mirbuilder_loop_ne_else_break_builder_only_canary_vm"
exit 0
