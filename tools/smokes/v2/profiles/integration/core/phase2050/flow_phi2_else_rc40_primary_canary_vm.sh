#!/bin/bash
# PRIMARY hv1-inline: 2-stage PHI + branch (else path) -> rc=40
set -euo pipefail
...
tmp="/tmp/phi2_else_$$.json"; printf '%s' "$json" > "$tmp"
set +e
rc=$(HAKO_PRIMARY_NO_FALLBACK=1 HAKO_VERIFY_PRIMARY=hakovm verify_v1_inline_file "$tmp" || true)
set -e
rm -f "$tmp"

if [[ "$rc" =~ ^-?[0-9]+$ ]] && [ "$rc" -eq 40 ]; then
  echo "[PASS] flow_phi2_else_rc40_primary_canary_vm"
  exit 0
fi
echo "[FAIL] flow_phi2_else_rc40_primary_canary_vm (rc=$rc, expect 40)" >&2
exit 1
