...
if [ "$rc" -eq 0 ]; then
  echo "[PASS] selfhost_s1_s2_from_builder_compare_ret_canary_vm"
  exit 0
fi
# JSON整形の差異がある場合は dev-only としてSKIP（emit_compare_retのstring直組み依存）
...
if echo "$out" | grep -qE "(Loop lowering failed|StepTree lowering returned None|loop pattern is not supported|cap_missing/NestedLoop|Argument list too long|strict mode: pattern not matched)"; then
  echo "[SKIP] selfhost_s1_s2_from_builder_compare_ret_canary_vm: Known route gap (see investigation doc)" >&2
