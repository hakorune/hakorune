#!/bin/bash
# PRIMARY no-fallback: hv1 inline executes v1 compare/branch → rc=1
set -euo pipefail
...
tmp="/tmp/primary_cmp_$$.json"; printf '%s' "$json" > "$tmp"
set +e
...
if [[ "$rc" =~ ^-?[0-9]+$ ]] && [ "$rc" -eq 1 ]; then
  echo "[PASS] primary_no_fallback_v1_compare_branch_rc_canary_vm"
  exit 0
fi
echo "[FAIL] primary_no_fallback_v1_compare_branch_rc_canary_vm (rc=$rc, expect 1)" >&2
exit 1
