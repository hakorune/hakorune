#!/bin/bash
# extern: env.get — provider reads an environment key; we ignore return and rc=0 via explicit ret
set -euo pipefail
...
cat > "$tmp_json" <<'JSON'
{
  ...
  "functions": [
    {"name":"main","blocks":[
      {"id":0,"instructions":[
        {"op":"const","dst":2, "value": {"type": {"kind":"handle","box_type":"StringBox"}, "value":"HOME"}},
        {"op":"mir_call", "dst": 1, "callee": {"type":"Extern","name":"env.get"}, "args": [2]},
