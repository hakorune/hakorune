#!/bin/bash
# RVP-C26: vm-hako capability smoke for MapBox.set(non-string key, value)
#
# Contract:
...
SMOKE_NAME="vm_hako_caps_mapbox_set_bad_key_ported_vm"
INPUT="${1:-$NYASH_ROOT/apps/tests/vm_hako_caps/mapbox_set_bad_key_ported_min.hako}"
...
if printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^\[vm-hako/unimplemented op=boxcall method=set\]$'; then
  ...
  test_fail "vm_hako_caps_mapbox_set_bad_key_ported_vm: stale unimplemented set blocker remained"
  exit 1
fi
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^\[map/bad-key\] key must be string$'; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "vm_hako_caps_mapbox_set_bad_key_ported_vm: expected stable bad-key text"
  exit 1
fi
if [ "$VM_HAKO_CAPS_EXIT_CODE" -ne 0 ]; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "vm_hako_caps_mapbox_set_bad_key_ported_vm: expected RC=0 (got rc=$VM_HAKO_CAPS_EXIT_CODE)"
  exit 1
fi

vm_hako_caps_assert_no_unimplemented "$SMOKE_NAME" "$OUTPUT_CLEAN" || exit 1
test_pass "vm_hako_caps_mapbox_set_bad_key_ported_vm: PASS (RVP-C26 ported contract pinned)"
