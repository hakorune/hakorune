#!/bin/bash
# RVP-C20: vm-hako capability smoke for AtomicCoreBox.fence_i64()
#
# Contract:
# 1) MIR preflight must contain `AtomicCoreBox.fence_i64()` call shape.
# 2) vm-hako route must print `0` and exit with rc=0.
...
vm_hako_caps_assert_mir_jq \
  "$SMOKE_NAME" \
  "$TMP_MIR" \
  '.functions[]?.blocks[]?.instructions[]? | select(.op=="call" and .callee.type=="Global" and .callee.name=="AtomicCoreBox.fence_i64/0")' \
  "MIR missing AtomicCoreBox.fence_i64() call shape" || exit 1
...
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^0$'; then
  ...
  test_fail "vm_hako_caps_atomic_fence_ported_vm: expected printed 0"
  exit 1
fi
if [ "$VM_HAKO_CAPS_EXIT_CODE" -ne 0 ]; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "vm_hako_caps_atomic_fence_ported_vm: expected RC=0 (got rc=$VM_HAKO_CAPS_EXIT_CODE)"
  exit 1
fi
...
test_pass "vm_hako_caps_atomic_fence_ported_vm: PASS (RVP-C20 ported contract pinned)"
