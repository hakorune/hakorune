#!/bin/bash
# RVP-C10: vm-hako capability smoke for compare(>=) route (ported pin)
#
# Contract:
...
# 4) runtime alias path (`op_kind=Ge`) must remain executable via MiniVmS0Entry.
...
SMOKE_NAME="vm_hako_caps_compare_ge_ported_vm"
INPUT="${1:-$NYASH_ROOT/apps/tests/vm_hako_caps/compare_ge_block_min.hako}"
GE_ALIAS_INPUT="${GE_ALIAS_INPUT:-$NYASH_ROOT/apps/tests/vm_hako_caps/compare_op_kind_ge_mir_v0.json}"
RUN_TIMEOUT_SECS="${RUN_TIMEOUT_SECS:-60}"
TMP_MIR="$(mktemp /tmp/vm_hako_caps_c10.XXXXXX.json)"
TMP_DRIVER="$(mktemp /tmp/vm_hako_caps_c10_driver.XXXXXX.hako)"
cleanup() {
...
vm_hako_caps_require_fixture "$SMOKE_NAME" "$GE_ALIAS_INPUT" || exit 1
...
if [ "$VM_HAKO_CAPS_EXIT_CODE" -ne 0 ]; then
  ...
  test_fail "vm_hako_caps_compare_ge_ported_vm: expected success exit (rc=0), got rc=$VM_HAKO_CAPS_EXIT_CODE"
  exit 1
fi
...
if echo "$VM_HAKO_CAPS_OUTPUT" | rg -q '^\[vm-hako/emit-error\]'; then
  echo "$VM_HAKO_CAPS_OUTPUT" | tail -n 80 || true
  test_fail "vm_hako_caps_compare_ge_ported_vm: unexpected emit-error tag"
  exit 1
fi
vm_hako_caps_assert_rc_marker "$SMOKE_NAME" "$VM_HAKO_CAPS_OUTPUT" "0" || exit 1

# runtime alias pin: run MiniVmS0Entry directly with op_kind=Ge payload
GE_ALIAS_JSON_PAYLOAD="$(tr -d '\n\r' < "$GE_ALIAS_INPUT")"
cat >"$TMP_DRIVER" <<'HKO'
...
GE_ALIAS_OUTPUT=$(
  env -u HAKO_VERIFY_PRIMARY -u HAKO_ROUTE_HAKOVM \
    NYASH_VERIFY_JSON="$GE_ALIAS_JSON_PAYLOAD" \
    NYASH_PREINCLUDE=1 \
    NYASH_USING_AST=1 \
...
if [ "$GE_ALIAS_RC" -eq 124 ]; then
  test_fail "vm_hako_caps_compare_ge_ported_vm: op_kind=Ge runtime alias probe timed out"
  exit 1
fi
if [ "$GE_ALIAS_RC" -ne 0 ]; then
  echo "$GE_ALIAS_OUTPUT" | tail -n 100 || true
  test_fail "vm_hako_caps_compare_ge_ported_vm: op_kind=Ge runtime alias probe failed (rc=$GE_ALIAS_RC)"
  exit 1
fi
if echo "$GE_ALIAS_OUTPUT" | rg -q '^\[vm-hako/unimplemented\]'; then
  echo "$GE_ALIAS_OUTPUT" | tail -n 100 || true
  test_fail "vm_hako_caps_compare_ge_ported_vm: op_kind=Ge emitted vm-hako unimplemented tag"
  exit 1
fi
if echo "$GE_ALIAS_OUTPUT" | rg -q '^\[vm-hako/contract'; then
  echo "$GE_ALIAS_OUTPUT" | tail -n 100 || true
  test_fail "vm_hako_caps_compare_ge_ported_vm: op_kind=Ge emitted vm-hako contract tag"
  exit 1
fi

test_pass "vm_hako_caps_compare_ge_ported_vm: PASS (RVP-C10 ported contract + op_kind=Ge runtime alias pinned)"
