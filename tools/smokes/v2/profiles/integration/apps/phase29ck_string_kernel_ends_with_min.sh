#!/bin/bash
# Phase 29ck string kernel ends_with narrow pilot smoke
#
# Contract pin:
# - `lang.runtime.kernel.string.search` resolves to a narrow `.hako` kernel module.
# - The kernel module provides `ends_with` for the smoke fixture.
...
SMOKE_NAME="phase29ck_string_kernel_ends_with_min"
APP="$NYASH_ROOT/apps/tests/string_kernel_ends_with_min.hako"
...
if ! grep -Fq "OK: string ends_with" "$TMP_LOG"; then
    tail -n 120 "$TMP_LOG" || true
    test_fail "$SMOKE_NAME: expected success marker missing"
    exit 1
fi

test_pass "$SMOKE_NAME: PASS (string search ends_with narrow pilot)"
