...
WORK_DIR="$(mktemp -d -t phase29cc_plg07_filebox.XXXXXX)"
...
if ! grep -q 'file_bytes=PLG07_BINARY_OK' "$OUTPUT_FILE"; then
  tail -n 80 "$OUTPUT_FILE" || true
  test_fail "$SMOKE_NAME: expected output 'file_bytes=PLG07_BINARY_OK' not found"
  exit 1
fi
