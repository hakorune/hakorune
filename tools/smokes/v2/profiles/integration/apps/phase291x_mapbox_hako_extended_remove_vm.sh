#!/bin/bash
# Phase 291x: vm-hako MapBox.remove() source-route smoke.
set -euo pipefail
...
SMOKE_NAME="phase291x_mapbox_hako_extended_remove_vm"
INPUT="${1:-$ROOT/apps/tests/phase291x_mapbox_hako_remove_min.hako}"
RUN_TIMEOUT_SECS="${RUN_TIMEOUT_SECS:-30}"
TMP_MIR="$(mktemp /tmp/phase291x_mapbox_remove.XXXXXX.json)"
cleanup() {
  rm -f "$TMP_MIR"
}
trap cleanup EXIT
...
if printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^\[vm/method/stub:(remove|delete|has|size)\]$'; then
...
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^1$'; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "$SMOKE_NAME: expected printed size result 1"
  exit 1
fi
...
test_pass "$SMOKE_NAME: PASS (MapBox.remove vm-hako alias pinned)"
