#!/bin/bash
# typed-object-method-min EXE smoke
#
# Contract pin:
# - MIR emits user_box_method_routes for Pair.birth/2 and Pair.sum/0.
# - pure-first EXE treats newbox args as allocation-adjacent metadata only.
# - sum lowers through same-module uniform ABI and consumes TypedObjectPlan.
...
SMOKE_NAME="typed_object_method_min_exe"
APP="$HAKO_ROOT/apps/typed-object-method-min/main.hako"
NY_LLVM_C="$HAKO_ROOT/target/release/ny-llvmc"
RUN_TIMEOUT_SECS="${RUN_TIMEOUT_SECS:-120}"
TMP_ROOT="${TMPDIR:-/tmp}/hakorune_typed_object_method_min_$$"
