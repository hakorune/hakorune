#!/bin/bash
# APP-PERF-02: kilo_kernel_small cross-language contract smoke
#
# Contract:
# 1) bench_compare_c_py_vs_hako.sh exits with code 0
# 2) Output contains [bench4] name=kilo_kernel_small
...
set +e
OUTPUT=$(PERF_VM_TIMEOUT="${PERF_VM_TIMEOUT:-60s}" HAKO_VM_MAX_STEPS="${HAKO_VM_MAX_STEPS:-300000000}" "${SCRIPT}" "${KEY}" 1 1 2>&1)
RC=$?
set -e
...
echo "${OUTPUT}"
test_pass "${SMOKE_NAME}: PASS (kilo_kernel_small 4-way cross-language contract pinned)"
