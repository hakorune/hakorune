...
if ! grep -q 'CodegenBridgeBox.emit_object_args' "$ROOT_DIR/lang/src/shared/backend/llvm_backend_box.hako"; then
  echo "[FAIL] phase29ck_llvm_backend_box_capi_link_min (owner no longer routes via CodegenBridgeBox.emit_object_args)" >&2
  exit 1
fi
