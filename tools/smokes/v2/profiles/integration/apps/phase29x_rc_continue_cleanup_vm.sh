#!/bin/bash
# Phase 29x X10: continue cleanup smoke (VM + selfcheck)
#
# Contract pin:
# 1) Baseline runtime path (default build) stays behavior-stable (exit 0).
# 2) Feature path (`rc-insertion-minimal`) keeps continue-path cleanup contract green via
#    `rc_insertion_selfcheck` (continue edge minimal case).
...
TMP_LOG="$(mktemp "${TMPDIR:-/tmp}/phase29x_rc_continue_cleanup_XXXX.log")"
trap 'rm -f "$TMP_LOG"' EXIT

if [ ! -f "$INPUT" ]; then
    test_fail "phase29x_rc_continue_cleanup_vm: fixture missing: $INPUT"
    exit 1
fi
...
if [ "$BASE_RC" -eq 124 ]; then
    test_fail "phase29x_rc_continue_cleanup_vm: baseline run timed out (>${RUN_TIMEOUT_SECS}s)"
    exit 1
fi
if [ "$BASE_RC" -ne 0 ]; then
    ...
    test_fail "phase29x_rc_continue_cleanup_vm: baseline execution failed"
    exit 1
fi
...
if [ "$SELFCHECK_RC" -eq 124 ]; then
    test_fail "phase29x_rc_continue_cleanup_vm: rc_insertion_selfcheck timed out (>${SELFCHECK_TIMEOUT_SECS}s)"
    exit 1
fi
if [ "$SELFCHECK_RC" -ne 0 ]; then
    ...
    test_fail "phase29x_rc_continue_cleanup_vm: selfcheck failed"
    exit 1
fi
...
if ! rg -q "^\[PASS\] rc_insertion_selfcheck$" "$TMP_LOG"; then
    ...
    echo "[INFO] Log tail:"
    tail -n 80 "$TMP_LOG" || true
    test_fail "phase29x_rc_continue_cleanup_vm: missing selfcheck PASS marker"
    exit 1
fi

test_pass "phase29x_rc_continue_cleanup_vm: PASS (baseline rc=0 + selfcheck rc=0)"
