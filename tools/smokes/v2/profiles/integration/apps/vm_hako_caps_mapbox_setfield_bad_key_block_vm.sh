#!/bin/bash
# RVP-C28: blocked pin for MapBox.setField(non-string key, value) visible bad-key contract
#
# Contract:
# 1) MIR preflight must contain `boxcall(method=setField,args=2)`.
# 2) vm-hako route currently reports `[vm-hako/unimplemented] ... op=boxcall(setField:args>1)`.
...
SMOKE_NAME="vm_hako_caps_mapbox_setfield_bad_key_block_vm"
INPUT="${1:-$NYASH_ROOT/apps/tests/vm_hako_caps/mapbox_setfield_bad_key_block_min.hako}"
...
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^\[vm-hako/unimplemented\] .*op=boxcall\(setField:args>1\)$'; then
  ...
  test_fail "vm_hako_caps_mapbox_setfield_bad_key_block_vm: expected stale unimplemented setField blocker"
  exit 1
fi
if [ "$VM_HAKO_CAPS_EXIT_CODE" -eq 0 ]; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "vm_hako_caps_mapbox_setfield_bad_key_block_vm: expected non-zero rc while stale blocker remains"
  exit 1
fi

test_pass "vm_hako_caps_mapbox_setfield_bad_key_block_vm: PASS (RVP-C28 blocked pin)"
