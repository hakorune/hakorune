#!/bin/bash
# Phase 121: StepTree→Normalized shadow parity check (LLVM EXE)
#
# Verifies that shadow lowering for if-only patterns works correctly in dev mode
# and that strict mode does not fail (parity check passes) with LLVM backend.
...
echo "[INFO] Phase 121: StepTree→Normalized shadow parity (LLVM EXE)"
...
OUTPUT=$(timeout "$RUN_TIMEOUT_SECS" env \
  ...
  "$NYASH_BIN" --backend llvm "$INPUT" 2>&1)
EXIT_CODE=$?
set -e
...
if [ "$FAIL_COUNT" -eq 0 ]; then
  test_pass "phase121_shadow_if_only_llvm_exe: All tests passed"
  exit 0
else
  test_fail "phase121_shadow_if_only_llvm_exe: $FAIL_COUNT test(s) failed"
  exit 1
fi
