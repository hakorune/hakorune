#!/bin/bash
# RVP-C21: blocked pin for MapBox.delete(key) after MapBox.has port
#
# Contract:
# 1) MIR preflight must contain `boxcall(method=delete,args=1)`.
# 2) vm-hako route currently stops at `op=boxcall1 method=delete`.
...
SMOKE_NAME="vm_hako_caps_mapbox_delete_block_vm"
INPUT="${1:-$NYASH_ROOT/apps/tests/vm_hako_caps/mapbox_delete_block_min.hako}"
...
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^\[vm-hako/unimplemented op=boxcall1 method=delete\]$'; then
  ...
  test_fail "vm_hako_caps_mapbox_delete_block_vm: expected stale delete blocker"
  exit 1
fi
if [ "$VM_HAKO_CAPS_EXIT_CODE" -eq 0 ]; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "vm_hako_caps_mapbox_delete_block_vm: expected non-zero exit while blocked"
  exit 1
fi

test_pass "vm_hako_caps_mapbox_delete_block_vm: PASS (RVP-C21 blocked pin)"
