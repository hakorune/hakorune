#!/bin/bash
# RVP-C25: blocked pin for MapBox.get(non-string key) visible bad-key contract
#
# Contract:
...
SMOKE_NAME="vm_hako_caps_mapbox_get_bad_key_block_vm"
INPUT="${1:-$NYASH_ROOT/apps/tests/vm_hako_caps/mapbox_get_bad_key_block_min.hako}"
...
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^0$'; then
  ...
  test_fail "vm_hako_caps_mapbox_get_bad_key_block_vm: expected stale scalar 0 for bad-key get"
  exit 1
fi
if [ "$VM_HAKO_CAPS_EXIT_CODE" -ne 0 ]; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "vm_hako_caps_mapbox_get_bad_key_block_vm: expected RC=0 while stale contract remains"
  exit 1
fi

test_pass "vm_hako_caps_mapbox_get_bad_key_block_vm: PASS (RVP-C25 blocked pin)"
