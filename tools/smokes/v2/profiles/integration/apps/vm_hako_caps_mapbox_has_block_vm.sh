#!/bin/bash
# RVP-C20: blocked pin for MapBox.has(key) after MapBox.get port
#
# Contract:
# 1) MIR preflight must contain `boxcall(method=has,args=1)`.
# 2) vm-hako route currently stops at `op=boxcall1 method=has`.
...
SMOKE_NAME="vm_hako_caps_mapbox_has_block_vm"
INPUT="${1:-$NYASH_ROOT/apps/tests/vm_hako_caps/mapbox_has_block_min.hako}"
...
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^\[vm-hako/unimplemented op=boxcall1 method=has\]$'; then
  ...
  test_fail "vm_hako_caps_mapbox_has_block_vm: expected stale has blocker"
  exit 1
fi
if [ "$VM_HAKO_CAPS_EXIT_CODE" -eq 0 ]; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "vm_hako_caps_mapbox_has_block_vm: expected non-zero exit while blocked"
  exit 1
fi

test_pass "vm_hako_caps_mapbox_has_block_vm: PASS (RVP-C20 blocked pin)"
