#!/bin/bash
# RVP-C19: blocked pin for MapBox.get(key) value semantics after MapBox.size port
#
# Contract:
# 1) MIR preflight must contain `boxcall(method=get,args=1)`.
# 2) vm-hako route currently completes with stale `0` output instead of `42`.
...
SMOKE_NAME="vm_hako_caps_mapbox_get_block_vm"
INPUT="${1:-$NYASH_ROOT/apps/tests/vm_hako_caps/mapbox_get_block_min.hako}"
...
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^0$'; then
  ...
  test_fail "vm_hako_caps_mapbox_get_block_vm: expected stale zero output"
  exit 1
fi
if [ "$VM_HAKO_CAPS_EXIT_CODE" -ne 0 ]; then
  echo "$OUTPUT_CLEAN" | tail -n 120 || true
  test_fail "vm_hako_caps_mapbox_get_block_vm: expected RC=0 (got rc=$VM_HAKO_CAPS_EXIT_CODE)"
  exit 1
fi

test_pass "vm_hako_caps_mapbox_get_block_vm: PASS (RVP-C19 blocked pin)"
