...
#
# Contract pin:
# - MIR emits typed_object_plans for init-only user fields after observing
#   FieldSet storage.
# - i64 and handle fields lower through the runtime slot-object ABI.
...
if ! grep -Fq '"box_name": "Holder"' "$MIR_OUT"; then
  cat "$MIR_OUT" >&2
  test_fail "$SMOKE_NAME: Holder typed object plan missing"
  exit 1
fi

if ! grep -Fq '"storage": "i64"' "$MIR_OUT"; then
  cat "$MIR_OUT" >&2
  test_fail "$SMOKE_NAME: inferred i64 storage missing"
  exit 1
fi

if ! grep -Fq '"storage": "handle"' "$MIR_OUT"; then
  cat "$MIR_OUT" >&2
  test_fail "$SMOKE_NAME: inferred handle storage missing"
  exit 1
fi
