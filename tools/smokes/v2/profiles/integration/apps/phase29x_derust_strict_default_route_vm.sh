...
# 3) explicit compat fallback in strict/dev still emits `source=hako-skeleton` with `lane=compat-fallback`.
...
if ! echo "$OUT_STRICT_DEFAULT" | rg -q "^\[derust-route/select\] backend=vm lane=vm-hako source=hako-skeleton reason=strict-dev-prefer$"; then
    echo "[INFO] strict default output:"
    echo "$OUT_STRICT_DEFAULT" | head -n 120 || true
...
if ! echo "$OUT_STRICT_RUST_THIN" | rg -q "^\[derust-route/select\] backend=vm lane=vm source=rust-thin-explicit reason=default$"; then
    echo "[INFO] strict explicit rust-thin output:"
    echo "$OUT_STRICT_RUST_THIN" | head -n 120 || true
...
if ! echo "$OUT_STRICT_FALLBACK" | rg -q "^\[derust-route/select\] backend=vm lane=compat-fallback source=hako-skeleton reason=env:NYASH_VM_USE_FALLBACK=1$"; then
    echo "[INFO] strict fallback output:"
    echo "$OUT_STRICT_FALLBACK" | head -n 120 || true
