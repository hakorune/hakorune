#!/bin/bash
# Phase 291x: vm-hako StringBox.startsWith(prefix) acceptance smoke.
set -euo pipefail
...
SMOKE_NAME="phase291x_stringbox_startswith_vm"
INPUT="${1:-$ROOT/apps/tests/phase291x_stringbox_startswith_min.hako}"
...
ACTUAL=$(printf '%s\n' "$OUTPUT_CLEAN" | rg '^(true|false)$' || true)
EXPECTED=$(cat <<'EXPECT'
true
false
true
