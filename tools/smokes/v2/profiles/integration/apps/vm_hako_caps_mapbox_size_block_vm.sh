#!/bin/bash
# RVP-C18: blocked pin for MapBox.size() after MapBox.set route port
#
# Contract:
# 1) MIR preflight must contain `boxcall(method=size,args=0)`.
# 2) vm-hako route currently fails at runtime with `op=boxcall0 method=size`.
...
SMOKE_NAME="vm_hako_caps_mapbox_size_block_vm"
INPUT="${1:-$NYASH_ROOT/apps/tests/vm_hako_caps/mapbox_size_block_min.hako}"
...
if ! printf '%s\n' "$OUTPUT_CLEAN" | rg -q '^\[vm-hako/unimplemented op=boxcall0 method=size\]'; then
  ...
  test_fail "vm_hako_caps_mapbox_size_block_vm: expected runtime blocker op=boxcall0 method=size"
  exit 1
fi

test_pass "vm_hako_caps_mapbox_size_block_vm: PASS (RVP-C18 blocked pin)"
