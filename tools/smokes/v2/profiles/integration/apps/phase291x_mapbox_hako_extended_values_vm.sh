#!/bin/bash
# Phase 291x: vm-hako MapBox.values() source-route smoke.
set -euo pipefail
...
SMOKE_NAME="phase291x_mapbox_hako_extended_values_vm"
INPUT="${1:-$ROOT/apps/tests/phase291x_mapbox_hako_values_min.hako}"
RUN_TIMEOUT_SECS="${RUN_TIMEOUT_SECS:-30}"
TMP_MIR="$(mktemp /tmp/phase291x_mapbox_values.XXXXXX.json)"
cleanup() {
  rm -f "$TMP_MIR"
}
trap cleanup EXIT
...
test_pass "$SMOKE_NAME: PASS (MapBox.values vm-hako shape pinned)"
