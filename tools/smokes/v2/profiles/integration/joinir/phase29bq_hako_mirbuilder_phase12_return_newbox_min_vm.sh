#!/bin/bash
# phase29bq_hako_mirbuilder_phase12_return_newbox_min_vm.sh
# Contract pin: accept Return(NewBox) minimal shape
...
FIXTURE="$NYASH_ROOT/apps/tests/phase29bq_hako_mirbuilder_phase12_return_newbox_min.hako"
...
RUN_ID="phase29bq_hako_mirbuilder_phase12_return_newbox_min_${$}"
...
if ! rg -q '"op":"newbox"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase12_return_newbox pin: MIR missing newbox op" >&2
...
if ! rg -q '"type":"ArrayBox"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase12_return_newbox pin: MIR missing ArrayBox type" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
if [ "$EXEC_RC" -ne 0 ]; then
  echo "[FAIL] hako_mirbuilder phase12_return_newbox pin: --mir-json-file failed (rc=$EXEC_RC)" >&2
...
if [ -n "$OUT" ]; then
  echo "[FAIL] hako_mirbuilder phase12_return_newbox pin: unexpected stdout (expected='' got='$OUT')" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
echo "[PASS] hako_mirbuilder phase12_return_newbox pin: PASS"
