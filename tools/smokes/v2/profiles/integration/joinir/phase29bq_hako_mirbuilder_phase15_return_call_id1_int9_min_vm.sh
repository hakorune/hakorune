...
# Contract pin: reject the retired Stage1 Return(Call id(9)) writer before
...
if [ "$EXEC_RC" -eq 0 ]; then
  echo "[FAIL] hako_mirbuilder phase15_return_call_id1_int9 pin: retired Return(Call) unexpectedly succeeded" >&2
...
if ! rg -q '\[freeze:contract\]\[stage1/return-call-legacy-stopped\]' "$ROUTE_LOG"; then
  echo "[FAIL] hako_mirbuilder phase15_return_call_id1_int9 pin: missing typed Return(Call) stop" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] route_log=$ROUTE_LOG" >&2
...
echo "[PASS] hako_mirbuilder phase15_return_call_id1_int9 pin: typed Stop PASS"
