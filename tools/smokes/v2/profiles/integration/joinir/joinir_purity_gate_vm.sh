#!/bin/bash
# joinir_purity_gate_vm.sh - JoinIR purity gate (strict/dev fallback visibility)
...
run_if_phi_join() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "joinir_purity_gate_vm: if_phi_join timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 0 ]; then
        echo "[FAIL] if_phi_join exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "joinir_purity_gate_vm: if_phi_join failed"
        exit 1
    fi
    assert_flowbox_adopt "if_phi_join" "$output" ""
    if ! validate_numeric_output 1 "$expected" "$output"; then
        echo "$output" | tail -n 80 || true
        test_fail "joinir_purity_gate_vm: if_phi_join output mismatch"
        exit 1
    fi
...
run_scan_with_init() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "joinir_purity_gate_vm: scan_with_init timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 1 ]; then
        echo "[FAIL] scan_with_init exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "joinir_purity_gate_vm: scan_with_init failed"
        exit 1
    fi
...
run_is_integer_strict_reject() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "joinir_purity_gate_vm: is_integer strict reject timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 1 ]; then
        echo "[FAIL] is_integer strict reject exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "joinir_purity_gate_vm: is_integer strict reject failed"
        exit 1
    fi
...
log_success "joinir_purity_gate_vm: PASS"
exit 0
