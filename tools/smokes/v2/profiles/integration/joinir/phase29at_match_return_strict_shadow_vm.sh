#!/bin/bash
# phase29at_match_return_strict_shadow_vm.sh - match return-only strict/dev shadow adopt
...
if [ "$EXIT_CODE" -eq 124 ]; then
    log_error "phase29at_match_return_strict_shadow_vm: hakorune timed out (>${RUN_TIMEOUT_SECS}s)"
    exit 1
fi

if [ "$EXIT_CODE" -ne 20 ]; then
    log_error "phase29at_match_return_strict_shadow_vm: expected exit code 20, got $EXIT_CODE"
    echo "$OUTPUT"
    exit 1
fi
...
if ! grep -qF "[flowbox/adopt box_kind=Seq" <<<"$OUTPUT" \
    ...
    test_fail "phase29at_match_return_strict_shadow_vm: Missing FlowBox tag"
    exit 1
fi
...
if grep -qF "[plan/fallback:" <<<"$OUTPUT"; then
    ...
    echo "$OUTPUT" | tail -n 40 || true
    test_fail "phase29at_match_return_strict_shadow_vm: Fallback tag present"
    exit 1
fi

log_success "phase29at_match_return_strict_shadow_vm: PASS (exit=20)"
exit 0
