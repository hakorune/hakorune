#!/bin/bash
# phase29bq_joinir_port07_expr_parity_seed_vm.sh
# JIR-PORT-07 contract:
# - expression seed (unary + compare + logical) is accepted in .hako selfhost route.
# - hako route must run via selfhost-first (no direct-emit/delegate fallback).
# - emitted main must keep expression control shape (branch + ret, >=3 blocks),
#   i.e. not collapse to single-block const+ret fallback.
# - executing emitted MIR via --mir-json-file returns EXPECTED_RC.
...
TMP_MIR="$(mktemp /tmp/phase29bq_port07_hako_mir.XXXXXX.json)"
HAKO_LOG="$(mktemp /tmp/phase29bq_port07_hako_log.XXXXXX.log)"
...
if ! joinir_port_require_main_ops "$SMOKE_NAME" "$TMP_MIR" branch ret; then
  retain_debug_artifacts
  test_fail "$SMOKE_NAME: expression seed skeleton (branch+ret) missing"
  exit 1
fi
...
' "$TMP_MIR" >/dev/null; then
  retain_debug_artifacts
  test_fail "$SMOKE_NAME: main blocks collapsed (<3), fallback regression suspected"
  exit 1
fi
...
test_pass "$SMOKE_NAME: PASS (selfhost expression seed locked)"
