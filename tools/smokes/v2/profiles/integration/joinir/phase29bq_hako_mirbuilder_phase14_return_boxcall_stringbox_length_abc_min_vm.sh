#!/bin/bash
# phase29bq_hako_mirbuilder_phase14_return_boxcall_stringbox_length_abc_min_vm.sh
# Contract pin: accept Return(Method) as BoxCall minimal shape
#   return new StringBox("abc").length()
...
if ! rg -q '"op":"boxcall"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase14_return_boxcall pin: MIR missing boxcall op" >&2
...
if ! rg -q '"method":"length"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase14_return_boxcall pin: MIR missing method=length" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
if ! rg -q '"type":"StringBox"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase14_return_boxcall pin: MIR missing StringBox constructor" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
if rg -q '"op":"mir_call"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase14_return_boxcall pin: MIR contains mir_call (v0 lane)" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
if [ "$EXEC_RC" -ne 3 ]; then
  echo "[FAIL] hako_mirbuilder phase14_return_boxcall pin: --mir-json-file failed (rc=$EXEC_RC)" >&2
  echo "[FAIL] expected rc=3" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
if [ -n "$OUT" ]; then
  echo "[FAIL] hako_mirbuilder phase14_return_boxcall pin: unexpected stdout (expected='' got='$OUT')" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
echo "[PASS] hako_mirbuilder phase14_return_boxcall pin: PASS"
