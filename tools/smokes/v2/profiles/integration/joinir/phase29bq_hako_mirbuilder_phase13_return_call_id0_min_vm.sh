#!/bin/bash
# phase29bq_hako_mirbuilder_phase13_return_call_id0_min_vm.sh
# Contract pin: accept Return(Call id()) minimal shape
...
FIXTURE="$NYASH_ROOT/apps/tests/phase29bq_hako_mirbuilder_phase13_return_call_id0_min.hako"
...
RUN_ID="phase29bq_hako_mirbuilder_phase13_return_call_id0_min_${$}"
...
if ! rg -q '"op":"call"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase13_return_call_id0 pin: MIR missing call op" >&2
...
if ! rg -q '"name":"id"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase13_return_call_id0 pin: MIR missing helper function name=id" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
if rg -q '"op":"mir_call"' "$MJSON"; then
  echo "[FAIL] hako_mirbuilder phase13_return_call_id0 pin: MIR contains mir_call (v0 lane)" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
if [ "$EXEC_RC" -ne 7 ]; then
  echo "[FAIL] hako_mirbuilder phase13_return_call_id0 pin: --mir-json-file failed (rc=$EXEC_RC)" >&2
...
if [ -n "$OUT" ]; then
  echo "[FAIL] hako_mirbuilder phase13_return_call_id0 pin: unexpected stdout (expected='' got='$OUT')" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
echo "[PASS] hako_mirbuilder phase13_return_call_id0 pin: PASS"
