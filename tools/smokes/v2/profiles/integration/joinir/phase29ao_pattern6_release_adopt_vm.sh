...
if [ "$EXIT_CODE" -eq 124 ]; then
    test_fail "phase29ao_pattern6_release_adopt_vm: hakorune timed out (>${RUN_TIMEOUT_SECS}s)"
    exit 1
fi
...
if grep -qF "[flowbox/" <<<"$OUTPUT"; then
    ...
    test_fail "phase29ao_pattern6_release_adopt_vm: Unexpected tag"
    exit 1
fi
...
if [ "$EXIT_CODE" -ne 1 ]; then
    ...
    test_fail "phase29ao_pattern6_release_adopt_vm: Unexpected RC"
    exit 1
fi

test_pass "phase29ao_pattern6_release_adopt_vm: PASS (exit=1)"
exit 0
