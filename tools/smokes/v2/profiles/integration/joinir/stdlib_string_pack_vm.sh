#!/bin/bash
# stdlib_string_pack_vm.sh - Phase 29aq stdlib subset pack (VM)
# Contract:
...
if ! command -v timeout >/dev/null 2>&1; then
    log_error "stdlib_string_pack_vm: 'timeout' command not found"
    exit 2
fi
...
run_case_script() {
    ...
    if [ ! -f "$script_path" ]; then
        log_error "stdlib_string_pack_vm: missing script: $script_path"
        return 1
    fi

    log_info "stdlib_string_pack_vm: ${label} (${script_basename})"
    ...
    if ! timeout "${SMOKES_DEFAULT_TIMEOUT:-120}" env \
        ...
        log_error "stdlib_string_pack_vm: ${label} failed"
        return 1
    fi
...
log_success "stdlib_string_pack_vm: all stdlib subset filters passed"
exit 0
