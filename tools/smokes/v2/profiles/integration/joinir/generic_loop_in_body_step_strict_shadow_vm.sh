#!/bin/bash
# generic_loop_in_body_step_strict_shadow_vm.sh - generic loop v0.2 in-body step (strict/dev)
# Expected:
# - Exit code 3
...
if [ "$EXIT_CODE" -eq 124 ]; then
    test_fail "generic_loop_in_body_step_strict_shadow_vm: hakorune timed out (>${RUN_TIMEOUT_SECS}s)"
    exit 1
fi
...
if [ "$EXIT_CODE" -ne 3 ]; then
    ...
    test_fail "generic_loop_in_body_step_strict_shadow_vm: Unexpected RC"
    exit 1
fi

if grep -qF "[flowbox/adopt box_kind=Loop" <<<"$OUTPUT" \
    && grep -qF "via=shadow" <<<"$OUTPUT"; then
    test_pass "generic_loop_in_body_step_strict_shadow_vm: PASS (exit=3, flowbox tag)"
    exit 0
fi

if planner_first_tag_matches "$OUTPUT" "[joinir/planner_first rule=LoopSimpleWhile]"; then
    test_pass "generic_loop_in_body_step_strict_shadow_vm: PASS (exit=3, planner-first tag)"
    exit 0
fi

echo "[FAIL] Missing tag (flowbox shadow adopt OR planner-first)"
echo "$OUTPUT" | tail -n 40 || true
test_fail "generic_loop_in_body_step_strict_shadow_vm: Missing tag"
exit 1
