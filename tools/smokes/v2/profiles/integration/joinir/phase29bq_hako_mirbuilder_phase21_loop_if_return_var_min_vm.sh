...
FIXTURE="$NYASH_ROOT/apps/tests/phase29bq_hako_mirbuilder_phase21_loop_if_return_var_min.hako"
...
RUN_ID="phase29bq_hako_mirbuilder_phase21_min_${$}"
...
if [ "$EXEC_RC" -ne 3 ]; then
  echo "[FAIL] hako_mirbuilder phase21 pin: unexpected rc (expected=3 got=$EXEC_RC)" >&2
...
if [ -n "$OUT" ]; then
  echo "[FAIL] hako_mirbuilder phase21 pin: unexpected stdout (expected='' got='$OUT')" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
echo "[PASS] hako_mirbuilder phase21 pin: PASS"
