#!/bin/bash
# current semantic wrapper; canonical entry for loop_break body-local smoke

source "$(dirname "$0")/../../../lib/test_runner.sh"
export SMOKES_USE_PYVM=0
require_env || exit 2

SEMANTIC_STEM="loop_break_body_local_vm"
...
INPUT="$NYASH_ROOT/apps/tests/loop_break_body_local_min.hako"
...
EXPECTED_TAG_OLD="[plan/pattern2/promotion_hint:DigitPos]"
EXPECTED_TAG_NEW="[plan/loop_break/promotion_hint:DigitPos]"
...
if echo "$OUTPUT_CLEAN" | grep -q "^2$" || echo "$OUTPUT" | grep -q "^RC: 2$"; then
    if echo "$OUTPUT_CLEAN" | grep -qF "$EXPECTED_TAG_NEW" || echo "$OUTPUT_CLEAN" | grep -qF "$EXPECTED_TAG_OLD"; then
        test_pass "${LABEL_PREFIX}: body-local promotion succeeded (output: 2)"
        exit 0
    fi
