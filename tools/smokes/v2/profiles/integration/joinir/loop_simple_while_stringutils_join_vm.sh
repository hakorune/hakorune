#!/bin/bash
# loop_simple_while_stringutils_join_vm.sh - StringUtils.join via plan/composer (VM)
...
compare_outputs "$expected" "$output" "loop_simple_while_stringutils_join_vm" || exit 1
