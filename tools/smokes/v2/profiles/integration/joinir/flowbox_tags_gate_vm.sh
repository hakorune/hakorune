#!/bin/bash
# flowbox_tags_gate_vm.sh - FlowBox schema tags gate (strict/non-strict)
...
run_if_phi_join_strict() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tags_gate_vm: if_phi_join timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 0 ]; then
        echo "[FAIL] if_phi_join exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tags_gate_vm: if_phi_join failed"
        exit 1
    fi
    assert_has_adopt_tag "if_phi_join" "$output" "Loop"
    if ! validate_numeric_output 1 "$expected" "$output"; then
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tags_gate_vm: if_phi_join output mismatch"
        exit 1
    fi
}
...
run_match_return_strict() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tags_gate_vm: match_return strict timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 20 ]; then
        echo "[FAIL] match_return strict exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tags_gate_vm: match_return strict failed"
        exit 1
    fi
...
run_match_return_release() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tags_gate_vm: match_return release timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 20 ]; then
        echo "[FAIL] match_return release exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tags_gate_vm: match_return release failed"
        exit 1
    fi
...
log_success "flowbox_tags_gate_vm: PASS"
exit 0
