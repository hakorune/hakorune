#!/bin/bash
# flowbox_tag_coverage_gate_vm.sh - FlowBox tag coverage gate (strict/non-strict)
...
run_scan_with_init_strict() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tag_coverage_gate_vm: scan_with_init strict timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 1 ]; then
        echo "[FAIL] scan_with_init strict exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tag_coverage_gate_vm: scan_with_init strict failed"
        exit 1
    fi
...
run_scan_with_init_release() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tag_coverage_gate_vm: scan_with_init release timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 1 ]; then
        echo "[FAIL] scan_with_init release exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tag_coverage_gate_vm: scan_with_init release failed"
        exit 1
    fi
...
run_split_scan_strict() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tag_coverage_gate_vm: split_scan strict timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 1 ]; then
...
run_split_scan_release() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tag_coverage_gate_vm: split_scan release timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 3 ]; then
        echo "[FAIL] split_scan release exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tag_coverage_gate_vm: split_scan release failed"
        exit 1
    fi
...
run_is_integer_strict() {
    local input="$NYASH_ROOT/apps/tests/string_is_integer_min.hako"
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tag_coverage_gate_vm: is_integer strict timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 1 ]; then
        echo "[FAIL] is_integer strict exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tag_coverage_gate_vm: is_integer strict failed"
        exit 1
    fi
...
run_is_integer_release() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tag_coverage_gate_vm: is_integer release timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 0 ]; then
        echo "[FAIL] is_integer release exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tag_coverage_gate_vm: is_integer release failed"
        exit 1
    fi
...
TXT
    ...
    compare_outputs "$expected" "$output_clean" "flowbox_tag_coverage_gate_vm" || exit 1
}
...
run_match_return_strict() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tag_coverage_gate_vm: match_return strict timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 20 ]; then
        echo "[FAIL] match_return strict exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tag_coverage_gate_vm: match_return strict failed"
        exit 1
    fi
...
run_match_return_release() {
    ...
    if [ "$exit_code" -eq 124 ]; then
        test_fail "flowbox_tag_coverage_gate_vm: match_return release timed out"
        exit 1
    fi
    if [ "$exit_code" -ne 20 ]; then
        echo "[FAIL] match_return release exit code $exit_code"
        echo "$output" | tail -n 80 || true
        test_fail "flowbox_tag_coverage_gate_vm: match_return release failed"
        exit 1
    fi
...
log_success "flowbox_tag_coverage_gate_vm: PASS"
exit 0
