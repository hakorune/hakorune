...
grep -q '^pipeline_plan_v0=1$' "$REPORT"
grep -q '^compile_options_v0=1$' "$REPORT"
grep -q '^mir_future_rewrite_option=env_future_externs$' "$REPORT"
grep -q '^mir_future_rewrite_route=env_forced_llvm_future_externs$' "$REPORT"
grep -q '^method_id_injector_plan_enabled=1$' "$REPORT"
grep -q '^pipeline_joinir_experiment_enabled=0$' "$REPORT"
grep -q '^joinir_experiment_hook_plan_enabled=1$' "$REPORT"
