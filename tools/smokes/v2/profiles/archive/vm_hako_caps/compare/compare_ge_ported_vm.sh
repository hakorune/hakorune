#!/bin/bash
# RVP-C10: retired vm-hako compare(>=) evidence after phase-96x demotion
#
# Contract:
...
SMOKE_NAME="vm_hako_caps_compare_ge_ported_archive_vm"
...
TMP_MIR="$(mktemp /tmp/vm_hako_caps_c10_archive.XXXXXX.json)"
TMP_DRIVER="$(mktemp /tmp/vm_hako_caps_c10_archive_driver.XXXXXX.hako)"
cleanup() {
...
if [ "$VM_HAKO_CAPS_EXIT_CODE" -ne 0 ]; then
  ...
  test_fail "$SMOKE_NAME: expected success exit (rc=0), got rc=$VM_HAKO_CAPS_EXIT_CODE"
  exit 1
fi
...
if echo "$VM_HAKO_CAPS_OUTPUT" | rg -q '^\[vm-hako/emit-error\]'; then
  echo "$VM_HAKO_CAPS_OUTPUT" | tail -n 80 || true
  test_fail "$SMOKE_NAME: unexpected emit-error tag"
  exit 1
fi
...
if [ "$GE_ALIAS_RC" -eq 124 ]; then
  test_fail "$SMOKE_NAME: op_kind=Ge runtime alias probe timed out"
  exit 1
fi
if [ "$GE_ALIAS_RC" -ne 0 ]; then
  echo "$GE_ALIAS_OUTPUT" | tail -n 100 || true
  test_fail "$SMOKE_NAME: op_kind=Ge runtime alias probe failed (rc=$GE_ALIAS_RC)"
  exit 1
fi
if echo "$GE_ALIAS_OUTPUT" | rg -q '^\[vm-hako/unimplemented\]'; then
  echo "$GE_ALIAS_OUTPUT" | tail -n 100 || true
  test_fail "$SMOKE_NAME: op_kind=Ge emitted vm-hako unimplemented tag"
  exit 1
fi
if echo "$GE_ALIAS_OUTPUT" | rg -q '^\[vm-hako/contract'; then
  echo "$GE_ALIAS_OUTPUT" | tail -n 100 || true
  test_fail "$SMOKE_NAME: op_kind=Ge emitted vm-hako contract tag"
  exit 1
fi

test_pass "$SMOKE_NAME: PASS (retired vm-hako compare>= evidence pinned)"
