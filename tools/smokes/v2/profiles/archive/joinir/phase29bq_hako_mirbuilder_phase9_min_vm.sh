...
# Phase-9 pin: Stage-0 Program(JSON v0) fixture -> (.hako mirbuilder) MIR JSON v0 -> --mir-json-file execution
#
...
FIXTURE="$NYASH_ROOT/apps/tests/phase29bq_hako_mirbuilder_phase9_print_binop_min.hako"
...
RUN_ID="phase29bq_hako_mirbuilder_phase9_min_${$}"
...
# 2) .hako mirbuilder (Phase-9): Program(JSON v0) → MIR JSON v0
...
if [ "$EXEC_RC" -ne 0 ]; then
  echo "[FAIL] hako_mirbuilder phase9 pin: unexpected rc (expected=0 got=$EXEC_RC)" >&2
...
if [ "$OUT" != "1" ]; then
  echo "[FAIL] hako_mirbuilder phase9 pin: unexpected stdout (expected='1' got='$OUT')" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
