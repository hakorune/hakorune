...
# Phase-5 pin: Stage-0 Program(JSON v0) fixture -> (.hako mirbuilder) MIR JSON v0 -> --mir-json-file execution
#
...
source "$ROOT_DIR/lib/program_json_v0_compat.sh"
require_env || exit 2
...
FIXTURE="$NYASH_ROOT/apps/tests/phase29bq_hako_mirbuilder_phase5_local_print_var_return_var_min.hako"
...
RUN_ID="phase29bq_hako_mirbuilder_phase5_min_${$}"
...
program_json_v0_compat_emit_to_file "$NYASH_BIN" "$PJSON" "$FIXTURE" >/dev/null

# 2) .hako mirbuilder (Phase-5): Program(JSON v0) → MIR JSON v0
...
if [ "$EXEC_RC" -ne 7 ]; then
  echo "[FAIL] hako_mirbuilder phase5 pin: --mir-json-file failed (rc=$EXEC_RC)" >&2
...
if [ "$OUT" != "7" ]; then
  echo "[FAIL] hako_mirbuilder phase5 pin: unexpected stdout (expected='7' got='$OUT')" >&2
  echo "[FAIL] fixture=$FIXTURE" >&2
  echo "[FAIL] entry=$ENTRY" >&2
  echo "[FAIL] program_json=$PJSON" >&2
  echo "[FAIL] mir_json=$MJSON" >&2
...
echo "[PASS] hako_mirbuilder phase5 pin: PASS"
