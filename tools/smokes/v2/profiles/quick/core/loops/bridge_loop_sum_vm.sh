#!/bin/bash
# Bridge(JSON v0) → MIR Interpreter での簡単な loop 検証
...
if [ "${SMOKES_ENABLE_LOOP_BRIDGE:-0}" != "1" ]; then
  test_skip "bridge_loop_sum_vm" "opt-in (set SMOKES_ENABLE_LOOP_BRIDGE=1)" && exit 0
fi

test_bridge_loop_sum() {
  local tmp_json="/tmp/nyash_bridge_loop_sum_$$.json"
  cat > "$tmp_json" <<'JSON'
