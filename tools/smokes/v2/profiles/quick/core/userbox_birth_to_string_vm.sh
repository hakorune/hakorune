...
output=$(run_quick_vm_release driver.hako --dev)
if pending_reason=$(quick_userbox_pending_reason "$output"); then
  cd /
  rm -rf "$TEST_DIR"
  test_skip "userbox_birth_to_string_vm" "$pending_reason"
  exit 0
fi
