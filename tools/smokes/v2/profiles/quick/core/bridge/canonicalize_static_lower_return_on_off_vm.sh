#!/bin/bash
# canonicalize_static_lower_return_on_off_vm.sh — Verify LLVMReturnInstructionBox.lower_return rewrite
...
json_path="/tmp/ny_v1_lower_return_$$.json"
mut_on="/tmp/ny_v1_lower_return_on_$$.json"
mut_off="/tmp/ny_v1_lower_return_off_$$.json"

cat >"$json_path" <<'JSON'
{"schema_version":"1.0","functions":[{"name":"main","blocks":[{"id":0,"instructions":[{"op":"mir_call","mir_call":{"callee":{"type":"ModuleFunction","name":"LLVMReturnInstructionBox.lower_return"},"args":[1]}},{"op":"ret"}]}]}]}
JSON
