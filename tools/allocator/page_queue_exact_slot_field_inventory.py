#!/usr/bin/env python3
"""Inventory page-queue exact-slot field traffic after residual facade selection."""
...
def field_family(receiver_type: str) -> str:
    if receiver_type == "HakoAllocObjectLifecyclePageQueue":
        return "page_queue_receiver_state"
    ...
    if receiver_type == "HakoAllocObjectLifecycleFacade":
        return "facade_bridge"
    return "unknown"


def parse_page_queue_callers(perf_report: Path) -> dict[str, float]:
...
def main() -> int:
    ...
    require(owner, "output_contract", "object-lifecycle-facade-residual-field-owner-selection-v0")
    require(owner, "selected_owner", "page_queue_exact_slot_field_inventory")
    require(owner, "summary", "ok")

    page_queue_callers = parse_page_queue_callers(args.perf_report)
    if not page_queue_callers:
        raise SystemExit("no page queue exact-slot callsites found")
    ...
    for method in sorted(page_queue_callers):
    ...
    top_method, top_method_pct = max(page_queue_callers.items(), key=lambda item: item[1])
    dominant_family, dominant_family_count = family_counts.most_common(1)[0]

    if positive_net_cache_candidates > 0:
        selected_next = "page_queue_field_owner_selection"
    else:
        selected_next = "post_page_queue_inventory_owner_refresh"
    ...
    lines = [
        ...
        f"page_queue_method_count={len(page_queue_callers)}",
        f"page_queue_exact_slot_get_count={op_counts['field_get']}",
        f"page_queue_exact_slot_set_count={op_counts['field_set']}",
        f"page_queue_exact_slot_field_op_count={len(field_ops)}",
        f"top_page_queue_method={top_method}",
        f"top_page_queue_method_pct={top_method_pct:.2f}",
        f"dominant_field_family={dominant_family}",
        f"dominant_field_family_count={dominant_family_count}",
        f"field_family.page_queue_receiver_state_count={family_counts['page_queue_receiver_state']}",
        ...
        f"field_family.facade_bridge_count={family_counts['facade_bridge']}",
    ...
    for idx, (method, pct) in enumerate(sorted(page_queue_callers.items(), key=lambda item: item[1], reverse=True)):
        lines.append(f"page_queue_method_{idx}_symbol={method}")
        lines.append(f"page_queue_method_{idx}_pct={pct:.2f}")
        lines.append(f"page_queue_method_{idx}_field_op_count={method_counts[method]}")
