#!/usr/bin/env python3
"""Measure exact-EXE after the small-alloc .hako reason-bind keeper."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hakorune_post_hako_reason_bind.") as tmp:
        tmp_dir = Path(tmp)
    ...
    lines.extend(
        [
            f"after_hako_elapsed_median_ms={after_median}",
            ...
            f"keeper_effect={classify_effect(after_median)}",
            "winner_claim=0",
