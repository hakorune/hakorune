#!/usr/bin/env python3
"""Measure the selected page queue same-block get/set fusion keeper."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hakorune_page_queue_get_set_measure.") as tmp:
        base_report = Path(tmp) / "page-queue-get-set.out"
        subprocess.run(
            [
                "python3",
                str(BASE_TOOL),
    ...
    lines = [
        ...
        f"selected_page_queue_get_set_body_elapsed_ns={require_key(values, 'exact_slot_helper_body_elapsed_ns')}",
        f"body_elapsed_delta_ns={require_key(values, 'body_elapsed_delta_ns')}",
        f"selected_page_queue_get_set_body_ratio_pct={require_key(values, 'exact_slot_helper_body_ratio_pct')}",
        ...
        f"selected_page_queue_get_set_external_elapsed_ms={require_key(values, 'exact_slot_helper_external_elapsed_ms')}",
        f"keeper_effect={keeper_effect}",
        f"selected_page_queue_get_set_keeper={keeper}",
        "winner_claim=0",
