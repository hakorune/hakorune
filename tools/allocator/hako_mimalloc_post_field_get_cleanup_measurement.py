#!/usr/bin/env python3
"""Measure object-lifecycle exact-EXE after field_get result-chain cleanup."""
...
def run_sample(tmp_dir: Path, sample: int) -> dict[str, str]:
    ...
    require(values, "in_process_operation_repeat", "8192", label)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hakorune_post_field_get_cleanup.") as tmp:
        tmp_dir = Path(tmp)
    ...
    previous_checkpoint = 560

    if elapsed_median < previous_checkpoint:
        measurement_effect = "improved"
    elif elapsed_median > previous_checkpoint:
        measurement_effect = "regressed"
    else:
        measurement_effect = "neutral"
    ...
    lines.extend(
        [
            f"after_hako_elapsed_median_ms={elapsed_median}",
            ...
            f"previous_checkpoint_hako_elapsed_median_ms={previous_checkpoint}",
            f"delta_hako_elapsed_median_ms={elapsed_median - previous_checkpoint}",
            f"measurement_effect={measurement_effect}",
            "structural_keeper=1",
            "winner_claim=0",
