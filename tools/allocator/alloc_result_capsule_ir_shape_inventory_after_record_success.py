#!/usr/bin/env python3
"""Inventory alloc-result capsule IR shape after recordSuccess helper fusion."""
...
def main() -> int:
    ...
    require(owner, "output_contract", "post-release-result-capsule-owner-refresh-after-record-success-helper-fusion-v0")
    require(owner, "selected_owner", "alloc_result_capsule_ir_shape_inventory_after_record_success_helper_fusion")
    ...
    for fn in data.get("functions", []):
        ...
        if not name.startswith("HakoAllocObjectLifecycleAllocResult."):
            continue
        method_count += 1
    ...
    lines = [
        ...
        f"alloc_result_method_count={method_count}",
        f"alloc_result_field_get_count={field_get}",
        f"alloc_result_field_set_count={field_set}",
        f"alloc_result_field_op_count={field_get + field_set}",
        f"alloc_result_copy_count={totals['copy']}",
        f"alloc_result_call_count={call_count}",
        f"alloc_result_phi_count={totals['phi']}",
        f"alloc_result_branch_count={totals['branch']}",
        f"top_alloc_method={top[0]}",
        f"top_alloc_method_field_op_count={top[1]}",
        f"top_alloc_hot_method={top_hot[0]}",
        f"top_alloc_hot_method_field_op_count={top_hot[1]}",
