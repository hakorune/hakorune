#!/usr/bin/env python3
"""Measure the typed-object field RMW fusion keeper on the exact-EXE workload."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hakorune_rmw_fusion_measure.") as tmp:
        base_report = Path(tmp) / "exact-slot-plus-rmw.out"
        subprocess.run(
            [
                "python3",
                str(BASE_TOOL),
    ...
    rmw_keeper = "1" if keeper_effect == "accepted" else "0"
    ...
    lines = [
        ...
        f"rmw_fusion_body_elapsed_ns={require_key(values, 'exact_slot_helper_body_elapsed_ns')}",
        f"body_elapsed_delta_ns={require_key(values, 'body_elapsed_delta_ns')}",
        f"rmw_fusion_body_ratio_pct={require_key(values, 'exact_slot_helper_body_ratio_pct')}",
        ...
        f"rmw_fusion_external_elapsed_ms={require_key(values, 'exact_slot_helper_external_elapsed_ms')}",
        f"keeper_effect={keeper_effect}",
        f"rmw_fusion_keeper={rmw_keeper}",
        "winner_claim=0",
