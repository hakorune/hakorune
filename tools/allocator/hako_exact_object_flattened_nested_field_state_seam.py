#!/usr/bin/env python3
"""Report the passive state-sharing seam for flattened nested field plans."""
...
def main() -> int:
    ...
    plan = backend.build_passive_flattened_nested_state_plan(
    ...
    validation = backend.validate_flattened_nested_state_plan(plan)
    summary = "ok" if validation.get("state_sharing_seam_defined") else "blocked"
    ...
    lines = [
        ...
        "nested_object=HakoAllocObjectLifecycleAlignmentResult",
        f"state_sharing_seam_defined={validation.get('state_sharing_seam_defined', 0)}",
        "typed_newbox_preempts_local_aggregate=1",
        f"field_access_flattened_nested_route_enabled={validation.get('field_access_flattened_nested_route_enabled', 1)}",
        f"method_call_flattened_nested_route_enabled={validation.get('method_call_flattened_nested_route_enabled', 1)}",
