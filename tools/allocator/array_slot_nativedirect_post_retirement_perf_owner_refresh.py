#!/usr/bin/env python3
"""Refresh ArraySlot NativeDirect ownership after the retirement smoke."""
...
def classify(rows: list[tuple[str, float]]) -> dict[str, str]:
    ...
    for symbol, pct in rows:
        ...
        if ("nyash.array.slot_store_" in symbol or "nyash.array.slot_load_" in symbol) and "nyash.array.slot_load_store_i64_hihi" not in symbol:
            legacy_array += pct
        if "nyash.runtime_data." in symbol:
            legacy_public += pct
    ...
    # Keep the public ArrayBox helper surface separate from the legacy helper/cache
    # retirement total so the refresh row can decide whether to open ArrayRepr.
    ...
    if dominates:
        ...
        selected_boundary = "array_slot_nativedirect_post_retirement_perf_owner_refresh"
        next_diagnostic = "array_slot_nativedirect_post_retirement_perf_owner_refresh"
        selected_next = "array_slot_nativedirect_post_retirement_perf_owner_refresh"
        selected_reason = "direct_array_path_not_yet_dominant_enough_for_arrayrepr_design"
    ...
    return {
        ...
        "arraybox_public_helper_pct": fmt_pct(legacy_public),
