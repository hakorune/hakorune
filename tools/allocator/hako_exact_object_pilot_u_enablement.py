#!/usr/bin/env python3
"""Report the first guarded exact-object pilot enablement."""
...
def has_active_lowering_call(repo_root: Path, rel: str, call_name: str) -> bool:
    src = text(repo_root / rel)
    return call_name in src
...
def main() -> int:
    ...
    backend_lowering_enabled = int(
        getattr(backend, "FLATTENED_NESTED_FIELD_LOWERING_ENABLED", False)
    )
    ...
    route_wiring_ready = int(
        state_seam_defined and field_access_route_enabled and method_call_route_enabled
    )
    pilot_enabled = int(route_wiring_ready and backend_lowering_enabled)
