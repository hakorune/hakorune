#!/usr/bin/env python3
"""Refresh the collection-method direct-array lane perf owner after semantic smoke."""
...
def classify(rows: list[tuple[str, float]]) -> dict[str, str]:
    ...
    hako_method = 0.0
    arraybox_public = 0.0
    array_slot_backend_safe = 0.0
    array_handle_cache = 0.0

    for symbol, pct in rows:
        if "nyash_kernel::plugin::array_direct_i64_buffer::direct_array_i64_store_i64" in symbol:
            direct_store += pct
        if "nyash_kernel::plugin::array_direct_i64_buffer::direct_array_i64_load_i64" in symbol:
            direct_load += pct
        if "nyash.array.slot_load_store_i64_hihi" in symbol:
            direct_fused += pct
        if "nyash_kernel::plugin::array_slot_backend::safe_store_i64" in symbol:
            array_slot_backend_safe += pct
        if "nyash_kernel::plugin::array_slot_backend::safe_load_encoded_i64" in symbol:
            array_slot_backend_safe += pct
        if "nyash_kernel::plugin::array_handle_cache::" in symbol:
            array_handle_cache += pct
        ...
        if symbol.startswith("HakoAlloc"):
            hako_method += pct
        if "nyash.runtime_data.get_hh" in symbol or "nyash.runtime_data.set_hhh" in symbol:
            arraybox_public += pct

    direct_total = direct_store + direct_load + direct_fused
    legacy_total = legacy_field + legacy_array + legacy_hash
    arraybox_runtime_total = arraybox_public + array_slot_backend_safe + array_handle_cache
    dominates = direct_total >= legacy_total and direct_total >= hako_method

    if dominates:
        selected_boundary = "collection_method_call_direct_array_lane_legacy_helper_cache_retirement_selection"
        next_diagnostic = "collection_method_call_direct_array_lane_legacy_helper_cache_retirement_selection"
        selected_next = "collection_method_call_direct_array_lane_legacy_helper_cache_retirement_selection"
        selected_reason = "direct_array_path_dominates_legacy_helper_cache_after_semantic_smoke"
        retirement_open = "1"
    else:
        selected_boundary = "collection_method_call_direct_array_lane_post_semantic_perf_owner_refresh"
        next_diagnostic = "collection_method_call_direct_array_lane_post_semantic_perf_owner_refresh"
    ...
    return {
        ...
        "arraybox_public_helper_pct": fmt_pct(arraybox_public),
        "array_slot_backend_safe_pct": fmt_pct(array_slot_backend_safe),
        "array_handle_cache_pct": fmt_pct(array_handle_cache),
        "arraybox_runtime_total_pct": fmt_pct(arraybox_runtime_total),
