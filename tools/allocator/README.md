...
hakozuna_mixed_ws_report_replacement_front_subject_core.py:
  Hakozuna mixed-ws replacement-front static subject metadata assembly only;
  no benchmark execution logic
...
hakozuna_mixed_ws_gap_summary_sections.py:
  gap-ladder provider and replacement-front section rendering only; no CLI
  orchestration or file writing
...
hako_mimalloc_algorithm_coverage_context.py:
  coverage report context derivation only; no report I/O or CLI orchestration

hako_mimalloc_algorithm_coverage_model_fields.py:
...
hako_mimalloc_algorithm_coverage_perf_fields.py:
  coverage perf-attribution report field rendering only; no report I/O or CLI
  orchestration
