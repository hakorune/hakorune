#!/usr/bin/env python3
"""Refresh exact-slot owner after page-model hotpath is rejected as exercised."""
...
def main() -> int:
    ...
    parser.add_argument("--page-model-selection-report", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args()

    refresh = read_kv(args.owner_refresh_report)
    selection = read_kv(args.page_model_selection_report)
    ...
    require(selection, "selected_owner", "post_page_model_hotpath_owner_refresh_after_record_success_helper_fusion")
