#!/usr/bin/env python3
"""Measure object-lifecycle exact-EXE after LocalSSA same-block field_get reuse."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hakorune_post_local_ssa_reuse.") as tmp:
        tmp_dir = Path(tmp)
    ...
    if elapsed_median < PREVIOUS_CHECKPOINT_MS:
        measurement_effect = "improved"
    elif elapsed_median > PREVIOUS_CHECKPOINT_MS:
    ...
    lines.extend(
        [
            ...
            f"delta_hako_elapsed_median_ms={elapsed_median - PREVIOUS_CHECKPOINT_MS}",
