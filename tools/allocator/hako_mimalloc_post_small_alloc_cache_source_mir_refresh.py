#!/usr/bin/env python3
"""Refresh source/MIR keeper selection after the small-alloc cache keeper."""
...
def main() -> int:
    ...
    require(
        measurement,
        "output_contract",
        "hako-mimalloc-post-small-alloc-cache-keeper-measurement-v0",
        "measurement",
    )
    ...
    if (
        ...
        and release_report.get("confirmed_risk_kind") == "array_access"
    ):
        selected = release_report
        next_keeper = "release_known_page_object_cache"
        next_kind = "box_shape"
        inactive_risk = "select_page_loop_inactive_for_single_page_workload" if select_fallback == 0 else "none"
    else:
