#!/usr/bin/env python3
"""Measure the object-lifecycle EXE after the small-alloc cache keeper."""
...
def run_sample(tmp_dir: Path, sample: int) -> dict[str, str]:
    ...
    require(values, "select_page_single_fallback_count", "0", f"sample {sample}")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hakorune_post_small_alloc_cache_keeper.") as tmp:
        tmp_dir = Path(tmp)
    ...
    lines.extend(
        [
            ...
            "previous_keeper=select_page_single_page_fast_path",
            "winner_claim=0",
