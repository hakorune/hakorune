...
usage() {
  cat <<'USAGE'
Usage:
  tools/hako_check/state_explain.sh --mir-json app.mir.json [options]
  tools/hako_check/state_explain.sh --app app.hako [options]
  tools/hako_check/state_explain.sh app.hako [options]

Options:
  --mir-json PATH     Read an existing MIR JSON artifact.
  --app PATH          Emit MIR JSON for a .hako app, then explain it.
  --box NAME          Filter to an exact box name.
...
if [ -n "$APP_PATH" ]; then
  ...
  TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hako_check_state.XXXXXX")"
...
ARGS=(
  python3
  "$ROOT_DIR/tools/hako_check/state_explain.py"
  --mir-json "$MIR_JSON"
  --topn "$TOPN"
)
