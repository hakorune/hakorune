...
def _lowering_evidence_rows(
    *,
    current_owner_count: int,
    owner_eq_count: int,
    atomic_remote_head_push_lowerable: int,
    atomic_remote_head_drain_lowerable: int,
    atomic_remote_head_push_count: int,
    atomic_remote_head_drain_count: int,
    verified_table: list[dict[str, Any]],
    verified_field_load: list[dict[str, Any]],
    verified_field_store: list[dict[str, Any]],
    ...
    atomic_remote_head_plans: list[dict[str, Any]],
    drain_remote_list_to_local_plans: list[dict[str, Any]],
    state: dict[str, Any],
) -> list[tuple[str, str]]:
    return [
        ...
        (
            ...
            str(atomic_remote_head_push_lowerable if state.get("remote_free_open", False) else 0),
        ),
        ("memop_atomic_remote_head_push_count", str(atomic_remote_head_push_count)),
        ("memop_atomic_remote_head_drain_count", str(atomic_remote_head_drain_count)),
        ("memop_table_index_layout_ref_created_count", str(len(verified_table))),
        (
            "memop_field_load_layout_ref_consumed_count",
            str(len(verified_field_load)),
        ),
        (
            "memop_field_store_layout_ref_consumed_count",
            str(len(verified_field_store)),
        ),
        (
            "memop_local_free_push_layout_ref_consumed_count",
            str(len(verified_local_free_push)),
        ),
        (
            "memop_local_free_pop_layout_ref_consumed_count",
            str(len(verified_local_free_pop)),
        ),
        (
            "memop_free_head_push_layout_ref_consumed_count",
            str(len(verified_free_head_push)),
        ),
        (
            "memop_free_head_pop_layout_ref_consumed_count",
            str(len(verified_free_head_pop)),
        ),
        ...
        (
            ...
            str(len(state["verified_plans"]) - len(state["verified_plans"])),
        ),
        ...
        (
            ...
            str(state["local_free_incomplete"]),
        ),
        (
            "fastmem_free_head_access_plan_incomplete_count",
            str(state["free_head_incomplete"]),
        ),
        ...
        (
            ...
            str(int_flag(bool(state["verified_local_free_push"]))),
        ),
        (
            "fastmem_local_free_pop_lowering_uses_verified_plan",
            str(int_flag(bool(state["verified_local_free_pop"]))),
        ),
        (
        ...
        (
            ...
            str(int_flag(bool(state["verified_free_head_push"]))),
        ),
        (
            "fastmem_free_head_pop_lowering_uses_verified_plan",
            str(int_flag(bool(state["verified_free_head_pop"]))),
        ),
        (
...
def build_tail_rows(state: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    rows: list[tuple[str, str]] = [
        *_lowering_evidence_rows(
            current_owner_count=current_owner_count,
            owner_eq_count=owner_eq_count,
            atomic_remote_head_push_lowerable=atomic_remote_head_push_lowerable,
            atomic_remote_head_drain_lowerable=atomic_remote_head_drain_lowerable,
            atomic_remote_head_push_count=atomic_remote_head_push_count,
            atomic_remote_head_drain_count=atomic_remote_head_drain_count,
            verified_table=verified_table,
            verified_field_load=verified_field_load,
            verified_field_store=verified_field_store,
            ...
            atomic_remote_head_plans=atomic_remote_head_plans,
            drain_remote_list_to_local_plans=drain_remote_list_to_local_plans,
            state=state,
        ),
