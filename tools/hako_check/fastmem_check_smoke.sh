...
PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_page_local_alloc_route_cfg_producer.XXXXXX")"
BAD_PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_bad_page_local_alloc_route_cfg_producer.XXXXXX")"
...
if ! bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/page_local_alloc_route_cfg_producer_inventory.kv" \
  --format kv \
  >"$PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT"; then
  echo "[TEST/FAIL] fastmem-check rejected page-local alloc route CFG producer inventory" >&2
  cat "$PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT" >&2 || true
  exit 1
fi

grep -q '^failure_count=0$' "$PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT"
grep -q '^summary=ok$' "$PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT"

if bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/bad_page_local_alloc_route_cfg_producer_inventory.kv" \
  --format kv \
  >"$BAD_PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT"; then
  echo "[TEST/FAIL] fastmem-check accepted bad page-local alloc route CFG producer inventory" >&2
  exit 1
fi

grep -q '^failure_count=1$' "$BAD_PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT"
grep -q '^failure_0_reason=page_local_alloc_route_cfg_lowering_enabled$' \
  "$BAD_PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT"
grep -q '^summary=failed$' "$BAD_PAGE_LOCAL_ALLOC_ROUTE_CFG_PRODUCER_OUT"
...
ABANDONED_RECLAIM_PRODUCER_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_abandoned_reclaim_producer.XXXXXX")"
BAD_ABANDONED_RECLAIM_PRODUCER_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_bad_abandoned_reclaim_producer.XXXXXX")"

if ! bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/abandoned_reclaim_producer_inventory.kv" \
  --format kv \
  >"$ABANDONED_RECLAIM_PRODUCER_OUT"; then
  echo "[TEST/FAIL] fastmem-check rejected abandoned reclaim producer inventory" >&2
  cat "$ABANDONED_RECLAIM_PRODUCER_OUT" >&2 || true
  exit 1
fi

grep -q '^failure_count=0$' "$ABANDONED_RECLAIM_PRODUCER_OUT"
grep -q '^summary=ok$' "$ABANDONED_RECLAIM_PRODUCER_OUT"

if bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/bad_abandoned_reclaim_producer_inventory.kv" \
  --format kv \
  >"$BAD_ABANDONED_RECLAIM_PRODUCER_OUT"; then
  echo "[TEST/FAIL] fastmem-check accepted bad abandoned reclaim producer inventory" >&2
  exit 1
fi

grep -q '^failure_count=1$' "$BAD_ABANDONED_RECLAIM_PRODUCER_OUT"
grep -q '^failure_0_reason=product_activation$' \
  "$BAD_ABANDONED_RECLAIM_PRODUCER_OUT"
grep -q '^summary=failed$' "$BAD_ABANDONED_RECLAIM_PRODUCER_OUT"

PRODUCT_ACTIVATION_PREFLIGHT_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_product_activation_preflight.XXXXXX")"
BAD_PRODUCT_ACTIVATION_PREFLIGHT_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_bad_product_activation_preflight.XXXXXX")"

if ! bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/product_activation_preflight_inventory.kv" \
  --format kv \
  >"$PRODUCT_ACTIVATION_PREFLIGHT_OUT"; then
  echo "[TEST/FAIL] fastmem-check rejected product activation preflight inventory" >&2
  cat "$PRODUCT_ACTIVATION_PREFLIGHT_OUT" >&2 || true
  exit 1
fi

grep -q '^failure_count=0$' "$PRODUCT_ACTIVATION_PREFLIGHT_OUT"
grep -q '^summary=ok$' "$PRODUCT_ACTIVATION_PREFLIGHT_OUT"

if bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/bad_product_activation_preflight_inventory.kv" \
  --format kv \
  >"$BAD_PRODUCT_ACTIVATION_PREFLIGHT_OUT"; then
  echo "[TEST/FAIL] fastmem-check accepted bad product activation preflight inventory" >&2
  exit 1
fi

grep -q '^failure_count=1$' "$BAD_PRODUCT_ACTIVATION_PREFLIGHT_OUT"
grep -q '^failure_0_reason=hook_install$' \
...
HOOK_INSTALL_PREFLIGHT_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_hook_install_preflight.XXXXXX")"
BAD_HOOK_INSTALL_PREFLIGHT_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_bad_hook_install_preflight.XXXXXX")"

if ! bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/hook_install_preflight_inventory.kv" \
  --format kv \
  >"$HOOK_INSTALL_PREFLIGHT_OUT"; then
  echo "[TEST/FAIL] fastmem-check rejected hook install preflight inventory" >&2
  cat "$HOOK_INSTALL_PREFLIGHT_OUT" >&2 || true
  exit 1
fi

grep -q '^failure_count=0$' "$HOOK_INSTALL_PREFLIGHT_OUT"
grep -q '^summary=ok$' "$HOOK_INSTALL_PREFLIGHT_OUT"

if bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/bad_hook_install_preflight_inventory.kv" \
  --format kv \
  >"$BAD_HOOK_INSTALL_PREFLIGHT_OUT"; then
  echo "[TEST/FAIL] fastmem-check accepted bad hook install preflight inventory" >&2
  exit 1
fi

grep -q '^failure_count=1$' "$BAD_HOOK_INSTALL_PREFLIGHT_OUT"
...
GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_global_allocator_claim_preflight.XXXXXX")"
BAD_GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_bad_global_allocator_claim_preflight.XXXXXX")"

if ! bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/global_allocator_claim_preflight_inventory.kv" \
  --format kv \
  >"$GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_OUT"; then
  echo "[TEST/FAIL] fastmem-check rejected global allocator claim preflight inventory" >&2
  cat "$GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_OUT" >&2 || true
  exit 1
fi

grep -q '^failure_count=0$' "$GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_OUT"
grep -q '^summary=ok$' "$GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_OUT"

if bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/bad_global_allocator_claim_preflight_inventory.kv" \
  --format kv \
  >"$BAD_GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_OUT"; then
  echo "[TEST/FAIL] fastmem-check accepted bad global allocator claim preflight inventory" >&2
  exit 1
fi

grep -q '^failure_count=1$' "$BAD_GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_OUT"
grep -q '^failure_0_reason=winner_claim$' \
...
WINNER_CLAIM_PREFLIGHT_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_winner_claim_preflight.XXXXXX")"
BAD_WINNER_CLAIM_PREFLIGHT_OUT="$(mktemp "${TMPDIR:-/tmp}/hako_fastmem_check_bad_winner_claim_preflight.XXXXXX")"

if ! bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/winner_claim_preflight_inventory.kv" \
  --format kv \
  >"$WINNER_CLAIM_PREFLIGHT_OUT"; then
  echo "[TEST/FAIL] fastmem-check rejected winner claim preflight inventory" >&2
  cat "$WINNER_CLAIM_PREFLIGHT_OUT" >&2 || true
  exit 1
fi

grep -q '^failure_count=0$' "$WINNER_CLAIM_PREFLIGHT_OUT"
grep -q '^summary=ok$' "$WINNER_CLAIM_PREFLIGHT_OUT"

if bash "$ROOT/tools/hako_check.sh" fastmem-check \
  --inventory "$FIXTURE_DIR/bad_winner_claim_preflight_inventory.kv" \
  --format kv \
  >"$BAD_WINNER_CLAIM_PREFLIGHT_OUT"; then
  echo "[TEST/FAIL] fastmem-check accepted bad winner claim preflight inventory" >&2
  exit 1
fi

grep -q '^failure_count=1$' "$BAD_WINNER_CLAIM_PREFLIGHT_OUT"
grep -q '^failure_0_reason=winner_claim$' \
