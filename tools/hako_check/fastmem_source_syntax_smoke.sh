...
grep -q '^atomic_remote_head_remote_owner_missing_count=1$' "$ATOMIC_REMOTE_HEAD_PUSH_MIR_INV"
grep -q '^atomic_remote_head_block_next_required=1$' "$ATOMIC_REMOTE_HEAD_PUSH_MIR_INV"
grep -q '^atomic_remote_head_block_next_missing_count=1$' "$ATOMIC_REMOTE_HEAD_PUSH_MIR_INV"
...
if bash "$ROOT/tools/hako_check.sh" fastmem-mir-to-llvm-producer-report \
  ...
  cat "$ATOMIC_REMOTE_HEAD_PUSH_LLVM_REPORT" >&2 || true
  exit 1
fi
