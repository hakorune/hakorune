...
grep -q '^fastmem_numeric_verified_direct_count=2$' "$GOOD_INV"
grep -q '^fastmem_numeric_required_route_miss_count=0$' "$GOOD_INV"
if grep -q '^fastmem_dedicated_binary_op_lowering_count=' "$GOOD_INV"; then
  grep -q '^fastmem_dedicated_binary_op_lowering_count=0$' "$GOOD_INV"
fi
grep -q '^fastmem_memop_table_index_count=1$' "$GOOD_INV"
grep -q '^fastmem_index_access_site_count=1$' "$GOOD_INV"
grep -q '^index_access_required_verified_table_count=1$' "$GOOD_INV"
grep -q '^index_access_required_verified_table_miss_count=0$' "$GOOD_INV"
grep -q '^fastmem_dedicated_index_lowering_count=0$' "$GOOD_INV"
