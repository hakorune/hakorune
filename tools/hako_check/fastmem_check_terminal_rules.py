...
def check_terminal_rules(rows: dict[str, str]) -> list[str]:
    ...
    if tls_backing_transfer_preflight_refresh_profile(rows):
        ...
        for key in TLS_BACKING_TRANSFER_PREFLIGHT_REFRESH_EXPECTED_POSITIVE:
            ...
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if tls_backing_transfer_producer_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "tls_backing_transfer_producer_refresh"
        ...
        if "OwnerSlotReuse" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in TLS_BACKING_TRANSFER_PRODUCER_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
    ...
    if owner_slot_reuse_preflight_refresh_profile(rows):
        ...
        for key in OWNER_SLOT_REUSE_PREFLIGHT_REFRESH_EXPECTED_POSITIVE:
            ...
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if owner_slot_reuse_producer_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "owner_slot_reuse_producer_refresh"
        ...
        if "AbandonedReclaim" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in OWNER_SLOT_REUSE_PRODUCER_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in OWNER_SLOT_REUSE_PRODUCER_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if abandoned_reclaim_preflight_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "abandoned_reclaim_preflight_refresh"
        ...
        if "ProductActivation" not in deferred:
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in ABANDONED_RECLAIM_PREFLIGHT_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in ABANDONED_RECLAIM_PREFLIGHT_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if abandoned_reclaim_producer_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "abandoned_reclaim_producer_refresh"
        ...
        if "ProductActivation" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in ABANDONED_RECLAIM_PRODUCER_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in ABANDONED_RECLAIM_PRODUCER_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if product_activation_preflight_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "product_activation_preflight_refresh"
        ...
        if rows.get("fastmem_branch_cfg_source_guard") != "branch_cfg_open":
            reasons.append("fastmem_branch_cfg_source_guard")
        if "ProductActivationProducer" not in rows.get(
            "replacement_front_deferred_memop_kinds", ""
        ).split(","):
            reasons.append("replacement_front_deferred_memop_kinds")
        if "HookInstall" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in PRODUCT_ACTIVATION_PREFLIGHT_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in PRODUCT_ACTIVATION_PREFLIGHT_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if product_activation_producer_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "product_activation_producer_refresh"
        ...
        if "HookInstall" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in PRODUCT_ACTIVATION_PRODUCER_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in PRODUCT_ACTIVATION_PRODUCER_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if hook_install_preflight_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != "hook_install_preflight_refresh":
            reasons.append("replacement_front_selected_route")
        ...
        if rows.get("fastmem_branch_cfg_source_guard") != "branch_cfg_open":
            reasons.append("fastmem_branch_cfg_source_guard")
        if "HookInstallProducer" not in rows.get(
            "replacement_front_deferred_memop_kinds", ""
        ).split(","):
            reasons.append("replacement_front_deferred_memop_kinds")
        if "GlobalAllocatorClaim" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in HOOK_INSTALL_PREFLIGHT_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in HOOK_INSTALL_PREFLIGHT_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if hook_install_producer_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "hook_install_producer_refresh"
        ...
        if "GlobalAllocatorClaim" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in HOOK_INSTALL_PRODUCER_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
    ...
    if global_allocator_claim_preflight_refresh_profile(rows):
        ...
        for key in GLOBAL_ALLOCATOR_CLAIM_PREFLIGHT_REFRESH_EXPECTED_POSITIVE:
            ...
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if global_allocator_claim_producer_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "global_allocator_claim_producer_refresh"
        ...
        if "WinnerClaim" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in GLOBAL_ALLOCATOR_CLAIM_PRODUCER_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in GLOBAL_ALLOCATOR_CLAIM_PRODUCER_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if winner_claim_preflight_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "winner_claim_preflight_refresh"
        ...
        if "WinnerClaimProducer" not in rows.get(
            ...
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in WINNER_CLAIM_PREFLIGHT_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in WINNER_CLAIM_PREFLIGHT_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
    if winner_claim_producer_refresh_profile(rows):
        if rows.get("replacement_front_producer") != "mir_to_llvm_lowering":
            reasons.append("replacement_front_producer")
        if rows.get("replacement_front_selected_route") != (
            "winner_claim_producer_refresh"
        ...
        if rows.get("replacement_front_deferred_memop_kinds") != "none":
            reasons.append("replacement_front_deferred_memop_kinds")
        for key in WINNER_CLAIM_PRODUCER_REFRESH_EXPECTED_ZERO:
            if int_count(rows, key) != 0:
                reasons.append(key)
        for key in WINNER_CLAIM_PRODUCER_REFRESH_EXPECTED_POSITIVE:
            if key.endswith("_lowered_count"):
                continue
            if int_count(rows, key) <= 0:
                reasons.append(key)
