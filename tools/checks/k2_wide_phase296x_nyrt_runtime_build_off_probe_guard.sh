...
TMP_DIR="$(mktemp -d /tmp/hakorune_nyrt_runtime_build_off_guard.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[nyrt-runtime-build-off] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_not_missing() {
  ...
  if grep -q "^${key}=missing$" "$file"; then
    echo "[nyrt-runtime-build-off] key unexpectedly missing in ${file#$ROOT_DIR/}: $key" >&2
    exit 1
  fi
}
