...
guard_require_files "$TAG" "$ENTRY" "$MODEL" "$TEST"

for file in "$ENTRY" "$MODEL"; do
...
if rg -n '^(import subprocess|import tempfile|EMIT_ROUTE|TRACE_BUNDLE|def emit_mir_json\(|def emit_llvm_asm_bundle\()' "$MODEL"; then
