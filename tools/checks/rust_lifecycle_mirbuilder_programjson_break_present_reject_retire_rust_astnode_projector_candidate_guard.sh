...
TAG="rust-lifecycle-mirbuilder-programjson-break-present-reject-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-break-present-reject-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonBreakPresentRejectRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
    == "MIRBUILDER-PROGRAMJSON-BREAK-PRESENT-REJECT-RETIRE-RUST-ASTNODE-PROJECTOR-CANDIDATE-001",
    "bad token",
)
...
need(scope.get("retire_candidate") == "LoopCondContinueBreakPresentRejectTokenSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "LoopCondContinueBreakPresentRejectV1", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonLoopCondContinueWithReturnSnapshotV1", "bad owner")
need(scope.get("parity_row_id") == "reject_break_present", "bad parity row")
...
row = rows.get("reject_break_present") or {}
summary = ((row.get("rust_astnode_token_oracle") or {}).get("expected_summary") or "")
need(summary == "accepted=0;reason=break_present", "break-present oracle row drift")
...
need(criteria.get("programjson_route_observes_break_node") == 1, "break-node observation proof missing")
...
need(
    row.get("selected_next_card") == "MIRBUILDER-PROGRAMJSON-NEXT-SHAPE-SELECTION-003",
    "bad next card",
)
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-break-present-reject-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-BREAK-PRESENT-REJECT-RETIRE-RUST-ASTNODE-PROJECTOR-CANDIDATE-001
