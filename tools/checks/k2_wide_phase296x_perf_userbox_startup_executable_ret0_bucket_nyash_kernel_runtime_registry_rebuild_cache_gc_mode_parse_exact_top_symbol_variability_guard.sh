...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_rebuild_cache_gc_mode_parse_exact_top_symbol_variability.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-gc-mode-parse-exact-top-symbol-variability] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-gc-mode-parse-exact-top-symbol-variability] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-gc-mode-parse-exact-top-symbol-variability] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
...
registry_exact_gc_mode_parse = "gc_mode_parse" if exact_family_counts["gc_mode_parse"] > 0 else "missing"
registry_exact_gc_mode_parse_count = exact_family_counts["gc_mode_parse"]
...
print(
    ...
    registry_exact_create_default_registry_mode_count,
    registry_exact_gc_mode_parse,
    registry_exact_gc_mode_parse_count,
...
cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-gc-mode-parse-exact-top-symbol-variability-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_rebuild_cache_gc_mode_parse_exact_top_symbol_variability
...
registry_exact_gc_mode_parse=$registry_exact_gc_mode_parse
registry_exact_gc_mode_parse_count=$registry_exact_gc_mode_parse_count
...
if [ "${registry_exact_gc_mode_parse_count:-0}" -le 0 ]; then
  echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-gc-mode-parse-exact-top-symbol-variability] expected aggregate registry_exact_gc_mode_parse_count > 0" >&2
  exit 1
fi
