...
guard_expect_fixed_in_file "$TAG" '### DA-FAM-001: Contract Lock' "$CARD_374" "mini task board must expose contract lock"
guard_expect_fixed_in_file "$TAG" '### DA-FAM-002: Scoped Helper Backend Retirement Guard' "$CARD_374" "mini task board must expose scoped helper backend guard"
guard_expect_fixed_in_file "$TAG" '### DA-FAM-003: Implementation Slice' "$CARD_374" "mini task board must expose implementation slice"
guard_expect_fixed_in_file "$TAG" '### DA-FAM-004: Post-Retirement Semantic Smoke' "$CARD_374" "mini task board must expose semantic smoke"
guard_expect_fixed_in_file "$TAG" '### DA-FAM-005: Post-Retirement Perf Owner Refresh' "$CARD_374" "mini task board must expose perf refresh"
guard_expect_fixed_in_file "$TAG" '### DA-FAM-006: ArrayRepr Design Row' "$CARD_374" "mini task board must expose ArrayRepr design"
guard_expect_fixed_in_file "$TAG" '### DA-FAM-007: .hako ArrayCore Owner Alignment' "$CARD_374" "mini task board must expose ArrayCore alignment"
guard_expect_fixed_in_file "$TAG" '### DA-FAM-008: DirectArray Family Extension Gate' "$CARD_374" "mini task board must expose extension gate"
