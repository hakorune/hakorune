...
require_text "$TASK_BREAKDOWN" "M87-M96 are landed"
require_text "$TASK_BREAKDOWN" "M96 is the latest runtime diagnostic checkpoint"
require_text "$TASK_BREAKDOWN" "next safe row is M97 selection decision CLI surface"
require_text "$CURRENT_STATE" 'latest_card = "293x-150-M96-ALLOCATOR-PROVIDER-SELECTION-DECISION-DIAGNOSTIC-REPORT"'
require_text "$CURRENT_STATE" 'latest_card_path = "docs/development/current/main/phases/phase-293x/293x-150-M96-ALLOCATOR-PROVIDER-SELECTION-DECISION-DIAGNOSTIC-REPORT.md"'
