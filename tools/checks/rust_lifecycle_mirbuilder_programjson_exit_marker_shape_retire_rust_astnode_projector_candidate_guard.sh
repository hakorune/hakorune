...
TAG="rust-lifecycle-mirbuilder-programjson-exit-marker-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-exit-marker-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonExitMarkerShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ExitMarkerShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Break/Continue marker rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonExitMarkerShapeScanV1", "bad owner")
...
need(criteria.get("cfg_construction") == 0, "CFG construction must remain unclaimed")
need(criteria.get("exit_lowering") == 0, "exit lowering must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_exit_marker_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-exit-marker-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-EXIT-MARKER-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ExitMarkerShapeSnapshotV1
shape_scope=covered ProgramJSON Break/Continue marker rows
covered_rows=10
