...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD" "card must remain current while the long-timing pack is exercised"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-LONG-PROCESS-REPEAT-TIMING-CLOSEOUT-295X-002' "$CARD" "card must select the long process-repeat closeout follow-on"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-LONG-PROCESS-REPEAT-TIMING-PACK-295X-002' "$TASKBOARD" "taskboard must expose the pack blocker"
...
guard_expect_in_file "$TAG" '| 234 | `MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-LONG-PROCESS-REPEAT-TIMING-PACK-295X-002` | Current |' "$TASKBOARD" "taskboard must expose the pack row as current"
guard_expect_in_file "$TAG" '234' "$CURRENT_STATE" "current state must point at the pack card"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-LONG-PROCESS-REPEAT-TIMING-PACK-295X-002' "$CURRENT_STATE" "current state must expose the pack blocker"
