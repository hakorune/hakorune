...
TAG="rust-lifecycle-mirbuilder-generic-loop-body-check-continue-if-step-validation-projection-policy-guard"
source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

TOOL="$ROOT_DIR/tools/rust_lifecycle/mirbuilder_generic_loop_body_check_continue_if_step_validation_projection_policy.py"
FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-generic-loop-body-check-continue-if-step-validation-projection-policy-v0.json"
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/1916-MIRBUILDER-GENERIC-LOOP-BODY-CHECK-CONTINUE-IF-STEP-VALIDATION-PROJECTION-POLICY-001.md"
...
fixture = json.loads(Path("docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-generic-loop-body-check-continue-if-step-validation-projection-policy-v0.json").read_text())
card = Path("docs/development/current/main/phases/phase-296x/1916-MIRBUILDER-GENERIC-LOOP-BODY-CHECK-CONTINUE-IF-STEP-VALIDATION-PROJECTION-POLICY-001.md").read_text()

token = "MIRBUILDER-GENERIC-LOOP-BODY-CHECK-CONTINUE-IF-STEP-VALIDATION-PROJECTION-POLICY-001"
if fixture.get("kind") != "MirBuilderGenericLoopBodyCheckContinueIfStepValidationProjectionPolicyV1":
...
if state["selected_subcluster_id"] != "ContinueIfStepValidation":
...
surface = surfaces[0]
if surface["validator_role"] != "strict_reject_continue_if_tail_validator":
...
if descriptor["descriptor_id"] != "generic_loop_body_check_continue_if_step_validation_v1":
    raise SystemExit("descriptor id drift")
if descriptor["source_extraction"] != "rust_continue_if_tail_validation":
...
if entry["symbol"] != "validate_continue_if_step":
    raise SystemExit("entry symbol drift")
if entry["allowed_tail_shapes"] != ["empty_tail", "single_break", "single_return"]:
    raise SystemExit("allowed tail shapes drift")
if entry["reject_reason"] != "ContinueIfStepRequiresTrailingExit":
...
if decision["selected_next_card"] != "MIRBUILDER-GENERIC-LOOP-BODY-CHECK-BREAK-ELSE-IF-STEP-VALIDATION-PROJECTION-POLICY-001":
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-generic-loop-body-check-continue-if-step-validation-projection-policy-v0
subcluster=ContinueIfStepValidation
source_count=1
