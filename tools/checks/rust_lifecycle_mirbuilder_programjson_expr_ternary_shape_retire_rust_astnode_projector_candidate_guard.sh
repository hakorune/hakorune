...
TAG="rust-lifecycle-mirbuilder-programjson-expr-ternary-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-expr-ternary-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonExprTernaryShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ExprTernaryShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Expr.expr Ternary rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonExprTernaryShapeScanV1", "bad owner")
...
need(criteria.get("ternary_select_lowering_semantics") == 0, "ternary lowering must remain unclaimed")
...
for key in [
    ...
    "ternary_select_lowering_semantics",
    "runtime_fallback",
    "new_backend_route",
    "new_abi",
]:
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_expr_ternary_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-expr-ternary-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-EXPR-TERNARY-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ExprTernaryShapeSnapshotV1
shape_scope=covered ProgramJSON Expr.expr Ternary rows
covered_rows=9
