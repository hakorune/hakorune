...
TAG="rust-lifecycle-mirbuilder-loop-cond-bc-continue-if-else-pattern-projection-policy-guard"
source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-loop-cond-bc-continue-if-else-pattern-projection-policy-v0.json"
...
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/1837-MIRBUILDER-LOOP-COND-BC-CONTINUE-IF-ELSE-PATTERN-PROJECTION-POLICY-001.md"
SOURCE="$ROOT_DIR/src/mir/builder/control_flow/plan/features/loop_cond_bc_continue_if.rs"
...
fixture = json.loads(Path("docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-loop-cond-bc-continue-if-else-pattern-projection-policy-v0.json").read_text())
...
card = Path("docs/development/current/main/phases/phase-296x/1837-MIRBUILDER-LOOP-COND-BC-CONTINUE-IF-ELSE-PATTERN-PROJECTION-POLICY-001.md").read_text()
source = Path("src/mir/builder/control_flow/plan/features/loop_cond_bc_continue_if.rs").read_text()
callsite = Path("src/mir/builder/control_flow/plan/features/loop_cond_bc_item.rs").read_text()

token = "MIRBUILDER-LOOP-COND-BC-CONTINUE-IF-ELSE-PATTERN-PROJECTION-POLICY-001"
if fixture.get("kind") != "MirBuilderLoopCondBcContinueIfElsePatternProjectionPolicyV1":
...
report_items = {
    ...
    if item.get("loop_cond_bc_else_pattern_subcluster") == "LoopCondBcContinueIfElseCluster"
}
surfaces = fixture.get("source_surfaces") or []
if len(surfaces) != 1:
    raise SystemExit("expected exactly one continue-if else surface")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-loop-cond-bc-continue-if-else-pattern-projection-policy-v0
