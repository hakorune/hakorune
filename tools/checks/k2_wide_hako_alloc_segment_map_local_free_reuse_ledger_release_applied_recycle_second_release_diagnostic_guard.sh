...
guard_expect_in_file "$TAG" 'Status: (landed|selected current)' "$CARD_209A" "MIMAP-209A must be selected current or landed after diagnostic"
...
guard_expect_in_file "$TAG" 'MIMAP-210A' "$CADENCE" "cadence SSOT must name diagnostic closeout boundary"
