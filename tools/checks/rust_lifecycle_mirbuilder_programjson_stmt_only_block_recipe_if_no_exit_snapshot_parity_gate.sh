...
TAG="rust-lifecycle-mirbuilder-programjson-stmt-only-block-recipe-if-no-exit-snapshot-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-stmt-only-block-recipe-if-no-exit-snapshot-parity-v0.json"
SNAPSHOT_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_stmt_only_block_recipe_snapshot.hako"
PREREQ_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_stmt_only_block_recipe_loop_no_exit_snapshot_retire_rust_astnode_projector_candidate_guard.sh"
...
if ! grep -q '^summary=ok$' <<<"$PREREQ_OUT"; then
  printf '%s\n' "$PREREQ_OUT" >&2
  guard_fail "$TAG" "ProgramJSON LoopNoExit retire-candidate prerequisite is not green"
fi

TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-if-no-exit-block-recipe.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_if_no_exit_block_recipe_probe.hako"
MIR_JSON="$TMP_DIR/programjson_if_no_exit_block_recipe_probe.mir.json"
EXE="$TMP_DIR/programjson_if_no_exit_block_recipe_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonStmtOnlyBlockRecipeIfNoExitSnapshotParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-STMT-ONLY-BLOCK-RECIPE-IF-NO-EXIT-SNAPSHOT-PARITY-001":
...
for key, value in {
    ...
    "row_count": 1,
    "loop_no_exit_retire_candidate_prerequisite_green": 1,
}.items():
...
if len(rows) != 1:
    raise SystemExit("IfNoExit ProgramJSON parity requires exactly 1 row")
...
for needle in [
    ...
    "BoxHelpers.map_get(item1, \"then_item\")",
    "BoxHelpers.map_get(item1, \"else_item\")",
    "IfNoExit",
    "StmtOnlyBlockRecipeBox.build_summary(stmt0, stmt1)",
]:
    if needle not in text:
        raise SystemExit(f"missing IfNoExit bridge token: {needle}")
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for ProgramJSON IfNoExit block recipe probe"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-stmt-only-block-recipe-if-no-exit-snapshot-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-STMT-ONLY-BLOCK-RECIPE-IF-NO-EXIT-SNAPSHOT-PARITY-001
