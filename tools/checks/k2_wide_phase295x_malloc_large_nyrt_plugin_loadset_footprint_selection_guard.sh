...
guard_expect_in_file "$TAG" 'Status: Landed' "$CARD" "card must be landed after the selection row hands off to the diagnostic follow-on"
...
guard_expect_in_file "$TAG" '| 201 | `MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-SELECTION-295X-002` | Landed |' "$TASKBOARD" "taskboard must mark the plugin load-set selection row as landed"
guard_expect_in_file "$TAG" '| 202 | `MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-DIAGNOSTIC-295X-002` | Current |' "$TASKBOARD" "taskboard must expose the plugin load-set diagnostic row as current"
