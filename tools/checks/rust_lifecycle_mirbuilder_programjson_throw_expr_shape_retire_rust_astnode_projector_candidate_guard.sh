...
TAG="rust-lifecycle-mirbuilder-programjson-throw-expr-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-throw-expr-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonThrowExprShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ThrowExprShapeSnapshotV1", "bad retire candidate")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonThrowExprShapeScanV1", "bad owner")
...
need(criteria.get("catch_finally_matching") == 0, "catch/finally matching must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_throw_expr_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-throw-expr-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-THROW-EXPR-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ThrowExprShapeSnapshotV1
shape_scope=covered ProgramJSON Throw.expr rows
covered_rows=9
