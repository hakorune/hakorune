...
CARD="docs/development/current/main/phases/phase-296x/1788-VARIABLE-CONTEXT-OWNED-READ-SNAPSHOT-HAKO-ADOPTION-DECISION-001.md"
FIXTURE="docs/development/current/main/design/fixtures/rust-lifecycle/variable-context-owned-read-snapshot-hako-adoption-decision-v0.json"
...
python3 - "$CARD" "$FIXTURE" "$RERUN" "$ROUTE_MANIFEST" "$STATE" "$TASK_ORDER" "$INDEX" <<'PY'
...
token = "VARIABLE-CONTEXT-OWNED-READ-SNAPSHOT-HAKO-ADOPTION-DECISION-001"
...
require("returned_mutable_borrow_selected=0" in card, "card mutable borrow drift")
require("raw_variable_map_alias_selected=0" in card, "card raw alias drift")

require(fixture.get("kind") == "VariableContextOwnedReadSnapshotHakoAdoptionDecisionV1", "fixture kind drift")
...
require(decision.get("reason_token") == "NativeSurfaceOwnerPresentAndOwnedReadSnapshotGreen", "fixture reason drift")
...
for scope in included_scopes[:4]:
...
require("tools/checks/rust_lifecycle_variable_context_owned_read_snapshot_hako_adoption_decision_guard.sh" in index, "check index missing guard")

print(f"output_contract={output_contract}")
print(f"surface_id={surface_id}")
print("decision=Adopt")
print("included_scope_count=5")
print("native_hako_source_owner_present=1")
print("native_behavior_guard_green=1")
print("owned_read_snapshot_projection_green=1")
