...
USIZE_BIN_CARD="docs/development/current/main/phases/phase-294x/294x-71-HAKO-ALLOC-USIZE-PAGE-QUEUE-BIN.md"
...
guard_expect_in_file "$TAG" 'bin: usize' "$QUEUE_BOX" "page queue bin must be exact usize size-class index storage"
guard_expect_fixed_in_file "$TAG" 'birth(bin: usize)' "$QUEUE_BOX" "page queue birth must carry exact usize bin"
...
for name in ("bin", "add_count", "select_count", "direct_hit_count", "refresh_count", "reject_count", "page_count", "direct_page_index"):
    field = fields.get(name)
    if field is None or field.get("declared_type") != "usize" or field.get("storage") != "usize":
        raise SystemExit(f"page queue {name} must be exact usize storage: {field}")
for name in ("has_direct_page",):
    field = fields.get(name)
