...
TAG="rust-lifecycle-mirbuilder-programjson-expr-logical-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-expr-logical-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonExprLogicalShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ExprLogicalShapeSnapshotV1", "bad retire candidate")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonExprLogicalShapeScanV1", "bad owner")
...
need(criteria.get("boolean_short_circuit_lowering_semantics") == 0, "short-circuit lowering must remain unclaimed")
...
for key in [
    ...
    "boolean_short_circuit_lowering_semantics",
    "runtime_fallback",
    "new_backend_route",
    "new_abi",
]:
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_expr_logical_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-expr-logical-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-EXPR-LOGICAL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ExprLogicalShapeSnapshotV1
shape_scope=covered ProgramJSON Expr.expr Logical rows
covered_rows=9
...
selected_next_card=MIRBUILDER-PROGRAMJSON-LOCAL-ARRAY-LITERAL-SHAPE-SCAN-CAPABILITY-001
summary=ok
REPORT
