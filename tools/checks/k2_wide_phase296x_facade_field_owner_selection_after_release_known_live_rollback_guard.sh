...
DOC="$ROOT_DIR/docs/development/current/main/phases/phase-296x/296x-273-FACADE-FIELD-OWNER-SELECTION-AFTER-RELEASE-KNOWN-LIVE-ROLLBACK.md"
PREV="$ROOT_DIR/docs/development/current/main/phases/phase-296x/296x-272-FACADE-EXACT-SLOT-IR-SHAPE-DIFF-INVENTORY-AFTER-RELEASE-KNOWN-LIVE-ROLLBACK.md"
TOOL="$ROOT_DIR/tools/allocator/object_lifecycle_facade_field_owner_selection_after_rollback.py"
TMP_DIR="$(mktemp -d /tmp/hakorune_row273_facade_owner.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[row273-facade-owner-after-rollback] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_line "$DOC" "output_contract=facade-field-owner-selection-after-release-known-live-rollback-v0"
...
require_line "$DOC" "facade_receiver_state_count=15"
...
require_line "$DOC" "next_diagnostic=post_facade_inventory_owner_refresh_after_release_known_live_rollback"
...
python3 "$TOOL" --inventory-report "$INVENTORY" --out "$REPORT" >/dev/null

require_line "$REPORT" "output_contract=facade-field-owner-selection-after-release-known-live-rollback-v0"
...
require_line "$REPORT" "next_diagnostic=post_facade_inventory_owner_refresh_after_release_known_live_rollback"
