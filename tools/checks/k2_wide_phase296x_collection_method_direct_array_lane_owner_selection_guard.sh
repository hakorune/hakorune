...
TAG="k2-wide-phase296x-collection-method-direct-array-lane-owner-selection"
cd "$ROOT_DIR"
...
SELF_SCRIPT="tools/checks/k2_wide_phase296x_collection_method_direct_array_lane_owner_selection_guard.sh"

echo "[$TAG] checking collection-method direct-array lane owner selection"
...
guard_expect_fixed_in_file "$TAG" 'output_contract=collection-method-direct-array-lane-owner-selection-v0' "$CARD_404" "row404 must define output contract"
guard_expect_fixed_in_file "$TAG" 'input_contract=collection-method-route-order-inventory-v0' "$CARD_404" "row404 must consume row403"
guard_expect_fixed_in_file "$TAG" 'direct_array_lane_exact_only=1' "$CARD_404" "row404 must keep the direct array lane exact-only"
guard_expect_fixed_in_file "$TAG" 'selected_next=collection_method_call_direct_array_lane_guard_surface' "$CARD_404" "row404 must choose the direct-array lane guard surface"
...
guard_expect_fixed_in_file "$TAG" 'shared_route_order_surface=collection_method_call.py' "$CARD_404" "row404 must keep collection_method_call.py as the primary route-order surface"
