...
python3 - "$FIXTURE" "$CARD" "$TASK_ORDER" "$MANIFEST" "$STRING_SOURCE" "$STRING_TEST" <<'PY'
import json
import sys
...
string_source = Path(sys.argv[5]).read_text(encoding="utf-8")
string_test = Path(sys.argv[6]).read_text(encoding="utf-8")
