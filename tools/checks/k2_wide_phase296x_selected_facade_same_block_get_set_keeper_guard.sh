...
DOC="$ROOT_DIR/docs/development/current/main/phases/phase-296x/296x-230-SELECTED-FACADE-SAME-BLOCK-GET-SET-KEEPER.md"
PREV="$ROOT_DIR/docs/development/current/main/phases/phase-296x/296x-229-SELECTED-FACADE-SAME-BLOCK-GET-SET-GUARD-SURFACE.md"
...
TMP_DIR="$(mktemp -d /tmp/hakorune_row230_facade_keeper.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[row230-facade-keeper] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_contains() {
  ...
  local expected="$2"
  if ! grep -q "$expected" "$file"; then
    echo "[row230-facade-keeper] missing content in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_line "$DOC" "output_contract=selected-facade-same-block-get-set-keeper-v0"
require_line "$DOC" "input_contract=selected-facade-same-block-get-set-guard-surface-v0"
...
require_line "$DOC" "planned_erased_get_set_helper_calls=12"
require_line "$DOC" "planned_added_fused_helper_calls=6"
...
require_contains "$LOWER" "HakoAllocObjectLifecycleFacade.objectLifecycleReleaseDirectCachedPage/2"
require_contains "$LOWER" "HakoAllocObjectLifecycleFacade.objectLifecycleReleaseKnownPageIndex/1"
require_contains "$LOWER" "HakoAllocObjectLifecycleFacade.objectLifecycleSmallAlloc/1"
require_contains "$LOWER" "HakoAllocObjectLifecycleFacade.objectLifecycleSmallAllocAligned/2"
...
if [ "$fused_symbol_count" -lt 1 ]; then
  echo "[row230-facade-keeper] exact-EXE does not reference fused runtime symbol" >&2
  exit 1
fi
