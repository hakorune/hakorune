...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD" "semantic selection card must be current"
...
guard_expect_fixed_in_file "$TAG" 'MIMALLOC-COMPARISON-REMOTE-FREE-PRODUCTION-FACADE-SELECTION-295X-002' "$CARD" "card must select remote-free production facade"
...
guard_expect_fixed_in_file "$TAG" '| 240 | `MIMALLOC-COMPARISON-NEXT-SEMANTIC-SEAM-SELECTION-295X-002` | Current |' "$TASKBOARD" "taskboard must expose semantic selection current row"

guard_expect_fixed_in_file "$TAG" '295x-240-MIMALLOC-COMPARISON-NEXT-SEMANTIC-SEAM-SELECTION' "$CURRENT_STATE" "current state must point at the semantic selection card"
