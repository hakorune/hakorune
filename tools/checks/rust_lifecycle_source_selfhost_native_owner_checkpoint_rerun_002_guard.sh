...
TAG="rust-lifecycle-source-selfhost-native-owner-checkpoint-rerun-002"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

CARD="docs/development/current/main/phases/phase-296x/2058-SOURCE-SELFHOST-NATIVE-OWNER-CHECKPOINT-RERUN-002.md"
FIXTURE="docs/development/current/main/design/fixtures/rust-lifecycle/source-selfhost-native-owner-checkpoint-rerun-002-v0.json"
...
TOOL="tools/rust_lifecycle/source_selfhost_native_owner_checkpoint_rerun_002.py"
...
token = "SOURCE-SELFHOST-NATIVE-OWNER-CHECKPOINT-RERUN-002"
contract = "rust-lifecycle-source-selfhost-native-owner-checkpoint-rerun-002-v0"
...
require(fixture.get("kind") == "SourceSelfhostNativeOwnerCheckpointRerunV2", "fixture kind drift")
...
local_authority = fixture.get("local_authority") or {}
require(local_authority.get("local_selection_authority") == "LocalMechanicalSelectorAuthorityV1", "local authority drift")
require(local_authority.get("worker_inventory") == "consumed", "worker inventory drift")
require(
    local_authority.get("worker_inventory_scope") == "read_only_current_fixtures_cards_ledgers",
    "worker inventory scope drift",
)

fresh = fixture.get("fresh_report") or {}
require(fresh.get("projection_descriptor_ledger_hash_fresh") is True, "projection ledger freshness drift")
require(fresh.get("native_owner_adoption_ledger_hash_fresh") is True, "adoption ledger freshness drift")
require(fresh.get("native_owner_adoption_delta_after_rerun_003_count") == 1, "adoption delta count drift")
require(
    fresh.get("native_owner_adoption_delta_after_rerun_003")
    == ["MIRBUILDER-EMISSION_SSA_PHI-HAKO-ADOPTION-DECISION-001"],
    "adoption delta token drift",
)

native_map = fixture.get("native_owner_map") or {}
require(native_map.get("native_owner_count") == 12, "native owner count drift")
...
carrier = blockers.get("CarrierTypeTransportPriority") or {}
route = blockers.get("RouteRepairNeeded") or {}
require(missing.get("candidate_count") == report_summary.get("missing_projection_policy_count") == 1004, "missing projection count drift")
require(missing.get("evidence_quality_count") == 819, "missing projection evidence quality drift")
require(missing.get("selection_eligible") is True, "missing projection must be eligible")
require(missing.get("next_card") == next_card, "missing projection next drift")
...
require(carrier.get("selection_eligible") is False, "carrier lane must not be selected")
...
require(decision.get("kind") == "SelectMissingProjectionPolicyClusterResolutionV4", "decision kind drift")
