...
TAG="k2-wide-phase295x-nyrt-rss-checkpoint-run"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-41-MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-RUN.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-40-MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-DIAGNOSTIC.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_nyrt_rss_checkpoint_run_guard.sh"
APP="apps/hako-alloc-mimalloc-comparison-empty-noio-exe-proof/main.hako"

echo "[$TAG] checking phase-295x NyRT RSS checkpoint run"

guard_require_files "$TAG" "$CARD" "$PREV_CARD" "$TASKBOARD" "$INDEX" "$SELF_SCRIPT" "$APP"
guard_require_exec_files "$TAG" "$SELF_SCRIPT"

guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-RUN-295X-001' "$CARD" "card must identify the current blocker"
