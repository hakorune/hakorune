...
bash "$ROOT_DIR/tools/checks/rustc_semir_adapter_binding_context_thir_body_inventory_guard.sh" >/dev/null
...
REPORT_JSON="$TMP_DIR/binding-context-mir-lifecycle.json"
...
(
  ...
  cargo run --quiet --features rustc-private -- --binding-context-mir-lifecycle-facts \
...
assert report["output_contract"] == "rustc-semir-adapter-binding-context-mir-lifecycle-facts-v0"
assert report["schema_version"] == 0
assert report["kind"] == "RustcSemirBindingContextMirLifecycleFacts"
...
for ref, body in bodies.items():
    assert body["thir_owner_reference"] == ref
    assert body["module_id"] == "crate::binding_context"
    assert body["basic_block_count"] > 0
    assert body["terminator_count"] > 0
    assert body["local_count"] > 0
    assert not body["source"]["path"].startswith("/")
    assert "Return" in body["terminator_kind_counts"]
    assert body["drop_classification"] in {
        "no_explicit_drop_terminator_observed",
        "explicit_drop_terminator_observed",
    }

assert bodies["body:crate::binding_context::BindingContext::insert"]["mutable_borrow_count"] > 0
assert bodies["body:crate::binding_context::BindingContext::lookup"]["shared_borrow_count"] > 0

coverage = report["coverage"]
assert coverage["binding_context_family_selected"] == 1
assert coverage["hir_owner_reference_used"] == 1
assert coverage["thir_owner_reference_used"] == 1
assert coverage["selected_definition_count"] >= len(required)
assert coverage["copy_move_inventory_present"] == 1
assert coverage["borrow_inventory_present"] == 1
assert coverage["drop_classification_present"] == 1

totals = report["totals"]
assert totals["copy_count"] + totals["move_count"] > 0
assert (
...
assert "drop_terminator_count" in totals

claims = report["claims"]
assert claims["HakoLifecyclePlan_emitted"] == 0
assert claims["hako_plan_emitted"] == 0
assert claims["hako_source_emitted"] == 0
assert claims["backend_behavior_changed"] == 0
assert claims["authority_promoted"] == 0
PY

cat <<'REPORT'
output_contract=rustc-semir-adapter-binding-context-mir-lifecycle-facts-guard-v0
