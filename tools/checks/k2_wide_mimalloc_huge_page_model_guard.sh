...
guard_expect_in_file "$TAG" 'huge_count: usize = 0' "$HUGE_MODEL" "M180 huge_count mirror must be exact usize after 294x-34"
guard_expect_in_file "$TAG" 'live_count: usize = 0' "$HUGE_MODEL" "M180 live_count mirror must be exact usize after 294x-34"
guard_expect_in_file "$TAG" 'allocate_count: usize = 0' "$HUGE_MODEL" "M180 allocate_count must be exact usize after 294x-35"
guard_expect_in_file "$TAG" 'release_count: usize = 0' "$HUGE_MODEL" "M180 release_count must be exact usize after 294x-35"
guard_expect_in_file "$TAG" 'release_reject_count: usize = 0' "$HUGE_MODEL" "M180 release_reject_count must be exact usize after 294x-35"
guard_expect_in_file "$TAG" 'zero_reject_count: usize = 0' "$HUGE_MODEL" "M180 zero_reject_count must be exact usize after 294x-35"
...
guard_expect_in_file "$TAG" 'last_requested_size: usize = 0' "$HUGE_MODEL" "M180 requested-size observer must be exact usize after 294x-186"
guard_expect_in_file "$TAG" 'last_committed_size: usize = 0' "$HUGE_MODEL" "M180 committed-size observer must be exact usize after 294x-186"
guard_expect_in_file "$TAG" 'last_failure_kind: i64 = 0' "$HUGE_MODEL" "M180 failure-kind observer must remain signed vocabulary"
