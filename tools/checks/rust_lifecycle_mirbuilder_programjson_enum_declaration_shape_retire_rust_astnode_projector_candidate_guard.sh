...
TAG="rust-lifecycle-mirbuilder-programjson-enum-declaration-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-enum-declaration-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonEnumDeclarationShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "EnumDeclarationShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON EnumDeclaration rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonEnumDeclarationShapeScanV1", "bad owner")
...
need(criteria.get("enum_lowering") == 0, "enum lowering must remain 0")
need(criteria.get("variant_resolution") == 0, "variant resolution must remain 0")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_enum_declaration_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-enum-declaration-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-ENUM-DECLARATION-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=EnumDeclarationShapeSnapshotV1
shape_scope=covered ProgramJSON EnumDeclaration rows
covered_rows=8
