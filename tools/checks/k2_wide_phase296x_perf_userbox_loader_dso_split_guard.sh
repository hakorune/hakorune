...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_loader_dso_split.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-loader-dso-split] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-loader-dso-split] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
if [ "$ld_linux_top_count" -lt 1 ]; then
  echo "[perf-userbox-loader-dso-split] expected ld-linux top rows in ${STARTUP_REPORT#$ROOT_DIR/}" >&2
  exit 1
fi
if [ "$libc_top_count" -lt 1 ]; then
  echo "[perf-userbox-loader-dso-split] expected libc top rows in ${STARTUP_REPORT#$ROOT_DIR/}" >&2
  exit 1
fi
...
cat <<EOF
output_contract=perf-userbox-loader-dso-split-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_loader_dso_split
