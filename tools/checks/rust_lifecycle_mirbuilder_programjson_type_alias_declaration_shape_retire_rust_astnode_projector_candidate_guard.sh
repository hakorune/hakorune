...
TAG="rust-lifecycle-mirbuilder-programjson-type-alias-declaration-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-type-alias-declaration-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonTypeAliasDeclarationShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "TypeAliasDeclarationShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON TypeAliasDeclaration rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonTypeAliasDeclarationShapeScanV1", "bad owner")
...
need(criteria.get("type_resolution") == 0, "type resolution must remain 0")
need(criteria.get("alias_expansion") == 0, "alias expansion must remain 0")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_type_alias_declaration_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-type-alias-declaration-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-TYPE-ALIAS-DECLARATION-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=TypeAliasDeclarationShapeSnapshotV1
shape_scope=covered ProgramJSON TypeAliasDeclaration rows
covered_rows=8
