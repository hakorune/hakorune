...
FIXTURE="$ROOT/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-carrier-type-scalar-known-write-push-surface-typed-direct-closeout-contract-basis-v0.json"
TOOL="$ROOT/tools/rust_lifecycle/mirbuilder_carrier_type_scalar_known_write_push_surface_typed_direct_closeout_contract_basis.py"
CARD="$ROOT/docs/development/current/main/phases/phase-296x/2120-MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-PUSH-SURFACE-TYPED-DIRECT-CLOSEOUT-CONTRACT-BASIS-001.md"
...
token = "MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-PUSH-SURFACE-TYPED-DIRECT-CLOSEOUT-CONTRACT-BASIS-001"
next_card = "MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-PUSH-SURFACE-DIRECT-CLOSEOUT-RERUN-001"

need(fixture.get("kind") == "MirBuilderCarrierTypeScalarKnownWritePushSurfaceTypedDirectCloseoutContractBasisV1", "bad kind")
need(fixture.get("token") == token, "bad token")
need(token in card, "card missing token")

inputs = fixture.get("input_state") or {}
need(inputs.get("selected_write_subsurface") == "PushSurfacePolicy", "selected subsurface drift")
...
need(contract.get("route_kind_set") == ["ArrayAppendAny"], "route set drift")
need(contract.get("proof_or_policy_source") == ["PushSurfacePolicy"], "policy source drift")
need(contract.get("runtime_mutation_authority") is False, "runtime mutation drift")
need(contract.get("delete_surface_policy_included") is False, "delete inclusion drift")
...
need(summary.get("write_push_surface_typed_direct_closeout_contract_basis") == 1, "summary basis drift")
need(summary.get("write_push_route_count") == 1, "summary route count drift")
for key in [
    "direct_contract_materialized",
    "write_push_direct_closeout_ready",
...
need(decision.get("kind") == "SelectWritePushSurfaceDirectCloseoutRerun", "decision kind drift")
need(decision.get("reason_token") == "WritePushSurfaceTypedDirectCloseoutContractBasisDefined", "reason drift")
need(decision.get("selected_next_card") == next_card, "next drift")

claims = fixture.get("claims") or {}
for key in ["write_push_surface_typed_direct_closeout_contract_basis", "basis_only"]:
...
need(manifest_row.get("card", "").endswith("2120-MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-PUSH-SURFACE-TYPED-DIRECT-CLOSEOUT-CONTRACT-BASIS-001.md"), "manifest card drift")
need(manifest_row.get("fixture", "").endswith("mirbuilder-carrier-type-scalar-known-write-push-surface-typed-direct-closeout-contract-basis-v0.json"), "manifest fixture drift")
need(manifest_row.get("legacy_guard", "").endswith("rust_lifecycle_mirbuilder_carrier_type_scalar_known_write_push_surface_typed_direct_closeout_contract_basis_guard.sh"), "manifest guard drift")

need(token in task_order, "task order missing token")
need(f"selected_next_card={next_card}" in task_order, "task order next drift")

print("output_contract=rust-lifecycle-mirbuilder-carrier-type-scalar-known-write-push-surface-typed-direct-closeout-contract-basis")
print("write_push_surface_typed_direct_closeout_contract_basis=1")
print("write_push_route_count=1")
