...
TAG="k2-wide-mimalloc-facade-small-alloc-exe"
cd "$ROOT_DIR"
source "$ROOT_DIR/tools/checks/lib/pure_first_exe_guard.sh"

APP="apps/mimalloc-facade-small-alloc-proof/main.hako"
APP_README="apps/mimalloc-facade-small-alloc-proof/README.md"
...
CARD="docs/development/current/main/phases/phase-293x/293x-357-MIMAP-014A-FACADE-SMALL-ALLOC.md"
...
rg -F -q 'objectLifecycleAllocPageId()' "$FACADE"
...
if rg -F -q 'local page = me.object_lifecycle_queue.selectPage()' "$FACADE"; then
  echo "[$TAG] ERROR: MIMAP-014A must not bind the selected page object returned by selectPage" >&2
  exit 1
fi
...
tmp_dir="$(mktemp -d /tmp/hakorune_mimap014a_facade_small_alloc.XXXXXX)"
trap 'rm -rf "$tmp_dir"' EXIT

mir_json="$tmp_dir/mimap014a.mir.json"
exe_out="$tmp_dir/mimap014a.exe"
...
rg -F -q 'mimalloc-facade-small-alloc-proof' "$run_log"
rg -F -q 'adds=0' "$run_log"
rg -F -q 'alloc_page=40' "$run_log"
rg -F -q 'alloc_block=0' "$run_log"
rg -F -q 'alloc_reason=0' "$run_log"
