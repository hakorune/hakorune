...
TAG="rust-lifecycle-mirbuilder-programjson-recipebodies-verifier-boundary-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipebodies-verifier-boundary-retire-rust-astnode-projector-candidate-v0.json"
PARITY_FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipebodies-verifier-boundary-parity-v0.json"
PARITY_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_recipebodies_verifier_boundary_parity_gate.sh"
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/3216-MIRBUILDER-PROGRAMJSON-RECIPEBODIES-VERIFIER-BOUNDARY-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001.md"
...
if ! grep -q '^summary=ok$' <<<"$PARITY_OUT"; then
  printf '%s\n' "$PARITY_OUT" >&2
  guard_fail "$TAG" "RecipeBodies verifier-boundary parity gate is not green"
fi
...
token = "MIRBUILDER-PROGRAMJSON-RECIPEBODIES-VERIFIER-BOUNDARY-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001"
next_card = "MIRBUILDER-PROGRAMJSON-RECIPEBODIES-AFTER-VERIFIER-BOUNDARY-NEXT-CONTRACT-SELECTION-001"
rows = ["local_loop_body_if_branch_return"]

if fixture.get("kind") != "MirBuilderProgramJsonRecipeBodiesVerifierBoundaryRustAstNodeProjectorRetireCandidateV1":
...
if parity.get("claims", {}).get("recipe_bodies_verifier_boundary_implemented") != 1:
    raise SystemExit("verifier-boundary implementation not proven")
...
if "3215 implements RecipeBodies verifier boundary parity" not in task_order:
    raise SystemExit("task-order 3215 marker missing")
PY

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-recipebodies-verifier-boundary-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-RECIPEBODIES-VERIFIER-BOUNDARY-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
...
selected_next_card=MIRBUILDER-PROGRAMJSON-RECIPEBODIES-AFTER-VERIFIER-BOUNDARY-NEXT-CONTRACT-SELECTION-001
summary=ok
REPORT
