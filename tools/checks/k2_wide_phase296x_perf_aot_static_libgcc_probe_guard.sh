...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_aot_static_libgcc.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

REPORT="$TMP_DIR/report.out"
EXE="$TMP_DIR/ret0-static-libgcc.exe"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-aot-static-libgcc] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
if grep -q 'libgcc_s' "$LDD_OUT" "$READELF_OUT"; then
  echo "[perf-aot-static-libgcc] libgcc_s must not be dynamically needed" >&2
  cat "$LDD_OUT" >&2
  cat "$READELF_OUT" >&2
  exit 1
fi
...
{
  echo "output_contract=perf-aot-static-libgcc-probe-v0"
  echo "input_contract=perf-userbox-startup-loader-owner-split-v0"
  echo "measurement_scope=exact_aot_static_libgcc_link_probe"
  echo "aot_ldflags=-static-libgcc"
  echo "hyphen_linker_flag_forwarded=1"
  echo "ret0_exact_aot_build_status=ok"
  echo "ret0_exact_aot_run_status=ok"
  echo "dynamic_needed_libgcc_s=0"
  echo "dynamic_needed_libm=$(grep -q 'libm.so' "$LDD_OUT" && echo 1 || echo 0)"
  ...
  echo "full_static_link=0"
  echo "default_link_mode_changed=0"
  echo "summary=ok"
} >"$REPORT"

require_line "$REPORT" "output_contract=perf-aot-static-libgcc-probe-v0"
require_line "$REPORT" "aot_ldflags=-static-libgcc"
require_line "$REPORT" "hyphen_linker_flag_forwarded=1"
