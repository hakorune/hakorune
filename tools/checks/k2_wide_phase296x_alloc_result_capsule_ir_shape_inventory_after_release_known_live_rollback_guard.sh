...
DOC="$ROOT_DIR/docs/development/current/main/phases/phase-296x/296x-275-ALLOC-RESULT-CAPSULE-IR-SHAPE-INVENTORY-AFTER-RELEASE-KNOWN-LIVE-ROLLBACK.md"
PREV="$ROOT_DIR/docs/development/current/main/phases/phase-296x/296x-274-POST-FACADE-INVENTORY-OWNER-REFRESH-AFTER-RELEASE-KNOWN-LIVE-ROLLBACK.md"
APP="$ROOT_DIR/apps/hako-alloc-mimalloc-comparison-in-process-object-lifecycle-small-block-proof/main.hako"
TMP_DIR="$(mktemp -d /tmp/hakorune_row275_result_capsule.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[row275-alloc-result-capsule-inventory] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_line "$DOC" "output_contract=alloc-result-capsule-ir-shape-inventory-after-release-known-live-rollback-v0"
require_line "$DOC" "input_contract=post-facade-inventory-owner-refresh-after-release-known-live-rollback-v0"
...
require_line "$DOC" "selected_next=result_capsule_owner_selection_after_release_known_live_rollback"
...
def collect(prefix: str):
    ...
    for fn in data.get("functions", []):
        ...
        if field_ops > top[1]:
            top = (name, field_ops)
        if ("recordSuccess" in name or "recordRequest" in name) and field_ops > hot[1]:
            hot = (name, field_ops)
    return method_count, totals, top, hot

alloc_count, alloc, alloc_top, alloc_hot = collect("HakoAllocObjectLifecycleAllocResult.")
release_count, release, release_top, release_hot = collect("HakoAllocObjectLifecycleReleaseResult.")
...
print("output_contract=alloc-result-capsule-ir-shape-inventory-after-release-known-live-rollback-v0")
print("input_contract=post-facade-inventory-owner-refresh-after-release-known-live-rollback-v0")
...
print(f"top_alloc_hot_method={alloc_hot[0]}")
print(f"top_alloc_hot_method_field_op_count={alloc_hot[1]}")
print(f"top_release_method={release_top[0]}")
print(f"top_release_method_field_op_count={release_top[1]}")
print(f"top_release_hot_method={release_hot[0]}")
print(f"top_release_hot_method_field_op_count={release_hot[1]}")
print("selected_next=result_capsule_owner_selection_after_release_known_live_rollback")
...
require_line "$REPORT" "output_contract=alloc-result-capsule-ir-shape-inventory-after-release-known-live-rollback-v0"
...
require_line "$REPORT" "selected_next=result_capsule_owner_selection_after_release_known_live_rollback"
