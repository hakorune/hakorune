...
TAG="k2-wide-phase296x-collection-method-direct-array-lane-selected-method-pilot"
cd "$ROOT_DIR"
...
SELF_SCRIPT="tools/checks/k2_wide_phase296x_collection_method_direct_array_lane_selected_method_pilot_guard.sh"

echo "[$TAG] checking collection-method direct-array lane selected-method pilot"

guard_require_files "$TAG" "$CARD_405" "$CARD_406" "$CARD_407" "$CARD_408" "$STATE" "$INDEX" "$SELF_SCRIPT"
...
guard_expect_fixed_in_file "$TAG" 'output_contract=collection-method-direct-array-lane-selected-method-pilot-v0' "$CARD_406" "row406 must define output contract"
guard_expect_fixed_in_file "$TAG" 'input_contract=collection-method-direct-array-lane-guard-surface-v0' "$CARD_406" "row406 must consume row405"
guard_expect_fixed_in_file "$TAG" 'selected_method=HakoAllocPageModel.acquire_usize/1' "$CARD_406" "row406 must keep the selected method pinned"
guard_expect_fixed_in_file "$TAG" 'selected_backend=direct_array_i64_exact' "$CARD_406" "row406 must keep the direct-array backend pinned"
guard_expect_fixed_in_file "$TAG" 'selected_method_only=1' "$CARD_406" "row406 must remain selected-method only"
guard_expect_fixed_in_file "$TAG" 'direct_array_lane_exact_only=1' "$CARD_406" "row406 must keep the direct array lane exact-only"
guard_expect_fixed_in_file "$TAG" 'receiver_origin_fact_value=ArrayRepr::DirectI64' "$CARD_406" "row406 must consume the explicit DirectI64 fact"
guard_expect_fixed_in_file "$TAG" 'selected_next=collection_method_call_direct_array_lane_semantic_smoke' "$CARD_406" "row406 must choose the semantic smoke row next"
guard_expect_fixed_in_file "$TAG" 'shared_route_order_surface=collection_method_call.py' "$CARD_406" "row406 must keep collection_method_call.py as the primary route-order surface"
