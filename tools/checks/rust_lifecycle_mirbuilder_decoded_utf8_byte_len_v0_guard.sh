...
guard_require_files "$TAG" "$LEAF" "$VIEW" "$BUDGET" "$FIXTURES"
...
python3 - "$LEAF" "$VIEW" "$BUDGET" "$FIXTURES" <<'PY'
import sys
from pathlib import Path

leaf_path, view_path, budget_path, fixtures_path = map(Path, sys.argv[1:])
...
for needle in ("猫", "😸", "e\\u{0301}", "\\0", "does_not_normalize"):
...
for path in (leaf_path, fixtures_path):
