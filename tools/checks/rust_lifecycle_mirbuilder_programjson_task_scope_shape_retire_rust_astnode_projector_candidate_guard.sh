...
TAG="rust-lifecycle-mirbuilder-programjson-task-scope-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-task-scope-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonTaskScopeShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "TaskScopeShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON TaskScope statement body rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonTaskScopeShapeScanV1", "bad owner")
...
need(criteria.get("task_lowering") == 0, "task lowering must remain unclaimed")
need(criteria.get("concurrency_semantics") == 0, "concurrency semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_task_scope_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-task-scope-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-TASK-SCOPE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=TaskScopeShapeSnapshotV1
shape_scope=covered ProgramJSON TaskScope statement body rows
covered_rows=10
