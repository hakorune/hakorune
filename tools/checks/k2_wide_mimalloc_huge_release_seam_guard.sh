...
guard_expect_in_file "$TAG" 'release_count: usize = 0' "$RELEASE_SEAM" "M181 release_count must be exact usize after 294x-36"
guard_expect_in_file "$TAG" 'unregister_count: usize = 0' "$RELEASE_SEAM" "M181 unregister_count must be exact usize after 294x-36"
guard_expect_in_file "$TAG" 'lookup_miss_count: usize = 0' "$RELEASE_SEAM" "M181 lookup_miss_count must be exact usize after 294x-36"
guard_expect_in_file "$TAG" 'not_huge_count: usize = 0' "$RELEASE_SEAM" "M181 not_huge_count must be exact usize after 294x-36"
guard_expect_in_file "$TAG" 'model_reject_count: usize = 0' "$RELEASE_SEAM" "M181 model_reject_count must be exact usize after 294x-36"
guard_expect_in_file "$TAG" 'reject_count: usize = 0' "$RELEASE_SEAM" "M181 reject_count must be exact usize after 294x-36"
guard_expect_in_file "$TAG" 'last_page_id: i64 = -1' "$RELEASE_SEAM" "M181 last_page_id sentinel must remain signed"
...
guard_expect_in_file "$TAG" 'me\.meta_store\.markReleasedAt\(index\)' "$HUGE_MODEL" "M181 model must delegate huge live-state clearing through C206e indexed C205d store seam"
