...
if registry_top_0_mode != "nyash_rust::box_factory::registry::UnifiedBoxRegistry::rebuild_cache":
    raise SystemExit(1)
if registry_exact_ring0_mode_count <= 0:
    raise SystemExit(1)
...
registry_exact_ring0_mode="${registry_exact_ring0_mode:-missing}"
registry_exact_ring0_mode_count="${registry_exact_ring0_mode_count:-0}"
...
cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-ring0-presence-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_rebuild_cache_ring0_presence
...
if [ "${registry_top_0_mode_count:-0}" -le 0 ]; then
  echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-ring0-presence] expected aggregate dominant count > 0" >&2
  exit 1
fi
