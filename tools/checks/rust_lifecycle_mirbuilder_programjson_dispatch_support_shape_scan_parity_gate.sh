...
TAG="rust-lifecycle-mirbuilder-programjson-dispatch-support-shape-scan-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-dispatch-support-shape-scan-parity-v0.json"
SCAN_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_dispatch_support_shape_scan.hako"
...
guard_require_files "$TAG" "$FIXTURE" "$SCAN_IMPL" "$SCANNER_IMPL" "$CONSUMER_IMPL" "$HAKO_BIN"

TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-dispatch-support-shape-scan.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_dispatch_support_shape_scan_parity.hako"
EXPECTED="$TMP_DIR/expected.json"
RUN_LOG="$TMP_DIR/run.log"
EXE="$TMP_DIR/programjson_dispatch_support_shape_scan_parity.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonDispatchSupportShapeScanParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-DISPATCH-SUPPORT-SHAPE-SCAN-CAPABILITY-001":
...
if emit.get("consumer") != "lang/src/compiler/mirbuilder/program_json_v0_phase_state_consumer_box.hako":
    raise SystemExit("missing dispatch consumer evidence")
if "Local" not in (emit.get("type_field_nodes") or []):
    raise SystemExit("missing type-field node evidence")
if "FunctionDeclaration" not in (emit.get("kind_field_nodes") or []):
    raise SystemExit("missing kind-field node evidence")

rows = fixture.get("rows") or []
if len(rows) < 10:
    raise SystemExit("dispatch-support capability must cover at least 10 rows")
...
for row in rows:
    ...
    calls.append(
        ...
        + " + ProgramJsonDispatchSupportShapeScanBox.build_summary("
...
source = "\n".join([
    "using lang.compiler.mirbuilder.program_json_dispatch_support_shape_scan as ProgramJsonDispatchSupportShapeScanBox",
    "",
    "static box Main {",
    "  main() {",
    *calls,
...
if ! bash "$HAKO_BIN" --backend mir --emit-exe "$EXE" "$APP" >"$EMIT_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit ProgramJSON dispatch-support shape scan parity executable"
fi

if ! "$EXE" >"$RUN_LOG" 2>&1; then
  tail -n 160 "$RUN_LOG" || true
  guard_fail "$TAG" "failed to run ProgramJSON dispatch-support shape scan parity executable"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-dispatch-support-shape-scan-parity-gate-v0
owner=ProgramJsonDispatchSupportShapeScanV1
input_contract=ProgramJSON-v0
output_contract=DispatchSupportShapeSnapshotV1
fixture=mirbuilder-programjson-dispatch-support-shape-scan-parity-v0.json
hako_implementation=lang/src/compiler/mirbuilder/program_json_dispatch_support_shape_scan.hako
scanner=lang/src/compiler/mirbuilder/program_json_v0_scanner_box.hako
consumer=lang/src/compiler/mirbuilder/program_json_v0_phase_state_consumer_box.hako
parity_rows=10
covered_rows=10
