...
TAG="rust-lifecycle-mirbuilder-function-region-stack-pop-hako-adoption-decision-guard"
source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-function-region-stack-pop-hako-adoption-decision-v0.json"
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/1819-MIRBUILDER-FUNCTION-REGION-STACK-POP-HAKO-ADOPTION-DECISION-001.md"
SEED_FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-function-region-stack-pop-hako-native-source-seed-v0.json"
SEED_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_function_region_stack_pop_hako_native_source_seed_guard.sh"
...
NATIVE_SOURCE="$ROOT_DIR/lang/src/compiler/lib/function_region_stack_pop_native_seed.hako"

guard_require_command "$TAG" python3
guard_require_command "$TAG" rg
guard_require_files "$TAG" "$FIXTURE" "$CARD" "$SEED_FIXTURE" "$SEED_GUARD" "$PROMOTION" "$VERIFIER" "$NATIVE_SOURCE"

bash "$SEED_GUARD" >/tmp/function_region_stack_pop_native_seed_guard.out

python3 - <<'PY'
import json
from pathlib import Path

fixture = json.loads(Path("docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-function-region-stack-pop-hako-adoption-decision-v0.json").read_text())
seed = json.loads(Path("docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-function-region-stack-pop-hako-native-source-seed-v0.json").read_text())
...
card = Path("docs/development/current/main/phases/phase-296x/1819-MIRBUILDER-FUNCTION-REGION-STACK-POP-HAKO-ADOPTION-DECISION-001.md").read_text()
native = Path("lang/src/compiler/lib/function_region_stack_pop_native_seed.hako").read_text()

token = "MIRBUILDER-FUNCTION-REGION-STACK-POP-HAKO-ADOPTION-DECISION-001"
if fixture.get("kind") != "MirBuilderFunctionRegionStackPopHakoAdoptionDecisionV1":
    raise SystemExit("fixture kind mismatch")
if fixture.get("token") != token:
    raise SystemExit("fixture token mismatch")
if f"# 1819 - {token}" not in card:
...
if promotion["decision"]["kind"] != "Promote" or promotion["selected_stage"] != "HakoMainline":
    raise SystemExit("FunctionRegionStackPop must be HakoMainline before adoption")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-function-region-stack-pop-hako-adoption-decision-v0
