...
require_text "$TASK_BREAKDOWN" "The next safe row is M78 selection decision diagnostic shape"
...
require_text "$CURRENT_STATE" 'latest_card = "293x-129-M77-ALLOCATOR-PROVIDER-REGISTRY-SNAPSHOT"'
require_text "$CURRENT_STATE" 'latest_card_path = "docs/development/current/main/phases/phase-293x/293x-129-M77-ALLOCATOR-PROVIDER-REGISTRY-SNAPSHOT.md"'
...
if [[ -e "$FUTURE_REGISTRY_FILE" ]]; then
  fail "future registry owner file must remain absent in M77: $FUTURE_REGISTRY_FILE"
fi
...
if rg -n 'ProviderSelectionRequest|ProviderSelectionDecision|select_allocator_provider|allocator_provider_select|allocator_provider_selection_env|NYASH_ALLOCATOR_PROVIDER' \
  ...
  fail "provider selection implementation must stay absent in M77"
fi
rm -f /tmp/"$TAG".provider_selection
...
if rg -n '#\[global_allocator\]|GlobalAlloc' \
  ...
  fail "process allocator replacement must stay inactive in M77"
fi
rm -f /tmp/"$TAG".global_allocator
