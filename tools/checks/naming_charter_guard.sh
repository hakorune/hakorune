...
REFERENCE_ARCH_VM="$ROOT_DIR/docs/reference/architecture/vm.md"
...
require_fixed "HAKORUNE-REFERENCE-DOCS-RESIDUAL-NAMING-DEFER-INVENTORY-001" "$SSOT"
require_fixed 'remaining `docs/reference` Nyash/Stage headings are not a' "$SSOT"
require_fixed "ABI / plugin ABI compatibility:" "$SSOT"
require_fixed "stage-profile policy surfaces:" "$SSOT"
require_fixed "Do not rewrite any deferred class by title cleanup." "$SSOT"
...
require_fixed "# Hakorune VM Execution Guide" "$REFERENCE_ARCH_VM"
if rg -n -F \
  -e "# Nyash VM 実行基盤ガイド（更新）" \
  "$REFERENCE_ARCH_VM"; then
  guard_fail "$TAG" "VM guide title must use Hakorune-first wording"
fi
