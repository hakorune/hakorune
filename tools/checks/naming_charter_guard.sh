...
guard_require_files "$TAG" "$SSOT" "$CHECK_INDEX" "$QUICK_STEPS" "$DOCS_LAYOUT" "$CARGO_TOML" "$README_MD" "$HACO_WRAPPER" "$MAIN_RS" "$HAKORUNE_BIN_RS" "$HAKORUNE_COMPAT_BIN_RS" "$BUILD_SHARED_RS" "$BUILD_PRODUCT_RS" "$BUILD_ENGINEERING_RS" "$HAKO_CHECK_SH" "$BUILD_LLVM_PS" "$BUILD_AOT_PS" "$USING_UNRESOLVED_SMOKE" "$USING_RESOLVE_SMOKE" "$USING_STRICT_PATH_FAIL_SMOKE" "$DEV_SELFHOST_LOOP" "$ENGINEERING_PARITY" "$SELFHOST_EXE_STAGEB" "$SELFHOST_STAGEB_PROOF_VM" "$SELFHOST_RUN_ROUTES" "$SELFHOST_BUILD" "$NY_PARSER_BRIDGE_SMOKE" "$PHI_TRACE_RUN" "$TEST_SHLIB" "$EMIT_MIR_ROUTE" "$HAKO_MIN_BINOP_SMOKE" "$HAKO_MIN_IF_SMOKE" "$HAKO_MIN_INDEX_SMOKE" "$HAKO_MIN_COMPILE_RETURN_SMOKE" "$HAKO_MAP_ESCAPE_SMOKE" "$GATE_C_V1_FILE_SMOKE" "$NYVM_WRAPPER_SMOKE" "$FASTMEM_PARSER_PARITY_SMOKE" "$PARSER_OPT_ANNOTATIONS_SMOKE" "$PARSER_TRY_COMPAT_SMOKE" "$PARSER_MIN_METHODS_SMOKE" "$PARSER_RUNE_DECL_TRACE_SMOKE" "$GATE_C_OOB_STRICT_SMOKE" "$NY_MIR_BUILDER" "$PHASE29X_L1_CACHE" "$PHASE29X_L2_CACHE" "$SMOKE_PREFLIGHT" "$SMOKE_PLUGIN_MANAGER" "$FASTMEM_SOURCE_SYNTAX_SMOKE" "$FASTMEM_TERMINAL_LADDER_SMOKE" "$FASTMEM_SOURCE_MANIFEST_RUNNER" "$SELFHOST_JSON_V0_TRY_CATCH_CANARY" "$SELFHOST_STAGEB_ROUTE_PARITY_SMOKE" "$COLLECTION_MAP_GET_SHARES_MAP_SMOKE" "$COLLECTION_MAP_GET_SHARES_ARRAY_SMOKE" "$COLLECTION_STRING_SIZE_ALIAS_SMOKE" "$GOLDEN_MACRO_RESOLVER" "$ENV_RS" "$ENV_PATHS_RS" "$ENV_DOC"
