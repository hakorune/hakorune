#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
TAG="k2-wide-phase296x-hako-mimalloc-post-select-first-page-cache-source-mir-refresh"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD_100="docs/development/current/main/phases/phase-296x/296x-100-HAKO-MIMALLOC-POST-SELECT-FIRST-PAGE-CACHE-SOURCE-MIR-REFRESH.md"
CARD_101="docs/development/current/main/phases/phase-296x/296x-101-HAKO-MIMALLOC-SELECT-SINGLE-PAGE-ACTIVE-FIELD-FAST-PATH-KEEPER.md"
TASKBOARD="docs/development/current/main/phases/phase-296x/296x-90-mimalloc-benchmark-taskboard.md"
CURRENT_STATE="docs/development/current/main/CURRENT_STATE.toml"
INDEX="docs/tools/check-scripts-index.md"
TOOL="tools/allocator/hako_mimalloc_post_select_first_page_cache_source_mir_refresh.py"
SELF_SCRIPT="tools/checks/k2_wide_phase296x_hako_mimalloc_post_select_first_page_cache_source_mir_refresh_guard.sh"

echo "[$TAG] checking post-select-first-page-cache source/MIR refresh"

guard_require_files "$TAG" "$CARD_100" "$CARD_101" "$TASKBOARD" "$CURRENT_STATE" "$INDEX" "$TOOL" "$SELF_SCRIPT"
guard_require_exec_files "$TAG" "$TOOL" "$SELF_SCRIPT"

guard_expect_fixed_in_file "$TAG" 'Status: Landed' "$CARD_100" "row100 card must be landed"
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_101" "row101 card must be current"
guard_expect_fixed_in_file "$TAG" 'output_contract=hako-mimalloc-post-select-first-page-cache-source-mir-refresh-v0' "$CARD_100" "row100 must record output contract"
guard_expect_fixed_in_file "$TAG" 'inactive_select_loop_risk=1' "$CARD_100" "row100 must park select loop risk"
guard_expect_fixed_in_file "$TAG" 'next_keeper=select_single_page_active_field_fast_path' "$CARD_100" "row100 must select active field fast path keeper"
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-100-HAKO-MIMALLOC-POST-SELECT-FIRST-PAGE-CACHE-SOURCE-MIR-REFRESH"' "$CURRENT_STATE" "current state latest card must advance to row100"
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "HAKO-MIMALLOC-SELECT-SINGLE-PAGE-ACTIVE-FIELD-FAST-PATH-KEEPER-296X-001"' "$CURRENT_STATE" "current state must select row101"
guard_expect_fixed_in_file "$TAG" '| 100 | `HAKO-MIMALLOC-POST-SELECT-FIRST-PAGE-CACHE-SOURCE-MIR-REFRESH-296X-001` | Landed |' "$TASKBOARD" "taskboard row100 must be landed"
guard_expect_fixed_in_file "$TAG" '| 101 | `HAKO-MIMALLOC-SELECT-SINGLE-PAGE-ACTIVE-FIELD-FAST-PATH-KEEPER-296X-001` | Current |' "$TASKBOARD" "taskboard row101 must be current"
guard_expect_fixed_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check index must list this guard"
guard_expect_fixed_in_file "$TAG" "$TOOL" "$INDEX" "check index must list this tool"

tmp_dir="$(mktemp -d /tmp/hakorune_phase296x_post_select_first_page_refresh.XXXXXX)"
trap 'rm -rf "$tmp_dir"' EXIT
cat > "$tmp_dir/measurement.out" <<'EOF'
output_contract=hako-mimalloc-post-select-first-page-cache-keeper-measurement-v0
select_page_single_fast_path_count=524288
select_page_single_fallback_count=0
release_known_page_fast_path_count=524288
release_known_page_fallback_count=0
summary=ok
EOF
cat > "$tmp_dir/select-single.join" <<'EOF'
output_contract=hako-source-mir-shape-join-v1
selected_method=HakoAllocObjectLifecyclePageQueue.trySelectSingleActivePage/0
source_target_method=trySelectSingleActivePage
method_hot_context=caller_repeated
source_risk_confirmed_in_mir=1
confirmed_risk_kind=field_access
mir_call_count=6
mir_field_access_count=20
summary=ok
EOF

report="$tmp_dir/report.out"
python3 "$TOOL" \
  --measurement-report "$tmp_dir/measurement.out" \
  --join-report "$tmp_dir/select-single.join" \
  --out "$report"

guard_expect_fixed_in_file "$TAG" 'output_contract=hako-mimalloc-post-select-first-page-cache-source-mir-refresh-v0' "$report" "tool must emit output contract"
guard_expect_fixed_in_file "$TAG" 'inactive_select_loop_risk=1' "$report" "tool must record inactive select loop risk"
guard_expect_fixed_in_file "$TAG" 'inactive_release_lookup_risk=1' "$report" "tool must record inactive release lookup risk"
guard_expect_fixed_in_file "$TAG" 'selected_source_method=trySelectSingleActivePage' "$report" "tool must select trySelectSingleActivePage"
guard_expect_fixed_in_file "$TAG" 'selected_risk_kind=field_access' "$report" "tool must select field risk"
guard_expect_fixed_in_file "$TAG" 'next_keeper=select_single_page_active_field_fast_path' "$report" "tool must select keeper"
guard_expect_fixed_in_file "$TAG" 'next_keeper_kind=box_count' "$report" "tool must keep keeper kind narrow"
guard_expect_fixed_in_file "$TAG" 'summary=ok' "$report" "tool must end ok"

echo "[$TAG] ok"
