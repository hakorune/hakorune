...
require_text "$TASK_BREAKDOWN" "M64-M72"
require_text "$TASK_BREAKDOWN" "M73 may add a debug guarded provider proof fixture"
...
require_text "$REAL_APP_TASKBOARD" '[x] `293x-124` M72 hako model provider proof fixture'
require_text "$CURRENT_STATE" 'latest_card = "293x-124-M72-HAKO-MODEL-PROVIDER-PROOF-FIXTURE"'
require_text "$CURRENT_STATE" 'latest_card_path = "docs/development/current/main/phases/phase-293x/293x-124-M72-HAKO-MODEL-PROVIDER-PROOF-FIXTURE.md"'
...
if [[ -e "$FUTURE_REGISTRY_FILE" ]]; then
  fail "future registry owner file must remain absent in M72: $FUTURE_REGISTRY_FILE"
fi
...
if rg -n 'AllocatorProviderRegistry|allocator_provider_registry|select_allocator_provider|allocator_provider_select|allocator_provider_selection_env|NYASH_ALLOCATOR_PROVIDER|ProviderRegistryEntry|ProviderRegistrySnapshot|ProviderRegistryBuildInput|ProviderSelectionRequest|ProviderSelectionDecision' \
  ...
  fail "provider registry/selection implementation must stay absent in M72"
fi
rm -f /tmp/"$TAG".provider_registry
...
if rg -n '#\[global_allocator\]|GlobalAlloc' \
  ...
  fail "process allocator replacement must stay inactive in M72"
fi
rm -f /tmp/"$TAG".global_allocator
