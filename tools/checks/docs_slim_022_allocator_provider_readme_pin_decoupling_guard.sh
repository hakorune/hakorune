...
TAG="docs-slim-022-allocator-provider-readme-pin-decoupling"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh
source tools/checks/lib/phase_card_paths.sh

CARD="$(guard_require_phase293x_card "$TAG" "293x-429-DOCS-SLIM-022-ALLOCATOR-PROVIDER-MANIFEST-READINESS-REGISTRY-README-PIN-DECOUPLING.md")"
...
SELF_SCRIPT="tools/checks/docs_slim_022_allocator_provider_readme_pin_decoupling_guard.sh"
...
echo "[$TAG] running DOCS-SLIM-022 allocator provider README pin decoupling guard"
...
guard_expect_in_file "$TAG" "DOCS-SLIM-022" "$CARD" "DOCS-SLIM-022 card must exist"
guard_expect_in_file "$TAG" "Do not move numbered cards in this row" "$CARD" "card must keep no-move stop-line"
guard_expect_in_file "$TAG" "Twenty-second Slimming Phase" "$ARCHIVE_POLICY" "archive policy must record DOCS-SLIM-022"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$CHECK_INDEX" "check index must list DOCS-SLIM-022 guard"
