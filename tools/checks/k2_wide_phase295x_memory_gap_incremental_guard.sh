...
TAG="k2-wide-phase295x-memory-gap-incremental"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-35-MIMALLOC-COMPARISON-MEMORY-GAP-INCREMENTAL.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-34-MIMALLOC-COMPARISON-MEMORY-GAP-BASELINE.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_memory_gap_incremental_guard.sh"
...
echo "[$TAG] checking phase-295x memory gap incremental evidence"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-MEMORY-GAP-INCREMENTAL-295X-001' "$CARD" "card must identify the current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-MEMORY-GAP-CLOSEOUT-295X-001' "$CARD" "card must select closeout follow-on"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-MEMORY-GAP-INCREMENTAL-295X-001' "$PREV_CARD" "previous row must select this incremental row"
...
tmp_dir="$(mktemp -d /tmp/hakorune_phase295x_memory_gap_incremental.XXXXXX)"
...
print("[phase295x-memory-gap-incremental] ok")
PY
