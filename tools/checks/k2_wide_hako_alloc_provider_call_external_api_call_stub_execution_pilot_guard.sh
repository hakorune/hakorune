...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_402A" "$CARD_404A" "$CARD_405A" "$CARD" "$NEXT_CARD" "$DESIGN" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MEMORY_README" "$MODULE_INDEX" "$OWNER" "$PREV_OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
...
guard_expect_in_file "$TAG" 'provider_call_external_api_call_stub_execution_pilot_box.hako' "$MODULE_INDEX" "memory module index must name external API call stub execution owner"
...
guard_expect_in_file "$TAG" 'execution_count: usize = 0' "$OWNER" "external stub owner-local counters must be exact usize"
...
guard_expect_in_file "$TAG" 'last_reason: i64 = 0' "$OWNER" "external stub reason vocabulary must remain signed"
...
owner = plans.get("HakoAllocProviderCallExternalApiCallStubExecutionPilot")
if owner is None:
    raise SystemExit("missing external provider API call stub execution typed object plan")
