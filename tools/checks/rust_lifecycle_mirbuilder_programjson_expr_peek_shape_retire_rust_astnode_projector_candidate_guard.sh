...
TAG="rust-lifecycle-mirbuilder-programjson-expr-peek-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-expr-peek-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonExprPeekShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ExprPeekShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Expr.expr Peek rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonExprPeekShapeScanV1", "bad owner")
...
need(criteria.get("peek_parse_lowering_semantics") == 0, "PeekParse lowering must remain unclaimed")
need(criteria.get("pattern_semantics") == 0, "pattern semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_expr_peek_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-expr-peek-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-EXPR-PEEK-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ExprPeekShapeSnapshotV1
shape_scope=covered ProgramJSON Expr.expr Peek rows
covered_rows=9
