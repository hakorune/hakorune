...
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-13-MIMALLOC-PROVIDER-NOOP-CALL-SMOKE"' "$CURRENT_STATE" "current state latest card must advance"
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "MIMALLOC-PROVIDER-ALLOC-FREE-SMOKE-296X-001"' "$CURRENT_STATE" "current state must expose alloc/free blocker"
