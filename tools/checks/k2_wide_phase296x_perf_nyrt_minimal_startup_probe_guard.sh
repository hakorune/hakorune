...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_nyrt_minimal_startup.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-nyrt-minimal-startup] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_not_line() {
  ...
  if grep -q "^${unexpected}$" "$file"; then
    echo "[perf-nyrt-minimal-startup] unexpected line in ${file#$ROOT_DIR/}: $unexpected" >&2
    exit 1
  fi
}
...
require_line "$REPORT" "minimal_startup_mode=1"
...
require_not_line "$REPORT" "startup_loader_top_3_symbol=nyash_rust::config::env::box_factory_flags::box_factory_policy_mode"
