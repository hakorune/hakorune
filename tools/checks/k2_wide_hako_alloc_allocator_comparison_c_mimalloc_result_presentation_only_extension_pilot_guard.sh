...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_552A" "$CARD_558A" "$CARD" "$DESIGN" "$DESIGN_552A" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MEMORY_README" "$OWNER" "$PREV_OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
...
guard_expect_in_file "$TAG" 'allocator_comparison_c_mimalloc_result_presentation_only_extension_pilot_box.hako' "$MEMORY_README" "memory README must name presentation-only extension pilot owner"
