...
TAG="rust-lifecycle-mirbuilder-programjson-layer4-if-recipe-dto-expanded-return-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-layer4-if-recipe-dto-expanded-return-parity-v0.json"
SNAPSHOT_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_if_recipe_dto_snapshot.hako"
BASE_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_exit_recipe_dto_expanded_return_retire_rust_astnode_projector_candidate_guard.sh"
LEGACY_IF_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_if_recipe_dto_parity_gate.sh"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-layer4-if-expanded-return-dto.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/layer4_if_expanded_return_dto_probe.hako"
MIR_JSON="$TMP_DIR/layer4_if_expanded_return_dto_probe.mir.json"
EXE="$TMP_DIR/layer4_if_expanded_return_dto_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonLayer4IfRecipeDtoExpandedReturnParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-LAYER4-IF-RECIPE-DTO-EXPANDED-RETURN-PARITY-001":
    raise SystemExit("bad fixture token")

rows = fixture.get("rows") or []
if len(rows) != 6:
    raise SystemExit("expanded If Recipe DTO return parity requires exactly 6 rows")
...
for row in rows:
    ...
    if not summary.startswith("snapshot_kind=IfRecipeDtoSnapshotV1;"):
        raise SystemExit(f"bad summary prefix: {row_id}")
    for token in [";err=0", ";matched=1", ";root_len=3", ";then_kind=Exit", ";then_return_kind="]:
    ...
    calls.append(
        ...
        + " + ProgramJsonIfRecipeDtoSnapshotBox.build_summary("
...
source = "\n".join([
    "using lang.compiler.mirbuilder.program_json_if_recipe_dto_snapshot as ProgramJsonIfRecipeDtoSnapshotBox",
    "",
    "static box Main {",
    "  main() {",
    *calls,
...
for needle in [
    ...
    if needle not in text:
        raise SystemExit(f"missing If DTO expanded token: {needle}")
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for expanded If Recipe DTO probe"
fi
...
snapshot = next((fn for fn in functions if fn.get("name") == "ProgramJsonIfRecipeDtoSnapshotBox.build_summary/1"), None)
if main is None:
...
main_routes = [row for row in routes(main) if row.get("symbol") == "ProgramJsonIfRecipeDtoSnapshotBox.build_summary/1"]
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-layer4-if-recipe-dto-expanded-return-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-LAYER4-IF-RECIPE-DTO-EXPANDED-RETURN-PARITY-001
