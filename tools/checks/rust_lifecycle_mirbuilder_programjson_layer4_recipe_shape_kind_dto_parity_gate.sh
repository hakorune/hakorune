...
TAG="rust-lifecycle-mirbuilder-programjson-layer4-recipe-shape-kind-dto-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-layer4-recipe-shape-kind-dto-parity-v0.json"
...
STMT_SEQ_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_recipe_stmt_seq_dto_parity_gate.sh"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-layer4-recipe-shape-kind-dto.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/layer4_recipe_shape_kind_dto_probe.hako"
MIR_JSON="$TMP_DIR/layer4_recipe_shape_kind_dto_probe.mir.json"
EXE="$TMP_DIR/layer4_recipe_shape_kind_dto_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonLayer4RecipeShapeKindDtoParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-LAYER4-RECIPE-SHAPE-KIND-DTO-PARITY-001":
...
if len(rows) < 7:
    raise SystemExit("Recipe shape-kind DTO parity requires at least 7 rows")
...
for row in rows:
    ...
    for token in [";err=0", ";matched=1", ";shape_kind=", ";has_print=", ";has_local=", ";has_assignment=", ";has_return="]:
...
for needle in [
    ...
    "MirJsonV0ShapeRecipeSeq._recipe_stmt_seq_sig(root",
    "me._shape_kind_summary(",
    "_shape_kind_summary(seq_sig, print_kind, return_kind, assign_rhs_kind, retv)",
]:
    if needle not in snapshot:
        raise SystemExit(f"missing Recipe shape-kind DTO implementation token: {needle}")
for needle in [
    "_recipe_stmt_seq_summary(recipe_root",
    "_recipe_stmt_seq_sig(recipe_root",
    "seq_sig=",
    "BoxHelpers.same_token(leaf_kind, \"Stmt\")",
]:
    if needle not in shape_seq:
        raise SystemExit(f"missing Recipe shape-kind helper token: {needle}")
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for Layer4 Recipe shape-kind DTO probe"
fi
...
for symbol in [
    ...
    "MirJsonV0ShapeRecipeSeq._recipe_stmt_seq_sig/2",
]:
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-layer4-recipe-shape-kind-dto-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-LAYER4-RECIPE-SHAPE-KIND-DTO-PARITY-001
