...
TAG="rust-lifecycle-mirbuilder-programjson-local-record-literal-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-local-record-literal-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonLocalRecordLiteralShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "LocalRecordLiteralShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Local.expr RecordLiteral rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonLocalRecordLiteralShapeScanV1", "bad owner")
...
need(criteria.get("record_lowering") == 0, "record lowering must remain unclaimed")
need(criteria.get("field_layout_semantics") == 0, "field layout semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_local_record_literal_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-local-record-literal-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-LOCAL-RECORD-LITERAL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=LocalRecordLiteralShapeSnapshotV1
shape_scope=covered ProgramJSON Local.expr RecordLiteral rows
covered_rows=9
