...
rg -F -q 'hako.osvm.page_size_i64' "$INVENTORY_DOC"
rg -F -q 'hako.osvm.reserve_bytes_i64' "$INVENTORY_DOC"
rg -F -q 'hako.intrin.clz_i64' "$INVENTORY_DOC"
rg -F -q 'hako.intrin.ctz_i64' "$INVENTORY_DOC"
rg -F -q 'hako.intrin.popcnt_i64' "$INVENTORY_DOC"
rg -F -q 'The current live implementation row is `VM-only page/free-list policy-state prototype`.' "$ALLOC_DOC"
rg -F -q 'Third-row acceptance (`VM-only page/free-list policy-state prototype`):' "$ALLOC_DOC"
