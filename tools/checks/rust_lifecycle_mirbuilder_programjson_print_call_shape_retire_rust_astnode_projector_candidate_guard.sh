...
TAG="rust-lifecycle-mirbuilder-programjson-print-call-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-print-call-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonPrintCallShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "PrintCallShapeSnapshotV1", "bad retire candidate")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonPrintCallShapeScanV1", "bad owner")
...
need(criteria.get("general_expr_call_ownership") == 0, "general call ownership must remain unclaimed")
need(criteria.get("console_log_route_execution") == 0, "console/log route execution must remain unclaimed")
need(criteria.get("backend_print_lowering") == 0, "backend print lowering must remain unclaimed")
...
for key in [
    ...
    "general_expr_call_ownership",
    "console_log_route_execution",
    "backend_print_lowering",
    "runtime_fallback",
    "new_backend_route",
    "new_abi",
]:
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_print_call_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-print-call-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-PRINT-CALL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=PrintCallShapeSnapshotV1
shape_scope=covered ProgramJSON Expr.expr Call(name="env.console.log") rows
covered_rows=9
