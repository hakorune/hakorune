...
TAG="rust-lifecycle-mirbuilder-programjson-type-alias-declaration-shape-scan-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-type-alias-declaration-shape-scan-parity-v0.json"
SCAN_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_type_alias_declaration_shape_scan.hako"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-type-alias-declaration-shape-scan.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_type_alias_declaration_shape_scan_parity.hako"
EXPECTED="$TMP_DIR/expected.json"
RUN_LOG="$TMP_DIR/run.log"
EXE="$TMP_DIR/programjson_type_alias_declaration_shape_scan_parity.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonTypeAliasDeclarationShapeScanParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-TYPE-ALIAS-DECLARATION-SHAPE-SCAN-CAPABILITY-001":
...
if emit.get("node_kind") != "TypeAliasDeclaration":
    raise SystemExit("bad ProgramJSON node kind evidence")
if emit.get("fields") != ["name", "target_type"]:
    raise SystemExit("bad ProgramJSON field evidence")

rows = fixture.get("rows") or []
if len(rows) < 8:
    raise SystemExit("type-alias capability must cover at least 8 rows")
...
for row in rows:
    ...
    calls.append(
        ...
        + " + ProgramJsonTypeAliasDeclarationShapeScanBox.build_summary("
...
source = "\n".join([
    "using lang.compiler.mirbuilder.program_json_type_alias_declaration_shape_scan as ProgramJsonTypeAliasDeclarationShapeScanBox",
    "",
    "static box Main {",
    "  main() {",
    *calls,
...
if ! bash "$HAKO_BIN" --backend mir --emit-exe "$EXE" "$APP" >"$EMIT_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit ProgramJSON type-alias shape scan parity executable"
fi

if ! "$EXE" >"$RUN_LOG" 2>&1; then
  tail -n 160 "$RUN_LOG" || true
  guard_fail "$TAG" "failed to run ProgramJSON type-alias shape scan parity executable"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-type-alias-declaration-shape-scan-parity-gate-v0
...
fixture=mirbuilder-programjson-type-alias-declaration-shape-scan-parity-v0.json
hako_implementation=lang/src/compiler/mirbuilder/program_json_type_alias_declaration_shape_scan.hako
