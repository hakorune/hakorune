...
CARD="$ROOT/docs/development/current/main/phases/phase-296x/3357-MIRBUILDER-RECIPEITEM-COND-RECIPE-OBSERVATION-BOUNDARY-SELECTION-CURRENT-001.md"
BASE_GUARD="$ROOT/tools/checks/rust_lifecycle_mirbuilder_recipeitem_cond_recipe_observation_boundary_selection_guard.sh"
...
token = "MIRBUILDER-RECIPEITEM-COND-RECIPE-OBSERVATION-BOUNDARY-SELECTION-CURRENT-001"
base_token = "MIRBUILDER-RECIPEITEM-COND-RECIPE-OBSERVATION-BOUNDARY-SELECTION-001"
next_card = "MIRBUILDER-RECIPEITEM-COND-RECIPE-DIAGNOSTIC-SUMMARY-OBSERVATION-001"
...
need(base_guard_path.exists(), "base observer selection guard missing")
...
grep -q '^selected_observer=RecipeItemDiagnosticSummaryObserver$' <<<"$BASE_OUT"
grep -q '^selected_next_card=MIRBUILDER-RECIPEITEM-COND-RECIPE-DIAGNOSTIC-SUMMARY-OBSERVATION-001$' <<<"$BASE_OUT"
grep -q '^cond_recipe_observation_boundary_selection=1$' <<<"$BASE_OUT"
grep -q '^selected_diagnostic_summary_observer=1$' <<<"$BASE_OUT"
grep -q '^cond_recipe_deep_observation_implementation=0$' <<<"$BASE_OUT"
grep -q '^verifier_cond_recipe_observer=0$' <<<"$BASE_OUT"
...
cat <<'EOF'
output_contract=rust-lifecycle-mirbuilder-recipeitem-cond-recipe-observation-boundary-selection-current
token=MIRBUILDER-RECIPEITEM-COND-RECIPE-OBSERVATION-BOUNDARY-SELECTION-CURRENT-001
cond_recipe_observation_boundary_selection_current=1
cond_recipe_observation_boundary_selection=1
selected_diagnostic_summary_observer=1
