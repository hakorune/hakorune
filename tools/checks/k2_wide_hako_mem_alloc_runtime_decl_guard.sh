...
TAG="k2-wide-hako-mem-alloc-runtime-decl"
cd "$ROOT_DIR"

echo "[$TAG] running hako_mem_alloc native pointer runtime-decl guard"
...
CARD = ROOT / "docs/development/current/main/phases/phase-293x/293x-052-HAKO-MEM-ALLOC-RUNTIME-DECL.md"
...
def fail(message: str) -> None:
    print(f"[k2-wide-hako-mem-alloc-runtime-decl][fail] {message}", file=sys.stderr)
    raise SystemExit(1)
...
for needle in [
    ...
    if needle not in generated:
        fail(f"{GENERATED}: missing generated hako_mem_alloc token {needle}")
...
for path, needle in [
    ...
    (CARD, "M10c-hako-mem-alloc-row is live as the first active native pointer runtime-decl row."),
    (CURRENT, "293x-052 hako_mem_alloc runtime-decl row landed"),
]:
