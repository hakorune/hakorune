...
TAG="k2-wide-phase295x-malloc-large-nyrt-plugin-loadset-footprint-diagnostic"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-202-MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-DIAGNOSTIC.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-201-MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-SELECTION.md"
...
SELF_SCRIPT="tools/checks/k2_wide_phase295x_malloc_large_nyrt_plugin_loadset_footprint_diagnostic_guard.sh"

echo "[$TAG] checking phase-295x malloc-large NyRT plugin load-set footprint diagnostic"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-DIAGNOSTIC-295X-002' "$CARD" "card must identify current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-CLOSEOUT-295X-002' "$CARD" "card must select the closeout follow-on"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-DIAGNOSTIC-295X-002' "$PREV_CARD" "previous row must select this diagnostic"
guard_expect_in_file "$TAG" '| 201 | `MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-SELECTION-295X-002` | Landed |' "$TASKBOARD" "taskboard must retain the selection row as landed"
guard_expect_in_file "$TAG" '| 202 | `MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-DIAGNOSTIC-295X-002` | Current |' "$TASKBOARD" "taskboard must expose the load-set diagnostic row"
