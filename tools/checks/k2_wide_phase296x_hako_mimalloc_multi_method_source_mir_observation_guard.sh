...
guard_expect_fixed_in_file "$TAG" 'next_keeper=small_alloc_selected_page_return_reuse' "$CARD_88" "row88 must select the next keeper"
...
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "HAKO-MIMALLOC-SMALL-ALLOC-SELECTED-PAGE-RETURN-KEEPER-296X-001"' "$CURRENT_STATE" "current state must select row 89"
...
guard_expect_fixed_in_file "$TAG" '| 89 | `HAKO-MIMALLOC-SMALL-ALLOC-SELECTED-PAGE-RETURN-KEEPER-296X-001` | Current |' "$TASKBOARD" "taskboard row 89 must be current"
...
guard_expect_fixed_in_file "$TAG" 'next_keeper=small_alloc_selected_page_return_reuse' "$report" "tool must select keeper"
guard_expect_fixed_in_file "$TAG" 'next_keeper_kind=box_count' "$report" "tool must keep keeper kind narrow"
guard_expect_fixed_in_file "$TAG" 'next_row=HAKO-MIMALLOC-SMALL-ALLOC-SELECTED-PAGE-RETURN-KEEPER-296X-001' "$report" "tool must select row 89"
