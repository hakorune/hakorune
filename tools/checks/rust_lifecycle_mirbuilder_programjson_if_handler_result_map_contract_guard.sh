...
TAG="rust-lifecycle-mirbuilder-programjson-if-handler-result-map-contract-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/hako-programjson-if-handler-result-map-contract-v0.json"
IF_HANDLER="$ROOT_DIR/lang/src/compiler/mirbuilder/stmt_handlers/if_stmt_handler.hako"
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/3208-HAKO-PROGRAMJSON-IF-HANDLER-RESULT-MAP-CONTRACT-001.md"
TASK_ORDER="$ROOT_DIR/docs/development/current/main/design/mirbuilder-rust-to-hako-converter-task-order-ssot.md"
HAKO_BIN="$ROOT_DIR/tools/bin/hako"

guard_require_command "$TAG" python3
guard_require_files "$TAG" "$FIXTURE" "$IF_HANDLER" "$CARD" "$TASK_ORDER" "$HAKO_BIN"

python3 - "$FIXTURE" "$IF_HANDLER" "$CARD" "$TASK_ORDER" <<'PY'
import json
import re
import sys
...
token = "HAKO-PROGRAMJSON-IF-HANDLER-RESULT-MAP-CONTRACT-001"
next_card = "MIRBUILDER-PROGRAMJSON-RECIPEBODIES-AFTER-LOOP-BODY-ARENA-NEXT-CONTRACT-SELECTION-001"

if fixture.get("kind") != "HakoProgramJsonIfHandlerResultMapContractV1":
...
for key in [
    ...
    "recipe_item_helpers_return_mapbox",
...
for required in [
    ...
    if required not in handler:
        raise SystemExit(f"missing recipe item helper annotation: {required}")

for forbidden in [
    '"" + BoxHelpers.map_get(ret_out, "err_line")',
    '"" + BoxHelpers.map_get(then_out, "err_line")',
    '"" + BoxHelpers.map_get(then_assign_out, "err_line")',
    '"" + BoxHelpers.map_get(else_assign_out, "err_line")',
]:
...
if re.search(r"return\s+null\b", handler):
    raise SystemExit("IfStmtHandler must not return null")

claims = fixture.get("claims") or {}
if claims.get("if_handler_result_map_contract") != 1:
    raise SystemExit("contract claim missing")
for key, value in claims.items():
    if key == "if_handler_result_map_contract":
        continue
    if value != 0:
...
if "3208 stabilizes IfStmtHandler result-map contracts" not in task_order:
    raise SystemExit("task-order 3208 marker missing")
PY

bash "$HAKO_BIN" --backend mir --verify "$IF_HANDLER" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-if-handler-result-map-contract-guard-v0
token=HAKO-PROGRAMJSON-IF-HANDLER-RESULT-MAP-CONTRACT-001
owner=IfStmtHandler
handler_returns_total_result_map=1
helper_result_maps_annotated=1
recipe_item_helpers_return_mapbox=1
