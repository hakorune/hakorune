...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_480A" "$CARD_484A" "$CARD" "$USIZE_SELECTION_CARD" "$USIZE_CARD" "$DESIGN" "$DESIGN_480A" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MODULE_INDEX" "$OWNER" "$PREV_OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
...
guard_expect_in_file "$TAG" 'allocator_comparison_c_mimalloc_result_presentation_extension_pilot_box.hako' "$MODULE_INDEX" "memory module index must name presentation extension pilot owner"
