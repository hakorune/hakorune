...
CARD="docs/development/current/main/phases/phase-296x/296x-787-MIMALLOC-HANDLE-REGISTRY-TYPED-HANDLE-BOUNDARY-ARRAY-RESIDENCE-PRODUCER-INPUT-SOURCE-SURFACE-001.md"
...
SELF_SCRIPT="tools/checks/k2_wide_phase296x_mimalloc_handle_registry_typed_handle_boundary_array_residence_producer_input_source_surface_guard.sh"

[[ -f "$CARD" ]] || { echo "[mimalloc-array-residence-producer-input-source-surface] missing card: $CARD" >&2; exit 1; }
[[ -f "$DESIGN_CARD" ]] || { echo "[mimalloc-array-residence-producer-input-source-surface] missing design card: $DESIGN_CARD" >&2; exit 1; }
[[ -f "$INPUT_INVENTORY_CARD" ]] || { echo "[mimalloc-array-residence-producer-input-source-surface] missing input inventory card: $INPUT_INVENTORY_CARD" >&2; exit 1; }
[[ -f "$ARRAY_REPR" ]] || { echo "[mimalloc-array-residence-producer-input-source-surface] missing ArrayRepr SSOT: $ARRAY_REPR" >&2; exit 1; }
[[ -f "$OBJECT_SSOT" ]] || { echo "[mimalloc-array-residence-producer-input-source-surface] missing ObjectStoragePlan SSOT: $OBJECT_SSOT" >&2; exit 1; }
[[ -f "$DIRECT_PLAN" ]] || { echo "[mimalloc-array-residence-producer-input-source-surface] missing DirectArrayAccessPlan source: $DIRECT_PLAN" >&2; exit 1; }

grep -Eq '^Status: (Active|Landed)$' "$CARD" || {
  echo "[mimalloc-array-residence-producer-input-source-surface] card must be Active or Landed" >&2
  exit 1
}
grep -Eq '^Status: Landed$' "$DESIGN_CARD" || {
  echo "[mimalloc-array-residence-producer-input-source-surface] design card must be Landed" >&2
  exit 1
}
grep -q "$SELF_SCRIPT" "$INDEX" || {
  echo "[mimalloc-array-residence-producer-input-source-surface] check index missing guard entry" >&2
  exit 1
}
...
require_line_in_file() {
  ...
  if ! grep -F -x -q "$expected" "$file"; then
    echo "[mimalloc-array-residence-producer-input-source-surface] missing line in $file: $expected" >&2
    exit 1
  fi
}
...
for expected in \
  ...
  "selected_source_candidate_confidence=high"; do
  require_line_in_file "$CARD" "$expected"
done

require_line_in_file "$DESIGN_CARD" "selected_design=array_receiver_residence_input_source"
require_line_in_file "$DESIGN_CARD" "input_source_must_not_be_direct_array_access_plan_only=1"
require_line_in_file "$INPUT_INVENTORY_CARD" "selected_blocker=missing_array_receiver_representation_input_source"
grep -F -q "DirectArrayAccessPlan" "$DIRECT_PLAN" || {
  echo "[mimalloc-array-residence-producer-input-source-surface] DirectArrayAccessPlan source missing type" >&2
  exit 1
}
grep -F -q "ArrayRepr" "$ARRAY_REPR" || {
  echo "[mimalloc-array-residence-producer-input-source-surface] ArrayRepr SSOT missing ArrayRepr" >&2
  exit 1
}
...
grep -F -q "do not implement the input source from this row" "$CARD" || {
  echo "[mimalloc-array-residence-producer-input-source-surface] missing input-source stop line" >&2
  exit 1
}
grep -F -q "backend direct handle bypass" "$CARD" || {
  echo "[mimalloc-array-residence-producer-input-source-surface] missing backend bypass stop line" >&2
  exit 1
}

echo "[mimalloc-array-residence-producer-input-source-surface] ok"
