...
guard_expect_in_file "$TAG" 'makeSchedulerReport\(fields: HakoAllocBoundedPurgeDecommitSchedulerReportFields\)' "$OWNER" "MIMAP-041A must expose the same-owner ReportFields helper"
guard_expect_in_file "$TAG" 'scheduler_report.status = fields.status' "$OWNER" "MIMAP-041A must materialize report box from record field reads"
guard_expect_in_file "$TAG" 'me.makeSchedulerReport\(fields\)' "$OWNER" "MIMAP-041A run path must use the ReportFields helper"
