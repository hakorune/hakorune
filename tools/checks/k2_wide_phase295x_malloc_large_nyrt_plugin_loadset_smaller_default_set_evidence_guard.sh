...
TAG="k2-wide-phase295x-malloc-large-nyrt-plugin-loadset-smaller-default-set-evidence"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-204-MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-SMALLER-DEFAULT-SET-EVIDENCE.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-203-MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-CLOSEOUT.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
RUNNER="tools/allocator/mimalloc_repeated_measurement_runner.py"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_malloc_large_nyrt_plugin_loadset_smaller_default_set_evidence_guard.sh"

echo "[$TAG] checking phase-295x smaller-default load-set evidence"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-SMALLER-DEFAULT-SET-EVIDENCE-295X-002' "$CARD" "card must identify the current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-SMALLER-DEFAULT-SET-CLOSEOUT-295X-002' "$CARD" "card must select the closeout row"
guard_expect_in_file "$TAG" 'representative-small-block-v0' "$CARD" "card must keep the repeated comparison evidence"
guard_expect_in_file "$TAG" '3,670,016' "$CARD" "card must record the empty-default median"
guard_expect_in_file "$TAG" '9,609,216' "$CARD" "card must record the root median"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-SMALLER-DEFAULT-SET-EVIDENCE-295X-002' "$TASKBOARD" "taskboard must expose the evidence row"
guard_expect_in_file "$TAG" '| 203 | `MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-CLOSEOUT-295X-002` | Landed |' "$TASKBOARD" "taskboard must mark the closeout row landed"
guard_expect_in_file "$TAG" '| 204 | `MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-SMALLER-DEFAULT-SET-EVIDENCE-295X-002` | Current |' "$TASKBOARD" "taskboard must expose the evidence row as current"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list this guard"
guard_expect_in_file "$TAG" 'default: empty' "$RUNNER" "runner must default to the empty hako runtime config"
guard_expect_in_file "$TAG" 'hako_runtime_config_default=empty' "$RUNNER" "runner must emit the default runtime config"

tmp_dir="$(mktemp -d /tmp/hakorune_phase295x_nyrt_loadset_smaller_default.XXXXXX)"
...
print("[phase295x-nyrt-plugin-loadset-smaller-default-set-evidence] ok")
PY
