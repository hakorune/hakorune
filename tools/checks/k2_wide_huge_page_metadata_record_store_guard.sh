...
guard_expect_in_file "$TAG" 'return me\.meta_store\.pageIdFor\(ptr\)' "$MODEL" "M180 owner must read page id through C205d store"
guard_expect_in_file "$TAG" 'return me\.meta_store\.requestedSizeFor\(ptr\)' "$MODEL" "M180 owner must read requested size through C205d store"
guard_expect_in_file "$TAG" 'return me\.meta_store\.committedSizeFor\(ptr\)' "$MODEL" "M180 owner must read committed size through C205d store"
guard_expect_in_file "$TAG" 'me\.meta_store\.markReleased\(ptr\)' "$MODEL" "M180 owner must release through C205d store"
