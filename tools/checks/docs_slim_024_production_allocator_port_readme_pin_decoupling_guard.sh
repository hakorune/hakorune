...
guard_expect_in_file "$TAG" "DOCS-SLIM-024" "$CARD" "DOCS-SLIM-024 card must exist"
guard_expect_in_file "$TAG" "Do not move numbered cards in this row" "$CARD" "card must keep no-move stop-line"
guard_expect_in_file "$TAG" "Twenty-fourth Slimming Phase" "$ARCHIVE_POLICY" "archive policy must record DOCS-SLIM-024"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$CHECK_INDEX" "check index must list DOCS-SLIM-024 guard"
