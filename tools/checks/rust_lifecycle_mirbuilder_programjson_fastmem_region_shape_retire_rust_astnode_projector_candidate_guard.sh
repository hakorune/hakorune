...
TAG="rust-lifecycle-mirbuilder-programjson-fastmem-region-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-fastmem-region-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonFastMemRegionShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "FastMemRegionShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON FastMemRegion statement body rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonFastMemRegionShapeScanV1", "bad owner")
...
need(criteria.get("fastmem_lowering") == 0, "fastmem lowering must remain unclaimed")
need(criteria.get("contract_execution") == 0, "contract execution must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_fastmem_region_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-fastmem-region-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-FASTMEM-REGION-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=FastMemRegionShapeSnapshotV1
shape_scope=covered ProgramJSON FastMemRegion statement body rows
covered_rows=10
