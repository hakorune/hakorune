...
TAG="rust-lifecycle-mirbuilder-programjson-exit-allowed-block-recipe-else-only-exit-snapshot-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-exit-allowed-block-recipe-else-only-exit-snapshot-parity-v0.json"
SNAPSHOT_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_exit_allowed_block_recipe_snapshot.hako"
IF_HANDLER="$ROOT_DIR/lang/src/compiler/mirbuilder/stmt_handlers/if_stmt_handler.hako"
PREREQ_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_block_recipe_after_join_then_else_next_contract_selection_guard.sh"
HAKO_BIN="$ROOT_DIR/tools/bin/hako"

guard_require_command "$TAG" python3
guard_require_command "$TAG" timeout
guard_require_files "$TAG" "$FIXTURE" "$SNAPSHOT_IMPL" "$IF_HANDLER" "$PREREQ_GUARD" "$HAKO_BIN"

PREREQ_OUT="$(guard_cached_run "$TAG" bash "$PREREQ_GUARD")"
if ! grep -q '^summary=ok$' <<<"$PREREQ_OUT"; then
  printf '%s\n' "$PREREQ_OUT" >&2
  guard_fail "$TAG" "ElseOnlyExit selection prerequisite is not green"
fi

TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-exit-allowed-else-only-exit.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_exit_allowed_else_only_exit_probe.hako"
MIR_JSON="$TMP_DIR/programjson_exit_allowed_else_only_exit_probe.mir.json"
EXE="$TMP_DIR/programjson_exit_allowed_else_only_exit_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonExitAllowedBlockRecipeElseOnlyExitSnapshotParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-EXIT-ALLOWED-BLOCK-RECIPE-ELSE-ONLY-EXIT-SNAPSHOT-PARITY-001":
...
if len(rows) != 1:
    raise SystemExit("ElseOnlyExit ProgramJSON parity requires exactly 1 row")

row = rows[0]
program_json = json.dumps(row["program_json"], separators=(",", ":"), ensure_ascii=False)
source = "\n".join([
    "using lang.compiler.mirbuilder.program_json_exit_allowed_block_recipe_snapshot as ProgramJsonExitAllowedBlockRecipeSnapshotBox",
    "",
    "static box Main {",
    "  main() {",
    "    print(\"dto:" + row["row_id"] + ":\" + ProgramJsonExitAllowedBlockRecipeSnapshotBox.build_summary(" + json.dumps(program_json) + "))",
    "    return 0",
    "  }",
    "}",
    "",
...
expected.write_text("dto:" + row["row_id"] + ":" + row["rust_oracle_expected_summary"] + "\n", encoding="utf-8")
PY

python3 - "$SNAPSHOT_IMPL" "$IF_HANDLER" <<'PY'
import sys
from pathlib import Path
...
for needle in [
    ...
    if needle not in handler:
        raise SystemExit(f"missing IfStmtHandler producer token: {needle}")
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for ProgramJSON ExitAllowed ElseOnlyExit probe"
fi
...
snapshot = next((fn for fn in functions if fn.get("name") == "ProgramJsonExitAllowedBlockRecipeSnapshotBox.build_summary/1"), None)
if main is None:
...
main_routes = [row for row in route_rows(main) if row.get("symbol") == "ProgramJsonExitAllowedBlockRecipeSnapshotBox.build_summary/1"]
...
for symbol in [
    ...
    "ExitAllowedBlockRecipeBox.build_summary/2",
]:
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-exit-allowed-block-recipe-else-only-exit-snapshot-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-EXIT-ALLOWED-BLOCK-RECIPE-ELSE-ONLY-EXIT-SNAPSHOT-PARITY-001
owner=ProgramJsonExitAllowedBlockRecipeElseOnlyExitSnapshotV1
row_count=1
programjson_traversal_used=1
recipe_root_traversal_used=1
if_stmt_handler_producer_extended=1
