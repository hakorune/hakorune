...
TAG="rust-lifecycle-mirbuilder-programjson-enum-ctor-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-enum-ctor-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonEnumCtorShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "EnumCtorShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON EnumCtor expression rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonEnumCtorShapeScanV1", "bad owner")
...
need(criteria.get("enum_lowering") == 0, "enum lowering must remain unclaimed")
need(criteria.get("payload_abi_materialization") == 0, "payload ABI materialization must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_enum_ctor_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-enum-ctor-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-ENUM-CTOR-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=EnumCtorShapeSnapshotV1
shape_scope=covered ProgramJSON EnumCtor expression rows
covered_rows=10
