...
if not fixture.get("loop_cond_bc_cleanup_subcluster_rules"):
    raise SystemExit("loop-cond break/continue cleanup subcluster rules missing")
if not fixture.get("loop_cond_bc_item_lowering_subcluster_rules"):
...
if not fixture.get("loop_cond_co_continue_if_subcluster_rules"):
...
if not fixture.get("loop_cond_co_statement_lowering_subcluster_rules"):
    raise SystemExit("loop-cond continue-only statement-lowering subcluster rules missing")
...
loop_cond_bc_cleanup_summary = fixture.get("loop_cond_bc_cleanup_subcluster_summary") or []
if not loop_cond_bc_cleanup_summary:
    raise SystemExit("loop-cond break/continue cleanup subcluster summary missing")
loop_cond_bc_item_lowering_summary = fixture.get("loop_cond_bc_item_lowering_subcluster_summary") or []
if not loop_cond_bc_item_lowering_summary:
    raise SystemExit("loop-cond break/continue item-lowering subcluster summary missing")
...
loop_cond_co_continue_if_summary = fixture.get("loop_cond_co_continue_if_subcluster_summary") or []
if not loop_cond_co_continue_if_summary:
    raise SystemExit("loop-cond continue-only continue-if subcluster summary missing")
...
loop_cond_co_statement_lowering_summary = fixture.get("loop_cond_co_statement_lowering_subcluster_summary") or []
if not loop_cond_co_statement_lowering_summary:
    raise SystemExit("loop-cond continue-only statement-lowering subcluster summary missing")
...
loop_cond_bc_cleanup_items = [
    item for item in loop_cond_bc_items
    if item.get("loop_cond_bc_subcluster") == "LoopCondBcCleanupCluster"
]
for item in loop_cond_bc_cleanup_items:
    if not item.get("loop_cond_bc_cleanup_subcluster"):
        raise SystemExit(f"LoopCondBcCleanupCluster item lacks subcluster: {item.get('source_id')}")
loop_cond_bc_cleanup_sum = sum(item.get("count", 0) for item in loop_cond_bc_cleanup_summary)
if loop_cond_bc_cleanup_sum != len(loop_cond_bc_cleanup_items):
    raise SystemExit("loop-cond break/continue cleanup subcluster summary count drift")
loop_cond_bc_item_lowering_items = [
    item for item in loop_cond_bc_items
    if item.get("loop_cond_bc_subcluster") == "LoopCondBcItemLoweringCluster"
]
for item in loop_cond_bc_item_lowering_items:
    if not item.get("loop_cond_bc_item_lowering_subcluster"):
        raise SystemExit(f"LoopCondBcItemLoweringCluster item lacks subcluster: {item.get('source_id')}")
loop_cond_bc_item_lowering_sum = sum(item.get("count", 0) for item in loop_cond_bc_item_lowering_summary)
if loop_cond_bc_item_lowering_sum != len(loop_cond_bc_item_lowering_items):
    raise SystemExit("loop-cond break/continue item-lowering subcluster summary count drift")
loop_cond_bc_pipeline_items = [
    item for item in loop_cond_bc_items
    if item.get("loop_cond_bc_subcluster") == "LoopCondBcPipelineCluster"
]
for item in loop_cond_bc_pipeline_items:
    if not item.get("loop_cond_bc_pipeline_subcluster"):
        raise SystemExit(f"LoopCondBcPipelineCluster item lacks subcluster: {item.get('source_id')}")
...
loop_cond_co_continue_if_items = [
    item for item in loop_cond_co_items
    if item.get("loop_cond_co_subcluster") == "LoopCondCoContinueIfCluster"
]
for item in loop_cond_co_continue_if_items:
    if not item.get("loop_cond_co_continue_if_subcluster"):
        raise SystemExit(f"LoopCondCoContinueIfCluster item lacks subcluster: {item.get('source_id')}")
loop_cond_co_continue_if_sum = sum(item.get("count", 0) for item in loop_cond_co_continue_if_summary)
if loop_cond_co_continue_if_sum != len(loop_cond_co_continue_if_items):
    raise SystemExit("loop-cond continue-only continue-if subcluster summary count drift")
loop_cond_co_group_if_items = [
    item for item in loop_cond_co_items
    if item.get("loop_cond_co_subcluster") == "LoopCondCoGroupIfCluster"
]
for item in loop_cond_co_group_if_items:
    if not item.get("loop_cond_co_group_if_subcluster"):
        raise SystemExit(f"LoopCondCoGroupIfCluster item lacks subcluster: {item.get('source_id')}")
...
loop_cond_co_statement_lowering_items = [
    item for item in loop_cond_co_items
    if item.get("loop_cond_co_subcluster") == "LoopCondCoStatementLoweringCluster"
]
for item in loop_cond_co_statement_lowering_items:
    if not item.get("loop_cond_co_statement_lowering_subcluster"):
        raise SystemExit(f"LoopCondCoStatementLoweringCluster item lacks subcluster: {item.get('source_id')}")
loop_cond_co_statement_lowering_sum = sum(item.get("count", 0) for item in loop_cond_co_statement_lowering_summary)
if loop_cond_co_statement_lowering_sum != len(loop_cond_co_statement_lowering_items):
    raise SystemExit("loop-cond continue-only statement-lowering subcluster summary count drift")
...
for key in [
    ...
    "loop_cond_co_statement_lowering_items_subclustered",
