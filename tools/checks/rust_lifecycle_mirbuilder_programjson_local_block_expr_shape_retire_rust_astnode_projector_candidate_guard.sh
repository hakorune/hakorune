...
TAG="rust-lifecycle-mirbuilder-programjson-local-block-expr-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-local-block-expr-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonLocalBlockExprShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "LocalBlockExprShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Local.expr BlockExpr rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonLocalBlockExprShapeScanV1", "bad owner")
...
need(criteria.get("block_expr_lowering") == 0, "block expr lowering must remain unclaimed")
need(criteria.get("prelude_execution_semantics") == 0, "prelude execution semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_local_block_expr_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-local-block-expr-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-LOCAL-BLOCK-EXPR-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=LocalBlockExprShapeSnapshotV1
shape_scope=covered ProgramJSON Local.expr BlockExpr rows
covered_rows=10
