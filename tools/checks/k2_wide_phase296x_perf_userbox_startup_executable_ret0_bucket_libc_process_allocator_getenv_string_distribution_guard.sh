...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_libc_process_allocator_getenv_string_distribution.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-libc-process-allocator-getenv-string-distribution] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
if bucket_counts["allocator"] < 1:
    raise SystemExit(1)
if bucket_counts["getenv"] < 1:
    raise SystemExit(1)
if bucket_counts["libc_string"] < 1:
    raise SystemExit(1)

print("output_contract=perf-userbox-startup-executable-ret0-bucket-libc-process-allocator-getenv-string-distribution-v0")
...
print(f"libc_process_bucket_getenv_count={bucket_counts['getenv']}")
print(f"libc_process_bucket_libc_string_count={bucket_counts['libc_string']}")
print(f"libc_process_bucket_other_count={bucket_counts['other']}")
print("libc_process_allocator_getenv_string_distribution_present=1")
