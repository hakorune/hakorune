...
guard_expect_fixed_in_file "$TAG" 'Status: Landed' "$CARD" "card must be landed after the baseline selection row is exercised"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-NYRT-PLUGIN-HOST-BASELINE-SELECTION-295X-002' "$CARD" "card must identify the baseline selection blocker"
...
guard_expect_in_file "$TAG" '| 224 | `MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-NYRT-PLUGIN-HOST-BASELINE-SELECTION-295X-002` | Landed |' "$TASKBOARD" "taskboard must retain the plugin-host baseline row as landed"
