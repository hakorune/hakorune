...
need(all(row.get("exists") is False for row in compare), "compare bridge files should be deleted")
need(all(row.get("builder_mod_declared") is False for row in compare), "compare bridge mod drift")
need(all(row.get("production_fastpath_reference_count") == 0 for row in compare), "compare bridge fastpath drift")
need(sum(row.get("line_count", 0) for row in compare) == 0, "compare bridge line total drift")
...
print("compare_proof_bridge_total_lines=0")
