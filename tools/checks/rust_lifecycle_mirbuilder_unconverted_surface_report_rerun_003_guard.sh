...
TAG="rust-lifecycle-mirbuilder-unconverted-surface-report-rerun-003"
...
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/2002-MIRBUILDER-UNCONVERTED-SURFACE-REPORT-RERUN-003.md"
...
previous_report = "MIRBUILDER-UNCONVERTED-SURFACE-REPORT-RERUN-002"
expected_delta = [
...
require(provenance.get("native_owner_adoption_ledger_hash") == native_owner_adoption_ledger_hash(manifest), "native owner adoption ledger hash is stale")
...
require(adoption_delta_after(manifest, previous_report) == expected_delta, "native owner adoption delta drift")
...
require(
    state.get("latest_card_path") == str(card_path.relative_to(Path.cwd())),
...
require(token in manifest_tokens, "family guard manifest missing rerun-003")
...
print("output_contract=rust-lifecycle-mirbuilder-unconverted-surface-report-rerun-003")
