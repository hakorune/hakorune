...
TAG="rust-lifecycle-mirbuilder-programjson-expr-me-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-expr-me-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonExprMeShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ExprMeShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Expr.expr Me rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonExprMeShapeScanV1", "bad owner")
...
need(criteria.get("receiver_origin_resolution") == 0, "receiver-origin resolution must remain 0")
need(criteria.get("me_lowering") == 0, "me lowering must remain 0")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_expr_me_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-expr-me-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-EXPR-ME-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ExprMeShapeSnapshotV1
shape_scope=covered ProgramJSON Expr.expr Me rows
covered_rows=8
