...
TAG="rust-lifecycle-mirbuilder-programjson-assignment-value-shape-scan-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-assignment-value-shape-scan-parity-v0.json"
SCAN_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_assignment_value_shape_scan.hako"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-assignment-value-shape-scan.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_assignment_value_shape_scan_parity.hako"
EXPECTED="$TMP_DIR/expected.json"
RUN_LOG="$TMP_DIR/run.log"
EXE="$TMP_DIR/programjson_assignment_value_shape_scan_parity.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonAssignmentValueShapeScanParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-ASSIGNMENT-VALUE-SHAPE-SCAN-CAPABILITY-001":
    raise SystemExit("bad fixture token")

rows = fixture.get("rows") or []
if len(rows) < 9:
    raise SystemExit("assignment-value capability must cover at least 9 rows")
...
for row in rows:
    body = row.get("program_json", {}).get("body") or []
    if body and body[0].get("type") == "Assign":
        raise SystemExit("legacy Assign rows are forbidden for this capability")
...
for row in rows:
    ...
    calls.append(
        ...
        + " + ProgramJsonAssignmentValueShapeScanBox.build_summary("
...
source = "\n".join(
    [
        "using lang.compiler.mirbuilder.program_json_assignment_value_shape_scan as ProgramJsonAssignmentValueShapeScanBox",
        "",
        "static box Main {",
        "  main() {",
        *calls,
...
if ! bash "$HAKO_BIN" --backend mir --emit-exe "$EXE" "$APP" >"$EMIT_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit ProgramJSON assignment-value shape scan parity executable"
fi

if ! "$EXE" >"$RUN_LOG" 2>&1; then
  tail -n 160 "$RUN_LOG" || true
  guard_fail "$TAG" "failed to run ProgramJSON assignment-value shape scan parity executable"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-assignment-value-shape-scan-parity-gate-v0
owner=ProgramJsonAssignmentValueShapeScanV1
input_contract=ProgramJSON-v0
output_contract=AssignmentValueShapeSnapshotV1
fixture=mirbuilder-programjson-assignment-value-shape-scan-parity-v0.json
hako_implementation=lang/src/compiler/mirbuilder/program_json_assignment_value_shape_scan.hako
