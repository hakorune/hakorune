...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_510A" "$CARD_514A" "$CARD" "$DESIGN" "$DESIGN_510A" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MEMORY_README" "$OWNER" "$PREV_OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
...
guard_expect_in_file "$TAG" 'allocator_comparison_c_mimalloc_result_presentation_extension_follow_on_extension_follow_on_extension_follow_on_pilot_box.hako' "$MEMORY_README" "memory README must name presentation extension follow-on extension follow-on extension follow-on pilot owner"
