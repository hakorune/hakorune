...
TAG="rust-lifecycle-mirbuilder-programjson-enum-match-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-enum-match-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonEnumMatchShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "EnumMatchShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON EnumMatch rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonEnumMatchShapeScanV1", "bad owner")
...
need(criteria.get("match_lowering_semantics") == 0, "match lowering must remain unclaimed")
need(criteria.get("enum_runtime_semantics") == 0, "enum runtime semantics must remain unclaimed")
...
for key in [
    ...
    "match_lowering_semantics",
    "enum_runtime_semantics",
    "runtime_fallback",
    "new_backend_route",
    "new_abi",
]:
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_enum_match_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-enum-match-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-ENUM-MATCH-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=EnumMatchShapeSnapshotV1
shape_scope=covered ProgramJSON EnumMatch rows
covered_rows=9
