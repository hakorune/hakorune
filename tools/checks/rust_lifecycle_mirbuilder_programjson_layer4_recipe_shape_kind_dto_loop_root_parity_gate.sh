...
TAG="rust-lifecycle-mirbuilder-programjson-layer4-recipe-shape-kind-dto-loop-root-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-layer4-recipe-shape-kind-dto-loop-root-parity-v0.json"
...
SELECTION_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_next_recipe_dto_capability_selection_guard.sh"
EXPANDED_SHAPE_KIND_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_recipe_shape_kind_dto_expanded_parity_gate.sh"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-layer4-recipe-shape-kind-loop-root.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/layer4_recipe_shape_kind_loop_root_probe.hako"
MIR_JSON="$TMP_DIR/layer4_recipe_shape_kind_loop_root_probe.mir.json"
EXE="$TMP_DIR/layer4_recipe_shape_kind_loop_root_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonLayer4RecipeShapeKindDtoLoopRootParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-LAYER4-RECIPE-SHAPE-KIND-DTO-LOOP-ROOT-PARITY-001":
...
for key, value in {
    ...
    "expanded_shape_kind_prerequisite_green": 1,
}.items():
...
if len(covered) != 6:
    raise SystemExit("loop-root Recipe shape-kind DTO parity requires exactly 6 covered rows")
expected_summary = source_rows.get("expected_summary")
if expected_summary != "snapshot_kind=RecipeShapeKindDtoSnapshotV1;err=0;matched=1;shape_kind=phase21_local_loop_if_varltint_then_return_int_body_inc_return_var_or_int;has_print=0;has_local=1;has_assignment=1;has_return=1":
...
for row_id in covered:
    ...
    rust_summary = row.get("rust_oracle_expected_summary") or ""
    if "shape_kind=phase21_local_loop_if_varltint_then_return_int_body_inc_return_var_or_int" not in rust_summary:
        raise SystemExit(f"source row lacks phase21 oracle: {row_id}")
...
for needle in [
    ...
    if needle not in snapshot:
        raise SystemExit(f"missing loop-root shape-kind token: {needle}")
for needle in [
    "_recipe_child_token(child)",
    "RecipeItemBox.kind_is(child, \"Loop\")",
    "RecipeItemBox.kind_is(child, \"If\")",
    "RecipeItemBox.kind_is(child, \"Exit\")",
]:
    if needle not in shape_seq:
        raise SystemExit(f"missing control-aware seq token: {needle}")
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for Recipe shape-kind loop-root probe"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-layer4-recipe-shape-kind-dto-loop-root-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-LAYER4-RECIPE-SHAPE-KIND-DTO-LOOP-ROOT-PARITY-001
