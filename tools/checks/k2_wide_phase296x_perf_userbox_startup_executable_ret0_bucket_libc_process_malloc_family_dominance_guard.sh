...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_libc_process_malloc_family_dominance.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-libc-process-malloc-family-dominance] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
malloc_family_count = 0
getenv_family_count = 0
malloc_related_symbol_counts = Counter()
...
for path in paths:
    ...
    while True:
        ...
        if family == "libc_process":
            ...
            if "malloc" in symbol.lower():
                malloc_family_count += 1
                malloc_related_symbol_counts[symbol] += 1
            if "getenv" in symbol.lower():
                getenv_family_count += 1
        idx += 1
...
if malloc_family_count <= getenv_family_count:
    raise SystemExit(1)
if dominant_symbol != "_int_malloc":
    raise SystemExit(1)

print("output_contract=perf-userbox-startup-executable-ret0-bucket-libc-process-malloc-family-dominance-v0")
...
print(f"libc_process_malloc_family_count={malloc_family_count}")
print(f"libc_process_getenv_family_count={getenv_family_count}")
print(f"libc_process_malloc_related_symbol_count={sum(malloc_related_symbol_counts.values())}")
print(f"libc_process_malloc_related_symbols={','.join(sorted(malloc_related_symbol_counts))}")
