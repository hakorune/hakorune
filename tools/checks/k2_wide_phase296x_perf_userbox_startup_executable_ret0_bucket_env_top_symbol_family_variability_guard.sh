...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_env_top_symbol_family_variability.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-env-top-symbol-family-variability] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-env-top-symbol-family-variability] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  env_count="$(awk -F= '$1=="startup_loader_ret0_exe_bucket_env_count" { print $2; exit }' "$trial_out")"
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-env-top-symbol-family-variability] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
...
read -r primary_bucket_env_count primary_bucket_once_count primary_bucket_path_count primary_bucket_minimal_main_count primary_bucket_nyash_kernel_runtime_count primary_bucket_kernel_count primary_bucket_other_count primary_bucket_missing_count primary_bucket_alloc_count primary_bucket_ffi_count primary_bucket_string_count ret0_count_min ret0_count_max trial_summaries bucket_env_present_count bucket_env_exact_symbol_mode bucket_env_exact_symbol_mode_count bucket_env_family_mode bucket_env_family_mode_count bucket_env_family_var_os_count bucket_env_family_getenv_count bucket_env_family_current_exe_count bucket_env_family_current_dir_count bucket_env_family_other_count bucket_env_family_missing_count < <(
...
env_exact_symbol_counts = Counter()
env_family_counts = Counter()
env_present_count = 0
ret0_min = None
ret0_max = None
summaries = []
...
def classify_env_family(symbol: str) -> str:
    text = symbol.lower()
    if "getenv" in text:
        return "getenv"
    if "var_os" in text:
...
for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
    ...
    if len(parts) != 4:
        continue
    primary, top_count_s, env_count_s, env_symbol = parts
    ...
    try:
        env_count = int(env_count_s)
    except ValueError:
        env_count = 0
    if env_count > 0 and env_symbol != "missing":
        env_present_count += 1
        env_exact_symbol_counts[env_symbol] += 1
        env_family_counts[classify_env_family(env_symbol)] += 1

exact_mode, exact_mode_count = mode(env_exact_symbol_counts)
family_mode, family_mode_count = mode(env_family_counts)
...
print(
    ...
    family_mode,
    family_mode_count,
    env_family_counts["var_os"],
    env_family_counts["getenv"],
    env_family_counts["current_exe"],
    env_family_counts["current_dir"],
    env_family_counts["other"],
    env_family_counts["missing"],
)
PY
)

cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-env-top-symbol-family-variability-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_env_top_symbol_family_variability
...
bucket_env_exact_symbol_mode=$bucket_env_exact_symbol_mode
bucket_env_exact_symbol_mode_count=$bucket_env_exact_symbol_mode_count
bucket_env_family_mode=$bucket_env_family_mode
bucket_env_family_mode_count=$bucket_env_family_mode_count
bucket_env_family_var_os_count=$bucket_env_family_var_os_count
bucket_env_family_getenv_count=$bucket_env_family_getenv_count
bucket_env_family_current_exe_count=$bucket_env_family_current_exe_count
bucket_env_family_current_dir_count=$bucket_env_family_current_dir_count
bucket_env_family_other_count=$bucket_env_family_other_count
bucket_env_family_missing_count=$bucket_env_family_missing_count
