...
TAG="rust-lifecycle-mirbuilder-control-form-is-if-classifier-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-control-form-is-if-classifier-rust-oracle-v0.json"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-control-form-is-if-classifier.XXXXXX)"
cleanup() {
  rm -rf "$TMP_DIR" >/dev/null 2>&1 || true
}
trap cleanup EXIT

APP="$TMP_DIR/control_form_is_if_classifier_parity.hako"
EXPECTED="$TMP_DIR/expected.txt"
ACTUAL="$TMP_DIR/actual.txt"
EXE="$TMP_DIR/control_form_is_if_classifier_parity.exe"
...
if actual != expected:
    print("[control-form-is-if-classifier/parity] mismatch")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-control-form-is-if-classifier-parity-gate-v0
owner=control_form_is_if_classifier
rust_oracle_fixture=mirbuilder-control-form-is-if-classifier-rust-oracle-v0.json
