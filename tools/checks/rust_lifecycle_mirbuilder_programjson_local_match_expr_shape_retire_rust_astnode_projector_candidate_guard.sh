...
TAG="rust-lifecycle-mirbuilder-programjson-local-match-expr-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-local-match-expr-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonLocalMatchExprShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "LocalMatchExprShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Local.expr Match rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonLocalMatchExprShapeScanV1", "bad owner")
...
need(criteria.get("match_lowering") == 0, "match lowering must remain unclaimed")
need(criteria.get("branch_execution_semantics") == 0, "branch execution semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_local_match_expr_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-local-match-expr-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-LOCAL-MATCH-EXPR-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=LocalMatchExprShapeSnapshotV1
shape_scope=covered ProgramJSON Local.expr Match rows
covered_rows=10
