...
TAG="rust-lifecycle-mirbuilder-programjson-looprange-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-looprange-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonLoopRangeShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "LoopRangeShapeSnapshotV1", "bad retire candidate")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonLoopRangeShapeScanV1", "bad owner")
...
need(criteria.get("looprange_lowering") == 0, "LoopRange lowering must remain unclaimed")
need(criteria.get("iterator_runtime_semantics") == 0, "iterator runtime semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_looprange_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-looprange-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-LOOPRANGE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=LoopRangeShapeSnapshotV1
shape_scope=covered ProgramJSON LoopRange statement rows
covered_rows=9
