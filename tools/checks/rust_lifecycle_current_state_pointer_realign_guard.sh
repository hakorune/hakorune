...
guard_require_files "$TAG" "$STATE" "$TASK_ORDER" "$ROADMAP" "$CARD" "$OLD_CARD" "$INDEX" "$0"

guard_expect_fixed_in_file "$TAG" 'latest_card = "CURRENT-STATE-POINTER-REALIGN-001"' "$STATE" "CURRENT_STATE latest_card must point at pointer realign"
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "CURRENT-STATE-POINTER-REALIGN-001"' "$STATE" "CURRENT_STATE current blocker must point at pointer realign"
guard_expect_fixed_in_file "$TAG" '1773-CURRENT-STATE-POINTER-REALIGN-001.md' "$STATE" "CURRENT_STATE latest_card_path must point at 1773"
...
guard_expect_fixed_in_file "$TAG" 'CURRENT-STATE-POINTER-REALIGN-001' "$TASK_ORDER" "task-order must select pointer realign"
