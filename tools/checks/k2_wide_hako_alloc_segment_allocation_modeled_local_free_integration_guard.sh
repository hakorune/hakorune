...
guard_expect_in_file "$TAG" 'makeLocalFreeIntegrationReport\(fields: HakoAllocSegmentAllocationModeledLocalFreeIntegrationReportFields\)' "$OWNER" "integration owner must expose the same-owner ReportFields helper"
guard_expect_in_file "$TAG" 'local_free_integration_report.did_integrate_local_free = fields.did_integrate_local_free' "$OWNER" "integration owner must copy record fields into the public report box"
guard_expect_in_file "$TAG" 'me.makeLocalFreeIntegrationReport\(fields\)' "$OWNER" "integration owner construction paths must use the ReportFields helper"
...
guard_expect_in_file "$TAG" 'local plan_result: Result<i64, i64>' "$OWNER" "integration owner must keep the apply-plan Result boundary local to integrateLocalFree"
guard_expect_in_file "$TAG" 'guard let Result::Ok\(apply_plan_row_index\) = plan_result else' "$OWNER" "integration owner must consume the apply-plan Result boundary through guard-let"
guard_expect_in_file "$TAG" 'local page_apply_result: Result<i64, i64>' "$OWNER" "integration owner must keep the page-apply Result boundary local to integrateLocalFree"
guard_expect_in_file "$TAG" 'guard let Result::Ok\(page_apply_row_index\) = page_apply_result else' "$OWNER" "integration owner must consume the page-apply Result boundary through guard-let"
