...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_84" "diff card must be current"
...
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-83-HAKO-CHECK-PERF-SURFACE-V1-MINIMAL"' "$CURRENT_STATE" "current state latest card must advance to row 83"
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "HAKO-MIMALLOC-KEEPER-BEFORE-AFTER-DIFF-ADAPTER-296X-001"' "$CURRENT_STATE" "current state must select row 84"
...
guard_expect_fixed_in_file "$TAG" '| 84 | `HAKO-MIMALLOC-KEEPER-BEFORE-AFTER-DIFF-ADAPTER-296X-001` | Current |' "$TASKBOARD" "taskboard row 84 must be current"
