...
TAG="k2-wide-hako-alloc-segment-allocation-modeled-local-free-reuse-ledger-release"
cd "$ROOT_DIR"
...
CARD="docs/development/current/main/phases/phase-293x/293x-640-MIMAP-134A-SEGMENT-ALLOCATION-MODELED-LOCAL-FREE-REUSE-LEDGER-RELEASE-ROUTE.md"
...
SELF_SCRIPT="tools/checks/k2_wide_hako_alloc_segment_allocation_modeled_local_free_reuse_ledger_release_guard.sh"

printf '[%s] checking MIMAP-134A segment allocation modeled local-free reuse ledger release route\n' "$TAG"
...
guard_expect_in_file "$TAG" 'Status: landed' "$CARD" "MIMAP-134A card must be landed"
guard_expect_in_file "$TAG" 'MIMAP-134A' "$PLAN" "granularity SSOT must describe MIMAP-134A"
guard_expect_in_file "$TAG" 'MIMAP-134A segment allocation modeled local-free reuse ledger release route' "$JOINT" "joint order must name MIMAP-134A"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list MIMAP-134A guard"
guard_expect_in_file "$TAG" 'id = "MIMAP-134A"' "$PROOF_MANIFEST" "proof app manifest must list MIMAP-134A"
guard_expect_in_file "$TAG" 'memory.segment_allocation_modeled_local_free_reuse_ledger_release_box = "memory/segment_allocation_modeled_local_free_reuse_ledger_release_box.hako"' "$MODULE" "hako module must export reuse ledger release owner"
guard_expect_in_file "$TAG" 'segment_allocation_modeled_local_free_reuse_ledger_release_box.hako` owns' "$MEMORY_README" "memory README must define reuse ledger release owner"
guard_expect_in_file "$TAG" 'recordReuseLedgerRelease' "$OWNER" "reuse ledger release owner must expose release route"
guard_expect_in_file "$TAG" 'local_free_reuse_ledger_release_present' "$OWNER" "release report must expose presence flag"
guard_expect_in_file "$TAG" 'check "mimap134a segment allocation modeled local-free reuse ledger release route"' "$APP" "MIMAP-134A proof must use labelled check block"
...
if rg -n 'segment_allocation_modeled_ledger_box|recordModeledConsume|releaseModeledToken' "$OWNER" >/tmp/"$TAG".bump_ledger_leak 2>&1; then
  echo "[$TAG] ERROR: MIMAP-134A owner must not widen or depend on the bump-shaped modeled ledger" >&2
...
if rg -n 'AtomicCoreBox|hako_atomic|cas_i64|fetch_add|spawn[[:space:]]*\(|thread::|worker_local|ChannelBox|TaskGroupBox|nowait|sync[[:space:]]+box|context[[:space:]]|wake|sleep|runQueue|run_queue|SegmentMap|lookupSegment|pointer_member|claimBitmap|unclaimBitmap|observeHeapPage[[:space:]]*\(|selectHeapPage[[:space:]]*\(|attemptHeapPage[[:space:]]*\(|allocateSegment[[:space:]]*\(|freeSegment[[:space:]]*\(|mutateFreeList|freeList|decommitPage[[:space:]]*\(|commitPage[[:space:]]*\(|reservePage[[:space:]]*\(|unreserve[[:space:]]*\(|releasePage[[:space:]]*\(|hako_osvm_(unreserve|release)' \
  ...
  echo "[$TAG] ERROR: MIMAP-134A must not add real segment execution, raw pointer, concurrency, segment-map, atomics, page-source/OS release seams, or backend-visible page release" >&2
...
if rg -n 'page\.(free|local_free|block_used)' "$OWNER" >/tmp/"$TAG".page_array_leak 2>&1; then
  echo "[$TAG] ERROR: MIMAP-134A owner must not mutate page arrays directly" >&2
...
if rg -n 'NYASH_|HAKO_|std::env|env::|getenv|global_allocator|GlobalAlloc|AllocatorProviderRegistry|selectProvider|install_hook|hook[A-Za-z0-9_]*[[:space:]]*\(|replace_allocator' \
  ...
  echo "[$TAG] ERROR: MIMAP-134A must not add env/provider/hook/replacement behavior" >&2
...
if rg -n 'hako-alloc-segment-allocation-modeled-local-free-reuse-ledger-release-proof|LocalFreeReuseLedgerRelease|recordReuseLedgerRelease' \
  ...
  echo "[$TAG] ERROR: MIMAP-134A app/owner matcher leaked into .inc" >&2
...
if rg -n 'k2_wide_hako_alloc_segment_allocation_modeled_local_free_reuse_ledger_release_guard\.sh' \
  "$DEV_GATE" "$ALLOCATOR_GATE" >/tmp/"$TAG".gate_growth 2>&1; then
  echo "[$TAG] ERROR: MIMAP-134A guard must stay local-run/index-listed by default" >&2
...
tmp_dir="$(mktemp -d /tmp/hakorune_mimap134a_local_free_reuse_ledger_release.XXXXXX)"
trap 'rm -rf "$tmp_dir"' EXIT

mir_json="$tmp_dir/mimap134a.mir.json"
exe_out="$tmp_dir/mimap134a.exe"
...
rg -F -q 'hako-alloc-segment-allocation-modeled-local-free-reuse-ledger-release-proof' "$vm_log"
rg -F -q 'release=1,0,0,-1,60018004,4,1,0' "$vm_log"
...
rg -F -q 'hako-alloc-segment-allocation-modeled-local-free-reuse-ledger-release-proof' "$run_log"
rg -F -q 'release=1,0,0,-1,60018004,4,1,0' "$run_log"
