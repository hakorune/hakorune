...
TAG="docs-slim-004-activation-closeout-resolver"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh
source tools/checks/lib/phase_card_paths.sh

CARD="$(guard_require_phase293x_card "$TAG" "293x-411-DOCS-SLIM-004-ACTIVATION-CLOSEOUT-RESOLVER.md")"
...
SELF_SCRIPT="tools/checks/docs_slim_004_activation_closeout_resolver_guard.sh"
...
echo "[$TAG] running DOCS-SLIM-004 activation closeout resolver guard"
...
guard_expect_in_file "$TAG" "DOCS-SLIM-004" "$CARD" "DOCS-SLIM-004 card must exist"
guard_expect_in_file "$TAG" "Do not move numbered cards in this row" "$CARD" "card must keep no-move stop-line"
guard_expect_in_file "$TAG" "Fourth Slimming Phase" "$ARCHIVE_POLICY" "archive policy must record DOCS-SLIM-004"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$CHECK_INDEX" "check index must list DOCS-SLIM-004 guard"
