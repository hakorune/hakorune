...
require_text "$TASK_BREAKDOWN" "M64-M70"
require_text "$TASK_BREAKDOWN" "M71 may add provider registry boundary docs"
...
require_text "$CURRENT_STATE" 'latest_card = "293x-122-M70-COMBINED-HOOK-PROVIDER-DRY-RUN-REPORT"'
require_text "$CURRENT_STATE" 'latest_card_path = "docs/development/current/main/phases/phase-293x/293x-122-M70-COMBINED-HOOK-PROVIDER-DRY-RUN-REPORT.md"'
...
if rg -n 'AllocatorProviderRegistry|allocator_provider_registry|select_allocator_provider|allocator_provider_select|allocator_provider_selection_env|NYASH_ALLOCATOR_PROVIDER' \
  ...
  fail "provider registry/selection implementation must stay absent in M70"
fi
rm -f /tmp/"$TAG".provider_selection
