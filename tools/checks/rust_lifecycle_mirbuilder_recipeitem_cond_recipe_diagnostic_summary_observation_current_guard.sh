...
CARD="$ROOT/docs/development/current/main/phases/phase-296x/3358-MIRBUILDER-RECIPEITEM-COND-RECIPE-DIAGNOSTIC-SUMMARY-OBSERVATION-CURRENT-GATE-001.md"
BASE_GUARD="$ROOT/tools/checks/rust_lifecycle_mirbuilder_recipeitem_cond_recipe_diagnostic_summary_observation_gate.sh"
...
token = "MIRBUILDER-RECIPEITEM-COND-RECIPE-DIAGNOSTIC-SUMMARY-OBSERVATION-CURRENT-GATE-001"
base_token = "MIRBUILDER-RECIPEITEM-COND-RECIPE-DIAGNOSTIC-SUMMARY-OBSERVATION-001"
next_card = "MIRBUILDER-RECIPEITEM-COND-RECIPE-CONSUME-BOUNDARY-SELECTION-001"
...
need(base_guard_path.exists(), "base diagnostic observation guard missing")
...
grep -q '^owner=RecipeItemBox.cond_recipe_summary$' <<<"$BASE_OUT"
grep -q '^cond_recipe_diagnostic_summary_observation=1$' <<<"$BASE_OUT"
grep -q '^cond_recipe_deep_observation_implementation=1$' <<<"$BASE_OUT"
...
cat <<'EOF'
output_contract=rust-lifecycle-mirbuilder-recipeitem-cond-recipe-diagnostic-summary-observation-current-gate
token=MIRBUILDER-RECIPEITEM-COND-RECIPE-DIAGNOSTIC-SUMMARY-OBSERVATION-CURRENT-GATE-001
cond_recipe_diagnostic_summary_observation_current_gate=1
cond_recipe_diagnostic_summary_observation=1
cond_recipe_deep_observation_implementation=1
...
runtime_fallback=0
source_selfhost_claim=0
selected_next_card=MIRBUILDER-RECIPEITEM-COND-RECIPE-CONSUME-BOUNDARY-SELECTION-001
summary=ok
EOF
