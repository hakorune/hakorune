...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_env_exact_top_symbol_variability.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-env-exact-top-symbol-variability] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-env-exact-top-symbol-variability] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  env_top_0_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_bucket_env_top_0_symbol" { print $2; exit }' "$trial_out")"
  env_top_1_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_bucket_env_top_1_symbol" { print $2; exit }' "$trial_out")"
  env_top_2_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_bucket_env_top_2_symbol" { print $2; exit }' "$trial_out")"
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-env-exact-top-symbol-variability] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
  ...
  printf '%s %s %s %s %s\n' \
    ...
    "$env_top_0_symbol" \
    "$env_top_1_symbol" \
    "$env_top_2_symbol" >>"$trial_results"
...
read -r primary_bucket_env_count primary_bucket_once_count primary_bucket_path_count primary_bucket_minimal_main_count primary_bucket_nyash_kernel_runtime_count primary_bucket_kernel_count primary_bucket_other_count primary_bucket_missing_count primary_bucket_alloc_count primary_bucket_ffi_count primary_bucket_string_count ret0_count_min ret0_count_max trial_summaries env_top_0_mode env_top_0_mode_count env_top_1_mode env_top_1_mode_count env_top_2_mode env_top_2_mode_count env_exact_env_mode env_exact_env_mode_count env_exact_getenv_mode env_exact_getenv_mode_count env_exact_current_exe_mode env_exact_current_exe_mode_count env_exact_current_dir_mode env_exact_current_dir_mode_count env_exact_other_mode env_exact_other_mode_count < <(
...
def classify_env_exact(symbol: str) -> str:
    text = symbol.lower()
    if "var_os" in text:
        return "env"
    if "getenv" in text:
...
for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
    ...
    if len(parts) != 5:
        continue
    primary, top_count_s, env_top_0_symbol, env_top_1_symbol, env_top_2_symbol = parts
    ...
    for idx, symbol in enumerate((env_top_0_symbol, env_top_1_symbol, env_top_2_symbol)):
        if symbol == "missing":
            continue
        top_symbol_counts[idx][symbol] += 1
        exact_family_counts[classify_env_exact(symbol)] += 1

env_top_0_mode, env_top_0_mode_count = mode(top_symbol_counts[0])
env_top_1_mode, env_top_1_mode_count = mode(top_symbol_counts[1])
env_top_2_mode, env_top_2_mode_count = mode(top_symbol_counts[2])
env_exact_env_mode = "env" if exact_family_counts["env"] > 0 else "missing"
env_exact_env_mode_count = exact_family_counts["env"]
env_exact_getenv_mode = "getenv" if exact_family_counts["getenv"] > 0 else "missing"
env_exact_getenv_mode_count = exact_family_counts["getenv"]
env_exact_current_exe_mode = "current_exe" if exact_family_counts["current_exe"] > 0 else "missing"
env_exact_current_exe_mode_count = exact_family_counts["current_exe"]
env_exact_current_dir_mode = "current_dir" if exact_family_counts["current_dir"] > 0 else "missing"
env_exact_current_dir_mode_count = exact_family_counts["current_dir"]
env_exact_other_mode = "other" if exact_family_counts["other"] > 0 else "missing"
env_exact_other_mode_count = exact_family_counts["other"]
...
print(
    ...
    ";".join(summaries),
    env_top_0_mode,
    env_top_0_mode_count,
    env_top_1_mode,
    env_top_1_mode_count,
    env_top_2_mode,
    env_top_2_mode_count,
    env_exact_env_mode,
    env_exact_env_mode_count,
    env_exact_getenv_mode,
    env_exact_getenv_mode_count,
    env_exact_current_exe_mode,
    env_exact_current_exe_mode_count,
    env_exact_current_dir_mode,
    env_exact_current_dir_mode_count,
    env_exact_other_mode,
    env_exact_other_mode_count,
)
PY
)

cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-env-exact-top-symbol-variability-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_env_exact_top_symbol_variability
...
env_top_0_mode=$env_top_0_mode
env_top_0_mode_count=$env_top_0_mode_count
env_top_1_mode=$env_top_1_mode
env_top_1_mode_count=$env_top_1_mode_count
env_top_2_mode=$env_top_2_mode
env_top_2_mode_count=$env_top_2_mode_count
env_exact_env_mode=$env_exact_env_mode
env_exact_env_mode_count=$env_exact_env_mode_count
env_exact_getenv_mode=$env_exact_getenv_mode
env_exact_getenv_mode_count=$env_exact_getenv_mode_count
env_exact_current_exe_mode=$env_exact_current_exe_mode
env_exact_current_exe_mode_count=$env_exact_current_exe_mode_count
env_exact_current_dir_mode=$env_exact_current_dir_mode
env_exact_current_dir_mode_count=$env_exact_current_dir_mode_count
env_exact_other_mode=$env_exact_other_mode
env_exact_other_mode_count=$env_exact_other_mode_count
