...
TAG="rust-lifecycle-mirbuilder-programjson-call-target-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-call-target-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonCallTargetShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "CallTargetShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Call.name/arity rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonCallTargetShapeScanV1", "bad owner")
...
need(criteria.get("call_resolution") == 0, "call resolution must remain unclaimed")
need(criteria.get("dispatch_selection") == 0, "dispatch selection must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_call_target_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-call-target-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-CALL-TARGET-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=CallTargetShapeSnapshotV1
shape_scope=covered ProgramJSON Call.name/arity rows
covered_rows=10
