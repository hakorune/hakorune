...
guard_require_files "$TAG" "$CARD_410" "$CARD_411" "$CARD_412" "$STATE" "$INDEX" "$SELF_SCRIPT"
...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_412" "row412 perf owner refresh must be current"
...
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-411-COLLECTION-METHOD-DIRECT-ARRAY-LANE-LEGACY-HELPER-CACHE-RETIREMENT-SEMANTIC-SMOKE"' "$STATE" "current state must land row411"
