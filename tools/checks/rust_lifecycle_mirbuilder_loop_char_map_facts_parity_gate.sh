...
TAG="rust-lifecycle-mirbuilder-loop-char-map-facts-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-loop-char-map-facts-rust-oracle-v0.json"
HAKO_IMPL="$ROOT_DIR/lang/src/compiler/lib/loop_char_map_facts.hako"
HAKO_BIN="$ROOT_DIR/tools/bin/hako"

guard_require_command "$TAG" python3
guard_require_files "$TAG" "$FIXTURE" "$HAKO_IMPL" "$HAKO_BIN"

TMP_DIR="$(mktemp -d /tmp/hakorune-loop-char-map-facts-parity.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/loop_char_map_facts_parity.hako"
EXPECTED="$TMP_DIR/expected.txt"
ACTUAL="$TMP_DIR/actual.txt"
EXE="$TMP_DIR/loop_char_map_facts_parity.exe"
...
for row in rows:
    calls.append(
        "    print(LoopCharMapFactsBox.build_summary("
        f"{json.dumps(row['observation_token'])}, {json.dumps(row['body_token'])}))"
    )

source = "\n".join([
    "using lang.compiler.lib.loop_char_map_facts as LoopCharMapFactsBox",
    "",
    "static box Main {",
    "  main() {",
    *calls,
...
if actual != expected:
    print("[loop-char-map-facts/parity] mismatch")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-loop-char-map-facts-parity-gate-v0
owner=loop_char_map_facts
input_contract=BackendSafeLoopCharMapFactsTokenSnapshotV1
rust_oracle_fixture=mirbuilder-loop-char-map-facts-rust-oracle-v0.json
hako_implementation=lang/src/compiler/lib/loop_char_map_facts.hako
parity_rows=10
parity_status=green
