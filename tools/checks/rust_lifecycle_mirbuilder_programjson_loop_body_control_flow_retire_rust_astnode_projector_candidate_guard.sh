...
TAG="rust-lifecycle-mirbuilder-programjson-loop-body-control-flow-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-loop-body-control-flow-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonLoopBodyControlFlowRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
for key in [
    "hako_snapshot_source",
    "parity_fixture",
    "parity_gate",
    "aot_publication_contract_gate",
]:
...
need(scope.get("retire_candidate") == "LoopBodyControlFlowSnapshotV1", "bad retire candidate")
need(
    scope.get("programjson_snapshot_owner") == "ProgramJsonLoopBodyControlFlowScanV1",
    "bad owner",
)
need(
    scope.get("rust_projector_runtime_dependency_removed") == 0,
    "runtime dependency removal must remain 0",
)
...
need(len(covered) == 10, "covered row count must be 10")

summary = parity_fixture.get("summary") or {}
need(summary.get("parity_row_count") == 10, "parity fixture row count drift")
...
need(criteria.get("aot_publication_contract_gate") == "Green", "AOT contract gate must be Green")
...
need(criteria.get("covered_row_count") == 10, "covered row criterion drift")
...
need(
    row.get("selected_next_card") == "MIRBUILDER-PROGRAMJSON-NEXT-CAPABILITY-SELECTION-001",
    "bad next card",
)
...
bash "$ROOT_DIR/tools/checks/hako_aot_route_value_type_publication_contract_gate.sh" >/dev/null
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_loop_body_control_flow_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-loop-body-control-flow-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-LOOP-BODY-CONTROL-FLOW-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=LoopBodyControlFlowSnapshotV1
shape_scope=covered ProgramJSON loop body control-flow rows
covered_rows=10
decision=RetireCandidateScoped
parity_gate=green
aot_publication_contract_gate=green
...
selected_next_card=MIRBUILDER-PROGRAMJSON-NEXT-CAPABILITY-SELECTION-001
summary=ok
REPORT
