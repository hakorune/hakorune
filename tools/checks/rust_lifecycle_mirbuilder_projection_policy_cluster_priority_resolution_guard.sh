...
if summary["excluded_existing_decision_cluster_count"] != 16:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 26:
...
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.context_registry::FixtureMapped::ContextRegistryCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-CONTEXT-REGISTRY-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-CONTEXT-REGISTRY-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 16:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 26:
...
selected_next_card=MIRBUILDER-CONTEXT-REGISTRY-PROJECTION-POLICY-001
