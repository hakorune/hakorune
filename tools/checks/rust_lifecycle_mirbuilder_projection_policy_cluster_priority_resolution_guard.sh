...
if summary["excluded_existing_decision_cluster_count"] != 11:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 31:
...
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.loop_cond_return_in_body::FixtureMapped::LoopCondReturnInBodyCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-LOOP-COND-RETURN-IN-BODY-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-LOOP-COND-RETURN-IN-BODY-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 11:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 31:
...
selected_next_card=MIRBUILDER-LOOP-COND-RETURN-IN-BODY-PROJECTION-POLICY-001
