...
if summary["excluded_existing_decision_cluster_count"] != 14:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 28:
    raise SystemExit("selectable cluster count drift")
if summary["selected_candidate_count"] != 24:
    raise SystemExit("selected candidate count drift")
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.loop_cond_plan::FixtureMapped::LoopCondPlanCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-LOOP-COND-PLAN-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-LOOP-COND-PLAN-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 14:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 28:
...
selected_next_card=MIRBUILDER-LOOP-COND-PLAN-PROJECTION-POLICY-001
