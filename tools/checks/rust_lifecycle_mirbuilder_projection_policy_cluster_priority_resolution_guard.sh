...
if summary["excluded_existing_decision_cluster_count"] != 28:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 14:
...
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.plan_parts_assembly::FixtureMapped::PlanPartsAssemblyCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-PLAN-PARTS-ASSEMBLY-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-PLAN-PARTS-ASSEMBLY-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 28:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 14:
...
selected_next_card=MIRBUILDER-PLAN-PARTS-ASSEMBLY-PROJECTION-POLICY-001
