...
if summary["excluded_existing_decision_cluster_count"] != 18:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 24:
...
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.plan_lowerer::FixtureMapped::PlanLowererCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-PLAN-LOWERER-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-PLAN-LOWERER-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 18:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 24:
...
selected_next_card=MIRBUILDER-PLAN-LOWERER-PROJECTION-POLICY-001
