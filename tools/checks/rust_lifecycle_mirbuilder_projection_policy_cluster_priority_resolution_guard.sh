...
if summary["excluded_existing_decision_cluster_count"] != 23:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 19:
...
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.planner_policy::FixtureMapped::PlannerPolicyCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-PLANNER-POLICY-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-PLANNER-POLICY-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 23:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 19:
...
selected_next_card=MIRBUILDER-PLANNER-POLICY-PROJECTION-POLICY-001
