...
if summary["excluded_existing_decision_cluster_count"] != 13:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 29:
...
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.loop_cond_utility::FixtureMapped::LoopCondUtilityCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-LOOP-COND-UTILITY-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-LOOP-COND-UTILITY-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 13:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 29:
...
selected_next_card=MIRBUILDER-LOOP-COND-UTILITY-PROJECTION-POLICY-001
