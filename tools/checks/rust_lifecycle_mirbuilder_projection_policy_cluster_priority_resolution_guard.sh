...
if summary["excluded_existing_decision_cluster_count"] != 33:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 9:
...
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.body_view_feature::FixtureMapped::BodyViewFeatureCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-BODY-VIEW-FEATURE-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-BODY-VIEW-FEATURE-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 33:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 9:
...
selected_next_card=MIRBUILDER-BODY-VIEW-FEATURE-PROJECTION-POLICY-001
