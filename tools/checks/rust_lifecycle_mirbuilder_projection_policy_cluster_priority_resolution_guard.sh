...
if summary["excluded_existing_decision_cluster_count"] != 9:
    raise SystemExit("excluded existing decision count drift")
if summary["selectable_cluster_count"] != 33:
    raise SystemExit("selectable cluster count drift")
if summary["selected_candidate_count"] != 53:
    raise SystemExit("selected candidate count drift")
expected_cluster = "projection_policy::UnsupportedDirectShape::shape.join_i_r_route_verify::FixtureMapped::JoinIRRouteVerifyCluster"
...
if ranked[0]["next_card"] != "MIRBUILDER-JOIN-I-R-ROUTE-VERIFY-PROJECTION-POLICY-001":
...
if decision["selected_next_card"] != "MIRBUILDER-JOIN-I-R-ROUTE-VERIFY-PROJECTION-POLICY-001":
...
if claims.get("excluded_existing_decision_cluster_count") != 9:
    raise SystemExit("excluded existing decision claim drift")
if claims.get("selectable_cluster_count") != 33:
...
selected_next_card=MIRBUILDER-JOIN-I-R-ROUTE-VERIFY-PROJECTION-POLICY-001
