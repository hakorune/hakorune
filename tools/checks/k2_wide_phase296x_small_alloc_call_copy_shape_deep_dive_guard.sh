...
guard_expect_fixed_in_file "$TAG" 'helper_call_count=7' "$report" "probe must record helper calls"
guard_expect_fixed_in_file "$TAG" 'helper_copy_count=28' "$report" "probe must record helper copies"
guard_expect_fixed_in_file "$TAG" 'receiver_copy_count=10' "$report" "probe must record receiver copies"
guard_expect_fixed_in_file "$TAG" 'arg_copy_count=4' "$report" "probe must record arg copies"
guard_expect_fixed_in_file "$TAG" 'result_copy_count=14' "$report" "probe must record result copies"
guard_expect_fixed_in_file "$TAG" 'local_ssa_copy_count=63' "$report" "probe must record local SSA copies"
