...
TAG="rust-lifecycle-mirbuilder-programjson-brand-expr-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-brand-expr-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonBrandExprShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "BrandExprShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON BrandConstruct/BrandUnwrap expression rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonBrandExprShapeScanV1", "bad owner")
...
need(criteria.get("brand_lowering") == 0, "brand lowering must remain unclaimed")
need(criteria.get("brand_runtime_semantics") == 0, "brand runtime semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_brand_expr_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-brand-expr-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-BRAND-EXPR-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=BrandExprShapeSnapshotV1
shape_scope=covered ProgramJSON BrandConstruct/BrandUnwrap expression rows
covered_rows=10
