...
TAG="rust-lifecycle-mirbuilder-programjson-no-exit-block-recipe-if-join-snapshot-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-no-exit-block-recipe-if-join-snapshot-parity-v0.json"
SNAPSHOT_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_no_exit_block_recipe_snapshot.hako"
PREREQ_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_block_recipe_recursive_contract_capability_selection_guard.sh"
...
if ! grep -q '^summary=ok$' <<<"$PREREQ_OUT"; then
  printf '%s\n' "$PREREQ_OUT" >&2
  guard_fail "$TAG" "NoExit IfJoin selection prerequisite is not green"
fi

TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-no-exit-if-join.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_no_exit_if_join_probe.hako"
MIR_JSON="$TMP_DIR/programjson_no_exit_if_join_probe.mir.json"
EXE="$TMP_DIR/programjson_no_exit_if_join_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonNoExitBlockRecipeIfJoinSnapshotParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-NO-EXIT-BLOCK-RECIPE-IF-JOIN-SNAPSHOT-PARITY-001":
...
if len(rows) != 1:
    raise SystemExit("NoExit IfJoin ProgramJSON parity requires exactly 1 row")
...
for row in rows:
    ...
    lines.append("    print(\"dto:" + row_id + ":\" + ProgramJsonNoExitBlockRecipeSnapshotBox.build_summary(" + json.dumps(program_json) + "))")
...
for needle in [
    ...
    if needle not in text:
        raise SystemExit(f"missing NoExit IfJoin bridge token: {needle}")
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for ProgramJSON NoExit IfJoin probe"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-no-exit-block-recipe-if-join-snapshot-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-NO-EXIT-BLOCK-RECIPE-IF-JOIN-SNAPSHOT-PARITY-001
