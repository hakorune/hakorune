...
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-14-MIMALLOC-PROVIDER-ALLOC-FREE-SMOKE"' "$CURRENT_STATE" "current state latest card must advance"
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "MIMALLOC-PROVIDER-EXPLICIT-REPEATED-MEASUREMENT-296X-001"' "$CURRENT_STATE" "current state must expose repeated measurement blocker"
