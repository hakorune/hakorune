...
TAG="k2-wide-phase295x-mimalloc-mixed-small-process-repeat-pack"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-238-MIMALLOC-COMPARISON-MIXED-SMALL-PROCESS-REPEAT-PACK.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-237-MIMALLOC-COMPARISON-REALLOC-ALIGNED-PROCESS-REPEAT-PACK.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
CURRENT_STATE="docs/development/current/main/CURRENT_STATE.toml"
INDEX="docs/tools/check-scripts-index.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_mimalloc_mixed_small_process_repeat_pack_guard.sh"
...
echo "[$TAG] checking phase-295x mixed-small process-repeat pack"
...
guard_expect_fixed_in_file "$TAG" 'Status: Landed' "$CARD" "card must be landed after the process-repeat closeout opens"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-MIXED-SMALL-PROCESS-REPEAT-PACK-295X-002' "$CARD" "card must identify the mixed-small process-repeat blocker"
guard_expect_in_file "$TAG" 'workload=representative-mixed-small-v0' "$CARD" "card must define workload id"
guard_expect_in_file "$TAG" 'operation_family=mixed-small' "$CARD" "card must define operation family"
guard_expect_in_file "$TAG" 'operation_sequence_id=representative-mixed-small-v0-seq' "$CARD" "card must define sequence id"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-PROCESS-REPEAT-PACK-CLOSEOUT-295X-002' "$CARD" "card must select the process-repeat closeout"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-MIXED-SMALL-PROCESS-REPEAT-PACK-295X-002' "$TASKBOARD" "taskboard must expose the mixed-small pack blocker"
...
guard_expect_in_file "$TAG" '| 237 | `MIMALLOC-COMPARISON-REALLOC-ALIGNED-PROCESS-REPEAT-PACK-295X-002` | Landed |' "$TASKBOARD" "taskboard must expose the realloc/aligned pack as landed"
guard_expect_in_file "$TAG" '| 238 | `MIMALLOC-COMPARISON-MIXED-SMALL-PROCESS-REPEAT-PACK-295X-002` | Landed |' "$TASKBOARD" "taskboard must expose the mixed-small process-repeat pack as landed"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-PROCESS-REPEAT-PACK-CLOSEOUT-295X-002' "$CURRENT_STATE" "current state must move on to the process-repeat closeout blocker"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list this guard"

tmp_dir="$(mktemp -d /tmp/hakorune_phase295x_mixed_small_repeat.XXXXXX)"
trap 'rm -rf "$tmp_dir"' EXIT
out="$tmp_dir/mixed-small-repeat.out"
...
guard_expect_in_file "$TAG" 'workload=representative-mixed-small-v0' "$APP" ".hako app must define workload id"
guard_expect_in_file "$TAG" 'operation_family=mixed-small' "$APP" ".hako app must define operation family"
guard_expect_in_file "$TAG" 'operation_sequence_id=representative-mixed-small-v0-seq' "$APP" ".hako app must define sequence id"
guard_expect_in_file "$TAG" 'free_order_id=ascending-release-v0' "$APP" ".hako app must define free order"
rg -F -q 'workload_0_id=representative-mixed-small-v0' "$out"
rg -F -q 'workload_0_operation_family=mixed-small' "$out"
...
print("[phase295x-mixed-small-process-repeat-pack] ok")
PY
