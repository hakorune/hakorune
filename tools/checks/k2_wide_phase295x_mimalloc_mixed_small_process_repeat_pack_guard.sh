...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD" "card must remain current while the mixed-small pack is exercised"
...
guard_expect_in_file "$TAG" '238' "$CURRENT_STATE" "current state must point at the mixed-small pack card"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-MIXED-SMALL-PROCESS-REPEAT-PACK-295X-002' "$CURRENT_STATE" "current state must expose the mixed-small pack blocker"
