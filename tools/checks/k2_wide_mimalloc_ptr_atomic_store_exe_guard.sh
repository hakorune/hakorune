...
TAG="k2-wide-mimalloc-ptr-atomic-store-exe"
cd "$ROOT_DIR"
source "$ROOT_DIR/tools/checks/lib/pure_first_exe_guard.sh"

APP="apps/mimalloc-ptr-atomic-store-proof/main.hako"
APP_README="apps/mimalloc-ptr-atomic-store-proof/README.md"
CARD="docs/development/current/main/phases/phase-293x/293x-087-M35-NATIVE-PTR-ATOMIC-STORE-ROUTE-PROOF.md"
TASKBOARD="docs/development/current/main/design/mimalloc-capability-taskboard-ssot.md"

echo "[$TAG] running M35 native pointer atomic store EXE guard"

guard_require_files "$TAG" "$APP" "$APP_README" "$CARD" "$TASKBOARD"

if rg -n 'mimalloc-ptr-atomic-store-proof' lang/c-abi/shims >/tmp/"$TAG".app_specific.inc 2>&1; then
  echo "[$TAG] ERROR: app-specific pointer atomic store matcher leaked into .inc" >&2
...
if rg -n 'hako_atomic_ptr_(load|cas)_ordered|HakoAtomicPtr(Load|Cas)Ordered|extern\\.hako_atomic\\.ptr_(load|cas)_ordered|ptr_fetch_add' \
  ...
  echo "[$TAG] ERROR: pointer atomic load/CAS/fetch_add rows must stay inactive in M35" >&2
...
tmp_dir="$(mktemp -d /tmp/hakorune_m35_ptr_atomic_store.XXXXXX)"
trap 'rm -rf "$tmp_dir"' EXIT

mir_json="$tmp_dir/m35.mir.json"
exe_out="$tmp_dir/m35.exe"
...
rg -F -q 'mimalloc-ptr-atomic-store-proof' "$run_log"
rg -F -q 'store=0' "$run_log"
rg -F -q 'summary=ok' "$run_log"

rg -F -q '| `M35 native pointer atomic route proof` | `live-narrow` |' "$TASKBOARD"
rg -F -q 'M35 Native Ptr Atomic Store Route Proof' "$CARD"
rg -F -q 'hako_atomic_ptr_store_ordered' "$APP_README"
rg -F -q 'k2_wide_mimalloc_ptr_atomic_store_exe_guard.sh' docs/tools/check-scripts-index.md
