...
SELF_SCRIPT="tools/checks/k2_wide_mimalloc_facade_huge_unreserve_exe_guard.sh"
...
guard_expect_in_file "$TAG" 'OsVmCoreBox\.unreserve_bytes_usize\(base, bytes\)' "$PAGE_SOURCE" "page-source policy must delegate to OSVM unreserve usize facade"
...
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list MIMAP-034A guard"
...
require_global("HakoAllocPageSourcePolicy.unreservePage/2", "OsVmCoreBox.unreserve_bytes_usize/2")
require_global("OsVmCoreBox.unreserve_bytes_usize/2", "OsVmCoreBox.unreserve_bytes_i64/2")
