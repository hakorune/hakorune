...
require_file() {
  local path="$1"
  guard_require_files "$TAG" "$path"
}

require_text() {
  local file="$1"
  local needle="$2"
  guard_expect_fixed_in_file "$TAG" "$needle" "$file" "missing text in $file: $needle"
}
...
require_text "$ACTIVATION_FAMILY" "tools/checks/k2_wide_allocator_provider_activation_entry_contract_guard.sh"
