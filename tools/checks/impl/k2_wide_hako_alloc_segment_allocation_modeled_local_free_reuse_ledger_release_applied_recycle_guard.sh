...
guard_expect_in_file "$TAG" 'segment_allocation_modeled_local_free_reuse_ledger_box.hako' "$MODULE_INDEX" "memory module index must define reuse ledger owner"
guard_expect_in_file "$TAG" 'MIMAP-142A' "$OWNER_CONTRACTS" "segment allocation owner contracts must name MIMAP-142A owner"
