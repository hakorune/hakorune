...
ACTIVATION_FAMILY="tools/checks/allocator/families/provider/activation.steps"
SAFETY_FAMILY="tools/checks/allocator/families/provider/activation_safety.steps"
...
require_file "$CARD"
require_file "$INDEX"
require_file "$ACTIVATION_FAMILY"
require_file "$SAFETY_FAMILY"
...
for guard in "${required_guards[@]}"; do
  ...
  if [[ "$guard" == *activation_entry_contract_guard.sh || \
        "$guard" == *registry_snapshot_guard.sh || \
        "$guard" == *selection_decision_guard.sh || \
        "$guard" == *proof_bundle_consumption_guard.sh || \
        "$guard" == *rollback_preflight_guard.sh ]]; then
    require_text "$ACTIVATION_FAMILY" "$guard"
  else
    require_text "$SAFETY_FAMILY" "$guard"
  fi
done
...
require_text "$SAFETY_FAMILY" "tools/checks/k2_wide_allocator_provider_activation_safety_closeout_guard.sh"
