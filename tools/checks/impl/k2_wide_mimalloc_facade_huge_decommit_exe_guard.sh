...
SELF_SCRIPT="tools/checks/k2_wide_mimalloc_facade_huge_decommit_exe_guard.sh"
...
guard_expect_in_file "$TAG" 'object_lifecycle_facade_huge_decommit_box.hako' "$MODULE_INDEX" "memory module index must name MIMAP-029A owner"
...
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list MIMAP-029A guard"
...
require_global("HakoAllocPageSourcePolicy.decommitPage/2", "OsVmCoreBox.decommit_bytes_usize/2")
