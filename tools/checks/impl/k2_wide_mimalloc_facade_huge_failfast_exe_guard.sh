...
SELF_SCRIPT="tools/checks/k2_wide_mimalloc_facade_huge_failfast_exe_guard.sh"
...
guard_expect_in_file "$TAG" 'object_lifecycle_facade_huge_failfast_box.hako' "$MODULE_INDEX" "memory module index must name MIMAP-022B owner"
guard_expect_in_file "$TAG" 'MIMAP-022B' "$CARD" "MIMAP-022B card missing"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list MIMAP-022B guard"
