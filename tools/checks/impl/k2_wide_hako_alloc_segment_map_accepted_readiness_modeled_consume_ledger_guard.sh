...
guard_expect_in_file "$TAG" 'id = "MIMAP-157A"' "$PROOF_MANIFEST_INCLUDE" "proof app manifest include must list MIMAP-157A"
guard_expect_in_file "$TAG" 'validation_profile = "scalar-mir"' "$PROOF_MANIFEST_INCLUDE" "MIMAP-157A must stay on scalar-mir validation"
guard_expect_in_file "$TAG" 'exe = "deferred-to-closeout"' "$PROOF_MANIFEST_INCLUDE" "MIMAP-157A EXE evidence must be deferred to closeout"
...
guard_expect_in_file "$TAG" 'MIMAP-157A' "$OWNER_CONTRACTS" "memory owner contracts must define MIMAP-157A owner"
