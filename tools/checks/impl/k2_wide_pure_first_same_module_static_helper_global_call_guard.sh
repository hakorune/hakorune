...
PUBLIC_GUARD="tools/checks/k2_wide_pure_first_same_module_static_helper_global_call_guard.sh"
...
guard_require_exec_files "$TAG" "$APP_TEST" "$PUBLIC_GUARD"
...
guard_expect_in_file "$TAG" "$PUBLIC_GUARD" "$INDEX" "check script index must list this guard"
