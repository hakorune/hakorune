...
OWNER_CONTRACTS="lang/src/hako_alloc/memory/OWNER_CONTRACTS_SEGMENT_ALLOCATION.md"
...
guard_expect_in_file "$TAG" 'id = "MIMAP-161A"' "$PROOF_MANIFEST_INCLUDE" "proof app manifest include must list MIMAP-161A"
guard_expect_in_file "$TAG" 'validation_profile = "scalar-mir"' "$PROOF_MANIFEST_INCLUDE" "MIMAP-161A must stay on scalar-mir validation"
guard_expect_in_file "$TAG" 'exe = "deferred-to-closeout"' "$PROOF_MANIFEST_INCLUDE" "MIMAP-161A EXE evidence must be deferred to closeout"
...
guard_expect_in_file "$TAG" 'MIMAP-161A' "$OWNER_CONTRACTS" "segment-allocation owner contracts must define MIMAP-161A owner"
