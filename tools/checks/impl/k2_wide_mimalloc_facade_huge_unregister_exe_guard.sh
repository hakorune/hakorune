...
SELF_SCRIPT="tools/checks/k2_wide_mimalloc_facade_huge_unregister_exe_guard.sh"
...
guard_expect_in_file "$TAG" 'object_lifecycle_facade_huge_unregister_box.hako' "$MODULE_INDEX" "memory module index must name MIMAP-026A owner"
guard_expect_in_file "$TAG" 'Status: landed' "$CARD" "MIMAP-026A card must be landed after implementation"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list MIMAP-026A guard"
