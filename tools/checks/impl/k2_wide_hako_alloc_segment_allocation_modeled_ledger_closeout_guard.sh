...
guard_expect_in_file "$TAG" "id = \"hako-alloc-segment-allocation-modeled-ledger-closeout\"" "$GUARD_MANIFEST" "guard manifest must include MIMAP-095A closeout row"
...
guard_expect_in_file "$TAG" "box HakoAllocSegmentAllocationModeledLedgerReport" "$OWNER" "report box must stay present"
