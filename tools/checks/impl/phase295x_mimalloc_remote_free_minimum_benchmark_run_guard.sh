...
check_output_contract() {
  ...
  rg -F -q 'backend_split_scope=remote-free-minimum-v0' "$run_log"
  rg -F -q 'backend_split_family=split-observation' "$run_log"
...
build_and_measure() {
  ...
  case "$label" in
    ...
    remote_free_publish_only)
      ...
      rg -F -q 'symbol=HakoAllocProductionFacade.remotePagePublishOnlyFastWithPort/4' "$build_log"
    ...
    remote_free_collect_only)
      ...
      rg -F -q 'mir_call_hako_mem_free_emit' "$build_log"
      rg -F -q 'symbol=HakoAllocProductionFacade.remotePagePort/5' "$build_log"
      rg -F -q 'symbol=HakoAllocProductionFacade.remotePageCollectOnlyFastWithPort/4' "$build_log"
      rg -F -q 'symbol=HakoAllocProductionFacade.remotePageResetPort/2' "$build_log"
      rg -F -q 'symbol=HakoAllocProductionFacade.remotePageAcquire/2' "$build_log"
      rg -F -q 'symbol=HakoAllocProductionFacade.remotePagePublishPort/4' "$build_log"
...

{
  ...
  echo "backend_split_scope=remote-free-minimum-v0"
  echo "backend_split_family=split-observation"
