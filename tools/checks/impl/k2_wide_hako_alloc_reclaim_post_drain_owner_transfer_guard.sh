...
report = plans.get("HakoAllocReclaimPostDrainOwnerTransferReport")
if report is None:
    raise SystemExit("missing typed object plan: HakoAllocReclaimPostDrainOwnerTransferReport")
report_fields = {field.get("name"): field for field in report.get("fields", [])}
...
missing_report = sorted(name for name in expected_report_fields if name not in report_fields)
if missing_report:
    raise SystemExit(f"missing report fields: {missing_report}")
for name in expected_report_fields:
...
route = plans.get("HakoAllocReclaimPostDrainOwnerTransfer")
if route is None:
    raise SystemExit("missing typed object plan: HakoAllocReclaimPostDrainOwnerTransfer")
route_fields = {field.get("name"): field for field in route.get("fields", [])}
...
missing_route = sorted(name for name in expected_route_fields if name not in route_fields)
if missing_route:
    raise SystemExit(f"missing route fields: {missing_route}")
for name in (
    "attempt_count",
    "success_count",
    "drain_block_count",
    ...
    if field.get("declared_type") != "i64" or field.get("storage") != "i64":
        raise SystemExit(f"bad route field {name}: {field}")
