...
SELF_SCRIPT="tools/checks/k2_wide_mimalloc_facade_huge_unreserve_failfast_exe_guard.sh"
...
guard_expect_in_file "$TAG" 'object_lifecycle_facade_huge_unreserve_failfast_box.hako' "$MODULE_INDEX" "memory module index must name MIMAP-035A owner"
guard_expect_in_file "$TAG" 'object_lifecycle_facade_huge_backing_set_box.hako' "$MODULE_INDEX" "memory module index must name MIMAP-037A helper"
...
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list MIMAP-035A guard"
