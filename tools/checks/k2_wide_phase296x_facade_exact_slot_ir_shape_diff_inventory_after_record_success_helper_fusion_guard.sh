...
DOC="$ROOT_DIR/docs/development/current/main/phases/phase-296x/296x-286-FACADE-EXACT-SLOT-IR-SHAPE-DIFF-INVENTORY-AFTER-RECORD-SUCCESS-HELPER-FUSION.md"
PREV="$ROOT_DIR/docs/development/current/main/phases/phase-296x/296x-285-WEIGHTED-EXACT-SLOT-OWNER-SELECTION-AFTER-RECORD-SUCCESS-HELPER-FUSION.md"
TOOL="$ROOT_DIR/tools/allocator/object_lifecycle_facade_exact_slot_field_inventory.py"
TMP_DIR="$(mktemp -d /tmp/hakorune_row286_facade_inventory.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[row286-facade-inventory-after-record-success] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_line "$DOC" "target_family_pct=13.47"
require_line "$DOC" "facade_method_count=5"
require_line "$DOC" "facade_exact_slot_get_count=18"
require_line "$DOC" "facade_exact_slot_set_count=9"
require_line "$DOC" "facade_exact_slot_field_op_count=27"
require_line "$DOC" "top_facade_method=HakoAllocObjectLifecycleFacade.objectLifecycleSmallAlloc/1"
require_line "$DOC" "top_facade_method_pct=7.14"
require_line "$DOC" "dominant_field_family=facade_receiver_state"
require_line "$DOC" "field_family.facade_receiver_state_count=14"
...
cat >"$PERF" <<'REPORT'
    13.85%  app.exe  app.exe               [.] nyash.object.exact_slot_set_i64_hii
               |--1.00%--HakoAllocObjectLifecycleFacade.objectLifecycleSmallAlloc/1
               |--1.20%--HakoAllocObjectLifecycleFacade.resetSmallAllocResult/0
     9.58%  app.exe  app.exe               [.] nyash.object.exact_slot_set_u64_hiu
               |--1.00%--HakoAllocObjectLifecycleFacade.objectLifecycleSmallAlloc/1
               |--0.64%--HakoAllocObjectLifecycleFacade.recordReleaseSuccess/2
     8.74%  app.exe  app.exe               [.] nyash.object.exact_slot_get_u64_hii
               |--2.29%--HakoAllocObjectLifecycleFacade.objectLifecycleReleaseDirectCachedPage/2
               |--1.00%--HakoAllocObjectLifecycleFacade.objectLifecycleSmallAlloc/1
     6.99%  app.exe  app.exe               [.] nyash.object.exact_slot_get_i64_hii
               |--2.00%--HakoAllocObjectLifecycleFacade.objectLifecycleSmallAlloc/1
     ...
     6.96%  app.exe  app.exe               [.] nyash.object.exact_slot_get_handle_hii
               |--2.14%--HakoAllocObjectLifecycleFacade.objectLifecycleSmallAlloc/1
               |--1.36%--HakoAllocObjectLifecycleFacade.resetReleaseResult/0
...
cat >"$MIR" <<'JSON'
{
  "functions": [
    ...
    },
    {
      ...
      "blocks": [
        {"id": 610, "instructions": [{"op": "field_get", "box": 1, "field": "alloc_result"}]}
      ]
    },
    {
      ...
      "blocks": [
        {"id": 620, "instructions": [{"op": "field_get", "box": 1, "field": "release_result"}]}
      ]
    }
  ]
}
...
require_line "$REPORT" "target_family_pct=13.47"
require_line "$REPORT" "facade_method_count=5"
require_line "$REPORT" "facade_exact_slot_get_count=18"
require_line "$REPORT" "facade_exact_slot_set_count=9"
require_line "$REPORT" "facade_exact_slot_field_op_count=27"
require_line "$REPORT" "dominant_field_family=facade_receiver_state"
require_line "$REPORT" "field_family.facade_receiver_state_count=14"
