...
TAG="rust-lifecycle-mirbuilder-programjson-recipebodies-loop-body-multi-body-arena-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipebodies-loop-body-multi-body-arena-retire-rust-astnode-projector-candidate-v0.json"
PARITY_FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipebodies-loop-body-multi-body-arena-parity-v0.json"
PARITY_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_recipebodies_loop_body_multi_body_arena_parity_gate.sh"
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/3207-MIRBUILDER-PROGRAMJSON-RECIPEBODIES-LOOP-BODY-MULTI-BODY-ARENA-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001.md"
...
if ! grep -q '^summary=ok$' <<<"$PARITY_OUT"; then
  printf '%s\n' "$PARITY_OUT" >&2
  guard_fail "$TAG" "RecipeBodies Loop-body arena parity gate is not green"
fi
...
token = "MIRBUILDER-PROGRAMJSON-RECIPEBODIES-LOOP-BODY-MULTI-BODY-ARENA-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001"
next_card = "MIRBUILDER-PROGRAMJSON-RECIPEBODIES-AFTER-LOOP-BODY-ARENA-NEXT-CONTRACT-SELECTION-001"
rows = ["local_loop_body_assignment_return"]

if fixture.get("kind") != "MirBuilderProgramJsonRecipeBodiesLoopBodyMultiBodyArenaRustAstNodeProjectorRetireCandidateV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != token:
    raise SystemExit("bad fixture token")
if fixture.get("input_state", {}).get("snapshot_kind") != "ProgramJsonRecipeBodiesLoopBodyArenaBuilderV1":
    raise SystemExit("snapshot kind drift")
if fixture.get("input_state", {}).get("programjson_owner") != "ProgramJsonRecipeBodiesLoopBodyArenaBuilderBox":
    raise SystemExit("owner drift")

parity_rows = [row.get("row_id") for row in parity.get("rows") or []]
if parity_rows != rows:
    raise SystemExit("parity row drift")
if parity.get("claims", {}).get("loop_body_multi_body_arena_implemented") != 1:
    raise SystemExit("Loop-body arena implementation not proven")
...
for key in [
    ...
    "loop_body_id_emitted",
]:
    if evidence.get(key) != 1:
        raise SystemExit(f"evidence missing: {key}")
if evidence.get("body_count") != 2:
    raise SystemExit("body_count must be 2")
...
if "3207 marks the covered ProgramJsonRecipeBodiesLoopBodyArenaBuilderV1 row" not in task_order:
    raise SystemExit("task-order 3207 marker missing")
PY

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-recipebodies-loop-body-multi-body-arena-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-RECIPEBODIES-LOOP-BODY-MULTI-BODY-ARENA-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate_recorded=1
covered_rows=local_loop_body_assignment_return
