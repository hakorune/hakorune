...
TAG="rust-lifecycle-mirbuilder-programjson-no-continue-return-only-reject-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-no-continue-return-only-reject-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonNoContinueReturnOnlyRejectRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
    == "MIRBUILDER-PROGRAMJSON-NO-CONTINUE-RETURN-ONLY-REJECT-RETIRE-RUST-ASTNODE-PROJECTOR-CANDIDATE-001",
    "bad token",
)
...
need(scope.get("retire_candidate") == "LoopCondContinueNoContinueReturnOnlyRejectTokenSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "LoopCondContinueNoContinueReturnOnlyRejectV1", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonLoopCondContinueWithReturnSnapshotV1", "bad owner")
need(scope.get("parity_row_id") == "reject_no_continue_return_only", "bad parity row")
...
row = rows.get("reject_no_continue_return_only") or {}
summary = ((row.get("rust_astnode_token_oracle") or {}).get("expected_summary") or "")
need(summary == "accepted=0;reason=no_continue", "return-only no-continue oracle row drift")
loop_body = (((row.get("program_json") or {}).get("body") or [{}])[0].get("body") or [])
need(len(loop_body) == 1 and loop_body[0].get("type") == "Return", "return-only row must contain exactly one Return")
...
need(criteria.get("programjson_route_observes_return_only_missing_continue") == 1, "return-only observation proof missing")
...
need(
    row.get("selected_next_card") == "MIRBUILDER-PROGRAMJSON-NEXT-SHAPE-SELECTION-008",
    "bad next card",
)
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-no-continue-return-only-reject-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-NO-CONTINUE-RETURN-ONLY-REJECT-RETIRE-RUST-ASTNODE-PROJECTOR-CANDIDATE-001
