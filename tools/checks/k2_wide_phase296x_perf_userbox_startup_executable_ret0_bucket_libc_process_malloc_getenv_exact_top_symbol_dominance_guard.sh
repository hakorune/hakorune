...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_libc_process_malloc_getenv_exact_top_symbol_dominance.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-libc-process-malloc-getenv-exact-top-symbol-dominance] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
malloc_count = symbol_counts.get("malloc", 0)
getenv_count = symbol_counts.get("getenv", 0)
if malloc_count == 0 or getenv_count == 0:
    raise SystemExit(1)

print("output_contract=perf-userbox-startup-executable-ret0-bucket-libc-process-malloc-getenv-exact-top-symbol-split-v0")
...
print(f"libc_process_malloc_symbol_count={malloc_count}")
print(f"libc_process_getenv_symbol_count={getenv_count}")
