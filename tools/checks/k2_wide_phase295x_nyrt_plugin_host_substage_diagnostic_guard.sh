...
TAG="k2-wide-phase295x-nyrt-plugin-host-substage-diagnostic"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-43-MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-SUBSTAGE-DIAGNOSTIC.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-42-MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-BASELINE-SELECTION.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
ENV_DOC="docs/reference/environment-variables.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_nyrt_plugin_host_substage_diagnostic_guard.sh"
...
echo "[$TAG] checking phase-295x NyRT plugin host substage diagnostic"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-SUBSTAGE-DIAGNOSTIC-295X-001' "$CARD" "card must identify current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-SELECTION-295X-001' "$CARD" "card must select loadset follow-on"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-SUBSTAGE-DIAGNOSTIC-295X-001' "$PREV_CARD" "previous row must select this diagnostic"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-SELECTION-295X-001' "$TASKBOARD" "taskboard must expose selected follow-on"
