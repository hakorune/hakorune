...
guard_expect_in_file "$TAG" 'Status: landed' "$NEXT_CARD" "MIMAP-022B card must be landed after implementation"
...
guard_expect_in_file "$TAG" 'phase_status = "docs/development/current/main/phases/phase-293x/293x-435-MIMAP-022C-POST-HUGE-FAILFAST-ROW-SELECTION.md"' "$STATE" "CURRENT_STATE must move phase_status after MIMAP-022B lands"
guard_expect_in_file "$TAG" 'current_blocker_token = "MIMAP-022C"' "$STATE" "CURRENT_STATE must set MIMAP-022C as blocker after MIMAP-022B lands"
guard_expect_in_file "$TAG" 'latest_card = "293x-434-MIMAP-022B-FACADE-HUGE-REQUEST-FAILFAST-ROUTING"' "$STATE" "CURRENT_STATE latest card must close MIMAP-022B"
guard_expect_in_file "$TAG" 'MIMAP-022C post-huge-failfast row selection' "$CURRENT_TASK" "CURRENT_TASK must restart on MIMAP-022C"
guard_expect_in_file "$TAG" 'MIMAP-022C post-huge-failfast row selection' "$RESUME" "quick resume must restart on MIMAP-022C"
guard_expect_in_file "$TAG" 'MIMAP-022C post-huge-failfast row selection' "$NOW" "Now mirror must restart on MIMAP-022C"
guard_expect_in_file "$TAG" '293x-435-MIMAP-022C-POST-HUGE-FAILFAST-ROW-SELECTION.md' "$PHASE_README" "phase README must point at MIMAP-022C as current blocker"
...
guard_expect_in_file "$TAG" '| `MIMAP-022B` | landed | Facade huge-request fail-fast routing before page-source attach/retry. | after MIMAP-022A |' "$TASKBOARD" "taskboard must close MIMAP-022B"
guard_expect_in_file "$TAG" '| `MIMAP-022C` | ready current | Post-huge-failfast allocator row selection. | current |' "$TASKBOARD" "taskboard must mark MIMAP-022C current"
...
guard_expect_in_file "$TAG" '`MIMAP-022C` | post-huge-failfast allocator row selection' "$GRANULARITY" "granularity SSOT must list MIMAP-022C follow-up"
