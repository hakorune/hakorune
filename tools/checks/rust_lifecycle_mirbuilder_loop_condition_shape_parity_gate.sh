...
TAG="rust-lifecycle-mirbuilder-loop-condition-shape-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-loop-condition-shape-rust-oracle-v0.json"
HAKO_IMPL="$ROOT_DIR/lang/src/compiler/lib/loop_condition_shape.hako"
HAKO_BIN="$ROOT_DIR/tools/bin/hako"

guard_require_command "$TAG" python3
guard_require_files "$TAG" "$FIXTURE" "$HAKO_IMPL" "$HAKO_BIN"

TMP_DIR="$(mktemp -d /tmp/hakorune-loop-condition-shape-parity.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/loop_condition_shape_parity.hako"
EXPECTED="$TMP_DIR/expected.txt"
ACTUAL="$TMP_DIR/actual.txt"
EXE="$TMP_DIR/loop_condition_shape_parity.exe"
...
for row in rows:
    calls.append(
        "    print(LoopConditionShapeBox.build_summary("
        f"{json.dumps(row['token'])}))"
    )

source = "\n".join([
    "using lang.compiler.lib.loop_condition_shape as LoopConditionShapeBox",
    "",
    "static box Main {",
    "  main() {",
    *calls,
...
if actual != expected:
    print("[loop-condition-shape/parity] mismatch")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-loop-condition-shape-parity-gate-v0
owner=loop_condition_shape
input_contract=BackendSafeLoopConditionShapeTokenSnapshotV1
rust_oracle_fixture=mirbuilder-loop-condition-shape-rust-oracle-v0.json
hako_implementation=lang/src/compiler/lib/loop_condition_shape.hako
parity_rows=15
parity_status=green
