...
TAG="rust-lifecycle-mirbuilder-exit-only-block-recipe-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-exit-only-block-recipe-rust-oracle-v0.json"
HAKO_IMPL="$ROOT_DIR/lang/src/compiler/lib/exit_only_block_recipe.hako"
HAKO_BIN="$ROOT_DIR/tools/bin/hako"

guard_require_command "$TAG" python3
guard_require_files "$TAG" "$FIXTURE" "$HAKO_IMPL" "$HAKO_BIN"

TMP_DIR="$(mktemp -d /tmp/hakorune-exit-only-block-recipe-parity.XXXXXX)"
cleanup() {
  rm -rf "$TMP_DIR" >/dev/null 2>&1 || true
}
trap cleanup EXIT

APP="$TMP_DIR/exit_only_block_recipe_parity.hako"
EXPECTED="$TMP_DIR/expected.txt"
ACTUAL="$TMP_DIR/actual.txt"
EXE="$TMP_DIR/exit_only_block_recipe_parity.exe"
...
for row in rows:
    ...
    calls.append(
        "    print(ExitOnlyBlockRecipeBox.build_summary("
        f"{json.dumps(tokens[0])}, {json.dumps(tokens[1])}))"
    )

source = "\n".join(
    [
        "using lang.compiler.lib.exit_only_block_recipe as ExitOnlyBlockRecipeBox",
        "",
        "static box Main {",
        "  main() {",
        *calls,
...
if actual != expected:
    print("[exit-only-block-recipe/parity] mismatch")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-exit-only-block-recipe-parity-gate-v0
owner=exit_only_block_recipe
input_contract=BackendSafeExitOnlyBlockTokenSnapshotV1
rust_oracle_fixture=mirbuilder-exit-only-block-recipe-rust-oracle-v0.json
hako_implementation=lang/src/compiler/lib/exit_only_block_recipe.hako
parity_rows=8
parity_status=green
