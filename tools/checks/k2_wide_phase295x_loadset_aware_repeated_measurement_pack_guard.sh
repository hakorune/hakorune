...
TAG="k2-wide-phase295x-loadset-aware-repeated-measurement-pack"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-64-MIMALLOC-COMPARISON-LOADSET-AWARE-REPEATED-MEASUREMENT-PACK.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-63-MIMALLOC-COMPARISON-RUNTIME-REFERENCE-LOADSET-STANDALONE-CLOSEOUT.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_loadset_aware_repeated_measurement_pack_guard.sh"
RUNNER="tools/allocator/mimalloc_repeated_measurement_runner.py"

echo "[$TAG] checking phase-295x loadset-aware repeated measurement pack"

guard_require_files "$TAG" "$CARD" "$PREV_CARD" "$TASKBOARD" "$INDEX" "$SELF_SCRIPT" "$RUNNER"
guard_require_exec_files "$TAG" "$SELF_SCRIPT" "$RUNNER"

guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-LOADSET-AWARE-REPEATED-MEASUREMENT-PACK-295X-001' "$CARD" "card must identify current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-LOADSET-AWARE-REPEATED-MEASUREMENT-CLOSEOUT-295X-001' "$CARD" "card must select closeout follow-on"
...
guard_expect_in_file "$TAG" 'hako_selected_loadset=empty' "$CARD" "card must require empty selected loadset"
guard_expect_in_file "$TAG" 'hako_selected_library_count=0' "$CARD" "card must require zero selected libraries"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-LOADSET-AWARE-REPEATED-MEASUREMENT-CLOSEOUT-295X-001' "$TASKBOARD" "taskboard must expose selected follow-on"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list this guard"

tmp_dir="$(mktemp -d /tmp/hakorune_phase295x_loadset_aware_full_pack.XXXXXX)"
trap 'rm -rf "$tmp_dir"' EXIT
out="$tmp_dir/loadset-aware-full-pack.out"
...
for key, value in expected.items():
    if values.get(key) != value:
        raise SystemExit(f"{key} expected {value}, got {values.get(key)!r}")
...
print("[phase295x-loadset-aware-repeated-measurement-pack] ok")
PY
