...
guard_expect_fixed_in_file "$TAG" 'baseline_page_acquire_mir_instruction_count=235' "$report" "tool must record baseline acquire shape"
...
guard_expect_fixed_in_file "$TAG" 'candidate_0_mir_instruction_count=147' "$report" "tool must record acquire_usize shape"
