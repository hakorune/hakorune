...
use super::declarations::{
    outer_cfg_syntax, parse_module_source_v1, validate_module_attributes, ModuleDeclarationV1,
};
...
use super::model::{
    ...
    ModuleInstanceKindV1, ModuleSourceObservationV1, DECLARED_MODULE_TOPOLOGY_SCHEMA_V1,
};
...
impl ModuleTraversalV1 {
    ...
    fn collect_root(
        ...
        let parsed = parse_module_source_v1(&self.relative(&lexical_path)?, &source)?;
        ...
        let result = self.walk_declarations(
            &root_id,
            "crate",
            &lexical_path,
    ...
    fn walk_declarations(
        ...
        declarations: &[ModuleDeclarationV1],
    ) -> Result<(), ModuleTopologyErrorV1> {
        for declaration in declarations {
            ...
            if cfg.state == CfgDecisionStateV1::Excluded {
                self.edges.push(DeclaredModuleEdgeV1 {
                    ...
                    kind: if declaration.inline_children.is_some() {
            ...
            if let Some(children) = &declaration.inline_children {
    ...
    fn add_external_module(
        ...
        let parsed = parse_module_source_v1(&selected_relative, &source)?;
        let observation_id = self.add_source_observation(
            &child_id,
    ...
    fn finish(self) -> Result<DeclaredModuleTopologyV1, ModuleTopologyErrorV1> {
        let included = self
            ...
            .count();
        if self.instances.len() != 1 + included
