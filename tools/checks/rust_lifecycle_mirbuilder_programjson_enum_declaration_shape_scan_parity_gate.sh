...
TAG="rust-lifecycle-mirbuilder-programjson-enum-declaration-shape-scan-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-enum-declaration-shape-scan-parity-v0.json"
SCAN_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_enum_declaration_shape_scan.hako"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-enum-declaration-shape-scan.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_enum_declaration_shape_scan_parity.hako"
EXPECTED="$TMP_DIR/expected.json"
RUN_LOG="$TMP_DIR/run.log"
EXE="$TMP_DIR/programjson_enum_declaration_shape_scan_parity.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonEnumDeclarationShapeScanParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-ENUM-DECLARATION-SHAPE-SCAN-CAPABILITY-001":
    raise SystemExit("bad fixture token")

emit = fixture.get("programjson_emit_evidence") or {}
if emit.get("source") != "src/macro/ast_json/joinir_compat.rs":
    raise SystemExit("missing ProgramJSON emit evidence")
if emit.get("node_kind") != "EnumDeclaration":
    raise SystemExit("bad ProgramJSON node kind evidence")
if emit.get("fields") != ["name", "variants", "type_parameters", "attrs"]:
    raise SystemExit("bad ProgramJSON field evidence")
if emit.get("variant_fields") != ["name", "payload_type", "tuple_payload_types", "record_fields"]:
    raise SystemExit("bad ProgramJSON variant field evidence")

rows = fixture.get("rows") or []
if len(rows) < 8:
    raise SystemExit("enum-declaration capability must cover at least 8 rows")
...
for row in rows:
    ...
    calls.append(
        ...
        + " + ProgramJsonEnumDeclarationShapeScanBox.build_summary("
...
source = "\n".join([
    "using lang.compiler.mirbuilder.program_json_enum_declaration_shape_scan as ProgramJsonEnumDeclarationShapeScanBox",
    "",
    "static box Main {",
    "  main() {",
    *calls,
...
if ! bash "$HAKO_BIN" --backend mir --emit-exe "$EXE" "$APP" >"$EMIT_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit ProgramJSON enum-declaration shape scan parity executable"
fi

if ! "$EXE" >"$RUN_LOG" 2>&1; then
  tail -n 160 "$RUN_LOG" || true
  guard_fail "$TAG" "failed to run ProgramJSON enum-declaration shape scan parity executable"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-enum-declaration-shape-scan-parity-gate-v0
owner=ProgramJsonEnumDeclarationShapeScanV1
input_contract=ProgramJSON-v0
output_contract=EnumDeclarationShapeSnapshotV1
fixture=mirbuilder-programjson-enum-declaration-shape-scan-parity-v0.json
hako_implementation=lang/src/compiler/mirbuilder/program_json_enum_declaration_shape_scan.hako
