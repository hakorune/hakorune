...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_subkind_variability.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-subkind-variability] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-subkind-variability] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-subkind-variability] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
...
def classify_env(symbol: str) -> str:
    text = symbol.lower()
    if "getenv" in text or "var_os" in text:
        return "getenv"
    if "current_exe" in text or "path" in text or "current_dir" in text:
        return "pathprep"
    return "other"


def classify_once(symbol: str) -> str:
    text = symbol.lower()
    if "futex" in text or "once_lock" in text or "once::" in text:
        return "futex"
    return "other"


def classify_path(symbol: str) -> str:
    text = symbol.lower()
...
def classify_minimal_main(symbol: str) -> str:
    text = symbol.lower()
    if text == "main" or "main" in text:
        return "main"
    return "other"


def classify_nyash_kernel_runtime(symbol: str) -> str:
    text = symbol.lower()
    if "registry" in text:
        return "registry"
    if "env" in text:
...
for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
    ...
    if env_symbol != "missing":
        present_counts["env"] += 1
        subkind_counts["env"][classify_env(env_symbol)] += 1
    if once_symbol != "missing":
        present_counts["once"] += 1
        subkind_counts["once"][classify_once(once_symbol)] += 1
    if path_symbol != "missing":
        present_counts["path"] += 1
        subkind_counts["path"][classify_path(path_symbol)] += 1
    if minimal_main_symbol != "missing":
        present_counts["minimal_main"] += 1
        subkind_counts["minimal_main"][classify_minimal_main(minimal_main_symbol)] += 1
    if nyash_kernel_runtime_symbol != "missing":
        present_counts["nyash_kernel_runtime"] += 1
        subkind_counts["nyash_kernel_runtime"][classify_nyash_kernel_runtime(nyash_kernel_runtime_symbol)] += 1
    summaries.append(f"{primary}:{top_count}")

env_mode, env_mode_count = mode(subkind_counts["env"])
once_mode, once_mode_count = mode(subkind_counts["once"])
path_mode, path_mode_count = mode(subkind_counts["path"])
minimal_main_mode, minimal_main_mode_count = mode(subkind_counts["minimal_main"])
nyash_kernel_runtime_mode, nyash_kernel_runtime_mode_count = mode(subkind_counts["nyash_kernel_runtime"])
...
print(
    ...
    present_counts["once"],
    present_counts["path"],
    present_counts["minimal_main"],
    present_counts["nyash_kernel_runtime"],
    env_mode,
    env_mode_count,
    once_mode,
    once_mode_count,
...
cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-subkind-variability-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_subkind_variability
...
bucket_env_subkind_mode=$bucket_env_subkind_mode
bucket_env_subkind_mode_count=$bucket_env_subkind_mode_count
bucket_once_subkind_mode=$bucket_once_subkind_mode
bucket_once_subkind_mode_count=$bucket_once_subkind_mode_count
bucket_path_subkind_mode=$bucket_path_subkind_mode
bucket_path_subkind_mode_count=$bucket_path_subkind_mode_count
bucket_minimal_main_subkind_mode=$bucket_minimal_main_subkind_mode
bucket_minimal_main_subkind_mode_count=$bucket_minimal_main_subkind_mode_count
bucket_nyash_kernel_runtime_subkind_mode=$bucket_nyash_kernel_runtime_subkind_mode
bucket_nyash_kernel_runtime_subkind_mode_count=$bucket_nyash_kernel_runtime_subkind_mode_count
