...
TAG="rust-lifecycle-mirbuilder-programjson-assign-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-assign-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonAssignShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "AssignShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Assign statement rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonAssignShapeScanV1", "bad owner")
...
need(criteria.get("mir_mutation") == 0, "MIR mutation must remain unclaimed")
need(criteria.get("assignment_lowering") == 0, "assignment lowering must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_assign_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-assign-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-ASSIGN-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=AssignShapeSnapshotV1
shape_scope=covered ProgramJSON Assign statement rows
covered_rows=10
