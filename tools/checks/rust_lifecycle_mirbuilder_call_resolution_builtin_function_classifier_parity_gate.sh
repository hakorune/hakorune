...
TAG="rust-lifecycle-mirbuilder-call-resolution-builtin-function-classifier-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-call-resolution-builtin-function-classifier-rust-oracle-v0.json"
HAKO_IMPL="$ROOT_DIR/lang/src/compiler/lib/call_resolution_builtin_function_classifier.hako"
HAKO_BIN="$ROOT_DIR/tools/bin/hako"

guard_require_command "$TAG" python3
guard_require_files "$TAG" "$FIXTURE" "$HAKO_IMPL" "$HAKO_BIN"

TMP_DIR="$(mktemp -d /tmp/hakorune-call-resolution-builtin-function-classifier.XXXXXX)"
cleanup() {
  rm -rf "$TMP_DIR" >/dev/null 2>&1 || true
}
trap cleanup EXIT

APP="$TMP_DIR/call_resolution_builtin_function_classifier_parity.hako"
EXPECTED="$TMP_DIR/expected.txt"
ACTUAL="$TMP_DIR/actual.txt"
EXE="$TMP_DIR/call_resolution_builtin_function_classifier_parity.exe"
...
for row in rows:
    calls.append(
        "    print(CallResolutionBuiltinFunctionClassifierBox."
        f"is_builtin_function({json.dumps(row['name'])}))"
    )

source = "\n".join(
    [
        "using lang.compiler.lib.call_resolution_builtin_function_classifier as CallResolutionBuiltinFunctionClassifierBox",
        "",
        "static box Main {",
        "  main() {",
        *calls,
...
if actual != expected:
    print("[call-resolution-builtin-function-classifier/parity] mismatch")
