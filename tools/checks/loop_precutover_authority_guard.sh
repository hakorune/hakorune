...
while IFS= read -r file; do
  [[ -z "$file" ]] && continue
  [[ "$file" == "$DEMAND_ISSUER" || "$file" == *_tests.rs ]] && continue
...
for file in "$RECURSIVE" "$RAW_LOOP" "$LOAN_PORT" "$ROOT_LOWERING" "$DEMAND_ISSUER" "$ROUTING"; do
...
echo "[$TAG] ok (legacy production edge=1, new Dynamic demand production callers=0)"
