...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_122" "row122 card must be current"
...
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-121-HAKO-MIMALLOC-SMALL-ALLOC-DUPLICATE-REASON-CALL-PROBE"' "$CURRENT_STATE" "current state latest card must advance to row121"
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "HAKO-MIMALLOC-SMALL-ALLOC-HAKO-REASON-BIND-PROBE-296X-001"' "$CURRENT_STATE" "current state must select row122"
...
guard_expect_fixed_in_file "$TAG" '| 122 | `HAKO-MIMALLOC-SMALL-ALLOC-HAKO-REASON-BIND-PROBE-296X-001` | Current |' "$TASKBOARD" "taskboard row122 must be current"
