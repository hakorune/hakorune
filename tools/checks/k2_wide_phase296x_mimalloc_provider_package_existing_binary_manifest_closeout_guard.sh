...
guard_require_files "$TAG" "$CARD_20" "$CARD_21" "$TASKBOARD" "$INDEX" "$TOOL" "$PREV_GUARD" "$SELF_SCRIPT"
...
guard_expect_fixed_in_file "$TAG" 'MIMALLOC-PROVIDER-PACKAGE-CLI-PACKAGE-PILOT-296X-001' "$TASKBOARD" "taskboard must expose CLI package pilot"
