...
TAG="docs-slim-012-inline-record-probe-resolver"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh
source tools/checks/lib/phase_card_paths.sh

CARD="$(guard_require_phase293x_card "$TAG" "293x-419-DOCS-SLIM-012-INLINE-RECORD-PROBE-RESOLVER.md")"
...
SELF_SCRIPT="tools/checks/docs_slim_012_inline_record_probe_resolver_guard.sh"

converted_scripts=(
  "tools/checks/k2_wide_arraybox_inline_record_probe_guard.sh"
  "tools/checks/k2_wide_arraybox_inline_record_plan_probe_guard.sh"
  "tools/checks/k2_wide_metadata_store_indexed_read_guard.sh"
)

echo "[$TAG] running DOCS-SLIM-012 inline record probe resolver guard"
...
guard_expect_in_file "$TAG" "DOCS-SLIM-012" "$CARD" "DOCS-SLIM-012 card must exist"
guard_expect_in_file "$TAG" "Do not move numbered cards in this row" "$CARD" "card must keep no-move stop-line"
guard_expect_in_file "$TAG" "Twelfth Slimming Phase" "$ARCHIVE_POLICY" "archive policy must record DOCS-SLIM-012"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$CHECK_INDEX" "check index must list DOCS-SLIM-012 guard"
