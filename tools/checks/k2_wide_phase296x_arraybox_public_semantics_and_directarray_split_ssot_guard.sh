...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_373" "split SSOT must be current"
...
guard_expect_fixed_in_file "$TAG" 'selected_next=array_slot_nativedirect_legacy_helper_cache_retirement_implementation' "$CARD_373" "row373 must reopen implementation after split SSOT"
...
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "ARRAYBOX-PUBLIC-SEMANTICS-AND-DIRECTARRAY-SPLIT-SSOT-296X-001"' "$STATE" "current state must point to row373"
