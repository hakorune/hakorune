...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_454A" "$CARD_455A" "$CARD_456A" "$CARD" "$DESIGN" "$DESIGN_455A" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MEMORY_README" "$OWNER" "$LEDGER_OWNER" "$DIAG_OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
