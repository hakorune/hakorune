...
guard_expect_in_file "$TAG" 'Status: Landed' "$CARD" "card must be landed after the closeout pack lands"
...
guard_expect_in_file "$TAG" '| 218 | `MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-CLOSEOUT-295X-002` | Landed |' "$TASKBOARD" "taskboard must mark the closeout row as landed"
guard_expect_in_file "$TAG" '| 219 | `MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-BASELINE-BREAKDOWN-SELECTION-295X-002` | Current |' "$TASKBOARD" "taskboard must expose the baseline-breakdown selection row as current"
