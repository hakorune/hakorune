...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_focus_presence.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-focus-presence] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-focus-presence] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-focus-presence] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
...
def classify_exact(symbol: str) -> str:
    text = symbol.lower()
    if "registry" in text or "rebuild_cache" in text or "register_many" in text or "create_default_registry" in text:
        return "registry"
...
registry_exact_rebuild_cache_mode = "registry" if exact_family_counts["registry"] > 0 else "missing"
registry_exact_rebuild_cache_mode_count = exact_family_counts["registry"]
registry_exact_register_many_mode = "registry" if exact_family_counts["registry"] > 0 else "missing"
registry_exact_register_many_mode_count = exact_family_counts["registry"]
...
cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-focus-presence-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_focus_presence
...
registry_focus_present=$(
  if [ "${registry_exact_registry_mode_count:-0}" -gt 0 ]; then
    echo 1
  else
    echo 0
  fi
...
if [ "${registry_exact_registry_mode_count:-0}" -le 0 ]; then
  echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-focus-presence] expected aggregate registry_exact_registry_mode_count > 0" >&2
  exit 1
fi
