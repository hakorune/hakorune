...
TAG="rust-lifecycle-mirbuilder-programjson-return-expr-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-return-expr-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonReturnExprShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ReturnExprShapeSnapshotV1", "bad retire candidate")
need(
    scope.get("programjson_snapshot_owner") == "ProgramJsonReturnExprShapeScanV1",
    "bad owner",
)
...
need(
    row.get("selected_next_card") == "MIRBUILDER-PROGRAMJSON-LOCAL-BINDING-SHAPE-SCAN-CAPABILITY-001",
    "bad next card",
)
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_return_expr_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-return-expr-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-RETURN-EXPR-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ReturnExprShapeSnapshotV1
shape_scope=covered ProgramJSON Return.expr rows
covered_rows=8
...
selected_next_card=MIRBUILDER-PROGRAMJSON-LOCAL-BINDING-SHAPE-SCAN-CAPABILITY-001
summary=ok
REPORT
