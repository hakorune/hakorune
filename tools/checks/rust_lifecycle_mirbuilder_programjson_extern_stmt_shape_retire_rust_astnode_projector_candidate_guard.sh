...
TAG="rust-lifecycle-mirbuilder-programjson-extern-stmt-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-extern-stmt-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonExternStmtShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ExternStmtShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Extern statement rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonExternStmtShapeScanV1", "bad owner")
...
need(criteria.get("extern_route_lowering") == 0, "extern route lowering must remain unclaimed")
need(criteria.get("extern_route_publication") == 0, "extern route publication must remain unclaimed")
...
for key in [
    ...
    "extern_route_lowering",
    "extern_route_publication",
    "runtime_fallback",
    "new_backend_route",
    "new_abi",
]:
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_extern_stmt_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-extern-stmt-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-EXTERN-STMT-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ExternStmtShapeSnapshotV1
shape_scope=covered ProgramJSON Extern statement rows
covered_rows=10
