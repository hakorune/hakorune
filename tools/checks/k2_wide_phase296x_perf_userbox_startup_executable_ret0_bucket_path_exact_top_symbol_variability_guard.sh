...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_path_exact_top_symbol_variability.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-path-exact-top-symbol-variability] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-path-exact-top-symbol-variability] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  path_top_0_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_bucket_path_top_0_symbol" { print $2; exit }' "$trial_out")"
  path_top_1_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_bucket_path_top_1_symbol" { print $2; exit }' "$trial_out")"
  path_top_2_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_bucket_path_top_2_symbol" { print $2; exit }' "$trial_out")"
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-path-exact-top-symbol-variability] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
  ...
  printf '%s %s %s %s %s\n' \
    ...
    "$path_top_0_symbol" \
    "$path_top_1_symbol" \
    "$path_top_2_symbol" >>"$trial_results"
...
read -r primary_bucket_env_count primary_bucket_once_count primary_bucket_path_count primary_bucket_minimal_main_count primary_bucket_nyash_kernel_runtime_count primary_bucket_kernel_count primary_bucket_other_count primary_bucket_missing_count primary_bucket_alloc_count primary_bucket_ffi_count primary_bucket_string_count ret0_count_min ret0_count_max trial_summaries path_top_0_mode path_top_0_mode_count path_top_1_mode path_top_1_mode_count path_top_2_mode path_top_2_mode_count path_exact_components_mode path_exact_components_mode_count path_exact_current_dir_mode path_exact_current_dir_mode_count path_exact_current_exe_mode path_exact_current_exe_mode_count path_exact_other_mode path_exact_other_mode_count < <(
...
def classify_exact(symbol: str) -> str:
    ...
    if "current_exe" in text:
        return "current_exe"
    if "path" in text or "osstr" in text or "osstring" in text or "readlink" in text:
        return "other"
    return "other"
...
for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
    ...
    if len(parts) != 5:
        continue
    primary, top_count_s, path_top_0_symbol, path_top_1_symbol, path_top_2_symbol = parts
    ...
    for idx, symbol in enumerate((path_top_0_symbol, path_top_1_symbol, path_top_2_symbol)):
        if symbol == "missing":
            continue
        top_symbol_counts[idx][symbol] += 1
        exact_family_counts[classify_exact(symbol)] += 1

path_top_0_mode, path_top_0_mode_count = mode(top_symbol_counts[0])
path_top_1_mode, path_top_1_mode_count = mode(top_symbol_counts[1])
path_top_2_mode, path_top_2_mode_count = mode(top_symbol_counts[2])
path_exact_components_mode = "components" if exact_family_counts["components"] > 0 else "missing"
path_exact_components_mode_count = exact_family_counts["components"]
path_exact_current_dir_mode = "current_dir" if exact_family_counts["current_dir"] > 0 else "missing"
path_exact_current_dir_mode_count = exact_family_counts["current_dir"]
path_exact_current_exe_mode = "current_exe" if exact_family_counts["current_exe"] > 0 else "missing"
path_exact_current_exe_mode_count = exact_family_counts["current_exe"]
path_exact_other_mode = "other" if exact_family_counts["other"] > 0 else "missing"
path_exact_other_mode_count = exact_family_counts["other"]
...
print(
    ...
    ";".join(summaries),
    path_top_0_mode,
    path_top_0_mode_count,
    path_top_1_mode,
    path_top_1_mode_count,
    path_top_2_mode,
    path_top_2_mode_count,
    path_exact_components_mode,
    path_exact_components_mode_count,
    path_exact_current_dir_mode,
    path_exact_current_dir_mode_count,
    path_exact_current_exe_mode,
    path_exact_current_exe_mode_count,
    path_exact_other_mode,
    path_exact_other_mode_count,
)
PY
)
...
cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-path-exact-top-symbol-variability-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_path_exact_top_symbol_variability
...
path_top_0_mode=$path_top_0_mode
path_top_0_mode_count=$path_top_0_mode_count
path_top_1_mode=$path_top_1_mode
path_top_1_mode_count=$path_top_1_mode_count
path_top_2_mode=$path_top_2_mode
path_top_2_mode_count=$path_top_2_mode_count
path_exact_components_mode=$path_exact_components_mode
path_exact_components_mode_count=$path_exact_components_mode_count
path_exact_current_dir_mode=$path_exact_current_dir_mode
path_exact_current_dir_mode_count=$path_exact_current_dir_mode_count
path_exact_current_exe_mode=$path_exact_current_exe_mode
path_exact_current_exe_mode_count=$path_exact_current_exe_mode_count
path_exact_other_mode=$path_exact_other_mode
path_exact_other_mode_count=$path_exact_other_mode_count
