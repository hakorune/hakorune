...
TAG="rust-lifecycle-mirbuilder-programjson-recipematcher-break-present-verified-recipe-support"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipematcher-break-present-verified-recipe-support-v0.json"
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/3240-MIRBUILDER-PROGRAMJSON-RECIPEMATCHER-BREAK-PRESENT-VERIFIED-RECIPE-SUPPORT-001.md"
TASK_ORDER="$ROOT_DIR/docs/development/current/main/design/mirbuilder-rust-to-hako-converter-task-order-ssot.md"
CURRENT_STATE="$ROOT_DIR/docs/development/current/main/CURRENT_STATE.toml"
PREV_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_loop_body_ifcontinue_ifreturn_assignment_boxcount_accepted_floor_gate.sh"
...
if ! grep -q '^if_continue_if_return_assignment_supported=1$' <<<"$PREV_OUT"; then
  printf '%s\n' "$PREV_OUT" >&2
  guard_fail "$TAG" "continue-present accepted-floor prerequisite is not green"
fi

TMP_DIR="$(mktemp -d /tmp/hakorune-ifbreak-ifreturn.XXXXXX)"
...
token = "MIRBUILDER-PROGRAMJSON-RECIPEMATCHER-BREAK-PRESENT-VERIFIED-RECIPE-SUPPORT-001"
if fixture.get("kind") != "MirBuilderProgramJsonRecipeMatcherBreakPresentVerifiedRecipeSupportV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != token:
    raise SystemExit("bad fixture token")
row = fixture.get("row") or {}
if row.get("row_id") != "local_loop_body_if_break_if_return_assignment":
    raise SystemExit("bad row id")
expected = {"matched": 1, "contract_kind": "LoopWithExit", "has_break": 1, "has_continue": 0, "has_return": 1}
...
for needle in [token, "MIRBUILDER-PROGRAMJSON-LOOP-BODY-IFCONTINUE-IFRETURN-ASSIGNMENT-BOXCOUNT-ACCEPTED-FLOOR-001"]:
...
for key in [
    "programjson_recipematcher_accepted_floor_break_present_row",
    "if_break_if_return_assignment_boxcount",
    "verified_recipe_three_stmt_loop_body_snapshot",
    "matcher_result_equal_for_break_present",
]:
    if claims.get(key) != 1:
...
for key, value in claims.items():
    if key in {
        ...
        "if_break_if_return_assignment_boxcount",
        "verified_recipe_three_stmt_loop_body_snapshot",
        "canonical_loop_facts_break_return_snapshot",
        "matcher_result_equal_for_break_present",
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-recipematcher-break-present-verified-recipe-support-v0
token=MIRBUILDER-PROGRAMJSON-RECIPEMATCHER-BREAK-PRESENT-VERIFIED-RECIPE-SUPPORT-001
row_count=1
row_id=local_loop_body_if_break_if_return_assignment
if_break_if_return_assignment_supported=1
