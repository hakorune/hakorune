...
run_quick() {
  ...
  run_step "Program(JSON v0) compat caller guard" \
  ...
  run_step "Stage1 Program(JSON) compat caller guard" \
  ...
  run_step "phase29ci verify probe surface guard" \
    bash tools/checks/phase29ci_verify_probe_surface_guard.sh

  run_step "phase216/217 normalization canary surface guard" \
    bash tools/checks/phase216217_normalization_canary_surface_guard.sh
  ...
  run_step "MIR builder CallTarget owner guard" \
  ...
  run_step "K2-wide TLS first-row guard" \
    bash tools/checks/k2_wide_tls_first_row_guard.sh

  run_step "K2-wide GC first-row guard" \
    bash tools/checks/k2_wide_gc_first_row_guard.sh

  run_step "K2-wide OSVM first-row guard" \
  ...
  run_step "K2-wide static const table load guard" \
  ...
  run_step "K2-wide Effect/Capability plan guard" \
  ...
  run_step "K2-wide runtime-decl return proof row guard" \
  ...
  run_step "K2-wide RawArray slot_load_i64 EXE guard" \
    bash tools/checks/k2_wide_rawarray_slot_load_exe_guard.sh

  run_step "K2-wide RawArray slot_store_i64 EXE guard" \
    bash tools/checks/k2_wide_rawarray_slot_store_exe_guard.sh

  run_step "K2-wide mimalloc raw-page EXE guard" \
  ...
  run_step "K2-wide mimalloc dynamic bin EXE guard" \
    bash tools/checks/k2_wide_mimalloc_dynamic_bin_exe_guard.sh

  run_step "K2-wide mimalloc size_to_bin inline EXE guard" \
    bash tools/checks/k2_wide_mimalloc_size_to_bin_inline_exe_guard.sh

  run_step "K2-wide mimalloc OSVM page EXE guard" \
  ...
  run_step "K2-wide mimalloc atomic load EXE guard" \
    bash tools/checks/k2_wide_mimalloc_atomic_load_exe_guard.sh

  run_step "K2-wide mimalloc atomic store EXE guard" \
  ...
  run_step "K2-wide atomic memory-order args vocab guard" \
