...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_384A" "$CARD_385A" "$CARD" "$NEXT_CARD" "$DESIGN" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MEMORY_README" "$OWNER" "$PREV_OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
...
guard_expect_in_file "$TAG" 'Status: selected current' "$NEXT_CARD" "MIMAP-387A must be selected current"
...
guard_expect_in_file "$TAG" 'provider_call_modeled_open_pilot_box.hako' "$MEMORY_README" "memory README must name provider-call modeled-open owner"
