...
require_text "$RUNTIME_OWNER" "activation_gate_open: false"
require_text "$RUNTIME_OWNER" "would_open_activation_gate: false"
require_text "$RUNTIME_OWNER" "would_activate_hook: false"
require_text "$RUNTIME_OWNER" "would_activate: false"
...
require_text "$TASK_BREAKDOWN" "The next safe row is M86 activation decision surface proposal."
...
require_text "$REAL_APP_TASKBOARD" '[x] `293x-137` M85 allocator provider activation safety closeout inventory'
require_text "$CURRENT_STATE" 'latest_card = "293x-137-M85-ALLOCATOR-PROVIDER-ACTIVATION-SAFETY-CLOSEOUT-INVENTORY"'
require_text "$CURRENT_STATE" 'latest_card_path = "docs/development/current/main/phases/phase-293x/293x-137-M85-ALLOCATOR-PROVIDER-ACTIVATION-SAFETY-CLOSEOUT-INVENTORY.md"'
