...
TAG="map-get-direct-mapbox-scalar-demand-validation"
CARD="docs/development/current/main/phases/phase-296x/296x-860-MIMALLOC-MAP-GET-DIRECT-MAPBOX-SCALAR-DEMAND-VALIDATION-001.md"
PREV_CARD="docs/development/current/main/phases/phase-296x/296x-859-MIMALLOC-MAP-GET-DIRECT-MAPBOX-SCALAR-DEMAND-IMPLEMENTATION-001.md"
INDEX="docs/tools/check-scripts-index.md"
SELF_SCRIPT="tools/checks/k2_wide_phase296x_map_get_direct_mapbox_scalar_demand_validation_guard.sh"
...
for stop_line in \
  ...
  "do not route unproven direct MapBox.get to scalar helper"; do
...
grep -F -q "selected_next=MIMALLOC-MAP-GET-DIRECT-MAPBOX-SCALAR-DEMAND-VALIDATION-001" "$PREV_CARD" || {
...
cargo build --release --bin hakorune >/tmp/${TAG}.hakorune_build.log 2>&1
...
if [[ "$scalar_route_count" -lt 2 ]]; then
  echo "[$TAG] expected at least two scalar map get routes, got $scalar_route_count" >&2
  exit 1
fi

if grep -F -q '"symbol": "nyash.map.slot_load_hh"' "$MIR_JSON"; then
  echo "[$TAG] source MIR still contains slot_load_hh for target front" >&2
  exit 1
fi

bash tools/build_hako_llvmc_ffi.sh >/tmp/${TAG}.ffi_build.log 2>&1
