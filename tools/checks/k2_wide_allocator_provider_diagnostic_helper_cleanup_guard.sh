...
require_text "$REGISTRY" "allocator_provider_toml_helpers"
require_text "$REGISTRY" "DiagnosticFactCheck"
require_text "$REGISTRY" "[DiagnosticFactCheck; 24]"
require_text "$REGISTRY" "Vec<DiagnosticFactCheck>"
...
if rg -n '^fn (text_field_matches|bool_field_false|nonempty_text_field|string_list_contains_all)\b' \
  "$REGISTRY" "$ACTIVATION_DECISION" >/tmp/"$TAG".local_helpers 2>&1; then
...
if rg -n '^struct (RegistrySnapshotFactCheck|SelectionDecisionFactCheck|ActivationSafetyFactCheck|ActivationDecisionFactCheck)\b' \
  "$REGISTRY" "$ACTIVATION_DECISION" >/tmp/"$TAG".fact_checks 2>&1; then
