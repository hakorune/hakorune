...
guard_require_files "$TAG" "$FIXTURE" "$TOOL" "$CARD" "$FACTS" "$EXACT_FACTS" "$LOCAL_SLOT" "$LOCAL_TESTS" "$WITNESS" "$ROUTES" "$TESTS" "$REFRESH"

python3 "$TOOL" --check
cargo test -q generic_method_route_facts
cargo test -q mapstore_i64_key_witness
cargo test -q exact_numeric_value_facts::tests::local_contract_write

python3 - "$FIXTURE" "$CARD" "$FACTS" "$EXACT_FACTS" "$LOCAL_SLOT" "$LOCAL_TESTS" "$WITNESS" "$ROUTES" "$TESTS" "$REFRESH" <<'PY'
import json
import sys
...
local_slot = Path(sys.argv[5]).read_text(encoding="utf-8")
local_tests = Path(sys.argv[6]).read_text(encoding="utf-8")
witness = Path(sys.argv[7]).read_text(encoding="utf-8")
routes = Path(sys.argv[8]).read_text(encoding="utf-8")
tests = Path(sys.argv[9]).read_text(encoding="utf-8")
refresh = Path(sys.argv[10]).read_text(encoding="utf-8")
...
local_bridge = candidates["fact.mapstore.key_domain.local_contract_write_i64"]
...
need(local_bridge["eligibility"] == "implemented_checked_local_bridge", "local bridge eligibility drift")
...
need(claims.get("local_contract_write_exact_i64_bridge") == 1, "local bridge claim missing")
...
need("ExactNumericValueFactSource::LocalContractWrite" in exact_facts, "local Fact source missing")
need("seed_local_contract_write_facts" in exact_facts, "local Fact producer missing")
need("fresh_local_identity_slots" in local_slot, "fresh local identity owner missing")
need("stale_contract_id_rejects_claim" in local_tests, "stale contract rejection missing")
need("mapstore_i64_key_from_two_checked_writes_through_phi" in local_tests, "PHI identity proof missing")
...
print("local_contract_write_exact_i64_bridge=1")
