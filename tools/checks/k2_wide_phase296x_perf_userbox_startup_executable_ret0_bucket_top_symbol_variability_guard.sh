...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_top_symbol_variability.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-top-symbol-variability] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-top-symbol-variability] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-top-symbol-variability] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac

  top0_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_top_0_symbol" { print $2; exit }' "$trial_out")"
  top1_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_top_1_symbol" { print $2; exit }' "$trial_out")"
  top2_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_top_2_symbol" { print $2; exit }' "$trial_out")"
...
read -r primary_bucket_env_count primary_bucket_once_count primary_bucket_path_count primary_bucket_minimal_main_count primary_bucket_nyash_kernel_runtime_count primary_bucket_kernel_count primary_bucket_other_count primary_bucket_missing_count primary_bucket_alloc_count primary_bucket_ffi_count primary_bucket_string_count ret0_count_min ret0_count_max trial_summaries bucket_env_top_symbol_present_count bucket_once_top_symbol_present_count bucket_path_top_symbol_present_count bucket_minimal_main_top_symbol_present_count bucket_nyash_kernel_runtime_top_symbol_present_count bucket_env_top_symbol_mode bucket_env_top_symbol_mode_count bucket_once_top_symbol_mode bucket_once_top_symbol_mode_count bucket_path_top_symbol_mode bucket_path_top_symbol_mode_count bucket_minimal_main_top_symbol_mode bucket_minimal_main_top_symbol_mode_count bucket_nyash_kernel_runtime_top_symbol_mode bucket_nyash_kernel_runtime_top_symbol_mode_count < <(
...
for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
    ...
    if len(parts) != 5:
        continue
    primary, top_count_s, top0_symbol, top1_symbol, top2_symbol = parts
    ...
    for symbol in (top0_symbol, top1_symbol, top2_symbol):
        if symbol == "missing":
            continue
        bucket = classify_ret0_bucket(symbol)
        if bucket in bucket_symbol_counts:
            bucket_present_counts[bucket] += 1
            bucket_symbol_counts[bucket][symbol] += 1

env_mode, env_mode_count = mode(bucket_symbol_counts["env"])
once_mode, once_mode_count = mode(bucket_symbol_counts["once"])
path_mode, path_mode_count = mode(bucket_symbol_counts["path"])
minimal_main_mode, minimal_main_mode_count = mode(bucket_symbol_counts["minimal_main"])
nyash_kernel_runtime_mode, nyash_kernel_runtime_mode_count = mode(bucket_symbol_counts["nyash_kernel_runtime"])
...
cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-top-symbol-variability-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_top_symbol_variability
...
bucket_env_top_symbol_present_count=$bucket_env_top_symbol_present_count
bucket_once_top_symbol_present_count=$bucket_once_top_symbol_present_count
bucket_path_top_symbol_present_count=$bucket_path_top_symbol_present_count
bucket_minimal_main_top_symbol_present_count=$bucket_minimal_main_top_symbol_present_count
bucket_nyash_kernel_runtime_top_symbol_present_count=$bucket_nyash_kernel_runtime_top_symbol_present_count
bucket_env_top_symbol_mode=$bucket_env_top_symbol_mode
bucket_env_top_symbol_mode_count=$bucket_env_top_symbol_mode_count
bucket_once_top_symbol_mode=$bucket_once_top_symbol_mode
bucket_once_top_symbol_mode_count=$bucket_once_top_symbol_mode_count
bucket_path_top_symbol_mode=$bucket_path_top_symbol_mode
bucket_path_top_symbol_mode_count=$bucket_path_top_symbol_mode_count
bucket_minimal_main_top_symbol_mode=$bucket_minimal_main_top_symbol_mode
bucket_minimal_main_top_symbol_mode_count=$bucket_minimal_main_top_symbol_mode_count
bucket_nyash_kernel_runtime_top_symbol_mode=$bucket_nyash_kernel_runtime_top_symbol_mode
bucket_nyash_kernel_runtime_top_symbol_mode_count=$bucket_nyash_kernel_runtime_top_symbol_mode_count
