...
guard_expect_fixed_in_file "$TAG" 'helper_copy_post_boxshape_status=unchanged' "$CARD_145" "row145 must record unchanged helper-copy status"
...
guard_expect_fixed_in_file "$TAG" 'helper_copy_count=62' "$report" "tool must record helper copy count"
guard_expect_fixed_in_file "$TAG" 'helper_copy_post_boxshape_status=unchanged' "$report" "tool must classify unchanged helper-copy status"
