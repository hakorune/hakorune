...
TAG="rust-lifecycle-mirbuilder-programjson-expr-unary-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-expr-unary-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonExprUnaryShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ExprUnaryShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Expr.expr Unary rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonExprUnaryShapeScanV1", "bad owner")
...
need(criteria.get("unary_lowering") == 0, "unary lowering must remain 0")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_expr_unary_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-expr-unary-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-EXPR-UNARY-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ExprUnaryShapeSnapshotV1
shape_scope=covered ProgramJSON Expr.expr Unary rows
covered_rows=8
