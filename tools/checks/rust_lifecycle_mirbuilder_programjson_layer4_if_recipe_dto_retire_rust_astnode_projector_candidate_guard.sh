...
TAG="rust-lifecycle-mirbuilder-programjson-layer4-if-recipe-dto-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-layer4-if-recipe-dto-retire-rust-astnode-projector-candidate-v0.json"
PARITY_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_if_recipe_dto_parity_gate.sh"
...
if ! grep -q '^runtime_parity_green=1$' <<<"$PARITY_OUT"; then
  printf '%s\n' "$PARITY_OUT" >&2
  guard_fail "$TAG" "If Recipe DTO runtime parity is not green"
fi
if ! grep -q '^mir_json_route_green=1$' <<<"$PARITY_OUT"; then
  printf '%s\n' "$PARITY_OUT" >&2
  guard_fail "$TAG" "If Recipe DTO MIR JSON route is not green"
fi
...
if fixture.get("kind") != "MirBuilderProgramJsonLayer4IfRecipeDtoRustAstNodeProjectorRetireCandidateV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-LAYER4-IF-RECIPE-DTO-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001":
    raise SystemExit("bad fixture token")

evidence = fixture.get("evidence") or {}
expected_hashes = {
    "hako_snapshot_source_hash": "sha256:096de465c636b29c56a7112af243cacbae1afeef79dc79eb7f97e05644d780f4",
    "parity_fixture_hash": "sha256:c06b9e720b890843feece1ed60549707f7fac05a5ccb7725924c12fea390feec",
    "parity_gate_hash": "sha256:2b3590d094024da333ba0f0253bab9436455ea5c032f125e27dbeedb31d8c3a6",
    "if_handler_source_hash": "sha256:f8407915df0b22a011574e528737480c3ca70d523cb261b9dac63c6956ae84be",
}
...
if scope.get("shape_scope") != "covered ProgramJSON Layer4 If Recipe DTO rows":
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-layer4-if-recipe-dto-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-LAYER4-IF-RECIPE-DTO-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=IfRecipeDtoSnapshotV1
shape_scope=covered ProgramJSON Layer4 If Recipe DTO rows
covered_rows=4
