...
TAG="rust-lifecycle-mirbuilder-programjson-token-snapshot-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-token-snapshot-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonTokenSnapshotRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "LoopCondContinueWithReturnTokenSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "LoopCondContinueThenReturnMinimalV1", "bad shape scope")
...
need(criteria.get("same_facade_output_equal_for_accept_shape") == 1, "facade parity proof missing")
...
need(
    row.get("selected_next_card") == "MIRBUILDER-PROGRAMJSON-NEXT-SHAPE-SELECTION-001",
    "bad next card",
)
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-token-snapshot-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-TOKEN-SNAPSHOT-RETIRE-RUST-ASTNODE-PROJECTOR-CANDIDATE-001
