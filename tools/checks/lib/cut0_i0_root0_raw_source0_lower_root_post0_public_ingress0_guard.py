...
RAW_LAMBDA_CAPTURE = ROOT / "src/mir/builder/raw_lambda_capture_lifecycle.rs"
SOURCES = (
...
def main() -> int:
    ...
    raw_lambda_capture = production_code(RAW_LAMBDA_CAPTURE); callable_lambda_port = text_between(raw_lambda_capture, "impl RawLambdaCaptureDemandPortV1 for RawInvocationChildPortV1", "struct RawLambdaCaptureEnvironmentV1"); lambda_dispatch = text_between(production_code(RAW_EXPRESSION_DISPATCH), "ASTNode::Lambda {", "ASTNode::BoxDeclaration {")
    ...
    if not all(x in callable_ledger + callable_lambda_port for x in ("ordered_capture_demands", "direct_lambda_captures", "callable_ledger")) or "RawLambdaLexicalObservationV1::observe" in callable_lambda_port or callable_lambda_port.index("if let Some(ledger) = &self.callable_ledger") > callable_lambda_port.index("let Some(ledger) = &self.semantic_ledger") or lambda_dispatch.index("selected_lambda_captures_v1") > lambda_dispatch.index("PreparedRawLambdaLexicalCaptureLifecycleV1::prepare"): raise AssertionError("callable direct-Lambda capture authority drift")
