...
def main() -> int:
    ...
    work_plan, script_runtime_work, script_direct_owner, script_program_item_admission, raw_expression_dispatch, top_level_admission, constructor_admission = production_code(ROOT / "src/mir/builder/program_root_work_plan.rs"), production_code(ROOT / "src/mir/builder/normal_script_runtime_work.rs"), production_code(ROOT / "src/mir/builder/normal_script_direct_statement_owner.rs"), production_code(ROOT / "src/mir/builder/normal_script_program_item_admission.rs"), production_code(RAW_EXPRESSION_DISPATCH), production_code(TOP_LEVEL_ADMISSION), production_code(CONSTRUCTOR_ADMISSION)
    ...
    ratchet = caller_manifest["compatibility_sunsets"]["SCRIPT-EXISTING-ROOT-LOWER-COMPAT-SUNSET-001"]; complete_floor = frozenset("integer_0_real_source script_andor_lexical_closure script_array_literal_compositional_allocation script_await_lexical_closure script_binary_lexical_closure script_binding_rebind script_check_lexical_closure script_compound_binding_rebind script_context_scope_diagnostic_boundary script_enum_declaration_completion script_enum_match_scrutinee script_enum_variant_producer script_fastmem_scope script_final_root_return script_fully_explicit_record_literal script_grouped_binding_rebind script_local_lexical_binding script_map_literal_compositional_mutation script_nowait_lexical_async_binding script_outbox_semantic_materialization script_print_lexical_closure script_prior_local_array_index_write script_pure_block_expr script_root_if_control script_root_match_control script_root_qmark_propagation script_scopebox script_selected_lambda_child_owner script_selected_unsupported_diagnostic_boundary script_static_const_u16_completion script_task_scope_lexical_preflight script_unary_lexical_closure script_weak_reference_compositional".split())
    ...
    assert complete_floor <= ratchet["complete_fixtures"].keys(), "Script Complete fixture ratchet regressed"; require_fixture_map(ratchet["complete_fixtures"]); require_fixture_map(ratchet["deferred_fixtures"], {"script_weak_unary": "UnsafeRuntimeStatement", "script_undefined_variable": "UndefinedVariable", "script_function_call_preflight": "FunctionCallPreflightAuthority"}); profile_residuals = ratchet["script_noncomplete_profile_residuals"]; assert set(profile_residuals) == {"raw_reference_lambda_capture_publication"}, "Script profile residual registry drift"; [require((ROOT / entry["fixture"]["path"]).read_text(), entry["fixture"]["anchor"], f"{entry_id} profile residual fixture") for entry_id, entry in profile_residuals.items() if all(entry.get(key) for key in ("semantic_terminal", "transport_owner", "operation_owner", "release_condition"))] or (_ for _ in ()).throw(AssertionError("Script profile residual ownership incomplete"))
    residuals = ratchet["script_deferred_residuals"]; expected_residuals = {"function_call": ("DirectPortAwareExpression", "FunctionCall", "CallObjectPreflight"), "call": ("DirectPortAwareExpression", "Call", "CallObject"), "method_call": ("DirectPortAwareExpression", "MethodCall", "CallObject"), "loop": ("DirectPortAwareExpression", "Loop", "JoinIrLoop"), "field_access": ("DirectPortAwareExpression", "FieldAccess", "CallObject"), "index": ("DirectPortAwareExpression", "Index", "CallObject"), "new": ("DirectPortAwareExpression", "New", "CallObject"), "record_update": ("DirectPortAwareExpression", "RecordUpdate", "CallObject"), "box_runtime": ("NonPlainInstanceFullLifecycle", "BoxDeclaration", "BoxRuntimeLifecycle"), "try_catch": ("DirectPortAwareExpression", "TryCatch", "ControlResult"), "throw": ("DirectPortAwareExpression", "Throw", "ControlResult"), "nonfinal_return": ("DirectPortAwareExpression", "Return", "ControlResult")}; assert set(residuals) == set(expected_residuals), "Script R4 residual registry drift"
    for residual_id, (admission, shape, family) in expected_residuals.items():
        residual = residuals[residual_id]; assert (residual["admission"], residual["shape"], residual["family"]) == (admission, shape, family), f"{residual_id} Script residual classification drift"; assert all(residual.get(key) for key in ("semantic_terminal", "transport_owner", "operation_owner", "release_condition")), f"{residual_id} Script residual ownership incomplete"; require((ROOT / residual["fixture"]["path"]).read_text(), residual["fixture"]["anchor"], f"{residual_id} residual fixture")
