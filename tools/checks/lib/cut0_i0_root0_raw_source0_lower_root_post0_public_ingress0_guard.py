...
MODULE_LIFECYCLE = ROOT / "src/mir/builder/module_lifecycle.rs"; TOP_LEVEL_ADMISSION = ROOT / "src/mir/builder/normal_top_level_function_admission.rs"
...
def main() -> int:
    ...
    work_plan, script_runtime_work, top_level_admission = production_code(ROOT / "src/mir/builder/program_root_work_plan.rs"), production_code(ROOT / "src/mir/builder/normal_script_runtime_work.rs"), production_code(TOP_LEVEL_ADMISSION)
    ...
    if not all(fragment in work_plan for fragment in ("struct PreparedProgramRootWorkPlanV1", "fn prepare", "fn classify_statement", "ProgramRootTerminalScheduleV1", "PreparedNormalScriptRuntimeWorkV1", "PreparedProgramRootRuntimeWorkV1", "ProgramRootWorkPlanAdmissionV1", "NormalTopLevelFunctionDraftAdmissionV1", "Self::RawCompatibility(statements.into_boxed_slice())", "Self::SelectedNormal(PreparedNormalScriptRuntimeWorkV1::prepare(statements))")) or work_plan.count("NormalTopLevelFunctionDraftAdmissionV1::seal(source_key)") != 1 or not all(fragment in top_level_admission for fragment in ("struct NormalTopLevelFunctionSourceKeyV1", "struct NormalTopLevelFunctionDraftAdmissionV1", "FunctionDraftKeyV1::LegacySymbol", "commit_legacy_symbol_pending", "lower_normal_top_level_function_v1")) or any(fragment in top_level_admission for fragment in ("LegacyChildDraftAdmissionV1", "ResolvedChildDraftAdmissionV1", "retry", "fallback", "NyashParser")) or not all(fragment in script_runtime_work for fragment in ("struct PreparedNormalScriptRuntimeWorkV1", "NormalScriptRuntimeStatementAdmissionV1", "CatalogedNonMainStaticBox", "InstancePrefixCompatibility", "drive_legacy_block_v1", "lower_normal_with_port_v1", "lower_runtime_prefix_with_port_v1")) or any(fragment in script_runtime_work for fragment in ("LegacyChildDraftAdmissionV1", "lower_raw_with_port_v1", "retry", "fallback", "NyashParser")) or "lower_program_statements_with_callable_port_v1" in program_root_lowering: raise AssertionError("Program-root work partition drift")
