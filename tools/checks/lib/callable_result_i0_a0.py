#!/usr/bin/env python3
"""Guard the disconnected callable-result I0-A0 owned activation product."""
...
def main() -> None:
    ...
    require_count(
        ...
        2,
        "explicit unselected producers",
    )
    require_count(activation, "results.disposition(target)", 1, "static result join")
    require_count(activation, "results.call_result(", 0, "instance result-row inference")
    require_count(
        activation,
    ...
    for path in (root / "src").rglob("*.rs"):
        ...
        if "/callable_result_representation/tests/" in relative:
            continue
    ...
    print("[callable-result-i0-a0] ok: plan=1 rows=owned calls=15 selected=2 consumers=0")
