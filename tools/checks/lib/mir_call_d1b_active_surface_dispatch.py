...
from mir_call_d1b_active_surface_dispatch_helpers import (
    ...
    check_wasm_legacy_reader_stop_r0,
...
def dispatch(row: object, state: dict, card: dict, proof: dict, root: Path, api) -> None:
    if row == api.PERFORMANCE_SNAPSHOT_ROW:
    ...
    elif row == api.PUBLISHED_VIEW_C_TRANSPORT_BOXSHAPE_S0_ROW:
        ...
        check_legacy_phi_candidate_retire_r0(state, root, api)
    elif row == api.METHOD_CALL_HANDLERS_POLICY_SPLIT_S0_ROW:
        check_method_call_handlers_policy_split_s0(state, root, api)
        ...
        check_legacy_reader_stop_r0(state, root, api)
    ...
    elif row == api.DOCS_HISTORY_RETIRE_R1_ROW:
        api.check_docs_history_retire_r1(state, card, root)
