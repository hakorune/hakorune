...
def check_verification_coreplan_varmap_reseal_loop_true_bc_r0(
    ...
    """Validate the two-site loop-true break/continue cache-only reseal row."""
    mode = state.get("work_mode")
    if mode not in {"design_stop", "fast", "closeout"}:
        api.fail("CorePlan loop-true reseal mode is invalid")
    if state.get("current_execution_row") != VARMAP_LOOP_TRUE_BC_ROW:
        api.fail("CorePlan loop-true reseal is not selected")
    if mode == "design_stop":
        if state.get("current_design_stop") != VARMAP_LOOP_TRUE_BC_ROW:
            api.fail("CorePlan loop-true reseal design-stop pointer drifted")
        if state.get("next_execution_card") != "none__design_stop":
            api.fail("CorePlan loop-true reseal must not open implementation")
    else:
        if state.get("current_design_stop") != "none":
            api.fail("CorePlan loop-true reseal must clear current_design_stop")
        if state.get("next_execution_card") != VARMAP_LOOP_TRUE_BC_ROW:
            api.fail("CorePlan loop-true reseal execution pointer drifted")
    if state.get("next_execution_card_path") != str(api.CARD_REL):
        api.fail("CorePlan loop-true reseal card path drifted")

    row = card.get(VARMAP_LOOP_TRUE_BC_KEY)
    if not isinstance(row, dict):
        api.fail(f"CorePlan loop-true reseal section is missing: {VARMAP_LOOP_TRUE_BC_KEY}")
    if _text(row, "task_id") != VARMAP_LOOP_TRUE_BC_ROW:
        api.fail("CorePlan loop-true reseal task id drifted")
    status = _text(row, "status")
    expected_statuses = {"active_design_stop"} if mode == "design_stop" else {"fast_open", "landed"}
    if status not in expected_statuses:
        api.fail("CorePlan loop-true reseal status drifted")
    if row.get("implementation_permission") is not (status == "fast_open"):
        api.fail("CorePlan loop-true reseal permission/status drifted")
    if _text(row, "parent_row") != VARMAP_LOOP_COND_BC_ROW:
        api.fail("CorePlan loop-true reseal parent drifted")
    decision = _text(row, "decision").lower()
    for token in ("two", "publish_emission_cache", "current_bindings", "boxshape"):
        if token not in decision:
            api.fail(f"CorePlan loop-true reseal decision lacks {token}")
    if "second owner" not in _text(row, "no_safe_slice").lower():
        api.fail("CorePlan loop-true reseal must reject a second owner")
    allowed = {
    ...
    if set(_list(row, "allowed_files")) != allowed:
        api.fail("CorePlan loop-true reseal allowed-file boundary drifted")

    writes, remove_or_clear = _collect_varmap_sites(root)
    target_path = "src/mir/builder/control_flow/plan/features/loop_true_break_continue_pipeline.rs"
    target = [site for site in writes if site[0] == target_path]
    expected_direct_sites = {0} if status == "landed" else {0, 2}
    if len(target) not in expected_direct_sites or any(site[2] != "insert" for site in target):
        api.fail(
            "CorePlan loop-true reseal source inventory drifted: "
            f"sites={len(target)} expected_one_of={sorted(expected_direct_sites)}"
        )
    if remove_or_clear:
        api.fail("CorePlan loop-true reseal found remove/clear under its boundary")
    source = root / target_path
    if len(source.read_text(encoding="utf-8").splitlines()) > 760:
        api.fail("CorePlan loop-true source reached the 760-line boundary")
    if len(target) == 0 and "publish_emission_cache" not in source.read_text(encoding="utf-8"):
        api.fail("CorePlan loop-true source lacks the canonical cache helper")
    helper = root / "src/mir/builder/control_flow/plan/parts/var_map_scope.rs"
    if "publish_emission_cache" not in helper.read_text(encoding="utf-8"):
        api.fail("CorePlan loop-true reseal helper owner is missing")
    print(
