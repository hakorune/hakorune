...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_rebuild_cache_register_many_box_factory_exact_top_symbol_dominance.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-ring0-global-hooks-exact-top-symbol-dominance] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-ring0-global-hooks-exact-top-symbol-dominance] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-ring0-global-hooks-exact-top-symbol-dominance] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
...
cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-register-many-box_factory-exact-top-symbol-dominance-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_rebuild_cache_register_many_box_factory_exact_top_symbol_dominance
...
if [ "${registry_top_0_mode_count:-0}" -le 0 ]; then
  echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-ring0-global-hooks-exact-top-symbol-dominance] expected aggregate dominant count > 0" >&2
  exit 1
fi
