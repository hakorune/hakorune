...
TAG="rust-lifecycle-mirbuilder-programjson-try-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-try-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonTryShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "TryShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Try statement rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonTryShapeScanV1", "bad owner")
...
need(criteria.get("exception_runtime_semantics") == 0, "exception runtime semantics must remain unclaimed")
need(criteria.get("catch_matching") == 0, "catch matching must remain unclaimed")
need(criteria.get("finally_execution_semantics") == 0, "finally execution semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_try_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-try-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-TRY-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=TryShapeSnapshotV1
shape_scope=covered ProgramJSON Try statement rows
covered_rows=9
