...
guard_require_files "$TAG" "$CARD_408" "$CARD_409" "$CARD_410" "$CARD_411" "$CARD_412" "$CARD_373" "$TOOL" "$INDEX" "$SELF_SCRIPT"
...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_412" "row412 must be current"
...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_412" "row412 must be current"
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "COLLECTION-METHOD-DIRECT-ARRAY-LANE-POST-RETIREMENT-PERF-OWNER-REFRESH-296X-001"' "$STATE" "current state must point to row412"
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-411-COLLECTION-METHOD-DIRECT-ARRAY-LANE-LEGACY-HELPER-CACHE-RETIREMENT-SEMANTIC-SMOKE"' "$STATE" "current state must keep row411 as the latest landed card"
