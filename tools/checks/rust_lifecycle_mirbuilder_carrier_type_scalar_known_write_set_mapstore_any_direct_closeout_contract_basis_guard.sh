...
FIXTURE="$ROOT/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-carrier-type-scalar-known-write-set-mapstore-any-direct-closeout-contract-basis-v0.json"
TOOL="$ROOT/tools/rust_lifecycle/mirbuilder_carrier_type_scalar_known_write_set_mapstore_any_direct_closeout_contract_basis.py"
CARD="$ROOT/docs/development/current/main/phases/phase-296x/2146-MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-ANY-DIRECT-CLOSEOUT-CONTRACT-BASIS-001.md"
...
TAG="rust-lifecycle-mirbuilder-carrier-type-scalar-known-write-set-mapstore-any-direct-closeout-contract-basis"
...
token = "MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-ANY-DIRECT-CLOSEOUT-CONTRACT-BASIS-001"
next_card = "MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-ANY-DIRECT-CLOSEOUT-RERUN-001"

need(fixture.get("kind") == "MirBuilderCarrierTypeScalarKnownWriteSetMapStoreAnyDirectCloseoutContractBasisV1", "bad kind")
need(fixture.get("token") == token, "bad token")
need(token in card, "card missing token")

inputs = fixture.get("input_state") or {}
need(inputs.get("selected_scoped_surface") == "SetSurfacePolicy/MapStoreAny", "selected surface drift")
...
need(contract.get("route_kind_set") == ["MapStoreAny"], "route set drift")
need(contract.get("proof_or_policy_source") == ["SetSurfacePolicy", "AnyWriteBoundaryDeclared"], "policy source drift")
...
need(contract.get("mapstore_any_included") is True, "MapStoreAny inclusion drift")
need(contract.get("any_write_boundary_declared") is True, "Any boundary declaration drift")
...
need(summary.get("write_set_mapstore_any_direct_closeout_contract_basis") == 1, "summary basis drift")
need(summary.get("write_set_mapstore_any_route_count") == 1, "summary route count drift")
need(summary.get("any_write_boundary_declared") == 1, "Any boundary declaration drift")
for key in [
    "any_write_boundary_opened",
    "direct_contract_materialized",
    "write_set_mapstore_any_direct_closeout_ready",
...
need(decision.get("kind") == "SelectWriteSetMapStoreAnyDirectCloseoutRerun", "decision kind drift")
need(decision.get("reason_token") == "WriteSetMapStoreAnyDirectCloseoutContractBasisDefined", "reason drift")
need(decision.get("selected_next_card") == next_card, "next drift")

claims = fixture.get("claims") or {}
for key in ["write_set_mapstore_any_direct_closeout_contract_basis", "basis_only", "any_write_boundary_declared"]:
...
need(manifest_row.get("card", "").endswith("2146-MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-ANY-DIRECT-CLOSEOUT-CONTRACT-BASIS-001.md"), "manifest card drift")
need(manifest_row.get("fixture", "").endswith("mirbuilder-carrier-type-scalar-known-write-set-mapstore-any-direct-closeout-contract-basis-v0.json"), "manifest fixture drift")
need(manifest_row.get("legacy_guard", "").endswith("rust_lifecycle_mirbuilder_carrier_type_scalar_known_write_set_mapstore_any_direct_closeout_contract_basis_guard.sh"), "manifest guard drift")

need(token in task_order, "task order missing token")
need(f"selected_next_card={next_card}" in task_order, "task order next drift")

print("output_contract=rust-lifecycle-mirbuilder-carrier-type-scalar-known-write-set-mapstore-any-direct-closeout-contract-basis")
print("write_set_mapstore_any_direct_closeout_contract_basis=1")
print("write_set_mapstore_any_route_count=1")
