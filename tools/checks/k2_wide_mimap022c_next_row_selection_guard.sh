...
guard_expect_in_file "$TAG" 'Status: ready' "$NEXT_CARD" "MIMAP-023A card must be ready"
...
guard_expect_in_file "$TAG" 'phase_status = "docs/development/current/main/phases/phase-293x/293x-436-MIMAP-023A-FACADE-HUGE-PAGE-MODEL-ROUTE.md"' "$STATE" "CURRENT_STATE must move phase_status to MIMAP-023A"
guard_expect_in_file "$TAG" 'current_blocker_token = "MIMAP-023A"' "$STATE" "CURRENT_STATE must set MIMAP-023A as blocker"
guard_expect_in_file "$TAG" 'latest_card = "293x-435-MIMAP-022C-POST-HUGE-FAILFAST-ROW-SELECTION"' "$STATE" "CURRENT_STATE latest card must close MIMAP-022C"
guard_expect_in_file "$TAG" 'MIMAP-023A facade huge-page model route' "$CURRENT_TASK" "CURRENT_TASK must restart on MIMAP-023A"
guard_expect_in_file "$TAG" 'MIMAP-023A facade huge-page model route' "$RESUME" "quick resume must restart on MIMAP-023A"
guard_expect_in_file "$TAG" 'MIMAP-023A facade huge-page model route' "$NOW" "Now mirror must restart on MIMAP-023A"
guard_expect_in_file "$TAG" '293x-436-MIMAP-023A-FACADE-HUGE-PAGE-MODEL-ROUTE.md' "$PHASE_README" "phase README must point at MIMAP-023A as current blocker"
...
guard_expect_in_file "$TAG" '| `MIMAP-023A` | ready current | Facade huge-page model route using the existing M180 huge-page model owner. | current |' "$TASKBOARD" "taskboard must mark MIMAP-023A current"
