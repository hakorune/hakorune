...
TAG="rust-lifecycle-mirbuilder-programjson-recipematcher-unsupported-condition-operator-reject-row"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipematcher-unsupported-condition-operator-reject-row-v0.json"
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/3255-MIRBUILDER-PROGRAMJSON-RECIPEMATCHER-UNSUPPORTED-CONDITION-OPERATOR-REJECT-ROW-001.md"
TASK_ORDER="$ROOT_DIR/docs/development/current/main/design/mirbuilder-rust-to-hako-converter-task-order-ssot.md"
CURRENT_STATE="$ROOT_DIR/docs/development/current/main/CURRENT_STATE.toml"
PREV_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_recipematcher_reject_floor_expansion_selection_guard.sh"
...
if ! grep -q '^selected_next_card=MIRBUILDER-PROGRAMJSON-RECIPEMATCHER-UNSUPPORTED-CONDITION-OPERATOR-REJECT-ROW-001$' <<<"$PREV_OUT"; then
  printf '%s\n' "$PREV_OUT" >&2
  guard_fail "$TAG" "unsupported-condition selection prerequisite is not green"
fi

TMP_DIR="$(mktemp -d /tmp/hakorune-unsupported-cond-reject.XXXXXX)"
...
fixture_path, card_path, task_order_path, current_state_path, snapshot_path, loop_handler_path, matcher_path, app_path = sys.argv[1:]
...
token = "MIRBUILDER-PROGRAMJSON-RECIPEMATCHER-UNSUPPORTED-CONDITION-OPERATOR-REJECT-ROW-001"
if fixture.get("kind") != "MirBuilderProgramJsonRecipeMatcherUnsupportedConditionOperatorRejectRowV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != token:
    raise SystemExit("bad fixture token")
row = fixture.get("row") or {}
if row.get("row_id") != "unsupported_condition_operator_reject":
    raise SystemExit("bad row id")
program_json = row.get("program_json")
if not isinstance(program_json, dict):
    raise SystemExit("program_json must be structured")
loop = program_json["body"][1]
if loop.get("cond", {}).get("op") != "<=":
    raise SystemExit("loop condition operator drift")
if row.get("expected_snapshot", {}).get("reason") != "unsupported_loop_cond":
...
positive = {
    "unsupported_condition_operator_reject_row_green",
    "reject_floor_row_green",
    "phase_state_allows_verified_recipe_for_unsupported_loop_cond",
...
for needle in [token, "MIRBUILDER-PROGRAMJSON-RECIPEMATCHER-REJECT-FLOOR-UPDATE-AXIS-SELECTION-001"]:
...
program_json_literal = json.dumps(json.dumps(program_json, separators=(",", ":"), ensure_ascii=False))
app.write_text("\n".join([
    ...
    "static box Main {",
    "  main() {",
    f"    local program_json = {program_json_literal}",
...
if ! timeout 90s env HAKO_MIR_JSON_CACHE=0 HAKO_EMIT_EXE_CACHE=0 \
    bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
...
joined = "\n".join(lines)
for needle in [
    "snapshot:snapshot_kind=ProgramJsonCanonicalLoopFactsInputSnapshotV1;ok=0;reason=unsupported_loop_cond",
    ";matcher_input_present=0",
    ";cond_kind=Unsupported",
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-recipematcher-unsupported-condition-operator-reject-row-v0
token=MIRBUILDER-PROGRAMJSON-RECIPEMATCHER-UNSUPPORTED-CONDITION-OPERATOR-REJECT-ROW-001
row_id=unsupported_condition_operator_reject
unsupported_condition_operator_reject_row_green=1
reject_floor_row_green=1
phase_state_allows_verified_recipe_for_unsupported_loop_cond=1
