...
TMP_DIR="$(mktemp -d /tmp/hakorune_nyrt_startup_floor_ab_guard.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[nyrt-startup-floor-ab] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_key() {
  ...
  if ! grep -q "^${key}=" "$file"; then
    echo "[nyrt-startup-floor-ab] missing key in ${file#$ROOT_DIR/}: $key" >&2
    exit 1
  fi
}
...
require_not_missing() {
  ...
  if grep -q "^${key}=missing$" "$file"; then
    echo "[nyrt-startup-floor-ab] key unexpectedly missing in ${file#$ROOT_DIR/}: $key" >&2
    exit 1
  fi
}
...
require_line "$REPORT" "measurement_scope=nyrt_entry_startup_floor"
require_line "$REPORT" "observation_only=1"
require_line "$REPORT" "rewrite_executed=0"
require_line "$REPORT" "default_behavior_changed=0"
require_line "$REPORT" "same_ny_main_object=1"
require_line "$REPORT" "entry_a=current_minimal_nyrt"
require_line "$REPORT" "entry_b=bare_libc_main"
...
require_key "$REPORT" "ny_main_object_sha256"
...
require_key "$REPORT" "current_minimal_top_0_symbol"
require_key "$REPORT" "bare_entry_top_0_symbol"
