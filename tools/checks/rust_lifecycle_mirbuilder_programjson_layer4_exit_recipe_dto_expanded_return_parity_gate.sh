...
TAG="rust-lifecycle-mirbuilder-programjson-layer4-exit-recipe-dto-expanded-return-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-layer4-exit-recipe-dto-expanded-return-parity-v0.json"
SNAPSHOT_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_exit_recipe_dto_snapshot.hako"
BASE_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_seq_recipe_dto_expanded_return_retire_rust_astnode_projector_candidate_guard.sh"
LEGACY_EXIT_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_exit_recipe_dto_parity_gate.sh"
LOOP_EXIT_GATE="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_exit_recipe_dto_loop_body_parity_gate.sh"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-layer4-exit-expanded-return-dto.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/layer4_exit_expanded_return_dto_probe.hako"
MIR_JSON="$TMP_DIR/layer4_exit_expanded_return_dto_probe.mir.json"
EXE="$TMP_DIR/layer4_exit_expanded_return_dto_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonLayer4ExitRecipeDtoExpandedReturnParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-LAYER4-EXIT-RECIPE-DTO-EXPANDED-RETURN-PARITY-001":
    raise SystemExit("bad fixture token")

rows = fixture.get("rows") or []
if len(rows) != 6:
    raise SystemExit("expanded Exit Recipe DTO return parity requires exactly 6 rows")
...
for row in rows:
    ...
    for token in [";err=0", ";matched=1", ";exit_path=root.if.then", ";payload_return_kind="]:
...
for needle in [
    ...
    if needle not in text:
        raise SystemExit(f"missing Exit DTO expanded token: {needle}")
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for expanded Exit Recipe DTO probe"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-layer4-exit-recipe-dto-expanded-return-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-LAYER4-EXIT-RECIPE-DTO-EXPANDED-RETURN-PARITY-001
