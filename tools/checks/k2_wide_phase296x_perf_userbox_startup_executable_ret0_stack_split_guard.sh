...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_stack_split.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-stack-split] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-stack-split] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
ret0_top_0_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_top_0_symbol" { print $2; exit }' "$STARTUP_REPORT")"
ret0_top_1_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_top_1_symbol" { print $2; exit }' "$STARTUP_REPORT")"
ret0_top_2_symbol="$(awk -F= '$1=="startup_loader_ret0_exe_top_2_symbol" { print $2; exit }' "$STARTUP_REPORT")"
...
if [ "$ret0_top_count" -lt 3 ]; then
  echo "[perf-userbox-startup-executable-ret0-stack-split] expected at least three ret0.exe rows in ${STARTUP_REPORT#$ROOT_DIR/}" >&2
  exit 1
fi
if [ "$ret0_top_0_symbol" = "missing" ] || [ "$ret0_top_1_symbol" = "missing" ] || [ "$ret0_top_2_symbol" = "missing" ]; then
  echo "[perf-userbox-startup-executable-ret0-stack-split] expected three ret0.exe symbols in ${STARTUP_REPORT#$ROOT_DIR/}" >&2
  exit 1
fi

cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-stack-split-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_stack_split
...
startup_loader_ret0_exe_top_0_symbol=$ret0_top_0_symbol
startup_loader_ret0_exe_top_1_symbol=$ret0_top_1_symbol
startup_loader_ret0_exe_top_2_symbol=$ret0_top_2_symbol
