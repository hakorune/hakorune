...
TAG="k2-wide-mimalloc-ptr-remote-free-list-exe"
cd "$ROOT_DIR"
source "$ROOT_DIR/tools/checks/lib/pure_first_exe_guard.sh"

APP="apps/mimalloc-ptr-remote-free-list-proof/main.hako"
APP_README="apps/mimalloc-ptr-remote-free-list-proof/README.md"
CARD="docs/development/current/main/phases/phase-293x/293x-093-M41-POINTER-CAS-REMOTE-FREE-LIST-PROOF.md"
TASKBOARD="docs/development/current/main/design/mimalloc-capability-taskboard-ssot.md"

echo "[$TAG] running M41 pointer CAS remote-free list EXE guard"

guard_require_files "$TAG" "$APP" "$APP_README" "$CARD" "$TASKBOARD"

if rg -n 'mimalloc-ptr-remote-free-list-proof' lang/c-abi/shims >/tmp/"$TAG".app_specific.inc 2>&1; then
  echo "[$TAG] ERROR: app-specific pointer remote-free list matcher leaked into .inc" >&2
...
if rg -n 'hako_atomic_ptr_fetch_add|ptr_fetch_add' \
  ...
  echo "[$TAG] ERROR: pointer atomic fetch_add rows must stay inactive in M41" >&2
...
tmp_dir="$(mktemp -d /tmp/hakorune_m41_ptr_remote_free_list.XXXXXX)"
trap 'rm -rf "$tmp_dir"' EXIT

mir_json="$tmp_dir/m41.mir.json"
exe_out="$tmp_dir/m41.exe"
...
seen_route_ids = {route.get("route_id") for route in routes}
unexpected = sorted(r for r in seen_route_ids if r not in allowed_route_ids)
if unexpected:
    raise SystemExit(f"unexpected extern route ids: {unexpected}")
...
for symbol, (route_id, core_op, arity, ret, demand, effects) in expected_routes.items():
    ...
    matching_plans = [
        ...
        if plan.get("source") == "extern_call_routes"
...
rg -F -q 'mimalloc-ptr-remote-free-list-proof' "$run_log"
rg -F -q 'stores=0,0,0' "$run_log"
rg -F -q 'first=1,1,1,1' "$run_log"
rg -F -q 'second=1,1,1,1,1' "$run_log"
rg -F -q 'summary=ok' "$run_log"

rg -F -q '| `M41 pointer CAS remote-free list proof` | `live-narrow` |' "$TASKBOARD"
rg -F -q 'M41 Pointer CAS Remote-Free List Proof' "$CARD"
rg -F -q 'hako_atomic_ptr_cas_ordered' "$APP_README"
rg -F -q 'k2_wide_mimalloc_ptr_remote_free_list_exe_guard.sh' docs/tools/check-scripts-index.md
