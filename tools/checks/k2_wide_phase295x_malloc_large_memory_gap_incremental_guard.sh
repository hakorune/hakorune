...
guard_expect_in_file "$TAG" 'Status: Current' "$CARD" "card must remain current while closeout is open"
...
guard_expect_in_file "$TAG" '| 192 | `MIMALLOC-COMPARISON-MALLOC-LARGE-MEMORY-GAP-INCREMENTAL-295X-001` | Current |' "$TASKBOARD" "taskboard must expose the incremental row"
