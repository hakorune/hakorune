...
TAG="rust-lifecycle-mirbuilder-programjson-local-map-literal-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-local-map-literal-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonLocalMapLiteralShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "LocalMapLiteralShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Local.expr Map literal rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonLocalMapLiteralShapeScanV1", "bad owner")
...
need(criteria.get("map_lowering") == 0, "map lowering must remain unclaimed")
need(criteria.get("map_allocation_semantics") == 0, "map allocation semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_local_map_literal_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-local-map-literal-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-LOCAL-MAP-LITERAL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=LocalMapLiteralShapeSnapshotV1
shape_scope=covered ProgramJSON Local.expr Map literal rows
covered_rows=10
