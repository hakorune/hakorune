...
require "$PHYSICAL" 'demand_tickets'
...
require "$RUNTIME_DEMAND" 'constructor_demand_tickets'
...
for file in "$TRANSFER" "$WINDOW" "$LIFECYCLE" "$PHYSICAL" "$DEMAND" "$RUNTIME_WORK" "$RUNTIME_DEMAND" "$WORK_PLAN"; do
  lines="$(wc -l < "$file")"
  if (( lines >= 760 )); then
