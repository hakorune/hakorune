...
TAG="rust-lifecycle-mirbuilder-programjson-context-scope-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-context-scope-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonContextScopeShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ContextScopeShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON ContextScope statement rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonContextScopeShapeScanV1", "bad owner")
...
need(criteria.get("context_lowering") == 0, "context lowering must remain 0")
need(criteria.get("context_runtime_semantics") == 0, "context runtime semantics must remain 0")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_context_scope_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-context-scope-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-CONTEXT-SCOPE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ContextScopeShapeSnapshotV1
shape_scope=covered ProgramJSON ContextScope statement rows
covered_rows=10
