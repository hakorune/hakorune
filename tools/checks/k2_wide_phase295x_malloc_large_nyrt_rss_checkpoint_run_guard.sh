...
TAG="k2-wide-phase295x-malloc-large-nyrt-rss-checkpoint-run"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-198-MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-RUN.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-197-MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-DIAGNOSTIC.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_malloc_large_nyrt_rss_checkpoint_run_guard.sh"
APP="apps/hako-alloc-mimalloc-comparison-empty-noio-exe-proof/main.hako"

echo "[$TAG] checking phase-295x malloc-large NyRT RSS checkpoint run"

guard_require_files "$TAG" "$CARD" "$PREV_CARD" "$TASKBOARD" "$INDEX" "$SELF_SCRIPT" "$APP"
guard_require_exec_files "$TAG" "$SELF_SCRIPT"

guard_expect_in_file "$TAG" 'Status: Current' "$CARD" "card must remain current until the plugin-host baseline seam is opened"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-RUN-295X-002' "$CARD" "card must identify the current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-BASELINE-SELECTION-295X-002' "$CARD" "card must select plugin-host follow-on"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-RUN-295X-002' "$PREV_CARD" "previous row must select this run"
guard_expect_in_file "$TAG" 'after_plugin_host' "$CARD" "card must identify plugin-host jump"
guard_expect_in_file "$TAG" '| 197 | `MIMALLOC-COMPARISON-NYRT-RSS-CHECKPOINT-DIAGNOSTIC-295X-001` | Landed |' "$TASKBOARD" "taskboard must retain selected follow-on as landed"
