...
guard_expect_in_file "$TAG" 'parse_range_header' "$PARSER" "parser must parse loop range header"
guard_expect_in_file "$TAG" 'ASTNode::LoopRange' "$PARSER" "parser must transport canonical LoopRange AST metadata"
