...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_382A" "$CARD_383A" "$CARD" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MEMORY_README" "$OWNER" "$PREV_OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
...
guard_expect_in_file "$TAG" 'provider_call_dry_run_unsupported_behavior_box.hako' "$MEMORY_README" "memory README must name provider-call dry-run owner"
