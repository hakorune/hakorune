...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_371" "perf owner refresh card must be current"
...
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "ARRAY-SLOT-NATIVEDIRECT-POST-SEMANTIC-PERF-OWNER-REFRESH-296X-001"' "$STATE" "current state must point to row371"
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-370-ARRAY-SLOT-NATIVEDIRECT-SELECTED-METHOD-SEMANTIC-SMOKE"' "$STATE" "current state must keep row370 as latest landed card"
