...
TAG="k2-wide-phase295x-huge-ish-evidence-run"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-26-MIMALLOC-COMPARISON-HUGE-ISH-EVIDENCE-RUN.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-25-MIMALLOC-COMPARISON-HUGE-ISH-CONTRACT-REFRESH.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_huge_ish_evidence_run_guard.sh"
...
echo "[$TAG] checking phase-295x huge-ish evidence run"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-HUGE-ISH-EVIDENCE-295X-RUN-001' "$CARD" "card must identify the current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-HUGE-ISH-CLOSEOUT-295X-001' "$CARD" "card must select closeout follow-on"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-HUGE-ISH-EVIDENCE-295X-RUN-001' "$PREV_CARD" "previous row must select this evidence run"
guard_expect_in_file "$TAG" '| 26 | `295x-26` | Landed |' "$TASKBOARD" "taskboard must retain this row as landed"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list this guard"

tmp_dir="$(mktemp -d /tmp/hakorune_phase295x_huge_evidence.XXXXXX)"
...
rg -F -q 'c_requested_bytes=4194321' "$report_out"
rg -F -q 'hako_large_request_count=1' "$report_out"
rg -F -q 'c_large_request_count=1' "$report_out"
...
print("[phase295x-huge-ish-evidence] ok")
PY
