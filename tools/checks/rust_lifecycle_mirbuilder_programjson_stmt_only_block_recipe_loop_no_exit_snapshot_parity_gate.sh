...
TAG="rust-lifecycle-mirbuilder-programjson-stmt-only-block-recipe-loop-no-exit-snapshot-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-stmt-only-block-recipe-loop-no-exit-snapshot-parity-v0.json"
SNAPSHOT_IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_stmt_only_block_recipe_snapshot.hako"
SELECTION_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_block_recipe_next_capability_selection_rerun_001_guard.sh"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-loop-no-exit-block-recipe.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_loop_no_exit_block_recipe_probe.hako"
MIR_JSON="$TMP_DIR/programjson_loop_no_exit_block_recipe_probe.mir.json"
EXE="$TMP_DIR/programjson_loop_no_exit_block_recipe_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonStmtOnlyBlockRecipeLoopNoExitSnapshotParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-STMT-ONLY-BLOCK-RECIPE-LOOP-NO-EXIT-SNAPSHOT-PARITY-001":
...
if len(rows) != 1:
    raise SystemExit("LoopNoExit ProgramJSON parity requires exactly 1 row")
...
for needle in [
    ...
    "BoxHelpers.map_get(item1, \"body_item\")",
    "LoopNoExit",
    "StmtOnlyBlockRecipeBox.build_summary(stmt0, stmt1)",
]:
    if needle not in text:
        raise SystemExit(f"missing LoopNoExit bridge token: {needle}")
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for ProgramJSON LoopNoExit block recipe probe"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-stmt-only-block-recipe-loop-no-exit-snapshot-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-STMT-ONLY-BLOCK-RECIPE-LOOP-NO-EXIT-SNAPSHOT-PARITY-001
