...
TAG="rust-lifecycle-mirbuilder-programjson-assignment-value-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-assignment-value-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonAssignmentValueShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "AssignmentValueShapeSnapshotV1", "bad retire candidate")
need(
    scope.get("shape_scope")
    == "covered ProgramJSON Local.name + Local.expr assignment-value rows",
    "bad shape scope",
)
need(scope.get("programjson_snapshot_owner") == "ProgramJsonAssignmentValueShapeScanV1", "bad owner")
...
need(criteria.get("legacy_assign_proof") == 0, "legacy Assign proof must remain 0")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_assignment_value_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-assignment-value-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-ASSIGNMENT-VALUE-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=AssignmentValueShapeSnapshotV1
shape_scope=covered ProgramJSON Local.name + Local.expr assignment-value rows
covered_rows=9
