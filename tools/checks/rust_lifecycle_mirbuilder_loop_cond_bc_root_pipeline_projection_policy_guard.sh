...
TAG="rust-lifecycle-mirbuilder-loop-cond-bc-root-pipeline-projection-policy-guard"
source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-loop-cond-bc-root-pipeline-projection-policy-v0.json"
...
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/1845-MIRBUILDER-LOOP-COND-BC-ROOT-PIPELINE-PROJECTION-POLICY-001.md"
...
fixture = json.loads(Path("docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-loop-cond-bc-root-pipeline-projection-policy-v0.json").read_text())
...
card = Path("docs/development/current/main/phases/phase-296x/1845-MIRBUILDER-LOOP-COND-BC-ROOT-PIPELINE-PROJECTION-POLICY-001.md").read_text()
...
token = "MIRBUILDER-LOOP-COND-BC-ROOT-PIPELINE-PROJECTION-POLICY-001"
if fixture.get("kind") != "MirBuilderLoopCondBcRootPipelineProjectionPolicyV1":
...
if fixture.get("decision", {}).get("selected_next_card") != "MIRBUILDER-LOOP-COND-BC-CARRIER-SYNC-PROJECTION-POLICY-001":
    raise SystemExit("selected next card drift")

report_items = {
    item["source_id"]: item
    for item in report.get("items", [])
    if item.get("loop_cond_bc_pipeline_subcluster") == "LoopCondBcRootPipelineCluster"
}
surfaces = fixture.get("source_surfaces") or []
if len(surfaces) != 1:
    raise SystemExit("expected exactly one root pipeline surface")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-loop-cond-bc-root-pipeline-projection-policy-v0
