...
[[ $# -le 1 ]] || fail "usage: $0 [reference_child_i0|canonical_v1_value_s0|core_direct_substring_product_aot_s0|core_direct_retire_r0|force_hv1_fate_d0|force_hv1_direct_historical_delete_r0|force_hv1_guard_current_lifecycle|wpre_readiness]"
PHASE="${1:-wpre_readiness}"
case "$PHASE" in
  wpre_readiness|reference_child_i0|canonical_v1_value_s0|core_direct_substring_product_aot_s0|core_direct_retire_r0|force_hv1_fate_d0|force_hv1_direct_historical_delete_r0|force_hv1_guard_current_lifecycle) ;;
...
if [[ "$PHASE" == "force_hv1_fate_d0" || "$PHASE" == "force_hv1_direct_historical_delete_r0" || "$PHASE" == "force_hv1_guard_current_lifecycle" ]]; then
  python3 "$ROOT_DIR/tools/checks/lib/force_hv1_caller_manifest_guard.py" \
    "$CARD" "$FORCE_MANIFEST" "$STATE" "$WORKSTREAM" "$PHASE"
  exit 0
fi
