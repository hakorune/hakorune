...
TMP_DIR="$(mktemp -d /tmp/hakorune_nyrt_entry_path_prep_off_guard.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[nyrt-entry-path-prep-off] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_not_missing() {
  ...
  if grep -q "^${key}=missing$" "$file"; then
    echo "[nyrt-entry-path-prep-off] key unexpectedly missing in ${file#$ROOT_DIR/}: $key" >&2
    exit 1
  fi
}
