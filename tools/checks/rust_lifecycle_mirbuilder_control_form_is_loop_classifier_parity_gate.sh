...
TAG="rust-lifecycle-mirbuilder-control-form-is-loop-classifier-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-control-form-is-loop-classifier-rust-oracle-v0.json"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-control-form-is-loop-classifier.XXXXXX)"
cleanup() {
  rm -rf "$TMP_DIR" >/dev/null 2>&1 || true
}
trap cleanup EXIT

APP="$TMP_DIR/control_form_is_loop_classifier_parity.hako"
EXPECTED="$TMP_DIR/expected.txt"
ACTUAL="$TMP_DIR/actual.txt"
EXE="$TMP_DIR/control_form_is_loop_classifier_parity.exe"
...
for idx, row in enumerate(rows):
    ...
    if kind == "loop":
        ...
        calls.append(f"    local {cf_name} = new ControlFormBox()")
        calls.append(f"    {cf_name}.from_if(3, 4, 5, 6)")
    calls.append(f"    print({cf_name}.is_loop())")
...
if actual != expected:
    print("[control-form-is-loop-classifier/parity] mismatch")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-control-form-is-loop-classifier-parity-gate-v0
owner=control_form_is_loop_classifier
rust_oracle_fixture=mirbuilder-control-form-is-loop-classifier-rust-oracle-v0.json
