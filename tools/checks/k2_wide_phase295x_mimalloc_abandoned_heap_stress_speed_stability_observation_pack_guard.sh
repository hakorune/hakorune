...
guard_require_files "$TAG" "$CARD" "$PREV_CARD" "$INDEX" "$SELF_SCRIPT" "$RUNNER" "$HAKO_RUNNER" "$C_RUNNER"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-SPEED-STABILITY-OBSERVATION-PACK-295X-002' "$CARD" "card must identify the observation blocker"
...
guard_expect_in_file "$TAG" '1ms / 1ms' "$CARD" "card must record the elapsed floor observation"
...
guard_expect_in_file "$TAG" 'Status: Landed' "$PREV_CARD" "previous closeout row must be landed before the observation row is exercised"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-ABANDONED-HEAP-STRESS-SPEED-STABILITY-OBSERVATION-PACK-295X-002' "$PREV_CARD" "previous row must select this observation pack"
guard_expect_in_file "$TAG" 'external_elapsed_median_ms' "$CARD" "card must preserve the elapsed evidence"
guard_expect_in_file "$TAG" 'external_elapsed_ms' "$RUNNER" "repeated runner must still expose elapsed evidence"
