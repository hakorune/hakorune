...
TAG="k2-wide-phase295x-hako-empty-exe-footprint-diagnostic"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-38-MIMALLOC-COMPARISON-HAKO-EMPTY-EXE-FOOTPRINT-DIAGNOSTIC.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-37-MIMALLOC-COMPARISON-HAKO-BASELINE-BREAKDOWN-SELECTION.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_hako_empty_exe_footprint_diagnostic_guard.sh"
...
echo "[$TAG] checking phase-295x hako empty EXE footprint diagnostic"
...
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-HAKO-EMPTY-EXE-FOOTPRINT-DIAGNOSTIC-295X-001' "$CARD" "card must identify the current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-HAKO-EMPTY-EXE-FOOTPRINT-CLOSEOUT-295X-001' "$CARD" "card must select closeout follow-on"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-HAKO-EMPTY-EXE-FOOTPRINT-DIAGNOSTIC-295X-001' "$PREV_CARD" "previous row must select this diagnostic row"
...
print("[phase295x-hako-empty-exe-footprint-diagnostic] ok")
PY
