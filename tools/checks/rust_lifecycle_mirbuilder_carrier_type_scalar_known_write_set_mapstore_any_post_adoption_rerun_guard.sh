...
FIXTURE="$ROOT/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-carrier-type-scalar-known-write-set-mapstore-any-post-adoption-rerun-v0.json"
TOOL="$ROOT/tools/rust_lifecycle/mirbuilder_carrier_type_scalar_known_write_set_mapstore_any_post_adoption_rerun.py"
CARD="$ROOT/docs/development/current/main/phases/phase-296x/2145-MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-ANY-POST-ADOPTION-RERUN-001.md"
TASK_ORDER="$ROOT/docs/development/current/main/design/mirbuilder-rust-to-hako-converter-task-order-ssot.md"
MANIFEST="$ROOT/docs/development/current/main/design/fixtures/rust-lifecycle/source-selfhost-family-guard-manifest-v0.json"

source "$ROOT/tools/checks/lib/guard_common.sh"

TAG="rust-lifecycle-mirbuilder-carrier-type-scalar-known-write-set-mapstore-any-post-adoption-rerun"
...
token = "MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-ANY-POST-ADOPTION-RERUN-001"
next_card = "MIRBUILDER-CARRIER-TYPE-SCALAR-KNOWN-WRITE-SET-MAPSTORE-ANY-DIRECT-CLOSEOUT-CONTRACT-BASIS-001"

need(fixture.get("kind") == "MirBuilderCarrierTypeScalarKnownWriteSetMapStoreAnyPostAdoptionRerunV1", "bad kind")
need(fixture.get("token") == token, "bad token")
need(token in card, "card missing token")

inputs = fixture.get("input_state") or {}
need(inputs.get("adoption_decision") == "Adopt", "adoption decision drift")
need(inputs.get("adopted_surface") == "SetSurfacePolicy/MapStoreAny", "adopted surface drift")
need(inputs.get("adopted_owner") == "write_set_mapstore_any_policy_classifier", "adopted owner drift")
...
need(rows["SetSurfacePolicy/MapStoreAny"].get("basis_selection_eligible") is True, "MapStoreAny eligibility drift")
need(rows["SetSurfacePolicy/MapStoreAny"].get("any_write_boundary_opened") is False, "Any boundary opened drift")
need(rows["SetSurfacePolicy/MapStoreAny"].get("any_write_boundary_declared") is True, "Any boundary declared drift")
...
need(rule.get("undeclared_any_write_boundary_not_eligible") is True, "Any boundary rule drift")
...
need(summary.get("write_set_mapstore_any_hako_adopted") == 1, "MapStoreAny adoption summary drift")
need(summary.get("basis_selection_eligible_surface_count") == 1, "eligible count drift")
need(summary.get("selected_scoped_surface") == "SetSurfacePolicy/MapStoreAny", "selected surface drift")
need(summary.get("any_write_boundary_declared") == 1, "Any boundary declaration drift")
for key in [
    "any_write_boundary_opened",
    "write_set_mapstore_any_direct_closeout_materialized",
...
need(decision.get("kind") == "SelectWriteSetMapStoreAnyDirectCloseoutContractBasis", "decision kind drift")
need(decision.get("reason_token") == "ExactlyOneHakoAdoptedSetMapStoreAnyPilotNeedsScopedCloseout", "reason drift")
need(decision.get("selected_scoped_surface") == "SetSurfacePolicy/MapStoreAny", "decision surface drift")
need(decision.get("selected_next_card") == next_card, "next drift")

claims = fixture.get("claims") or {}
for key in [
    "write_set_mapstore_any_post_adoption_rerun",
...
print("selected_scoped_surface=SetSurfacePolicy/MapStoreAny")
print("any_write_boundary_declared=1")
print("any_write_boundary_opened=0")
print("write_set_mapstore_any_direct_closeout_materialized=0")
