...
TAG="rust-lifecycle-mirbuilder-programjson-recipebodies-one-shape-arena-builder-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipebodies-one-shape-arena-builder-parity-v0.json"
SELECTION_FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipebodies-after-minimal-dto-next-contract-selection-v0.json"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-recipebodies-one-shape-arena.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/recipebodies_one_shape_arena_probe.hako"
MIR_JSON="$TMP_DIR/recipebodies_one_shape_arena_probe.mir.json"
EXE="$TMP_DIR/recipebodies_one_shape_arena_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonRecipeBodiesOneShapeArenaBuilderParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-RECIPEBODIES-ONE-SHAPE-ARENA-BUILDER-PARITY-001":
...
if claims.get("one_shape_arena_builder_implemented") != 1:
    raise SystemExit("implementation claim missing")
for key, value in claims.items():
    if key == "one_shape_arena_builder_implemented":
        continue
    if value != 0:
...
if len(rows) != 3:
    raise SystemExit("one-shape arena builder parity requires exactly 3 rows")

lines = [
    "using lang.compiler.mirbuilder.program_json_recipebodies_one_shape_arena_builder as ProgramJsonRecipeBodiesOneShapeArenaBuilderBox",
    "",
    "static box Main {",
    "  main() {",
]
...
for row in rows:
    ...
    for token in [
        "snapshot_kind=ProgramJsonRecipeBodiesOneShapeArenaBuilderV1;",
        ";err=0",
        ";root_body_id=0",
        ";body_count=1",
        ";body0_item_count=",
    ...
    lines.append(
        "    print(\"arena:"
        + row_id
        + ":\" + ProgramJsonRecipeBodiesOneShapeArenaBuilderBox.build_summary("
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for RecipeBodies one-shape arena probe"
fi
...
builder = next((fn for fn in functions if fn.get("name") == "ProgramJsonRecipeBodiesOneShapeArenaBuilderBox.build_summary/1"), None)
arena = next((fn for fn in functions if fn.get("name") == "ProgramJsonRecipeBodiesOneShapeArenaBuilderBox.build_arena/1"), None)
if main is None:
...
main_routes = [row for row in route_rows(main) if row.get("symbol") == "ProgramJsonRecipeBodiesOneShapeArenaBuilderBox.build_summary/1"]
if len(main_routes) < 1:
    raise SystemExit("main does not call one-shape arena summary")

bad = []
for fn in (builder, arena):
    bad.extend(row for row in route_rows(fn) if row.get("tier") == "Unsupported" or row.get("reason"))
if bad:
    raise SystemExit(f"one-shape arena functions have unsupported routes: {bad[:5]}")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-recipebodies-one-shape-arena-builder-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-RECIPEBODIES-ONE-SHAPE-ARENA-BUILDER-PARITY-001
