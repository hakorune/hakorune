...
TAG="rust-lifecycle-mirbuilder-accum-const-loop-facts-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-accum-const-loop-facts-rust-oracle-v0.json"
HAKO_IMPL="$ROOT_DIR/lang/src/compiler/lib/accum_const_loop_facts.hako"
HAKO_BIN="$ROOT_DIR/tools/bin/hako"

guard_require_command "$TAG" python3
guard_require_files "$TAG" "$FIXTURE" "$HAKO_IMPL" "$HAKO_BIN"

TMP_DIR="$(mktemp -d /tmp/hakorune-accum-const-loop-facts-parity.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/accum_const_loop_facts_parity.hako"
EXPECTED="$TMP_DIR/expected.txt"
ACTUAL="$TMP_DIR/actual.txt"
EXE="$TMP_DIR/accum_const_loop_facts_parity.exe"
...
for row in rows:
    calls.append(
        "    print(AccumConstLoopFactsBox.build_summary("
        f"{json.dumps(row['observation_token'])}, {json.dumps(row['body_token'])}))"
    )

source = "\n".join([
    "using lang.compiler.lib.accum_const_loop_facts as AccumConstLoopFactsBox",
    "",
    "static box Main {",
    "  main() {",
    *calls,
...
if actual != expected:
    print("[accum-const-loop-facts/parity] mismatch")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-accum-const-loop-facts-parity-gate-v0
owner=accum_const_loop_facts
input_contract=BackendSafeAccumConstLoopFactsTokenSnapshotV1
rust_oracle_fixture=mirbuilder-accum-const-loop-facts-rust-oracle-v0.json
hako_implementation=lang/src/compiler/lib/accum_const_loop_facts.hako
parity_rows=9
parity_status=green
