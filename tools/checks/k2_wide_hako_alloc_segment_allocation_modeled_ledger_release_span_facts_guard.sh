...
guard_expect_in_file "$TAG" 'memory.segment_allocation_modeled_ledger_report_box = "memory/segment_allocation_modeled_ledger_report_box.hako"' "$MODULE" "hako module must export MIMAP-104A report helper"
guard_expect_in_file "$TAG" 'segment_allocation_modeled_ledger_report_box.hako` owns MIMAP-104A report capsules' "$MEMORY_README" "memory README must define MIMAP-104A report helper"
guard_expect_in_file "$TAG" 'using selfhost.hako_alloc.memory.segment_allocation_modeled_ledger_report_box as HakoAllocSegmentAllocationModeledLedgerReportBox' "$OWNER" "MIMAP-104A owner must import report helper"
guard_expect_fixed_in_file "$TAG" 'report(accepted, reason, row_index, existing_index, segment_id, page_id, old_page_used, page_capacity, request_blocks, new_page_used, remaining_blocks, modeled_block_start, modeled_allocation_token)' "$OWNER" "MIMAP-104A owner must expose direct report builder"
guard_expect_fixed_in_file "$TAG" 'releaseReport(did_release, reason, row_index, modeled_allocation_token, segment_id, page_id, modeled_block_start, live_before, live_after)' "$OWNER" "MIMAP-104A owner must expose direct release report builder"
...
guard_expect_in_file "$TAG" 'box HakoAllocSegmentAllocationModeledLedgerReleaseReport' "$HELPER" "MIMAP-104A release report box must live in helper"
