...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_nyash_kernel_runtime_build_registry_exact_top_symbol_variability.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-build-registry-exact-top-symbol-variability] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-build-registry-exact-top-symbol-variability] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-build-registry-exact-top-symbol-variability] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
...
read -r primary_bucket_env_count primary_bucket_once_count primary_bucket_path_count primary_bucket_minimal_main_count primary_bucket_nyash_kernel_runtime_count primary_bucket_kernel_count primary_bucket_other_count primary_bucket_missing_count primary_bucket_alloc_count primary_bucket_ffi_count primary_bucket_string_count ret0_count_min ret0_count_max trial_summaries build_top_0_mode build_top_0_mode_count build_top_1_mode build_top_1_mode_count build_top_2_mode build_top_2_mode_count build_exact_build_mode build_exact_build_mode_count build_exact_registry_mode build_exact_registry_mode_count build_exact_runtime_mode build_exact_runtime_mode_count build_exact_once_mode build_exact_once_mode_count build_exact_other_mode build_exact_other_mode_count < <(
...
build_top_0_mode, build_top_0_mode_count = mode(top_symbol_counts[0])
build_top_1_mode, build_top_1_mode_count = mode(top_symbol_counts[1])
build_top_2_mode, build_top_2_mode_count = mode(top_symbol_counts[2])
build_exact_build_mode = "build" if exact_family_counts["build"] > 0 else "missing"
build_exact_build_mode_count = exact_family_counts["build"]
build_exact_registry_mode = "registry" if exact_family_counts["registry"] > 0 else "missing"
build_exact_registry_mode_count = exact_family_counts["registry"]
build_exact_runtime_mode = "runtime" if exact_family_counts["runtime"] > 0 else "missing"
build_exact_runtime_mode_count = exact_family_counts["runtime"]
build_exact_once_mode = "once" if exact_family_counts["once"] > 0 else "missing"
build_exact_once_mode_count = exact_family_counts["once"]
build_exact_other_mode = "other" if exact_family_counts["other"] > 0 else "missing"
build_exact_other_mode_count = exact_family_counts["other"]
...
print(
    ...
    ";".join(summaries),
    build_top_0_mode,
    build_top_0_mode_count,
    build_top_1_mode,
    build_top_1_mode_count,
    build_top_2_mode,
    build_top_2_mode_count,
    build_exact_build_mode,
    build_exact_build_mode_count,
    build_exact_registry_mode,
    build_exact_registry_mode_count,
    build_exact_runtime_mode,
    build_exact_runtime_mode_count,
    build_exact_once_mode,
    build_exact_once_mode_count,
    build_exact_other_mode,
    build_exact_other_mode_count,
)
PY
)
...
cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-build-registry-exact-top-symbol-variability-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_nyash_kernel_runtime_build_registry_exact_top_symbol_variability
...
build_top_0_mode=$build_top_0_mode
build_top_0_mode_count=$build_top_0_mode_count
build_top_1_mode=$build_top_1_mode
build_top_1_mode_count=$build_top_1_mode_count
build_top_2_mode=$build_top_2_mode
build_top_2_mode_count=$build_top_2_mode_count
build_exact_build_mode=$build_exact_build_mode
build_exact_build_mode_count=$build_exact_build_mode_count
build_exact_registry_mode=$build_exact_registry_mode
build_exact_registry_mode_count=$build_exact_registry_mode_count
build_exact_runtime_mode=$build_exact_runtime_mode
build_exact_runtime_mode_count=$build_exact_runtime_mode_count
build_exact_once_mode=$build_exact_once_mode
build_exact_once_mode_count=$build_exact_once_mode_count
build_exact_other_mode=$build_exact_other_mode
build_exact_other_mode_count=$build_exact_other_mode_count
...
if [ "${build_exact_registry_mode_count:-0}" -le 0 ]; then
  echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-build-registry-exact-top-symbol-variability] expected aggregate build_exact_registry_mode_count > 0" >&2
  exit 1
fi
