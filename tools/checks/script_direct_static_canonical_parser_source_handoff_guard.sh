...
for file in "$frontdoor" "$handoff" "$plan_input" "$plan_product" "$postpass" "$string_entry"; do
...
rg -q 'string_postpass_entry::parse_postpass' "$frontdoor"
...
rg -q 'pub\(crate\) fn parse_postpass' "$string_entry"
rg -q 'ParserBacked\(CompletedParserPostpassV1\)' "$plan_product"
rg -q 'PreparedNormalSourcePlanInputV1::from_parser_postpass' "$plan_input"
