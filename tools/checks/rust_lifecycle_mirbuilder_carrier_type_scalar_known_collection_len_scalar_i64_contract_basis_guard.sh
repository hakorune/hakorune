...
python3 - "$FIXTURE" "$CARD" "$STATE" "$TASK_ORDER" "$MANIFEST" "$RUST_BOUNDARY" "$COLLECTION_SOURCE" "$COLLECTION_TEST" <<'PY'
import json
import sys
import tomllib
...
collection_source = Path(sys.argv[7]).read_text(encoding="utf-8")
collection_test = Path(sys.argv[8]).read_text(encoding="utf-8")
