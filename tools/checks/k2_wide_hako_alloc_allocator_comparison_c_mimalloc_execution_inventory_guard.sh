...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_447A" "$CARD" "$NEXT_CARD" "$USIZE_SELECTION_CARD" "$USIZE_CARD" "$DESIGN" "$PREV_DESIGN" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MEMORY_README" "$MEMORY_INDEX" "$OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
...
guard_expect_in_file "$TAG" 'allocator_comparison_c_mimalloc_execution_inventory_box.hako' "$MEMORY_INDEX" "memory module index must name C mimalloc execution inventory owner"
