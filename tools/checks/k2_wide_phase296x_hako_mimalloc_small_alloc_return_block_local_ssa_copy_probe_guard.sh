...
guard_expect_fixed_in_file "$TAG" '296x-120 Classified return-block local SSA copy pressure and selected duplicate reason global calls as the next probe.' "$CURRENT_STATE" "current state landed tail must include row120"
...
guard_expect_fixed_in_file "$TAG" '| 121 | `HAKO-MIMALLOC-SMALL-ALLOC-DUPLICATE-REASON-CALL-PROBE-296X-001` | Landed |' "$TASKBOARD" "taskboard row121 must exist"
