...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_rebuild_cache_global_hooks_set_from_runtime_exact_top_symbol_presence.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-global-hooks-set-from-runtime-exact-top-symbol-presence] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-global-hooks-set-from-runtime-exact-top-symbol-presence] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
for trial in $(seq 1 "$TRIALS"); do
  ...
  case "$primary_bucket" in
    env|once|path|minimal_main|nyash_kernel_runtime|kernel|other|missing|alloc|ffi|string)
      ...
      ;;
    *)
      echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-global-hooks-set-from-runtime-exact-top-symbol-presence] unexpected primary bucket '$primary_bucket' in ${trial_out#$ROOT_DIR/}" >&2
      exit 1
      ;;
  esac
...
read -r primary_bucket_env_count primary_bucket_once_count primary_bucket_path_count primary_bucket_minimal_main_count primary_bucket_nyash_kernel_runtime_count primary_bucket_kernel_count primary_bucket_other_count primary_bucket_missing_count primary_bucket_alloc_count primary_bucket_ffi_count primary_bucket_string_count ret0_count_min ret0_count_max trial_summaries registry_top_0_mode registry_top_0_mode_count registry_top_1_mode registry_top_1_mode_count registry_top_2_mode registry_top_2_mode_count registry_exact_rebuild_cache_mode registry_exact_rebuild_cache_mode_count registry_exact_plugin_mode registry_exact_plugin_mode_count registry_exact_factory_type_mode registry_exact_factory_type_mode_count registry_exact_registry_mode registry_exact_registry_mode_count registry_exact_global_hooks_mode registry_exact_global_hooks_mode_count registry_exact_global_hooks_set_from_runtime_mode registry_exact_global_hooks_set_from_runtime_mode_count registry_exact_create_default_registry_mode registry_exact_create_default_registry_mode_count registry_exact_register_many_mode registry_exact_register_many_mode_count registry_exact_runtime_mode registry_exact_runtime_mode_count registry_exact_other_mode registry_exact_other_mode_count < <(
...
def classify_exact(symbol: str) -> str:
    text = symbol.lower()
    if "pluginboxfactory" in text or "box_types" in text:
        return "plugin"
    if "factory_type" in text:
        return "factory_type"
    if "rebuild_cache" in text:
        return "rebuild_cache"
    if "set_from_runtime" in text:
        return "global_hooks_set_from_runtime"
    if "global_hooks" in text:
        return "global_hooks"
    if "create_default_registry" in text or "createdefaultregistry" in text:
        return "create_default_registry"
    if "register_many" in text:
        return "register_many"
    if "ring0registry" in text or "init_global_ring0" in text:
        return "runtime"
    if "scheduler::new" in text or "get_factory_order_by_policy" in text:
        return "scheduler"
...
registry_exact_plugin_mode = "plugin" if exact_family_counts["plugin"] > 0 else "missing"
registry_exact_plugin_mode_count = exact_family_counts["plugin"]
registry_exact_factory_type_mode = "factory_type" if exact_family_counts["factory_type"] > 0 else "missing"
registry_exact_factory_type_mode_count = exact_family_counts["factory_type"]
...
registry_exact_global_hooks_mode = "global_hooks" if exact_family_counts["global_hooks"] > 0 else "missing"
registry_exact_global_hooks_mode_count = exact_family_counts["global_hooks"]
registry_exact_global_hooks_set_from_runtime_mode = "global_hooks_set_from_runtime" if exact_family_counts["global_hooks_set_from_runtime"] > 0 else "missing"
registry_exact_global_hooks_set_from_runtime_mode_count = exact_family_counts["global_hooks_set_from_runtime"]
...
if registry_top_0_mode != "nyash_rust::box_factory::registry::UnifiedBoxRegistry::rebuild_cache":
    raise SystemExit(1)
if registry_exact_global_hooks_set_from_runtime_mode_count <= 0:
    raise SystemExit(1)
...
print(
    ...
    registry_exact_plugin_mode,
    registry_exact_plugin_mode_count,
    registry_exact_factory_type_mode,
    registry_exact_factory_type_mode_count,
    registry_exact_registry_mode,
    registry_exact_registry_mode_count,
    registry_exact_global_hooks_mode,
    registry_exact_global_hooks_mode_count,
    registry_exact_global_hooks_set_from_runtime_mode,
    registry_exact_global_hooks_set_from_runtime_mode_count,
...
registry_top_0_mode="${registry_top_0_mode:-missing}"
registry_top_0_mode_count="${registry_top_0_mode_count:-0}"
registry_top_1_mode="${registry_top_1_mode:-missing}"
registry_top_1_mode_count="${registry_top_1_mode_count:-0}"
registry_top_2_mode="${registry_top_2_mode:-missing}"
registry_top_2_mode_count="${registry_top_2_mode_count:-0}"
registry_exact_rebuild_cache_mode="${registry_exact_rebuild_cache_mode:-missing}"
registry_exact_rebuild_cache_mode_count="${registry_exact_rebuild_cache_mode_count:-0}"
registry_exact_plugin_mode="${registry_exact_plugin_mode:-missing}"
registry_exact_plugin_mode_count="${registry_exact_plugin_mode_count:-0}"
registry_exact_factory_type_mode="${registry_exact_factory_type_mode:-missing}"
registry_exact_factory_type_mode_count="${registry_exact_factory_type_mode_count:-0}"
registry_exact_registry_mode="${registry_exact_registry_mode:-missing}"
registry_exact_registry_mode_count="${registry_exact_registry_mode_count:-0}"
registry_exact_global_hooks_mode="${registry_exact_global_hooks_mode:-missing}"
registry_exact_global_hooks_mode_count="${registry_exact_global_hooks_mode_count:-0}"
registry_exact_global_hooks_set_from_runtime_mode="${registry_exact_global_hooks_set_from_runtime_mode:-missing}"
registry_exact_global_hooks_set_from_runtime_mode_count="${registry_exact_global_hooks_set_from_runtime_mode_count:-0}"
registry_exact_create_default_registry_mode="${registry_exact_create_default_registry_mode:-missing}"
registry_exact_create_default_registry_mode_count="${registry_exact_create_default_registry_mode_count:-0}"
registry_exact_register_many_mode="${registry_exact_register_many_mode:-missing}"
registry_exact_register_many_mode_count="${registry_exact_register_many_mode_count:-0}"
registry_exact_runtime_mode="${registry_exact_runtime_mode:-missing}"
registry_exact_runtime_mode_count="${registry_exact_runtime_mode_count:-0}"
registry_exact_other_mode="${registry_exact_other_mode:-missing}"
registry_exact_other_mode_count="${registry_exact_other_mode_count:-0}"

cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-global-hooks-set-from-runtime-exact-top-symbol-presence-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_bucket_nyash_kernel_runtime_registry_rebuild_cache_global_hooks_set_from_runtime_exact_top_symbol_presence
...
registry_exact_plugin_mode=$registry_exact_plugin_mode
registry_exact_plugin_mode_count=$registry_exact_plugin_mode_count
registry_exact_factory_type_mode=$registry_exact_factory_type_mode
registry_exact_factory_type_mode_count=$registry_exact_factory_type_mode_count
...
registry_exact_global_hooks_mode=$registry_exact_global_hooks_mode
registry_exact_global_hooks_mode_count=$registry_exact_global_hooks_mode_count
registry_exact_global_hooks_set_from_runtime_mode=$registry_exact_global_hooks_set_from_runtime_mode
registry_exact_global_hooks_set_from_runtime_mode_count=$registry_exact_global_hooks_set_from_runtime_mode_count
...
registry_focus_dominant_mode=$registry_top_0_mode
registry_focus_dominant_mode_count=$registry_top_0_mode_count
registry_focus_runner_up_mode=$registry_top_1_mode
registry_focus_runner_up_mode_count=$registry_top_1_mode_count
summary=ok
EOF

if [ "${registry_top_0_mode_count:-0}" -le 0 ]; then
  echo "[perf-userbox-startup-executable-ret0-bucket-nyash-kernel-runtime-registry-rebuild-cache-global-hooks-set-from-runtime-exact-top-symbol-presence] expected aggregate dominant count > 0" >&2
  exit 1
fi
