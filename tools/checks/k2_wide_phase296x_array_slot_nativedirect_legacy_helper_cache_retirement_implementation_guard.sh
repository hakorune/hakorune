...
guard_expect_fixed_in_file "$TAG" 'Status: Current' "$CARD_375" "implementation row must be current"
...
guard_expect_fixed_in_file "$TAG" 'current_blocker_token = "ARRAY-SLOT-NATIVEDIRECT-LEGACY-HELPER-CACHE-RETIREMENT-IMPLEMENTATION-296X-001"' "$STATE" "current state must point to row375"
guard_expect_fixed_in_file "$TAG" 'latest_card = "296x-373-ARRAYBOX-PUBLIC-SEMANTICS-AND-DIRECTARRAY-SPLIT-SSOT"' "$STATE" "current state must keep latest landed card at row373"
