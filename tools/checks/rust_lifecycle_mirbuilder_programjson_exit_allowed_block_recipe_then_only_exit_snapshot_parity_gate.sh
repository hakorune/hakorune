...
TAG="rust-lifecycle-mirbuilder-programjson-exit-allowed-block-recipe-then-only-exit-snapshot-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-exit-allowed-block-recipe-then-only-exit-snapshot-parity-v0.json"
...
PREREQ_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_block_recipe_after_else_only_exit_next_contract_selection_guard.sh"
...
if ! grep -q '^summary=ok$' <<<"$PREREQ_OUT"; then
  printf '%s\n' "$PREREQ_OUT" >&2
  guard_fail "$TAG" "ThenOnlyExit selection prerequisite is not green"
fi

TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-exit-allowed-then-only-exit.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_exit_allowed_then_only_exit_probe.hako"
MIR_JSON="$TMP_DIR/programjson_exit_allowed_then_only_exit_probe.mir.json"
EXE="$TMP_DIR/programjson_exit_allowed_then_only_exit_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonExitAllowedBlockRecipeThenOnlyExitSnapshotParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-EXIT-ALLOWED-BLOCK-RECIPE-THEN-ONLY-EXIT-SNAPSHOT-PARITY-001":
...
if len(rows) != 1:
    raise SystemExit("ThenOnlyExit ProgramJSON parity requires exactly 1 row")
...
for needle in [
    ...
    if needle not in snapshot:
        raise SystemExit(f"missing ThenOnlyExit snapshot token: {needle}")
for needle in [
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for ProgramJSON ExitAllowed ThenOnlyExit probe"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-exit-allowed-block-recipe-then-only-exit-snapshot-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-EXIT-ALLOWED-BLOCK-RECIPE-THEN-ONLY-EXIT-SNAPSHOT-PARITY-001
