...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_family_split.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-family-split] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
require_positive_key() {
  ...
  if ! awk -F= -v key="$key" '$1 == key { found=1; exit !($2 + 0 > 0) } END { if (!found) exit 1 }' "$file"; then
    echo "[perf-userbox-startup-executable-ret0-family-split] expected positive ${key} in ${file#$ROOT_DIR/}" >&2
    exit 1
  fi
}
...
ret0_primary_family="$(awk -F= '$1=="startup_loader_ret0_exe_primary_family" { print $2; exit }' "$STARTUP_REPORT")"
ret0_minimal_main_count="$(awk -F= '$1=="startup_loader_ret0_exe_family_minimal_main_count" { print $2; exit }' "$STARTUP_REPORT")"
...
if [ "$ret0_top_count" -lt 1 ]; then
  echo "[perf-userbox-startup-executable-ret0-family-split] expected ret0.exe rows in ${STARTUP_REPORT#$ROOT_DIR/}" >&2
  exit 1
fi
if [ "$ret0_primary_family" != "minimal_main" ]; then
  echo "[perf-userbox-startup-executable-ret0-family-split] expected minimal_main primary family in ${STARTUP_REPORT#$ROOT_DIR/}" >&2
  exit 1
fi
if [ "$ret0_minimal_main_count" -lt 1 ]; then
  echo "[perf-userbox-startup-executable-ret0-family-split] expected minimal_main ret0.exe family rows in ${STARTUP_REPORT#$ROOT_DIR/}" >&2
  exit 1
fi

cat <<EOF
output_contract=perf-userbox-startup-executable-ret0-family-split-v0
input_contract=perf-userbox-startup-loader-owner-split-v0
measurement_scope=exact_aot_startup_executable_ret0_family_split
...
startup_loader_ret0_exe_primary_family=$ret0_primary_family
startup_loader_ret0_exe_family_minimal_main_count=$ret0_minimal_main_count
startup_loader_ret0_exe_family_process_spawn_wait_count=$(awk -F= '$1=="startup_loader_ret0_exe_family_process_spawn_wait_count" { print $2; exit }' "$STARTUP_REPORT")
startup_loader_ret0_exe_family_libc_process_count=$(awk -F= '$1=="startup_loader_ret0_exe_family_libc_process_count" { print $2; exit }' "$STARTUP_REPORT")
