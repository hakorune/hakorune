...
TAG="rust-lifecycle-mirbuilder-programjson-box-declaration-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-box-declaration-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonBoxDeclarationShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "BoxDeclarationShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON BoxDeclaration rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonBoxDeclarationShapeScanV1", "bad owner")
...
need(criteria.get("box_lowering") == 0, "box lowering must remain 0")
need(criteria.get("method_lowering") == 0, "method lowering must remain 0")
need(criteria.get("field_layout") == 0, "field layout must remain 0")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_box_declaration_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-box-declaration-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-BOX-DECLARATION-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=BoxDeclarationShapeSnapshotV1
shape_scope=covered ProgramJSON BoxDeclaration rows
covered_rows=9
