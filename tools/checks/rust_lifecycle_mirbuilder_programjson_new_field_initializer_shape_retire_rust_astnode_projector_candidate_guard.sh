...
TAG="rust-lifecycle-mirbuilder-programjson-new-field-initializer-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-new-field-initializer-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonNewFieldInitializerShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "NewFieldInitializerShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON New.field_initializers rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonNewFieldInitializerShapeScanV1", "bad owner")
...
need(criteria.get("box_field_initializer_lowering") == 0, "box field initializer lowering must remain unclaimed")
need(criteria.get("object_allocation") == 0, "object allocation must remain unclaimed")
...
for key in [
    ...
    "box_field_initializer_lowering",
    "object_allocation",
    "runtime_fallback",
    "new_backend_route",
    "new_abi",
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_new_field_initializer_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-new-field-initializer-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-NEW-FIELD-INITIALIZER-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=NewFieldInitializerShapeSnapshotV1
shape_scope=covered ProgramJSON New.field_initializers rows
covered_rows=10
