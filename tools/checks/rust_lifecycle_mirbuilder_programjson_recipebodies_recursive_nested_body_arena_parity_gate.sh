...
TAG="rust-lifecycle-mirbuilder-programjson-recipebodies-recursive-nested-body-arena-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipebodies-recursive-nested-body-arena-parity-v0.json"
SELECTION_FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-recipebodies-after-loop-body-arena-next-contract-selection-v0.json"
IMPL="$ROOT_DIR/lang/src/compiler/mirbuilder/program_json_recipebodies_recursive_nested_arena_builder.hako"
...
TMP_DIR="$(mktemp -d /tmp/hakorune-recipebodies-recursive-nested-arena.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/recipebodies_recursive_nested_arena_probe.hako"
MIR_JSON="$TMP_DIR/recipebodies_recursive_nested_arena_probe.mir.json"
EXE="$TMP_DIR/recipebodies_recursive_nested_arena_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonRecipeBodiesRecursiveNestedBodyArenaParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-RECIPEBODIES-RECURSIVE-NESTED-BODY-ARENA-PARITY-001":
...
for key, value in {
    ...
    "nested_body_references_emitted": 1,
    "body_count": 4,
...
if claims.get("recursive_nested_body_arena_implemented") != 1:
    raise SystemExit("implementation claim missing")
for key, value in claims.items():
    if key == "recursive_nested_body_arena_implemented":
        continue
    if value != 0:
...
if len(rows) != 1:
    raise SystemExit("recursive nested arena parity requires exactly 1 row")
...
for row in rows:
    ...
    for token in [
        "snapshot_kind=ProgramJsonRecipeBodiesRecursiveNestedArenaBuilderV1;",
        ";err=0",
        ";root_body_id=0",
        ";body_count=4",
        ";body0_items=Stmt:Local,LoopRef,Stmt:Return",
        "body0.item1->loop(body=1)",
        "body1.item0->stmt1.body0.if(then=2,else=3)",
        ";body1_items=IfRef,Stmt:Assignment",
        ";body2_items=Exit:Return",
        ";body3_item_count=0",
    ...
    lines.append(
        "    print(\"arena:"
        + row_id
        + ":\" + ProgramJsonRecipeBodiesRecursiveNestedArenaBuilderBox.build_summary("
...
for needle in [
    ...
    "RecipeItemBox.kind_is(item1, \"Loop\")",
    "RecipeItemBox.kind_is(loop_item0, \"If\")",
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for RecipeBodies recursive nested arena probe"
fi
...
builder = next((fn for fn in functions if fn.get("name") == "ProgramJsonRecipeBodiesRecursiveNestedArenaBuilderBox.build_summary/1"), None)
arena = next((fn for fn in functions if fn.get("name") == "ProgramJsonRecipeBodiesRecursiveNestedArenaBuilderBox.build_arena/1"), None)
if main is None:
...
main_routes = [row for row in route_rows(main) if row.get("symbol") == "ProgramJsonRecipeBodiesRecursiveNestedArenaBuilderBox.build_summary/1"]
if len(main_routes) < 1:
    raise SystemExit("main does not call recursive nested arena summary")

bad = []
for fn in (builder, arena):
    bad.extend(row for row in route_rows(fn) if row.get("tier") == "Unsupported" or row.get("reason"))
if bad:
    raise SystemExit(f"recursive nested arena functions have unsupported routes: {bad[:5]}")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-recipebodies-recursive-nested-body-arena-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-RECIPEBODIES-RECURSIVE-NESTED-BODY-ARENA-PARITY-001
owner=ProgramJsonRecipeBodiesRecursiveNestedArenaBuilderBox
snapshot_kind=ProgramJsonRecipeBodiesRecursiveNestedArenaBuilderV1
row_count=1
programjson_traversal_used=1
recipe_root_used=1
