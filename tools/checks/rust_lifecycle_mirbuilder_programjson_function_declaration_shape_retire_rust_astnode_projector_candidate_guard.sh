...
TAG="rust-lifecycle-mirbuilder-programjson-function-declaration-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-function-declaration-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonFunctionDeclarationShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "FunctionDeclarationShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON FunctionDeclaration rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonFunctionDeclarationShapeScanV1", "bad owner")
...
need(criteria.get("function_lowering") == 0, "function lowering must remain 0")
need(criteria.get("function_route_selection") == 0, "function route selection must remain 0")

row = decision.get("decision") or {}
need(row.get("kind") == "RetireCandidateScoped", "bad decision kind")
need(
    row.get("selected_next_card") == "MIRBUILDER-PROGRAMJSON-CAPABILITY-BATCH-CONTINUATION",
    "bad next card",
)
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_function_declaration_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-function-declaration-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-FUNCTION-DECLARATION-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=FunctionDeclarationShapeSnapshotV1
shape_scope=covered ProgramJSON FunctionDeclaration rows
covered_rows=10
