...
TAG="rust-lifecycle-mirbuilder-programjson-block-recipe-after-else-only-exit-next-contract-selection-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-block-recipe-after-else-only-exit-next-contract-selection-v0.json"
...
if fixture.get("kind") != "MirBuilderProgramJsonBlockRecipeAfterElseOnlyExitNextContractSelectionV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-BLOCK-RECIPE-AFTER-ELSE-ONLY-EXIT-NEXT-CONTRACT-SELECTION-001":
...
if selected.get("name") != "ProgramJsonExitAllowedBlockRecipeThenOnlyExitSnapshotV1":
...
if selected.get("required_programjson_producer") != "IfStmtHandler then-return/else-local recipe_root projection":
    raise SystemExit("producer boundary drift")
if selected.get("required_projection_tokens") != ["IfThenReturnElseLocal"]:
    raise SystemExit("projection token drift")
if selected.get("required_reducer_outputs") != ["IfExitAllowed", "ThenOnlyExit"]:
    raise SystemExit("reducer output drift")
if selected.get("selected_next_card") != "MIRBUILDER-PROGRAMJSON-EXIT-ALLOWED-BLOCK-RECIPE-THEN-ONLY-EXIT-SNAPSHOT-PARITY-001":
    raise SystemExit("bad next card")

candidates = {row.get("capability"): row for row in fixture.get("candidates", [])}
if candidates.get("ProgramJsonExitAllowedBlockRecipeThenOnlyExitSnapshotV1", {}).get("status") != "Selected":
    raise SystemExit("ThenOnlyExit candidate must be selected")
...
if decision.get("kind") != "SelectProgramJsonExitAllowedThenOnlyExitAfterElseOnlyExit":
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-block-recipe-after-else-only-exit-next-contract-selection-guard-v0
token=MIRBUILDER-PROGRAMJSON-BLOCK-RECIPE-AFTER-ELSE-ONLY-EXIT-NEXT-CONTRACT-SELECTION-001
...
programjson_then_only_exit_parity_green=0
...
selected_next_card=MIRBUILDER-PROGRAMJSON-EXIT-ALLOWED-BLOCK-RECIPE-THEN-ONLY-EXIT-SNAPSHOT-PARITY-001
summary=ok
REPORT
