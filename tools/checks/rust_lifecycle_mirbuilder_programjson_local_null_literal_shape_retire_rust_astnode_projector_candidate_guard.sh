...
TAG="rust-lifecycle-mirbuilder-programjson-local-null-literal-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-local-null-literal-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonLocalNullLiteralShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "LocalNullLiteralShapeSnapshotV1", "bad retire candidate")
need(scope.get("shape_scope") == "covered ProgramJSON Local.expr Null literal rows", "bad shape scope")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonLocalNullLiteralShapeScanV1", "bad owner")
...
need(criteria.get("null_lowering") == 0, "null lowering must remain unclaimed")
need(criteria.get("option_semantics") == 0, "option semantics must remain unclaimed")
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_local_null_literal_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-local-null-literal-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-LOCAL-NULL-LITERAL-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=LocalNullLiteralShapeSnapshotV1
shape_scope=covered ProgramJSON Local.expr Null literal rows
covered_rows=9
