...
guard_require_files "$TAG" "$APP" "$APP_README" "$APP_TEST" "$CARD_534A" "$CARD_538A" "$CARD" "$DESIGN" "$DESIGN_534A" "$INDEX" "$PROOF_MANIFEST_INCLUDE" "$MODULE" "$MODULE_INDEX" "$OWNER" "$PREV_OWNER" "$SELF_SCRIPT" "$RUN_PROOF"
