...
TAG="rust-lifecycle-mirbuilder-programjson-layer4-next-recipe-dto-capability-selection-rerun-001-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-layer4-next-recipe-dto-capability-selection-rerun-001-v0.json"
PREVIOUS_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_layer4_recipe_shape_kind_dto_loop_root_retire_rust_astnode_projector_candidate_guard.sh"
...
do
  if ! grep -q "$required" <<<"$PREVIOUS_OUT"; then
    printf '%s\n' "$PREVIOUS_OUT" >&2
    guard_fail "$TAG" "previous loop-root shape-kind retire-candidate drift: $required"
  fi
done
...
need(fixture.get("kind") == "MirBuilderProgramJsonLayer4NextRecipeDtoCapabilitySelectionRerun001V1", "bad kind")
need(fixture.get("token") == "MIRBUILDER-PROGRAMJSON-LAYER4-NEXT-RECIPE-DTO-CAPABILITY-SELECTION-RERUN-001", "bad token")
...
need(selected[0].get("name") == "ProgramJsonSeqRecipeDtoLoopRootV1", "bad selected candidate")
...
need(cap.get("name") == "ProgramJsonSeqRecipeDtoLoopRootV1", "bad selected capability")
need(cap.get("expected_seq_sig") == "Local>Loop>Return", "bad expected seq sig")
...
need(cap.get("next_card") == "MIRBUILDER-PROGRAMJSON-LAYER4-SEQ-RECIPE-DTO-LOOP-ROOT-PARITY-001", "bad next card")

acceptance = fixture.get("acceptance") or {}
for key in [
    "must_consume_programjson_structure",
    "must_construct_structured_recipe_dto",
    "must_use_root_sequence_scanner",
    "must_include_shape_kind",
...
need(
    decision.get("selected_next_card") == "MIRBUILDER-PROGRAMJSON-LAYER4-SEQ-RECIPE-DTO-LOOP-ROOT-PARITY-001",
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-layer4-next-recipe-dto-capability-selection-rerun-001-guard-v0
token=MIRBUILDER-PROGRAMJSON-LAYER4-NEXT-RECIPE-DTO-CAPABILITY-SELECTION-RERUN-001
