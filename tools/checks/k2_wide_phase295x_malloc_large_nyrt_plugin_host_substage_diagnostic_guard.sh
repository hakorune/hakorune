...
TAG="k2-wide-phase295x-malloc-large-nyrt-plugin-host-substage-diagnostic"
cd "$ROOT_DIR"
source tools/checks/lib/guard_common.sh

CARD="docs/development/current/main/phases/phase-295x/295x-200-MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-SUBSTAGE-DIAGNOSTIC.md"
PREV_CARD="docs/development/current/main/phases/phase-295x/295x-199-MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-BASELINE-SELECTION.md"
TASKBOARD="docs/development/current/main/phases/phase-295x/295x-90-mimalloc-comparison-execution-taskboard.md"
INDEX="docs/tools/check-scripts-index.md"
ENV_DOC="docs/reference/environment-variables.md"
SELF_SCRIPT="tools/checks/k2_wide_phase295x_malloc_large_nyrt_plugin_host_substage_diagnostic_guard.sh"
...
echo "[$TAG] checking phase-295x malloc-large NyRT plugin host substage diagnostic"
...
guard_expect_in_file "$TAG" 'Status: Current' "$CARD" "card must remain current while the diagnostic row is being exercised"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-SUBSTAGE-DIAGNOSTIC-295X-002' "$CARD" "card must identify current blocker"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-LOADSET-FOOTPRINT-SELECTION-295X-002' "$CARD" "card must select loadset follow-on"
guard_expect_in_file "$TAG" 'MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-SUBSTAGE-DIAGNOSTIC-295X-002' "$PREV_CARD" "previous row must select this diagnostic"
...
guard_expect_in_file "$TAG" '| 199 | `MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-BASELINE-SELECTION-295X-002` | Landed |' "$TASKBOARD" "taskboard must retain the baseline selection row as landed"
guard_expect_in_file "$TAG" '| 200 | `MIMALLOC-COMPARISON-NYRT-PLUGIN-HOST-SUBSTAGE-DIAGNOSTIC-295X-002` | Current |' "$TASKBOARD" "taskboard must expose the plugin-host substage row"
guard_expect_in_file "$TAG" "$SELF_SCRIPT" "$INDEX" "check script index must list this guard"

tmp_dir="$(mktemp -d /tmp/hakorune_phase295x_plugin_substage_large.XXXXXX)"
...
print(f"[phase295x-plugin-substage-large] config_delta_bytes={config_delta}")
print(f"[phase295x-plugin-substage-large] library_loop_delta_bytes={library_delta}")
print(f"[phase295x-plugin-substage-large] prebirth_delta_bytes={prebirth_delta}")
print(f"[phase295x-plugin-substage-large] total_plugin_host_delta_bytes={total_delta}")
PY
