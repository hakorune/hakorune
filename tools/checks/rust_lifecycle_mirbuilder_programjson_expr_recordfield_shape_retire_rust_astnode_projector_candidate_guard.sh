...
TAG="rust-lifecycle-mirbuilder-programjson-expr-recordfield-shape-retire-rust-astnode-projector-candidate-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

DECISION="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-expr-recordfield-shape-retire-rust-astnode-projector-candidate-v0.json"
...
need(
    decision.get("kind")
    == "MirBuilderProgramJsonExprRecordFieldShapeRustAstNodeProjectorRetireCandidateV1",
    "bad kind",
)
need(
    decision.get("token")
...
need(scope.get("retire_candidate") == "ExprRecordFieldShapeSnapshotV1", "bad retire candidate")
need(scope.get("programjson_snapshot_owner") == "ProgramJsonExprRecordFieldShapeScanV1", "bad owner")
...
need(
    row.get("selected_next_card") == "MIRBUILDER-PROGRAMJSON-EXPR-BINARY-SHAPE-SCAN-CAPABILITY-001",
    "bad next card",
)
...
bash "$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_expr_recordfield_shape_scan_parity_gate.sh" >/dev/null

cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-expr-recordfield-shape-retire-rust-astnode-projector-candidate-guard-v0
token=MIRBUILDER-PROGRAMJSON-EXPR-RECORDFIELD-SHAPE-RUST-ASTNODE-PROJECTOR-RETIRE-CANDIDATE-001
retire_candidate=ExprRecordFieldShapeSnapshotV1
shape_scope=covered ProgramJSON Expr.expr RecordField rows
covered_rows=9
...
selected_next_card=MIRBUILDER-PROGRAMJSON-EXPR-BINARY-SHAPE-SCAN-CAPABILITY-001
summary=ok
REPORT
