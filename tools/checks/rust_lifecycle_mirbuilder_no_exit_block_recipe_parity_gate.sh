...
TAG="rust-lifecycle-mirbuilder-no-exit-block-recipe-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-no-exit-block-recipe-rust-oracle-v0.json"
HAKO_IMPL="$ROOT_DIR/lang/src/compiler/lib/no_exit_block_recipe.hako"
HAKO_BIN="$ROOT_DIR/tools/bin/hako"

guard_require_command "$TAG" python3
guard_require_files "$TAG" "$FIXTURE" "$HAKO_IMPL" "$HAKO_BIN"

TMP_DIR="$(mktemp -d /tmp/hakorune-no-exit-block-recipe-parity.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/no_exit_block_recipe_parity.hako"
EXPECTED="$TMP_DIR/expected.txt"
ACTUAL="$TMP_DIR/actual.txt"
EXE="$TMP_DIR/no_exit_block_recipe_parity.exe"
...
for row in rows:
    ...
    calls.append(
        "    print(NoExitBlockRecipeBox.build_summary("
...
if actual != expected:
    print("[no-exit-block-recipe/parity] mismatch")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-no-exit-block-recipe-parity-gate-v0
owner=no_exit_block_recipe
input_contract=BackendSafeNoExitBlockTokenSnapshotV1
rust_oracle_fixture=mirbuilder-no-exit-block-recipe-rust-oracle-v0.json
hako_implementation=lang/src/compiler/lib/no_exit_block_recipe.hako
parity_rows=11
parity_status=green
