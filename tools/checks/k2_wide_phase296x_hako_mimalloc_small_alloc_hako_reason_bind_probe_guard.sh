...
guard_expect_fixed_in_file "$TAG" '296x-122 Proved .hako reason-local binding removes duplicate small-alloc failure reason calls and selected the keeper row.' "$CURRENT_STATE" "current state landed tail must include row122"
...
guard_expect_fixed_in_file "$TAG" '| 123 | `HAKO-MIMALLOC-SMALL-ALLOC-HAKO-REASON-BIND-KEEPER-296X-001` | Landed |' "$TASKBOARD" "taskboard row123 must exist"
