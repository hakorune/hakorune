...
TAG="rust-lifecycle-mirbuilder-carrier-merge-assignment-hako-adoption-decision-guard"
source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-carrier-merge-assignment-hako-adoption-decision-v0.json"
CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/1872-MIRBUILDER-CARRIER-MERGE-ASSIGNMENT-STATEMENT-HAKO-ADOPTION-DECISION-001.md"
SEED_FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-carrier-merge-assignment-hako-native-source-seed-v0.json"
SEED_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_carrier_merge_assignment_hako_native_source_seed_guard.sh"
...
NATIVE_SOURCE="$ROOT_DIR/lang/src/compiler/lib/carrier_merge_assignment_native_seed.hako"

guard_require_command "$TAG" python3
guard_require_command "$TAG" rg
guard_require_files "$TAG" "$FIXTURE" "$CARD" "$SEED_FIXTURE" "$SEED_GUARD" "$PROMOTION" "$CONTRACT" "$NATIVE_SOURCE"

bash "$SEED_GUARD" >/tmp/carrier_merge_assignment_native_seed_guard.out

python3 - <<'PY'
import json
from pathlib import Path

fixture = json.loads(Path("docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-carrier-merge-assignment-hako-adoption-decision-v0.json").read_text())
seed = json.loads(Path("docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-carrier-merge-assignment-hako-native-source-seed-v0.json").read_text())
...
card = Path("docs/development/current/main/phases/phase-296x/1872-MIRBUILDER-CARRIER-MERGE-ASSIGNMENT-STATEMENT-HAKO-ADOPTION-DECISION-001.md").read_text()
native = Path("lang/src/compiler/lib/carrier_merge_assignment_native_seed.hako").read_text()

token = "MIRBUILDER-CARRIER-MERGE-ASSIGNMENT-STATEMENT-HAKO-ADOPTION-DECISION-001"
if fixture.get("kind") != "MirBuilderCarrierMergeAssignmentHakoAdoptionDecisionV1":
    raise SystemExit("fixture kind mismatch")
if fixture.get("token") != token:
    raise SystemExit("fixture token mismatch")
if f"# 1872 - {token}" not in card:
...
if promotion["decision"]["kind"] != "Promote" or promotion["selected_stage"] != "HakoMainline":
    raise SystemExit("CarrierMergeAssignment must be HakoMainline before adoption")
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-carrier-merge-assignment-hako-adoption-decision-v0
