...
TMP_DIR="$(mktemp -d /tmp/hakorune_perf_userbox_startup_executable_ret0_bucket_libc_process_env_string_scan_distribution.XXXXXX)"
...
require_line() {
  ...
  if ! grep -q "^${expected}$" "$file"; then
    echo "[perf-userbox-startup-executable-ret0-bucket-libc-process-env-string-scan-distribution] missing line in ${file#$ROOT_DIR/}: $expected" >&2
    exit 1
  fi
}
...
if bucket_counts["env_api"] < 1:
    raise SystemExit(1)
if bucket_counts["env_string_scan"] < 1:
    raise SystemExit(1)

print("output_contract=perf-userbox-startup-executable-ret0-bucket-libc-process-env-string-scan-distribution-v0")
...
print(f"libc_process_bucket_env_api_count={bucket_counts['env_api']}")
print(f"libc_process_bucket_env_string_scan_count={bucket_counts['env_string_scan']}")
print(f"libc_process_bucket_memory_string_count={bucket_counts['memory_string']}")
print(f"libc_process_bucket_allocator_count={bucket_counts['allocator']}")
print(f"libc_process_bucket_other_count={bucket_counts['other']}")
print("libc_process_env_string_scan_distribution_present=1")
