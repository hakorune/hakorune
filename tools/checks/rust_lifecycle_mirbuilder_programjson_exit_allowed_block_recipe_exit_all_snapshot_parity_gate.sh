...
TAG="rust-lifecycle-mirbuilder-programjson-exit-allowed-block-recipe-exit-all-snapshot-parity-gate"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-programjson-exit-allowed-block-recipe-exit-all-snapshot-parity-v0.json"
...
PREREQ_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_programjson_block_recipe_after_then_only_exit_next_contract_selection_guard.sh"
...
if ! grep -q '^summary=ok$' <<<"$PREREQ_OUT"; then
  printf '%s\n' "$PREREQ_OUT" >&2
  guard_fail "$TAG" "ExitAll selection prerequisite is not green"
fi

TMP_DIR="$(mktemp -d /tmp/hakorune-programjson-exit-allowed-exit-all.XXXXXX)"
cleanup() { rm -rf "$TMP_DIR" >/dev/null 2>&1 || true; }
trap cleanup EXIT

APP="$TMP_DIR/programjson_exit_allowed_exit_all_probe.hako"
MIR_JSON="$TMP_DIR/programjson_exit_allowed_exit_all_probe.mir.json"
EXE="$TMP_DIR/programjson_exit_allowed_exit_all_probe.exe"
...
if fixture.get("kind") != "MirBuilderProgramJsonExitAllowedBlockRecipeExitAllSnapshotParityV1":
    raise SystemExit("bad fixture kind")
if fixture.get("token") != "MIRBUILDER-PROGRAMJSON-EXIT-ALLOWED-BLOCK-RECIPE-EXIT-ALL-SNAPSHOT-PARITY-001":
...
if len(rows) != 1:
    raise SystemExit("ExitAll ProgramJSON parity requires exactly 1 row")
...
for needle in [
    ...
    if needle not in snapshot:
        raise SystemExit(f"missing ExitAll snapshot token: {needle}")
for needle in [
    "_read_else_break_item",
...
if ! timeout 90s bash "$HAKO_BIN" --backend mir --emit-mir-json "$MIR_JSON" "$APP" >"$BUILD_LOG" 2>&1; then
  ...
  guard_fail "$TAG" "failed to emit MIR JSON for ProgramJSON ExitAllowed ExitAll probe"
fi
...
cat <<'REPORT'
output_contract=rust-lifecycle-mirbuilder-programjson-exit-allowed-block-recipe-exit-all-snapshot-parity-gate-v0
token=MIRBUILDER-PROGRAMJSON-EXIT-ALLOWED-BLOCK-RECIPE-EXIT-ALL-SNAPSHOT-PARITY-001
