...
guard_require_files "$TAG" "$TOOL" "$README" "$INDEX" "$SELF_SCRIPT"
...
guard_expect_fixed_in_file "$TAG" "tools/docs/phase_row.py" "$README" "tools docs README must name the row writer"
guard_expect_fixed_in_file "$TAG" "dry-run" "$README" "tools docs README must explain the dry-run default"
...
# The selected workstream board has no phase-row Current Blocker/Queue shape.
# Keep this pilot focused on dry-run card and CURRENT_STATE output; the optional
# queue mutation needs a separate phase-board fixture before this guard owns it.
python3 "$TOOL" create \
  --row 295x-999 \
  --row-number 999 \
...
while IFS= read -r line; do
  case "$line" in
    "[phase-row] would write "*) card_rel="${line#\[phase-row\] would write }"; break ;;
  esac
done < "$dry_out"
if [[ -z "$card_rel" || "$card_rel" = /* ]]; then
  guard_fail "$TAG" "dry-run must report a repo-relative card path"
fi
if [[ -e "$card_rel" ]]; then
