...
TAG="rust-lifecycle-mirbuilder-post-rhs-materialization-intent-next-seam-selection-guard"

source "$ROOT_DIR/tools/checks/lib/guard_common.sh"

CARD="$ROOT_DIR/docs/development/current/main/phases/phase-296x/3330-MIRBUILDER-POST-RHS-MATERIALIZATION-INTENT-NEXT-SEAM-SELECTION-001.md"
FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-post-rhs-materialization-intent-next-seam-selection-v0.json"
PREV_FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-hard-authority-pilot-compare-rhs-materialization-intent-v0.json"
CANDIDATE_FIXTURE="$ROOT_DIR/docs/development/current/main/design/fixtures/rust-lifecycle/mirbuilder-compare-rhs-materialization-readonly-resolution-parity-v0.json"
PREV_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_hard_authority_pilot_compare_rhs_materialization_intent_guard.sh"
CANDIDATE_GUARD="$ROOT_DIR/tools/checks/rust_lifecycle_mirbuilder_compare_rhs_materialization_readonly_resolution_parity_gate.sh"
...
if ! grep -q '^compare_rhs_materialization_intent_owner=1$' <<<"$PREV_OUT"; then
  printf '%s\n' "$PREV_OUT" >&2
...
if ! grep -q '^rhs_valueid_resolution_plan_snapshot=1$' <<<"$CANDIDATE_OUT"; then
  printf '%s\n' "$CANDIDATE_OUT" >&2
  guard_fail "$TAG" "RHS ValueId resolution plan evidence is not green"
fi
...
token = "MIRBUILDER-POST-RHS-MATERIALIZATION-INTENT-NEXT-SEAM-SELECTION-001"
output_contract = "rust-lifecycle-mirbuilder-post-rhs-materialization-intent-next-seam-selection-v0"
...
follow_on_cards = [
    ...
    "MIRBUILDER-HARD-AUTHORITY-PILOT-COMPARE-RHS-SYMBOLREF-LOOKUP-BRIDGE-001",
]
follow_on_paths = [
    card_path.as_posix(),
    ...
    "docs/development/current/main/phases/phase-296x/3334-MIRBUILDER-POST-RHS-VALUEID-REQUEST-ABI-NEXT-SEAM-SELECTION-001.md",
    "docs/development/current/main/phases/phase-296x/3335-MIRBUILDER-HARD-AUTHORITY-PILOT-COMPARE-RHS-LITERAL-I64-CONSTANT-EMISSION-BRIDGE-001.md",
    "docs/development/current/main/phases/phase-296x/3336-MIRBUILDER-HARD-AUTHORITY-PILOT-COMPARE-RHS-SYMBOLREF-LOOKUP-CONTRACT-PARITY-001.md",
    "docs/development/current/main/phases/phase-296x/3337-MIRBUILDER-HARD-AUTHORITY-PILOT-COMPARE-RHS-SYMBOLREF-LOOKUP-BRIDGE-001.md",
]
...
need(fixture.get("kind") == "MirBuilderPostRhsMaterializationIntentNextSeamSelectionV1", "fixture kind drift")
...
need((prev.get("claims") or {}).get("compare_rhs_materialization_intent_owner") == 1, "previous pilot evidence missing")
need((prev.get("claims") or {}).get("source_selfhost_claim") == 0, "previous pilot Source Selfhost claim drift")

need(candidate_fixture.get("owner") == "CompareRhsValueIdResolutionPlanSnapshotBox", "candidate owner drift")
need(candidate_fixture.get("output_contract") == "CompareRhsValueIdResolutionPlanSnapshotV1", "candidate output drift")
need((candidate_fixture.get("claims") or {}).get("compare_rhs_materialization_readonly_resolution_parity") == 1, "candidate parity not green")
need((candidate_fixture.get("claims") or {}).get("rhs_value_id_resolution") == 0, "candidate actual resolution must remain 0")
...
need(selected.get("owner_id") == "CompareRhsValueIdResolutionPlanSnapshotBox", "selected owner drift")
need(selected.get("input_surface") == "CompareRhsMaterializationIntentSnapshotV1", "input surface drift")
need(selected.get("output_surface") == "CompareRhsValueIdResolutionPlanSnapshotV1", "output surface drift")
need(selected.get("downstream_consumer") == "CompareRhsValueIdResolutionRequestSnapshotBox", "consumer drift")
...
need(decision.get("reason_token") == "CompareRhsValueIdResolutionPlanIsDirectReadOnlyDownstreamSeam", "reason token drift")
need(decision.get("selected_next_card") == selected_card, "selected next drift")

claims = fixture.get("claims") or {}
for key in [
    "post_rhs_materialization_intent_next_seam_selected",
    "first_rhs_intent_pilot_evidence_consumed",
    "compare_rhs_valueid_resolution_plan_selected",
]:
...
need("tools/checks/rust_lifecycle_mirbuilder_post_rhs_materialization_intent_next_seam_selection_guard.sh" in index, "check index missing guard")

print(f"output_contract={output_contract}")
print("decision=SelectNextHardAuthoritySeam")
print("reason_token=CompareRhsValueIdResolutionPlanIsDirectReadOnlyDownstreamSeam")
print(f"selected_candidate={selected_candidate}")
print(f"selected_next_card={selected_card}")
print("post_rhs_materialization_intent_next_seam_selected=1")
