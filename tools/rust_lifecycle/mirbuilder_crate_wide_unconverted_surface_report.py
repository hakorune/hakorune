...
LOOP_COND_CO_CONTINUE_IF_SUBCLUSTER_RULES = [
    ("LoopCondCoContinueIfPreludeSpanCluster", ("lower_continue_if_prelude_span",)),
    ("LoopCondCoContinueIfNoElseCluster", ("lower_continue_if_no_else",)),
    ("LoopCondCoContinueIfGroupPreludeCluster", ("lower_continue_if_group_prelude",)),
]

LOOP_COND_CO_GROUP_IF_SUBCLUSTER_RULES = [
    ("LoopCondCoGroupIfBranchCluster", ("lower_group_if",)),
...
LOOP_COND_CO_STATEMENT_LOWERING_SUBCLUSTER_RULES = [
    ("LoopCondCoStatementDispatcherCluster", ("lower_continue_only_stmt",)),
    ("LoopCondCoAstStatementLoweringCluster", ("lower_stmt_ast",)),
]
...
def stable_deny_reason_repair_rules() -> dict[str, str]:
    if not STABLE_DENY_REASON_REPAIR.exists():
        return {}
    fixture = read_json(STABLE_DENY_REASON_REPAIR)
    rules: dict[str, str] = {}
    for item in fixture.get("rules", []):
        confidence = item.get("match", {}).get("owner_edge_confidence")
        reason = item.get("stable_deny_reason")
        if confidence and reason:
            rules[confidence] = reason
    return rules


def apply_stable_deny_reason_repair(items: list[dict[str, Any]]) -> None:
    rules = stable_deny_reason_repair_rules()
    if not rules:
        return
    evidence = rel(STABLE_DENY_REASON_REPAIR)
    for item in items:
        if item.get("classification") != "MissingProjectionPolicy":
            continue
        reason = rules.get(item.get("owner_edge_confidence") or "None")
        if not reason:
            continue
        item["stable_deny_reason"] = reason
        refs = list(item.get("evidence_refs", []))
        if evidence not in refs:
            refs.append(evidence)
        item["evidence_refs"] = refs
...
def loop_cond_co_continue_if_subcluster(item: dict[str, Any]) -> str | None:
    if item.get("loop_cond_co_subcluster") != "LoopCondCoContinueIfCluster":
        return None
    symbol = item["symbol"]
    for subcluster, symbols in LOOP_COND_CO_CONTINUE_IF_SUBCLUSTER_RULES:
        if symbol in symbols:
            return subcluster
    return "OtherLoopCondCoContinueIfCluster"


def loop_cond_co_group_if_subcluster(item: dict[str, Any]) -> str | None:
...
def loop_cond_co_statement_lowering_subcluster(item: dict[str, Any]) -> str | None:
    if item.get("loop_cond_co_subcluster") != "LoopCondCoStatementLoweringCluster":
        return None
    symbol = item["symbol"]
    for subcluster, symbols in LOOP_COND_CO_STATEMENT_LOWERING_SUBCLUSTER_RULES:
        if symbol in symbols:
            return subcluster
...
def build_report() -> dict[str, Any]:
    ...
    for item in classified:
        ...
        co_continue_if_subcluster = loop_cond_co_continue_if_subcluster(item)
        if co_continue_if_subcluster is not None:
            item["loop_cond_co_continue_if_subcluster"] = co_continue_if_subcluster
    ...
    loop_cond_co_statement_lowering_subcluster_counts: dict[str, int] = {}
    for item in items:
                ...
                if subcluster == "PlanFeatureMaterializerCluster":
                    ...
                    if feature_subcluster == "LoopCondFeatureCluster":
                        ...
                        if loop_cond_subcluster == "LoopCondContinueOnlyCluster":
                            ...
                            if co_subcluster == "LoopCondCoContinueIfCluster":
                                co_continue_if_subcluster = item["loop_cond_co_continue_if_subcluster"] or "OtherLoopCondCoContinueIfCluster"
                                loop_cond_co_continue_if_subcluster_counts[co_continue_if_subcluster] = loop_cond_co_continue_if_subcluster_counts.get(co_continue_if_subcluster, 0) + 1
                            ...
                            if co_subcluster == "LoopCondCoStatementLoweringCluster":
                                co_statement_lowering_subcluster = item["loop_cond_co_statement_lowering_subcluster"] or "OtherLoopCondCoStatementLoweringCluster"
                                loop_cond_co_statement_lowering_subcluster_counts[co_statement_lowering_subcluster] = loop_cond_co_statement_lowering_subcluster_counts.get(co_statement_lowering_subcluster, 0) + 1
    ...
    return {
        ...
        "loop_cond_co_continue_if_subcluster_summary": [
            {"subcluster": subcluster, "count": count}
            for subcluster, count in sorted(loop_cond_co_continue_if_subcluster_counts.items(), key=lambda item: (-item[1], item[0]))
        ],
        ...
        "loop_cond_co_statement_lowering_subcluster_summary": [
            {"subcluster": subcluster, "count": count}
            for subcluster, count in sorted(loop_cond_co_statement_lowering_subcluster_counts.items(), key=lambda item: (-item[1], item[0]))
        ],
