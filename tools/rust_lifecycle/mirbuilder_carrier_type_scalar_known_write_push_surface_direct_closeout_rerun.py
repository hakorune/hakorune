#!/usr/bin/env python3
"""Rerun the Write Push surface typed direct closeout contract."""
...
def build_fixture() -> dict[str, Any]:
    basis = read_json(PUSH_BASIS)
    collection = read_json(COLLECTION_RERUN)
    prior_closeouts = collection.get("accepted_scoped_closeouts") or []
    ...
    accepted_closeouts = prior_closeouts + [push_contract]

    return {
        "schema_version": 0,
        "kind": "MirBuilderCarrierTypeScalarKnownWritePushSurfaceDirectCloseoutRerunV1",
        "token": TOKEN,
        "input_state": {
            "write_push_surface_contract_basis": rel(PUSH_BASIS),
            "write_push_surface_contract_basis_hash": sha256_file(PUSH_BASIS),
            "prior_collection_len_rerun": rel(COLLECTION_RERUN),
            "prior_collection_len_rerun_hash": sha256_file(COLLECTION_RERUN),
        ...
        "decision": {
            "kind": "SelectWriteRemainingSubsurfacePostPushCloseoutRerun",
            "reason_token": "PushScopedCloseoutMaterializedDeleteSetRemain",
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-carrier-type-scalar-known-write-push-surface-direct-closeout-rerun unchanged")
        return 0
