#!/usr/bin/env python3
"""Define the MapLoad `.hako` route-authority pilot basis."""
...
OUTPUT = FIXTURES / "mirbuilder-scalar-known-fastpath-mapload-hako-authority-pilot-basis-v0.json"
...
def build_fixture() -> dict[str, Any]:
    consultation = read_json(CONSULTATION)
    return {
        "schema_version": 0,
        "kind": "MirBuilderScalarKnownFastpathMaploadHakoAuthorityPilotBasisV1",
        "token": TOKEN,
        "input_state": {
            "consultation_fixture": rel(CONSULTATION),
            "consultation_fixture_hash": sha256_file(CONSULTATION),
            "consultation_selected_next_card": (consultation.get("decision") or {}).get(
                "selected_next_card"
            ),
            "mapload_hako_route_authority_pilot_basis_selected": (
                consultation.get("summary") or {}
            ).get("mapload_hako_route_authority_pilot_basis"),
        },
        "provenance": {
        ...
        "basis": {
            ...
            "hako_authority_source": "MAPLOAD_SCALAR_I64_HAKO_POLICY",
            "hako_authority_result_fields": [
                "route_kind",
                "core_op",
                "lowering_tier",
                "return_shape",
            ...
            "rust_oracle_compat_checker_contract": {
                "retained": True,
                "rust_computes_existing_mapload_decision": True,
                "rust_compares_against_hako_authority_result": True,
        ...
        "decision": {
            "kind": "SelectMapLoadRouteDecisionAuthorityPilotImplementation",
            "reason_token": "MapLoadAuthorityPilotBasisDefinedRustOracleRetained",
            "selected_next_card": NEXT_CARD,
        },
        "summary": {
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-scalar-known-fastpath-mapload-hako-authority-pilot-basis unchanged")
        return 0
