#!/usr/bin/env python3
"""Resolve GenericLoop break-else-if step validation projection policy."""
...
PREVIOUS_POLICY = FIXTURES / "mirbuilder-generic-loop-body-check-continue-if-step-validation-projection-policy-v0.json"
OUTPUT = FIXTURES / "mirbuilder-generic-loop-body-check-break-else-if-step-validation-projection-policy-v0.json"
TOKEN = "MIRBUILDER-GENERIC-LOOP-BODY-CHECK-BREAK-ELSE-IF-STEP-VALIDATION-PROJECTION-POLICY-001"
SUBCLUSTER_ID = "BreakElseIfStepValidation"
...
def build_policy() -> dict[str, Any]:
    ...
    if [surface["symbol"] for surface in surfaces] != ["validate_break_else_if_step"]:
        raise SystemExit(f"unexpected break-else-if step validation surfaces: {surfaces}")
    if previous_policy["decision"]["selected_next_card"] != TOKEN:
        raise SystemExit("continue-if step validation does not select break-else-if step validation")

    surface = surfaces[0]
    source_text = read_source(surface["source_path"])
    validator_descriptor = {
        "symbol": "validate_break_else_if_step",
        "version": "v0",
    ...
    return {
        "schema_version": 0,
        "kind": "MirBuilderGenericLoopBodyCheckBreakElseIfStepValidationProjectionPolicyV1",
        "token": TOKEN,
        "input_state": {
        ...
        "selection_axes": {
            ...
            "shape_signature": "shape.generic_loop_body_check_break_else_if_step_validation",
        ...
        "source_surfaces": [
            {
                ...
                "validator_role": "strict_reject_final_statement_validator",
            }
        ],
        "validator_descriptor": {
            "descriptor_id": "generic_loop_body_check_break_else_if_step_validation_v1",
            "source_extraction": "rust_break_else_if_final_statement_validation",
            "entry_count": 1,
        ...
        "selected_policy": {
            ...
            "owner_edge": "mirbuilder::generic_loop_body_check_break_else_if_step_validation",
            "validator_descriptor_selected": True,
            "hako_projection_selected": False,
            "reason_token": "BreakElseIfStepValidationRequiresFinalStatementDescriptorBeforeHakoProjection",
        },
        "decision": {
            ...
            "reason_token": "BreakElseIfStepValidationDescriptorMaterialized",
        },
        "claims": {
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-generic-loop-body-check-break-else-if-step-validation-projection-policy unchanged")
        return 0
