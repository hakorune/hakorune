#!/usr/bin/env python3
"""Define the representative crate-smoke probe closeout contract."""
...
REFERENCE = ROOT / "docs/development/current/main/design/fixtures/rust-lifecycle/box-compilation-context-crate-smoke-probe-closeout-contract-v0.json"


def define_box_compilation_context_crate_smoke_probe_closeout_contract() -> dict[str, Any]:
    ...
    require_current_task_pointer(current_task, "Define representative BoxCompilationContext crate smoke probe closeout contract", "current task pointer not set to probe closeout contract")
    ...
    return {
        "schema_version": 0,
        "kind": "MirBuilderEasyTierCrateSmokeProbeCloseoutContract",
        ...
        "selected_next_owner": "representative crate smoke probe closeout contract",
        "current_contract": "probe_closeout_contract_only",
        "source": {
        ...
        "decision": [
            "define the representative probe closeout contract after the probe result contract is fixed",
            "keep the closeout bounded to the landed ctor + is_empty slice",
...
def main() -> int:
    ...
    report = define_box_compilation_context_crate_smoke_probe_closeout_contract()
    if args.check_reference:
        expected = json.loads(REFERENCE.read_text())
        require(report == expected, "crate smoke probe closeout contract differs from reference fixture")
    if args.emit_json:
        print(json.dumps(report, indent=2, sort_keys=True))
        return 0

    print("output_contract=rust-mirbuilder-box-compilation-context-crate-smoke-probe-closeout-contract-v0")
    print("subject=BoxCompilationContext")
    print("crate_level_probe_candidate=BoxCompilationContext")
    print("selected_next_owner=representative crate smoke probe closeout contract")
    print("crate_level_probe_opened=0")
    print("nightly_rustc_adapter_opened=0")
    print("decision=probe_closeout_contract_only")
    print("summary=ok")
    return 0
