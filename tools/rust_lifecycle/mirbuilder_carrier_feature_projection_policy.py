#!/usr/bin/env python3
"""Resolve CarrierFeature projection policy for selected carrier finalizers."""
...
OUTPUT = FIXTURES / "mirbuilder-carrier-feature-projection-policy-v0.json"
SELECTED_CLUSTER_ID = (
    "projection_policy::UnsupportedDirectShape::shape.carrier_feature::"
    "FixtureMapped::CarrierFeatureCluster"
)
...
def selected_surfaces(report: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        and item.get("shape_signature") == "shape.carrier_feature"
        and item.get("owner_edge_confidence") == "FixtureMapped"
        and item.get("stable_deny_reason") == "UnsupportedDirectShape"
        and item.get("type_transport_axis") is None
        and item.get("return_type") == ""
    ]
...
def build_policy() -> dict[str, Any]:
    ...
    return {
        "schema_version": 0,
        "kind": "MirBuilderCarrierFeatureProjectionPolicyV1",
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-carrier-feature-projection-policy unchanged")
        return 0
