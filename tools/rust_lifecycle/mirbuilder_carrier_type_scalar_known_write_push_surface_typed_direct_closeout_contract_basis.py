#!/usr/bin/env python3
"""Define the Write Push surface typed direct closeout contract basis."""
...
def build_fixture() -> dict[str, Any]:
    rerun = read_json(POST_PUSH_RERUN)
    adoption = read_json(ADOPTION)

    return {
        "schema_version": 0,
        "kind": "MirBuilderCarrierTypeScalarKnownWritePushSurfaceTypedDirectCloseoutContractBasisV1",
        "token": TOKEN,
        "input_state": {
            "post_push_adoption_rerun": rel(POST_PUSH_RERUN),
            "post_push_adoption_rerun_hash": sha256_file(POST_PUSH_RERUN),
        ...
        "selection_rule": {
            "name": "WritePushSurfaceTypedDirectCloseoutContractBasisOnlyV1",
            "basis_only": True,
        ...
        "decision": {
            "kind": "SelectWritePushSurfaceDirectCloseoutRerun",
            "reason_token": "WritePushSurfaceTypedDirectCloseoutContractBasisDefined",
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-carrier-type-scalar-known-write-push-surface-typed-direct-closeout-contract-basis unchanged")
        return 0
