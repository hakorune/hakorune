#!/usr/bin/env python3
"""Resolve PlanFacts projection policy."""
...
OUTPUT = FIXTURES / "mirbuilder-plan-facts-projection-policy-v0.json"
SELECTED_CLUSTER_ID = (
    "projection_policy::UnsupportedDirectShape::shape.plan_facts::"
    "FixtureMapped::PlanFactsCluster"
)
...
def selected_surfaces(report: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        and item.get("shape_signature") == "shape.plan_facts"
        and item.get("likely_owner_cluster") == "JoinIRPlanCluster"
        and item.get("joinir_plan_subcluster") == "PlanFactsCluster"
...
def role_for(item: dict[str, Any]) -> str:
    symbol = item["symbol"]
    if symbol.startswith("is_supported_") or symbol.startswith("is_pure_"):
        return "generic_loop_expr_fact_predicate"
    if symbol == "exit_only_block_ends_with_exit_on_all_paths":
        return "exit_only_terminality_fact"
    if symbol == "detect_nested_loop":
        return "nested_loop_presence_fact"
    if symbol == "match_index_of_bound":
        return "scan_bound_matcher_fact"
    if symbol == "scan_nested_loop_body":
        return "nested_loop_body_profile_fact"
    if symbol == "log_accept":
        return "debug_accept_log_helper"
    return f"plan_facts_helper::{symbol}"
...
def build_policy() -> dict[str, Any]:
    ...
    return {
        "schema_version": 0,
        "kind": "MirBuilderPlanFactsProjectionPolicyV1",
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-plan-facts-projection-policy unchanged")
        return 0
