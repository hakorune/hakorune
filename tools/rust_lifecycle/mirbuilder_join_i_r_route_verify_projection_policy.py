#!/usr/bin/env python3
"""Resolve JoinIRRouteVerify projection policy for facts/verify helpers."""
...
OUTPUT = FIXTURES / "mirbuilder-join-i-r-route-verify-projection-policy-v0.json"
SELECTED_CLUSTER_ID = (
    "projection_policy::UnsupportedDirectShape::shape.join_i_r_route_verify::"
    "FixtureMapped::JoinIRRouteVerifyCluster"
)
...
def selected_surfaces(report: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        and item.get("shape_signature") == "shape.join_i_r_route_verify"
...
def surface_bucket(item: dict[str, Any]) -> str:
    ...
    if "/verify/diagnostics/" in path or "/verify/verifier/" in path:
        return "verify_diagnostic"
    if "/joinir/merge/rewriter/" in path:
        return "merge_rewriter"
    if "/joinir/merge/" in path:
        return "merge_contract_or_logging"
    if "/recipes/" in path:
        return "recipe_index"
    if "/edgecfg/" in path:
        return "edgecfg_compose"
    return "route_verify_helper"
...
def build_policy() -> dict[str, Any]:
    ...
    return {
        "schema_version": 0,
        "kind": "MirBuilderJoinIRRouteVerifyProjectionPolicyV1",
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-join-i-r-route-verify-projection-policy unchanged")
        return 0
