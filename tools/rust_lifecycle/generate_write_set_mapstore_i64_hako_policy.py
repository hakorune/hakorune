#!/usr/bin/env python3
"""Generate the checked-in MapStoreI64 Hako shadow policy artifact."""
...
def read_policy_row(source: Path) -> list[str]:
    prefix = '"map_store_i64_set_surface|'
    for line in source.read_text(encoding="utf-8").splitlines():
        ...
        if end < 0:
            raise SystemExit("MapStoreI64 policy row is unterminated")
        fields = rest[:end].split("|")
        if len(fields) != 13:
            raise SystemExit(f"MapStoreI64 policy row has {len(fields)} fields, expected 13")
        return fields
    raise SystemExit("MapStoreI64 policy row not found")
...
def render(fields: list[str]) -> str:
    ...
    if actual != expected:
        raise SystemExit(f"unexpected MapStoreI64 policy identity: {actual!r}")

    def lit(value: str) -> str:
        return json.dumps(value)

    return f"""// @generated by tools/rust_lifecycle/generate_write_set_mapstore_i64_hako_policy.py
