#!/usr/bin/env python3
"""Define the representative crate-smoke probe output contract."""
...
REFERENCE = ROOT / "docs/development/current/main/design/fixtures/rust-lifecycle/box-compilation-context-crate-smoke-probe-output-contract-v0.json"


def define_box_compilation_context_crate_smoke_probe_output_contract() -> dict[str, Any]:
    ...
    require_current_task_pointer(current_task, "Define representative BoxCompilationContext crate smoke probe output contract", "current task pointer not set to probe output contract")
    ...
    return {
        "schema_version": 0,
        "kind": "MirBuilderEasyTierCrateSmokeProbeOutputContract",
        ...
        "selected_next_owner": "representative crate smoke probe output contract",
        "current_contract": "probe_output_contract_only",
        "source": {
        ...
        "probe_output": [
            "output_contract=rust-mirbuilder-box-compilation-context-crate-smoke-probe-v0",
        ...
        "decision": [
            "define the representative probe output contract after the probe inventory is fixed",
            "keep the probe output bounded to the landed ctor + is_empty slice",
...
def main() -> int:
    ...
    report = define_box_compilation_context_crate_smoke_probe_output_contract()
    if args.check_reference:
        expected = json.loads(REFERENCE.read_text())
        require(report == expected, "crate smoke probe output contract differs from reference fixture")
    if args.emit_json:
        print(json.dumps(report, indent=2, sort_keys=True))
        return 0

    print("output_contract=rust-mirbuilder-box-compilation-context-crate-smoke-probe-output-contract-v0")
    print("subject=BoxCompilationContext")
    print("crate_level_probe_candidate=BoxCompilationContext")
    print("selected_next_owner=representative crate smoke probe output contract")
    print("crate_level_probe_opened=0")
    print("nightly_rustc_adapter_opened=0")
    print("decision=probe_output_contract_only")
    print("summary=ok")
    return 0
