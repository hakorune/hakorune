#!/usr/bin/env python3
"""Rerun the Source Selfhost native-owner checkpoint after report rerun 004."""
...
OUTPUT = FIXTURES / "source-selfhost-native-owner-checkpoint-rerun-002-v0.json"
...
REPORT_RERUN_CARD = ROOT / "docs/development/current/main/phases/phase-296x/2057-MIRBUILDER-UNCONVERTED-SURFACE-REPORT-RERUN-004.md"
...
def native_owner_rows(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in manifest.get("rows") or []:
        ...
        native_owner = (
            claims.get("native_hako_source_owner_present") == 1
            or post.get("native_edit_authority") == 1
        )
...
def blocker_quality(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "CarrierTypeTransportPriority": {
            "candidate_count": 0,
            "evidence_quality_count": 0,
            "selection_eligible": False,
            "reason_token": "RequiresCheckpointSelectedCarrierTypeBlockerClass",
...
def build_fixture() -> dict[str, Any]:
    ...
    route = blockers["RouteRepairNeeded"]

    if route["selection_eligible"]:
        decision = {
            "kind": "SelectRouteRepair",
            "reason_token": "RouteRepairEvidenceWinsCheckpoint",
            "selected_blocker_class": "RouteRepairNeeded",
            "selected_next_card": route["next_card"],
        }
    elif missing["selection_eligible"] and not borrow["selection_eligible"]:
        decision = {
            "kind": "SelectMissingProjectionPolicyClusterResolutionV4",
            "reason_token": "MissingProjectionPolicyEvidenceQualityWinsCheckpoint",
            "selected_blocker_class": "MissingProjectionPolicy",
            "selected_next_card": missing["next_card"],
        }
    else:
        decision = {
            "kind": "KeepStopped",
    ...
    return {
        "schema_version": 0,
        "kind": "SourceSelfhostNativeOwnerCheckpointRerunV2",
        ...
        "input_state": {
            ...
            "report_rerun_004_card": rel(REPORT_RERUN_CARD),
        },
        "local_authority": {
        ...
        "provenance": {
            ...
            "report_rerun_004_card_hash": sha256_file(REPORT_RERUN_CARD),
        },
        "fresh_report": {
            ...
            "projection_descriptor_ledger_hash_fresh": True,
            "native_owner_adoption_ledger_hash_fresh": True,
        },
        "native_owner_map": {
...
def main() -> int:
    ...
    if args.check:
        ...
        print("source-selfhost-native-owner-checkpoint-rerun-002 unchanged")
        return 0
