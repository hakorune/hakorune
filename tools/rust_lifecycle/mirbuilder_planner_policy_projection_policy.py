#!/usr/bin/env python3
"""Resolve PlannerPolicy projection policy."""
...
OUTPUT = FIXTURES / "mirbuilder-planner-policy-projection-policy-v0.json"
SELECTED_CLUSTER_ID = (
    "projection_policy::UnsupportedDirectShape::shape.planner_policy::"
    "FixtureMapped::PlannerPolicyCluster"
)
...
def selected_surfaces(report: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        and item.get("shape_signature") == "shape.planner_policy"
        and item.get("joinir_plan_subcluster") == "PlannerPolicyCluster"
...
def role_for(item: dict[str, Any]) -> str:
    symbol = item["symbol"]
    if symbol == "planner_first_tag":
        return "planner_first_stable_tag_builder"
    if symbol == "planner_first_tag_with_label":
        return "planner_first_stable_tag_with_label_builder"
    return f"planner_policy_helper::{symbol}"
...
def build_policy() -> dict[str, Any]:
    ...
    return {
        "schema_version": 0,
        "kind": "MirBuilderPlannerPolicyProjectionPolicyV1",
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-planner-policy-projection-policy unchanged")
        return 0
