#!/usr/bin/env python3
"""Rerun strict native-seed candidate selection after record adoption."""
...
RERUN_003 = FIXTURES / "mirbuilder-strict-converter-emission-native-seed-candidate-selection-rerun-003-v0.json"
RECORD_ADOPTION = FIXTURES / "mirbuilder-record-packed-layout-refresh-hako-adoption-decision-v0.json"
OUTPUT = FIXTURES / "mirbuilder-strict-converter-emission-native-seed-candidate-selection-rerun-004-v0.json"
...
def build_fixture() -> dict[str, Any]:
    previous = read_json(RERUN_003)
    adoption = read_json(RECORD_ADOPTION)

    if adoption["decision"]["value"] != "Adopt":
        raise SystemExit("record_packed_layout_refresh must be adopted before rerun 004")
    if adoption["family_id"] != "hakorune_mir_builder::record_packed_layout_refresh":
        raise SystemExit("record adoption family mismatch")
    if adoption["claims"]["source_selfhost_claim"] != 0:
        raise SystemExit("record adoption must not claim Source Selfhost")
    ...
    if selected:
        decision = {
            ...
            "reason_token": "PostRecordPackedLayoutAdoptionStrictEmissionCandidateSelected",
        ...
        decision = {
            "kind": "KeepStopped",
            "reason_token": "NoBridgeEligibleCandidateAfterRecordPackedLayoutAdoption",
    ...
    return {
        "schema_version": 0,
        "kind": "MirBuilderStrictConverterEmissionNativeSeedCandidateSelectionRerun004V1",
        "token": TOKEN,
        "input_state": {
            "current_blocker": "SOURCE-SELFHOST-WIDER-ROUTE-SELECTION-DESIGN-STOP-001",
            "previous_rerun": rel(RERUN_003),
            "record_packed_layout_refresh_adoption": rel(RECORD_ADOPTION),
        },
        "provenance": {
            "previous_rerun_hash": sha256_file(RERUN_003),
            "record_packed_layout_refresh_adoption_hash": sha256_file(RECORD_ADOPTION),
        },
        "candidate_pool": {
        ...
        "claims": {
            ...
            "record_packed_layout_refresh_adoption_consumed": 1,
...
def main() -> int:
    ...
    if args.check:
        ...
        print("mirbuilder-strict-converter-emission-native-seed-candidate-selection-rerun-004 unchanged")
        return 0
