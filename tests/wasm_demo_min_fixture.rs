...
#[test]
fn wasm_demo_default_route_const_binop_uses_native_helper_contract() {
    let fixture_rel = "apps/tests/phase29cc_wsm_p9_min1_const_binop_return.hako";
    let mir_module = compile_fixture_to_mir_module(fixture_rel);
    let bytes = compile_hako_native_shape_bytes(&mir_module)
        .expect("native helper should succeed")
        .expect("const-binop-return shape should be emitted by native helper");

    let backend = WasmBackend::new();
    let baseline = backend
        .build_minimal_i32_const_wasm(42)
        .expect("baseline writer must succeed");
    assert_eq!(
        bytes, baseline,
        "default-route native helper output mismatch for const-binop fixture"
    );
}
...
#[test]
fn wasm_demo_default_route_p10_min5_error_inventory_rejected_by_native_helper_contract() {
    let fixture_rel = "apps/tests/phase29cc_wsm_p10_min5_loop_extern_error_inventory.hako";
    let mir_module = compile_fixture_to_mir_module(fixture_rel);
    let bytes = compile_hako_native_shape_bytes(&mir_module)
        .expect("native helper should succeed and return boundary result for min5 error inventory");
    assert!(
        bytes.is_none(),
        "p10 min5 error inventory must stay bridge-only and be rejected by native helper"
    );
}
...
fn wasm_demo_default_route_p10_min6_warn_native_uses_native_helper_contract() {
    ...
    let bytes = compile_hako_native_shape_bytes(&mir_module)
        .expect("native helper should succeed")
        .expect("p10 min6 warn shape should be emitted by native helper");

    let backend = WasmBackend::new();
    let baseline = backend
        .build_loop_extern_warn_skeleton_wasm(4)
        .expect("loop extern warn skeleton baseline writer must succeed");
    assert_eq!(
        bytes, baseline,
        "default-route native helper output mismatch for p10 min6 warn fixture"
    );
}

#[test]
fn wasm_demo_default_route_p10_min7_info_native_uses_native_helper_contract() {
    let fixture_rel = "apps/tests/phase29cc_wsm_p10_min7_loop_extern_info_native.hako";
    let mir_module = compile_fixture_to_mir_module(fixture_rel);
    let bytes = compile_hako_native_shape_bytes(&mir_module)
        .expect("native helper should succeed")
        .expect("p10 min7 info shape should be emitted by native helper");

    let backend = WasmBackend::new();
    let baseline = backend
        .build_loop_extern_info_skeleton_wasm(4)
        .expect("loop extern info skeleton baseline writer must succeed");
    assert_eq!(
        bytes, baseline,
        "default-route native helper output mismatch for p10 min7 info fixture"
    );
}

#[test]
fn wasm_demo_default_route_p10_min8_error_native_uses_native_helper_contract() {
    let fixture_rel = "apps/tests/phase29cc_wsm_p10_min8_loop_extern_error_native.hako";
    let mir_module = compile_fixture_to_mir_module(fixture_rel);
    let bytes = compile_hako_native_shape_bytes(&mir_module)
        .expect("native helper should succeed")
        .expect("p10 min8 error shape should be emitted by native helper");

    let backend = WasmBackend::new();
    let baseline = backend
        .build_loop_extern_error_skeleton_wasm(4)
        .expect("loop extern error skeleton baseline writer must succeed");
    assert_eq!(
        bytes, baseline,
